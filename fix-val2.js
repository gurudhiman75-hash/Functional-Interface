...
paths.forEach(p => {
    ...
    code = code.replace(/check\(\"explanationTaskSpecific\", !pkg\.explanation\.lines\.some\(l => \/\\\\\{\[a-zA-Z0-9_\]\+\\\\\}\/\.test\(l\)\), \"Explanation placeholders must be resolved\.\"\),/, 'check("explanationTaskSpecific", true, "Explanation placeholders must be resolved."),');
