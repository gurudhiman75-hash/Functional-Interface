...
export function TestAnalyticsQualityPage() {
  ...
  return <div className="space-y-5">
    ...
    <Card className="border-dashed"><CardContent className="p-4"><p className="font-medium">Reliability thresholds and freshness</p><p className="mt-1 text-sm text-muted-foreground">Samples below {number(data?.thresholds.usableSample)} scored attempts are limited; {number(data?.thresholds.strongSample)} or more are strong. Latest canonical activity: {date(data?.freshness.latestActivityAt)}. Diagnostics never modify attempts.</p>{data?.truncated && <p className="mt-2 text-sm text-warning">Global totals include all matched publications; the detail table shows the first {number(data.resultLimit)} ordered by severity and recency.</p>}</CardContent></Card>
