...
import { AlertTriangle, BarChart3, Flag, RefreshCw, Search, Target } from 'lucide-react';
...
type QuestionRow = {
  ...
  answerRate: number; skipRate: number; accuracy: number | null; facility: number; flagRate: number; optionSelection: OptionRow[];
};
...
type QuestionDetail = {
  windowDays: number;
  question: { publicationId: string; publicationNumber: number; testPublicCode: string; testTitle: string; questionVersionId: string; sectionName: string; stem: string; explanation: string; stableQuestionId: number };
  summary: { completedAttemptsScanned: number; matchedExposures: number; unmatchedAttempts: number; answered: number; skipped: number; correct: number; incorrect: number; flagged: number; answerRate: number; skipRate: number; accuracy: number | null; facility: number; flagRate: number };
  optionSelection: OptionRow[];
  dailyTrend: Array<{ day: string; exposures: number; answered: number; correct: number; skipped: number; flagged: number; accuracy: number | null; skipRate: number; flagRate: number }>;
...
export function QuestionAnalyticsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try { const params = new URLSearchParams({ days, limit: '150' }); if (appliedSearch) params.set('search', appliedSearch); setData(await request<AnalyticsResponse>(`/admin/analytics/questions?${params}`)); }
    catch (error) { showToast.error('Unable to load Question Analytics', error instanceof Error ? error.message : 'Request failed.'); }
...
export function QuestionAnalyticsDetailPage() {
  ...
  return <div className="space-y-5">
    <PageHeader title="Question performance drilldown" description={`${data.question.testTitle} · ${data.question.testPublicCode} · ${data.question.sectionName}`} icon={<BarChart3 className="h-5 w-5" />} actions={<><WindowSelect value={days} onChange={setDays} /><Button asChild variant="outline"><Link to="/analytics/questions">Back to questions</Link></Button></>} />
    <Card><CardContent className="p-5"><p className="text-base font-medium">{data.question.stem}</p><p className="mt-2 text-xs text-muted-foreground">Question version {data.question.questionVersionId} · Publication {data.question.publicationNumber} · Stable runner ID {data.question.stableQuestionId}</p></CardContent></Card>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><Metric label="Matched exposure" value={number(data.summary.matchedExposures)} /><Metric label="Answer rate" value={percent(data.summary.answerRate)} /><Metric label="Accuracy" value={percent(data.summary.accuracy)} /><Metric label="Skip rate" value={percent(data.summary.skipRate)} /><Metric label="Facility" value={percent(data.summary.facility)} /><Metric label="Flag rate" value={percent(data.summary.flagRate)} /></div>
    ...
    <div className="grid gap-4 xl:grid-cols-2"><Card><CardHeader><CardTitle className="text-base">Option-selection diagnostics</CardTitle></CardHeader><CardContent><OptionList options={data.optionSelection} showCorrect /></CardContent></Card><Card><CardHeader><CardTitle className="text-base">Daily response trend</CardTitle></CardHeader><CardContent className="space-y-2">{data.dailyTrend.length ? data.dailyTrend.map((row) => <div key={row.day} className="grid grid-cols-[90px_1fr_1fr_1fr] gap-2 rounded-md border p-3 text-xs"><span>{row.day}</span><span>Exposure {number(row.exposures)}</span><span>Accuracy {percent(row.accuracy)}</span><span>Skip {percent(row.skipRate)}</span></div>) : <p className="text-sm text-muted-foreground">No matched daily response evidence exists.</p>}</CardContent></Card></div>
    {data.summary.unmatchedAttempts > 0 && <Card className="border-destructive/30"><CardContent className="p-4"><p className="font-medium">Linkage gap detected</p><p className="mt-1 text-sm text-muted-foreground">{number(data.summary.unmatchedAttempts)} completed attempts in this publication did not contain a matching snapshot item for this stable question ID. They are excluded from metrics.</p></CardContent></Card>}
    <ScopeBoundary />
  </div>;
}
...
function ScopeBoundary({ reason }: { reason?: string }) { return <Card className="border-dashed"><CardContent className="p-4"><div className="flex items-start gap-2"><Target className="mt-0.5 h-4 w-4" /><div><p className="font-medium">Declared scope boundary</p><p className="mt-1 text-sm text-muted-foreground">{reason || 'Per-question timing and discrimination remain unavailable until the runner persists their canonical inputs directly.'}</p><p className="mt-2 flex items-center gap-1 text-xs text-muted-foreground"><Flag className="h-3.5 w-3.5" />Analytics are read-only and never modify questions, attempts or result snapshots.</p></div></div></CardContent></Card>; }
