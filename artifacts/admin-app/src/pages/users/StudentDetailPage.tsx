...
import { Activity, ArrowLeft, KeyRound, Laptop, MessageSquare, PencilLine, RefreshCw, Shield, UserRound } from 'lucide-react';
...
import { addStudentNote, getStudentNotes, recoverStudentFirebaseIdentity, revokeStudentSession, updateStudentProfile, type StudentAccountAction, type StudentNote } from '@/features/students/api';
...
export function StudentDetailPage() {
  ...
  const requireReason = () => {
    if (reason.trim().length >= 12) return true;
    showToast.warning('Reason required', 'Enter a meaningful operational reason of at least 12 characters. It will be written to the audit log.');
    return false;
  };
  ...
  const recoverFirebaseAccess = async () => {
    if (!id || !data?.student) return;
    if (reason.trim().length < 20) return showToast.warning('Detailed reason required', 'Account recovery requires a reason of at least 20 characters.');
    if (recoveryEmail.trim().toLowerCase() !== data.student.email.trim().toLowerCase()) return showToast.warning('Email confirmation does not match', 'Enter the canonical student email exactly before relinking Firebase access.');
    setRecovering(true);
    try {
      const result = await recoverStudentFirebaseIdentity(id, { confirmationEmail: recoveryEmail.trim(), reason: reason.trim(), expectedStatus: data.student.status });
      await refresh();
      showToast.success('Firebase access recovered', `${result.operation.firebaseIdentitiesRevoked} Firebase identity session set(s) and ${result.operation.sessionsRevoked} canonical session(s) were revoked. Account status is ${title(result.operation.status)}.`);
      setRecoveryEmail('');
      setReason('');
    } catch (caught) {
      showToast.error('Account recovery failed', caught instanceof Error ? caught.message : 'Unable to recover Firebase access.');
    } finally {
      setRecovering(false);
    }
  };
  ...
  return <div className="space-y-5">
    ...
    <Tabs defaultValue="profile"><TabsList><TabsTrigger value="profile">Profile</TabsTrigger><TabsTrigger value="recovery">Recovery</TabsTrigger><TabsTrigger value="attempts">Attempts ({attempts.length})</TabsTrigger><TabsTrigger value="sessions">Sessions ({sessions.length})</TabsTrigger><TabsTrigger value="notes">Notes ({notes.length})</TabsTrigger><TabsTrigger value="timeline">Timeline ({timeline.length})</TabsTrigger></TabsList>
      <TabsContent value="profile" className="mt-4 space-y-4"><Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><PencilLine className="h-4 w-4" />Editable canonical profile</CardTitle></CardHeader><CardContent className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Field label="Display name"><Input value={form.displayName} onChange={(event) => setForm((current) => ({ ...current, displayName: event.target.value }))} /></Field><Field label="Email"><Input type="email" value={form.email} onChange={(event) => setForm((current) => ({ ...current, email: event.target.value }))} /></Field><Field label="Phone"><Input value={form.phone} onChange={(event) => setForm((current) => ({ ...current, phone: event.target.value }))} placeholder="Optional" /></Field><Field label="Preferred language"><Select value={form.preferredLanguageCode} onValueChange={(value) => setForm((current) => ({ ...current, preferredLanguageCode: value }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="en">English</SelectItem><SelectItem value="hi">Hindi</SelectItem><SelectItem value="pa">Punjabi</SelectItem></SelectContent></Select></Field></div><Button onClick={() => void saveProfile()} disabled={savingProfile || mutating}>{savingProfile ? 'Saving…' : 'Save profile changes'}</Button></CardContent></Card><Card><CardHeader><CardTitle className="text-base">Account operations</CardTitle></CardHeader><CardContent className="space-y-3"><div className="flex flex-wrap gap-2">{student.status === 'disabled' ? <Button onClick={() => void execute('enable')} disabled={mutating}>Enable as suspended</Button> : <>{student.status === 'suspended' ? <Button onClick={() => void execute('reactivate')} disabled={mutating}>Reactivate account</Button> : <Button variant="destructive" onClick={() => void execute('suspend')} disabled={mutating}>Suspend and revoke sessions</Button>}<Button variant="destructive" onClick={() => void execute('disable')} disabled={mutating}>Disable account</Button></>}<Button variant="outline" onClick={() => void execute('revoke-sessions')} disabled={mutating || student.activeSessionCount === 0}>Revoke all active sessions</Button></div><p className="text-xs text-muted-foreground">Enabling a disabled account restores it as suspended, requiring a separate reactivation review before login is allowed.</p></CardContent></Card><Card><CardContent className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3"><Detail label="Registration code" value={student.registrationCode} /><Detail label="Auth providers" value={student.authProviders.join(', ') || '—'} /><Detail label="Registered" value={fmt(student.createdAt)} /><Detail label="Last login" value={fmt(student.lastLoginAt)} /><Detail label="Latest attempt" value={fmt(student.latestAttemptAt)} /><Detail label="Student UUID" value={student.id} /></CardContent></Card></TabsContent>
      <TabsContent value="recovery" className="mt-4"><Card className="border-warning/40"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><KeyRound className="h-4 w-4" />Admin-assisted Firebase recovery</CardTitle></CardHeader><CardContent className="space-y-4"><div className="rounded-lg border bg-muted/20 p-3 text-sm text-muted-foreground">Use this only when the student has recreated or recovered the Firebase/Google account for the same canonical email. ExamTree verifies that the Firebase email exists, is verified, is enabled and is not linked to another student. All old sessions are revoked. Active accounts return as suspended; disabled accounts remain disabled.</div><Field label="Confirm canonical email"><Input type="email" value={recoveryEmail} onChange={(event) => setRecoveryEmail(event.target.value)} placeholder={student.email} autoComplete="off" /></Field><div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-xs text-destructive">This changes the authentication identity attached to the student’s permanent attempts and history. Verify the support case and identity evidence before continuing.</div><Button variant="destructive" onClick={() => void recoverFirebaseAccess()} disabled={recovering || recoveryEmail.trim().toLowerCase() !== student.email.trim().toLowerCase() || reason.trim().length < 20}>{recovering ? 'Recovering access…' : 'Relink verified Firebase identity'}</Button></CardContent></Card></TabsContent>
