...
const statusTone = (status: string) => status === 'evaluated' || status === 'practice_evaluated' ? 'success' : status === 'in_progress' ? 'warning' : 'neutral';
...
type AttemptStats = { total: number; inProgress: number; evaluated: number; practiceEvaluated: number };
...
export function AttemptsWorkspacePage() {
  ...
  const [stats, setStats] = useState<AttemptStats>({ total: 0, inProgress: 0, evaluated: 0, practiceEvaluated: 0 });
