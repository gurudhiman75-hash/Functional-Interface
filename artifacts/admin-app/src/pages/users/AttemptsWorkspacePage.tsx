...
import { Activity, AlertTriangle, ClipboardList, RefreshCw, Search } from 'lucide-react';
...
export function AttemptsWorkspacePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setSelected((current) => current.filter((id) => result.attempts.some((attempt) => attempt.id === id && attempt.stale)));
    } catch (error) {
  ...
  const bulkAbandon = async () => {
    ...
    try {
      const result = await request<{ attempted: number; succeeded: number; failed: number }>('/admin/attempts/bulk/actions/abandon', {
        method: 'POST',
        body: JSON.stringify({ attemptIds: selected, reason: bulkReason.trim() }),
      });
      ...
      showToast.error('Bulk abandonment failed', error instanceof Error ? error.message : 'Request failed.');
...
export function AttemptDetailPage() {
  ...
  const load = async () => {
    if (!id) return;
    setLoading(true);
    try { const result = await request<{ attempt: AttemptDetail; timeline: AttemptTimelineEvent[] }>(`/admin/attempts/${encodeURIComponent(id)}`); setAttempt(result.attempt); setTimeline(result.timeline); }
