...
type BatchItem = {
  id: string;
  taxonomySnapshot: { code?: string; name?: string; nodeType?: string };
  targetCoverage: number;
  priority: number;
...
export function NotesStudioPlanningPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [nextOptions, batchResult] = await Promise.all([
      ...
      setSelectedBatchId((current) => current && batchResult.batches.some((batch) => batch.id === current)
        ? current
        : batchResult.batches[0]?.id ?? null);
      setDraft((current) => ({
        ...current,
        examId: current.examId || nextOptions.exams[0]?.id || '',
        sourceLanguage: nextOptions.languages.some((language) => language.code === current.sourceLanguage)
          ? current.sourceLanguage
          : nextOptions.languages[0]?.code || 'en',
      }));
    } catch (error) {
  ...
  useEffect(() => { void load(); }, []);
  useEffect(() => {
    if (selectedBatchId) void loadDetail(selectedBatchId);
    else setDetail(null);
  }, [selectedBatchId]);
  useEffect(() => {
  ...
  const previewPlan = async () => {
    if (!draft.examId || !draft.rootTaxonomyNodeId) {
      showToast.warning('Planning selection incomplete', 'Choose an exam and taxonomy root first.');
      return;
    }
    setWorking(true);
    try {
      ...
      if (result.eligibleCount === 0) {
        showToast.warning('No eligible note units', 'Choose a different taxonomy root or unit selection.');
      }
    } catch (error) {
  ...
  const createPlan = async () => {
    ...
    try {
      ...
      showToast.success('Syllabus plan created', `${result.batch.itemCount} canonical note unit(s) were frozen into the backlog.`);
      setPreview(null);
      await load();
  ...
  const createJobs = async () => {
    ...
    try {
      const result = await adminRequest<{ createdCount: number }>(`/admin/notes-studio/planning/batches/${detail.batch.id}/create-jobs`, {
        method: 'POST',
        body: JSON.stringify({ limit: Math.min(options?.maxJobCreationBatch ?? 100, 50) }),
      });
      showToast.success('Brief jobs created', `${result.createdCount} job(s) now start the normal Notes Studio pipeline at Brief.`);
      await Promise.all([load(), loadDetail(detail.batch.id)]);
    } catch (error) {
  ...
  const updateItem = async (item: BatchItem, action: 'skip' | 'restore') => {
    ...
    setWorking(true);
    try {
      await adminRequest(`/admin/notes-studio/planning/items/${item.id}`, {
        method: 'PATCH',
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="space-y-2">
            <Label>Exam</Label>
            <Select value={draft.examId} onValueChange={(value) => { setDraft((current) => ({ ...current, examId: value, rootTaxonomyNodeId: '' })); setPreview(null); }}>
              <SelectTrigger><SelectValue placeholder="Choose exam" /></SelectTrigger>
              <SelectContent>{(options?.exams ?? []).map((exam) => <SelectItem key={exam.id} value={exam.id}>{exam.familyName} · {exam.name}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            <Label>Taxonomy root</Label>
            <Select value={draft.rootTaxonomyNodeId} onValueChange={(value) => { setDraft((current) => ({ ...current, rootTaxonomyNodeId: value })); setPreview(null); }}>
              <SelectTrigger><SelectValue placeholder="Choose mapped syllabus root" /></SelectTrigger>
              <SelectContent>{availableRoots.map((root) => <SelectItem key={`${root.examVersionId}:${root.id}`} value={root.id}>{root.name} · {root.nodeType}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-2"><Label>Plan title</Label><Input value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} placeholder="Optional — exam + syllabus area" /></div>
        ...
        {preview && <div className="rounded-lg border bg-muted/30 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><div className="font-semibold">{preview.exam.name} · {preview.root.name}</div><div className="text-sm text-muted-foreground">{preview.eligibleCount} note units · max {preview.maxPlanItems} · {preview.leafOnly ? 'deepest units only' : 'all selected levels'}</div></div>
            <Badge variant={preview.eligibleCount > 0 ? 'default' : 'destructive'}>{preview.eligibleCount} units</Badge>
          </div>
        ...
        <CardContent className="space-y-4">
          {!detail ? <div className="text-sm text-muted-foreground">Select a planning batch.</div> : <>
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">Planned</div><div className="mt-1 text-xl font-bold">{detail.batch.plannedCount}</div></div>
              <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">Jobs created</div><div className="mt-1 text-xl font-bold">{detail.batch.jobCreatedCount}</div></div>
              <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">Skipped</div><div className="mt-1 text-xl font-bold">{detail.batch.skippedCount}</div></div>
              <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">Approved / materialized</div><div className="mt-1 text-xl font-bold">{detail.batch.approvedOrMaterializedCount}</div></div>
            </div>
            <div className="space-y-2">{detail.items.map((item) => <div key={item.id} className="flex flex-col gap-3 rounded-lg border p-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0"><div className="font-medium">{item.taxonomySnapshot.name || 'Taxonomy note unit'}</div><div className="mt-1 text-xs text-muted-foreground">{item.taxonomySnapshot.code} · {item.taxonomySnapshot.nodeType} · target {item.targetCoverage} · priority {item.priority}</div>{item.authoringJobState && <div className="mt-1 text-xs">Authoring job: <Badge variant="outline">{pretty(item.authoringJobState)}</Badge></div>}</div>
              <div className="flex shrink-0 items-center gap-2"><Badge variant={item.itemState === 'job_created' ? 'default' : 'outline'}>{pretty(item.itemState)}</Badge>{canEdit && item.itemState === 'planned' && <Button size="sm" variant="ghost" onClick={() => void updateItem(item, 'skip')} disabled={working}><SkipForward className="mr-1 h-3.5 w-3.5" />Skip</Button>}{canEdit && item.itemState === 'skipped' && <Button size="sm" variant="ghost" onClick={() => void updateItem(item, 'restore')} disabled={working}><RotateCcw className="mr-1 h-3.5 w-3.5" />Restore</Button>}</div>
            </div>)}</div>
          </>}
        </CardContent>
      </Card>
