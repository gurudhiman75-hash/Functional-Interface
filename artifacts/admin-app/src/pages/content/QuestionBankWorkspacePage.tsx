            ...
            <>
              <div className="hidden overflow-x-auto md:block">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y">{visible.map((question) => <tr key={question.id} className="hover:bg-muted/20"><td className="px-4 py-3"><input type="checkbox" checked={selectedIds.has(question.id)} onChange={() => toggleSelected(question.id)} aria-label={`Select ${question.publicCode}`} /></td><td className="max-w-xl px-4 py-3"><Link to={`/content/questions/${question.id}`} className="block line-clamp-2 font-medium leading-5 hover:text-primary"><MathText content={question.stem} /></Link><div className="mt-1 flex flex-wrap gap-1.5 text-xs text-muted-foreground"><span>{question.publicCode}</span><span>•</span><span>v{question.versionNumber}</span><span>•</span><span>{question.questionType}</span></div></td><td className="px-4 py-3 text-muted-foreground"><p>{question.examName || 'No exam'}</p><p className="text-xs">{taxonomyName(question, 'subject') || 'No subject'}{taxonomyName(question, 'topic') ? ` • ${taxonomyName(question, 'topic')}` : ''}</p></td><td className="px-4 py-3">{question.publishedVersionId ? <Badge className="bg-primary/10 text-primary">Published</Badge> : isReadyToPublish(question) ? <Badge className="bg-success/10 text-success">Ready</Badge> : !hasCompleteTaxonomy(question) ? <Badge variant="outline">Needs taxonomy</Badge> : <Badge variant="secondary">Not ready</Badge>}</td><td className="px-4 py-3"><Badge variant="secondary">{question.difficulty}</Badge></td><td className="px-4 py-3"><Badge className={statusClass(question.status)}>{formatStatus(question.status)}</Badge>{question.publishedVersionId && question.status !== 'published' && <p className="mt-1 text-[10px] font-medium text-primary">Published version active</p>}</td><td className="px-4 py-3 text-muted-foreground">{formatDate(question.updatedAt)}</td></tr>)}</tbody>
                </table>
              </div>
              <div className="divide-y md:hidden">{visible.map((question) => <div key={question.id} className="p-4"><div className="flex items-start gap-3"><input type="checkbox" checked={selectedIds.has(question.id)} onChange={() => toggleSelected(question.id)} className="mt-1" aria-label={`Select ${question.publicCode}`} /><Link to={`/content/questions/${question.id}`} className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><Badge variant="outline">{question.publicCode}</Badge><Badge className={statusClass(question.status)}>{formatStatus(question.status)}</Badge></div><MathText content={question.stem} className="mt-3 line-clamp-3 text-sm font-medium leading-5" /><div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground"><span>{question.examName || 'No exam'}</span><span>•</span><span>{taxonomyName(question, 'topic') || 'No topic'}</span><span>•</span><span>{question.difficulty}</span></div></Link></div></div>)}</div>
