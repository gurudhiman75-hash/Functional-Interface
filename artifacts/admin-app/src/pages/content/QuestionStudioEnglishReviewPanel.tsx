...
const CPS = [
  ...
  { id: 'ENG-001-CP002', label: 'CP002 · Tenses and Sequence of Tenses', subtopic: 'Tenses and Sequence of Tenses', version: 'V1', ruleLabel: 'tense', rules: TENSE_RULES },
  { id: 'ENG-001-CP003', label: 'CP003 · Articles and Determiners', subtopic: 'Articles and Determiners', version: 'V1', ruleLabel: 'article / determiner', rules: ARTICLE_RULES },
  { id: 'ENG-001-CP004', label: 'CP004 · Pronouns', subtopic: 'Pronouns', version: 'V1', ruleLabel: 'pronoun', rules: PRONOUN_RULES },
...
export function QuestionStudioEnglishReviewPanel() {
  ...
  const maxBatchCount = selectedCp.id === 'ENG-001-CP009' || selectedCp.id === 'ENG-001-CP010' || selectedCp.id === 'ENG-001-CP011' || selectedCp.id === 'ENG-001-CP012' ? 20 : 50;
  ...
  const generate = async () => {
    if (!available) { showToast.error('ENG-001 checkpoint is not registered', `${selectedCp.id} is unavailable in the language-v1 package.`); return; }
    ...
    try {
      ...
      showToast.error('ENG-001 generation failed', caught instanceof Error ? caught.message : 'Unable to generate English questions.');
    ...
    <Card className="border-info/20">
      <CardHeader className="space-y-3">
        <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-start">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Badge variant="outline">12 CPs · 3 QLs · 122 grammar rules</Badge>
        ...
        <div className="rounded-lg border border-info/20 bg-info/5 p-3 text-xs text-muted-foreground">
          CP001 through CP012 are approved for Question Studio review generation. Approval here records editorial acceptance only. Question Bank storage, tests, mock tests, public publication, inline editing, and automatic learner delivery remain locked. Fix defects in the source generator and generate a fresh batch.
        </div>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-7">
          ...
          <Field label="Checkpoint" className="xl:col-span-2"><Select value={cpId} onValueChange={changeCp}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{CPS.map((entry) => <SelectItem key={entry.id} value={entry.id}>{entry.label}</SelectItem>)}</SelectContent></Select></Field>
        ...
        <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-end">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => void refresh()} disabled={loading || generating}><RefreshCw className={cn('mr-1.5 h-4 w-4', loading && 'animate-spin')} /> Refresh</Button>
        ...
        <div className="border-t pt-5">
          <div className="mb-3 grid gap-3 md:grid-cols-[1fr_minmax(18rem,32rem)] md:items-end">
            <div><p className="text-sm font-semibold">Recent ENG-001 review runs</p><p className="text-xs text-muted-foreground">CP001 through CP012 share this review surface. Approved items cannot enter Question Bank from this package.</p></div>
...
function Field({ label, children, className }: { label: string; children: React.ReactNode; className?: string }) {
  return <div className={cn('space-y-1.5', className)}><Label className="text-xs">{label}</Label>{children}</div>;
}
