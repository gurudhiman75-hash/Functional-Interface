...
import { NotesStudioBriefEditorCard } from './NotesStudioBriefEditorCard';
...
const sourcePackEditableStates = new Set(['brief', 'sources_ready']);
...
export function NotesStudioSourcePackPage() {
  ...
  const createJob = async () => {
    ...
    try {
      ...
      showToast.success('Authoring job created', 'Review the brief and assemble the governed source pack.');
    } catch (error) {
  ...
  const attachUrl = async () => {
    if (!selectedJobId || !urlSource.url.trim() || !sourcePackEditable) return;
    setWorking(true);
    try {
      ...
      showToast.success('URL source attached', 'The source is now part of this authoring job’s governed provenance pack.');
    } catch (error) {
  ...
  const attachPdf = async () => {
    if (!selectedJobId || !pdfFile || !sourcePackEditable) {
      if (!pdfFile) showToast.warning('Choose a PDF', 'Select the PDF you want to add to this source pack.');
      return;
    }
    setWorking(true);
    try {
      const params = new URLSearchParams({ fileName: pdfFile.name, rightsBasis: pdfMeta.rightsBasis });
  ...
  return <div className="space-y-5">
    <PageHeader
      title="Notes Studio"
      description="Define the authoring brief, assemble a rights-aware source pack, and hand reference-only sources to the reviewed Reference Evidence workflow."
    ...
    <div className="grid gap-3 sm:grid-cols-3">
      ...
      <Card><CardContent className="p-4"><div className="text-xs uppercase tracking-wide text-muted-foreground">Retained-text sources</div><div className="mt-1 text-2xl font-bold">{jobs.reduce((sum, job) => sum + job.generatableSourceCount, 0)}</div><div className="mt-1 text-xs text-muted-foreground">Reference evidence is reviewed separately.</div></CardContent></Card>
    </div>
    ...
    {createOpen && <Card className="border-primary/30">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="space-y-1.5"><Label htmlFor="notes-job-source-language">Source language</Label><select id="notes-job-source-language" className={nativeSelectClassName} value={jobDraft.sourceLanguage} onChange={(event) => setJobDraft((current) => ({ ...current, sourceLanguage: event.target.value }))}>{options.languages.length === 0 ? <option value="en">English</option> : options.languages.map((language) => <option key={language.id} value={language.code}>{language.name}</option>)}</select></div>
          <div className="space-y-1.5"><Label htmlFor="notes-job-depth">Depth</Label><select id="notes-job-depth" className={nativeSelectClassName} value={jobDraft.depth} onChange={(event) => setJobDraft((current) => ({ ...current, depth: event.target.value }))}><option value="quick_revision">Quick revision</option><option value="standard">Standard</option><option value="comprehensive">Comprehensive</option></select></div>
          <div className="space-y-1.5"><Label htmlFor="notes-job-learner-level">Learner level</Label><select id="notes-job-learner-level" className={nativeSelectClassName} value={jobDraft.learnerLevel} onChange={(event) => setJobDraft((current) => ({ ...current, learnerLevel: event.target.value }))}><option value="foundation">Foundation</option><option value="standard">Standard</option><option value="advanced">Advanced</option></select></div>
        </div>
        <div className="space-y-1.5"><Label>Syllabus / PYQ emphasis</Label><Textarea value={jobDraft.syllabusEmphasis} onChange={(event) => setJobDraft((current) => ({ ...current, syllabusEmphasis: event.target.value }))} placeholder="Record the syllabus boundaries, recurring PYQ angles, exclusions and high-yield emphasis." /></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
      <Card>
        ...
        <CardContent className="space-y-2">
          {jobs.length === 0 && <div className="rounded-lg border border-dashed p-5 text-sm text-muted-foreground">No authoring jobs yet. Create one to define its brief and research sources.</div>}
          {jobs.map((job) => <button type="button" key={job.id} onClick={() => setSelectedJobId(job.id)} className={`w-full rounded-lg border p-3 text-left transition ${selectedJobId === job.id ? 'border-primary bg-primary/[0.04]' : 'hover:bg-muted/40'}`}>
            ...
            <div className="mt-2 text-xs text-muted-foreground">{job.sourceCount} sources · {job.generatableSourceCount} retained-text ready</div>
          </button>)}
        </CardContent>
      </Card>

      <div className="space-y-5">
        {!selectedJob && <Card><CardContent className="p-8 text-center text-sm text-muted-foreground">Choose or create an authoring job to edit its brief and build its source pack.</CardContent></Card>}
        {selectedJob && <>
          <Card>
            <CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle>{selectedJob.title}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{selectedJob.brief?.topicLabel || 'Unmapped topic'} · {prettyState(selectedJob.brief?.depth || 'standard')} · updated {readableDate(selectedJob.updatedAt)}</p></div><Badge variant="outline">{prettyState(selectedJob.state)}</Badge></div></CardHeader>
            <CardContent><div className="flex items-start gap-3 rounded-lg border bg-muted/20 p-3 text-sm"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><div><div className="font-medium">Rights-aware evidence paths</div><p className="mt-1 text-muted-foreground">Authorized sources may retain extracted text. Reference-only URLs retain metadata only; after an editor reviews the source and records an independent factual paraphrase in Reference Evidence, that reviewed note can support claims without storing publisher wording.</p></div></div></CardContent>
          </Card>

          <NotesStudioBriefEditorCard job={selectedJob} options={options} canEdit={canEdit} onSaved={() => load()} />

          {canEdit && sourcePackEditable && <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Globe2 className="h-4 w-4" />Attach URL</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                <Input value={urlSource.url} onChange={(event) => setUrlSource((current) => ({ ...current, url: event.target.value }))} placeholder="https://official-source.gov/page" />
                <div className="grid gap-3 sm:grid-cols-2"><Input value={urlSource.title} onChange={(event) => setUrlSource((current) => ({ ...current, title: event.target.value }))} placeholder="Source title" /><Input value={urlSource.publisher} onChange={(event) => setUrlSource((current) => ({ ...current, publisher: event.target.value }))} placeholder="Publisher / authority" /></div>
                <Select value={urlSource.rightsBasis} onValueChange={(value) => setUrlSource((current) => ({ ...current, rightsBasis: value }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{rightsOptions.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select>
                <p className="text-xs text-muted-foreground">For Reference only, Notes Studio stores the URL and metadata without scraping or retaining the publisher page body.</p>
                <Button className="w-full" onClick={() => void attachUrl()} disabled={working || !urlSource.url.trim()}>{working ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <Globe2 className="mr-1.5 h-4 w-4" />}Attach web source</Button>
              </CardContent>
            </Card>
            <Card>
              <CardHeader><CardTitle className="flex items-center gap-2 text-base"><FileUp className="h-4 w-4" />Attach PDF</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                <Input id="notes-source-pdf" type="file" accept="application/pdf" onChange={(event) => setPdfFile(event.target.files?.[0] ?? null)} />
                <div className="grid gap-3 sm:grid-cols-2"><Input value={pdfMeta.title} onChange={(event) => setPdfMeta((current) => ({ ...current, title: event.target.value }))} placeholder="Source title" /><Input value={pdfMeta.publisher} onChange={(event) => setPdfMeta((current) => ({ ...current, publisher: event.target.value }))} placeholder="Publisher / author" /></div>
                <Input value={pdfMeta.originUrl} onChange={(event) => setPdfMeta((current) => ({ ...current, originUrl: event.target.value }))} placeholder="Optional original HTTPS URL" />
                <Select value={pdfMeta.rightsBasis} onValueChange={(value) => setPdfMeta((current) => ({ ...current, rightsBasis: value }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{rightsOptions.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select>
                <Button className="w-full" onClick={() => void attachPdf()} disabled={working || !pdfFile}>{working ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <FileUp className="mr-1.5 h-4 w-4" />}Extract and attach PDF</Button>
                <p className="text-xs text-muted-foreground">Uploaded PDF bytes are processed transiently; the raw file is not stored by Notes Studio.</p>
              </CardContent>
            </Card>
          </div>}

          {canEdit && !sourcePackEditable && <Card className="border-amber-200"><CardContent className="p-4 text-sm text-muted-foreground"><strong className="text-foreground">Source pack frozen.</strong> Brief and source membership stop changing after evidence work begins. Use Research Restart if the research intent or source pack must be rebuilt.</CardContent></Card>}
          ...
          {preview && <Card className="border-primary/20"><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">Source preview — {preview.title}</CardTitle><p className="mt-1 text-xs text-muted-foreground">{preview.retentionMode === 'extracted_text' ? 'Preview from retained extracted text.' : 'Reference-only source: publisher text was not retained. Open the source from Reference Evidence when reviewing facts.'}</p></div><Button size="sm" variant="ghost" onClick={() => setPreview(null)}>Close</Button></div></CardHeader><CardContent>{preview.previewAvailable ? <pre className="max-h-[420px] overflow-auto whitespace-pre-wrap rounded-lg border bg-muted/20 p-4 text-xs leading-relaxed">{preview.preview}</pre> : <div className="rounded-lg border border-dashed p-5 text-sm text-muted-foreground">No retained text preview is available for this source.</div>}</CardContent></Card>}
        </>}
      </div>
    </div>
  </div>;
