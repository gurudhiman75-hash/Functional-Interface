...
function taxonomyFromVersion(version: QuestionVersion): TaxonomyState {
  return {
    ...
    topicId: version.taxonomy.find((node) => node.isPrimary)?.id
      ?? version.taxonomy.find((node) => node.nodeType === 'topic')?.id
      ?? '',
  };
}
...
export function QuestionDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const current = next.versions.find((version) => version.id === next.question.displayVersionId) ?? next.versions[0];
  ...
  const topics = useMemo(
    () => allowedNodes.filter((node) => node.nodeType === 'topic' && node.parentIds.includes(taxonomy.subjectId)),
  ...
  const saveTaxonomy = async () => {
    ...
    try {
      ...
      showToast.error('Unable to save taxonomy', caught instanceof Error ? caught.message : 'Taxonomy update failed.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        {question.approvedVersionId && <Badge className="bg-success/10 text-success hover:bg-success/10">Approved version</Badge>}
        {question.publishedVersionId && <Badge className="bg-primary/10 text-primary hover:bg-primary/10">Published v{detail.versions.find((version) => version.id === question.publishedVersionId)?.versionNumber ?? '—'}</Badge>}
        ...
        <div className="space-y-6">
          {editor ? (
            ...
            <>
              ...
              <Card><CardHeader><CardTitle className="text-base">Metadata</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2"><Meta label="Difficulty" value={viewedVersion.difficulty} /><Meta label="Question type" value={viewedVersion.questionType} /><Meta label="Exam" value={viewedVersion.examName ?? '—'} /><Meta label="Family" value={viewedVersion.examFamilyName ?? '—'} /><Meta label="Topic" value={viewedVersion.taxonomy.find((node) => node.nodeType === 'topic')?.name ?? generationValue(viewedVersion, 'topic') || '—'} /><Meta label="Subject" value={viewedVersion.taxonomy.find((node) => node.nodeType === 'subject')?.name ?? '—'} /><Meta label="Generation run" value={generationValue(viewedVersion, 'generationRunCode') || '—'} /><Meta label="Package" value={generationValue(viewedVersion, 'packageId') || '—'} /></CardContent></Card>
            </>
          )}

          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Tags className="h-4 w-4" /> Exam and taxonomy</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              {publishedVersionFrozen && <div className="rounded-lg border border-warning/30 bg-warning/5 p-3 text-sm text-warning">The published version is frozen. Create a new draft version before changing taxonomy.</div>}
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2"><Label>Exam family</Label><Select value={taxonomy.familyId} onValueChange={(value) => setTaxonomy({ ...EMPTY_TAXONOMY, familyId: value })} disabled={!canEdit || publishedVersionFrozen}><SelectTrigger><SelectValue placeholder="Select family" /></SelectTrigger><SelectContent>{taxonomyOptions?.families.map((family) => <SelectItem key={family.id} value={family.id}>{family.name}</SelectItem>)}</SelectContent></Select></div>
                <div className="space-y-2"><Label>Exam</Label><Select value={taxonomy.examId} onValueChange={(value) => { const exam = exams.find((entry) => entry.id === value); setTaxonomy((current) => ({ ...current, examId: value, examVersionId: exam?.currentVersionId ?? '', subjectId: '', topicId: '' })); }} disabled={!canEdit || publishedVersionFrozen || !taxonomy.familyId}><SelectTrigger><SelectValue placeholder="Select exam" /></SelectTrigger><SelectContent>{exams.map((exam) => <SelectItem key={exam.id} value={exam.id}>{exam.name}</SelectItem>)}</SelectContent></Select></div>
                <div className="space-y-2"><Label>Subject</Label><Select value={taxonomy.subjectId} onValueChange={(value) => setTaxonomy((current) => ({ ...current, subjectId: value, topicId: '' }))} disabled={!canEdit || publishedVersionFrozen || !taxonomy.examVersionId}><SelectTrigger><SelectValue placeholder="Select subject" /></SelectTrigger><SelectContent>{subjects.map((node) => <SelectItem key={node.id} value={node.id}>{node.name}</SelectItem>)}</SelectContent></Select></div>
                <div className="space-y-2"><Label>Topic</Label><Select value={taxonomy.topicId} onValueChange={(value) => setTaxonomy((current) => ({ ...current, topicId: value }))} disabled={!canEdit || publishedVersionFrozen || !taxonomy.subjectId}><SelectTrigger><SelectValue placeholder="Select topic" /></SelectTrigger><SelectContent>{topics.map((node) => <SelectItem key={node.id} value={node.id}>{node.name}</SelectItem>)}</SelectContent></Select></div>
              </div>
              {canEdit && <div className="flex justify-end"><Button onClick={() => void saveTaxonomy()} disabled={saving || publishedVersionFrozen}><Save className="mr-1.5 h-4 w-4" /> Save taxonomy</Button></div>}
            </CardContent>
          </Card>
        </div>

        <div className="space-y-6">
          <Card><CardHeader><CardTitle className="text-base">Publishing readiness</CardTitle></CardHeader><CardContent>{question.publishedVersionId ? <div className="space-y-2"><p className="text-sm font-medium text-primary">Available to Test Builder</p><p className="text-xs text-muted-foreground">Published {formatDate(question.publishedAt)}</p></div> : detail.publicationIssues.length === 0 ? <p className="text-sm font-medium text-success">Ready to publish.</p> : <ul className="space-y-2 text-sm text-muted-foreground">{detail.publicationIssues.map((issue) => <li key={issue} className="flex gap-2"><XCircle className="mt-0.5 h-4 w-4 shrink-0 text-warning" />{issue}</li>)}</ul>}</CardContent></Card>

          <Card><CardHeader><CardTitle className="text-base">Editorial actions</CardTitle></CardHeader><CardContent className="space-y-3"><div className="space-y-2"><Label>Action reason</Label><Textarea value={actionReason} onChange={(event) => setActionReason(event.target.value)} rows={3} placeholder="Required for needs-fix, unpublish, restore and archive" /></div>{canEdit && !editor && question.status !== 'archived' && <Button variant="outline" className="w-full justify-start" onClick={beginEdit}><PencilLine className="mr-1.5 h-4 w-4" /> Edit as new version</Button>}{canEdit && ['draft', 'needs_fix', 'generated'].includes(question.status) && <Button className="w-full justify-start" onClick={() => void runAction('submit-review', 'Submitted for review')} disabled={saving}><Send className="mr-1.5 h-4 w-4" /> Submit for review</Button>}{canApprove && !['approved', 'published', 'archived'].includes(question.status) && <Button className="w-full justify-start" onClick={() => void runAction('approve', 'Question approved')} disabled={saving}><CheckCircle2 className="mr-1.5 h-4 w-4" /> Approve current version</Button>}{canApprove && question.status === 'approved' && <Button className="w-full justify-start" onClick={() => void runAction('publish', 'Question published')} disabled={saving || detail.publicationIssues.length > 0}><Rocket className="mr-1.5 h-4 w-4" /> Publish to Test Builder</Button>}{canApprove && question.publishedVersionId && <Button variant="outline" className="w-full justify-start" onClick={() => void runAction('unpublish', 'Question unpublished')} disabled={saving}><EyeOff className="mr-1.5 h-4 w-4" /> Unpublish</Button>}{canApprove && !['needs_fix', 'archived', 'published'].includes(question.status) && <Button variant="outline" className="w-full justify-start" onClick={() => void runAction('needs-fix', 'Question sent for fixes')} disabled={saving}><XCircle className="mr-1.5 h-4 w-4" /> Mark needs fix</Button>}{canEdit && question.status === 'archived' && <Button variant="outline" className="w-full justify-start" onClick={() => void runAction('restore-draft', 'Question restored')} disabled={saving}><Undo2 className="mr-1.5 h-4 w-4" /> Restore as draft</Button>}{canArchive && question.status !== 'archived' && <Button variant="destructive" className="w-full justify-start" onClick={() => void runAction('archive', 'Question archived')} disabled={saving}><Archive className="mr-1.5 h-4 w-4" /> Archive question</Button>}</CardContent></Card>

          <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><History className="h-4 w-4" /> Version history</CardTitle></CardHeader><CardContent className="space-y-2">{detail.versions.map((version) => (<button key={version.id} type="button" onClick={() => { setSelectedVersionId(version.id); setEditor(null); }} className={version.id === viewedVersion.id ? 'w-full rounded-lg border border-primary/40 bg-primary/5 p-3 text-left' : 'w-full rounded-lg border p-3 text-left hover:bg-muted/30'}><div className="flex items-center justify-between"><span className="font-medium">Version {version.versionNumber}</span><span className="text-xs text-muted-foreground">{formatDate(version.createdAt)}</span></div><p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{version.changeReason}</p><div className="mt-2 flex flex-wrap gap-1">{version.id === question.currentDraftVersionId && <Badge variant="outline">Current draft</Badge>}{version.id === question.approvedVersionId && <Badge className="bg-success/10 text-success hover:bg-success/10">Approved</Badge>}{version.id === question.publishedVersionId && <Badge className="bg-primary/10 text-primary hover:bg-primary/10">Published</Badge>}</div></button>))}</CardContent></Card>
