...
import {
  INT_CP010_QUESTION_STUDIO_CP_ID,
  INT_CP010_QUESTION_STUDIO_INTEGRATION_VERSION,
  INT_CP010_QUESTION_STUDIO_LANGUAGES,
  INT_CP010_QUESTION_STUDIO_PACKAGE_ID,
  generateIntCp010QuestionStudioBatch,
  listIntCp010QuestionStudioPackages,
} from "../quant-v4/topics/Arithmetic/subtopics/Interest/INT-001/cp010-question-studio-integration-v1";

const router = Router();
const LANGUAGES = new Set<string>(INT_CP010_QUESTION_STUDIO_LANGUAGES);
const QL_IDS = new Set(["INT-QL-130", "INT-QL-131"]);
...
function publicRunCode() {
  ...
  return `INT10-${date}-${randomUUID().replaceAll("-", "").slice(0, 8).toUpperCase()}`;
}
...
async function persistRun(questionPackages: readonly any[], questions: readonly any[], requestSnapshot: Record<string, unknown>, actorUserId: string) {
  if (!questions.length || questions.length !== questionPackages.length) throw new Error("No valid INT-CP-010 questions matched the request.");
  ...
  await sqlClient.begin(async (tx) => {
    await tx`
      INSERT INTO content.generation_runs (
        ...
        'examtree', 'quant-v4-int-cp010-frozen-multilingual', 0, 0, 0, 0,
    ...
    for (let index = 0; index < questions.length; index += 1) {
      ...
      const payload = {
        ...question,
        sourcePackage: questionPackages[index],
    ...
    await tx`
      INSERT INTO platform.audit_events (
        ...
        'question_studio.interest_cp010_run.created', 'generation_run', ${runId}::uuid,
        'INT-CP-010 frozen multilingual questions entered the Question Studio review queue with downstream release locks preserved',
        ${`Created ${questions.length} INT-CP-010 review items in ${publicCode}`},
        ${JSON.stringify({ requestSnapshot, integrationAuthority: INT_CP010_QUESTION_STUDIO_INTEGRATION_VERSION, questionBankWritable: false, testEligible: false, publiclyPublishable: false })}::jsonb
      )
    `;
    await tx`
      ...
      VALUES (
        ...
        'question_studio.interest_cp010_run.created',
        ${JSON.stringify({ runId, publicCode, itemCount: questions.length, packageId: INT_CP010_QUESTION_STUDIO_PACKAGE_ID, checkpointId: INT_CP010_QUESTION_STUDIO_CP_ID, reviewOnly: true })}::jsonb
      )
    `;
  });
...
router.get("/quant/interest/cp010/package", requireAdminPermission("content.generation.read"), (_req, res) => {
  res.json({
    generationSystem: "quant-v4",
    activationMode: "FROZEN_MULTILINGUAL_REVIEW",
    package: listIntCp010QuestionStudioPackages()[0],
    maxBatchSize: 50,
    permanentQlCount: 2,
...
router.get("/quant/interest/cp010/preview", requireAdminPermission("content.generation.read"), async (req, res) => {
  try {
    ...
    const result = await generateIntCp010QuestionStudioBatch({
    ...
    res.json({ ...result, integrationAuthority: INT_CP010_QUESTION_STUDIO_INTEGRATION_VERSION, reviewOnly: true, productionEligible: false });
  } catch (error) {
    res.status(400).json({ error: error instanceof Error ? error.message : "Unable to preview Interest CP-010 questions." });
  }
});

router.post("/quant/interest/cp010/runs", requireAdminPermission("content.generation.run"), async (req, res) => {
  ...
  try {
    ...
    const result = await generateIntCp010QuestionStudioBatch({ canonicalProblemId: INT_CP010_QUESTION_STUDIO_CP_ID, language, questionLanguageId: qlId || undefined, seed, count });
    const requestSnapshot = {
      packageId: INT_CP010_QUESTION_STUDIO_PACKAGE_ID,
      checkpointId: INT_CP010_QUESTION_STUDIO_CP_ID,
      language, qlId: qlId || null, count, seed: seed || null,
      integrationAuthority: INT_CP010_QUESTION_STUDIO_INTEGRATION_VERSION,
    ...
    res.status(201).json({ ...persisted, generationSystem: "quant-v4", packageId: INT_CP010_QUESTION_STUDIO_PACKAGE_ID, checkpointId: INT_CP010_QUESTION_STUDIO_CP_ID, integrationAuthority: INT_CP010_QUESTION_STUDIO_INTEGRATION_VERSION, reviewOnly: true, questionBankWritable: false, testEligible: false, mockTestEligible: false, publiclyPublishable: false });
  } catch (error) {
    console.error("Interest CP-010 Question Studio run failed", error);
    res.status(500).json({ error: error instanceof Error ? error.message : "Unable to create Interest CP-010 review run." });
  }
});

router.get("/quant/interest/cp010/status", requireAdminPermission("content.generation.read"), async (_req, res) => {
  try {
    const rows = await sqlClient`
      ...
      WHERE v.payload ->> 'packageId' = ${INT_CP010_QUESTION_STUDIO_PACKAGE_ID}
        AND v.payload ->> 'canonicalProblemId' = ${INT_CP010_QUESTION_STUDIO_CP_ID}
        AND v.payload ->> 'integrationAuthority' = ${INT_CP010_QUESTION_STUDIO_INTEGRATION_VERSION}
    `;
    res.json({
      packageId: INT_CP010_QUESTION_STUDIO_PACKAGE_ID,
      checkpointId: INT_CP010_QUESTION_STUDIO_CP_ID,
    ...
    res.status(500).json({ error: error instanceof Error ? error.message : "Unable to read Interest CP-010 Question Studio status." });
  }
});
