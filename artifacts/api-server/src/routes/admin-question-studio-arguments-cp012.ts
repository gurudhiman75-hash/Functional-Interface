...
} from "../reasoning-v1/topics/Statement-and-Arguments/ARG-001/cp012-question-studio-adapter.ts";
import { ARG_CP012_CHECKPOINT_ID } from "../reasoning-v1/topics/Statement-and-Arguments/ARG-001/cp012-editorial-real-paper-remediation.ts";
...
function currentPackage(pkg: any) {
  ...
  return Object.freeze({
    ...base,
    cpIds: Object.freeze([...new Set([...base.cpIds, ...ARG_CP012_QUESTION_STUDIO_PACKAGE.cpIds, "ARG-CP-007"])]),
    currentCoreCheckpointId: ARG_CP012_QUESTION_STUDIO_PACKAGE.currentCoreCheckpointId,
    currentRealPaperCheckpointId: ARG_CP012_QUESTION_STUDIO_PACKAGE.currentRealPaperCheckpointId,
    currentQuestionStudioAuthority: ARG_CP012_QUESTION_STUDIO_PACKAGE.currentQuestionStudioAuthority,
    coreEnglishAuthority: ARG_CP012_QUESTION_STUDIO_PACKAGE.coreEnglishAuthority,
    coreLocalizationAuthority: ARG_CP012_QUESTION_STUDIO_PACKAGE.coreLocalizationAuthority,
    historicalRealPaperAuthority: ARG_CP012_QUESTION_STUDIO_PACKAGE.historicalRealPaperAuthority,
    realPaperAuthority: ARG_CP012_QUESTION_STUDIO_PACKAGE.realPaperAuthority,
    runtimeMode: ARG_CP012_QUESTION_STUDIO_PACKAGE.runtimeMode,
    reviewStatus: ARG_CP012_QUESTION_STUDIO_PACKAGE.reviewStatus,
    realPaperParityStatus: "CP012_EDITORIAL_REMEDIATION_REVIEW_CONNECTED" as const,
    examProfiles: ARG_CP012_QUESTION_STUDIO_PACKAGE.examProfiles,
    learnerRelease: ARG_CP012_QUESTION_STUDIO_PACKAGE.learnerRelease,
...
function normalizedArgRequest(body: Readonly<Record<string, unknown>>, count: number): ArgCp012QuestionStudioInput {
...
router.get("/capabilities", requireAdminPermission("content.generation.read"), async (_req, res) => {
  try {
    ...
    res.json({
      ...
      languages: ["en", "hi", "pa"],
      maxBatchSize: 50,
      arg001CurrentAuthority: ARG_CP012_QUESTION_STUDIO_AUTHORITY,
    });
  } catch (error) {
    console.error("ARG-001 CP012 capabilities failed", error);
...
router.post("/runs", requireAdminPermission("content.generation.run"), async (req, res, next) => {
  ...
  if (!isArgCp012CurrentReviewRequest(body)) {
    next();
    return;
  }
  ...
  const realPaper = isArgCp012RealPaperRequest(generationInput);
  const runId = randomUUID();
  const code = publicRunCode();
  const timestamp = new Date().toISOString();

  try {
    const result = generateArgCp012QuestionStudioBatch(generationInput);
    const questions = result.questions;
    if (questions.length === 0) {
      res.status(422).json({ error: "The ARG-001 CP012 generation engine returned no questions" });
      return;
    }
    ...
    const requestSnapshot = {
      ...
      resolvedCheckpointId: ARG_CP012_CHECKPOINT_ID,
    ...
    await sqlClient.begin(async (tx) => {
      await tx`INSERT INTO content.generation_runs (
        ...
        ${JSON.stringify(requestSnapshot)}, ${JSON.stringify(requestSnapshot)}, 'examtree', 'reasoning-v1-arg-001-cp012-editorial-remediation',
      ...
      for (let index = 0; index < questions.length; index += 1) {
        ...
        ) throw new Error("ARG-001 CP012 attempted to persist a question with a learner-delivery gate open.");
        if (question.currentQuestionStudioAuthority !== ARG_CP012_QUESTION_STUDIO_AUTHORITY) {
          throw new Error("ARG-001 CP012 attempted to persist a question from a stale review authority.");
        }
      ...
      await tx`INSERT INTO platform.audit_events (
        ...
        'Admin generated an ARG-001 CP012 editorial-remediation review batch',
        ${`Generated ${questions.length} CP012 Statement & Arguments questions in ${code}`},
      ...
      await tx`INSERT INTO platform.outbox_events (
        ...
        ${JSON.stringify({ runId, publicCode: code, itemCount: questions.length, chapter: "ARG-001", checkpointId: ARG_CP012_CHECKPOINT_ID, profileMode: realPaper ? "real-paper" : "core" })}
      )`;
    });
    ...
    console.error("ARG-001 CP012 editorial-remediation generation failed", error);
