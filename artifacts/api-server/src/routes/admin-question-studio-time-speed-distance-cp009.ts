...
import {
  TSD_CP009_QUESTION_STUDIO_DIFFICULTIES,
  TSD_CP009_QUESTION_STUDIO_INTEGRATION_AUTHORITY,
  TSD_CP009_QUESTION_STUDIO_LANGUAGES,
  TSD_CP009_QUESTION_STUDIO_PACKAGE_ID,
  TSD_CP009_QUESTION_STUDIO_REVIEW_PACKAGE,
  previewTsdCp009QuestionStudioReview,
  type TsdCp009QuestionStudioDifficulty,
  type TsdCp009QuestionStudioLanguage,
  type TsdCp009QuestionStudioQlId,
} from "../quant-v4/topics/Arithmetic/subtopics/TimeSpeedDistance/TSD-002/cp009/question-studio-review-adapter";
import { TSD_CP009_PERMANENT_QL_IDS } from "../quant-v4/topics/Arithmetic/subtopics/TimeSpeedDistance/TSD-002/cp009/ql-allocation";

const router = Router();
const LANGUAGES = new Set<string>(TSD_CP009_QUESTION_STUDIO_LANGUAGES);
const DIFFICULTIES = new Set<string>(TSD_CP009_QUESTION_STUDIO_DIFFICULTIES);
const QL_IDS = new Set<string>(TSD_CP009_PERMANENT_QL_IDS);
type ReviewQuestion = ReturnType<typeof previewTsdCp009QuestionStudioReview>["questions"][number];
...
function publicRunCode() {
  ...
  return `TSD9-${date}-${randomUUID().replaceAll("-", "").slice(0, 8).toUpperCase()}`;
}
...
async function persistRun(questions: readonly ReviewQuestion[], requestSnapshot: Record<string, unknown>, actorUserId: string) {
  if (!questions.length) throw new Error("No TSD-CP-009 questions matched the request.");
  ...
  await sqlClient.begin(async (tx) => {
    await tx`
      INSERT INTO content.generation_runs (
        ...
        'examtree', 'quant-v4-tsd-cp009-frozen-review', 0, 0, 0, 0,
    ...
    await tx`
      INSERT INTO platform.audit_events (
        ...
        'question_studio.tsd_cp009_run.created', 'generation_run', ${runId}::uuid,
        'TSD-CP-009 entered the Question Studio review queue with downstream release locks preserved',
        ${`Created ${questions.length} TSD-CP-009 review items in ${publicCode}`},
        ${JSON.stringify({
          requestSnapshot,
...
router.get("/quant/time-speed-distance/cp009/package", requireAdminPermission("content.generation.read"), (_req, res) => {
  res.json({
    generationSystem: "quant-v4",
    activationMode: "REVIEW_ONLY",
    package: TSD_CP009_QUESTION_STUDIO_REVIEW_PACKAGE,
    maxBatchSize: 50,
    permanentQlCount: TSD_CP009_PERMANENT_QL_IDS.length,
    deterministicReviewCombinations: TSD_CP009_QUESTION_STUDIO_REVIEW_PACKAGE.deterministicReviewCombinations,
...
router.get("/quant/time-speed-distance/cp009/preview", requireAdminPermission("content.generation.read"), (req, res) => {
  try {
    const language = (asString(req.query.language) || "en") as TsdCp009QuestionStudioLanguage;
    ...
    const result = previewTsdCp009QuestionStudioReview({
      language,
      difficulty: difficulty ? difficulty as TsdCp009QuestionStudioDifficulty : undefined,
      qlId: qlId ? qlId as TsdCp009QuestionStudioQlId : undefined,
      familyId: familyId || undefined,
      seed: asString(req.query.seed) || undefined,
      count: asCount(req.query.count, 1, 20),
    });
    res.json({ ...result, integrationAuthority: TSD_CP009_QUESTION_STUDIO_INTEGRATION_AUTHORITY, productionEligible: false, reviewOnly: true });
  } catch (error) {
    res.status(400).json({ error: error instanceof Error ? error.message : "Unable to preview TSD CP-009 questions." });
  }
});

router.post("/quant/time-speed-distance/cp009/runs", requireAdminPermission("content.generation.run"), async (req, res) => {
  ...
  try {
    const language = (asString(req.body?.language) || "en") as TsdCp009QuestionStudioLanguage;
    ...
    console.error("TSD CP-009 Question Studio run failed", error);
    res.status(500).json({ error: error instanceof Error ? error.message : "Unable to create TSD CP-009 review run." });
  }
});

router.get("/quant/time-speed-distance/cp009/status", requireAdminPermission("content.generation.read"), async (_req, res) => {
  try {
    const rows = await sqlClient`
      ...
      WHERE v.payload ->> 'packageId' = ${TSD_CP009_QUESTION_STUDIO_PACKAGE_ID}
        AND v.payload ->> 'canonicalProblemId' = 'TSD-CP-009'
        AND v.payload ->> 'integrationAuthority' = ${TSD_CP009_QUESTION_STUDIO_INTEGRATION_AUTHORITY}
    `;
    res.json({
      packageId: TSD_CP009_QUESTION_STUDIO_PACKAGE_ID,
      checkpointId: "TSD-CP-009",
    ...
    console.error("TSD CP-009 Question Studio status failed", error);
    res.status(500).json({ error: "Unable to read TSD CP-009 Question Studio status." });
  }
});
