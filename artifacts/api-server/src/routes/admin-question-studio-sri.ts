...
router.get("/capabilities", requireAdminPermission("content.generation.read"), async (_req, res) => {
  try {
    ...
    console.error("SRI Question Studio capabilities failed", error);
...
router.post("/runs", requireAdminPermission("content.generation.run"), async (req, res, next) => {
  if (!isSriQuestionStudioRequestV1(req.body ?? {})) {
    next();
    return;
  }
  ...
  const patternId = asString(req.body?.patternId) || packageId || "SRI";
  ...
  const topic = asString(req.body?.topic) || "Number System";
  const subtopic = asString(req.body?.subtopic) || "Surds & Indices";
  ...
  try {
    ...
    if (generatedQuestions.length === 0) {
      res.status(422).json({ error: "The SRI generation engine returned no questions" });
      return;
    }
    ...
    await sqlClient.begin(async (tx) => {
      await tx`INSERT INTO content.generation_runs (
        ...
        'examtree', 'quant-v4-sri', 0, 0, 0, 0, ${timestamp}, ${timestamp}, ${timestamp}, ${timestamp}
      )`;
      ...
      for (let index = 0; index < generatedQuestions.length; index += 1) {
        ...
        if (question.questionBankWritable !== false || question.testEligible !== false || question.publiclyPublishable !== false) {
          throw new Error("SRI Question Studio attempted to persist a question with downstream release gates open.");
        }
        const payload = {
          ...question,
      ...
      await tx`INSERT INTO platform.audit_events (
        ...
        ${`Generated ${generatedQuestions.length} Surds & Indices questions in ${code}`},
      ...
      await tx`INSERT INTO platform.outbox_events (
        ...
        ${JSON.stringify({ runId, publicCode: code, itemCount: generatedQuestions.length, chapter: "SRI" })}
      )`;
    });
    ...
    console.error("SRI Question Studio generation failed", error);
