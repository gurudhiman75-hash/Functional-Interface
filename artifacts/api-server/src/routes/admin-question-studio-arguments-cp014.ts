...
import {
  ...
  isArgCp014RealPaperRequest,
  type ArgCp014QuestionStudioInput,
} from "../reasoning-v1/topics/Statement-and-Arguments/ARG-001/cp014-manual-editorial-approval.ts";
...
function currentPackage(pkg: any) {
  ...
  return Object.freeze({
    ...base,
    cpIds: Object.freeze([...new Set([...base.cpIds, ...ARG_CP014_QUESTION_STUDIO_PACKAGE.cpIds, "ARG-CP-007"])]),
    currentCoreCheckpointId: ARG_CP014_QUESTION_STUDIO_PACKAGE.currentCoreCheckpointId,
    currentRealPaperCheckpointId: ARG_CP014_QUESTION_STUDIO_PACKAGE.currentRealPaperCheckpointId,
    sourceFinalEditorialCheckpointId: ARG_CP014_QUESTION_STUDIO_PACKAGE.sourceFinalEditorialCheckpointId,
    currentReleaseCheckpointId: ARG_CP014_QUESTION_STUDIO_PACKAGE.currentReleaseCheckpointId,
    currentQuestionStudioAuthority: ARG_CP014_QUESTION_STUDIO_PACKAGE.currentQuestionStudioAuthority,
    finalEditorialSurfaceAuthority: ARG_CP014_QUESTION_STUDIO_PACKAGE.finalEditorialSurfaceAuthority,
    sourceQuestionStudioAuthority: ARG_CP014_QUESTION_STUDIO_PACKAGE.sourceQuestionStudioAuthority,
    approvalAuthority: ARG_CP014_QUESTION_STUDIO_PACKAGE.approvalAuthority,
    runtimeMode: ARG_CP014_QUESTION_STUDIO_PACKAGE.runtimeMode,
    reviewStatus: ARG_CP014_QUESTION_STUDIO_PACKAGE.reviewStatus,
    learnerRelease: ARG_CP014_QUESTION_STUDIO_PACKAGE.learnerRelease,
...
function normalizedArgRequest(body: Readonly<Record<string, unknown>>, count: number): ArgCp014QuestionStudioInput {
...
router.get("/capabilities", requireAdminPermission("content.generation.read"), async (_req, res) => {
  try {
    ...
    res.json({
      ...
      languages: ["en", "hi", "pa"],
      maxBatchSize: 50,
      arg001CurrentAuthority: ARG_CP014_QUESTION_STUDIO_AUTHORITY,
    });
  } catch (error) {
    console.error("ARG-001 CP014 capabilities failed", error);
...
router.post("/runs", requireAdminPermission("content.generation.run"), async (req, res, next) => {
  ...
  if (!isArgCp014CurrentRequest(body)) {
    next();
    return;
  }
  ...
  const realPaper = isArgCp014RealPaperRequest(generationInput);
  const runId = randomUUID();
  const code = publicRunCode();
  const timestamp = new Date().toISOString();

  try {
    const result = generateArgCp014QuestionStudioBatch(generationInput);
    const questions = result.questions;
    if (questions.length === 0) {
      res.status(422).json({ error: "The ARG-001 CP014 generation engine returned no questions" });
      return;
    }
    ...
    const requestSnapshot = {
      ...
      resolvedCheckpointId: ARG_CP014_CHECKPOINT_ID,
    ...
    await sqlClient.begin(async (tx) => {
      await tx`INSERT INTO content.generation_runs (
        ...
        ${JSON.stringify(requestSnapshot)}, ${JSON.stringify(requestSnapshot)}, 'examtree', 'reasoning-v1-arg-001-cp014-editorially-approved',
      ...
      for (let index = 0; index < questions.length; index += 1) {
        ...
        ) throw new Error("ARG-001 CP014 attempted to persist a question outside the approved internal-only lifecycle boundary.");
        if (question.currentQuestionStudioAuthority !== ARG_CP014_QUESTION_STUDIO_AUTHORITY) {
          throw new Error("ARG-001 CP014 attempted to persist a question from a stale authority.");
        }
      ...
      await tx`INSERT INTO platform.audit_events (
        ...
        'Admin generated an ARG-001 CP014 editor-approved internal-eligible batch',
        ${`Generated ${questions.length} CP014 Statement & Arguments questions in ${code}`},
      ...
      await tx`INSERT INTO platform.outbox_events (
        ...
        ${JSON.stringify({ runId, publicCode: code, itemCount: questions.length, chapter: "ARG-001", checkpointId: ARG_CP014_CHECKPOINT_ID, profileMode: realPaper ? "real-paper" : "core" })}
      )`;
    });
    ...
    console.error("ARG-001 CP014 approved generation failed", error);
