...
router.post('/:attemptId/actions/abandon', requireAdminPermission('users.students.manage'), async (req, res) => {
  ...
  try {
    const operation = await sqlClient.begin(async (tx) => {
      ...
      if (expectedUpdatedAt && new Date(String(attempt.updatedAt)).toISOString() !== new Date(expectedUpdatedAt).toISOString()) {
      ...
      await tx`UPDATE learning.attempts SET status = 'abandoned', updated_at = now() WHERE id = ${attemptId}::uuid`;
      ...
      await tx`
        INSERT INTO platform.audit_events (
          ...
          'student.attempt.abandoned', 'attempt', ${attemptId}::uuid,
          ${reason}, ${`Abandoned stale attempt for ${String(attempt.studentName)}`},
          ${tx.json({
      ...
      `;
      return { attemptId, previousStatus: 'in_progress', status: 'abandoned', inactiveSeconds, auditEventId, occurredAt: new Date().toISOString() };
    });
