...
import { TSD_CP007_PERMANENT_QL_IDS } from "../quant-v4/topics/Arithmetic/subtopics/TimeSpeedDistance/TSD-002/cp007/ql-allocation";

const router = Router();
const LANGUAGES = new Set<string>(TSD_CP007_QUESTION_STUDIO_LANGUAGES);
const DIFFICULTIES = new Set<string>(TSD_CP007_QUESTION_STUDIO_DIFFICULTIES);
const QL_IDS = new Set<string>(TSD_CP007_PERMANENT_QL_IDS);

type TsdReviewQuestion = Readonly<{
  stem: string;
...
function publicRunCode() {
  ...
  return `TSD-${date}-${randomUUID().replaceAll("-", "").slice(0, 8).toUpperCase()}`;
}

function reviewPayload(question: TsdReviewQuestion) {
  const explanation = [...question.explanation.steps, question.explanation.conclusion].filter(Boolean).join("\n");
  return {
    text: question.stem,
    stem: question.stem,
...
async function persistRun(
  questions: readonly TsdReviewQuestion[],
  requestSnapshot: Record<string, unknown>,
  actorUserId: string,
) {
  if (!questions.length) throw new Error("No TSD-CP-007 questions matched the request.");
  ...
  await sqlClient.begin(async (tx) => {
    await tx`
      INSERT INTO content.generation_runs (
        ...
        'examtree', 'quant-v4-tsd-cp007-frozen-review', 0, 0, 0, 0,
    ...
    await tx`
      INSERT INTO platform.audit_events (
        ...
        'question_studio.tsd_cp007_run.created', 'generation_run', ${runId}::uuid,
        'TSD-CP-007 entered the Question Studio review queue with downstream release locks preserved',
        ${`Created ${questions.length} TSD-CP-007 review items in ${publicCode}`},
        ${JSON.stringify({
          requestSnapshot,
...
router.get(
  ...
  (_req, res) => {
    res.json({
      ...
      package: TSD_CP007_QUESTION_STUDIO_REVIEW_PACKAGE,
      maxBatchSize: 50,
      permanentQlCount: TSD_CP007_PERMANENT_QL_IDS.length,
...
router.get(
  ...
  (req, res) => {
    try {
      const language = (asString(req.query.language) || "en") as TsdCp007QuestionStudioLanguage;
      ...
      const result = previewTsdCp007QuestionStudioReview({
        language,
        difficulty: difficulty ? difficulty as TsdCp007QuestionStudioDifficulty : undefined,
        qlId: qlId ? qlId as TsdCp007QuestionStudioQlId : undefined,
        seed: asString(req.query.seed) || undefined,
        count: asCount(req.query.count, 1, 20),
      });
      res.json({ ...result, integrationAuthority: TSD_CP007_QUESTION_STUDIO_INTEGRATION_AUTHORITY, productionEligible: false, reviewOnly: true });
    } catch (error) {
      res.status(400).json({ error: error instanceof Error ? error.message : "Unable to preview TSD CP-007 questions." });
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const language = (asString(req.body?.language) || "en") as TsdCp007QuestionStudioLanguage;
      ...
      const questions = result.questions as readonly TsdReviewQuestion[];
      ...
      console.error("TSD CP-007 Question Studio run failed", error);
      res.status(500).json({ error: error instanceof Error ? error.message : "Unable to create TSD CP-007 review run." });
    }
  },
);
  ...
  async (_req, res) => {
    try {
      const rows = await sqlClient`
        ...
        WHERE v.payload ->> 'packageId' = ${TSD_CP007_QUESTION_STUDIO_PACKAGE_ID}
          AND v.payload ->> 'canonicalProblemId' = 'TSD-CP-007'
          AND v.payload ->> 'integrationAuthority' = ${TSD_CP007_QUESTION_STUDIO_INTEGRATION_AUTHORITY}
      `;
      res.json({
        packageId: TSD_CP007_QUESTION_STUDIO_PACKAGE_ID,
        checkpointId: "TSD-CP-007",
      ...
      console.error("TSD CP-007 Question Studio status failed", error);
      res.status(500).json({ error: "Unable to load TSD CP-007 review status." });
    }
  },
);
