...
export type AdminSessionRouteDependencies = {
  ...
  hasAdministratorAccess: (firebaseUid: string) => Promise<boolean>;
  bootstrap: (input: {
    firebaseUid: string;
    email?: string;
    displayName?: string;
...
export async function hasCanonicalAdministratorAccess(firebaseUid: string): Promise<boolean> {
...
export function createAdminSessionRouter(
  dependencies: AdminSessionRouteDependencies = {
    ...
    hasAdministratorAccess: hasCanonicalAdministratorAccess,
  ...
  router.post("/bootstrap", dependencies.authenticate, async (req, res, next) => {
    try {
      ...
      if (!(await dependencies.hasAdministratorAccess(req.user.id))) {
