...
function isRnkRequestBody(body: any): boolean {
  return isRnk001QuestionStudioRequest({
    packageId: text(body?.packageId) || undefined,
    patternId: text(body?.patternId) || undefined,
    topic: text(body?.topic) || undefined,
    subtopic: text(body?.subtopic) || undefined,
 ...
 * generation-run/item/version tables. Non-RNK requests skip this route before
 * authentication and fall through to the existing routes unchanged.
 *
