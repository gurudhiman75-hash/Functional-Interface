...
} from "../question-studio/shared-generation-engine-cp013";
...
/**
 * This route is mounted immediately before the legacy Question Studio router.
 * It returns the additive CP013 capability surface; non-CP013 POST requests
 * deliberately fall through to the existing router unchanged.
 */
router.get(
  "/capabilities",
  ...
  async (_req, res) => {
    try {
      ...
      console.error("CP013 Question Studio capabilities failed", error);
...
router.post(
  ...
  async (req, res, next) => {
    if (!isNumCp013QuestionStudioRequest(req.body ?? {})) {
      next();
      return;
    }
    ...
    const canonicalProblemId = asString(req.body?.canonicalProblemId) || asString(req.body?.cpId) || "NUM-CP-013";
    ...
    try {
      ...
      await sqlClient.begin(async (tx) => {
        await tx`
          INSERT INTO content.generation_runs (
            ...
            'examtree', 'quant-v4-num-cp013', 0, 0, 0, 0,
        ...
        await tx`
          INSERT INTO platform.audit_events (
            ...
            ${`Generated ${generatedQuestions.length} NUM-002/NUM-CP-013 questions in ${code}`},
      ...
      console.error("NUM-CP-013 Question Studio generation failed", error);
