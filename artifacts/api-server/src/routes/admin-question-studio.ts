...
router.get(
  ...
  async (_req, res) => {
    ...
    `;
      ...
      res
        .status(500)
        .json({ error: "Unable to load Question Studio dashboard" });
    }
  },
);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (generatedQuestions.length === 0) {
        res
          .status(422)
          .json({ error: "The generation engine returned no questions" });
        return;
      }
...
router.patch(
  ...
  async (req, res) => {
    ...
    const itemIds = [...new Set(rawIds.map(asString).filter(Boolean))].slice(
      0,
      100,
    );
    ...
    if (itemIds.length === 0) {
      res
        .status(400)
        .json({ error: "At least one generated item is required" });
      return;
    }
    ...
    try {
      const result = await sqlClient.begin(async (tx) => {
        ...
        const runIds = [
          ...new Set(changed.map((item) => item.generationRunId)),
        ];
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unable to update generated items";
