...
} from "../question-studio/shared-generation-engine-cp014";
...
router.get("/capabilities", requireAdminPermission("content.generation.read"), async (_req, res) => {
  try {
    const packages = listQuestionStudioPackages().map((pkg: any) => ({
      packageId: String(pkg.packageId), topic: String(pkg.topic), subtopic: String(pkg.subtopic),
      subject: asString(pkg.subject) || undefined, label: String(pkg.label), enabled: Boolean(pkg.enabled),
    ...
    console.error("CP014 Question Studio capabilities failed", error);
...
router.post("/runs", requireAdminPermission("content.generation.run"), async (req, res, next) => {
  if (!isNumCp014QuestionStudioRequest(req.body ?? {})) { next(); return; }
  const count = asPositiveInteger(req.body?.count, 5, 50);
  const language = normalizeLanguage(req.body?.language);
  const seed = asString(req.body?.seed) || undefined;
  const canonicalProblemId = asString(req.body?.canonicalProblemId) || asString(req.body?.cpId) || "NUM-CP-014";
  ...
  const subtopic = asString(req.body?.subtopic) || "Number System";
  const runId = randomUUID();
  const code = publicRunCode();
  const timestamp = new Date().toISOString();
  const requestSnapshot = { exam, subject, difficulty: "Hard", count, packageId: "NUM-002", patternId, topic, subtopic, canonicalProblemId, questionLanguageId, language, seed, requestedByFirebaseUid: req.user?.id };

  try {
    const result = await generateQuestion({ packageId: "NUM-002", patternId, topic, subtopic, canonicalProblemId, questionLanguageId, difficulty: "Hard", language, seed, count });
    const generatedQuestions = Array.isArray(result.questions) ? result.questions : [];
    if (generatedQuestions.length === 0) { res.status(422).json({ error: "The generation engine returned no questions" }); return; }
    ...
    await sqlClient.begin(async (tx) => {
      await tx`INSERT INTO content.generation_runs (
        ...
        'examtree', 'quant-v4-num-cp014', 0, 0, 0, 0, ${timestamp}, ${timestamp}, ${timestamp}, ${timestamp}
      )`;
      for (let index = 0; index < generatedQuestions.length; index += 1) {
        const itemId = randomUUID(), versionId = randomUUID();
        ...
        await tx`INSERT INTO content.generation_run_items (id, generation_run_id, item_number, status, current_version_number, created_at, updated_at)
          VALUES (${itemId}::uuid, ${runId}::uuid, ${index + 1}, 'unreviewed'::generation_item_status, 1, ${timestamp}, ${timestamp})`;
        await tx`INSERT INTO content.generation_item_versions (id, generation_item_id, version_number, payload, provider_item_id, created_at)
          VALUES (${versionId}::uuid, ${itemId}::uuid, 1, ${JSON.stringify(payload)}, ${asString(question.questionId) || null}, ${timestamp})`;
      }
      await tx`INSERT INTO platform.audit_events (id, actor_type, actor_user_id, action_key, entity_type, entity_id, reason, summary, metadata)
        VALUES (${randomUUID()}::uuid, 'user'::audit_actor_type, ${req.adminSession?.user.id ?? null}::uuid,
        'question_studio.generation_run.created', 'generation_run', ${runId}::uuid, 'Admin generated a Question Studio batch',
        ${`Generated ${generatedQuestions.length} NUM-002/NUM-CP-014 questions in ${code}`}, ${JSON.stringify({ firebaseUid: req.user?.id, requestSnapshot })})`;
      await tx`INSERT INTO platform.outbox_events (id, aggregate_type, aggregate_id, event_type, payload)
        VALUES (${randomUUID()}::uuid, 'generation_run', ${runId}::uuid, 'question_studio.generation_run.created',
        ${JSON.stringify({ runId, publicCode: code, itemCount: generatedQuestions.length })})`;
    });

    res.status(201).json({ id: runId, publicCode: code, status: "review", itemCount: generatedQuestions.length, generationSystem: "quant-v4" });
  } catch (error) {
    console.error("NUM-CP-014 Question Studio generation failed", error);
