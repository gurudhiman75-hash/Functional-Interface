...
const router = Router();
const LANGUAGES = new Set<string>(TSD_CP008_QUESTION_STUDIO_LANGUAGES);
const DIFFICULTIES = new Set<string>(TSD_CP008_QUESTION_STUDIO_DIFFICULTIES);
const QL_IDS = new Set<string>(TSD_CP008_PERMANENT_QL_IDS);

type TsdCp008ReviewQuestion = Readonly<{
  stem: string;
...
function publicRunCode() {
  ...
  return `TSD8-${date}-${randomUUID().replaceAll("-", "").slice(0, 8).toUpperCase()}`;
}

function reviewPayload(question: TsdCp008ReviewQuestion) {
...
async function persistRun(
  questions: readonly TsdCp008ReviewQuestion[],
  requestSnapshot: Record<string, unknown>,
  actorUserId: string,
) {
  if (!questions.length) throw new Error("No TSD-CP-008 questions matched the request.");
  ...
  await sqlClient.begin(async (tx) => {
    await tx`
      INSERT INTO content.generation_runs (
        ...
        'examtree', 'quant-v4-tsd-cp008-frozen-review', 0, 0, 0, 0,
    ...
    await tx`
      INSERT INTO platform.audit_events (
        ...
        'question_studio.tsd_cp008_run.created', 'generation_run', ${runId}::uuid,
        'TSD-CP-008 entered the Question Studio review queue with downstream release locks preserved',
        ${`Created ${questions.length} TSD-CP-008 review items in ${publicCode}`},
        ${JSON.stringify({
          requestSnapshot,
...
router.get(
  ...
  (_req, res) => {
    res.json({
      ...
      maxBatchSize: 50,
      permanentQlCount: TSD_CP008_PERMANENT_QL_IDS.length,
...
router.get(
  ...
  (req, res) => {
    try {
      const language = (asString(req.query.language) || "en") as TsdCp008QuestionStudioLanguage;
      ...
      const result = previewTsdCp008QuestionStudioReview({
        language,
        difficulty: difficulty ? difficulty as TsdCp008QuestionStudioDifficulty : undefined,
        qlId: qlId ? qlId as TsdCp008QuestionStudioQlId : undefined,
        familyId: familyId || undefined,
        seed: asString(req.query.seed) || undefined,
        count: asCount(req.query.count, 1, 20),
      });
      res.json({ ...result, integrationAuthority: TSD_CP008_QUESTION_STUDIO_INTEGRATION_AUTHORITY, productionEligible: false, reviewOnly: true });
    } catch (error) {
      res.status(400).json({ error: error instanceof Error ? error.message : "Unable to preview TSD CP-008 questions." });
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const language = (asString(req.body?.language) || "en") as TsdCp008QuestionStudioLanguage;
      ...
      const questions = result.questions as readonly TsdCp008ReviewQuestion[];
      ...
      console.error("TSD CP-008 Question Studio run failed", error);
      res.status(500).json({ error: error instanceof Error ? error.message : "Unable to create TSD CP-008 review run." });
    }
  },
);
  ...
  async (_req, res) => {
    try {
      const rows = await sqlClient`
        ...
        WHERE v.payload ->> 'packageId' = ${TSD_CP008_QUESTION_STUDIO_PACKAGE_ID}
          AND v.payload ->> 'canonicalProblemId' = 'TSD-CP-008'
          AND v.payload ->> 'integrationAuthority' = ${TSD_CP008_QUESTION_STUDIO_INTEGRATION_AUTHORITY}
      `;
      res.json({
        packageId: TSD_CP008_QUESTION_STUDIO_PACKAGE_ID,
        checkpointId: "TSD-CP-008",
      ...
      console.error("TSD CP-008 Question Studio status failed", error);
      res.status(500).json({ error: "Unable to load TSD CP-008 review status." });
    }
  },
);
