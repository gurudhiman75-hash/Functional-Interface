...
import {
  INT_CP004_QUESTION_STUDIO_DIFFICULTIES,
  INT_CP004_QUESTION_STUDIO_INTEGRATION_AUTHORITY,
  INT_CP004_QUESTION_STUDIO_LANGUAGES,
  INT_CP004_QUESTION_STUDIO_PACKAGE_ID,
  INT_CP004_QUESTION_STUDIO_REVIEW_PACKAGE,
  previewIntCp004QuestionStudioReview,
  type IntCp004QuestionStudioDifficulty,
  type IntCp004QuestionStudioLanguage,
} from "../quant-v4/topics/Arithmetic/subtopics/Interest/INT-001/cp004-question-studio-review-adapter";
import {
  INT_CP004_QL_IDS,
  type IntCp004QlId,
} from "../quant-v4/topics/Arithmetic/subtopics/Interest/INT-001/cp004-frequency-math";

const router = Router();
const LANGUAGES = new Set<string>(INT_CP004_QUESTION_STUDIO_LANGUAGES);
const DIFFICULTIES = new Set<string>(INT_CP004_QUESTION_STUDIO_DIFFICULTIES);
const QL_IDS = new Set<string>(INT_CP004_QL_IDS);

type InterestPreviewResult = ReturnType<typeof previewIntCp004QuestionStudioReview>;
type InterestPreviewQuestion = InterestPreviewResult["questions"][number];
...
function explanationText(question: InterestPreviewQuestion) {
  return [
...
function reviewPayload(question: InterestPreviewQuestion) {
  return {
    text: question.stem,
    stem: question.stem,
...
async function persistRun(
  questions: readonly InterestPreviewQuestion[],
  requestSnapshot: Record<string, unknown>,
  actorUserId: string,
) {
  if (questions.length === 0) {
    throw new Error("No INT-CP-004 questions matched the request.");
  }
  ...
  await sqlClient.begin(async (tx) => {
    await tx`
      INSERT INTO content.generation_runs (
        ...
        'examtree', 'quant-v4-int-cp004-frozen-review', 0, 0, 0, 0,
    ...
    await tx`
      INSERT INTO platform.audit_events (
        ...
        'question_studio.interest_cp004_run.created', 'generation_run', ${runId}::uuid,
        'INT-CP-004 entered the Question Studio review queue with downstream release locks preserved',
        ${`Created ${questions.length} INT-CP-004 review items in ${publicCode}`},
        ${JSON.stringify({
          requestSnapshot,
    ...
    await tx`
      INSERT INTO platform.outbox_events (
        ...
        'question_studio.interest_cp004_run.created',
        ${JSON.stringify({
          runId,
          publicCode,
...
router.get(
  "/quant/interest/cp004/package",
  requireAdminPermission("content.generation.read"),
  (_req, res) => {
    res.json({
      generationSystem: "quant-v4",
      activationMode: "REVIEW_ONLY",
      package: INT_CP004_QUESTION_STUDIO_REVIEW_PACKAGE,
      maxBatchSize: 50,
      permanentQlCount: INT_CP004_QL_IDS.length,
      approvedReviewPayloadCount: INT_CP004_QUESTION_STUDIO_REVIEW_PACKAGE.approvedReviewPayloadCount,
...
router.get(
  "/quant/interest/cp004/preview",
  requireAdminPermission("content.generation.read"),
  (req, res) => {
    try {
      const language = (asString(req.query.language) || "hi") as IntCp004QuestionStudioLanguage;
      ...
      const result = previewIntCp004QuestionStudioReview({
        language,
        difficulty: difficulty ? difficulty as IntCp004QuestionStudioDifficulty : undefined,
        qlId: qlId ? qlId as IntCp004QlId : undefined,
      ...
      res.status(400).json({
        error: error instanceof Error ? error.message : "Unable to preview Interest CP-004 questions.",
      });
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const language = (asString(req.body?.language) || "hi") as IntCp004QuestionStudioLanguage;
      ...
      console.error("Interest CP-004 Question Studio run failed", error);
      res.status(500).json({
        error: error instanceof Error ? error.message : "Unable to create Interest CP-004 review run.",
      });
    }
  },
);
  ...
  async (_req, res) => {
    try {
      const rows = await sqlClient`
        ...
        WHERE v.payload ->> 'packageId' = ${INT_CP004_QUESTION_STUDIO_PACKAGE_ID}
          AND v.payload ->> 'canonicalProblemId' = 'INT-CP-004'
          AND v.payload ->> 'integrationAuthority' = ${INT_CP004_QUESTION_STUDIO_INTEGRATION_AUTHORITY}
      `;
      res.json({
        packageId: INT_CP004_QUESTION_STUDIO_PACKAGE_ID,
        checkpointId: "INT-CP-004",
      ...
      console.error("Interest CP-004 Question Studio status failed", error);
      res.status(500).json({ error: "Unable to load Interest CP-004 review status." });
    }
  },
);
