...
router.get("/business", async (req, res) => {
  ...
  try {
    const [summaryRows, comparisonRows, daily, products, coupons, qualityRows] = await Promise.all([
      sqlClient`
        WITH bounds AS (SELECT now() - make_interval(days => ${days}) AS start_at),
        paid AS (
          SELECT o.id, o.user_id, o.total_minor, o.discount_minor, o.currency
          FROM commerce.orders o, bounds b
          WHERE o.paid_at >= b.start_at AND o.status IN ('paid','partially_refunded','refunded')
        ), refunds AS (
          ...
          WHERE r.status = 'processed' AND r.processed_at >= b.start_at
        ), attempts AS (
        ...
        SELECT
          ...
          COUNT(DISTINCT user_id)::int AS "payingStudents",
          COALESCE(SUM(total_minor),0)::float8 AS "grossRevenueMinor",
          (SELECT amount FROM refunds)::float8 AS "refundsMinor",
          ...
          COALESCE(MIN(currency),'INR') AS currency
        FROM paid
      `,
      sqlClient`
        WITH bounds AS (
          ...
          SELECT CASE WHEN o.paid_at >= b.current_start THEN 'current' ELSE 'previous' END AS bucket,
                 COUNT(*)::int AS orders,
                 COALESCE(SUM(o.total_minor),0)::float8 AS gross
          FROM commerce.orders o, bounds b
          WHERE o.paid_at >= b.previous_start AND o.paid_at < now()
        ...
        SELECT COALESCE(MAX(orders) FILTER (WHERE bucket='current'),0)::int AS "currentOrders",
               ...
               COALESCE(MAX(gross) FILTER (WHERE bucket='previous'),0)::float8 AS "previousGrossMinor"
        FROM bucketed
      `,
      sqlClient`
        WITH days AS (
          ...
          WHERE o.paid_at >= current_date - (${days - 1})::int
            AND o.status IN ('paid','partially_refunded','refunded')
          GROUP BY 1
        ), refunds AS (
          SELECT r.processed_at::date AS day, COALESCE(SUM(r.amount_minor),0)::float8 AS refunded
          FROM commerce.refunds r
          WHERE r.status='processed' AND r.processed_at >= current_date - (${days - 1})::int
          GROUP BY 1
        )
        SELECT d.day, COALESCE(p.orders,0)::int AS orders, COALESCE(p.gross,0)::float8 AS "grossMinor",
               COALESCE(r.refunded,0)::float8 AS "refundsMinor",
               (COALESCE(p.gross,0)-COALESCE(r.refunded,0))::float8 AS "netMinor"
      ...
      sqlClient`
        ...
        LEFT JOIN commerce.orders o ON o.id=oi.order_id AND o.paid_at >= now()-make_interval(days => ${days}) AND o.status IN ('paid','partially_refunded','refunded')
      ...
      sqlClient`
        SELECT c.id::text AS "couponId", c.code, c.discount_type AS "discountType",
          COUNT(DISTINCT cr.id)::int AS redemptions,
          COUNT(DISTINCT o.user_id)::int AS "students",
          COALESCE(SUM(o.discount_minor),0)::float8 AS "discountMinor",
          COALESCE(SUM(o.total_minor),0)::float8 AS "revenueMinor"
        ...
        LEFT JOIN commerce.orders o ON o.id=cr.order_id
    ...
    res.json({
      ...
      scope: { studentRanks: false, piiExport: false, readOnly: true },
    });
  } catch (error) {
  ...
  try {
    const rows = await sqlClient`
      SELECT o.order_number::text AS "orderNumber", o.paid_at AS "paidAt", o.status, o.currency,
        ...
        o.total_minor::float8 AS "grossMinor",
    ...
    res.setHeader("Content-Disposition", `attachment; filename="business-analytics-${days}d.csv"`);
    res.send(`\uFEFF${lines.join("\n")}`);
  } catch (error) {
