...
} from "../reasoning-v1/topics/Statement-and-Arguments/ARG-001/cp007-exam-profile-generator.ts";
import { ARG_QL_IDS, type ArgQlId } from "../reasoning-v1/topics/Statement-and-Arguments/ARG-001/types.ts";

const router = Router();
const PROFILE_IDS = new Set(Object.keys(ARG_CP007_EXAM_PROFILES));
...
function normalizeProfile(value: unknown): ArgCp007ExamProfile | undefined {
  ...
  return PROFILE_IDS.has(raw) ? raw as ArgCp007ExamProfile : undefined;
}

function normalizeQl(value: unknown): ArgQlId | undefined {
  const raw = asString(value).toUpperCase();
  return (ARG_QL_IDS as readonly string[]).includes(raw) ? raw as ArgQlId : undefined;
}

function defaultDifficulty(profile: ArgCp007ExamProfile): ArgCp007Difficulty {
  return ARG_CP007_EXAM_PROFILES[profile].supportedDifficulties[0];
}

function normalizeDifficulty(profile: ArgCp007ExamProfile, value: unknown): ArgCp007Difficulty {
  const raw = asString(value).toLowerCase();
  const normalized = raw === "easy" ? "Easy" : raw === "hard" ? "Hard" : raw === "medium" || raw === "moderate" ? "Medium" : defaultDifficulty(profile);
  const supported = ARG_CP007_EXAM_PROFILES[profile].supportedDifficulties as readonly string[];
  if (!supported.includes(normalized)) throw new Error(`${profile} does not support ${normalized}. Supported: ${supported.join(", ")}`);
  return normalized;
}
...
function isCp007Request(body: Record<string, unknown>) {
  const profile = normalizeProfile(body.examProfile ?? body.paperProfile ?? body.deliveryProfile);
  const checkpoint = asString(body.cpId).toUpperCase();
  const packageId = asString(body.packageId).toUpperCase();
  const qlId = asString(body.qlId ?? body.canonicalProblemId).toUpperCase();
  return Boolean(profile)
    || checkpoint === ARG_CP007_CHECKPOINT_ID
    || (packageId === "ARG-001" && asString(body.profileMode).toLowerCase() === "real-paper")
    || (qlId.startsWith("ARG-QL-") && asString(body.profileMode).toLowerCase() === "real-paper");
}
...
router.get("/capabilities", requireAdminPermission("content.generation.read"), async (_req, res) => {
  try {
    const packages = listQuestionStudioPackages().map((pkg: any) => {
      ...
      return {
        ...base,
        cpIds: [...new Set([...base.cpIds, ARG_CP007_CHECKPOINT_ID])],
        examProfileCheckpointId: ARG_CP007_CHECKPOINT_ID,
        realPaperParityStatus: "CP007_REVIEW_CONNECTED",
        examProfiles: Object.values(ARG_CP007_EXAM_PROFILES),
      };
    });
    res.json({
    ...
    console.error("ARG-001 CP007 Question Studio capabilities failed", error);
...
router.post("/runs", requireAdminPermission("content.generation.run"), async (req, res, next) => {
  ...
  const count = asPositiveInteger(body.count, 5, 50);
  const language = normalizeLanguage(body.language);
  const seed = asString(body.seed) || undefined;
  const exam = asString(body.exam) || (profile.startsWith("BANKING") ? "Banking" : "SSC / State Exam");
  const runId = randomUUID();
  const code = publicRunCode();
  const timestamp = new Date().toISOString();
  const requestSnapshot = {
    exam,
    subject: asString(body.subject) || "Reasoning Ability",
    topic: asString(body.topic) || "Reasoning",
    subtopic: asString(body.subtopic) || "Statement & Arguments",
    packageId: "ARG-001",
  ...
  try {
    const result = generateArgCp007ExamProfileBatch({
      profile,
      difficulty,
      qlId,
    ...
    const generatedQuestions = result.questions.map((raw) => Object.freeze({
      ...raw,
      ...
      language,
      runtimeMode: "REVIEW_ONLY_CP007_REAL_PAPER_PARITY" as const,
    ...
    await sqlClient.begin(async (tx) => {
      await tx`INSERT INTO content.generation_runs (
        ...
        'examtree', 'reasoning-v1-arg-001-cp007', 0, 0, 0, 0, ${timestamp}, ${timestamp}, ${timestamp}, ${timestamp}
      )`;
      ...
      for (let index = 0; index < generatedQuestions.length; index += 1) {
        ...
        ) throw new Error("ARG-001 CP007 attempted to persist a question with downstream release gates open.");
        ...
        await tx`INSERT INTO content.generation_item_versions (
          ...
          ${versionId}::uuid, ${itemId}::uuid, 1, ${JSON.stringify({ ...question, generationContext: result.generationContext, validationResult: "pending" })},
      ...
      await tx`INSERT INTO platform.audit_events (
        ...
        ${`Generated ${generatedQuestions.length} ${profile} Statement & Arguments questions in ${code}`},
      ...
      await tx`INSERT INTO platform.outbox_events (
        ...
        ${JSON.stringify({ runId, publicCode: code, itemCount: generatedQuestions.length, chapter: "ARG-001", cpId: ARG_CP007_CHECKPOINT_ID, examProfile: profile })}
      )`;
    });
    ...
    console.error("ARG-001 CP007 Question Studio generation failed", error);
