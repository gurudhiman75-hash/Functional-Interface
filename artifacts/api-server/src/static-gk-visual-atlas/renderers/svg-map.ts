...
interface Projector { project(position: Position): ProjectedPoint }
...
function forEachPosition(geometry: GeoJsonAreaGeometry, visit: (position: Position) => void): void {
  ...
  for (const polygon of polygons) for (const ring of polygon) for (const position of ring) visit(position);
}
export function createIndiaMercatorProjector(geometry: IndiaAdminFeatureCollection, viewport: StaticGkSceneViewport): Projector {
  ...
  if (![minX, maxX, minY, maxY].every(Number.isFinite) || maxX <= minX || maxY <= minY) throw new Error("India geometry has an invalid projected extent");
  ...
  const availableWidth = mapRight - mapLeft, availableHeight = mapBottom - mapTop;
  ...
  return { project(position: Position) { const raw = mercatorRaw(position); return { x: raw.x * scale + offsetX, y: offsetY - raw.y * scale }; } };
}
function ringPath(ring: Position[], projector: Projector): string {
  if (!ring.length) return "";
  return `${ring.map((position, index) => { const p = projector.project(position); return `${index === 0 ? "M" : "L"}${p.x.toFixed(2)},${p.y.toFixed(2)}`; }).join(" ")} Z`;
}
...
function linePath(geometry: GeoJsonLineString, projector: Projector): string {
  return geometry.coordinates.map((position, index) => { const p = projector.project(position); return `${index === 0 ? "M" : "L"}${p.x.toFixed(2)},${p.y.toFixed(2)}`; }).join(" ");
}
...
function currentCueText(cues: readonly StaticGkSceneCue[], timeMs: number): string | undefined {
  return [...cues].filter((cue) => cue.startMs <= timeMs && timeMs < cue.endMs && cue.text).sort((a, b) => b.startMs - a.startMs)[0]?.text;
}
function targetName(cue: StaticGkSceneCue | undefined, prefix: string): string | undefined {
  return cue?.targetRef?.startsWith(prefix) ? cue.targetRef.slice(prefix.length) : undefined;
}
function quizCard(quiz: StaticGkSceneQuiz, visible: boolean): string {
  if (!visible) return "";
  return `<g><rect x="90" y="1450" width="900" height="310" rx="34" fill="#FFFFFF" stroke="#CBD5E1" stroke-width="2"/><text x="540" y="1535" text-anchor="middle" font-size="34" font-weight="700" fill="#0F172A">${escapeXml(quiz.question)}</text>${quiz.options.map((option, index) => `<text x="160" y="${1605 + index * 38}" font-size="27" fill="#334155">${String.fromCharCode(65 + index)}. ${escapeXml(option)}</text>`).join("")}</g>`;
}
function svgShell(viewport: StaticGkSceneViewport, title: string, subtitle: string, body: string, overlay: string): string {
...
export function renderTropicCancerSvgFrame(scene: StaticGkMapPathSceneRecipe, geometry: IndiaAdminFeatureCollection, timeMs: number): string {
...
export function renderStandardMeridianSvgFrame(scene: StaticGkMeridianSceneRecipe, geometry: IndiaAdminFeatureCollection, timeMs: number): string {
  ...
  if (!scene.districtOfInterest.meridianSegments.length) throw new Error("Standard Meridian scene has no verified Mirzapur district intersection");
  if (!Number.isFinite(timeMs) || timeMs < 0) throw new Error("Invalid frame timestamp");
  const projector = createIndiaMercatorProjector(geometry, scene.viewport);
  ...
  const meridian = meridianVisible ? scene.meridian.indiaSegments.map((segment) => `<path d="${linePath(segment, projector)}" fill="none" stroke="#B42318" stroke-width="7" stroke-linecap="round"/>`).join("") : "";
  const verifiedDistrictLine = districtVisible ? scene.districtOfInterest.meridianSegments.map((segment) => `<path d="${linePath(segment, projector)}" fill="none" stroke="#7C2D12" stroke-width="12" stroke-linecap="round"/>`).join("") : "";
  const districtLabel = districtVisible ? `<text x="540" y="1540" text-anchor="middle" font-size="54" font-weight="700" fill="#0F172A">Mirzapur district</text>` : "";
  const info = !quizVisible && cueText ? `<text x="540" y="1630" text-anchor="middle" font-size="34" font-weight="600" fill="#334155">${escapeXml(cueText)}</text>` : "";
  return svgShell(scene.viewport, scene.title, scene.meridian.editorialLabel, `${boundaries}${meridian}${verifiedDistrictLine}`, `${districtLabel}${info}${quizCard(scene.quiz, quizVisible)}`);
}
