...
const FRAME_TIMES = [1_000, 3_000, 5_500, 10_000, 16_750, 23_500, 29_000] as const;
...
async function main(): Promise<void> {
  ...
  if (!geometryPath || !receiptPath || !outputDirectory) {
    throw new Error(
      "Usage: export-tropic-contact-sheet <canonical-admin.geojson> <ingest-receipt.json> <output-directory>",
    );
  }
  ...
  for (const timeMs of FRAME_TIMES) {
    ...
    const svg = renderTropicCancerSvgFrame(scene, geometry, timeMs);
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(scene.title)} · contact sheet</title><style>body{font-family:system-ui,sans-serif;margin:0;padding:24px;background:#f1f5f9;color:#0f172a}header{max-width:1100px;margin:0 auto 24px}h1{margin:0 0 8px;font-size:28px}p{margin:0;color:#475569}.grid{max-width:1100px;margin:0 auto;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:18px}figure{margin:0;background:white;border:1px solid #cbd5e1;border-radius:16px;padding:12px}img{display:block;width:100%;height:auto;border-radius:10px;background:white}figcaption{text-align:center;padding-top:10px;font-weight:700}</style></head><body><header><h1>${escapeHtml(scene.title)}</h1><p>Visual QA contact sheet · ${escapeHtml(receipt.sourceProductCode)} · ${escapeHtml(basename(geometryPath))}</p></header><main class="grid">${cards.join("")}</main></body></html>`;
...
main().catch((error: unknown) => {
  ...
  process.stderr.write(`[static-gk-visual-atlas] contact-sheet export failed: ${message}\n`);
  process.exitCode = 1;
});
