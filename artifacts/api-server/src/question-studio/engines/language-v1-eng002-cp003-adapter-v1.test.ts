...
assert.ok(eng002);
assert.deepEqual(eng002.cpIds, ["ENG-002-CP001", "ENG-002-CP002", "ENG-002-CP003"]);
...
for (const difficulty of ["Easy", "Medium", "Hard"] as const) {
  const result = await languageV1QuestionStudioAdapter.generate({ ...baseRequest, difficulty, count: 5, seed: `eng002:cp003:${difficulty}:adapter-test` });
  assert.equal(result.questions.length, 5);
  for (const question of result.questions) {
    assert.equal(question.cpId, "ENG-002-CP003");
...
const filtered = await languageV1QuestionStudioAdapter.generate({ ...baseRequest, difficulty: "Easy", canonicalProblemId: "GR-ART-002", count: 3, seed: "eng002:cp003:rule-filter" });
assert.equal(filtered.questions.every((question) => question.ruleId === "GR-ART-002"), true);
...
await assert.rejects(languageV1QuestionStudioAdapter.generate({ ...baseRequest, difficulty: "Easy", canonicalProblemId: "GR-ART-007", count: 1 }), /does not support easy/i);

console.log("ENG-002 CP003 Question Studio review-only adapter tests passed.");
