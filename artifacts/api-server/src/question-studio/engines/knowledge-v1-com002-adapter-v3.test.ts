...
import {
  COM002_STANDARD_BANK_ONLY_PACKAGE_V3,
...
assert.equal(COM002_STANDARD_BANK_ONLY_PACKAGE_V3.enabled, true);
assert.deepEqual(COM002_STANDARD_BANK_ONLY_PACKAGE_V3.supportedLanguages, ["en", "hi", "pa"]);
assert.equal(COM002_STANDARD_BANK_ONLY_PACKAGE_V3.lifecycleStage, "BANK_ONLY");
assert.equal(COM002_STANDARD_BANK_ONLY_PACKAGE_V3.questionBankWritable, true);
assert.equal(COM002_STANDARD_BANK_ONLY_PACKAGE_V3.testEligible, false);
assert.equal(COM002_STANDARD_BANK_ONLY_PACKAGE_V3.mockTestEligible, false);
assert.equal(COM002_STANDARD_BANK_ONLY_PACKAGE_V3.publiclyPublishable, false);
assert.equal(COM002_STANDARD_BANK_ONLY_PACKAGE_V3.metadata?.operationalFreezeAuthorityId, COM002_V6_V5_OPERATIONAL_FREEZE.authorityId);
...
for (const qlId of qlIds) {
  for (const language of languages) {
    for (let index = 0; index < 10; index += 1) {
      ...
      assert.equal(question.canonicalQuestionPersistenceAllowed, true);
...
for (const language of languages) {
  ...
  assert.equal(batch.generationContext.stage, "BANK_ONLY");
  assert.equal(batch.generationContext.reviewRunPersistenceAllowed, true);
  assert.equal(batch.generationContext.canonicalQuestionPersistenceAllowed, true);
...
await assert.rejects(
  () => knowledgeV1Com002QuestionStudioAdapterV3.generate({
    packageId: "COM-002",
    runtimeMode: "review-only",
    language: "en",
    difficulty: "Hard",
...
console.log(`[COM002-V6-V5-QUESTION-STUDIO-ADAPTER-V3] PASS audited=${audited} lifecycle=BANK_ONLY persistence=true bank=true downstream=false`);
