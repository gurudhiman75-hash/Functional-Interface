...
const baseRequest = {
  ...
  subtopic: "Gerunds, Infinitives and Participles",
  language: "en" as const,
  runtimeMode: "review-only",
  canonicalProblemId: "ENG-002-CP009",
};
...
for (const difficulty of ["Easy", "Medium", "Hard"] as const) {
  const result = await languageV1QuestionStudioAdapter.generate({ ...baseRequest, difficulty, count: 5, seed: `eng002:cp009:${difficulty}:adapter-test` });
  assert.equal(result.questions.length, 5);
  for (const question of result.questions) {
    assert.equal(question.cpId, "ENG-002-CP009");
...
const filtered = await languageV1QuestionStudioAdapter.generate({ ...baseRequest, difficulty: "Easy", canonicalProblemId: "GR-GIP-001", count: 3, seed: "eng002:cp009:rule-filter" });
assert.equal(filtered.questions.every((question) => question.ruleId === "GR-GIP-001"), true);
...
console.log("ENG-002 CP009 Question Studio review-only adapter tests passed.");
