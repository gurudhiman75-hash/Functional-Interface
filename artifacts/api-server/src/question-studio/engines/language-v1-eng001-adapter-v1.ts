...
import type { Eng001QlId, EnglishDifficulty, GrammarRuleId } from "../../english-v1/core/types";
import type {
...
export const ENG001_QUESTION_STUDIO_CP_ID_V1 = "ENG-001-CP001" as const;
...
function normalizeSelectors(request: QuestionStudioGenerationRequest) {
  ...
  const unknown = candidates.filter(
    (value) =>
      ...
      value !== ENG001_QUESTION_STUDIO_CP_ID_V1 &&
...
export const ENG001_STANDARD_REVIEW_ONLY_PACKAGE_V1: QuestionStudioPackageDefinition = {
  ...
  label: "English · Error Spotting · Subject–Verb Agreement · CP001 V4",
  enabled: true,
  cpIds: [ENG001_QUESTION_STUDIO_CP_ID_V1],
  supportedLanguages,
  ...
  metadata: {
    ...lifecycle,
    registrationAuthorityId: ENG001_CP001_HUMAN_EDITORIAL_APPROVAL_V1.authorityId,
    approvedReviewBlobSha: ENG001_CP001_HUMAN_EDITORIAL_APPROVAL_V1.approvedReviewBlobSha,
    approvedGeneratorHeadSha: ENG001_CP001_HUMAN_EDITORIAL_APPROVAL_V1.approvedGeneratorHeadSha,
...
export function isEng001QuestionStudioRequestV1(request: QuestionStudioGenerationRequest) {
  ...
  const selectors = [request.patternId, request.canonicalProblemId, request.questionLanguageId]
    .map((value) => text(value).toUpperCase());
  return (
    selectors.some((value) => value.startsWith("ENG-001") || value.startsWith("GR-SVA-")) ||
    (subject === "english" && topic === "error spotting" && subtopic === "subject–verb agreement") ||
    (subject === "english" && topic === "error spotting" && subtopic === "subject-verb agreement")
  );
}
...
export const languageV1Eng001QuestionStudioAdapterV1: QuestionStudioEngineAdapter = {
  ...
  async generate(request: QuestionStudioGenerationRequest): Promise<QuestionStudioGenerationResult> {
    ...
    for (let index = 0; index < count; index += 1) {
      let accepted: ReturnType<typeof generateEng001Cp001QuestionV4> | null = null;
      let acceptedSeed = "";
      for (let attempt = 0; attempt < 2_000; attempt += 1) {
        const seed = `${baseSeed}:${index}:${attempt}`;
        const generated = generateEng001Cp001QuestionV4({ seed, difficulty, qlId, ruleId });
      ...
      if (!accepted) throw new Error(`ENG-001 could not produce ${count} distinct review questions`);
      ...
      questions.push({
        ...
        cpId: ENG001_QUESTION_STUDIO_CP_ID_V1,
    ...
    return {
      ...
      generationContext: {
        ...
        ruleSelection: ruleId ?? "DETERMINISTIC_ACROSS_APPROVED_SVA_RULES",
        permanentQlIds: [...qlIds],
        grammarRuleIds: [...ruleIds],
        seed: baseSeed,
        count,
