...
import {
  GEO_VEG_001_CONTENT_AUTHORITY_VERSION_V1,
  GEO_VEG_001_QUESTION_STUDIO_CORPUS_V1,
  GEO_VEG_001_STANDARD_REVIEW_ONLY_PACKAGE_V1,
...
assert.equal(audit.qlCoverage, 108);
...
const packageDef = GEO_VEG_001_STANDARD_REVIEW_ONLY_PACKAGE_V1;
assert.equal(packageDef.packageId, "GEO-VEG-001");
assert.equal(packageDef.subject, "Static GK");
assert.equal(packageDef.topic, "Indian Geography");
assert.equal(packageDef.subtopic, "Natural Vegetation & Wildlife of India");
...
assert.equal(GEO_VEG_001_CONTENT_AUTHORITY_VERSION_V1, "GEO-VEG-001-CONTENT-CLOSED-V1");

assert.equal(GEO_VEG_001_QUESTION_STUDIO_CORPUS_V1.length, 648);
const qlCounts = new Map<string, number>();
const cpCounts = new Map<string, number>();
for (const question of GEO_VEG_001_QUESTION_STUDIO_CORPUS_V1) {
...
for (let ql = 1; ql <= 108; ql += 1) {
  assert.equal(qlCounts.get(`GEO-VEG-001-QL-${String(ql).padStart(3, "0")}`), 6);
}
for (let cp = 1; cp <= 12; cp += 1) {
  assert.equal(cpCounts.get(`GEO-VEG-001-CP${String(cp).padStart(3, "0")}`), 54);
}

assert.equal(isGeoVeg001QuestionStudioRequestV1({ packageId: "GEO-VEG-001" }), true);
assert.equal(
  isGeoVeg001QuestionStudioRequestV1({ subject: "Static GK", topic: "Indian Geography", subtopic: "Natural Vegetation & Wildlife of India" }),
  true,
);
assert.equal(isGeoVeg001QuestionStudioRequestV1({ packageId: "GEO-RIV-001" }), false);

const packages = knowledgeV1QuestionStudioAdapter.listPackages();
assert.equal(packages.some((pkg) => pkg.packageId === "GEO-VEG-001"), true);
assert.equal(new Set(packages.map((pkg) => pkg.packageId)).size, packages.length);

const baseRequest = {
  packageId: "GEO-VEG-001",
  language: "en" as const,
  count: 12,
  seed: "geo-veg-001-question-studio-test",
};
const first = await knowledgeV1GeoVeg001QuestionStudioAdapterV1.generate(baseRequest);
const replay = await knowledgeV1GeoVeg001QuestionStudioAdapterV1.generate(baseRequest);
...
assert.equal(composite.questions.every((q) => q.packageId === "GEO-VEG-001"), true);

const cpFiltered = await knowledgeV1GeoVeg001QuestionStudioAdapterV1.generate({
  ...baseRequest,
  canonicalProblemId: "GEO-VEG-001-CP012",
  count: 4,
});
assert.equal(cpFiltered.questions.every((q) => q.cpId === "GEO-VEG-001-CP012"), true);

const qlFiltered = await knowledgeV1GeoVeg001QuestionStudioAdapterV1.generate({
  ...baseRequest,
  patternId: "GEO-VEG-001-QL-108",
  count: 2,
});
assert.equal(qlFiltered.questions.every((q) => q.qlId === "GEO-VEG-001-QL-108"), true);
assert.equal(qlFiltered.questions.every((q) => q.cpId === "GEO-VEG-001-CP012"), true);

const hard = await knowledgeV1GeoVeg001QuestionStudioAdapterV1.generate({
  ...baseRequest,
  difficulty: "Hard",
  count: 10,
});
...
await assert.rejects(
  knowledgeV1GeoVeg001QuestionStudioAdapterV1.generate({ ...baseRequest, canonicalProblemId: "GEO-VEG-001-CP013" }),
  /Unknown GEO-VEG-001 selector/i,
);
await assert.rejects(
  knowledgeV1GeoVeg001QuestionStudioAdapterV1.generate({ ...baseRequest, language: "hi" }),
