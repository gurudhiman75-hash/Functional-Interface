...
assert.ok(eng002);
assert.deepEqual(eng002.cpIds, ["ENG-002-CP001", "ENG-002-CP002", "ENG-002-CP003", "ENG-002-CP004"]);
...
for (const difficulty of ["Easy", "Medium", "Hard"] as const) {
  const result = await languageV1QuestionStudioAdapter.generate({ ...baseRequest, difficulty, count: 5, seed: `eng002:cp004:${difficulty}:adapter-test` });
  assert.equal(result.questions.length, 5);
  for (const question of result.questions) {
    assert.equal(question.cpId, "ENG-002-CP004");
...
const filtered = await languageV1QuestionStudioAdapter.generate({ ...baseRequest, difficulty: "Easy", canonicalProblemId: "GR-PRN-001", count: 3, seed: "eng002:cp004:rule-filter" });
assert.equal(filtered.questions.every((question) => question.ruleId === "GR-PRN-001"), true);
...
console.log("ENG-002 CP004 Question Studio review-only adapter tests passed.");
