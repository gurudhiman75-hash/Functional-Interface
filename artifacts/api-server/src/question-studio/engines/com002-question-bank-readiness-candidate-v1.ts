...
import { generateCom002CandidateBatchForAuditV1 } from "./knowledge-v1-com002-candidate-adapter";

export const COM002_REQUIRED_PREBANK_PROVENANCE_FIELDS_V1 = [
  "sourceIds",
  "sourceFactIds",
  "solverAuthority",
...
export type Com002QuestionBankReadinessCandidateV1 = {
  status: "PREBANK_PROVENANCE_READY_BUT_ACTIVATION_BLOCKED" | "BLOCKED_METADATA_PROVENANCE";
...
export async function auditCom002QuestionBankReadinessCandidateV1(): Promise<Com002QuestionBankReadinessCandidateV1> {
  ...
  for (const qlId of qlIds) {
    for (const language of languages) {
      const generated = await generateCom002CandidateBatchForAuditV1({
        ...
        difficulty: "Mixed",
        count: 10,
        seed: `com002-prebank-readiness-v1:${qlId}:${language}`,
      });
      ...
      for (const question of generated.questions) {
        ...
        for (const field of COM002_REQUIRED_PREBANK_PROVENANCE_FIELDS_V1) {
  ...
  return {
    status:
      ...
      allCandidatesRejectedByBankGate &&
      downstreamLifecycleLocked
        ? "PREBANK_PROVENANCE_READY_BUT_ACTIVATION_BLOCKED"
