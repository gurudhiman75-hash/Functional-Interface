...
import { PGK_001_CP016_FACTS } from "./pgk-001-cp016-facts";
import {
  PGK_001_CP016_REVIEW_BATCH_V1,
  auditPgk001Cp016ReviewBatchV1,
} from "./pgk-001-cp016-review-batch-v1";

describe("PGK-001 CP016 review candidate v1", () => {
  it("passes deterministic review guards", () => {
    const audit = auditPgk001Cp016ReviewBatchV1();
    expect(audit.valid).toBe(true);
    expect(audit.issues).toEqual([]);
  });

  it("contains 42 questions across seven QLs", () => {
    expect(PGK_001_CP016_REVIEW_BATCH_V1).toHaveLength(42);
    expect(new Set(PGK_001_CP016_REVIEW_BATCH_V1.map((question) => question.qlId)).size).toBe(7);
  });

  it("keeps the accepted exam-grade wording standard", () => {
    const learner = PGK_001_CP016_REVIEW_BATCH_V1
  ...
  it("uses registered facts and remains review-only", () => {
    const facts = new Set(PGK_001_CP016_FACTS.map((fact) => fact.id));
    for (const question of PGK_001_CP016_REVIEW_BATCH_V1) {
