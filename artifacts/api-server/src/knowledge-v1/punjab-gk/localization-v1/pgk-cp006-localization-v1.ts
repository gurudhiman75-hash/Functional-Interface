...
type EnglishQuestion = (typeof PGK_001_CP006_REVIEW_BATCH_V2)[number];
type NativeLocale = Exclude<PgkLocaleV1, "en">;

export type PgkCp006LocalizedQuestionV1 = {
  questionId: string;
  qlId: string;
  qlName: string;
...
function overlayFor(index: number, locale: NativeLocale): PgkNativeOverlayV1 {
  ...
  const overlay = locale === "hi" ? PGK_CP006_HI_V1[number] : PGK_CP006_PA_V1[number];
  if (!overlay) throw new Error(`PGK-001 CP006 #${number}: missing ${locale} localization overlay`);
  return overlay;
}
...
export function generatePgkCp006LocalizedReviewV1(locale: PgkLocaleV1): PgkCp006LocalizedQuestionV1[] {
  return PGK_001_CP006_REVIEW_BATCH_V2.map((q, index) => {
...
export const PGK_CP006_LOCALIZATION_V1_SUPPORTED_LOCALES = ["en", "hi", "pa"] as const;
export const PGK_CP006_LOCALIZATION_V1_SUPPORTED_CPS = ["PGK-001-CP-006"] as const;
