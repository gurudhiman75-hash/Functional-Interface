...
type EnglishQuestion = (typeof PGK_001_CP007_REVIEW_BATCH_V2)[number];
type NativeLocale = Exclude<PgkLocaleV1, "en">;

export type PgkCp007LocalizedQuestionV1 = {
  questionId: string;
  qlId: string;
  qlName: string;
...
function overlayFor(index: number, locale: NativeLocale): PgkNativeOverlayV1 {
  ...
  const overlay = locale === "hi" ? PGK_CP007_HI_V1[number] : PGK_CP007_PA_V1[number];
  if (!overlay) throw new Error(`PGK-001 CP007 #${number}: missing ${locale} localization overlay`);
  return overlay;
}
...
export function generatePgkCp007LocalizedReviewV1(locale: PgkLocaleV1): PgkCp007LocalizedQuestionV1[] {
  return PGK_001_CP007_REVIEW_BATCH_V2.map((q, index) => {
...
export const PGK_CP007_LOCALIZATION_V1_SUPPORTED_LOCALES = ["en", "hi", "pa"] as const;
export const PGK_CP007_LOCALIZATION_V1_SUPPORTED_CPS = ["PGK-001-CP-007"] as const;
