...
function assertNative(
  q: ReturnType<typeof generatePgkCp003LocalizedReviewV1>[number],
  locale: "hi" | "pa",
  english: (typeof PGK_001_CP003_REVIEW_BATCH_V2)[number],
) {
  ...
  fail(q.explanation.length >= 30, `${q.questionId}: localized explanation too short`);
  fail(q.explanation.length <= 440, `${q.questionId}: localized explanation too long`);
  ...
  fail(
    ...
    `${q.questionId}: numeric/elevation parity mismatch :: ${JSON.stringify(numbers(learnerText(english)))} vs ${JSON.stringify(numbers(text))}`,
  );
}

function assertParity(locale: PgkLocaleV1) {
  const localized = generatePgkCp003LocalizedReviewV1(locale);
  fail(localized.length === 42, `${locale}: expected 42 questions`);

  localized.forEach((q, index) => {
    const english = PGK_001_CP003_REVIEW_BATCH_V2[index]!;
...
fail(PGK_001_CP003_REVIEW_BATCH_V2.length === 42, "CP003 English V2 corpus must contain 42 questions");
fail(Object.keys(PGK_CP003_HI_V1).length === 42, "CP003 Hindi must contain 42 overlays");
fail(Object.keys(PGK_CP003_PA_V1).length === 42, "CP003 Punjabi must contain 42 overlays");

for (const locale of locales) assertParity(locale);

const all = locales.flatMap(generatePgkCp003LocalizedReviewV1);
...
console.log("PGK-001 CP003 multilingual audit passed: 42 questions per locale / 126 EN-HI-PA surfaces.");
