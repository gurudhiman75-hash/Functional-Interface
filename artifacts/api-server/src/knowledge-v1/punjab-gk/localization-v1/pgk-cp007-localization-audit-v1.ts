import { PGK_001_CP007_REVIEW_BATCH_V2 } from "../pgk-001-cp007-review-batch-v2";
import { PGK_CP007_HI_V1 } from "./pgk-cp007-hi-v1";
import { PGK_CP007_PA_V1 } from "./pgk-cp007-pa-v1";
...
function assertNative(
  q: ReturnType<typeof generatePgkCp007LocalizedReviewV1>[number],
  locale: "hi" | "pa",
  english: (typeof PGK_001_CP007_REVIEW_BATCH_V2)[number],
) {
...
function assertParity(locale: PgkLocaleV1) {
  const localized = generatePgkCp007LocalizedReviewV1(locale);
  fail(localized.length === 42, `${locale}: expected 42 questions`);

  localized.forEach((q, index) => {
    const english = PGK_001_CP007_REVIEW_BATCH_V2[index]!;
...
fail(PGK_001_CP007_REVIEW_BATCH_V2.length === 42, "CP007 English corpus must contain 42 questions");
fail(Object.keys(PGK_CP007_HI_V1).length === 42, "CP007 Hindi must contain 42 overlays");
fail(Object.keys(PGK_CP007_PA_V1).length === 42, "CP007 Punjabi must contain 42 overlays");

for (const locale of locales) assertParity(locale);

const all = locales.flatMap(generatePgkCp007LocalizedReviewV1);
...
console.log("PGK-001 CP007 multilingual audit passed: 42 questions per locale / 126 EN-HI-PA surfaces.");
