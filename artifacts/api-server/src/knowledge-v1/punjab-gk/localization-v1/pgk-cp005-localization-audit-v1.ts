...
function assertNative(
  q: ReturnType<typeof generatePgkCp005LocalizedReviewV1>[number],
  locale: "hi" | "pa",
  english: (typeof PGK_001_CP005_REVIEW_BATCH_V1)[number],
) {
...
function assertParity(locale: PgkLocaleV1) {
  const localized = generatePgkCp005LocalizedReviewV1(locale);
  fail(localized.length === 42, `${locale}: expected 42 questions`);

  localized.forEach((q, index) => {
    const english = PGK_001_CP005_REVIEW_BATCH_V1[index]!;
...
fail(PGK_001_CP005_REVIEW_BATCH_V1.length === 42, "CP005 English corpus must contain 42 questions");
fail(Object.keys(PGK_CP005_HI_V1).length === 42, "CP005 Hindi must contain 42 overlays");
fail(Object.keys(PGK_CP005_PA_V1).length === 42, "CP005 Punjabi must contain 42 overlays");

for (const locale of locales) assertParity(locale);

const all = locales.flatMap(generatePgkCp005LocalizedReviewV1);
...
console.log("PGK-001 CP005 multilingual audit passed: 42 questions per locale / 126 EN-HI-PA surfaces.");
