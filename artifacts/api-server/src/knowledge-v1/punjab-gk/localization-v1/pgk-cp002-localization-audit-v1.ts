import { PGK_001_CP002_REVIEW_BATCH_V3 } from "../pgk-001-cp002-review-batch-v3";
import { PGK_CP002_HI_V1 } from "./pgk-cp002-hi-v1";
import { PGK_CP002_PA_V1 } from "./pgk-cp002-pa-v1";
...
function assertNative(
  q: ReturnType<typeof generatePgkCp002LocalizedReviewV1>[number],
  locale: "hi" | "pa",
  english: (typeof PGK_001_CP002_REVIEW_BATCH_V3)[number],
) {
  ...
  fail(q.stem.length <= 420, `${q.questionId}: localized stem too long`);
  fail(q.explanation.length >= 35, `${q.questionId}: localized explanation too short`);
  ...
  fail(
    ...
    `${q.questionId}: numeric/date parity mismatch`,
  );
}

function assertParity(locale: PgkLocaleV1) {
  const localized = generatePgkCp002LocalizedReviewV1(locale);
  fail(localized.length === 42, `${locale}: expected 42 questions`);

  localized.forEach((q, index) => {
    const english = PGK_001_CP002_REVIEW_BATCH_V3[index]!;
...
fail(PGK_001_CP002_REVIEW_BATCH_V3.length === 42, "CP002 English V3 corpus must contain 42 questions");
fail(Object.keys(PGK_CP002_HI_V1).length === 42, "CP002 Hindi must contain 42 overlays");
fail(Object.keys(PGK_CP002_PA_V1).length === 42, "CP002 Punjabi must contain 42 overlays");

for (const locale of locales) assertParity(locale);

const all = locales.flatMap(generatePgkCp002LocalizedReviewV1);
...
console.log("PGK-001 CP002 multilingual audit passed: 42 questions per locale / 126 EN-HI-PA surfaces.");
