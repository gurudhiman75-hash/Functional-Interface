...
const ls:PgkLocaleV1[]=["en","hi","pa"],L={en:"English",hi:"Hindi",pa:"Punjabi"} as const,c=Object.fromEntries(ls.map(l=>[l,generatePgkCp018LocalizedReviewV1(l)])) as Record<PgkLocaleV1,ReturnType<typeof generatePgkCp018LocalizedReviewV1>>,a:string[]=["# PGK-001 CP018 — Multilingual Review V1","","Status: REVIEW-READY / AWAITING HUMAN APPROVAL","Scope: 42 questions × English/Hindi/Punjabi = 126 review surfaces",""];
...
const d=path.resolve(process.cwd(),"dist/pgk-review/PGK-MULTILINGUAL-V1");fs.mkdirSync(d,{recursive:true});const p=path.join(d,"PGK-MULTILINGUAL-V1-CP018-REVIEW.md");fs.writeFileSync(p,a.join("\n")+"\n");console.log(p);
