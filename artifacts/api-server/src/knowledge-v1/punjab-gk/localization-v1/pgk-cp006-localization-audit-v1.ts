import { PGK_001_CP006_REVIEW_BATCH_V2 } from "../pgk-001-cp006-review-batch-v2";
import { PGK_CP006_HI_V1 } from "./pgk-cp006-hi-v1";
import { PGK_CP006_PA_V1 } from "./pgk-cp006-pa-v1";
...
function assertNative(
  q: ReturnType<typeof generatePgkCp006LocalizedReviewV1>[number],
  locale: "hi" | "pa",
  english: (typeof PGK_001_CP006_REVIEW_BATCH_V2)[number],
) {
  ...
  fail(q.stem.length <= 460, `${q.questionId}: localized stem too long`);
  fail(q.explanation.length >= 25, `${q.questionId}: localized explanation too short`);
  fail(q.explanation.length <= 480, `${q.questionId}: localized explanation too long`);
  ...
  fail(
    ...
    `${q.questionId}: numeric/date parity mismatch :: ${JSON.stringify(numbers(learnerText(english)))} vs ${JSON.stringify(numbers(text))}`,
  );
}

function assertParity(locale: PgkLocaleV1) {
  const localized = generatePgkCp006LocalizedReviewV1(locale);
  fail(localized.length === 42, `${locale}: expected 42 questions`);

  localized.forEach((q, index) => {
    const english = PGK_001_CP006_REVIEW_BATCH_V2[index]!;
...
fail(PGK_001_CP006_REVIEW_BATCH_V2.length === 42, "CP006 English corpus must contain 42 questions");
fail(Object.keys(PGK_CP006_HI_V1).length === 42, "CP006 Hindi must contain 42 overlays");
fail(Object.keys(PGK_CP006_PA_V1).length === 42, "CP006 Punjabi must contain 42 overlays");

for (const locale of locales) assertParity(locale);

const all = locales.flatMap(generatePgkCp006LocalizedReviewV1);
...
console.log("PGK-001 CP006 multilingual audit passed: 42 questions per locale / 126 EN-HI-PA surfaces.");
