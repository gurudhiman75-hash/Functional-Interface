...
type EnglishQuestion = (typeof PGK_001_CP002_REVIEW_BATCH_V3)[number];
type NativeLocale = Exclude<PgkLocaleV1, "en">;

export type PgkCp002LocalizedQuestionV1 = {
  questionId: string;
  qlId: string;
  qlName: string;
...
function overlayFor(index: number, locale: NativeLocale): PgkNativeOverlayV1 {
  ...
  const overlay = locale === "hi" ? PGK_CP002_HI_V1[number] : PGK_CP002_PA_V1[number];
  if (!overlay) throw new Error(`PGK-001 CP002 #${number}: missing ${locale} localization overlay`);
  return overlay;
}
...
export function generatePgkCp002LocalizedReviewV1(locale: PgkLocaleV1): PgkCp002LocalizedQuestionV1[] {
  return PGK_001_CP002_REVIEW_BATCH_V3.map((q, index) => {
...
export const PGK_CP002_LOCALIZATION_V1_SUPPORTED_LOCALES = ["en", "hi", "pa"] as const;
export const PGK_CP002_LOCALIZATION_V1_SUPPORTED_CPS = ["PGK-001-CP-002"] as const;
