...
import {
  PGK_001_CP006_REVIEW_BATCH_V2,
  auditPgk001Cp006ReviewBatchV2,
} from "./pgk-001-cp006-review-batch-v2";
import {
...
const audit = auditPgk001Cp006ReviewBatchV2();
assert.equal(audit.valid, true, audit.issues.join(" | "));
assert.equal(PGK_001_CP006_REVIEW_BATCH_V2.length, 42);
assert.equal(Object.keys(PGK_001_CP006_QL_NAMES).length, 7);

for (const qlId of Object.keys(PGK_001_CP006_QL_NAMES)) {
  assert.equal(
    PGK_001_CP006_REVIEW_BATCH_V2.filter((question) => question.qlId === qlId).length,
    6,
...
for (const id of ["soil-erosion", "salt-affected-soils", "waterlogging", "groundwater-overuse", "brackish-groundwater"]) assert.ok(problemIds.has(id));

const conservationIds = new Set(PGK_001_CP006_CONSERVATION.map((row) => row.id));
for (const id of ["land-levelling", "field-drainage", "rainwater-harvesting", "watershed-treatment", "drip-irrigation", "contour-bunding"]) assert.ok(conservationIds.has(id));

const learnerText = PGK_001_CP006_REVIEW_BATCH_V2
