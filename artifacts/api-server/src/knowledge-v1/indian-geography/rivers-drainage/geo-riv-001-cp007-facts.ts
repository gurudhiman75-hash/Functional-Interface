...
export const GEO_RIV_001_CP007_HELD_UPSTREAM_CP_IDS = Object.freeze([
  "GEO-RIV-001-CP006",
] as const);
...
function cloneValue(value: KnowledgeFact["value"]): KnowledgeFact["value"] {
  switch (value.kind) {
    ...
    case "entity_ref":
      return { kind: "entity_ref", entityId: value.entityId, label: { ...value.label } };
    case "number":
      return { kind: "number", value: value.value, ...(value.unit ? { unit: value.unit } : {}) };
    case "date":
...
function upstreamToken(cpId: string) {
  return cpId.replace("GEO-RIV-001-", "").toLowerCase();
}

function sourceSuffix(factId: string) {
  return factId.replace(/^geo-riv-001-cp\d+-/, "");
}
...
function projectFact(fact: KnowledgeFact, sourceCpId: string): KnowledgeFact {
  ...
  return {
    ...
    entity: {
      ...fact.entity,
      label: { ...fact.entity.label },
      ...(fact.entity.aliases ? { aliases: fact.entity.aliases } : {}),
    },
    value: cloneValue(fact.value),
    contextGroupId: `geo-riv-001-cp007-${fact.contextGroupId}`,
    distractorGroupIds: fact.distractorGroupIds
      ? fact.distractorGroupIds.map((group) => `geo-riv-001-cp007-${group}`)
      : undefined,
    ...
    review: {
      ...
      confidence: Math.min(fact.review.confidence, 0.98),
    },
    freshness: {
      ...fact.freshness,
      class: "IMMUTABLE",
...
export function geoRiv001Cp007FactText(fact: KnowledgeFact) {
  const value = fact.value.kind === "entity_ref"
    ? fact.value.label.en
    : fact.value.kind === "text"
      ? fact.value.text.en
      : fact.value.kind === "number"
        ? `${fact.value.value}${fact.value.unit ? ` ${fact.value.unit}` : ""}`
