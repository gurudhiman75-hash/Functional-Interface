...
function q109(seed:string){ const selected=deterministicShuffle(ROWS.filter((r)=>r.city!=="Prayagraj"),`${seed}:rows`).slice(0,3); const correct=selected.map((r)=>pair(r.city,r.river)).join("; "); const alternatives=[correct,...[1,2,3].map((shift)=>selected.map((r,i)=>pair(r.city,selected[(i+shift)%selected.length].river)).join("; "))]; return build({qlId:GEO_RIV_001_CP012_QL_IDS_V1[8],seed,stem:"Which option correctly matches all three cities with their rivers?",answer:correct,pool:alternatives,explanation:selected.map((r)=>`${r.city} — ${displayRiver(r.river)}`).join("; ")+".",facts:selected.flatMap((r)=>cityFacts(r.city)),difficulty:"Hard",solver:"THREE_PAIR_MATCH_VERIFIER"}); }
