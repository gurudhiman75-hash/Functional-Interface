...
function q106(seed:string){ const group=deterministicPick(SHARED_RIVER_GROUPS,`${seed}:g`); const truePairs=cityPairs(group.cities); const answer=deterministicPick(truePairs,`${seed}:pair`); const wrong:string[]=[]; for(let i=0;i<CITIES.length;i+=1){ for(let j=i+1;j<CITIES.length;j+=1){ const candidate=`${CITIES[i]} and ${CITIES[j]}`; if(truePairs.includes(candidate)) continue; wrong.push(candidate); }} return build({qlId:GEO_RIV_001_CP012_QL_IDS_V1[5],seed,stem:`Which pair of cities is correctly associated with ${displayRiver(group.river)}?`,answer,pool:wrong,explanation:`Both cities in ${answer} are associated with ${displayRiver(group.river)}.`,facts:group.cities.flatMap(cityFacts),difficulty:"Medium",solver:"SHARED_RIVER_CITY_PAIR"}); }
