...
const lines: string[] = [
  ...
  "",
  `**Questions:** ${audit.polishedQuestionCount}  `,
  `**Semantic unique:** ${audit.polishedSemanticUniqueCount}  `,
  `**Difficulty:** Easy ${audit.difficultyCounts.Easy} · Medium ${audit.difficultyCounts.Medium} · Hard ${audit.difficultyCounts.Hard}  `,
  `**Answer positions:** A ${audit.polishedAnswerPositions[0]} · B ${audit.polishedAnswerPositions[1]} · C ${audit.polishedAnswerPositions[2]} · D ${audit.polishedAnswerPositions[3]}  `,
...
let currentQl = "";
GEO_RIV_001_CP007_REVIEW_BATCH_V3_POLISHED.forEach((q, index) => {
...
fs.writeFileSync(path.join(outDir, `${baseName}.json`), JSON.stringify({ audit, questions: GEO_RIV_001_CP007_REVIEW_BATCH_V3_POLISHED }, null, 2));
