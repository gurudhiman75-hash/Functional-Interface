...
function build(args: {
  ...
  return {
    questionId: `GEO-RIV-001-CP008-V1-${args.qlId}-${args.seed}`,
...
const GEO_PHYSICAL_VALUES = unique(SOURCE_GEO_FACTS.filter((fact) => fact.relation === "source_region" || fact.relation === "source_range").map(valueText));
...
function sourceClause(fact: KnowledgeFact, value = valueText(fact)) {
  ...
  if (fact.relation === "originates_near") return `originates near ${value}`;
  ...
  if (fact.relation === "source_region") return `rises in ${value}`;
...
function factSentence(fact: KnowledgeFact, value = valueText(fact)) {
  ...
  if (fact.relation === "has_mouth_type") return `${river} has a ${value.toLowerCase()} at its mouth.`;
...
function pairText(fact: KnowledgeFact, value = valueText(fact)) {
  ...
  if (fact.relation === "originates_near") return `${river} — originates near ${value}`;
...
function valuePoolFor(fact: KnowledgeFact) {
  ...
  if (fact.relation === "source_region" || fact.relation === "source_range") return GEO_PHYSICAL_VALUES;
...
function ql066(seed: string) {
  ...
  const value = valueText(fact);
  const relationPhrase = fact.relation === "originates_near" ? `originates near ${value}` : fact.relation === "originates_at" ? `originates at ${value}` : fact.relation === "source_point" ? `has its source point at ${value}` : `originates from ${value}`;
  return build({ qlId: "GEO-RIV-001-QL-066", seed, stem: `Which river ${relationPhrase}?`, answer: fact.entity.label.en, optionPool: SOURCE_RIVERS, explanation: factSentence(fact), facts: [fact], difficulty: "Easy", solverAuthority: "CANONICAL_SOURCE_RELATION" });
}
...
function ql067(seed: string) {
  ...
  const river = fact.entity.label.en;
  const stems: Record<string, string> = {
    source_region: `The ${river} rises in which source region?`,
  ...
  return build({ qlId: "GEO-RIV-001-QL-067", seed, stem: stems[fact.relation], answer: valueText(fact), optionPool: valuePoolFor(fact), explanation: factSentence(fact), facts: [fact], difficulty: "Medium", solverAuthority: "SOURCE_GEOGRAPHY_VERIFIER" });
}
...
for (const source of EXACT_SOURCE_FACTS) {
  for (const mouth of DRAIN_FACTS.filter((fact) => fact.entityId === source.entityId)) {
    rawChains.push({ source, mouth, river: source.entity.label.en, key: `${source.relation}|${valueText(source)}|${valueText(mouth)}` });
  }
}
const chainKeyEntities = new Map<string, Set<string>>();
for (const chain of rawChains) {
  const entities = chainKeyEntities.get(chain.key) ?? new Set<string>();
  entities.add(chain.source.entityId);
  chainKeyEntities.set(chain.key, entities);
}
const CHAIN_RECORDS = rawChains.filter((chain) => chainKeyEntities.get(chain.key)?.size === 1);
...
export function generateGeoRiv001Cp008ReviewV1(qlId: string, seed: string): GeoRiv001Cp008ReviewQuestion {
...
export const GEO_RIV_001_CP008_QL_IDS_V1 = Object.freeze(Object.keys(QL_NAMES));
