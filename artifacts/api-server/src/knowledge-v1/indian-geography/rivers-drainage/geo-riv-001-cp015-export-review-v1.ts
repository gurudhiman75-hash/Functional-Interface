...
const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-RIV-001-CP015-V1");
fs.mkdirSync(outDir, { recursive: true });
const letters = ["A", "B", "C", "D"];
const lines: string[] = [
  "# GEO-RIV-001 CP015 — Mixed Rivers Mastery — Review Batch V1",
  "",
  "> **Stacked review note:** CP015 currently includes the exact reviewed CP007 V3 candidate plus merged CP001–CP006 and CP008–CP014 authorities. CP015 cannot merge before CP007 receives explicit human approval and is merged/revalidated on New-main.",
  "",
...
GEO_RIV_001_CP015_REVIEW_BATCH_V1.forEach((question, index) => {
  lines.push(
...
const baseName = "GEO-RIV-001-CP015-REVIEW-BATCH-V1";
fs.writeFileSync(path.join(outDir, `${baseName}.md`), markdown);
fs.writeFileSync(path.join(outDir, `${baseName}.json`), JSON.stringify({ audit, questions: GEO_RIV_001_CP015_REVIEW_BATCH_V1 }, null, 2));
const escaped = markdown.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
fs.writeFileSync(path.join(outDir, `${baseName}.html`), `<!doctype html><html><head><meta charset="utf-8"><title>GEO-RIV-001 CP015 Review</title><style>body{font-family:Arial,sans-serif;max-width:980px;margin:40px auto;line-height:1.5;padding:0 20px}pre{white-space:pre-wrap;font-family:inherit}</style></head><body><pre>${escaped}</pre></body></html>`);
