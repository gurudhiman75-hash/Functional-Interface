...
function chain124(text:string){const [rv,basin,parentText]=text.split(" — ");const m=parentText?.match(/^System of River (.+)$/);return Boolean(m)&&basinTruth(raw(rv),basin,m![1])}
...
function bareRiver(text:string,name:string){const cleaned=text.split(`River ${name}`).join("");const re=new RegExp(`\\b${name.replace(/[.*+?^${}()|[\\]\\]/g,"\\$&")}\\b(?!\\s+(?:Basin|Dam|Reservoir|Sagar|Project|system))`);return re.test(cleaned)}
