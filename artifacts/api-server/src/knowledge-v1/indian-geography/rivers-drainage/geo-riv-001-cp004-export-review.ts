...
if (!audit.valid) throw new Error(`GEO-RIV-001 CP004 V2 review batch failed: ${audit.issues.join(", ")}`);

const outDir = resolve(process.cwd(), "dist/geography-review/GEO-RIV-001-CP004-V2");
...
writeFileSync(resolve(outDir, "GEO-RIV-001-CP004-REVIEW-BATCH-V2.json"), JSON.stringify({
  ...
  audit,
  questions: GEO_RIV_001_CP004_REVIEW_BATCH_V2,
}, null, 2), "utf8");

const lines: string[] = [
  "# GEO-RIV-001-CP004 Review Batch V2",
  "",
  ...
  "**Editorial authority:** V2 — seven-target pair diversity and cleaner source wording.",
...
const cards: string[] = [];
for (const [index, q] of GEO_RIV_001_CP004_REVIEW_BATCH_V2.entries()) {
...
writeFileSync(resolve(outDir, "GEO-RIV-001-CP004-REVIEW-BATCH-V2.md"), lines.join("\n"), "utf8");
const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>GEO-RIV-001 CP004 Review Batch V2</title><style>body{font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;margin:0;background:#f8fafc;color:#0f172a}main{max-width:900px;margin:0 auto;padding:24px}header{margin-bottom:24px}.summary{color:#475569;line-height:1.65}.question-card{background:white;border:1px solid #dbe3ec;border-radius:14px;padding:20px;margin:0 0 18px}.meta{font-size:12px;color:#64748b;font-weight:700}.question-card h2{font-size:17px;line-height:1.5;margin:10px 0 14px}.options{padding-left:24px;line-height:1.8}.options li{padding:2px 6px}.options .correct{background:#ecfdf5;border-radius:6px}.answer{margin-top:12px;padding:10px 12px;border-radius:8px;background:#ecfdf5}.explanation{margin-top:10px;line-height:1.6}details{margin-top:12px;font-size:12px;color:#64748b}summary{cursor:pointer;font-weight:700;color:#475569}</style></head><body><main><header><h1>GEO-RIV-001-CP004 Review Batch V2</h1><div class="summary">Brahmaputra River System · Review-only · ${audit.questionCount} questions.<br/>V2 editorial hardening applied. Maps/photos/diagrams are optional manual editorial attachments.</div></header>${cards.join("\n")}</main></body></html>`;
writeFileSync(resolve(outDir, "GEO-RIV-001-CP004-REVIEW-BATCH-V2.html"), html, "utf8");
