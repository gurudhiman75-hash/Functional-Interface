...
} from "./geo-riv-001-cp005-freeze-v1";

const authority = GEO_RIV_001_CP005_FREEZE_AUTHORITY_V1;
assert.equal(authority.authorityId, "GEO-RIV-001-CP005-ENGLISH-FREEZE-V1");
assert.equal(authority.cpId, "GEO-RIV-001-CP005");
...
assert.deepEqual(authority.corpus.difficultyCounts, { Easy: 9, Medium: 35, Hard: 10 });
...
assert.equal(GEO_RIV_001_CP005_FROZEN_QUESTIONS_V1.length, 54);
assert.equal(new Set(GEO_RIV_001_CP005_FROZEN_QUESTIONS_V1.map((q) => q.questionId)).size, 54);
assert.equal(new Set(GEO_RIV_001_CP005_FROZEN_QUESTIONS_V1.map((q) => q.qlId)).size, 9);
for (const question of GEO_RIV_001_CP005_FROZEN_QUESTIONS_V1) {
