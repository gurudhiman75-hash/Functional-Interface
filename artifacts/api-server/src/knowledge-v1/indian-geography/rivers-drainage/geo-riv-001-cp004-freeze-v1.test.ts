...
} from "./geo-riv-001-cp004-freeze-v1";

const authority = GEO_RIV_001_CP004_FREEZE_AUTHORITY_V1;
assert.equal(authority.authorityId, "GEO-RIV-001-CP004-ENGLISH-FREEZE-V1");
assert.equal(authority.cpId, "GEO-RIV-001-CP004");
...
assert.deepEqual(authority.corpus.difficultyCounts, { Easy: 10, Medium: 30, Hard: 14 });
...
assert.equal(GEO_RIV_001_CP004_FROZEN_QUESTIONS_V1.length, 54);
assert.equal(new Set(GEO_RIV_001_CP004_FROZEN_QUESTIONS_V1.map((q) => q.questionId)).size, 54);
assert.equal(new Set(GEO_RIV_001_CP004_FROZEN_QUESTIONS_V1.map((q) => q.qlId)).size, 9);
for (const question of GEO_RIV_001_CP004_FROZEN_QUESTIONS_V1) {
