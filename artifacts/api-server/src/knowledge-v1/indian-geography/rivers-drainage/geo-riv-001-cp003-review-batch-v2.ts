...
function scan(qlId: string) {
  return Array.from({ length: 1200 }, (_, index) =>
    generateGeoRiv001Cp003ReviewV2(
      qlId,
      `geo-riv-001-cp003-v2-review-${qlId}-${String(index + 1).padStart(4, "0")}`,
    ),
  );
}
...
function select(qlId: string, count: number, required: readonly Predicate[] = []) {
  ...
  for (const predicate of required) {
    ...
    if (!candidate) throw new Error(`CP003 V2 review batch could not satisfy ${qlId} required predicate`);
    add(candidate);
  }
  ...
  if (selected.length !== count) {
    throw new Error(`CP003 V2 selected ${selected.length}/${count} questions for ${qlId}`);
  }
  return selected;
}
...
const REQUIRED: Record<string, Predicate[]> = {
  "GEO-RIV-001-QL-021": [
    (q) => q.stem === "Which of the following is a right-bank tributary of the Ganga?",
    (q) => q.stem === "Which of the following is a left-bank tributary of the Ganga?",
    (q) => q.stem === "Which pair consists of right-bank tributaries of the Ganga?",
    (q) => q.stem === "Which pair consists only of left-bank tributaries of the Ganga?",
  ],
  "GEO-RIV-001-QL-022": [
...
export const GEO_RIV_001_CP003_REVIEW_BATCH_V2: GeoRiv001Cp003ReviewQuestion[] =
  Object.entries(REVIEW_COUNTS).flatMap(([qlId, count]) =>
    select(qlId, count, REQUIRED[qlId] ?? []),
  );

export function auditGeoRiv001Cp003ReviewBatchV2() {
  const issues: string[] = [];
  const semantic = new Set<string>();
  ...
  for (const q of GEO_RIV_001_CP003_REVIEW_BATCH_V2) {
    ...
    if (/matches the reviewed relation|associated with the source|exam trap|shortcut|near\s+near|Therefore,|characteristic of this setting|both banks|neither bank/i.test(visible)) {
      issues.push(`EDITORIAL_LANGUAGE:${q.questionId}`);
    }
    if (/both statement i|only statement i is|only statement ii is|neither statement i/.test(visible)) {
      issues.push(`LOWERCASE_STATEMENT_LABEL:${q.questionId}`);
    }
    if (/The correct pair is|correctly matched with/i.test(q.explanation)) {
      issues.push(`TAUTOLOGICAL_PAIR_EXPLANATION:${q.questionId}`);
    }
  ...
  if (GEO_RIV_001_CP003_REVIEW_BATCH_V2.length !== expectedTotal) issues.push(`TOTAL_COUNT:${GEO_RIV_001_CP003_REVIEW_BATCH_V2.length}:${expectedTotal}`);
  ...
  const ql021Stems = new Set(
    GEO_RIV_001_CP003_REVIEW_BATCH_V2.filter((q) => q.qlId === "GEO-RIV-001-QL-021").map((q) => q.stem),
  );
  for (const stem of [
    ...
    if (!ql021Stems.has(stem)) issues.push(`MISSING_BANK_MODE:${stem}`);
  }

  for (const qlId of ["GEO-RIV-001-QL-023", "GEO-RIV-001-QL-024"]) {
    const targets = new Set(
      GEO_RIV_001_CP003_REVIEW_BATCH_V2.filter((q) => q.qlId === qlId).map(pairTargetRiver),
    );
  ...
  for (const [qlId, answers] of Object.entries({
    ...
    const actual = new Set(
      GEO_RIV_001_CP003_REVIEW_BATCH_V2.filter((q) => q.qlId === qlId).map((q) => q.canonicalAnswer),
    );
