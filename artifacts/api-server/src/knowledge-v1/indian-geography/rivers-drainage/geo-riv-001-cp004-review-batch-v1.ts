...
function scan(qlId: string) {
  return Array.from({ length: 1800 }, (_, index) =>
    generateGeoRiv001Cp004ReviewV1(qlId, `geo-riv-001-cp004-review-${qlId}-${String(index + 1).padStart(4, "0")}`),
  );
}
...
function select(qlId: string, count: number, required: readonly Predicate[] = []) {
  ...
  for (const predicate of required) {
    ...
    if (!candidate) throw new Error(`CP004 review batch could not satisfy ${qlId} required predicate`);
    add(candidate);
  }
  ...
  if (selected.length !== count) throw new Error(`CP004 selected ${selected.length}/${count} questions for ${qlId}`);
  return selected;
}
...
export const GEO_RIV_001_CP004_REVIEW_BATCH_V1: GeoRiv001Cp004ReviewQuestion[] = Object.entries(REVIEW_COUNTS).flatMap(
  ([qlId, count]) => select(qlId, count, REQUIRED[qlId] ?? []),
);

export function auditGeoRiv001Cp004ReviewBatchV1() {
  const issues: string[] = [];
  const semantic = new Set<string>();
  ...
  for (const q of GEO_RIV_001_CP004_REVIEW_BATCH_V1) {
    ...
    if (/matches the reviewed relation|exam trap|shortcut|characteristic of this setting|near\s+near|Therefore,|both banks|neither bank/i.test(visible)) issues.push(`EDITORIAL_LANGUAGE:${q.questionId}`);
  ...
  if (GEO_RIV_001_CP004_REVIEW_BATCH_V1.length !== expectedTotal) issues.push(`TOTAL_COUNT:${GEO_RIV_001_CP004_REVIEW_BATCH_V1.length}:${expectedTotal}`);
  ...
  for (const qlId of ["GEO-RIV-001-QL-032", "GEO-RIV-001-QL-033"]) {
    const targets = new Set(GEO_RIV_001_CP004_REVIEW_BATCH_V1.filter((q) => q.qlId === qlId).map(pairTarget));
    if (targets.size !== REVIEW_COUNTS[qlId]) issues.push(`PAIR_TARGET_DIVERSITY:${qlId}:${targets.size}:${REVIEW_COUNTS[qlId]}`);
  }

  const bankStems = new Set(GEO_RIV_001_CP004_REVIEW_BATCH_V1.filter((q) => q.qlId === "GEO-RIV-001-QL-030").map((q) => q.stem));
  for (const stem of [
  ...
  return {
    ...
    questionCount: GEO_RIV_001_CP004_REVIEW_BATCH_V1.length,
