...
export function auditGeoRiv001Cp007FactsV1() {
  ...
  for (const fact of GEO_RIV_001_CP007_PROJECTED_FACTS_V1) {
    ...
    if (fact.tags.some((tag) => tag === "upstream:cp006" || tag.includes("cp006"))) issues.push(`UNAPPROVED_CP006_IMPORT:${fact.factId}`);
  ...
  if (GEO_RIV_001_CP007_PROJECTED_FACTS_V1.length < 40) {
    issues.push(`FACT_COUNT_TOO_LOW:${GEO_RIV_001_CP007_PROJECTED_FACTS_V1.length}`);
  }
  ...
  if (GEO_RIV_001_CP007_LINEAGE_V1.length !== GEO_RIV_001_CP007_PROJECTED_FACTS_V1.length) {
    issues.push("LINEAGE_COUNT_MISMATCH");
  }
  if (GEO_RIV_001_CP007_LINEAGE_V1.some((row) => row.upstreamFactId === "unknown" || row.upstreamCp === "unknown")) {
    issues.push("BROKEN_LINEAGE_ROW");
  }
  ...
  if (relationFamilies.parent < 20) issues.push(`PARENT_RELATIONS_TOO_LOW:${relationFamilies.parent}`);
  if (relationFamilies.confluence < 8) issues.push(`CONFLUENCE_RELATIONS_TOO_LOW:${relationFamilies.confluence}`);
  if (relationFamilies.bank < 8) issues.push(`BANK_RELATIONS_TOO_LOW:${relationFamilies.bank}`);

  return {
    valid: issues.length === 0,
    issues,
    factCount: GEO_RIV_001_CP007_PROJECTED_FACTS_V1.length,
    lineageCount: GEO_RIV_001_CP007_LINEAGE_V1.length,
    sourceCounts,
    relationCounts,
    relationFamilies,
  };
