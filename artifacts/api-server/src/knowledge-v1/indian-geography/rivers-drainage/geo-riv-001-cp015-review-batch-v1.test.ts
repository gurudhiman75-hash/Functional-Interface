...
import { GEO_RIV_001_CP015_SOURCE_CP_QUOTAS_V1 } from "./geo-riv-001-cp015-review-batch-v1";
import {
...
assert.equal(new Set(GEO_RIV_001_CP015_REVIEW_BATCH_V2.map((q) => q.sourceQuestionId)).size, 60);
assert.equal(new Set(GEO_RIV_001_CP015_REVIEW_BATCH_V2.map((q) => q.qlId)).size, 60);
assert.equal(new Set(GEO_RIV_001_CP015_REVIEW_BATCH_V2.map((q) => `${q.stem}::${q.canonicalAnswer}`)).size, 60);
