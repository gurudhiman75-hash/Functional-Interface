...
function signature(q:GeoRiv001Cp012ReviewQuestion){
  if(["101","102","105","106","107","108"].some((suffix)=>q.qlId.endsWith(suffix))) return `${q.stem}|${q.canonicalAnswer}`;
  return q.canonicalAnswer;
}

const generated:GeoRiv001Cp012ReviewQuestion[]=[];
for(const qlId of GEO_RIV_001_CP012_QL_IDS_V1){ const qs:GeoRiv001Cp012ReviewQuestion[]=[]; const seen=new Set<string>(); for(let a=1;a<=1000&&qs.length<6;a++){ const q=generateGeoRiv001Cp012ReviewV1(qlId,`review-${qlId}-${String(a).padStart(3,"0")}`); const s=signature(q); if(seen.has(s)) continue; seen.add(s); qs.push(q); } if(qs.length!==6) throw new Error(`Unable to build six semantically distinct CP012 questions for ${qlId}`); generated.push(...qs); }
...
function cityFromLocationLabel(label:string){ return [...CITIES].sort((a,b)=>b.length-a.length).find((city)=>label===city||label.startsWith(`${city}, `))??""; }
...
function semantic(q:GeoRiv001Cp012ReviewQuestion,issues:string[]){
  ...
  if(q.qlId.endsWith("105")){ const label=q.stem.match(/^Which river is associated with (.+)\?$/)?.[1]??""; const city=cityFromLocationLabel(label); const truth=q.options.map((r)=>truePair(city,r)); if(!city||truth.filter(Boolean).length!==1||!truth[q.correctIndex]) issues.push(`QL105_TRUTH:${q.questionId}`); }
  ...
  if(q.qlId.endsWith("107")){ const vals=statementLines(q.stem).map(evalClaim); const cities=statementLines(q.stem).map((line)=>line.match(/^(.+?) is situated/)?.[1]??""); if(new Set(cities).size!==2) issues.push(`QL107_DUPLICATE_CITY:${q.questionId}`); if(vals.length!==2||vals.some((x)=>x===null)||q.canonicalAnswer!==expectedTwo(vals as boolean[])) issues.push(`QL107_TRUTH:${q.questionId}`); }
  if(q.qlId.endsWith("108")){ const lines=statementLines(q.stem); const vals=lines.map(evalClaim); const cities=lines.map((line)=>line.match(/^(.+?) is situated/)?.[1]??""); if(new Set(cities).size!==3) issues.push(`QL108_DUPLICATE_CITY:${q.questionId}`); if(vals.length!==3||vals.some((x)=>x===null)||q.canonicalAnswer!==expectedCount((vals as boolean[]).filter(Boolean).length)) issues.push(`QL108_TRUTH:${q.questionId}`); }
...
export function auditGeoRiv001Cp012ReviewBatchV1(){ const issues:string[]=[]; const qlCounts:Record<string,number>={}; const difficultyCounts={Easy:0,Medium:0,Hard:0}; const answerPositions:Record<number,number>={0:0,1:0,2:0,3:0}; const ids=new Set<string>(); const signaturesByQl:Record<string,Set<string>>={}; for(const q of GEO_RIV_001_CP012_REVIEW_BATCH_V1){ if(ids.has(q.questionId))issues.push(`DUPLICATE_ID:${q.questionId}`);ids.add(q.questionId);qlCounts[q.qlId]=(qlCounts[q.qlId]??0)+1;(signaturesByQl[q.qlId]??=new Set()).add(signature(q));difficultyCounts[q.difficulty]+=1;answerPositions[q.correctIndex]+=1;if(q.options.length!==4||new Set(q.options).size!==4)issues.push(`OPTIONS:${q.questionId}`);if(q.options[q.correctIndex]!==q.canonicalAnswer)issues.push(`ANSWER_ALIGNMENT:${q.questionId}`);if(!q.sourceIds.length||!q.sourceFactIds.length)issues.push(`PROVENANCE:${q.questionId}`);if(!q.reviewOnly||q.runtimeRegistered)issues.push(`LIFECYCLE:${q.questionId}`);if(hasBareRiver(q.stem)||q.options.some(hasBareRiver)||hasBareRiver(q.explanation))issues.push(`RIVER_PREFIX:${q.questionId}`);if(/in this corpus|sourceFact|review-only|exam trap|shortcut/i.test(q.explanation))issues.push(`MACHINE_LANGUAGE:${q.questionId}`);semantic(q,issues); }
  if(GEO_RIV_001_CP012_REVIEW_BATCH_V1.length!==54)issues.push(`QUESTION_COUNT:${GEO_RIV_001_CP012_REVIEW_BATCH_V1.length}`);for(const id of GEO_RIV_001_CP012_QL_IDS_V1){if(qlCounts[id]!==6)issues.push(`QL_COUNT:${id}:${qlCounts[id]??0}`);if((signaturesByQl[id]?.size??0)!==6)issues.push(`DIVERSITY:${id}:${signaturesByQl[id]?.size??0}`);}if(difficultyCounts.Easy!==18||difficultyCounts.Medium!==24||difficultyCounts.Hard!==12)issues.push(`DIFFICULTY:${JSON.stringify(difficultyCounts)}`);if(answerPositions[0]!==14||answerPositions[1]!==14||answerPositions[2]!==13||answerPositions[3]!==13)issues.push(`ANSWER_POSITIONS:${JSON.stringify(answerPositions)}`);return Object.freeze({valid:issues.length===0,issues:Object.freeze(issues),questionCount:GEO_RIV_001_CP012_REVIEW_BATCH_V1.length,qlCounts:Object.freeze(qlCounts),difficultyCounts:Object.freeze(difficultyCounts),answerPositions:Object.freeze(answerPositions),semanticDiversityValid:GEO_RIV_001_CP012_QL_IDS_V1.every((id)=>(signaturesByQl[id]?.size??0)===6)}); }
