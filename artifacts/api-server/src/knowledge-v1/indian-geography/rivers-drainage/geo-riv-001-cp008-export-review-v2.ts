...
const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-RIV-001-CP008-V2");
...
const lines: string[] = [
  "# GEO-RIV-001 CP008 — Sources, Origins & Mouths — Review Batch V2",
  "",
  "**Lifecycle:** Review-only; CP006 is intentionally held out pending human approval.  ",
  "**Supersedes:** V1 review artifact after editorial hardening of source-location grammar, mouth-type grammar, distractor domains and semantic uniqueness.  ",
...
let currentQl = "";
GEO_RIV_001_CP008_REVIEW_BATCH_V2.forEach((question, index) => {
...
fs.writeFileSync(path.join(outDir, "GEO-RIV-001-CP008-REVIEW-BATCH-V2.md"), markdown);
fs.writeFileSync(path.join(outDir, "GEO-RIV-001-CP008-REVIEW-BATCH-V2.json"), JSON.stringify({ audit, questions: GEO_RIV_001_CP008_REVIEW_BATCH_V2 }, null, 2));
const escaped = markdown.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
fs.writeFileSync(path.join(outDir, "GEO-RIV-001-CP008-REVIEW-BATCH-V2.html"), `<!doctype html><html><head><meta charset="utf-8"><title>CP008 Review V2</title><style>body{font-family:Arial,sans-serif;max-width:980px;margin:40px auto;line-height:1.5;padding:0 20px}pre{white-space:pre-wrap;font-family:inherit}</style></head><body><pre>${escaped}</pre></body></html>`);
