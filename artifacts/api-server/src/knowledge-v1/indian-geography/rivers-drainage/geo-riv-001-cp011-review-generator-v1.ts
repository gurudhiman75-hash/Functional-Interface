...
function ql092(seed: string) {
  ...
  return build({
    ...
    answer: row.basin,
    optionPool: BASINS,
    explanation: `${displayRiver(row.river)} is part of the ${row.basin}. It belongs to the ${row.parentRiver} river system.`,
    facts: fact ? [fact] : [],
    difficulty: "Easy",
...
function ql093(seed: string) {
  ...
  return build({
    ...
    optionPool: pool,
    explanation: `${displayRiver(target.river)} belongs to the ${basin}; its parent river system is the ${target.parentRiver} system.`,
    facts: fact ? [fact] : [],
    difficulty: "Medium",
...
function ql094(seed: string) {
  ...
  return build({
    ...
    answer: pairs[target],
    optionPool: pairs,
    explanation: `${displayRiver(row.river)} belongs to the ${row.basin}. It is part of the ${row.parentRiver} river system.`,
...
function ql095(seed: string) {
  ...
  return build({
    ...
    answer: pairs[target],
    optionPool: pairs,
    explanation: `${displayRiver(row.river)} belongs to the ${row.basin}, not to the basin shown in that pair. It is part of the ${row.parentRiver} river system.`,
    facts: selected.map((item) => basinFactForRiver(item.river)).filter((fact): fact is KnowledgeFact => Boolean(fact)),
    difficulty: "Medium",
    solverAuthority: "MATCHED_RIVER_BASIN_VERIFIER",
  });
