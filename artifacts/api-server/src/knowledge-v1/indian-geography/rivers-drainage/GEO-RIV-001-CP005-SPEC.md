# GEO-RIV-001-CP005 — East-flowing Peninsular Rivers

**Chapter:** GEO-RIV-001 — Indian Rivers & Drainage System  
**CP:** GEO-RIV-001-CP005  
**Status:** APPROVED — ENGLISH FREEZE V1

## Scope

Deterministic, source-backed Static GK coverage of the major east-flowing Peninsular river systems tested in SSC, Railway, Banking and State objective exams.

## Core river systems

1. Godavari
2. Krishna
3. Mahanadi
4. Cauvery (Kaveri)
5. Pennar
6. Brahmani
7. Baitarani
8. Subarnarekha

## Canonical domains

### A. Origin and outfall
- source region/place
- source state
- east-flowing course
- Bay of Bengal outfall
- Cauvery/Kaveri alias
- Godavari/Dakshin Ganga nickname

### B. Major tributary membership
- Godavari major tributaries
- Krishna major tributaries
- Mahanadi major tributaries
- Cauvery major tributaries
- Pennar major tributaries
- selected Brahmani/Baitarani/Subarnarekha tributaries

### C. Bank-side classification
Bank-side questions are used only where explicitly supported by the basin authority:
- Godavari
- Krishna
- Mahanadi
- Cauvery
- Pennar

### D. Formation and hierarchy
- Wardha + Wainganga → Pranhita → Godavari
- Tunga + Bhadra → Tungabhadra → Krishna
- Sankh + Koel/South Koel → Brahmani

## Permanent question families

1. **QL-037 — Direct source/course association**
2. **QL-038 — Reverse association**
3. **QL-039 — Tributary / river-system identification**
4. **QL-040 — Formation / tributary hierarchy**
5. **QL-041 — Correct pair**
6. **QL-042 — Incorrect pair**
7. **QL-043 — River relation chain / order**
8. **QL-044 — Statement I/II**
9. **QL-045 — Multi-statement count**

## Approved review authority

`GEO-RIV-001-CP005-REVIEW-BATCH-V4`

- 54 questions
- 54 semantically unique tasks
- Easy 9 · Medium 35 · Hard 10
- QL distribution 7/6/8/5/7/7/4/6/4
- answer positions A/B/C/D = 14/14/13/13
- all eight core systems represented
- all three formation/hierarchy anchors represented
- five major systems represented in bank-side questions
- at least seven distinct targets in each pair section
- source and fact provenance on every question
- simple exam-like wording

## Freeze authority

`GEO-RIV-001-CP005-ENGLISH-FREEZE-V1`

Approval mode: `EXPLICIT_HUMAN_EDITORIAL_APPROVAL`.

The frozen corpus is immutable and read-only. Any change to approved stems, options, answers, explanations, difficulty labels, provenance, QL allocation, or editorial wording requires a new governed freeze version.

## Question Studio lifecycle

CP005 is registered under the existing `GEO-RIV-001` package after the approved freeze is present. Runtime remains `REVIEW_ONLY` and read-only. Question Bank writes, test/mock eligibility, public publication, and production release remain disabled.

## Visual policy

No automatic visual is required. Maps/photos/diagrams may be attached manually during editorial review when they genuinely improve an explanation.
