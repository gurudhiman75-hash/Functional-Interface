...
const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-RIV-001-CP007-V2");
...
const lines: string[] = [
  "# GEO-RIV-001 CP007 — Tributaries & Confluences — Approved-Upstream Review Batch V2",
  "",
...
let currentQl = "";
GEO_RIV_001_CP007_REVIEW_BATCH_V2.forEach((q, index) => {
  if (q.qlId !== currentQl) {
    currentQl = q.qlId;
...
const baseName = "GEO-RIV-001-CP007-REVIEW-BATCH-V2";
fs.writeFileSync(path.join(outDir, `${baseName}.md`), markdown);
fs.writeFileSync(path.join(outDir, `${baseName}.json`), JSON.stringify({ audit, questions: GEO_RIV_001_CP007_REVIEW_BATCH_V2 }, null, 2));
const escaped = markdown.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
fs.writeFileSync(path.join(outDir, `${baseName}.html`), `<!doctype html><html><head><meta charset="utf-8"><title>CP007 V2 Review</title><style>body{font-family:Arial,sans-serif;max-width:980px;margin:40px auto;line-height:1.5;padding:0 20px}pre{white-space:pre-wrap;font-family:inherit}</style></head><body><pre>${escaped}</pre></body></html>`);
