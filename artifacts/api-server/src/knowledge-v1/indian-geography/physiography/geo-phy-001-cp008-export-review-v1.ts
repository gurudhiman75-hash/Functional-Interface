...
if (!audit.valid) throw new Error(`GEO-PHY-001 CP008 export blocked: ${audit.issues.join(" | ")}`);

const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-PHY-001-CP008-V1");
fs.mkdirSync(outDir, { recursive: true });
const letters = ["A", "B", "C", "D"];
const lines: string[] = [
  "# GEO-PHY-001 CP008 — Important Hills, Ranges & Peaks — Review Batch V1",
  "",
...
GEO_PHY_001_CP008_REVIEW_BATCH_V1.forEach((question, index) => {
  lines.push(
...
const baseName = "GEO-PHY-001-CP008-REVIEW-BATCH-V1";
fs.writeFileSync(path.join(outDir, `${baseName}.md`), lines.join("\n"));
fs.writeFileSync(
  path.join(outDir, `${baseName}.json`),
  JSON.stringify({ audit, questions: GEO_PHY_001_CP008_REVIEW_BATCH_V1 }, null, 2),
);
