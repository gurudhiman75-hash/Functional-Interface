...
import {
  GEO_PHY_001_CP006_REVIEW_BATCH_V1,
  auditGeoPhy001Cp006ReviewBatchV1,
} from "./geo-phy-001-cp006-review-batch-v1";

const audit = auditGeoPhy001Cp006ReviewBatchV1();
if (!audit.valid) throw new Error(`GEO-PHY-001 CP006 export blocked: ${audit.issues.join(" | ")}`);

const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-PHY-001-CP006-V1");
fs.mkdirSync(outDir, { recursive: true });
const letters = ["A", "B", "C", "D"];
const lines: string[] = [
  "# GEO-PHY-001 CP006 — Coastal Plains — Review Batch V1",
  "",
...
GEO_PHY_001_CP006_REVIEW_BATCH_V1.forEach((question, index) => {
  lines.push(
...
const baseName = "GEO-PHY-001-CP006-REVIEW-BATCH-V1";
fs.writeFileSync(path.join(outDir, `${baseName}.md`), lines.join("\n"));
fs.writeFileSync(
  path.join(outDir, `${baseName}.json`),
  JSON.stringify({ audit, questions: GEO_PHY_001_CP006_REVIEW_BATCH_V1 }, null, 2),
);
