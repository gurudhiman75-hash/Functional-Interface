...
import {
  GEO_PHY_001_CP009_REQUIRED_FACTS_V1,
  GEO_PHY_001_CP009_REVIEW_BATCH_V1,
  auditGeoPhy001Cp009ReviewBatchV1,
} from "./geo-phy-001-cp009-review-batch-v1";

const audit = auditGeoPhy001Cp009ReviewBatchV1();
...
assert.equal(new Set(GEO_PHY_001_CP009_REVIEW_BATCH_V1.map((question) => question.qlId)).size, 9);

const usedFacts = new Set(GEO_PHY_001_CP009_REVIEW_BATCH_V1.flatMap((question) => question.sourceFactIds));
for (const factId of GEO_PHY_001_CP009_REQUIRED_FACTS_V1) {
  assert.ok(usedFacts.has(factId), `required CP009 fact is dormant: ${factId}`);
}

for (const question of GEO_PHY_001_CP009_REVIEW_BATCH_V1) {
  const learner = `${question.stem}\n${question.options.join("\n")}\n${question.explanation}`;
  assert.doesNotMatch(learner, /\bNCERT\b|sourceFact|review-only|runtimeRegistered|generator|qualification gate|physiographic nomenclature|geological provenance|morphological characteristic|highest motorable/i, question.questionId);
