...
import {
  GEO_PHY_001_CP006_REQUIRED_FACTS_V1,
...
assert.equal(new Set(GEO_PHY_001_CP006_REVIEW_BATCH_V1.map((question) => question.qlId)).size, 9);
...
for (const factId of GEO_PHY_001_CP006_REQUIRED_FACTS_V1) {
  assert.ok(usedFacts.has(factId), `required CP006 fact is dormant: ${factId}`);
}
...
for (const question of GEO_PHY_001_CP006_REVIEW_BATCH_V1) {
  ...
  assert.doesNotMatch(
    learner,
    /\bgeomorphology\b|localized water-related|moisture conditions|gains greater prominence|towards the western margins|\bundulating\b|flanked by stretch/i,
    question.questionId,
  );
