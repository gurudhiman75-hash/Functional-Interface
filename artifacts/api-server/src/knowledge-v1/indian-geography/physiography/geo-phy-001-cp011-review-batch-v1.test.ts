...
assert.equal(new Set(GEO_PHY_001_CP011_REVIEW_BATCH_V3.map((q) => q.qlId)).size, 9);
...
for (const question of GEO_PHY_001_CP011_REVIEW_BATCH_V3) {
  ...
  assert.doesNotMatch(
    learner,
    /physiographic|physical division|geologically|alluvial|emergent|submerged|crystalline|igneous|metamorphic|offshore|\barid\b|deposition|correctly classified|incorrectly classified|structurally folded/i,
