...
if (!audit.valid) throw new Error(`GEO-PHY-001 CP007 export blocked: ${audit.issues.join(" | ")}`);

const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-PHY-001-CP007-V1");
fs.mkdirSync(outDir, { recursive: true });
const letters = ["A", "B", "C", "D"];
const lines: string[] = [
  "# GEO-PHY-001 CP007 — Islands — Review Batch V1",
  "",
...
GEO_PHY_001_CP007_REVIEW_BATCH_V1.forEach((question, index) => {
  lines.push(
...
const baseName = "GEO-PHY-001-CP007-REVIEW-BATCH-V1";
fs.writeFileSync(path.join(outDir, `${baseName}.md`), lines.join("\n"));
fs.writeFileSync(
  path.join(outDir, `${baseName}.json`),
  JSON.stringify({ audit, questions: GEO_PHY_001_CP007_REVIEW_BATCH_V1 }, null, 2),
);
