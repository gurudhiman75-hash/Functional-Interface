...
import { GEO_VEG_001_CP010_RAW_PART1 } from "./geo-veg-001-cp010-raw-part1-v1";
import { GEO_VEG_001_CP010_RAW_PART2 } from "./geo-veg-001-cp010-raw-part2-v1";
import { GEO_VEG_001_CP010_RAW_PART3 } from "./geo-veg-001-cp010-raw-part3-v1";

const RAW = Object.freeze([
  ...GEO_VEG_001_CP010_RAW_PART1,
  ...GEO_VEG_001_CP010_RAW_PART2,
  ...GEO_VEG_001_CP010_RAW_PART3,
]);

export const GEO_VEG_001_CP010_REVIEW_BATCH_V1: readonly GeoVeg001Question[] = Object.freeze(
  RAW.map((raw, index) => {
    const correctIndex = index % 4;
    return Object.freeze({
      questionId: `GEO-VEG-001-CP010-Q${String(index + 1).padStart(3, "0")}`,
      qlId: raw.qlId,
      qlName: raw.qlName,
      difficulty: raw.difficulty as GeoVeg001Difficulty,
      stem: raw.stem,
...
export function auditGeoVeg001Cp010ReviewBatchV1() {
  const issues: string[] = [];
  ...
  for (const q of GEO_VEG_001_CP010_REVIEW_BATCH_V1) {
    ...
    if (distractors.some((o) => TRIVIAL_DISTRACTOR.test(o))) issues.push("TRIVIAL_DISTRACTOR:" + q.questionId);
    if (!q.sourceIds.length || !q.sourceFactIds.length) issues.push("PROVENANCE:" + q.questionId);
    if (!q.reviewOnly || q.runtimeRegistered) issues.push("LIFECYCLE:" + q.questionId);
    const learner = q.stem + "\n" + q.options.join("\n") + "\n" + q.explanation;
    if (BANNED.test(learner)) issues.push("STYLE:" + q.questionId);
  ...
  if (GEO_VEG_001_CP010_REVIEW_BATCH_V1.length !== 54) issues.push("COUNT:" + GEO_VEG_001_CP010_REVIEW_BATCH_V1.length);
  ...
  return Object.freeze({
    ...
    questionCount: GEO_VEG_001_CP010_REVIEW_BATCH_V1.length,
