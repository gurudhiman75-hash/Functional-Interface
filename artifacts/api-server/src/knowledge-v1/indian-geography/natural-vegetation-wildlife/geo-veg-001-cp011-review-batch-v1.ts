...
import { GEO_VEG_001_CP011_RAW_PART1 } from "./geo-veg-001-cp011-raw-part1-v1";
import { GEO_VEG_001_CP011_RAW_PART2 } from "./geo-veg-001-cp011-raw-part2-v1";
import { GEO_VEG_001_CP011_RAW_PART3 } from "./geo-veg-001-cp011-raw-part3-v1";

const RAW = Object.freeze([
  ...GEO_VEG_001_CP011_RAW_PART1,
  ...GEO_VEG_001_CP011_RAW_PART2,
  ...GEO_VEG_001_CP011_RAW_PART3,
]);

export const GEO_VEG_001_CP011_REVIEW_BATCH_V1: readonly GeoVeg001Question[] = Object.freeze(
  RAW.map((raw,index)=>{const baseIndex=index%4;const correctIndex=(index===0||index===20||index===40)?2:(index===1||index===21||index===41)?3:baseIndex;return Object.freeze({
    questionId:`GEO-VEG-001-CP011-Q${String(index+1).padStart(3,"0")}`,
    qlId:raw.qlId,qlName:raw.qlName,difficulty:raw.difficulty as GeoVeg001Difficulty,stem:raw.stem,
    options:placeGeoVegOptions(raw.answer,raw.distractors,correctIndex),correctIndex,canonicalAnswer:raw.answer,
    explanation:raw.explanation,sourceIds:GEO_VEG_001_SOURCE_IDS,sourceFactIds:Object.freeze([...raw.sourceFactIds]),
...
export function auditGeoVeg001Cp011ReviewBatchV1(){
 const issues:string[]=[];const ids=new Set<string>();const stems=new Set<string>();const explanations=new Set<string>();const qlCounts:Record<string,number>={};const difficultyCounts:Record<GeoVeg001Difficulty,number>={Easy:0,Medium:0,Hard:0};const answerPositions=[0,0,0,0];
 for(const q of GEO_VEG_001_CP011_REVIEW_BATCH_V1){
 ...
 if(GEO_VEG_001_CP011_REVIEW_BATCH_V1.length!==54)issues.push("COUNT:"+GEO_VEG_001_CP011_REVIEW_BATCH_V1.length);
 for(let n=91;n<=99;n++){const id="GEO-VEG-001-QL-"+String(n).padStart(3,"0");if(qlCounts[id]!==6)issues.push("QL_COUNT:"+id+":"+(qlCounts[id]??0));}
 ...
 if(answerPositions.join(",")!=="11,11,16,16")issues.push("ANSWER_POSITIONS:"+answerPositions.join(","));
 ...
 return Object.freeze({valid:issues.length===0,issues:Object.freeze(issues),questionCount:GEO_VEG_001_CP011_REVIEW_BATCH_V1.length,stemCount:stems.size,explanationCount:explanations.size,qlCounts:Object.freeze(qlCounts),difficultyCounts:Object.freeze(difficultyCounts),answerPositions:Object.freeze(answerPositions)});
}
