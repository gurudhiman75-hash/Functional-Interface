...
const audit = auditGeoAgr001Cp004ReviewBatchV1();
if (!audit.valid) throw new Error(audit.issues.join(" | "));

const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-AGR-001-CP004-V1");
fs.mkdirSync(outDir, { recursive: true });
const letters = ["A", "B", "C", "D"];
const lines = [
  "# GEO-AGR-001 CP004 — Agricultural Systems, Irrigation, Inputs & Green Revolution — Review Batch V1",
  "",
...
GEO_AGR_001_CP004_REVIEW_BATCH_V1.forEach((q, i) => {
  lines.push(
    "## " + (i + 1) + ". " + q.stem,
    "",
...
const name = "GEO-AGR-001-CP004-REVIEW-BATCH-V1";
fs.writeFileSync(path.join(outDir, name + ".md"), lines.join("\n"));
fs.writeFileSync(path.join(outDir, name + ".json"), JSON.stringify({ audit, questions: GEO_AGR_001_CP004_REVIEW_BATCH_V1 }, null, 2));
