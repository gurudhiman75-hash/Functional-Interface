...
const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-SOI-001-CP003-V1");
fs.mkdirSync(outDir, { recursive: true });
const letters = ["A","B","C","D"];
const lines = [
  "# GEO-SOI-001 CP003 — Black Soils — Review Batch V1",
  "",
...
GEO_SOI_001_CP003_REVIEW_BATCH_V1.forEach((q, i) => {
  lines.push(
    "## " + (i + 1) + ". " + q.stem,
    "",
...
const name = "GEO-SOI-001-CP003-REVIEW-BATCH-V1";
fs.writeFileSync(path.join(outDir, name + ".md"), lines.join("\n"));
fs.writeFileSync(path.join(outDir, name + ".json"), JSON.stringify({ audit, questions: GEO_SOI_001_CP003_REVIEW_BATCH_V1 }, null, 2));
