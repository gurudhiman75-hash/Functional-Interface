...
import {
  ...
  auditGeoCli001Cp004ReviewBatchV1,
} from "./geo-cli-001-cp004-review-batch-v1";

const audit = auditGeoCli001Cp004ReviewBatchV1();
if (!audit.valid) throw new Error(`GEO-CLI-001 CP004 V1 export blocked: ${audit.issues.join(" | ")}`);

const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-CLI-001-CP004-V1");
fs.mkdirSync(outDir, { recursive: true });
const letters = ["A", "B", "C", "D"];
const lines: string[] = [
  "# GEO-CLI-001 CP004 — Hot Weather Season & Local Winds — Review Batch V1",
  "",
...
GEO_CLI_001_CP004_REVIEW_BATCH_V1.forEach((question, index) => {
  lines.push(
...
const baseName = "GEO-CLI-001-CP004-REVIEW-BATCH-V1";
fs.writeFileSync(path.join(outDir, `${baseName}.md`), lines.join("\n"));
fs.writeFileSync(path.join(outDir, `${baseName}.json`), JSON.stringify({ audit, questions: GEO_CLI_001_CP004_REVIEW_BATCH_V1 }, null, 2));
