import assert from "node:assert/strict";
import {
  GEO_CLI_001_CP013_REVIEW_BATCH_V5,
  auditGeoCli001Cp013ReviewBatchV5,
} from "./geo-cli-001-cp013-review-batch-v5";
import { GEO_CLI_001_CP013_REVIEW_BATCH_V4 } from "./geo-cli-001-cp013-review-batch-v4";

const audit = auditGeoCli001Cp013ReviewBatchV5();
assert.equal(audit.valid, true, audit.issues.join("\n"));
assert.equal(audit.questionCount, 108);
assert.equal(audit.qlCount, 108);
assert.equal(audit.stemCount, 108);
assert.equal(audit.semanticCount, 108);
assert.equal(audit.explanationCount, 108);
assert.equal(audit.stemPatchCount, 108);
assert.deepEqual(audit.difficultyCounts, { Easy: 36, Medium: 60, Hard: 12 });
assert.deepEqual(audit.answerPositions, [27, 27, 27, 27]);

for (let n = 1; n <= 108; n += 1) {
  const id = `GEO-CLI-001-QL-${String(n).padStart(3, "0")}`;
  assert.equal(audit.qlCounts[id], 1, `${id} should appear exactly once`);
}

for (let index = 0; index < GEO_CLI_001_CP013_REVIEW_BATCH_V5.length; index += 1) {
  const question = GEO_CLI_001_CP013_REVIEW_BATCH_V5[index];
  const prior = GEO_CLI_001_CP013_REVIEW_BATCH_V4[index];
  assert.notEqual(question.stem, prior.stem, `${question.questionId} stem must be rewritten in V5`);
  assert.equal(question.qlId, prior.qlId);
  assert.equal(question.difficulty, prior.difficulty);
  assert.equal(question.correctIndex, prior.correctIndex);
  assert.equal(question.canonicalAnswer, prior.canonicalAnswer);
  assert.deepEqual(question.options, prior.options);
  assert.equal(question.explanation, prior.explanation);
  assert.deepEqual(question.sourceIds, prior.sourceIds);
  assert.deepEqual(question.sourceFactIds, prior.sourceFactIds);
  assert.equal(question.reviewOnly, true);
  assert.equal(question.runtimeRegistered, false);
  assert.equal(question.options.length, 4);
  assert.equal(new Set(question.options).size, 4);
  assert.equal(question.options[question.correctIndex], question.canonicalAnswer);
  const learnerText = `${question.stem}\n${question.options.join("\n")}\n${question.explanation}`;
  assert.equal(/\bbroad(?:ly)?\b|\bassociated with\b/i.test(learnerText), false);
  assert.equal(/^(?:compare\b|for\s+(?:cool-season rain|india[’']s? winter climate|a climate summary)\b)/i.test(question.stem.trim()), false);
}

console.log(JSON.stringify(audit, null, 2));
