...
if (!audit.valid) throw new Error(`GEO-CLI-001 CP001 V4 export blocked: ${audit.issues.join(" | ")}`);

const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-CLI-001-CP001-V4");
fs.mkdirSync(outDir, { recursive: true });
const letters = ["A", "B", "C", "D"];
const lines: string[] = [
  "# GEO-CLI-001 CP001 — Climate Controls & Monsoon Character — Review Batch V4",
  "",
...
GEO_CLI_001_CP001_REVIEW_BATCH_V4.forEach((question, index) => {
  lines.push(
...
const baseName = "GEO-CLI-001-CP001-REVIEW-BATCH-V4";
fs.writeFileSync(path.join(outDir, `${baseName}.md`), lines.join("\n"));
fs.writeFileSync(path.join(outDir, `${baseName}.json`), JSON.stringify({ audit, questions: GEO_CLI_001_CP001_REVIEW_BATCH_V4 }, null, 2));
