...
import {
  ...
  auditGeoCli001Cp010ReviewBatchV1,
} from "./geo-cli-001-cp010-review-batch-v1";

const audit = auditGeoCli001Cp010ReviewBatchV1();
if (!audit.valid) throw new Error(`GEO-CLI-001 CP010 V1 export blocked: ${audit.issues.join(" | ")}`);

const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-CLI-001-CP010-V1");
fs.mkdirSync(outDir, { recursive: true });
const letters = ["A", "B", "C", "D"];
const lines: string[] = [
  "# GEO-CLI-001 CP010 — Climate–Region / State Associations — Review Batch V1",
  "",
...
GEO_CLI_001_CP010_REVIEW_BATCH_V1.forEach((question, index) => {
  lines.push(
...
const baseName = "GEO-CLI-001-CP010-REVIEW-BATCH-V1";
fs.writeFileSync(path.join(outDir, `${baseName}.md`), lines.join("\n"));
fs.writeFileSync(path.join(outDir, `${baseName}.json`), JSON.stringify({ audit, questions: GEO_CLI_001_CP010_REVIEW_BATCH_V1 }, null, 2));
