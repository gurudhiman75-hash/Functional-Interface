...
const outDir = path.resolve(process.cwd(), "dist/geography-review/GEO-LOC-001-CP001-V1");
...
GEO_LOC_001_CP001_REVIEW_BATCH_V1.forEach((q, i) => {
  lines.push(
    "## " + (i + 1) + ". " + q.stem,
    "",
...
const name = "GEO-LOC-001-CP001-REVIEW-BATCH-V1";
fs.writeFileSync(path.join(outDir, name + ".md"), lines.join("\n"));
fs.writeFileSync(
  path.join(outDir, name + ".json"),
  JSON.stringify({ audit, questions: GEO_LOC_001_CP001_REVIEW_BATCH_V1 }, null, 2),
);
