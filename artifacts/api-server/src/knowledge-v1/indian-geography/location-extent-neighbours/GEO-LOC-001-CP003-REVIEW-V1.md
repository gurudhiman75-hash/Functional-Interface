# GEO-LOC-001 CP003 — Indian Standard Time & Longitude Applications — Review Batch V1

Questions: 54
Difficulty: Easy 18 / Medium 30 / Hard 6
Lifecycle: Review only; not runtime registered.

## 1. Which longitude is used as the Standard Meridian of India?

A. 82°30'E
B. 68°7'E
C. 97°25'E
D. 23°30'N

**Answer:** A. 82°30'E

**Explanation:** India uses 82°30'E as its Standard Meridian. Time based on this longitude is followed as Indian Standard Time across the country.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-019 — Standard Meridian — 82°30'E

## 2. Indian Standard Time is based on the local time of which meridian?

A. 75°E
B. 82°30'E
C. 90°E
D. 0°

**Answer:** B. 82°30'E

**Explanation:** The local time at 82°30'E provides the basis for Indian Standard Time. Using one reference meridian avoids different official times across India's long east–west extent.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-019 — Standard Meridian — 82°30'E

## 3. Why is 82°30'E important for timekeeping in India?

A. It is India's westernmost longitude
B. It is the Tropic of Cancer
C. It provides the reference longitude for IST
D. It marks the Equator

**Answer:** C. It provides the reference longitude for IST

**Explanation:** 82°30'E is the national reference meridian for standard time. It is a longitude, not a latitude or an extreme edge of the mainland.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-019 — Standard Meridian — 82°30'E

## 4. Which value is a longitude used for national time rather than a latitude?

A. 23°30'N
B. 8°4'N
C. 37°6'N
D. 82°30'E

**Answer:** D. 82°30'E

**Explanation:** The E suffix identifies 82°30'E as a longitude, and India uses it for standard time. The other values are latitudes used in questions on India's north–south position.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-019 — Standard Meridian — 82°30'E

## 5. A map marks a north–south line at 82°30'E across India. What does this line represent?

A. The Standard Meridian of India
B. The Tropic of Cancer
C. The Equator
D. India's eastern coastline

**Answer:** A. The Standard Meridian of India

**Explanation:** Meridians run north–south and are measured as longitudes. The meridian at 82°30'E is used as India's standard-time reference.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-019 — Standard Meridian — 82°30'E

## 6. Which change would alter the reference longitude used for IST?

A. Changing the Tropic of Cancer
B. Changing India's Standard Meridian from 82°30'E
C. Changing India's coastline length
D. Changing the north–south distance

**Answer:** B. Changing India's Standard Meridian from 82°30'E

**Explanation:** IST is tied to the chosen standard meridian at 82°30'E. Latitude, coastline length and mainland dimensions do not determine the reference longitude for standard time.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-019 — Standard Meridian — 82°30'E

## 7. India's Standard Meridian passes through which place?

A. Jaipur in Rajasthan
B. Ahmedabad in Gujarat
C. Mirzapur in Uttar Pradesh
D. Guwahati in Assam

**Answer:** C. Mirzapur in Uttar Pradesh

**Explanation:** The 82°30'E Standard Meridian passes through Mirzapur in Uttar Pradesh. This place is commonly used to identify India's time-reference meridian in exam geography.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-020 — Mirzapur reference and central meridian

## 8. Mirzapur, used as a reference in questions on IST, is in which state?

A. Madhya Pradesh
B. Rajasthan
C. Bihar
D. Uttar Pradesh

**Answer:** D. Uttar Pradesh

**Explanation:** Mirzapur is in Uttar Pradesh and lies on the 82°30'E Standard Meridian. Its name is commonly paired with India's standard-time reference.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-020 — Mirzapur reference and central meridian

## 9. Which place–longitude combination is used for Indian Standard Time?

A. Mirzapur — 82°30'E
B. Ahmedabad — 68°7'E
C. Kanyakumari — 23°30'N
D. Guwahati — 97°25'E

**Answer:** A. Mirzapur — 82°30'E

**Explanation:** Mirzapur in Uttar Pradesh lies on 82°30'E, India's Standard Meridian. The other figures refer to different geographic limits or reference lines.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-020 — Mirzapur reference and central meridian

## 10. Why is Mirzapur frequently mentioned with Indian Standard Time?

A. It is India's easternmost city
B. The Standard Meridian at 82°30'E passes through it
C. It lies on the Equator
D. It is India's southernmost point

**Answer:** B. The Standard Meridian at 82°30'E passes through it

**Explanation:** Mirzapur is used because the 82°30'E standard meridian passes through the area. Its importance here comes from longitude and timekeeping, not from an extreme-point location.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-020 — Mirzapur reference and central meridian

## 11. A question gives Mirzapur as a clue. Which geography fact is most likely being tested?

A. India's western coastline
B. The Tropic of Cancer
C. India's Standard Meridian
D. The southernmost point of India

**Answer:** C. India's Standard Meridian

**Explanation:** Mirzapur is the familiar place clue for 82°30'E, the Standard Meridian of India. Extreme-point and coastline questions use different locations.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-020 — Mirzapur reference and central meridian

## 12. If a map places Mirzapur on India's standard-time meridian, which longitude should be marked?

A. 23°30'N
B. 68°7'E
C. 97°25'E
D. 82°30'E

**Answer:** D. 82°30'E

**Explanation:** Mirzapur is used to locate India's 82°30'E Standard Meridian. The answer must therefore be a longitude, not the Tropic of Cancer or an extreme longitude.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-020 — Mirzapur reference and central meridian

## 13. What is the standard clock time followed across India called?

A. Indian Standard Time
B. Local Solar Time
C. Greenwich Local Time
D. Indian Seasonal Time

**Answer:** A. Indian Standard Time

**Explanation:** India follows Indian Standard Time, or IST, as a common national clock time. Local solar time can vary from place to place because longitude changes.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-021 — IST as one national standard time

## 14. Why is a common standard time useful for a country as wide as India?

A. It makes sunrise occur everywhere together
B. It keeps official time uniform across different longitudes
C. It removes longitude differences
D. It changes India's geographic position

**Answer:** B. It keeps official time uniform across different longitudes

**Explanation:** Local solar time varies with longitude, which would create different clock times from east to west. A common standard time keeps administration, transport and daily schedules uniform.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-021 — IST as one national standard time

## 15. Two Indian cities have different local solar times. What happens to their official clock time?

A. Each uses a separate national time zone
B. Only the eastern city follows IST
C. Both follow the same IST
D. Only the western city follows IST

**Answer:** C. Both follow the same IST

**Explanation:** Differences in local solar time do not create separate official time zones within India. Both cities use the same Indian Standard Time.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-021 — IST as one national standard time

## 16. Which statement separates local solar time from IST?

A. Both change every degree across India
B. IST varies with altitude
C. Local solar time is identical at all longitudes
D. Local solar time varies with longitude, while IST is one national reference time

**Answer:** D. Local solar time varies with longitude, while IST is one national reference time

**Explanation:** Local solar time depends on a place's longitude and therefore differs across the country. IST uses one selected meridian so official clocks remain uniform.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-021 — IST as one national standard time

## 17. A railway timetable lists one time standard for stations from Gujarat to Arunachal Pradesh. Which time is being used?

A. Indian Standard Time
B. Each station's local solar time
C. Greenwich Mean Time only
D. A separate eastern time zone

**Answer:** A. Indian Standard Time

**Explanation:** National timetables use IST rather than changing with local solar time at each station. This gives one consistent official time across India.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-021 — IST as one national standard time

## 18. What problem would arise if every Indian longitude used its own official local time?

A. Latitude would change
B. Schedules would vary continuously from east to west
C. The coastline would become longer
D. The Tropic of Cancer would shift

**Answer:** B. Schedules would vary continuously from east to west

**Explanation:** Local time changes gradually with longitude. Using each local time officially would make national schedules unnecessarily complicated, which is why a standard time is used.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-021 — IST as one national standard time

## 19. How far ahead of UTC is Indian Standard Time?

A. 4 hours 30 minutes
B. 5 hours
C. 5 hours 30 minutes
D. 6 hours 30 minutes

**Answer:** C. 5 hours 30 minutes

**Explanation:** IST is UTC+5:30. This means Indian Standard Time is five hours and thirty minutes ahead of Coordinated Universal Time.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-022 — IST and UTC/GMT offset (+5:30)

## 20. Which expression correctly represents Indian Standard Time?

A. UTC − 5:30
B. UTC + 2:00
C. UTC − 2:00
D. UTC + 5:30

**Answer:** D. UTC + 5:30

**Explanation:** Indian Standard Time is five hours and thirty minutes ahead of UTC. The plus sign is important because India lies east of the Greenwich reference meridian.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-022 — IST and UTC/GMT offset (+5:30)

## 21. If it is 06:00 UTC, what is the corresponding IST?

A. 11:30
B. 00:30
C. 10:30
D. 12:30

**Answer:** A. 11:30

**Explanation:** IST is 5 hours 30 minutes ahead of UTC. Adding 5:30 to 06:00 gives 11:30 IST, so the Indian clock reads late morning.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-022 — IST and UTC/GMT offset (+5:30)

## 22. If it is 18:00 IST, what is the corresponding UTC?

A. 23:30 UTC
B. 12:30 UTC
C. 13:30 UTC
D. 11:30 UTC

**Answer:** B. 12:30 UTC

**Explanation:** To convert IST to UTC, subtract 5 hours 30 minutes. Subtracting 5:30 from 18:00 gives 12:30 UTC, which is the same instant on the UTC clock.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-022 — IST and UTC/GMT offset (+5:30)

## 23. Why is IST ahead of UTC rather than behind it?

A. India lies west of Greenwich
B. India lies on the Equator
C. India lies east of the Greenwich meridian
D. India lies in the Southern Hemisphere

**Answer:** C. India lies east of the Greenwich meridian

**Explanation:** Local time becomes later as one moves eastward from Greenwich. India lies east of the Prime Meridian, so IST is ahead of UTC.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-022 — IST and UTC/GMT offset (+5:30)

## 24. At 23:30 UTC on Monday, what is the date and time in IST?

A. 18:00 on Monday
B. 05:00 on Monday
C. 04:00 on Tuesday
D. 05:00 on Tuesday

**Answer:** D. 05:00 on Tuesday

**Explanation:** Adding 5 hours 30 minutes to 23:30 gives 05:00 after midnight. Because the addition crosses 00:00, the date advances to Tuesday in India.

**Difficulty:** Hard
**QL:** GEO-LOC-001-QL-022 — IST and UTC/GMT offset (+5:30)

## 25. A difference of 1° in longitude corresponds to about how much difference in local solar time?

A. 4 minutes
B. 1 minute
C. 15 minutes
D. 60 minutes

**Answer:** A. 4 minutes

**Explanation:** Earth rotates 360° in about 24 hours, so it turns 15° in one hour. Dividing 60 minutes by 15 gives about 4 minutes of time for each degree of longitude.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-023 — Longitude–time relation: 1° = 4 minutes

## 26. Two places differ by 2° of longitude. About how much do their local solar times differ?

A. 2 minutes
B. 8 minutes
C. 4 minutes
D. 30 minutes

**Answer:** B. 8 minutes

**Explanation:** Each degree of longitude corresponds to about four minutes of local solar time. A 2° difference therefore produces about 8 minutes.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-023 — Longitude–time relation: 1° = 4 minutes

## 27. What local-time difference is produced by 5° of longitude?

A. 5 minutes
B. 10 minutes
C. 20 minutes
D. 25 minutes

**Answer:** C. 20 minutes

**Explanation:** At roughly four minutes per degree, 5° × 4 minutes equals 20 minutes. The eastern location would be ahead by this amount.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-023 — Longitude–time relation: 1° = 4 minutes

## 28. Two places have a local-time difference of 40 minutes. About how many degrees of longitude separate them?

A. 4°
B. 20°
C. 40°
D. 10°

**Answer:** D. 10°

**Explanation:** One degree corresponds to about four minutes. Dividing 40 minutes by 4 minutes per degree gives a longitude difference of about 10°.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-023 — Longitude–time relation: 1° = 4 minutes

## 29. A town 3° east of another town will have local solar time approximately how much ahead?

A. 12 minutes
B. 3 minutes
C. 7 minutes
D. 18 minutes

**Answer:** A. 12 minutes

**Explanation:** The time difference is about four minutes for each degree of longitude. Three degrees to the east therefore makes local solar time about 12 minutes ahead.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-023 — Longitude–time relation: 1° = 4 minutes

## 30. Place A is at 75°E and Place B at 82°E. If A's local time is 10:00, what is B's approximate local time?

A. 09:32
B. 10:28
C. 10:07
D. 10:42

**Answer:** B. 10:28

**Explanation:** B is 7° east of A, giving 7 × 4 = 28 minutes of time difference. Eastern places are ahead, so B's local time is about 10:28.

**Difficulty:** Hard
**QL:** GEO-LOC-001-QL-023 — Longitude–time relation: 1° = 4 minutes

## 31. A difference of 15° in longitude corresponds to about how much local-time difference?

A. 15 minutes
B. 30 minutes
C. 1 hour
D. 2 hours

**Answer:** C. 1 hour

**Explanation:** Earth rotates about 15° in one hour. Therefore two places 15° apart in longitude differ by about one hour in local solar time.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-024 — 15° longitude = 1 hour

## 32. How many degrees of longitude correspond to a two-hour local-time difference?

A. 2°
B. 15°
C. 60°
D. 30°

**Answer:** D. 30°

**Explanation:** Fifteen degrees of longitude corresponds to one hour of local solar time. A two-hour difference therefore represents about 30° of longitude.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-024 — 15° longitude = 1 hour

## 33. Places X and Y differ by 45° of longitude. About how much do their local solar times differ?

A. 3 hours
B. 45 minutes
C. 1 hour
D. 4 hours

**Answer:** A. 3 hours

**Explanation:** At 15° per hour, a 45° longitude difference gives 45 ÷ 15 = 3 hours. The eastern place would be ahead by that amount.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-024 — 15° longitude = 1 hour

## 34. A place is 30° east of another place. What is the approximate local-time difference?

A. 30 minutes ahead
B. 2 hours ahead
C. 1 hour ahead
D. 2 hours behind

**Answer:** B. 2 hours ahead

**Explanation:** Thirty degrees equals two groups of 15°, and each 15° represents about one hour. Since the place is east, its local solar time is about two hours ahead.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-024 — 15° longitude = 1 hour

## 35. Which longitude difference would produce about a 30-minute local-time difference?

A. 15°
B. 30°
C. 7.5°
D. 2°

**Answer:** C. 7.5°

**Explanation:** One hour corresponds to 15°, so half an hour corresponds to half of 15°. That gives a longitude difference of about 7.5°.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-024 — 15° longitude = 1 hour

## 36. It is 09:00 local solar time at 60°E. What is the approximate local solar time at 90°E?

A. 07:00
B. 09:30
C. 10:00
D. 11:00

**Answer:** D. 11:00

**Explanation:** The longitude difference is 30°, equal to two hours. Since 90°E lies east of 60°E, its local solar time is two hours ahead, or about 11:00.

**Difficulty:** Hard
**QL:** GEO-LOC-001-QL-024 — 15° longitude = 1 hour

## 37. Which place has the later local solar time: one farther east or one farther west?

A. The place farther east
B. The place farther west
C. Both always have identical local time
D. Latitude alone decides it

**Answer:** A. The place farther east

**Explanation:** Earth rotates from west to east, so places farther east encounter the Sun's daily position earlier. Their local solar time is therefore ahead of places farther west.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-025 — Eastern places are ahead in local solar time

## 38. If Town B lies east of Town A, how does B's local solar time compare with A's?

A. B is behind A
B. B is ahead of A
C. Both must be identical
D. B's time depends only on altitude

**Answer:** B. B is ahead of A

**Explanation:** Local solar time increases eastward with longitude. A town to the east therefore has a later local solar time than a town to the west.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-025 — Eastern places are ahead in local solar time

## 39. Sunrise is generally experienced earlier by local solar time in eastern India than in western India. Which factor explains this?

A. Difference in national currency
B. Difference in coastline length
C. Difference in longitude
D. Difference in state population

**Answer:** C. Difference in longitude

**Explanation:** Eastern locations are at greater east longitudes and reach the Sun's daily positions earlier. Longitude, not administrative or demographic differences, explains the local-time shift.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-025 — Eastern places are ahead in local solar time

## 40. Town P is at 70°E and Town Q at 90°E. Which town is ahead in local solar time?

A. Town P
B. Both are identical
C. Neither has local solar time
D. Town Q

**Answer:** D. Town Q

**Explanation:** Town Q lies 20° farther east than Town P. Eastern longitudes are ahead in local solar time, so Q's local time is later.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-025 — Eastern places are ahead in local solar time

## 41. A traveller moves westward across India. What happens to local solar time?

A. It becomes earlier
B. It becomes later
C. It remains fixed by longitude
D. It changes only with latitude

**Answer:** A. It becomes earlier

**Explanation:** Moving west means moving to smaller east longitudes. Local solar time becomes earlier as one travels westward, even though official clocks continue to show IST.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-025 — Eastern places are ahead in local solar time

## 42. Two towns follow the same IST, but Town X is 12° east of Town Y. Which statement is correct?

A. X's IST is 48 minutes ahead
B. X's local solar time is about 48 minutes ahead of Y
C. Y's local solar time is 48 minutes ahead
D. Their local solar times must be identical

**Answer:** B. X's local solar time is about 48 minutes ahead of Y

**Explanation:** Twelve degrees of longitude gives about 12 × 4 = 48 minutes. Official IST remains the same, but the eastern town X has local solar time about 48 minutes ahead.

**Difficulty:** Hard
**QL:** GEO-LOC-001-QL-025 — Eastern places are ahead in local solar time

## 43. What is the approximate local-time difference between Gujarat and Arunachal Pradesh?

A. Thirty minutes
B. One hour
C. Two hours
D. Four hours

**Answer:** C. Two hours

**Explanation:** India's east–west extent produces an approximate two-hour difference in local solar time between Gujarat and Arunachal Pradesh. Official clocks in both places still follow IST.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-026 — Gujarat–Arunachal local-time difference

## 44. Which state has the earlier local solar time: Gujarat or Arunachal Pradesh?

A. Arunachal Pradesh
B. Both are always identical
C. Neither uses local solar time
D. Gujarat

**Answer:** D. Gujarat

**Explanation:** Gujarat lies farther west, so its local solar time is earlier. Arunachal Pradesh lies much farther east and is roughly two hours ahead in local solar time.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-026 — Gujarat–Arunachal local-time difference

## 45. Why can sunrise occur much earlier in Arunachal Pradesh than in Gujarat while official clocks show the same time?

A. Longitude changes local solar time, but both states use IST
B. Arunachal Pradesh uses a separate national time zone
C. Gujarat lies in another hemisphere
D. IST changes with altitude

**Answer:** A. Longitude changes local solar time, but both states use IST

**Explanation:** Arunachal Pradesh lies much farther east, so the Sun's daily cycle is earlier there in local solar time. Both states nevertheless use the same official IST.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-026 — Gujarat–Arunachal local-time difference

## 46. The Gujarat–Arunachal local-time contrast shows India's large extent in which direction?

A. North to south
B. East to west
C. Vertical altitude
D. Ocean depth

**Answer:** B. East to west

**Explanation:** The time difference arises because Gujarat and Arunachal Pradesh are widely separated in longitude. Longitude measures east–west position, so the contrast reflects India's east–west extent.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-026 — Gujarat–Arunachal local-time difference

## 47. If local noon occurs earlier in Arunachal Pradesh than Gujarat, which state lies farther east?

A. Gujarat
B. Both lie on the same longitude
C. Arunachal Pradesh
D. The comparison cannot be made

**Answer:** C. Arunachal Pradesh

**Explanation:** Eastern places reach local noon earlier by local solar time. Arunachal Pradesh is east of Gujarat, matching the observed time difference.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-026 — Gujarat–Arunachal local-time difference

## 48. If Gujarat's local solar time is about 08:00, what would the usual two-hour approximation suggest for Arunachal Pradesh?

A. About 06:00
B. About 08:30
C. About 12:00
D. About 10:00

**Answer:** D. About 10:00

**Explanation:** Arunachal Pradesh is about two hours ahead of Gujarat in local solar time. Adding two hours to 08:00 gives an approximate local time of 10:00.

**Difficulty:** Hard
**QL:** GEO-LOC-001-QL-026 — Gujarat–Arunachal local-time difference

## 49. A place 5° east of another place is ahead in local solar time by about how much?

A. 20 minutes
B. 5 minutes
C. 10 minutes
D. 1 hour

**Answer:** A. 20 minutes

**Explanation:** Each degree of longitude represents about four minutes of local solar time. Five degrees east therefore gives 5 × 4 = 20 minutes ahead.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-027 — Mixed IST / longitude-time calculations

## 50. Which calculation converts 10:00 UTC to IST?

A. Subtract 5 hours 30 minutes
B. Add 5 hours 30 minutes
C. Add 2 hours
D. Subtract 30 minutes

**Answer:** B. Add 5 hours 30 minutes

**Explanation:** IST is UTC+5:30. Therefore converting from UTC to IST requires adding five hours and thirty minutes to the UTC reading.

**Difficulty:** Easy
**QL:** GEO-LOC-001-QL-027 — Mixed IST / longitude-time calculations

## 51. It is 10:00 local solar time at 75°E. What is the approximate local solar time at 82.5°E?

A. 09:30
B. 10:07
C. 10:30
D. 11:00

**Answer:** C. 10:30

**Explanation:** The longitude difference is 7.5°, which equals about 30 minutes of time. Since 82.5°E is east of 75°E, its local solar time is about 10:30.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-027 — Mixed IST / longitude-time calculations

## 52. At 12:00 UTC, what time does an Indian clock following IST show?

A. 06:30
B. 12:30
C. 18:00
D. 17:30

**Answer:** D. 17:30

**Explanation:** Indian Standard Time is five hours and thirty minutes ahead of UTC. Adding 5:30 to 12:00 gives 17:30 IST, or 5:30 in the evening.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-027 — Mixed IST / longitude-time calculations

## 53. Two places differ by 22.5° in longitude. What is their approximate local-time difference?

A. 1 hour 30 minutes
B. 22.5 minutes
C. 45 minutes
D. 2 hours 15 minutes

**Answer:** A. 1 hour 30 minutes

**Explanation:** Fifteen degrees corresponds to one hour, and 7.5° adds another 30 minutes. A 22.5° difference therefore gives about 1 hour 30 minutes.

**Difficulty:** Medium
**QL:** GEO-LOC-001-QL-027 — Mixed IST / longitude-time calculations

## 54. Place A is 10° west of the Standard Meridian. When the local solar time at 82°30'E is 12:00, what is A's approximate local solar time?

A. 12:40
B. 11:20
C. 11:50
D. 10:40

**Answer:** B. 11:20

**Explanation:** Ten degrees of longitude corresponds to about 40 minutes. A lies west of the Standard Meridian, so its local solar time is earlier: about 11:20.

**Difficulty:** Hard
**QL:** GEO-LOC-001-QL-027 — Mixed IST / longitude-time calculations
