...
import { generateChemistryLocalizedCpV1, SCI_CHEMISTRY_WAVE1_SUPPORTED_CPS_V1 } from "./sci-chemistry-localization-generator-v1";

const nativeLocales = ["hi", "pa"] as const;
const forbiddenEnglishProse = /\b(which|what|why|how|select|correct|incorrect|statement|option|answer|because|contains|particles|solution|atom|molecule|electron|proton|neutron|solid|liquid|gas)\b/i;
...
for (const cpId of SCI_CHEMISTRY_WAVE1_SUPPORTED_CPS_V1) {
...
const nativeQuestions = SCI_CHEMISTRY_WAVE1_SUPPORTED_CPS_V1.flatMap((cpId) => nativeLocales.flatMap((locale) => [...generateChemistryLocalizedCpV1(cpId, locale)]));
assert.equal(nativeQuestions.length, 240, "Wave 1 must expose 240 native localized questions");
assert.equal(new Set(nativeQuestions.map((q) => q.questionId)).size, 240, "Wave 1 localized question IDs must be unique");

console.log(JSON.stringify({
  status: "PASS",
  wave: "Chemistry localization V1 Wave 1",
  cps: [...SCI_CHEMISTRY_WAVE1_SUPPORTED_CPS_V1],
  nativeLocales: [...nativeLocales],
  questionsPerLocale: 120,
  nativeQuestions: 240,
  lifecycle: "review-only / runtime closed",
  nextGate: "human review of CP011-012 Hindi/Punjabi",
}, null, 2));
