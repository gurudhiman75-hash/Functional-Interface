...
import { generateChemistryLocalizedCpV1, SCI_CHEMISTRY_WAVE2_SUPPORTED_CPS_V1 } from "./sci-chemistry-localization-generator-v1";

const nativeLocales = ["hi", "pa"] as const;
const forbiddenEnglishProse = /\b(which|what|why|how|select|correct|incorrect|statement|option|answer|because|contains|particles|solution|atom|molecule|electron|proton|neutron|solid|liquid|gas|reaction|compound|element|period|group|metal|non-metal)\b/i;
...
for (const cpId of SCI_CHEMISTRY_WAVE2_SUPPORTED_CPS_V1) {
  const en = generateChemistryLocalizedCpV1(cpId, "en");
  assert.equal(en.length, 60, `${cpId}/en: expected 60`);

  for (const locale of nativeLocales) {
    const localized = generateChemistryLocalizedCpV1(cpId, locale);
    ...
    localized.forEach((q, index) => {
      ...
      assert.ok(q.explanation.length >= 18, `${q.questionId}: explanation too thin`);
      ...
      assert.ok(!ids.has(q.questionId), `${q.questionId}: duplicate localized ID`);
      ids.add(q.questionId);
      const stemKey = q.stem.trim().replace(/\s+/g, " ").toLowerCase();
      assert.ok(!stems.has(stemKey), `${q.questionId}: duplicate localized stem`);
      stems.add(stemKey);
      positions[q.correctIndex] += 1;
      difficulty[q.difficulty] += 1;
    });

    assert.deepEqual(positions, [15, 15, 15, 15], `${cpId}/${locale}: answer-position profile drift`);
    assert.deepEqual(difficulty, { Easy: 18, Medium: 30, Hard: 12 }, `${cpId}/${locale}: difficulty profile drift`);
  }
}

const nativeQuestions = SCI_CHEMISTRY_WAVE2_SUPPORTED_CPS_V1.flatMap((cpId) =>
  nativeLocales.flatMap((locale) => [...generateChemistryLocalizedCpV1(cpId, locale)])
);
assert.equal(nativeQuestions.length, 240, "Wave 2 must expose 240 native localized questions");
assert.equal(new Set(nativeQuestions.map((q) => q.questionId)).size, 240, "Wave 2 localized question IDs must be unique");

console.log(JSON.stringify({
  status: "PASS",
  wave: "Chemistry localization V1 Wave 2",
  cps: [...SCI_CHEMISTRY_WAVE2_SUPPORTED_CPS_V1],
  nativeLocales: [...nativeLocales],
  questionsPerLocale: 120,
  nativeQuestions: 240,
  lifecycle: "review-only / runtime closed",
  nextGate: "human review of CP013-014 Hindi/Punjabi",
}, null, 2));
