...
import { generateChemistryLocalizedCpV1, SCI_CHEMISTRY_WAVE4_SUPPORTED_CPS_V1 } from "./sci-chemistry-localization-generator-v1";
...
for (const cpId of SCI_CHEMISTRY_WAVE4_SUPPORTED_CPS_V1) {
...
const nativeQuestions = SCI_CHEMISTRY_WAVE4_SUPPORTED_CPS_V1.flatMap((cpId) =>
  nativeLocales.flatMap((locale) => [...generateChemistryLocalizedCpV1(cpId, locale)])
);
assert.equal(nativeQuestions.length, 240, "Wave 4 must expose 240 native localized questions");
assert.equal(new Set(nativeQuestions.map((q) => q.questionId)).size, 240, "Wave 4 localized question IDs must be unique");

console.log(JSON.stringify({
  status: "PASS",
  wave: "Chemistry localization V1 Wave 4",
  cps: [...SCI_CHEMISTRY_WAVE4_SUPPORTED_CPS_V1],
  nativeLocales: [...nativeLocales],
  questionsPerLocale: 120,
  nativeQuestions: 240,
  lifecycle: "review-only / runtime closed",
  nextGate: "human review of CP017-018 Hindi/Punjabi",
}, null, 2));
