...
import { PART_1 } from "./sci-cp013-review-v1-part1";
import { PART_2 } from "./sci-cp013-review-v1-part2";
import { PART_3 } from "./sci-cp013-review-v1-part3";
import { PART_4 } from "./sci-cp013-review-v1-part4";
...
export type SciCp013ReviewQuestion = {
  questionId:string; chapterId:"SCI-001"; cpId:"SCI-CP-013"; qlId:string; qlName:string;
...
export const SCI_CP013_QL_NAMES_V1: Record<number,string> = {
  1:"Element and compound classification",
  2:"Common element symbols and familiar formulae",
  3:"Metals, non-metals, metalloids and table regions",
  4:"Groups, periods and table structure",
  5:"Important periodic families",
  6:"Electronic configuration and periodic position",
  7:"Basic periodic trends",
  8:"Development of periodic classification",
  9:"Integrated periodic reasoning",
...
export const SCI_CP013_REVIEW_V1: readonly SciCp013ReviewQuestion[] = Object.freeze(
  REVIEW_SPECS.map((spec,index)=>{
    ...
    return Object.freeze({
      ...
      chapterId:"SCI-001" as const, cpId:"SCI-CP-013" as const,
      qlId:`SCI-013-QL-${pad3(ql)}`, qlName:SCI_CP013_QL_NAMES_V1[ql],
      difficulty, stem, options:buildOptions(answer,distractors,correctIndex), correctIndex,
...
export function validateSciCp013ReviewV1(){
  ...
  for(const q of SCI_CP013_REVIEW_V1){
  ...
  for(let ql=1;ql<=10;ql++){const id=`SCI-013-QL-${pad3(ql)}`;if(qlCounts[id]!==6)errors.push(`${id}: expected 6, found ${qlCounts[id]??0}`);}
  ...
  return{valid:errors.length===0,errors,totalQuestions:SCI_CP013_REVIEW_V1.length,qlCounts,difficultyCounts,answerPositionCounts};
}
