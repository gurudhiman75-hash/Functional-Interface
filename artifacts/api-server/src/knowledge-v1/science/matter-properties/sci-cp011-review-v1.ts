...
import { PART_1 } from "./sci-cp011-review-v1-part1";
import { PART_2 } from "./sci-cp011-review-v1-part2";
import { PART_3 } from "./sci-cp011-review-v1-part3";
import { PART_4 } from "./sci-cp011-review-v1-part4";
...
export type SciCp011ReviewQuestion = {
  questionId:string; chapterId:"SCI-001"; cpId:"SCI-CP-011"; qlId:string; qlName:string;
...
const SOURCE_IDS = Object.freeze(["NCERT-SCIENCE-IX-MATTER-SURROUNDINGS", "NCERT-SCIENCE-IX-IS-MATTER-PURE", "NIOS-SECONDARY-SCIENCE-MATTER"]);

export const SCI_CP011_QL_NAMES_V1: Record<number,string> = {
  1: "States of matter and basic physical properties",
  2: "Particle model and physical changes",
  3: "Pure substances, mixtures and solution basics",
  4: "Change of state, latent heat and pressure effects",
  5: "Solutions, concentration, saturation and solubility",
  6: "Suspensions, colloids and Tyndall effect",
  7: "Separation techniques",
  8: "Evaporation, diffusion, sublimation and crystallization",
  9: "Integrated statement reasoning",
  10: "Mixed matter and separation application",
};
...
export function validateSciCp011ReviewV1():SciCp011Validation{
  ...
  for(const q of SCI_CP011_REVIEW_V1){
  ...
  for(let ql=1;ql<=10;ql++){const id=`SCI-011-QL-${pad3(ql)}`;if(qlCounts[id]!==6)errors.push(`${id}: expected 6, found ${qlCounts[id]??0}`);}
  ...
  return{valid:errors.length===0,errors,totalQuestions:SCI_CP011_REVIEW_V1.length,qlCounts,difficultyCounts,answerPositionCounts};
}
