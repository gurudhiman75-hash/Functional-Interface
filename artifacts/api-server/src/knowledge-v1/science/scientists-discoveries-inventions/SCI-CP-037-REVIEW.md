# SCI-CP-037 — Scientists, Discoveries & Inventions — Review V1

Review-only candidate. 60 questions; 10 QLs × 6; Easy 18 / Medium 30 / Hard 12; A/B/C/D balanced 15 each.

## Q01 · Easy · Mechanics, pressure and classical physics

The laws of motion are most closely associated with:

A. Isaac Newton
B. Archimedes
C. Blaise Pascal
D. Galileo Galilei

**Answer:** A. Isaac Newton

**Explanation:** Newton formulated the three laws of motion that form the basis of classical mechanics.

**Source fact:** `NEWTON-LAWS-OF-MOTION`

## Q02 · Easy · Mechanics, pressure and classical physics

The principle of buoyancy is associated with:

A. Isaac Newton
B. Archimedes
C. Evangelista Torricelli
D. Robert Boyle

**Answer:** B. Archimedes

**Explanation:** Archimedes' principle relates buoyant force to the weight of displaced fluid.

**Source fact:** `ARCHIMEDES-PRINCIPLE`

## Q03 · Medium · Mechanics, pressure and classical physics

Pascal's law, used in hydraulic systems, is associated with:

A. Daniel Bernoulli
B. Michael Faraday
C. Blaise Pascal
D. James Watt

**Answer:** C. Blaise Pascal

**Explanation:** Pascal stated that pressure applied to a confined fluid is transmitted throughout the fluid.

**Source fact:** `PASCAL-LAW`

## Q04 · Medium · Mechanics, pressure and classical physics

The mercury barometer is historically associated with:

A. Galileo Galilei
B. Robert Hooke
C. Christiaan Huygens
D. Evangelista Torricelli

**Answer:** D. Evangelista Torricelli

**Explanation:** Torricelli demonstrated atmospheric pressure using a mercury column and developed the barometer.

**Source fact:** `TORRICELLI-BAROMETER`

## Q05 · Medium · Mechanics, pressure and classical physics

Which scientist's experiments strongly challenged the old idea that heavier bodies must fall much faster than lighter ones?

A. Galileo Galilei
B. Gregor Mendel
C. Louis Pasteur
D. J. J. Thomson

**Answer:** A. Galileo Galilei

**Explanation:** Galileo's work on falling bodies helped establish the modern study of motion.

**Source fact:** `GALILEO-FALLING-BODIES`

## Q06 · Hard · Mechanics, pressure and classical physics

A question mentions universal gravitation and the three laws of motion. Which scientist correctly matches both contributions?

A. Archimedes
B. Isaac Newton
C. Blaise Pascal
D. Evangelista Torricelli

**Answer:** B. Isaac Newton

**Explanation:** Newton is associated with both the laws of motion and the law of universal gravitation.

**Source fact:** `NEWTON-MOTION-GRAVITATION`

## Q07 · Easy · Electricity and magnetism

Ohm's law is named after:

A. Alessandro Volta
B. Michael Faraday
C. Georg Simon Ohm
D. André-Marie Ampère

**Answer:** C. Georg Simon Ohm

**Explanation:** Ohm's law relates voltage, current and resistance in an electrical conductor under suitable conditions.

**Source fact:** `OHM-LAW`

## Q08 · Easy · Electricity and magnetism

The electric battery or voltaic pile is associated with:

A. Hans Christian Oersted
B. James Clerk Maxwell
C. Heinrich Hertz
D. Alessandro Volta

**Answer:** D. Alessandro Volta

**Explanation:** Volta developed the voltaic pile, an early source of continuous electric current.

**Source fact:** `VOLTA-BATTERY`

## Q09 · Medium · Electricity and magnetism

Who discovered the magnetic effect of electric current in a wire?

A. Hans Christian Oersted
B. Michael Faraday
C. Georg Ohm
D. Thomas Edison

**Answer:** A. Hans Christian Oersted

**Explanation:** Oersted observed that an electric current deflects a magnetic compass needle.

**Source fact:** `OERSTED-MAGNETIC-EFFECT-CURRENT`

## Q10 · Medium · Electricity and magnetism

Electromagnetic induction is most closely associated with:

A. Alessandro Volta
B. Michael Faraday
C. Blaise Pascal
D. J. J. Thomson

**Answer:** B. Michael Faraday

**Explanation:** Faraday showed that a changing magnetic environment can induce an electric current.

**Source fact:** `FARADAY-INDUCTION`

## Q11 · Medium · Electricity and magnetism

The SI unit ampere is named after the scientist associated with the study of:

A. Radioactivity only
B. Plant genetics
C. Electric currents and their magnetic effects
D. Atmospheric pressure

**Answer:** C. Electric currents and their magnetic effects

**Explanation:** André-Marie Ampère made major contributions to electrodynamics and the magnetic effects of current.

**Source fact:** `AMPERE-ELECTRODYNAMICS`

## Q12 · Hard · Electricity and magnetism

Which sequence correctly matches scientist and contribution?

A. Volta — electromagnetic induction; Faraday — voltaic pile
B. Ohm — radio waves; Hertz — resistance law
C. Ampère — barometer; Torricelli — electric current
D. Oersted — current's magnetic effect; Faraday — electromagnetic induction

**Answer:** D. Oersted — current's magnetic effect; Faraday — electromagnetic induction

**Explanation:** Oersted linked current with magnetism, while Faraday established electromagnetic induction.

**Source fact:** `OERSTED-FARADAY-DISTINCTION`

## Q13 · Easy · Light, radiation and modern physics

X-rays were discovered by:

A. Wilhelm Conrad Röntgen
B. Henri Becquerel
C. Ernest Rutherford
D. Niels Bohr

**Answer:** A. Wilhelm Conrad Röntgen

**Explanation:** Röntgen discovered X-rays while studying electrical discharges in vacuum tubes.

**Source fact:** `RONTGEN-XRAYS`

## Q14 · Easy · Light, radiation and modern physics

The discovery of natural radioactivity is associated with:

A. Marie Curie
B. Henri Becquerel
C. J. J. Thomson
D. Albert Einstein

**Answer:** B. Henri Becquerel

**Explanation:** Becquerel discovered spontaneous radiation from uranium salts.

**Source fact:** `BECQUEREL-RADIOACTIVITY`

## Q15 · Medium · Light, radiation and modern physics

The electron was discovered by:

A. Ernest Rutherford
B. James Chadwick
C. J. J. Thomson
D. Niels Bohr

**Answer:** C. J. J. Thomson

**Explanation:** Thomson identified the electron through cathode-ray experiments.

**Source fact:** `THOMSON-ELECTRON`

## Q16 · Medium · Light, radiation and modern physics

The atomic nucleus was revealed through the gold-foil experiment led by:

A. J. J. Thomson
B. Max Planck
C. Wilhelm Röntgen
D. Ernest Rutherford

**Answer:** D. Ernest Rutherford

**Explanation:** Rutherford's scattering experiment showed that most positive charge and mass are concentrated in a tiny nucleus.

**Source fact:** `RUTHERFORD-NUCLEUS`

## Q17 · Medium · Light, radiation and modern physics

The neutron was discovered by:

A. James Chadwick
B. Niels Bohr
C. Henri Becquerel
D. Albert Einstein

**Answer:** A. James Chadwick

**Explanation:** Chadwick identified the neutron as an electrically neutral particle in the atomic nucleus.

**Source fact:** `CHADWICK-NEUTRON`

## Q18 · Hard · Light, radiation and modern physics

Which pair is correctly distinguished?

A. Röntgen — neutron; Chadwick — X-rays
B. Becquerel — natural radioactivity; Röntgen — X-rays
C. Rutherford — electron; Thomson — nucleus
D. Bohr — radioactivity; Becquerel — atomic model

**Answer:** B. Becquerel — natural radioactivity; Röntgen — X-rays

**Explanation:** Becquerel discovered natural radioactivity, whereas Röntgen discovered X-rays.

**Source fact:** `BECQUEREL-RONTGEN-DISTINCTION`

## Q19 · Easy · Chemistry and atomic ideas

The periodic table is most closely associated with:

A. John Dalton
B. Antoine Lavoisier
C. Dmitri Mendeleev
D. Amedeo Avogadro

**Answer:** C. Dmitri Mendeleev

**Explanation:** Mendeleev organized elements into a periodic table and predicted properties of undiscovered elements.

**Source fact:** `MENDELEEV-PERIODIC-TABLE`

## Q20 · Easy · Chemistry and atomic ideas

The modern atomic theory of matter is strongly associated with:

A. Robert Hooke
B. Louis Pasteur
C. Charles Darwin
D. John Dalton

**Answer:** D. John Dalton

**Explanation:** Dalton proposed an influential atomic theory explaining matter in terms of atoms.

**Source fact:** `DALTON-ATOMIC-THEORY`

## Q21 · Medium · Chemistry and atomic ideas

The law of conservation of mass in chemistry is closely associated with:

A. Antoine Lavoisier
B. Dmitri Mendeleev
C. Alfred Nobel
D. Fritz Haber

**Answer:** A. Antoine Lavoisier

**Explanation:** Lavoisier's quantitative experiments helped establish conservation of mass in chemical reactions.

**Source fact:** `LAVOISIER-CONSERVATION-MASS`

## Q22 · Medium · Chemistry and atomic ideas

Avogadro's hypothesis connects equal volumes of gases at the same temperature and pressure with equal numbers of:

A. Neutrons
B. Molecules
C. Electrons only
D. Atomic nuclei only

**Answer:** B. Molecules

**Explanation:** Amedeo Avogadro proposed that equal gas volumes under the same conditions contain equal numbers of molecules.

**Source fact:** `AVOGADRO-HYPOTHESIS`

## Q23 · Medium · Chemistry and atomic ideas

The industrial synthesis of ammonia by combining nitrogen and hydrogen is associated with the:

A. Solvay process
B. Hall process
C. Haber process
D. Bessemer process

**Answer:** C. Haber process

**Explanation:** Fritz Haber developed the chemical basis for ammonia synthesis; industrial implementation was later scaled by Carl Bosch.

**Source fact:** `HABER-AMMONIA`

## Q24 · Hard · Chemistry and atomic ideas

Which statement gives the more accurate historical match?

A. Bosch discovered the electron and Haber the neutron
B. Haber created the periodic table alone
C. Mendeleev invented the Haber process
D. Haber developed ammonia synthesis chemistry, while Bosch helped develop the large-scale industrial process

**Answer:** D. Haber developed ammonia synthesis chemistry, while Bosch helped develop the large-scale industrial process

**Explanation:** The ammonia process is commonly called Haber–Bosch because Haber developed the chemistry and Bosch helped industrialize it.

**Source fact:** `HABER-BOSCH-DISTINCTION`

## Q25 · Easy · Cells, microbiology and biological discovery

The term 'cell' was introduced after microscopic observations by:

A. Robert Hooke
B. Louis Pasteur
C. Robert Koch
D. Anton van Leeuwenhoek

**Answer:** A. Robert Hooke

**Explanation:** Hooke used the term 'cell' after observing compartments in cork.

**Source fact:** `HOOKE-CELL`

## Q26 · Easy · Cells, microbiology and biological discovery

Early observations of living microorganisms with microscopes are associated with:

A. Robert Hooke
B. Anton van Leeuwenhoek
C. Charles Darwin
D. Gregor Mendel

**Answer:** B. Anton van Leeuwenhoek

**Explanation:** Leeuwenhoek observed microorganisms such as bacteria and protozoa using powerful simple microscopes.

**Source fact:** `LEEUWENHOEK-MICROORGANISMS`

## Q27 · Medium · Cells, microbiology and biological discovery

Pasteurization is named after:

A. Robert Koch
B. Edward Jenner
C. Louis Pasteur
D. Alexander Fleming

**Answer:** C. Louis Pasteur

**Explanation:** Pasteur developed heat-treatment methods to reduce spoilage-causing and harmful microorganisms.

**Source fact:** `PASTEUR-PASTEURIZATION`

## Q28 · Medium · Cells, microbiology and biological discovery

Robert Koch is remembered for work showing that specific microorganisms can cause:

A. Planetary motion
B. Radioactivity
C. Inheritance of pea traits
D. Specific diseases

**Answer:** D. Specific diseases

**Explanation:** Koch's work linked particular microorganisms with particular infectious diseases.

**Source fact:** `KOCH-DISEASE-MICROBES`

## Q29 · Medium · Cells, microbiology and biological discovery

Which scientist's swan-neck flask experiments supported the idea that microorganisms do not arise spontaneously from sterile broth?

A. Louis Pasteur
B. Gregor Mendel
C. Robert Brown
D. Isaac Newton

**Answer:** A. Louis Pasteur

**Explanation:** Pasteur's experiments showed that microbial growth came from contamination rather than spontaneous generation.

**Source fact:** `PASTEUR-SWAN-NECK`

## Q30 · Hard · Cells, microbiology and biological discovery

Which distinction is correct?

A. Leeuwenhoek discovered X-rays; Hooke discovered radioactivity
B. Hooke named cells from cork observations; Leeuwenhoek observed living microorganisms
C. Hooke developed vaccination; Leeuwenhoek discovered insulin
D. Both scientists are known for the periodic table

**Answer:** B. Hooke named cells from cork observations; Leeuwenhoek observed living microorganisms

**Explanation:** Hooke described cork compartments as cells, while Leeuwenhoek later observed living microscopic organisms.

**Source fact:** `HOOKE-LEEUWENHOEK-DISTINCTION`

## Q31 · Easy · Evolution and genetics

The theory of evolution by natural selection is most closely associated with:

A. Gregor Mendel
B. Louis Pasteur
C. Charles Darwin
D. Edward Jenner

**Answer:** C. Charles Darwin

**Explanation:** Darwin developed the theory of evolution by natural selection, with Alfred Russel Wallace independently reaching a similar idea.

**Source fact:** `DARWIN-NATURAL-SELECTION`

## Q32 · Easy · Evolution and genetics

Gregor Mendel is known for experiments on:

A. X-rays
B. Electric current
C. Vaccination
D. Inheritance in pea plants

**Answer:** D. Inheritance in pea plants

**Explanation:** Mendel's pea experiments established basic principles of inheritance.

**Source fact:** `MENDEL-PEA-INHERITANCE`

## Q33 · Medium · Evolution and genetics

Who independently developed the idea of natural selection at about the same time as Darwin?

A. Alfred Russel Wallace
B. Robert Hooke
C. James Chadwick
D. John Dalton

**Answer:** A. Alfred Russel Wallace

**Explanation:** Wallace independently formulated natural selection, prompting the joint presentation of their ideas.

**Source fact:** `WALLACE-NATURAL-SELECTION`

## Q34 · Medium · Evolution and genetics

The double-helix model of DNA was proposed by:

A. Gregor Mendel and Charles Darwin
B. James Watson and Francis Crick
C. Robert Hooke and Leeuwenhoek
D. Banting and Best

**Answer:** B. James Watson and Francis Crick

**Explanation:** Watson and Crick proposed the double-helix model using crucial experimental evidence from several researchers.

**Source fact:** `WATSON-CRICK-DOUBLE-HELIX`

## Q35 · Medium · Evolution and genetics

Rosalind Franklin's X-ray diffraction work contributed importantly to understanding the:

A. Law of motion
B. Periodic table
C. Structure of DNA
D. Smallpox vaccine

**Answer:** C. Structure of DNA

**Explanation:** Franklin's diffraction data provided key evidence about DNA's helical structure and dimensions.

**Source fact:** `FRANKLIN-DNA-XRAY`

## Q36 · Hard · Evolution and genetics

Which statement best reflects the history of the DNA double-helix model?

A. Franklin alone invented DNA
B. Mendel proposed the double helix from pea plants
C. Darwin discovered DNA using fossils
D. Watson and Crick proposed the model using evidence that included Franklin's X-ray diffraction data

**Answer:** D. Watson and Crick proposed the model using evidence that included Franklin's X-ray diffraction data

**Explanation:** The model was proposed by Watson and Crick, while Franklin's experimental work was a crucial part of the evidence base.

**Source fact:** `DNA-ATTRIBUTION-NUANCE`

## Q37 · Easy · Medicine, vaccines and human physiology

The first successful smallpox vaccine is associated with:

A. Edward Jenner
B. Alexander Fleming
C. Robert Koch
D. William Harvey

**Answer:** A. Edward Jenner

**Explanation:** Jenner demonstrated vaccination against smallpox using material related to cowpox.

**Source fact:** `JENNER-SMALLPOX-VACCINE`

## Q38 · Easy · Medicine, vaccines and human physiology

Penicillin was discovered by:

A. Edward Jenner
B. Alexander Fleming
C. Louis Pasteur
D. Karl Landsteiner

**Answer:** B. Alexander Fleming

**Explanation:** Fleming observed the antibacterial effect of Penicillium mould, leading to the discovery of penicillin.

**Source fact:** `FLEMING-PENICILLIN`

## Q39 · Medium · Medicine, vaccines and human physiology

The circulation of blood through the body was described systematically by:

A. Robert Hooke
B. John Dalton
C. William Harvey
D. Michael Faraday

**Answer:** C. William Harvey

**Explanation:** Harvey demonstrated that the heart pumps blood through a circulatory system.

**Source fact:** `HARVEY-BLOOD-CIRCULATION`

## Q40 · Medium · Medicine, vaccines and human physiology

The ABO blood-group system was discovered by:

A. Alexander Fleming
B. Jonas Salk
C. Edward Jenner
D. Karl Landsteiner

**Answer:** D. Karl Landsteiner

**Explanation:** Landsteiner identified the ABO blood groups, making safer transfusion matching possible.

**Source fact:** `LANDSTEINER-ABO`

## Q41 · Medium · Medicine, vaccines and human physiology

The discovery and early therapeutic development of insulin is closely associated with Frederick Banting and:

A. Charles Best
B. James Watt
C. Robert Boyle
D. Niels Bohr

**Answer:** A. Charles Best

**Explanation:** Banting and Best played central roles in the work that led to insulin treatment, with important contributions from their Toronto colleagues.

**Source fact:** `BANTING-BEST-INSULIN`

## Q42 · Hard · Medicine, vaccines and human physiology

Which pair is correctly matched with a medical contribution?

A. Jenner — insulin; Banting — smallpox vaccine
B. Landsteiner — ABO blood groups; Fleming — penicillin
C. Harvey — X-rays; Röntgen — circulation
D. Koch — periodic table; Mendeleev — tuberculosis bacterium

**Answer:** B. Landsteiner — ABO blood groups; Fleming — penicillin

**Explanation:** Landsteiner identified ABO blood groups, while Fleming discovered penicillin's antibacterial effect.

**Source fact:** `MEDICAL-SCIENTIST-DISTINCTION`

## Q43 · Easy · Communication and familiar inventions

The telephone is historically associated with:

A. Guglielmo Marconi
B. John Logie Baird
C. Alexander Graham Bell
D. James Watt

**Answer:** C. Alexander Graham Bell

**Explanation:** Bell is widely credited with key development and patenting of the telephone.

**Source fact:** `BELL-TELEPHONE`

## Q44 · Easy · Communication and familiar inventions

Practical radio communication is closely associated with:

A. Alexander Graham Bell
B. John Logie Baird
C. Alfred Nobel
D. Guglielmo Marconi

**Answer:** D. Guglielmo Marconi

**Explanation:** Marconi developed and demonstrated practical wireless telegraphy systems.

**Source fact:** `MARCONI-RADIO`

## Q45 · Medium · Communication and familiar inventions

Early practical television development is associated with:

A. John Logie Baird
B. Thomas Edison
C. James Watt
D. Michael Faraday

**Answer:** A. John Logie Baird

**Explanation:** Baird demonstrated an early working television system using mechanical scanning.

**Source fact:** `BAIRD-TELEVISION`

## Q46 · Medium · Communication and familiar inventions

The World Wide Web was invented by:

A. Charles Babbage
B. Tim Berners-Lee
C. Alan Turing
D. Alexander Fleming

**Answer:** B. Tim Berners-Lee

**Explanation:** Berners-Lee created the core concepts and technologies of the World Wide Web at CERN.

**Source fact:** `BERNERSLEE-WWW`

## Q47 · Medium · Communication and familiar inventions

The dynamite invention is associated with:

A. Michael Faraday
B. John Dalton
C. Alfred Nobel
D. Robert Koch

**Answer:** C. Alfred Nobel

**Explanation:** Nobel developed dynamite by making nitroglycerin safer to handle in a practical explosive form.

**Source fact:** `NOBEL-DYNAMITE`

## Q48 · Hard · Communication and familiar inventions

Which pair is correctly distinguished?

A. Marconi — telephone; Bell — radioactivity
B. Baird — dynamite; Nobel — television
C. Berners-Lee — penicillin; Fleming — World Wide Web
D. Bell — telephone; Marconi — practical wireless telegraphy

**Answer:** D. Bell — telephone; Marconi — practical wireless telegraphy

**Explanation:** Bell is associated with the telephone, while Marconi is known for practical wireless telegraphy and radio communication.

**Source fact:** `BELL-MARCONI-DISTINCTION`

## Q49 · Easy · Indian scientists and major contributions

The Raman effect was discovered by:

A. C. V. Raman
B. S. N. Bose
C. Meghnad Saha
D. Homi J. Bhabha

**Answer:** A. C. V. Raman

**Explanation:** C. V. Raman discovered the change in wavelength of light associated with inelastic scattering.

**Source fact:** `CV-RAMAN-RAMAN-EFFECT`

## Q50 · Medium · Indian scientists and major contributions

The Saha ionization equation is associated with:

A. C. V. Raman
B. Meghnad Saha
C. Jagadish Chandra Bose
D. Vikram Sarabhai

**Answer:** B. Meghnad Saha

**Explanation:** Meghnad Saha developed the ionization equation used to explain ionization states in stellar atmospheres.

**Source fact:** `SAHA-IONIZATION`

## Q51 · Medium · Indian scientists and major contributions

Satyendra Nath Bose is remembered for work that led to:

A. The smallpox vaccine
B. The periodic table
C. Bose–Einstein statistics
D. The barometer

**Answer:** C. Bose–Einstein statistics

**Explanation:** Bose developed quantum-statistical ideas that Einstein extended, giving rise to Bose–Einstein statistics.

**Source fact:** `SN-BOSE-STATISTICS`

## Q52 · Medium · Indian scientists and major contributions

Jagadish Chandra Bose made pioneering contributions to radio/microwave research and also studied:

A. Blood groups
B. X-rays
C. The neutron
D. Responses in plants

**Answer:** D. Responses in plants

**Explanation:** J. C. Bose worked on electromagnetic waves and developed instruments for studying responses in plants.

**Source fact:** `JC-BOSE-CONTRIBUTIONS`

## Q53 · Hard · Indian scientists and major contributions

Which Indian scientist is correctly matched with a contribution?

A. Meghnad Saha — ionization theory of stellar atmospheres
B. C. V. Raman — ABO blood groups
C. S. N. Bose — penicillin
D. Homi Bhabha — smallpox vaccination

**Answer:** A. Meghnad Saha — ionization theory of stellar atmospheres

**Explanation:** Saha's ionization equation became important in astrophysics for understanding stellar spectra.

**Source fact:** `INDIAN-SCIENTIST-MATCH`

## Q54 · Hard · Indian scientists and major contributions

Which distinction is correct?

A. Raman — radioactivity; Bose — X-rays
B. C. V. Raman — Raman effect; S. N. Bose — quantum statistics
C. Raman — insulin; Bose — vaccination
D. Raman — periodic law; Bose — conservation of mass

**Answer:** B. C. V. Raman — Raman effect; S. N. Bose — quantum statistics

**Explanation:** Raman is associated with light scattering, while Bose is associated with quantum statistics.

**Source fact:** `RAMAN-BOSE-DISTINCTION`

## Q55 · Easy · Mixed scientist–discovery–invention reasoning

Which scientist–contribution pair is correct?

A. J. J. Thomson — nucleus
B. Rutherford — electron
C. James Chadwick — neutron
D. Röntgen — radioactivity

**Answer:** C. James Chadwick — neutron

**Explanation:** Chadwick discovered the neutron; the other listed pairings mismatch well-known contributions.

**Source fact:** `MIXED-CHADWICK-NEUTRON`

## Q56 · Medium · Mixed scientist–discovery–invention reasoning

A question asks for the scientist linked with both cathode-ray experiments and discovery of the electron. The answer is:

A. Ernest Rutherford
B. Niels Bohr
C. James Chadwick
D. J. J. Thomson

**Answer:** D. J. J. Thomson

**Explanation:** Thomson's cathode-ray experiments established the electron as a subatomic particle.

**Source fact:** `IDENTIFY-THOMSON`

## Q57 · Medium · Mixed scientist–discovery–invention reasoning

Which scientist should be selected for the periodic classification that successfully predicted properties of undiscovered elements?

A. Dmitri Mendeleev
B. John Dalton
C. Antoine Lavoisier
D. Amedeo Avogadro

**Answer:** A. Dmitri Mendeleev

**Explanation:** Mendeleev's periodic arrangement left gaps and predicted properties of elements later discovered.

**Source fact:** `IDENTIFY-MENDELEEV-PREDICTION`

## Q58 · Medium · Mixed scientist–discovery–invention reasoning

Which pairing correctly separates vaccination from antibiotics?

A. Fleming — vaccination; Jenner — penicillin
B. Jenner — vaccination; Fleming — penicillin
C. Pasteur — X-rays; Röntgen — vaccine
D. Koch — telephone; Bell — bacteria

**Answer:** B. Jenner — vaccination; Fleming — penicillin

**Explanation:** Jenner is associated with smallpox vaccination, while Fleming discovered penicillin.

**Source fact:** `JENNER-FLEMING-DISTINCTION`

## Q59 · Hard · Mixed scientist–discovery–invention reasoning

A question gives four names: Hooke, Leeuwenhoek, Pasteur and Koch. Which sequence correctly matches cell naming, microorganisms, pasteurization and specific disease-causing microbes?

A. Leeuwenhoek → Hooke → Koch → Pasteur
B. Pasteur → Koch → Hooke → Leeuwenhoek
C. Hooke → Leeuwenhoek → Pasteur → Koch
D. Koch → Pasteur → Leeuwenhoek → Hooke

**Answer:** C. Hooke → Leeuwenhoek → Pasteur → Koch

**Explanation:** Hooke named cells, Leeuwenhoek observed microbes, Pasteur developed pasteurization, and Koch linked specific microbes to diseases.

**Source fact:** `BIOLOGY-SCIENTIST-SEQUENCE`

## Q60 · Hard · Mixed scientist–discovery–invention reasoning

Which set contains only correctly matched associations?

A. Faraday — vaccination; Mendeleev — radioactivity; Mendel — X-rays
B. Ohm — natural selection; Darwin — resistance law; Jenner — telephone
C. Rutherford — penicillin; Fleming — nucleus; Hooke — gravity
D. Faraday — induction; Mendeleev — periodic table; Mendel — inheritance

**Answer:** D. Faraday — induction; Mendeleev — periodic table; Mendel — inheritance

**Explanation:** Faraday, Mendeleev and Mendel are correctly associated with electromagnetic induction, the periodic table and inheritance respectively.

**Source fact:** `MIXED-ALL-CORRECT-SET`

