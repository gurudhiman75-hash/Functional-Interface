...
for(const locale of ["en","hi","pa"] as const){
 const qs=[...generatePolCp003LocalizedReviewV1(locale),...generatePolCp004LocalizedReviewV1(locale)];
 const lines=[`# POL-001 CP003–CP004 — ${names[locale]} Localization Review V1`,"",`Questions: ${qs.length}`,""];
 for(const q of qs){
 ...
 fs.writeFileSync(path.join(dir,`POL-CP003-CP004-${locale.toUpperCase()}-REVIEW-V1.md`),lines.join("\n"));
}
console.log("POL-001 CP003–CP004 multilingual review Markdown generated.");
