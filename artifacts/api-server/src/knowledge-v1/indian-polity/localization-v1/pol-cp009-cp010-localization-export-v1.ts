...
for(const locale of ["en","hi","pa"] as const){
 const qs=[...generatePolCp009LocalizedReviewV1(locale),...generatePolCp010LocalizedReviewV1(locale)];
 const lines=[`# POL-001 CP009–CP010 — ${names[locale]} Localization Review V1`,"",`Questions: ${qs.length}`,""];
 for(const q of qs){
 ...
 fs.writeFileSync(path.join(dir,`POL-CP009-CP010-${locale.toUpperCase()}-REVIEW-V1.md`),lines.join("\n"));
}
console.log("POL-001 CP009–CP010 multilingual review Markdown generated.");