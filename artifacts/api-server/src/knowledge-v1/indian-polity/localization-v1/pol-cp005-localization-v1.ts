...
function stem(q:(typeof ENGLISH)[number],l:NativeLocale):string{
  ...
  if(ql===8){const a=articleForSubject(q.stem.split(" was added")[0]??"");return l==="hi"?`${a?native(DPSP_SUBJECT[a]!,l):"यह नीति-निदेशक तत्व"} किस संविधान संशोधन से जोड़ा गया?`:`${a?native(DPSP_SUBJECT[a]!,l):"ਇਹ ਨੀਤੀ-ਨਿਰਦੇਸ਼ਕ ਤੱਤ"} ਕਿਹੜੀ ਸੰਵਿਧਾਨ ਸੋਧ ਨਾਲ ਜੋੜਿਆ ਗਿਆ?`;}
...
function explanation(q:(typeof ENGLISH)[number],l:NativeLocale):string{
  ...
  if(ql===4){const c=article39ForSubject(q.canonicalAnswer)!;return l==="hi"?`${c} ${native(A39[c]!,l)} से संबंधित है।`:`${c} ${native(A39[c]!,l)} ਬਾਰੇ ਹੈ।`;}
  if(ql===5)return l==="hi"?`सही सुमेलित जोड़ी है: ${ans}।`:`ਸਹੀ ਮਿਲੀ ਜੋੜੀ: ${ans}।`;
  ...
  if([18,19,20].includes(ql))return l==="hi"?`सही उत्तर है: ${ans}। हर कथन को संबंधित अनुच्छेद या मौलिक कर्तव्य से मिलाकर देखें।`:`ਸਹੀ ਜਵਾਬ: ${ans}। ਹਰ ਬਿਆਨ ਨੂੰ ਸੰਬੰਧਿਤ ਅਨੁਛੇਦ ਜਾਂ ਮੂਲ ਫ਼ਰਜ਼ ਨਾਲ ਮਿਲਾ ਕੇ ਵੇਖੋ।`;
