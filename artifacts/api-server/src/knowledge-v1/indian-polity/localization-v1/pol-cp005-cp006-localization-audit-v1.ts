...
const cps=[
 ["POL-CP-005",generatePolCp005ReviewBatchV3().map(applyPolityFinalEditorialStemPass),generatePolCp005LocalizedReviewV1],
 ["POL-CP-006",generatePolCp006ReviewBatchV3().map(applyPolityFinalEditorialStemPass),generatePolCp006LocalizedReviewV1],
] as const;
...
const PA_STEM_BANNED=["ਇਨ੍ਹਾਂ ਵਿੱਚੋਂ ਇਨ੍ਹਾਂ ਵਿੱਚੋਂ","ਕਿਹੜੇ ਕਿਸਮ","ਕਿਹੜੇ ਸਾਬਕਾ","ਕਿਹੜਾ ਅਨੁਛੇਦ 51A","ਕਿਸ ਅਨੁਛੇਦ ਦਾ ਵਿਸ਼ਾ","ਕਿਹੜੇ ਅਨੁਛੇਦ ਹੇਠ","ਹੇਠ ਹੇਠ","ਸੰਵਿਧਾਨ ਸੋਧ ਦਾ ਕਿਹੜਾ ਜੋੜ","— ਇਹ ਕਿਹੜੀ ਸੰਵਿਧਾਨ ਸੋਧ ਨਾਲ ਹੋਇਆ?","ਇਸ ਅਧਿਆਇ ਵਿੱਚ ਮੁੱਖ ਤੌਰ 'ਤੇ"] as const;
...
function native(locale:"hi"|"pa",q:PolLocalizedQuestionV1){
 ...
 const stripped=original
   .replace(/\b(?:I|II|III|IV|IVA|V|VI|VII|VIII|IX|IXA|X|XI|XII)\b/gu,"");
...
assert.equal(generatePolCp005ReviewBatchV3().length,80,"CP005 authority must contain 80 questions");
assert.equal(generatePolCp006ReviewBatchV3().length,90,"CP006 authority must contain 90 questions");
assert.equal(total,170,"CP005-CP006 authority must contain 170 questions");
const evidence={chapterId:"POL-001",cps:["POL-CP-005","POL-CP-006"],localizationVersion:"POL-LOCALIZATION-V1",englishQuestions:total,questionsPerLocale:total,locales,totalReviewSurfaces:total*3,semanticInvariant:true,optionOrderInvariant:true,correctIndexInvariant:true,qlInvariant:true,sourceInvariant:true,numericFormInvariant:true,punjabiNativeEditorialGuard:true,examStandardGuard:true,reviewOnly:true,runtimeRegistered:false};
const dir=path.resolve("dist/polity-review/POL-MULTILINGUAL-V1");fs.mkdirSync(dir,{recursive:true});
fs.writeFileSync(path.join(dir,"POL-CP005-CP006-MULTILINGUAL-PROOF.json"),JSON.stringify(evidence,null,2));
