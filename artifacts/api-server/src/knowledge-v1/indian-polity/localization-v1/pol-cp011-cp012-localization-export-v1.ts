...
for(const locale of ["en","hi","pa"] as const){
 const qs=[...generatePolCp011LocalizedReviewV1(locale),...generatePolCp012LocalizedReviewV1(locale)];
 const lines=[`# POL-001 CP011–CP012 — ${names[locale]} Localization Review V1`,"",`Questions: ${qs.length}`,""];
 for(const q of qs){
 ...
 fs.writeFileSync(path.join(dir,`POL-CP011-CP012-${locale.toUpperCase()}-REVIEW-V1.md`),lines.join("\n"));
}
console.log("POL-001 CP011–CP012 multilingual review Markdown generated.");
