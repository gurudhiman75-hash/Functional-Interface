...
export function generatePolCp017ReviewBatchV1(): PolCp017ReviewQuestion[] {
  ...
  const questions = seeds.map((seed, index): PolCp017ReviewQuestion => {
    ...
    return {
      questionId: `POL-CP017-V1-${String(index + 1).padStart(3, "0")}`,
      qlId: seed.qlId,
  ...
  if (new Set(questions.map((q) => q.explanation)).size !== 96) throw new Error("POL-CP-017 explanations must be unique");
