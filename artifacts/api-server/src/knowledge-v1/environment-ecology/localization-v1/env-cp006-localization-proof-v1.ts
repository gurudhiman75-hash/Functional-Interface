...
import { ENV_CP006_ENGLISH_AUTHORITY_V1, generateEnvCp006LocalizedReviewV1 } from "./env-cp006-localization-v1";
...
assert.equal(ENV_CP006_ENGLISH_AUTHORITY_V1.length,48);
for(const locale of locales){
  const localized=generateEnvCp006LocalizedReviewV1(locale);
  assert.equal(localized.length,48);
  localized.forEach((q,i)=>{
    const s=ENV_CP006_ENGLISH_AUTHORITY_V1[i];
...
const evidence={chapterId:"ENV-001",cpId:"ENV-CP-006",authorityVersion:"V1",questionsPerLocale:48,totalReviewSurfaces:144,locales,semanticInvariant:true,optionOrderInvariant:true,correctIndexInvariant:true,sourceInvariant:true,nativeScriptGuard:true,reviewOnly:true,runtimeRegistered:false};
const dir=path.resolve("dist/environment-review/ENV-MULTILINGUAL-V1");fs.mkdirSync(dir,{recursive:true});
fs.writeFileSync(path.join(dir,"ENV-CP006-MULTILINGUAL-PROOF.json"),JSON.stringify(evidence,null,2));console.log(JSON.stringify(evidence));
