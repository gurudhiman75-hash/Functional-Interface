...
import { ENV_CP004_ENGLISH_AUTHORITY_V1, generateEnvCp004LocalizedReviewV1 } from "./env-cp004-localization-v1";
...
const evidence={chapterId:"ENV-001",cpId:"ENV-CP-004",questionsPerLocale:48,totalReviewSurfaces:144,locales,semanticInvariant:true,optionOrderInvariant:true,correctIndexInvariant:true,sourceInvariant:true,nativeScriptGuard:true,reviewOnly:true,runtimeRegistered:false};
const dir=path.resolve("dist/environment-review/ENV-MULTILINGUAL-V1");fs.mkdirSync(dir,{recursive:true});
fs.writeFileSync(path.join(dir,"ENV-CP004-MULTILINGUAL-PROOF.json"),JSON.stringify(evidence,null,2));console.log(JSON.stringify(evidence));
