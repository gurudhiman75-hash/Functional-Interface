...
import { ENV_CP005_FACT_IDS_V1 } from "./env-cp005-facts";
import { generateEnvCp005ReviewBatchV1 } from "./env-cp005-review-generator-v1";

describe("ENV-CP-005 V1 review batch", () => {
  const questions = generateEnvCp005ReviewBatchV1();
  ...
  it("keeps every item valid and review-only", () => {
    for (const q of questions) {
      ...
      expect(q.questionId).toMatch(/^ENV-CP005-V1-/);
      ...
      for (const id of q.sourceFactIds) expect(ENV_CP005_FACT_IDS_V1.has(id)).toBe(true);
    }
  });
  ...
  it("keeps learner-facing language concise", () => {
    ...
    for (const q of questions) {
      if (!q.stem.startsWith("Consider the following statements:")) expect(q.stem.length).toBeLessThanOrEqual(115);
      expect(q.explanation.length).toBeLessThanOrEqual(170);
    }
