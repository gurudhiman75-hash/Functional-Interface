...
import { ENV_CP008_FACT_IDS_V1 } from "./env-cp008-facts";
import { generateEnvCp008ReviewBatchV1 } from "./env-cp008-review-generator-v1";

describe("ENV-CP-008 V1 review batch", () => {
  const questions = generateEnvCp008ReviewBatchV1();
  ...
  it("keeps every item valid and review-only", () => {
    for (const q of questions) {
      ...
      expect(q.questionId).toMatch(/^ENV-CP008-V1-/);
      ...
      for (const id of q.sourceFactIds) expect(ENV_CP008_FACT_IDS_V1.has(id)).toBe(true);
    }
  });

  it("keeps exam-facing language clean", () => {
    const signatures = questions.map((q) => `${q.stem}::${q.canonicalAnswer}`);
    expect(new Set(signatures).size).toBe(48);
    for (const q of questions) {
      if (!q.stem.startsWith("Consider the following statements:")) expect(q.stem.length).toBeLessThanOrEqual(135);
      expect(q.explanation.length).toBeLessThanOrEqual(125);
    }
    const text = questions.map((q) => `${q.stem}\n${q.explanation}`).join("\n");
    expect(text).not.toMatch(/associated with/i);
    expect(text).not.toMatch(/option\s+[abcd]/i);
    expect(text).not.toMatch(/which should you select|in an exam|what can be concluded/i);
  });

  it("avoids mutable named-species status trivia", () => {
    const text = questions.map((q) => q.stem).join("\n");
    expect(text).not.toMatch(/tiger|elephant|rhino|lion|vulture|turtle|dolphin/i);
  });
});
