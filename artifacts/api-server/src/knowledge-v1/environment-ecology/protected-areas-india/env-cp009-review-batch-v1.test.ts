...
import { ENV_CP009_FACT_IDS_V1 } from "./env-cp009-facts";
import { generateEnvCp009ReviewBatchV1 } from "./env-cp009-review-generator-v1";

describe("ENV-CP-009 V1 review batch", () => {
  const questions = generateEnvCp009ReviewBatchV1();
  ...
  it("keeps every item valid and review-only", () => {
    for (const q of questions) {
      ...
      expect(q.questionId).toMatch(/^ENV-CP009-V1-/);
      ...
      for (const id of q.sourceFactIds) expect(ENV_CP009_FACT_IDS_V1.has(id)).toBe(true);
    }
  });

  it("keeps learner-facing language exam-grade and simple", () => {
    const signatures = questions.map((q) => `${q.stem}::${q.canonicalAnswer}`);
    expect(new Set(signatures).size).toBe(48);
    for (const q of questions) {
      if (!q.stem.startsWith("Consider the following statements:")) expect(q.stem.length).toBeLessThanOrEqual(145);
    ...
    expect(text).not.toMatch(/how many (national parks|sanctuaries|protected areas|conservation reserves|community reserves)/i);
  });
});
