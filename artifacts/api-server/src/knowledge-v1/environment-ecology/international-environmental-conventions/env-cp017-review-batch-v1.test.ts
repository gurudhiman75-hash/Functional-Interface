...
import { ENV_CP017_FACT_IDS_V1 } from "./env-cp017-facts";
import { ENV_CP017_REVIEW_QUESTIONS_V1 } from "./env-cp017-review-generator-v1";

describe("ENV-CP-017 V1 review batch", () => {
  const questions = ENV_CP017_REVIEW_QUESTIONS_V1;
  ...
  it("keeps every item valid, sourced and review-only", () => {
    for (const q of questions) {
      ...
      expect(q.questionId).toMatch(/^ENV-CP017-V1-/);
      ...
      for (const id of q.sourceFactIds) expect(ENV_CP017_FACT_IDS_V1.has(id)).toBe(true);
    }
  });

  it("keeps wording concise and excludes mutable treaty trivia", () => {
    ...
    expect(text).not.toMatch(/current number of parties|current cop|latest cop|current ramsar sites|latest ndc/i);
  });
});
