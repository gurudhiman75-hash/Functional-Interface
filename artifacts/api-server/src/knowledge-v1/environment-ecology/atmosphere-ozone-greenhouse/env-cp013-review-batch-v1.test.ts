...
import { ENV_CP013_FACT_IDS_V1 } from "./env-cp013-facts";
import { generateEnvCp013ReviewBatchV1 } from "./env-cp013-review-generator-v1";

describe("ENV-CP-013 V1 review batch", () => {
  const questions = generateEnvCp013ReviewBatchV1();
  ...
  it("keeps every item valid, sourced and review-only", () => {
    for (const q of questions) {
      ...
      expect(q.questionId).toMatch(/^ENV-CP013-V1-/);
      ...
      for (const id of q.sourceFactIds) expect(ENV_CP013_FACT_IDS_V1.has(id)).toBe(true);
    }
  });

  it("keeps learner-facing language exam-grade and stable", () => {
    const signatures = questions.map((q) => `${q.stem}::${q.canonicalAnswer}`);
    expect(new Set(signatures).size).toBe(48);
    for (const q of questions) {
      if (!q.stem.startsWith("Consider the following statements:")) expect(q.stem.length).toBeLessThanOrEqual(165);
      expect(q.explanation.length).toBeLessThanOrEqual(175);
    }
    ...
    expect(text).not.toMatch(/current (ozone hole|co2 concentration|methane concentration|greenhouse gas concentration)/i);
    expect(text).not.toMatch(/by what year will the ozone|recovery year/i);
  });
});