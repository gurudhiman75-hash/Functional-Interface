...
import { ENV_CP012_FACT_IDS_V1 } from "./env-cp012-facts";
import { generateEnvCp012ReviewBatchV1 } from "./env-cp012-review-generator-v1";

describe("ENV-CP-012 V1 review batch", () => {
  const questions = generateEnvCp012ReviewBatchV1();
  ...
  it("keeps every item valid and review-only", () => {
    for (const q of questions) {
      ...
      expect(q.questionId).toMatch(/^ENV-CP012-V1-/);
      ...
      for (const id of q.sourceFactIds) expect(ENV_CP012_FACT_IDS_V1.has(id)).toBe(true);
    }
  });
  ...
  it("keeps stems exam-grade and explanations simple", () => {
    ...
    for (const q of questions) {
      if (!q.stem.startsWith("Consider the following statements:")) expect(q.stem.length).toBeLessThanOrEqual(160);
    ...
    expect(text).not.toMatch(/current AQI|today's AQI|current limit/i);
    expect(text).not.toMatch(/CFC|ozone hole|greenhouse effect/i);
  });
});
