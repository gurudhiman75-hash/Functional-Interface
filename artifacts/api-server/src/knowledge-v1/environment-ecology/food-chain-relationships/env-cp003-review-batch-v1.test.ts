...
import { ENV_CP003_FACT_IDS_V1 } from "./env-cp003-facts";
import { generateEnvCp003ReviewBatchV1 } from "./env-cp003-review-generator-v1";

describe("ENV-CP-003 V1 review batch",()=>{
  const questions=generateEnvCp003ReviewBatchV1();
  ...
  it("uses all four answer positions in every QL",()=>{
    const byQl=new Map<string,typeof questions>();
    for(const q of questions){const b=byQl.get(q.qlId)??[];b.push(q);byQl.set(q.qlId,b);}
    for(const b of byQl.values()){
      expect(b).toHaveLength(4);
      expect(new Set(b.map(q=>q.correctIndex))).toEqual(new Set([0,1,2,3]));
    }
  });
  it("keeps questions structurally valid and review-only",()=>{
    for(const q of questions){
      ...
      for(const id of q.sourceFactIds) expect(ENV_CP003_FACT_IDS_V1.has(id)).toBe(true);
    }
  });
