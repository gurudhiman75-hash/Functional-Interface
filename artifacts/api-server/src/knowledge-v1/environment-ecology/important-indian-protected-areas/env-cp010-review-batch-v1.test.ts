...
import { ENV_CP010_FACT_IDS_V1 } from "./env-cp010-facts";
import { generateEnvCp010ReviewBatchV1 } from "./env-cp010-review-generator-v1";

describe("ENV-CP-010 V1 review batch", () => {
  const questions = generateEnvCp010ReviewBatchV1();
  ...
  it("keeps every item valid and review-only", () => {
    for (const q of questions) {
      ...
      expect(q.questionId).toMatch(/^ENV-CP010-V1-/);
      ...
      for (const id of q.sourceFactIds) expect(ENV_CP010_FACT_IDS_V1.has(id)).toBe(true);
    }
  });
  ...
  it("keeps learner-facing language exam-grade and stable", () => {
    ...
    expect(text).not.toMatch(/how many (national parks|protected areas|tigers|lions|rhinos|rhinoceroses|snow leopards)/i);
    expect(text).not.toMatch(/current population|latest population|population count/i);
  });
});
