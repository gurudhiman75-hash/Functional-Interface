...
import { generateEnvCp007ReviewBatchV1 } from "./env-cp007-review-generator-v1";

describe("ENV-CP-007 V1 review batch", () => {
  const questions = generateEnvCp007ReviewBatchV1();
  ...
  it("keeps every item valid, sourced and review-only", () => {
    for (const q of questions) {
      ...
      expect(q.questionId).toMatch(/^ENV-CP007-V1-/);
  ...
  it("keeps learner-facing language concise and avoids mutable count trivia", () => {
    const signatures = questions.map((q) => `${q.stem}::${q.canonicalAnswer}`);
    expect(new Set(signatures).size).toBe(48);
    for (const q of questions) {
      if (!q.stem.startsWith("Consider the statements:")) expect(q.stem.length).toBeLessThanOrEqual(150);
    ...
    expect(text).not.toMatch(/how many biodiversity hotspots.*world/i);
    expect(text).not.toMatch(/36 regions/i);
  });
});
