...
import {
  ...
  generateEcoCp001Cp016LocalizedReviewV1,
...
function assertParity(source: readonly any[], localized: readonly EcoLocalizedQuestionV1[], locale: EcoLocaleV1) {
  ...
  localized.forEach((question, index) => {
    ...
    if (["ECO-CP-009", "ECO-CP-010", "ECO-CP-011", "ECO-CP-012", "ECO-CP-013", "ECO-CP-014", "ECO-CP-015", "ECO-CP-016"].includes(english.cpId)) assertExamStandardStem(question);
...
for (const locale of locales) {
  fail(generateEcoCp001Cp016LocalizedReviewV1(locale).length === 700, `${locale}: CP001-CP016 expected 700`);
}

fail(
  locales.flatMap((locale) => generateEcoCp001Cp016LocalizedReviewV1(locale)).length === 2100,
  "CP001-CP016 expected 2100 EN-HI-PA surfaces",
);

console.log("Economy CP001-CP016 multilingual audit passed: 700 questions per locale / 2100 surfaces.");
