...
import {
  ...
  generateEcoCp001Cp020LocalizedReviewV1,
...
function assertParity(source: readonly any[], localized: readonly EcoLocalizedQuestionV1[], locale: EcoLocaleV1) {
  ...
  localized.forEach((question, index) => {
    ...
    if (["ECO-CP-009", "ECO-CP-010", "ECO-CP-011", "ECO-CP-012", "ECO-CP-013", "ECO-CP-014", "ECO-CP-015", "ECO-CP-016", "ECO-CP-017", "ECO-CP-018", "ECO-CP-019", "ECO-CP-020"].includes(english.cpId)) assertExamStandardStem(question);
...
for (const locale of locales) {
  fail(generateEcoCp001Cp020LocalizedReviewV1(locale).length === 876, `${locale}: CP001-CP020 expected 876`);
}

fail(
  locales.flatMap((locale) => generateEcoCp001Cp020LocalizedReviewV1(locale)).length === 2628,
  "CP001-CP020 expected 2628 EN-HI-PA surfaces",
);

console.log("Economy CP001-CP020 multilingual audit passed: 876 questions per locale / 2628 surfaces.");
