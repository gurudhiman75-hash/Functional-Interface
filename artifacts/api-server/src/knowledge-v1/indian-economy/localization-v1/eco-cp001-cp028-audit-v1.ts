...
import {
  ...
  generateEcoCp001Cp028LocalizedReviewV1,
...
function assertParity(source: readonly any[], localized: readonly EcoLocalizedQuestionV1[], locale: EcoLocaleV1) {
  ...
  localized.forEach((question, index) => {
    ...
    if (["ECO-CP-009", "ECO-CP-010", "ECO-CP-011", "ECO-CP-012", "ECO-CP-013", "ECO-CP-014", "ECO-CP-015", "ECO-CP-016", "ECO-CP-017", "ECO-CP-018", "ECO-CP-019", "ECO-CP-020", "ECO-CP-021", "ECO-CP-022", "ECO-CP-023", "ECO-CP-024", "ECO-CP-025", "ECO-CP-026", "ECO-CP-027", "ECO-CP-028"].includes(english.cpId)) assertExamStandardStem(question);
    if (["ECO-CP-021", "ECO-CP-022", "ECO-CP-023", "ECO-CP-024", "ECO-CP-025", "ECO-CP-026", "ECO-CP-027", "ECO-CP-028"].includes(english.cpId)) fail(!/associated with/iu.test(question.stem), `${question.questionId}: mechanical stem wording: /associated with/iu`);
...
const batches = [
  ...
  ["CP024", ECO_CP024_REVIEW_V1, generateEcoCp024LocalizedReviewV1, 28],
  ["CP025", ECO_CP025_REVIEW_V1, generateEcoCp025LocalizedReviewV1, 32],
  ["CP026", ECO_CP026_REVIEW_V1, generateEcoCp026LocalizedReviewV1, 20],
  ["CP027", ECO_CP027_REVIEW_V1, generateEcoCp027LocalizedReviewV1, 20],
  ["CP028", ECO_CP028_REVIEW_V1, generateEcoCp028LocalizedReviewV1, 24],
] as const;
...
for (const locale of locales) {
  fail(generateEcoCp001Cp028LocalizedReviewV1(locale).length === 1132, `${locale}: CP001-CP028 expected 1132`);
}

fail(
  locales.flatMap((locale) => generateEcoCp001Cp028LocalizedReviewV1(locale)).length === 3396,
  "CP001-CP028 expected 3396 EN-HI-PA surfaces",
);

console.log("Economy CP001-CP028 coverage-gap closure audit passed: 1132 questions per locale / 3396 surfaces.");
