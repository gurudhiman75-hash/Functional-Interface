...
import {
  ...
  generateEcoCp001Cp018LocalizedReviewV1,
...
function assertParity(source: readonly any[], localized: readonly EcoLocalizedQuestionV1[], locale: EcoLocaleV1) {
  ...
  localized.forEach((question, index) => {
    ...
    if (["ECO-CP-009", "ECO-CP-010", "ECO-CP-011", "ECO-CP-012", "ECO-CP-013", "ECO-CP-014", "ECO-CP-015", "ECO-CP-016", "ECO-CP-017", "ECO-CP-018"].includes(english.cpId)) assertExamStandardStem(question);
...
for (const locale of locales) {
  fail(generateEcoCp001Cp018LocalizedReviewV1(locale).length === 788, `${locale}: CP001-CP018 expected 788`);
}

fail(
  locales.flatMap((locale) => generateEcoCp001Cp018LocalizedReviewV1(locale)).length === 2364,
  "CP001-CP018 expected 2364 EN-HI-PA surfaces",
);

console.log("Economy CP001-CP018 multilingual audit passed: 788 questions per locale / 2364 surfaces.");
