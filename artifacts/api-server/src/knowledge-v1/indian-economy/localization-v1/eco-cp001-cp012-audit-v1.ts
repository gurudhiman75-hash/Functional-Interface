...
import {
  ...
  generateEcoCp001Cp012LocalizedReviewV1,
...
function assertParity(source: readonly any[], localized: readonly EcoLocalizedQuestionV1[], locale: EcoLocaleV1) {
  ...
  localized.forEach((question, index) => {
    ...
    if (["ECO-CP-009", "ECO-CP-010", "ECO-CP-011", "ECO-CP-012"].includes(english.cpId)) assertExamStandardStem(question);
...
for (const locale of locales) {
  fail(generateEcoCp001Cp012LocalizedReviewV1(locale).length === 524, `${locale}: CP001-CP012 expected 524`);
}

fail(
  locales.flatMap((locale) => generateEcoCp001Cp012LocalizedReviewV1(locale)).length === 1572,
  "CP001-CP012 expected 1572 EN-HI-PA surfaces",
);

console.log("Economy CP001-CP012 multilingual audit passed: 524 questions per locale / 1572 surfaces.");
