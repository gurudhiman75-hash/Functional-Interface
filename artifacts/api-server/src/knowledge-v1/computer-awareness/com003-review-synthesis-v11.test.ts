...
import { COM003_ENGLISH_REVIEW_CORPUS_V11, buildCom003EnglishReviewCorpusV11 } from "./com003-review-synthesis-v11";

const BANNED_NON_EXAM_LANGUAGE = /\b(?:appropriate|action|task|practical effect|correct choice|correctly represents|associated with|example or description|requirement|invokes?|distinguish|quickest keyboard|key combination)\b/i;

assert.equal(COM003_ENGLISH_REVIEW_CORPUS_V11.length, 228);
assert.equal(new Set(COM003_ENGLISH_REVIEW_CORPUS_V11.map((q) => q.qlId)).size, 19);

for (const ql of COM003_PERMANENT_QLS) {
  const qs = COM003_ENGLISH_REVIEW_CORPUS_V11.filter((q) => q.qlId === ql.qlId);
...
for (const q of COM003_ENGLISH_REVIEW_CORPUS_V11) {
  ...
  assert.equal(q.stemAuthority, "COM003_V11_PLAIN_EXAM_LANGUAGE_AUTHORITY");
...
assert.deepEqual(
  buildCom003EnglishReviewCorpusV11({ seedPrefix: "replay" }),
  buildCom003EnglishReviewCorpusV11({ seedPrefix: "replay" }),
);

console.log("[COM003-V11]", {
  questions: COM003_ENGLISH_REVIEW_CORPUS_V11.length,
  qls: 19,
  wording: "PLAIN_STANDARD_EXAM_LANGUAGE",
