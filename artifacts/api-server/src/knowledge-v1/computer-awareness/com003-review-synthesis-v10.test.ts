...
import { COM003_ENGLISH_REVIEW_CORPUS_V10, buildCom003EnglishReviewCorpusV10 } from "./com003-review-synthesis-v10";
...
assert.equal(COM003_ENGLISH_REVIEW_CORPUS_V10.length, 228);
assert.equal(new Set(COM003_ENGLISH_REVIEW_CORPUS_V10.map((q) => q.qlId)).size, 19);

for (const ql of COM003_PERMANENT_QLS) {
  const qs = COM003_ENGLISH_REVIEW_CORPUS_V10.filter((q) => q.qlId === ql.qlId);
...
for (const q of COM003_ENGLISH_REVIEW_CORPUS_V10) {
  ...
  assert.equal(q.stemAuthority, "COM003_V10_SIMPLE_STANDARD_EXAM_AUTHORITY");
...
assert.deepEqual(
  buildCom003EnglishReviewCorpusV10({ seedPrefix: "replay" }),
  buildCom003EnglishReviewCorpusV10({ seedPrefix: "replay" }),
);

console.log("[COM003-V10]", {
  questions: COM003_ENGLISH_REVIEW_CORPUS_V10.length,
  qls: 19,
