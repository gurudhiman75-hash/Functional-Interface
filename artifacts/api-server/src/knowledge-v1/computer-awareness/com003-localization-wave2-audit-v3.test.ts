...
const hindiIssues = audit("hi", COM003_HINDI_LOCALIZATION_WAVE2_V3);
const punjabiIssues = audit("pa", COM003_PUNJABI_LOCALIZATION_WAVE2_V3);
const issues = [...hindiIssues, ...punjabiIssues];

assert.equal(COM003_LOCALIZATION_WAVE2_AUTHORITY_V3.englishSourceQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE2_AUTHORITY_V3.hindiQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE2_AUTHORITY_V3.punjabiQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE2_AUTHORITY_V3.totalLocalizedQuestionCount, 120);
assert.equal(COM003_LOCALIZATION_WAVE2_AUTHORITY_V3.localizationFrozen, false);
assert.equal(COM003_LOCALIZATION_WAVE2_AUTHORITY_V3.questionStudioRegistrationAuthorized, false);

if (issues.length) console.error("[COM003-LOCALIZATION-WAVE2-V3-FAILURES]", JSON.stringify(issues, null, 2));
assert.equal(issues.length, 0, issues.join("\n"));

console.log("[COM003-LOCALIZATION-WAVE2-AUDIT-V3]", {
  valid: true,
  qlCount: WAVE2_QL_IDS.length,
  hindi: COM003_HINDI_LOCALIZATION_WAVE2_V3.length,
  punjabi: COM003_PUNJABI_LOCALIZATION_WAVE2_V3.length,
  total: COM003_LOCALIZATION_WAVE2_AUTHORITY_V3.totalLocalizedQuestionCount,
  issues: 0,
