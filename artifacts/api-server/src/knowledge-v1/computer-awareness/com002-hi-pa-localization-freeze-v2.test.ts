...
assert.equal(COM002_HI_PA_LOCALIZATION_FREEZE_CANDIDATE_V2.lifecycle.reviewRunPersistenceAllowed, false);
assert.equal(COM002_HI_PA_LOCALIZATION_FREEZE_CANDIDATE_V2.lifecycle.canonicalQuestionPersistenceAllowed, false);
...
assert.ok(
  audit.issues.includes("ENGLISH_V3_OPERATIONAL_FREEZE_BLOCKED_PENDING_EXECUTION_AND_EXPLICIT_APPROVAL"),
  audit.issues.join("\n"),
);
assert.equal(
...
console.log("[COM002-HI-PA-LOCALIZATION-FREEZE-V2] PASS hashCandidate=true promotable=false english=V3 localization=V2");
