import { COM003_ENGLISH_REVIEW_CORPUS_V2 } from "./com003-review-synthesis-v2";
...
export function auditCom003EditorialQualityV1() {
  ...
  for (const question of corpus) {
    ...
    if (stemLength > 260) blockers.push(`STEM_TOO_LONG:${question.questionId}:${stemLength}`);
    if (!/[?]$/.test(question.stem)) blockers.push(`STEM_NOT_QUESTION:${question.questionId}`);
    ...
    if (!normalized(question.explanation).includes(normalized(question.canonicalAnswer))) {
      blockers.push(`EXPLANATION_OMITS_ANSWER:${question.questionId}`);
    }
  ...
  const uniqueStems = new Set(corpus.map((question) => normalized(question.stem)));
  if (uniqueStems.size !== corpus.length) blockers.push(`DUPLICATE_STEMS_GLOBAL:${corpus.length - uniqueStems.size}`);
