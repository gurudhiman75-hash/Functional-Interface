...
const issues: string[] = [];
const expectedQls = ["COM-004-QL-005", "COM-004-QL-006", "COM-004-QL-007", "COM-004-QL-008"] as const;
...
if (COM004_ENGLISH_PRODUCTION_WAVE2_V1.length !== 48) issues.push(`COUNT:${COM004_ENGLISH_PRODUCTION_WAVE2_V1.length}`);
if (COM004_ENGLISH_PRODUCTION_WAVE2_AUTHORITY_V1.questionCount !== 48) issues.push("AUTHORITY_COUNT_DRIFT");
if (COM004_ENGLISH_PRODUCTION_WAVE2_AUTHORITY_V1.questionsPerQl !== 12) issues.push("AUTHORITY_PER_QL_DRIFT");

const questionIds = new Set<string>();
const allStems = new Set<string>();
for (const question of COM004_ENGLISH_PRODUCTION_WAVE2_V1) {
  ...
  if (!allocation) {
    issues.push(`UNKNOWN_QL:${question.questionId}:${question.qlId}`);
  } else {
...
for (const qlId of expectedQls) {
  const questions = COM004_ENGLISH_PRODUCTION_WAVE2_V1.filter((question) => question.qlId === qlId);
  ...
  const openingFamilies = new Set(
    questions.map((question) => question.stem.toLowerCase().replace(/[‘’'“”"():,.?`]/g, "").split(/\s+/).slice(0, 3).join(" ")),
  );
  if (openingFamilies.size < 7) issues.push(`MECHANICAL_STEM_OPENINGS:${qlId}:${openingFamilies.size}`);
...
const urlQl = COM004_ENGLISH_PRODUCTION_WAVE2_V1.filter((question) => question.qlId === "COM-004-QL-008");
if (!urlQl.some((question) => /does not (?:by itself )?prove|does not prove|not prove/i.test(question.explanation))) {
  issues.push("URL_TLD_MISCONCEPTION_LOCK_MISSING");
}
if (urlQl.some((question) => /(?:every|all)\s+\.org.*(?:nonprofit|non-profit)|\.com.*must belong/i.test(question.explanation))) {
  issues.push("URL_TLD_FALSE_CERTAINTY");
}

const browserQl = COM004_ENGLISH_PRODUCTION_WAVE2_V1.filter((question) => question.qlId === "COM-004-QL-006");
if (browserQl.some((question) => /market share|default browser ranking|best browser/i.test(question.stem + " " + question.explanation))) {
  issues.push("MUTABLE_BROWSER_TRIVIA_LEAK");
}

const governance = COM004_ENGLISH_PRODUCTION_WAVE2_AUTHORITY_V1.governance;
for (const [key, value] of Object.entries(governance)) {
  if (value !== false) issues.push(`PREMATURE_GOVERNANCE:${key}`);
}
if (COM004_ENGLISH_PRODUCTION_WAVE2_AUTHORITY_V1.status !== "REVIEW_CANDIDATE_NOT_FROZEN") issues.push("PREMATURE_FREEZE");
if (COM004_ENGLISH_PRODUCTION_WAVE2_AUTHORITY_V1.nextGate !== "COM004_ENGLISH_PRODUCTION_WAVE2_EDITORIAL_AUDIT") issues.push("NEXT_GATE_DRIFT");

if (issues.length) {
  throw new Error(`COM-004 English Production Wave 2 V1 audit failed:\n${issues.join("\n")}`);
}

console.log(JSON.stringify({
  checkpoint: "COM004_ENGLISH_PRODUCTION_WAVE2_V1_REVIEW_CANDIDATE",
  valid: true,
  qlCount: expectedQls.length,
  questionCount: COM004_ENGLISH_PRODUCTION_WAVE2_V1.length,
  questionsPerQl: 12,
  uniqueStemCount: allStems.size,
  urlTldMisconceptionLock: true,
  runtimeAuthorized: false,
  localizationAuthorized: false,
  questionBankWritesAuthorized: false,
  nextGate: COM004_ENGLISH_PRODUCTION_WAVE2_AUTHORITY_V1.nextGate,
}, null, 2));
