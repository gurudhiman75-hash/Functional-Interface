...
function audit(language: "hi" | "pa", corpus: readonly Com003LocalizedQuestionV1[]) {
  ...
  for (const localized of corpus) {
    ...
    if (source.versionScoped && /SHORTCUT|FORMATTING_SHORTCUT/i.test(source.surfaceMode) && !/Windows desktop/i.test(localized.stem)) issues.push(`WINDOWS_DESKTOP_CONTEXT_LOST:${language}:${localized.localizationId}`);
...
const hindiIssues = audit("hi", COM003_HINDI_LOCALIZATION_WAVE1_V3);
const punjabiIssues = audit("pa", COM003_PUNJABI_LOCALIZATION_WAVE1_V3);
const issues = [...hindiIssues, ...punjabiIssues];

if (issues.length) console.log("[COM003-LOCALIZATION-WAVE1-V3-FAILURES]", JSON.stringify(issues, null, 2));
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V3.englishSourceQuestionCount, 48);
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V3.hindiQuestionCount, 48);
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V3.punjabiQuestionCount, 48);
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V3.totalLocalizedQuestionCount, 96);
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V3.localizationFrozen, false);
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V3.questionStudioRegistrationAuthorized, false);
assert.equal(issues.length, 0, issues.join("\n"));

console.log("[COM003-LOCALIZATION-WAVE1-AUDIT-V3]", {
  valid: true,
  hindi: COM003_HINDI_LOCALIZATION_WAVE1_V3.length,
  punjabi: COM003_PUNJABI_LOCALIZATION_WAVE1_V3.length,
  total: COM003_LOCALIZATION_WAVE1_AUTHORITY_V3.totalLocalizedQuestionCount,
  issues: 0,
