...
function audit(language: "hi" | "pa", corpus: typeof COM003_HINDI_LOCALIZATION_WAVE1_V1) {
  const issues: string[] = [];
...
const hindiIssues = audit("hi", COM003_HINDI_LOCALIZATION_WAVE1_V1);
const punjabiIssues = audit("pa", COM003_PUNJABI_LOCALIZATION_WAVE1_V1);
const issues = [...hindiIssues, ...punjabiIssues];

assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V1.englishSourceQuestionCount, 48);
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V1.hindiQuestionCount, 48);
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V1.punjabiQuestionCount, 48);
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V1.totalLocalizedQuestionCount, 96);
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V1.localizationFrozen, false);
assert.equal(COM003_LOCALIZATION_WAVE1_AUTHORITY_V1.questionStudioRegistrationAuthorized, false);
assert.equal(issues.length, 0, issues.join("\n"));

console.log("[COM003-LOCALIZATION-WAVE1-AUDIT-V1]", {
  valid: true,
  hindi: COM003_HINDI_LOCALIZATION_WAVE1_V1.length,
  punjabi: COM003_PUNJABI_LOCALIZATION_WAVE1_V1.length,
  total: COM003_LOCALIZATION_WAVE1_AUTHORITY_V1.totalLocalizedQuestionCount,
  issues: 0,
