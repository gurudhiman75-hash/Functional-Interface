...
export function buildCom003QuestionReviewHtmlV1() {
  if (!COM003_EXAM_REALNESS_AUDIT_V1.valid) {
    throw new Error(`COM-003 V6 exam-realness audit failed: ${COM003_EXAM_REALNESS_AUDIT_V1.blockers.join(", ")}`);
  }

  const groups = COM003_PERMANENT_QLS.map((ql) => ({
    ql,
    questions: COM003_ENGLISH_REVIEW_CORPUS_V6.filter((question) => question.qlId === ql.qlId),
  }));
  ...
  <title>COM-003 Office & Productivity Software - English V6 Exam-Realness Review</title>
  <style>
  ...
  </style></head><body><main class="wrap"><header class="hero"><h1>COM-003 - Office & Productivity Software</h1><p>English V6 Exam-Realness Remediation Candidate</p><div class="stats"><div class="stat">19 permanent QLs</div><div class="stat">228 questions</div><div class="stat">12 per QL</div><div class="stat">V6 · review only</div></div><div class="rejected"><b>Previous V4 disposition:</b> REJECTED — stems not exam level. <b>V5 technical disposition:</b> REJECTED — duplicate-stem gate failed. Neither version is eligible for refreeze or Question Studio replacement.</div><div class="review-note"><b>Review V6 for:</b> SSC/Banking/Punjab-state exam realness, concise natural stems, wording variety, option quality, ambiguity, factual correctness and QL coverage. Correct options are highlighted for reviewer convenience. Approval of this file is required before any refreeze, relocalization or Question Studio replacement.</div></header>${sections}</main></body></html>`;
}
...
if (process.argv[1]?.includes("com003-question-review-file-v1")) {
  ...
  console.log(`[COM003-QUESTION-REVIEW-FILE-V1] questions=${COM003_ENGLISH_REVIEW_CORPUS_V6.length}`);
