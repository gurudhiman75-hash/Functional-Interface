...
function qlTitle(qlId: string) {
  const ql = COM003_PERMANENT_QLS.find((item) => item.qlId === qlId);
  return ql ? `${ql.qlId} - ${ql.title}` : qlId;
}

export function buildCom003QuestionReviewHtmlV1() {
  const groups = COM003_PERMANENT_QLS.map((ql) => ({
    ql,
    questions: COM003_ENGLISH_REVIEW_CORPUS_V4.filter((question) => question.qlId === ql.qlId),
  }));
...
if (process.argv[1]?.includes("com003-question-review-file-v1")) {
  ...
  console.log(`[COM003-QUESTION-REVIEW-FILE-V1] questions=${COM003_ENGLISH_REVIEW_CORPUS_V4.length}`);
}
