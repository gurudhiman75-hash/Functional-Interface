...
const WAVE3_QL_IDS = ["COM-003-QL-010", "COM-003-QL-011", "COM-003-QL-012", "COM-003-QL-013", "COM-003-QL-014"];
...
function isProtectedTechnical(value: string) {
  return /^(?:SUM|AVERAGE|COUNT|MAX|MIN|\$?[A-Z]\$?\d+(?::\$?[A-Z]\$?\d+)?)$/.test(value.trim());
}
...
function answerLeaks(stem: string, answer: string) {
  const a = answer.trim();
  if (isProtectedTechnical(a)) {
    if (/^[A-Z]+$/.test(a)) {
      return new RegExp(`(^|[^\\p{L}\\p{N}_])${escapeRegex(a)}([^\\p{L}\\p{N}_]|$)`, "iu").test(stem);
    }
    return stem.includes(a);
  }
  return normalized(stem).includes(normalized(a));
}
...
function audit(language: "hi" | "pa", corpus: readonly Com003LocalizedQuestionV1[]) {
  ...
  for (const qlId of WAVE3_QL_IDS) {
  ...
  for (const localized of corpus) {
    ...
    if (!WAVE3_QL_IDS.includes(localized.qlId)) issues.push(`OUT_OF_SCOPE_QL:${language}:${localized.localizationId}`);
...
const hindiIssues = audit("hi", COM003_HINDI_LOCALIZATION_WAVE3_V1);
const punjabiIssues = audit("pa", COM003_PUNJABI_LOCALIZATION_WAVE3_V1);
const issues = [...hindiIssues, ...punjabiIssues];

assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V1.englishSourceQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V1.hindiQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V1.punjabiQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V1.totalLocalizedQuestionCount, 120);
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V1.localizationFrozen, false);
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V1.questionStudioRegistrationAuthorized, false);

if (issues.length) console.error("[COM003-LOCALIZATION-WAVE3-V1-FAILURES]", JSON.stringify(issues, null, 2));
assert.equal(issues.length, 0, issues.join("\n"));

console.log("[COM003-LOCALIZATION-WAVE3-AUDIT-V1]", {
  valid: true,
  qlCount: WAVE3_QL_IDS.length,
  hindi: COM003_HINDI_LOCALIZATION_WAVE3_V1.length,
  punjabi: COM003_PUNJABI_LOCALIZATION_WAVE3_V1.length,
  total: COM003_LOCALIZATION_WAVE3_AUTHORITY_V1.totalLocalizedQuestionCount,
  issues: 0,
