...
export function auditCom003EditorialQualityV3() {
  const blockers: string[] = [];
  const advisories: string[] = [];
  const corpus = COM003_ENGLISH_REVIEW_CORPUS_V4;
  ...
  const coverage = COM003_PERMANENT_QLS.map((ql) => {
    ...
    if (explanations.size < 6) advisories.push(`LOW_EXPLANATION_DIVERSITY:${ql.qlId}:${explanations.size}`);
  ...
  return {
    ...
    blockers,
    advisories,
    status: blockers.length === 0 && advisories.length === 0
      ? "EDITORIAL_AUDIT_CLEAN" as const
      : blockers.length === 0
        ? "EDITORIAL_AUDIT_PASS_WITH_ADVISORIES" as const
        : "EDITORIAL_REMEDIATION_REQUIRED" as const,
    contentFrozen: false,
