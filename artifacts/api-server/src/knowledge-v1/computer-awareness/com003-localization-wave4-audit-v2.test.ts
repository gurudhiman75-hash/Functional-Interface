...
function hasPlaceholderBoilerplate(item: Com003LocalizedQuestionV1) {
  const text = `${item.stem} ${item.options.join(" ")} ${item.explanation}`
    .replaceAll("Placeholder (प्लेसहोल्डर)", "")
    .replaceAll("Placeholder (ਪਲੇਸਹੋਲਡਰ)", "")
    .replace(/\bplaceholders\b/gi, "");
  return /TODO|TBD|AWAITING|TRANSLATE|(?:^|[\[({:_-])PLACEHOLDER(?:$|[\])}:_-])/i.test(text);
}
...
function audit(language: "hi" | "pa", corpus: readonly Com003LocalizedQuestionV1[]) {
  ...
  for (const localized of corpus) {
    ...
    if (hasPlaceholderBoilerplate(localized)) issues.push(`PLACEHOLDER_TEXT:${language}:${localized.localizationId}`);
...
const issues = [...audit("hi", COM003_HINDI_LOCALIZATION_WAVE4_V2), ...audit("pa", COM003_PUNJABI_LOCALIZATION_WAVE4_V2)];
assert.equal(COM003_LOCALIZATION_WAVE4_AUTHORITY_V2.englishSourceQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE4_AUTHORITY_V2.hindiQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE4_AUTHORITY_V2.punjabiQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE4_AUTHORITY_V2.totalLocalizedQuestionCount, 120);
assert.equal(COM003_LOCALIZATION_WAVE4_AUTHORITY_V2.localizationFrozen, false);
assert.equal(COM003_LOCALIZATION_WAVE4_AUTHORITY_V2.questionStudioRegistrationAuthorized, false);
if (issues.length) console.error("[COM003-LOCALIZATION-WAVE4-V2-FAILURES]", JSON.stringify(issues, null, 2));
assert.equal(issues.length, 0, issues.join("\n"));
console.log("[COM003-LOCALIZATION-WAVE4-AUDIT-V2]", { valid: true, qlCount: WAVE4_QL_IDS.length, hindi: 60, punjabi: 60, total: 120, issues: 0, localizationFrozen: false, runtimeRegistered: false });
