...
const authority = COM003_LOCALIZATION_WAVE4_FREEZE_AUTHORITY_V1;
...
assert.equal(authority.governance.remainingLocalizationAuthorized, false);
...
assert.equal(authority.validationGate.versionContextLosses, 0);
assert.equal(authority.validationGate.placeholderBoilerplateCount, 0);
assert.equal(COM003_HINDI_LOCALIZATION_WAVE4_V2.length, 60);
assert.equal(COM003_PUNJABI_LOCALIZATION_WAVE4_V2.length, 60);
assert.equal(new Set(COM003_HINDI_LOCALIZATION_WAVE4_V2.map((item) => item.sourceQuestionId)).size, 60);
assert.equal(new Set(COM003_PUNJABI_LOCALIZATION_WAVE4_V2.map((item) => item.sourceQuestionId)).size, 60);

console.log("[COM003-LOCALIZATION-WAVE4-FREEZE-V1]", {
