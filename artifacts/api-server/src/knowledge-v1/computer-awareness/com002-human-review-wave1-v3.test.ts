...
for (const qlId of qlIds) {
  for (const suffix of ["A", "B"] as const) {
    ...
    assert.ok(question.questionId.endsWith("-V3"));
    assert.equal(question.options.length, 4);
    assert.equal(new Set(question.options).size, 4);
    assert.equal(question.options[question.correctIndex], question.canonicalAnswer);

    if (qlId === "COM-002-QL-004" && question.surfaceMode === "CORE_COMPONENT") {
    ...
    console.log(`\n[COM002-HUMAN-REVIEW-V3] Q${String(number).padStart(2, "0")} ${qlId} ${question.surfaceMode}`);
...
console.log(`\n[com002-human-review-wave1-v3] PASS questions=${number}`);
