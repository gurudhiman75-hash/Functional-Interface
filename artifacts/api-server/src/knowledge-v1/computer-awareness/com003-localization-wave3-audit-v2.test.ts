...
function answerLeaks(stem: string, answer: string) {
  ...
  if (isProtectedTechnical(a)) {
    if (/^[A-Z]+$/.test(a)) return new RegExp(`(^|[^\\p{L}\\p{N}_])${escapeRegex(a)}([^\\p{L}\\p{N}_]|$)`, "iu").test(stem);
...
const issues = [...audit("hi", COM003_HINDI_LOCALIZATION_WAVE3_V2), ...audit("pa", COM003_PUNJABI_LOCALIZATION_WAVE3_V2)];
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V2.englishSourceQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V2.hindiQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V2.punjabiQuestionCount, 60);
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V2.totalLocalizedQuestionCount, 120);
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V2.localizationFrozen, false);
assert.equal(COM003_LOCALIZATION_WAVE3_AUTHORITY_V2.questionStudioRegistrationAuthorized, false);
if (issues.length) console.error("[COM003-LOCALIZATION-WAVE3-V2-FAILURES]", JSON.stringify(issues, null, 2));
assert.equal(issues.length, 0, issues.join("\n"));
console.log("[COM003-LOCALIZATION-WAVE3-AUDIT-V2]", { valid: true, qlCount: WAVE3_QL_IDS.length, hindi: 60, punjabi: 60, total: 120, issues: 0, localizationFrozen: false, runtimeRegistered: false });
