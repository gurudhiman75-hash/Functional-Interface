...
function englishV3CorpusProjection() {
  return QL_IDS.flatMap((qlId) =>
    Array.from({ length: 40 }, (_, index) => {
      const seed = `english-v3-freeze-candidate:${qlId}:${index}`;
...
function englishV3ReviewProjection() {
  return QL_IDS.flatMap((qlId) =>
...
export function computeCom002EnglishV3FreezeCandidateFingerprints() {
  const corpus = englishV3CorpusProjection();
  const reviewPack = englishV3ReviewProjection();
  ...
  const combinedFingerprint = fingerprint({
    ...
    integrityAuthorityId: COM002_ENGLISH_HUMAN_REVIEW_INTEGRITY_V2.authorityId,
    integrityStatus: COM002_ENGLISH_HUMAN_REVIEW_INTEGRITY_V2.status,
...
export const COM002_ENGLISH_V3_FREEZE_CANDIDATE_PINS = {
  englishCorpusFingerprint: "PENDING",
  reviewPackFingerprint: "PENDING",
  combinedFingerprint: "PENDING",
} as const;

export const COM002_ENGLISH_V3_FREEZE_CANDIDATE = Object.freeze({
  candidateId: "COM-002-ENGLISH-V3-FREEZE-CANDIDATE" as const,
  ...
  materializedReviewPack: COM002_ENGLISH_HUMAN_REVIEW_INTEGRITY_V2.reviewPack,
  fingerprints: COM002_ENGLISH_V3_FREEZE_CANDIDATE_PINS,
  proofGuarantees: {
    deterministicReplay: true,
    exactlyFourUniqueOptions: true,
    canonicalAnswerVerified: true,
    ql004KernelSafetyRemediationPreserved: true,
  ...
  promotionBlocker: "EXPLICIT_PRODUCT_OWNER_APPROVAL_OF_EXACT_26_QUESTION_V3_PACK_REQUIRED" as const,
  lifecycle: {
    englishV3MachineAuditExecuted: true,
    englishV3MachineFingerprintsPinned: false,
    englishV3AuthorityFrozen: false,
  ...
  nextGate: "COM002_EXPLICIT_APPROVAL_THEN_PROMOTE_ENGLISH_V3_FREEZE" as const,
});

export function auditCom002EnglishV3FreezeCandidate() {
  const actual = computeCom002EnglishV3FreezeCandidateFingerprints();
  const pins = COM002_ENGLISH_V3_FREEZE_CANDIDATE_PINS;
  const issues: string[] = [];
  ...
  if (!COM002_ENGLISH_HUMAN_REVIEW_INTEGRITY_V2.humanReview.explicitApprovalVerified) {
