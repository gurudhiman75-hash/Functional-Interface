...
const issues: string[] = [];
const expectedQls = ["COM-004-QL-001", "COM-004-QL-002", "COM-004-QL-003", "COM-004-QL-004"] as const;
...
for (const question of COM004_ENGLISH_PRODUCTION_WAVE1_V1) {
...
for (const qlId of expectedQls) {
  const questions = COM004_ENGLISH_PRODUCTION_WAVE1_V1.filter((question) => question.qlId === qlId);
...
const governance = COM004_ENGLISH_PRODUCTION_WAVE1_AUTHORITY_V1.governance;
for (const [key, value] of Object.entries(governance)) {
  if (value !== false) issues.push(`PREMATURE_GOVERNANCE:${key}`);
}
if (COM004_ENGLISH_PRODUCTION_WAVE1_AUTHORITY_V1.status !== "REVIEW_CANDIDATE_NOT_FROZEN") issues.push("PREMATURE_FREEZE");
...
console.log(JSON.stringify({
  ...
  nextGate: COM004_ENGLISH_PRODUCTION_WAVE1_AUTHORITY_V1.nextGate,
}, null, 2));
