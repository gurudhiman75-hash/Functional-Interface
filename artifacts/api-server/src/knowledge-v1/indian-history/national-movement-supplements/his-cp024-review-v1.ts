...
const canonicalExplanation=(ids:readonly string[])=>ids.map(id=>{
 const f=HIS_CP024_FACT_BY_ID_V1.get(id); if(!f)throw new Error(`Unknown HIS-CP-024 fact ${id}`); return f[1];
}).join(" ");
...
export function generateHisCp024ReviewBatchV1():HisCp024ReviewQuestion[]{
 return HIS_CP024_SPECS_V1.map((s,i)=>{
  const n=i+1; const [ql,stem,answer,distractors,factIds]=s;
  const base=[answer,...distractors]; const shift=i%4;
  const options=[...base.slice(shift),...base.slice(0,shift)];
  const correctIndex=options.indexOf(answer);
  const note=HIS_CP024_EXPLANATION_NOTES_V1[n]; if(!note)throw new Error(`Missing note ${n}`);
  return{questionId:`HIS-CP024-V1-${String(n).padStart(3,"0")}`,chapterId:"HIS-001",cpId:"HIS-CP-024",
   qlId:`HIS-024-QL-${String(ql).padStart(3,"0")}`,qlName:HIS_CP024_QL_NAMES_V1[ql],difficulty:difficulty(ql),
   stem,options,correctIndex,canonicalAnswer:answer,explanation:`${canonicalExplanation(factIds)} ${note}`.trim(),
...
export function auditHisCp024ReviewBatchV1(){
 const issues:string[]=[]; const ids=new Set<string>(),semantic=new Set<string>(); const qlCounts:Record<string,number>={};
 const answerPositions=[0,0,0,0]; const difficultyCounts={Easy:0,Medium:0,Hard:0};
 const prior=[...HIS_CP015_FACTS_V1,...HIS_CP016_FACTS_V1] as readonly (readonly [string,string,readonly string[]])[];
 const priorText=new Map(prior.map(f=>[normalize(f[1]),f[0]] as const));
 for(const f of HIS_CP024_FACTS_V1){
  for(const sid of f[2])if(!HIS_CP024_SOURCE_IDS_V1.has(sid))issues.push(`UNKNOWN_SOURCE:${f[0]}:${sid}`);
  const old=priorText.get(normalize(f[1])); if(old)issues.push(`V1_FACT_DUPLICATE:${f[0]}:${old}`);
