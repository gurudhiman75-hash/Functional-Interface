...
import { HIS_CP020_FACTS_V1,HIS_CP020_FACT_BY_ID_V1,HIS_CP020_SOURCE_IDS_V1,HIS_CP020_SOURCES_V1 } from "./his-cp020-facts-v1";
import { HIS_CP020_QL_NAMES_V1,HIS_CP020_SPECS_V1 } from "./his-cp020-review-specs-v1";
import { HIS_CP020_EXPLANATION_NOTES_V1 } from "./his-cp020-explanation-notes-v1";
...
export type HisCp020ReviewQuestion={
  questionId:string;chapterId:"HIS-001";cpId:"HIS-CP-020";qlId:string;qlName:string;
...
const canonicalExplanation=(ids:readonly string[])=>ids.map(id=>{
  const f=HIS_CP020_FACT_BY_ID_V1.get(id);
  if(!f)throw new Error(`Unknown HIS-CP-020 fact ${id}`);
  return f[1];
}).join(" ");
const sourceIds=(ids:readonly string[])=>[...new Set(ids.flatMap(id=>HIS_CP020_FACT_BY_ID_V1.get(id)?.[2]??[]))];
...
export function generateHisCp020ReviewBatchV1():HisCp020ReviewQuestion[]{
  return HIS_CP020_SPECS_V1.map((s,i)=>{
    ...
    const note=HIS_CP020_EXPLANATION_NOTES_V1[n];
    if(!note)throw new Error(`Missing HIS-CP-020 explanation note ${n}`);
    const explanation=`${canonicalExplanation(factIds)} ${note}`.trim();
    return{
      questionId:`HIS-CP020-V1-${String(n).padStart(3,"0")}`,
      chapterId:"HIS-001",cpId:"HIS-CP-020",
      qlId:`HIS-020-QL-${String(ql).padStart(3,"0")}`,
      qlName:HIS_CP020_QL_NAMES_V1[ql],
...
export const HIS_CP020_REVIEW_BATCH_V1=Object.freeze(generateHisCp020ReviewBatchV1().map(q=>Object.freeze(q)));
export const HIS_CP020_REQUIRED_FACTS_V1=Object.freeze([...new Set(HIS_CP020_REVIEW_BATCH_V1.flatMap(q=>q.sourceFactIds))].sort());

export function auditHisCp020ReviewBatchV1(){
  const issues:string[]=[];
  const ids=new Set<string>(),semantic=new Set<string>();
  const qlCounts:Record<string,number>={};
  const answerPositions=[0,0,0,0];
  const difficultyCounts={Easy:0,Medium:0,Hard:0};
  const priorFacts=[...HIS_CP008_FACTS_V1,...HIS_CP009_FACTS_V1,...HIS_CP010_FACTS_V1,...HIS_CP011_FACTS_V1] as readonly (readonly [string,string,readonly string[]])[];
  const priorText=new Map(priorFacts.map(f=>[normalize(f[1]),f[0]] as const));

  for(const f of HIS_CP020_FACTS_V1){
    for(const s of f[2])if(!HIS_CP020_SOURCE_IDS_V1.has(s))issues.push(`UNKNOWN_SOURCE:${f[0]}:${s}`);
  ...
  for(const q of HIS_CP020_REVIEW_BATCH_V1){
    ...
    if(q.stem.length>225)issues.push(`STEM_TOO_LONG:${q.questionId}:${q.stem.length}`);
    if(q.explanation.length<150)issues.push(`EXPLANATION_TOO_SHORT:${q.questionId}:${q.explanation.length}`);
    if(q.explanation.length>760)issues.push(`EXPLANATION_TOO_LONG:${q.questionId}:${q.explanation.length}`);
  ...
  for(let i=1;i<=10;i++){const q=`HIS-020-QL-${String(i).padStart(3,"0")}`;if(qlCounts[q]!==6)issues.push(`QL_COUNT:${q}:${qlCounts[q]??0}`);}
  if(HIS_CP020_REVIEW_BATCH_V1.length!==60)issues.push(`QUESTION_COUNT:${HIS_CP020_REVIEW_BATCH_V1.length}`);
  if(HIS_CP020_FACTS_V1.length!==60)issues.push(`FACT_COUNT:${HIS_CP020_FACTS_V1.length}`);
  if(HIS_CP020_REQUIRED_FACTS_V1.length!==60)issues.push(`FACT_COVERAGE:${HIS_CP020_REQUIRED_FACTS_V1.length}/60`);
  if(Object.keys(HIS_CP020_EXPLANATION_NOTES_V1).length!==60)issues.push(`EXPLANATION_NOTE_COUNT:${Object.keys(HIS_CP020_EXPLANATION_NOTES_V1).length}`);
  if(Math.max(...answerPositions)-Math.min(...answerPositions)>1)issues.push(`ANSWER_POSITION_IMBALANCE:${answerPositions.join(",")}`);
  return{valid:issues.length===0,questionCount:HIS_CP020_REVIEW_BATCH_V1.length,semanticCount:semantic.size,canonicalFactCount:HIS_CP020_FACTS_V1.length,requiredFactCount:HIS_CP020_REQUIRED_FACTS_V1.length,sourceCount:HIS_CP020_SOURCES_V1.length,qlCounts,difficultyCounts,answerPositions,issues};
}
