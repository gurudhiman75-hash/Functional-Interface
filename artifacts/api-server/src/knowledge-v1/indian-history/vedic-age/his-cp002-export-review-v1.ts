...
import { HIS_CP002_REVIEW_BATCH_V1, auditHisCp002ReviewBatchV1 } from "./his-cp002-review-v1";

const audit = auditHisCp002ReviewBatchV1();
if (!audit.valid) throw new Error(`HIS-CP-002 export blocked: ${audit.issues.join(" | ")}`);

const outDir = path.resolve(process.cwd(), "dist/history-review/HIS-001-CP002-V1");
fs.mkdirSync(outDir, { recursive: true });
const letters = ["A", "B", "C", "D"];
const lines: string[] = [
  "# HIS-CP-002 — Vedic Age — Review Batch V1",
  "",
...
HIS_CP002_REVIEW_BATCH_V1.forEach((question, index) => {
  lines.push(
...
const baseName = "HIS-001-CP002-REVIEW-BATCH-V1";
fs.writeFileSync(path.join(outDir, `${baseName}.md`), lines.join("\n"));
fs.writeFileSync(path.join(outDir, `${baseName}.json`), JSON.stringify({ audit, questions: HIS_CP002_REVIEW_BATCH_V1 }, null, 2));
