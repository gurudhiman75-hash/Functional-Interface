...
import { HIS_CP011_HI_V1 } from "./his-cp011-hi-v1";
import { HIS_CP011_PA_V1 } from "./his-cp011-pa-v1";
...
fail(HIS_CP011_REVIEW_BATCH_V1.length === 60, `CP011 English expected 60, found ${HIS_CP011_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP011_HI_V1).length === 60, `CP011 Hindi overlays expected 60, found ${Object.keys(HIS_CP011_HI_V1).length}`);
fail(Object.keys(HIS_CP011_PA_V1).length === 60, `CP011 Punjabi overlays expected 60, found ${Object.keys(HIS_CP011_PA_V1).length}`);

for (const locale of locales) assertParity(HIS_CP011_REVIEW_BATCH_V1, generateHisCp011LocalizedReviewV1(locale), locale);
const all = locales.flatMap((locale) => generateHisCp011LocalizedReviewV1(locale));
fail(all.length === 180, `CP011 expected 180 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP011 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces.");
