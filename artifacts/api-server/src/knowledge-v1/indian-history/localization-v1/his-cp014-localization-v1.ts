...
type EnglishQuestion = (typeof HIS_CP014_REVIEW_BATCH_V1)[number];
type NativeLocale = Exclude<HisLocaleV1, "en">;

function overlayFor(index: number, locale: NativeLocale): HisNativeOverlayV1 {
  const n = index + 1;
  const overlay = locale === "hi" ? HIS_CP014_HI_V1[n] : HIS_CP014_PA_V1[n];
  if (!overlay) throw new Error(`HIS-CP-014 #${n}: missing ${locale} localization overlay`);
  return overlay;
}
...
export const HIS_CP014_LOCALIZATION_V1_SUPPORTED_LOCALES = ["en", "hi", "pa"] as const;
export const HIS_CP014_LOCALIZATION_V1_SUPPORTED_CPS = ["HIS-CP-014"] as const;
