...
import { HIS_CP005_HI_V1 } from "./his-cp005-hi-v1";
import { HIS_CP005_PA_V1 } from "./his-cp005-pa-v1";
...
fail(HIS_CP005_REVIEW_BATCH_V1.length === 60, `CP005 English expected 60, found ${HIS_CP005_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP005_HI_V1).length === 60, `CP005 Hindi overlays expected 60, found ${Object.keys(HIS_CP005_HI_V1).length}`);
fail(Object.keys(HIS_CP005_PA_V1).length === 60, `CP005 Punjabi overlays expected 60, found ${Object.keys(HIS_CP005_PA_V1).length}`);

for (const locale of locales) assertParity(HIS_CP005_REVIEW_BATCH_V1, generateHisCp005LocalizedReviewV1(locale), locale);
const all = locales.flatMap((locale) => generateHisCp005LocalizedReviewV1(locale));
fail(all.length === 180, `CP005 expected 180 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP005 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces.");
