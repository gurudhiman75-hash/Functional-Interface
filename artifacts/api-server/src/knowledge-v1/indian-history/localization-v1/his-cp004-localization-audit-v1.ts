...
import { HIS_CP004_HI_V1 } from "./his-cp004-hi-v1";
import { HIS_CP004_PA_V1 } from "./his-cp004-pa-v1";
...
function assertNative(question: HisLocalizedQuestionV1, locale: "hi" | "pa") {
  ...
  fail(question.stem.length <= 360, `${question.questionId}: localized stem too long: ${question.stem.length}`);
  fail(question.explanation.length >= 90, `${question.questionId}: localized explanation too short: ${question.explanation.length}`);
  fail(question.explanation.length <= 480, `${question.questionId}: localized explanation too long: ${question.explanation.length}`);
...
fail(HIS_CP004_REVIEW_BATCH_V1.length === 60, `CP004 English expected 60, found ${HIS_CP004_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP004_HI_V1).length === 60, `CP004 Hindi overlays expected 60, found ${Object.keys(HIS_CP004_HI_V1).length}`);
fail(Object.keys(HIS_CP004_PA_V1).length === 60, `CP004 Punjabi overlays expected 60, found ${Object.keys(HIS_CP004_PA_V1).length}`);

for (const locale of locales) {
  assertParity(HIS_CP004_REVIEW_BATCH_V1, generateHisCp004LocalizedReviewV1(locale), locale);
}

const all = locales.flatMap((locale) => generateHisCp004LocalizedReviewV1(locale));
fail(all.length === 180, `CP004 expected 180 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP004 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces.");
