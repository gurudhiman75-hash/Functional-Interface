...
const lines:string[]=[
  "# History Multilingual V1 — HIS-CP-017 Review",
  "",
  "Review-only candidate. HIS-001-ENGLISH-FREEZE-V2 is the semantic authority. Hindi and Punjabi preserve CP, QL, difficulty, source provenance, option order and correct-index parity.",
  "",
];
for(const locale of locales){
  lines.push(`## ${labels[locale]}`,"");
  generateHisCp017LocalizedReviewV1(locale).forEach((q,index)=>lines.push(...renderQuestion(q,index)));
}
const target=path.join(outDir,"HIS-MULTILINGUAL-V1-CP017-REVIEW.md");
