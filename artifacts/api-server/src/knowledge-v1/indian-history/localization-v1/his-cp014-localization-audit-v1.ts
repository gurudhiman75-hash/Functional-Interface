...
import { HIS_CP014_HI_V1 } from "./his-cp014-hi-v1";
import { HIS_CP014_PA_V1 } from "./his-cp014-pa-v1";
...
fail(HIS_CP014_REVIEW_BATCH_V1.length === 60, `CP014 English expected 60, found ${HIS_CP014_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP014_HI_V1).length === 60, `CP014 Hindi overlays expected 60, found ${Object.keys(HIS_CP014_HI_V1).length}`);
fail(Object.keys(HIS_CP014_PA_V1).length === 60, `CP014 Punjabi overlays expected 60, found ${Object.keys(HIS_CP014_PA_V1).length}`);

for (const locale of locales) assertParity(HIS_CP014_REVIEW_BATCH_V1, generateHisCp014LocalizedReviewV1(locale), locale);
const all = locales.flatMap((locale) => generateHisCp014LocalizedReviewV1(locale));
fail(all.length === 180, `CP014 expected 180 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP014 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces.");
