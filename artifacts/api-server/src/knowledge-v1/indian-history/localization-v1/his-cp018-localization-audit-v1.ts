...
fail(HIS_CP018_REVIEW_BATCH_V1.length===60,`CP018 English expected 60, found ${HIS_CP018_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP018_HI_V1).length===60,`CP018 Hindi overlays expected 60, found ${Object.keys(HIS_CP018_HI_V1).length}`);
fail(Object.keys(HIS_CP018_PA_V1).length===60,`CP018 Punjabi overlays expected 60, found ${Object.keys(HIS_CP018_PA_V1).length}`);

for(const locale of locales)assertParity(HIS_CP018_REVIEW_BATCH_V1,generateHisCp018LocalizedReviewV1(locale),locale);
const all=locales.flatMap((locale)=>generateHisCp018LocalizedReviewV1(locale));
fail(all.length===180,`CP018 expected 180 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP018 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces; English authority HIS-001-ENGLISH-FREEZE-V3.");
