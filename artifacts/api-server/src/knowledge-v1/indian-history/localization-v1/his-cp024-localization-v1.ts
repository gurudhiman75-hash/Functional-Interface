...
type EnglishQuestion = (typeof HIS_CP024_REVIEW_BATCH_V1)[number];
type NativeLocale = Exclude<HisLocaleV1, "en">;

function overlayFor(index:number,locale:NativeLocale):HisNativeOverlayV1{
  const n=index+1;
  const overlay=locale==="hi"?HIS_CP024_HI_V1[n]:HIS_CP024_PA_V1[n];
  if(!overlay)throw new Error("HIS-CP-024 #"+n+": missing "+locale+" localization overlay");
  return overlay;
}
...
function localizedBase(
  ...
  return {
    ...
    localizationV1:{
      ...
      englishFreeze:HIS_ENGLISH_FREEZE_V5,
...
export function generateHisCp024LocalizedReviewV1(locale:HisLocaleV1):HisLocalizedQuestionV1[]{
  return HIS_CP024_REVIEW_BATCH_V1.map((q,index)=>{
...
export const HIS_CP024_LOCALIZATION_V1_SUPPORTED_LOCALES=["en","hi","pa"] as const;
export const HIS_CP024_LOCALIZATION_V1_SUPPORTED_CPS=["HIS-CP-024"] as const;
