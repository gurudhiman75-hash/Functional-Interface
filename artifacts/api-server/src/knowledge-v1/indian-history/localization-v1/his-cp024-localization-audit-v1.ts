...
import { HIS_CP024_HI_V1 } from "./his-cp024-hi-v1";
import { HIS_CP024_PA_V1 } from "./his-cp024-pa-v1";
...
function assertNative(question:HisLocalizedQuestionV1,locale:"hi"|"pa",english:any){
  ...
  fail(question.stem.length<=390,question.questionId+": localized stem too long: "+question.stem.length);
  fail(question.explanation.length>=90,question.questionId+": localized explanation too short: "+question.explanation.length);
  fail(question.explanation.length<=650,question.questionId+": localized explanation too long: "+question.explanation.length);
  ...
  if(locale==="hi"){
    ...
    const editorialBans=["ਕੜਾ","ਕੜੀ","ਕੜੀਆਂ","ਟਾਈਟ","ਉਪਵਾਸ","ਨੇਤ੍ਰਿਤਵ","ਕਿਹੜੇ ਕਿਸਮ","ਸਹੀ ਤਰ੍ਹਾਂ","ਹੇਠ ਹੇਠ"];
...
fail(HIS_CP024_REVIEW_BATCH_V1.length===60,"CP024 English expected 60, found "+HIS_CP024_REVIEW_BATCH_V1.length);
fail(Object.keys(HIS_CP024_HI_V1).length===60,"CP024 Hindi overlays expected 60, found "+Object.keys(HIS_CP024_HI_V1).length);
fail(Object.keys(HIS_CP024_PA_V1).length===60,"CP024 Punjabi overlays expected 60, found "+Object.keys(HIS_CP024_PA_V1).length);

for(const locale of locales)assertParity(HIS_CP024_REVIEW_BATCH_V1,generateHisCp024LocalizedReviewV1(locale),locale);

const all=locales.flatMap((locale)=>generateHisCp024LocalizedReviewV1(locale));
fail(all.length===180,"CP024 expected 180 EN-HI-PA surfaces, found "+all.length);

console.log("History CP024 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces; English authority HIS-001-ENGLISH-FREEZE-V5.");
