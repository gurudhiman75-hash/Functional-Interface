...
import { HIS_CP009_HI_V1 } from "./his-cp009-hi-v1";
import { HIS_CP009_PA_V1 } from "./his-cp009-pa-v1";
...
fail(HIS_CP009_REVIEW_BATCH_V1.length === 60, `CP009 English expected 60, found ${HIS_CP009_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP009_HI_V1).length === 60, `CP009 Hindi overlays expected 60, found ${Object.keys(HIS_CP009_HI_V1).length}`);
fail(Object.keys(HIS_CP009_PA_V1).length === 60, `CP009 Punjabi overlays expected 60, found ${Object.keys(HIS_CP009_PA_V1).length}`);

for (const locale of locales) assertParity(HIS_CP009_REVIEW_BATCH_V1, generateHisCp009LocalizedReviewV1(locale), locale);
const all = locales.flatMap((locale) => generateHisCp009LocalizedReviewV1(locale));
fail(all.length === 180, `CP009 expected 180 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP009 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces.");
