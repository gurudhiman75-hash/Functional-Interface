...
import { HIS_CP010_HI_V1 } from "./his-cp010-hi-v1";
import { HIS_CP010_PA_V1 } from "./his-cp010-pa-v1";
...
fail(HIS_CP010_REVIEW_BATCH_V1.length === 60, `CP010 English expected 60, found ${HIS_CP010_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP010_HI_V1).length === 60, `CP010 Hindi overlays expected 60, found ${Object.keys(HIS_CP010_HI_V1).length}`);
fail(Object.keys(HIS_CP010_PA_V1).length === 60, `CP010 Punjabi overlays expected 60, found ${Object.keys(HIS_CP010_PA_V1).length}`);

for (const locale of locales) assertParity(HIS_CP010_REVIEW_BATCH_V1, generateHisCp010LocalizedReviewV1(locale), locale);
const all = locales.flatMap((locale) => generateHisCp010LocalizedReviewV1(locale));
fail(all.length === 180, `CP010 expected 180 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP010 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces.");
