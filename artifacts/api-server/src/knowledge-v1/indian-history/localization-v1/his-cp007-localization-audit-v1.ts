...
import { HIS_CP007_HI_V1 } from "./his-cp007-hi-v1";
import { HIS_CP007_PA_V1 } from "./his-cp007-pa-v1";
...
fail(HIS_CP007_REVIEW_BATCH_V1.length === 60, `CP007 English expected 60, found ${HIS_CP007_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP007_HI_V1).length === 60, `CP007 Hindi overlays expected 60, found ${Object.keys(HIS_CP007_HI_V1).length}`);
fail(Object.keys(HIS_CP007_PA_V1).length === 60, `CP007 Punjabi overlays expected 60, found ${Object.keys(HIS_CP007_PA_V1).length}`);

for (const locale of locales) assertParity(HIS_CP007_REVIEW_BATCH_V1, generateHisCp007LocalizedReviewV1(locale), locale);
const all = locales.flatMap((locale) => generateHisCp007LocalizedReviewV1(locale));
fail(all.length === 180, `CP007 expected 180 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP007 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces.");
