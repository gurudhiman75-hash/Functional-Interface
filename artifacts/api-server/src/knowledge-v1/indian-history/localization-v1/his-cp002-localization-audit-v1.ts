...
import { HIS_CP002_HI_V1 } from "./his-cp002-hi-v1";
import { HIS_CP002_PA_V1 } from "./his-cp002-pa-v1";
...
function assertNative(question: HisLocalizedQuestionV1, locale: "hi" | "pa") {
  ...
  fail(question.stem.length <= 320, `${question.questionId}: localized stem too long: ${question.stem.length}`);
  fail(question.explanation.length >= 90, `${question.questionId}: localized explanation too short: ${question.explanation.length}`);
  fail(question.explanation.length <= 440, `${question.questionId}: localized explanation too long: ${question.explanation.length}`);
...
fail(HIS_CP002_REVIEW_BATCH_V1.length === 60, `CP002 English expected 60, found ${HIS_CP002_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP002_HI_V1).length === 60, `CP002 Hindi overlays expected 60, found ${Object.keys(HIS_CP002_HI_V1).length}`);
fail(Object.keys(HIS_CP002_PA_V1).length === 60, `CP002 Punjabi overlays expected 60, found ${Object.keys(HIS_CP002_PA_V1).length}`);

for (const locale of locales) {
  assertParity(HIS_CP002_REVIEW_BATCH_V1, generateHisCp002LocalizedReviewV1(locale), locale);
}

const all = locales.flatMap((locale) => generateHisCp002LocalizedReviewV1(locale));
fail(all.length === 180, `CP002 expected 180 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP002 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces.");
