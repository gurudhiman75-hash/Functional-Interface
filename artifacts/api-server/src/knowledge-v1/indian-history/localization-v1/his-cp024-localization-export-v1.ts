...
const lines:string[]=[
  "# History Multilingual V1 — HIS-CP-024 Review",
  "",
  "Review-only candidate. HIS-001-ENGLISH-FREEZE-V5 is the semantic authority. Hindi and Punjabi preserve CP, QL, difficulty, source provenance, option order and correct-index parity.",
  "",
];
for(const locale of locales){
  lines.push("## "+labels[locale],"");
  generateHisCp024LocalizedReviewV1(locale).forEach((q,index)=>lines.push(...renderQuestion(q,index)));
}
const target=path.join(outDir,"HIS-MULTILINGUAL-V1-CP024-REVIEW.md");
