...
type EnglishQuestion = (typeof HIS_CP001_REVIEW_BATCH_V1)[number];
type NativeLocale = Exclude<HisLocaleV1, "en">;

function overlayFor(index: number, locale: NativeLocale): HisNativeOverlayV1 {
  const n = index + 1;
  const overlay = locale === "hi" ? HIS_CP001_HI_V1[n] : HIS_CP001_PA_V1[n];
  if (!overlay) throw new Error(`HIS-CP-001 #${n}: missing ${locale} localization overlay`);
  return overlay;
}
...
function localizedBase(
  ...
  return {
    ...
    localizationV1: {
      ...
      englishFreeze: HIS_ENGLISH_FREEZE_V1,
...
function localizeNative(q: EnglishQuestion, index: number, locale: NativeLocale): HisLocalizedQuestionV1 {
  ...
  const options = q.options.map((englishOption) => {
    ...
    if (!localized) {
      throw new Error(`${q.questionId}: missing ${locale} option mapping for "${englishOption}"`);
    }
    return localized;
  });
...
export function generateHisCp001LocalizedReviewV1(locale: HisLocaleV1): HisLocalizedQuestionV1[] {
  return HIS_CP001_REVIEW_BATCH_V1.map((q, index) => {
...
export const HIS_CP001_LOCALIZATION_V1_SUPPORTED_LOCALES = ["en", "hi", "pa"] as const;
export const HIS_CP001_LOCALIZATION_V1_SUPPORTED_CPS = ["HIS-CP-001"] as const;
