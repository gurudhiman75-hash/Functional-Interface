...
type EnglishQuestion = (typeof HIS_CP007_REVIEW_BATCH_V1)[number];
type NativeLocale = Exclude<HisLocaleV1, "en">;

function overlayFor(index: number, locale: NativeLocale): HisNativeOverlayV1 {
  const n = index + 1;
  const overlay = locale === "hi" ? HIS_CP007_HI_V1[n] : HIS_CP007_PA_V1[n];
  if (!overlay) throw new Error(`HIS-CP-007 #${n}: missing ${locale} localization overlay`);
  return overlay;
}
...
export function generateHisCp007LocalizedReviewV1(locale: HisLocaleV1): HisLocalizedQuestionV1[] {
  return HIS_CP007_REVIEW_BATCH_V1.map((q, index) => {
...
export const HIS_CP007_LOCALIZATION_V1_SUPPORTED_LOCALES = ["en", "hi", "pa"] as const;
export const HIS_CP007_LOCALIZATION_V1_SUPPORTED_CPS = ["HIS-CP-007"] as const;
