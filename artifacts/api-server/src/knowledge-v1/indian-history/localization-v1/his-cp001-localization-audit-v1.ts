...
import { HIS_CP001_HI_V1 } from "./his-cp001-hi-v1";
import { HIS_CP001_PA_V1 } from "./his-cp001-pa-v1";
...
function assertNative(question: HisLocalizedQuestionV1, locale: "hi" | "pa") {
  ...
  fail(question.stem.length <= 260, `${question.questionId}: localized stem too long: ${question.stem.length}`);
...
fail(HIS_CP001_REVIEW_BATCH_V1.length === 54, `CP001 English expected 54, found ${HIS_CP001_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP001_HI_V1).length === 54, `CP001 Hindi overlays expected 54, found ${Object.keys(HIS_CP001_HI_V1).length}`);
fail(Object.keys(HIS_CP001_PA_V1).length === 54, `CP001 Punjabi overlays expected 54, found ${Object.keys(HIS_CP001_PA_V1).length}`);

for (const locale of locales) {
  assertParity(HIS_CP001_REVIEW_BATCH_V1, generateHisCp001LocalizedReviewV1(locale), locale);
}

const all = locales.flatMap((locale) => generateHisCp001LocalizedReviewV1(locale));
fail(all.length === 162, `CP001 expected 162 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP001 multilingual audit passed: 54 questions per locale / 162 EN-HI-PA surfaces.");
