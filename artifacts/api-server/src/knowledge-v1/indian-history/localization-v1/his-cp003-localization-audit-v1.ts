...
import { HIS_CP003_HI_V1 } from "./his-cp003-hi-v1";
import { HIS_CP003_PA_V1 } from "./his-cp003-pa-v1";
...
function assertNative(question: HisLocalizedQuestionV1, locale: "hi" | "pa") {
  ...
  fail(question.stem.length <= 340, `${question.questionId}: localized stem too long: ${question.stem.length}`);
  fail(question.explanation.length >= 90, `${question.questionId}: localized explanation too short: ${question.explanation.length}`);
  fail(question.explanation.length <= 460, `${question.questionId}: localized explanation too long: ${question.explanation.length}`);
...
fail(HIS_CP003_REVIEW_BATCH_V1.length === 60, `CP003 English expected 60, found ${HIS_CP003_REVIEW_BATCH_V1.length}`);
fail(Object.keys(HIS_CP003_HI_V1).length === 60, `CP003 Hindi overlays expected 60, found ${Object.keys(HIS_CP003_HI_V1).length}`);
fail(Object.keys(HIS_CP003_PA_V1).length === 60, `CP003 Punjabi overlays expected 60, found ${Object.keys(HIS_CP003_PA_V1).length}`);

for (const locale of locales) {
  assertParity(HIS_CP003_REVIEW_BATCH_V1, generateHisCp003LocalizedReviewV1(locale), locale);
}

const all = locales.flatMap((locale) => generateHisCp003LocalizedReviewV1(locale));
fail(all.length === 180, `CP003 expected 180 EN-HI-PA surfaces, found ${all.length}`);

console.log("History CP003 multilingual audit passed: 60 questions per locale / 180 EN-HI-PA surfaces.");
