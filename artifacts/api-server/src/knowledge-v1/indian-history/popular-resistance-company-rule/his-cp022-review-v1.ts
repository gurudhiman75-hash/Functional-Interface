...
import { HIS_CP022_FACTS_V1,HIS_CP022_FACT_BY_ID_V1,HIS_CP022_SOURCE_IDS_V1,HIS_CP022_SOURCES_V1 } from "./his-cp022-facts-v1";
import { HIS_CP022_QL_NAMES_V1,HIS_CP022_SPECS_V1 } from "./his-cp022-review-specs-v1";
import { HIS_CP022_EXPLANATION_NOTES_V1 } from "./his-cp022-explanation-notes-v1";
...
export type HisCp022ReviewQuestion={
  questionId:string;chapterId:"HIS-001";cpId:"HIS-CP-022";qlId:string;qlName:string;
...
const canonicalExplanation=(ids:readonly string[])=>ids.map(id=>{
  const f=HIS_CP022_FACT_BY_ID_V1.get(id);
  if(!f)throw new Error(`Unknown HIS-CP-022 fact ${id}`);
  return f[1];
}).join(" ");
const sourceIds=(ids:readonly string[])=>[...new Set(ids.flatMap(id=>HIS_CP022_FACT_BY_ID_V1.get(id)?.[2]??[]))];
...
export function generateHisCp022ReviewBatchV1():HisCp022ReviewQuestion[]{
  return HIS_CP022_SPECS_V1.map((s,i)=>{
    const n=i+1;
    ...
    const note=HIS_CP022_EXPLANATION_NOTES_V1[n];
    if(!note)throw new Error(`Missing HIS-CP-022 explanation note ${n}`);
    const explanation=`${canonicalExplanation(factIds)} ${note}`.trim();
    return{
      questionId:`HIS-CP022-V1-${String(n).padStart(3,"0")}`,
      chapterId:"HIS-001",cpId:"HIS-CP-022",
      qlId:`HIS-022-QL-${String(ql).padStart(3,"0")}`,
      qlName:HIS_CP022_QL_NAMES_V1[ql],
...
export const HIS_CP022_REVIEW_BATCH_V1=Object.freeze(generateHisCp022ReviewBatchV1().map(q=>Object.freeze(q)));
export const HIS_CP022_REQUIRED_FACTS_V1=Object.freeze([...new Set(HIS_CP022_REVIEW_BATCH_V1.flatMap(q=>q.sourceFactIds))].sort());

export function auditHisCp022ReviewBatchV1(){
  const issues:string[]=[];
  const ids=new Set<string>(),semantic=new Set<string>();
  const qlCounts:Record<string,number>={};
  const answerPositions=[0,0,0,0];
  const difficultyCounts={Easy:0,Medium:0,Hard:0};
  const priorFacts=[...HIS_CP012_FACTS_V1,...HIS_CP013_FACTS_V1,...HIS_CP014_FACTS_V1] as readonly (readonly [string,string,readonly string[]])[];
  const priorText=new Map(priorFacts.map(f=>[normalize(f[1]),f[0]] as const));

  for(const f of HIS_CP022_FACTS_V1){
    for(const s of f[2])if(!HIS_CP022_SOURCE_IDS_V1.has(s))issues.push(`UNKNOWN_SOURCE:${f[0]}:${s}`);
  ...
  for(const q of HIS_CP022_REVIEW_BATCH_V1){
    ...
    if(q.explanation.length>820)issues.push(`EXPLANATION_TOO_LONG:${q.questionId}:${q.explanation.length}`);
  ...
  for(let i=1;i<=10;i++){const q=`HIS-022-QL-${String(i).padStart(3,"0")}`;if(qlCounts[q]!==6)issues.push(`QL_COUNT:${q}:${qlCounts[q]??0}`);}
  if(HIS_CP022_REVIEW_BATCH_V1.length!==60)issues.push(`QUESTION_COUNT:${HIS_CP022_REVIEW_BATCH_V1.length}`);
  if(HIS_CP022_FACTS_V1.length!==60)issues.push(`FACT_COUNT:${HIS_CP022_FACTS_V1.length}`);
  if(HIS_CP022_REQUIRED_FACTS_V1.length!==60)issues.push(`FACT_COVERAGE:${HIS_CP022_REQUIRED_FACTS_V1.length}/60`);
  if(Object.keys(HIS_CP022_EXPLANATION_NOTES_V1).length!==60)issues.push(`EXPLANATION_NOTE_COUNT:${Object.keys(HIS_CP022_EXPLANATION_NOTES_V1).length}`);
  if(Math.max(...answerPositions)-Math.min(...answerPositions)>1)issues.push(`ANSWER_POSITION_IMBALANCE:${answerPositions.join(",")}`);
  return{valid:issues.length===0,questionCount:HIS_CP022_REVIEW_BATCH_V1.length,semanticCount:semantic.size,canonicalFactCount:HIS_CP022_FACTS_V1.length,requiredFactCount:HIS_CP022_REQUIRED_FACTS_V1.length,sourceCount:HIS_CP022_SOURCES_V1.length,qlCounts,difficultyCounts,answerPositions,issues};
}
