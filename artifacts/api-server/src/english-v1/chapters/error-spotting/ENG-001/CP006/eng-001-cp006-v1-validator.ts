import type { ComparisonRuleId, Eng001Question, ValidationIssue, ValidationResult } from "../../../../core/types";
import { COMPARISON_RULE_BY_ID } from "../../../../grammar/comparison";
import { CP006_SCENES_V4 } from "./cp006-catalog-v4";
...
const inventedFormBan = /\b(?:wellly|gooder|goodest|badder|baddest|more good|more bad|most good|most bad)\b/i;

const issue = (code: ValidationIssue["code"], message: string): ValidationIssue => ({ code, message });
const sceneIdFromCandidateId = (candidateId: string): string => candidateId.replace(/^CMP-V1:/, "");

export function validateEng001Cp006QuestionV1(question: Eng001Question): ValidationResult {
  const issues: ValidationIssue[] = [];
  const { metadata } = question;
  if (metadata.cpId !== "ENG-001-CP006") issues.push(issue("RULE_MUTATION_MISMATCH", `Expected CP006 metadata, got ${metadata.cpId}.`));
  if (!String(metadata.ruleId).startsWith("GR-CMP-")) issues.push(issue("RULE_MUTATION_MISMATCH", `CP006 must use a GR-CMP rule, got ${metadata.ruleId}.`));
  const rule = COMPARISON_RULE_BY_ID[metadata.ruleId as ComparisonRuleId];
  if (!rule) issues.push(issue("RULE_MUTATION_MISMATCH", `Unknown CP006 rule ${metadata.ruleId}.`));
  ...
  const scene = CP006_SCENES_V4.find((entry) => entry.id === sceneIdFromCandidateId(metadata.candidateId));
  if (!scene) {
    issues.push(issue("ERROR_INDEX", `Unknown CP006 candidate ${metadata.candidateId}.`));
  } else {
  ...
  if (inventedFormBan.test(visible)) issues.push(issue("EXPLANATION_MISMATCH", "Question uses an artificial giveaway form instead of a realistic exam error."));
...
export function assertValidEng001Cp006QuestionV1(question: Eng001Question): void {
  const result = validateEng001Cp006QuestionV1(question);
