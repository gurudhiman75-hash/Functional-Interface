...
export function buildEng001Cp008CandidateV1(input: { seed: string; difficulty: EnglishDifficulty; ruleId?: NounQuantifierRuleId; sceneId?: string }): Eng001SentenceCandidate {
  const pool = cp008ScenePoolV1(input.difficulty, input.ruleId);
  const selected = input.sceneId ? pool.find((candidate) => candidate.id === input.sceneId) : deterministicPick(`${input.seed}:cp008:scene`, pool);
  if (!selected) throw new Error(`Unknown CP008 ${input.difficulty} scene ${input.sceneId}.`);
  ...
  const rule = NOUN_QUANTIFIER_RULE_BY_ID[selected.ruleId];
  return {
    candidateId: `NQN-V1:${selected.id}`,
    ...
    tags: [`domain:${selected.domain}`, `scene:${selected.id}`, `noun-quantifier-rule:${selected.ruleId}`],
  };
}
...
function renderExplanation(input: { seed: string; qlId: Eng001QlId; candidate: Eng001SentenceCandidate; answerLabel: string; correctedSentence: string; noError: boolean }): string {
  ...
  const styleSeed = `${seed}:cp008:explanation:${candidate.candidateId}:${qlId}`;
...
function shapeQl002(segments: readonly string[], errorIndex: number, seed: string): { segments: string[]; errorIndex: number } {
  if (segments.length !== 4) throw new Error("CP008 QL002 expects four authored segments.");
  const mergeCandidates = [0, 1, 2].filter((at) => at !== errorIndex && at + 1 !== errorIndex);
  if (!mergeCandidates.length) throw new Error(`CP008 QL002 cannot preserve keyed segment ${errorIndex}.`);
  const mergeAt = deterministicPick(`${seed}:cp008:ql002-merge`, mergeCandidates);
  const out: string[] = [];
  let mapped = -1;
  ...
  if (mapped < 0) throw new Error("CP008 QL002 lost the keyed error segment.");
  return { segments: out, errorIndex: mapped };
}

export interface GenerateEng001Cp008V1Input {
  seed: string;
...
export function generateEng001Cp008QuestionV1(input: GenerateEng001Cp008V1Input): Eng001Question {
  const qlId = input.qlId ?? deterministicPick(`${input.seed}:cp008:ql`, ["ENG-001-QL001", "ENG-001-QL002", "ENG-001-QL007"] as const);
  const candidate = buildEng001Cp008CandidateV1(input);
  ...
  return {
    questionId: `ENG-001-CP008-V1:${qlId}:${candidate.candidateId}:${input.seed}`,
    stem: STEMS[qlId],
    ...
    metadata: {
      track: "english", chapterId: "ENG-001", cpId: "ENG-001-CP008", qlId,
