...
import { CONDITIONAL_RULE_BY_ID, type ConditionalMutationId, type ConditionalRuleId } from "../../../../grammar/conditionals";
...
export interface ConditionalCandidateV1 {
  candidateId: string;
...
export function cp011ScenePoolV1(difficulty: EnglishDifficulty, ruleId?: ConditionalRuleId): readonly ConditionalSceneV1[] {
  const base = CP011_SCENES_BY_DIFFICULTY_V1[difficulty];
  const filtered = ruleId ? base.filter((scene) => scene.ruleId === ruleId) : base;
  if (!filtered.length) throw new Error(`No CP011 ${difficulty} scene is available${ruleId ? ` for ${ruleId}` : ""}.`);
  return filtered;
}

export function rulesForDifficultyCp011V1(difficulty: EnglishDifficulty): readonly ConditionalRuleId[] {
  return [...new Set(CP011_SCENES_BY_DIFFICULTY_V1[difficulty].map((scene) => scene.ruleId))];
}

export function buildEng001Cp011CandidateV1(input: { seed: string; difficulty: EnglishDifficulty; ruleId?: ConditionalRuleId; sceneId?: string; }): ConditionalCandidateV1 {
  const pool = cp011ScenePoolV1(input.difficulty, input.ruleId);
  const selected = input.sceneId ? pool.find((candidate) => candidate.id === input.sceneId) : deterministicPick(`${input.seed}:cp011:scene`, pool);
  if (!selected) throw new Error(`Unknown CP011 ${input.difficulty} scene ${input.sceneId}.`);
  ...
  const rule = CONDITIONAL_RULE_BY_ID[selected.ruleId];
  return {
    candidateId: `CND-V1:${selected.id}`,
    ...
    tags: [`domain:${selected.domain}`, `scene:${selected.id}`, `conditional-rule:${selected.ruleId}`],
  };
}

const sentenceFromSegments = (segments: readonly string[]): string => segments.join(" ").replace(/\s+([,.!?;:])/g, "$1").replace(/\s+/g, " ").trim();

function renderExplanation(input: { candidate: ConditionalCandidateV1; answerLabel: string; correctedSentence: string; noError: boolean; }): string {
...
function shapeQl002(segments: readonly string[], errorIndex: number, seed: string): { segments: string[]; errorIndex: number } {
  if (segments.length !== 4) throw new Error("CP011 QL002 expects four authored segments.");
  const mergeCandidates = [0, 1, 2].filter((at) => at !== errorIndex && at + 1 !== errorIndex);
  if (!mergeCandidates.length) throw new Error(`CP011 QL002 cannot preserve keyed segment ${errorIndex}.`);
  const mergeAt = deterministicPick(`${seed}:cp011:ql002-merge`, mergeCandidates);
  const out: string[] = [];
  let mapped = -1;
  ...
  if (mapped < 0) throw new Error("CP011 QL002 lost the keyed error segment.");
  return { segments: out, errorIndex: mapped };
}

export interface GenerateEng001Cp011V1Input { seed: string; difficulty: EnglishDifficulty; qlId?: Eng001QlId; ruleId?: ConditionalRuleId; sceneId?: string; }

export function generateEng001Cp011QuestionV1(input: GenerateEng001Cp011V1Input): Eng001Question {
  const qlId = input.qlId ?? deterministicPick(`${input.seed}:cp011:ql`, ["ENG-001-QL001", "ENG-001-QL002", "ENG-001-QL007"] as const);
  const candidate = buildEng001Cp011CandidateV1(input);
  ...
  const metadata = {
    track: "english", chapterId: "ENG-001", cpId: "ENG-001-CP011", qlId,
  ...
  return {
    questionId: `ENG-001-CP011-V1:${qlId}:${candidate.candidateId}:${input.seed}`,
