...
import { CP009_SCENES_BY_DIFFICULTY_V3 } from "./cp009-catalog-v3";
...
export function cp009ScenePoolV1(
  ...
  const base = CP009_SCENES_BY_DIFFICULTY_V3[difficulty];
...
export function rulesForDifficultyCp009V1(difficulty: EnglishDifficulty): readonly GerundInfinitiveParticipleRuleId[] {
  return [...new Set(CP009_SCENES_BY_DIFFICULTY_V3[difficulty].map((scene) => scene.ruleId))];
}
...
export function buildEng001Cp009CandidateV1(input: {
  ...
  return {
    ...
    tags: [`domain:${selected.domain}`, `scene:${selected.id}`, `gip-rule:${selected.ruleId}`, "presentation:v3-balanced"],
  };
}
