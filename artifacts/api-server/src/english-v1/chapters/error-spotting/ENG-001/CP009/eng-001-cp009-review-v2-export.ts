...
export function renderEng001Cp009ReviewV2(): string {
  const lines: string[] = [
    "# ENG-001 CP009 — Gerunds, Infinitives & Participles — Human Review V2",
    "",
    "Status: `REVIEW_CANDIDATE_V2__HUMAN_REVIEW_PENDING__NOT_QUESTION_STUDIO_REGISTERED`",
    "",
    "All 60 authored invalid mutations are included. Each difficulty has an exact A=5, B=5, C=5, D=5 source-answer spread. Ambiguous participial parses found during self-audit were removed at source level.",
    "",
  ];
...
export function writeEng001Cp009ReviewV2(outputPath: string): void {
  mkdirSync(dirname(outputPath), { recursive: true });
  const rendered = renderEng001Cp009ReviewV2();
  writeFileSync(outputPath, rendered, "utf8");
  console.log(`[ENG-001-CP009-REVIEW-V2] wrote ${outputPath} (${Buffer.byteLength(rendered)} bytes)`);
}
