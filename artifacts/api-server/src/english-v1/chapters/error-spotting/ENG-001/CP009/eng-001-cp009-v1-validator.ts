...
const sceneIdFromCandidateId = (candidateId: string): string => candidateId.replace(/^GIP-V1:/, "");

export function validateEng001Cp009QuestionV1(question: Eng001Question): ValidationResult {
  const issues: ValidationIssue[] = [];
  const { metadata } = question;

  if (metadata.cpId !== "ENG-001-CP009") {
    issues.push(issue("RULE_MUTATION_MISMATCH", `Expected CP009 metadata, got ${metadata.cpId}.`));
  }
  if (!String(metadata.ruleId).startsWith("GR-GIP-")) {
    issues.push(issue("RULE_MUTATION_MISMATCH", `CP009 must use a GR-GIP rule, got ${metadata.ruleId}.`));
  }

  const rule = GERUND_INFINITIVE_PARTICIPLE_RULE_BY_ID[
    metadata.ruleId as GerundInfinitiveParticipleRuleId
  ];
  if (!rule) {
    issues.push(issue("RULE_MUTATION_MISMATCH", `Unknown CP009 rule ${metadata.ruleId}.`));
  } else if (rule.mutationId !== metadata.mutationId) {
    issues.push(issue("RULE_MUTATION_MISMATCH", `${metadata.ruleId} must map to ${rule.mutationId}.`));
  }
  ...
  const scene = CP009_SCENES_V1.find((entry) => entry.id === sceneIdFromCandidateId(metadata.candidateId));
  if (!scene) {
    issues.push(issue("ERROR_INDEX", `Unknown CP009 candidate ${metadata.candidateId}.`));
  } else {
    if (scene.ruleId !== metadata.ruleId) {
...
export function assertValidEng001Cp009QuestionV1(question: Eng001Question): void {
  const result = validateEng001Cp009QuestionV1(question);
  if (!result.ok) {
