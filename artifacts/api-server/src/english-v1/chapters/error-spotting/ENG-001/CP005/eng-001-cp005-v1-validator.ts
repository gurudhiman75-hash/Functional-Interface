...
const sceneIdFromCandidateId = (candidateId: string): string => candidateId.replace(/^PRP-V1:/, "");

export function validateEng001Cp005QuestionV1(question: Eng001Question): ValidationResult {
  const issues: ValidationIssue[] = [];
  const { metadata } = question;
  if (metadata.cpId !== "ENG-001-CP005") issues.push(issue("RULE_MUTATION_MISMATCH", `Expected CP005 metadata, got ${metadata.cpId}.`));
  if (!String(metadata.ruleId).startsWith("GR-PRP-")) issues.push(issue("RULE_MUTATION_MISMATCH", `CP005 must use a GR-PRP rule, got ${metadata.ruleId}.`));
  const rule = PREPOSITION_RULE_BY_ID[metadata.ruleId as PrepositionRuleId];
  if (!rule) issues.push(issue("RULE_MUTATION_MISMATCH", `Unknown CP005 rule ${metadata.ruleId}.`));
  ...
  const scene = rawScene ? remediateCp005SceneForClosureV1(rawScene) : undefined;
  if (!scene) {
    ...
    if (!scene.correction.includes(changedCorrect) && !changedCorrect.includes(scene.correction)) issues.push(issue("CORRECTION_MISSING", `Scene ${scene.id} correction is not connected to the corrected segment.`));
  ...
  if (!/(Correct sentence:|corrected sentence is:|sentence should read:|Corrected sentence:|sentence remains:)/i.test(question.explanation)) issues.push(issue("EXPLANATION_MISMATCH", "Explanation must show the full correct sentence."));
...
export function assertValidEng001Cp005QuestionV1(question: Eng001Question): void {
  const result = validateEng001Cp005QuestionV1(question);
