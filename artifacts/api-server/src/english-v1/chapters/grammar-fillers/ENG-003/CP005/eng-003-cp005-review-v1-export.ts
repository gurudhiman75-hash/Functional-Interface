...
import { generateEng003Cp005QuestionV1 } from "./eng-003-cp005-v1";

const OUTPUT = resolve(process.cwd(), "dist/english-v1/ENG-003-CP005-REVIEW-V1.md");
...
function reviewScenes(difficulty: EnglishDifficulty) {
  ...
  if (selected.length !== SAMPLES_PER_DIFFICULTY) {
    throw new Error(`ENG-003 CP005 needs ${SAMPLES_PER_DIFFICULTY} ${difficulty} review scenes; received ${selected.length}`);
  }
  return selected;
}

const lines: string[] = [
  "# ENG-003-CP005 — Preposition Grammar Fillers — Review V1",
  "",
...
for (const difficulty of DIFFICULTIES) {
  ...
  for (const scene of reviewScenes(difficulty)) {
    const seed = `eng003-cp005-review-v1:${difficulty}:${scene.id}`;
...
lines.push(
  ...
  "- Only the tested preposition is blanked; the surrounding phrase remains visible.",
  "- All four options are concise preposition choices.",
  "- Exactly one option is defensible from the sentence meaning or fixed combination.",
