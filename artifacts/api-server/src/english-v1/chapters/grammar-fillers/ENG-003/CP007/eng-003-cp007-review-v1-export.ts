...
import { generateEng003Cp007QuestionV1 } from "./eng-003-cp007-v1";

const OUTPUT = resolve(process.cwd(), "dist/english-v1/ENG-003-CP007-REVIEW-V1.md");
...
function reviewScenes(difficulty: EnglishDifficulty) {
  ...
  if (selected.length !== SAMPLES_PER_DIFFICULTY) throw new Error(`ENG-003 CP007 needs ${SAMPLES_PER_DIFFICULTY} ${difficulty} review scenes; received ${selected.length}`);
  return selected;
}

const lines: string[] = [
  "# ENG-003-CP007 — Conjunctions and Parallelism Grammar Fillers — Review V1",
  "",
...
for (const difficulty of DIFFICULTIES) {
  ...
  for (const scene of reviewScenes(difficulty)) {
    const seed = `eng003-cp007-review-v1:${difficulty}:${scene.id}`;
    const question = generateEng003Cp007QuestionV1({ difficulty, ruleId: scene.ruleId, sceneId: scene.id, seed });
...
lines.push(
  ...
  "- Only the approved conjunction/parallelism target is blanked.",
  "- Phrase-level blanks are retained when the rule depends on the whole phrase.",
  "- Exactly one option is defensible.",
  "- Distractors stay within the same grammar distinction and do not look mechanically generated.",
