...
import { generateEng003Cp006QuestionV1 } from "./eng-003-cp006-v1";

const OUTPUT = resolve(process.cwd(), "dist/english-v1/ENG-003-CP006-REVIEW-V1.md");
...
function reviewScenes(difficulty: EnglishDifficulty) {
  ...
  if (selected.length !== SAMPLES_PER_DIFFICULTY) {
    throw new Error(`ENG-003 CP006 needs ${SAMPLES_PER_DIFFICULTY} ${difficulty} review scenes; received ${selected.length}`);
  }
  return selected;
}

const lines: string[] = [
  "# ENG-003-CP006 — Adjectives, Adverbs and Comparison Grammar Fillers — Review V1",
  "",
...
for (const difficulty of DIFFICULTIES) {
  ...
  for (const scene of reviewScenes(difficulty)) {
    const seed = `eng003-cp006-review-v1:${difficulty}:${scene.id}`;
...
lines.push(
  ...
  "- Only the focused adjective/adverb/comparison target is blanked.",
  "- Exactly one option is defensible.",
  "- Distractors stay inside the same grammar distinction.",
  "- No fabricated comparison forms appear.",
