...
import { cp010ScenePoolV1 } from "../../../error-spotting/ENG-001/CP010/eng-001-cp010-v1";
import { generateEng003Cp010QuestionV1 } from "./eng-003-cp010-v1";

const OUTPUT = resolve(process.cwd(), "dist/english-v1/ENG-003-CP010-REVIEW-V1.md");
...
function reviewScenes(difficulty: EnglishDifficulty) {
  ...
  if (selected.length !== SAMPLES_PER_DIFFICULTY) {
    throw new Error(`ENG-003 CP010 needs ${SAMPLES_PER_DIFFICULTY} ${difficulty} review scenes; received ${selected.length}`);
  }
  return selected;
}

const lines: string[] = [
  "# ENG-003-CP010 — Modifiers and Placement Grammar Fillers — Review V1",
  "",
...
for (const difficulty of DIFFICULTIES) {
  ...
  for (const scene of reviewScenes(difficulty)) {
    const seed = `eng003-cp010-review-v1:${difficulty}:${scene.id}`;
...
lines.push(
  ...
  "- The blank contains the smallest natural phrase needed to test modifier scope or attachment.",
  "- Exactly one option gives the intended, grammatically correct modifier placement.",
  "- Distractors remain natural enough to require grammar rather than pattern guessing.",
  "- No duplicated or mechanically generated modifier phrase appears.",
