...
import { ENG002_CP013_STEM, generateEng002Cp013QuestionV1, type Eng002Cp013QuestionV1 } from "./eng-002-cp013-v1";

export interface Eng002Cp013ReviewItemV1 { number: number; difficulty: EnglishDifficulty; question: Eng002Cp013QuestionV1 }

export function buildEng002Cp013ReviewV1(): Eng002Cp013ReviewItemV1[] {
  const items: Eng002Cp013ReviewItemV1[] = [];
  let number = 1;
  for (const difficulty of ["easy", "medium", "hard"] as const) {
    cp013ScenePoolV1(difficulty).forEach((scene, index) => {
      items.push({
        number,
        difficulty,
        question: generateEng002Cp013QuestionV1({
          seed: `eng002-cp013-review:${difficulty}:${scene.id}`,
          difficulty,
          ruleId: scene.ruleId,
          sceneId: scene.id,
...
function underlinedSentence(question: Eng002Cp013QuestionV1) {
...
export function renderEng002Cp013ReviewMarkdownV1() {
  const lines: string[] = [
    "# ENG-002 CP013 — Sentence Improvement: Common Usage / Idiomatic Grammar — Review V1",
    "",
    `**Instruction:** ${ENG002_CP013_STEM}`,
    "",
    "Review-only artifact. Not authorized for Question Bank, tests, mocks, public publication or production release.",
    "",
  ];
  let currentDifficulty = "";
  for (const item of buildEng002Cp013ReviewV1()) {
