...
import { generateEng002Cp004QuestionV1, type Eng002Cp004QuestionV1 } from "./eng-002-cp004-v1";

export interface Eng002Cp004ReviewItemV1 {
  difficulty: EnglishDifficulty;
  question: Eng002Cp004QuestionV1;
}

function buildSlice(difficulty: EnglishDifficulty): Eng002Cp004ReviewItemV1[] {
  const scenes = cp004ScenePoolV1(difficulty);
  if (scenes.length !== 20) throw new Error(`ENG-002 CP004 needs 20 ${difficulty} review scenes; received ${scenes.length}`);
  return scenes.map((scene, index) => ({
    difficulty,
    question: generateEng002Cp004QuestionV1({
      difficulty,
      ruleId: scene.ruleId,
      sceneId: scene.id,
      noImprovement: index % 4 === 3,
      seed: `eng002-cp004-review-v1:${difficulty}:${scene.id}`,
    }),
  }));
}

export function buildEng002Cp004ReviewV1(): Eng002Cp004ReviewItemV1[] {
  return (["easy", "medium", "hard"] as const).flatMap(buildSlice);
}

function renderSentence(question: Eng002Cp004QuestionV1): string {
...
export function renderEng002Cp004ReviewMarkdownV1(): string {
  const review = buildEng002Cp004ReviewV1();
  const lines: string[] = [
    "# ENG-002-CP004 — Sentence Improvement: Pronouns — Review V1",
    "",
