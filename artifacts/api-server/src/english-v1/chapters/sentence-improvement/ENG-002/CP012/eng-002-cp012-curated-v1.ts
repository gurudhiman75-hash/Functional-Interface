...
function generateReportingVerb(input: GenerateEng002Cp012V1Input): Eng002Cp012QuestionV1 {
  const candidate = buildEng001Cp012CandidateV1({ seed: input.seed, difficulty: input.difficulty, ruleId: "GR-VNR-012", sceneId: input.sceneId });
  ...
  const wrongs = reportingVerbWrongs(correct.body, wrong.body).filter((value) => value.toLowerCase() !== targetText.toLowerCase());
  const choices = noImprovement ? wrongs.slice(0, 3) : unique([correct.body, ...wrongs]).slice(0, 3);
  if (choices.length !== 3) throw new Error(`GR-VNR-012 could not build three curated choices for ${correct.body}`);
  ...
  const principle = VOICE_NARRATION_RULE_BY_ID["GR-VNR-012"].principle.replace(/^./, (c) => c.toUpperCase());
...
export function generateEng002Cp012CuratedQuestionV1(input: GenerateEng002Cp012V1Input): Eng002Cp012QuestionV1 {
  if (input.ruleId === "GR-VNR-012") return generateReportingVerb(input);
  try {
    ...
    if (candidate.ruleId === "GR-VNR-012") return generateReportingVerb({ ...input, ruleId: "GR-VNR-012", sceneId: tag(candidate, "scene:") });
    throw error;
  }
}
