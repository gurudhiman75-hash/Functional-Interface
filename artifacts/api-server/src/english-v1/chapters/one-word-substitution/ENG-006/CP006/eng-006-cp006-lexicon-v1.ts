...
for(const raw of RAW.split("\n")){const line=raw.trim();if(!line)continue;if(line.startsWith("#")){category=line.slice(1);within=0;continue;}const [answer,definition]=line.split("|");if(!answer||!definition)throw new Error(`Invalid ENG-006 CP006 row: ${line}`);const difficulty:Eng006Cp006Difficulty=within<5?"easy":within<13?"medium":"hard";rows.push({id:`OWS6-${String(rows.length+1).padStart(3,"0")}`,answer,definition,category,difficulty});within++;}
