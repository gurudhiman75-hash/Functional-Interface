...
const OUT=resolve(process.cwd(),"dist/english-v1/ENG-006-CHAPTER-REVIEW-V1.md"),L=["A","B","C","D"]as const,D=["easy","medium","hard"]as const;
const lines:string[]=["# ENG-006 — One-word Substitution — Full Chapter Review V1","","Status: `HUMAN_EDITORIAL_REVIEW_PENDING__REVIEW_ONLY`","","Total bank: **840 unique substitutions**.","","Review sample: **120 questions — 30 from each checkpoint, balanced across Easy / Medium / Hard**.","","---",""];
const specs=[
...
for(const spec of specs){lines.push(`# ${spec.cp}`,"");for(const d of D){lines.push(`## ${d[0]!.toUpperCase()+d.slice(1)}`,"");const p=spec.pool(d);for(let i=0;i<10;i++){const e:any=p[Math.floor(i*p.length/10)]!,seed=`eng006-chapter-review:${spec.cp}:${d}:${e.id}`,q:any=spec.gen({seed,difficulty:d,entryId:e.id});lines.push(`### Q${String(n++).padStart(3,"0")}`,"",q.stem,"");q.options.forEach((o:string,j:number)=>lines.push(`${L[j]}. ${o}`));lines.push("",`**Answer:** ${L[q.correctOptionIndex]}. ${q.options[q.correctOptionIndex]}`,"",`**Explanation:** ${q.explanation}`,"",`**Domain:** ${q.metadata.category}  `,`**Entry:** ${q.metadata.entryId}  `,"","---","");}}}
lines.push("## Chapter review checklist","","- Definitions are precise and exam-standard.","- Exactly one option is correct.","- Distractors are plausible and from the same semantic family.","- Advanced entries are useful rather than needlessly obscure.","- Easy/Medium/Hard calibration feels meaningful.","- Explanations are simple and direct.","");
