...
const configs={
 ...
 BANKING_PRELIMS_RC:{defaultCount:9,allowed:[8,9],passages:ENG008_CP003_PASSAGES_V1,core:["BP-F01","BP-F02","BP-F03","BP-F04","BP-F05","BP-F07","BP-F08"],optional:["BP-F06","BP-F09"]},
...
function selectedFamilies(profile:Eng008SetProfile,count:number,seed:string){const cfg=configs[profile],all=[...cfg.core,...rotate(cfg.optional,`${seed}:optional`)];return all.slice(0,count);}
...
function generate(profile:Eng008SetProfile,passageId:string,family:string,seed:string){
 ...
 if(profile==="BANKING_PRELIMS_RC")return generateEng008Cp003QuestionV1({seed,difficulty:d,familyId:family as any,authorityId:(ENG008_CP003_PASSAGES_V1.find(p=>p.id===passageId)!.questions.find(q=>q.familyId===family)!.id)});
...
export function generateEng008Cp006SetV1(input:GenerateEng008Cp006SetV1Input){
 ...
 return{setId:`ENG-008-CP006-V1:${input.profile}:${passage.id}:${input.seed}`,profile:input.profile,passageId:passage.id,passage:passage.text,questionCount:questions.length,questions,reviewOnly:true as const};
}
