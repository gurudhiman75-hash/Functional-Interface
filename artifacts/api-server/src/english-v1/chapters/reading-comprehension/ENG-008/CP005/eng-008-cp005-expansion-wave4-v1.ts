...
The next phase will test improved occupancy estimates and compare behaviour among frequent commuters, occasional riders and passengers who receive the same information on stop displays rather than through the app. Researchers also plan to measure whether crowding is redistributed rather than reduced. If many informed passengers simply move from one bus to the next, the first vehicle may become less crowded while the following one becomes more crowded. That pattern would still affect comfort, but it would be different from a genuine reduction in total passenger density. The authority also plans to compare predicted crowding with actual on-board counts after each trip. That will show whether inaccurate alerts weaken trust over time. A passenger may stop using the feature if several warnings prove wrong, even if the system is generally useful. Reliability therefore needs to be measured alongside behavioural change.`,
 questions:[
