import { ENG008_CP001_EXPANSION_WAVE8_V1 } from "./eng-008-cp001-expansion-wave8-v1";
import { ENG008_CP001_EXPANSION_WAVE7_V1 } from "./eng-008-cp001-expansion-wave7-v1";
import { ENG008_CP001_EXPANSION_WAVE6_V1 } from "./eng-008-cp001-expansion-wave6-v1";
import { ENG008_CP001_EXPANSION_WAVE5_V1 } from "./eng-008-cp001-expansion-wave5-v1";
import { ENG008_CP001_EXPANSION_WAVE4_V1 } from "./eng-008-cp001-expansion-wave4-v1";
import { ENG008_CP001_EXPANSION_WAVE3_V1 } from "./eng-008-cp001-expansion-wave3-v1";
...
...ENG008_CP001_EXPANSION_WAVE4_V1,
...ENG008_CP001_EXPANSION_WAVE3_V1,
...
The notebook also contained a rough list of dates beside the interview questions. Mira guessed that the grandson had already begun planning the conversation carefully. When she later saw the notebook returned, she understood why losing even a small object can disrupt work that depends on preparation. The episode also changed how she treated other lost items in public places: instead of assuming they were unimportant, she began thinking about the purpose they might serve for someone else.`,
 questions:[
...
For the rest of the week, customers kept pointing at the sign and telling newcomers about the change. Mr Sen noticed that the confusion disappeared faster once people had a clear explanation for why their usual habit no longer worked. The clock itself had needed only a mechanical repair; the lane needed a short period of adjustment. He later joked that fixing a machine can be easier than fixing the expectations people build around it.`,
 questions:[
...
The students soon divided simple responsibilities. One group watered the plants before assembly, while another recorded which herbs grew best in the patch. The science teacher did not turn the garden into a formal experiment, but the students began noticing differences in shade, soil moisture and insect activity. The space became useful partly because it stayed small enough for students to observe regularly rather than becoming a project too large to maintain.`,
 questions:[
...
The following week, the transport department replaced the damaged notice with a printed board and added an arrow toward the temporary stop. Leena passed the market again and saw fewer people asking for directions. The change did not make the detour shorter, but it made the new arrangement easier to understand. She realised that people often accept inconvenience more calmly when the information around it is clear and timely.`,
 questions:[
...
The committee also mapped places where shade already worked well. In some stretches, a narrow awning protected pedestrians for most of the afternoon even though no tree was present. This helped the group avoid recommending the same solution everywhere. Their final note suggested that trees, awnings and seating should be planned according to pavement width, underground utilities and pedestrian movement rather than added simply to increase the number of visible features.`,
 questions:[
...
After the repair, students continued taking readings for another two weeks. Overnight use remained low, which gave the maintenance team confidence that the leak had been the main source of unexplained loss. Lunch-time consumption, however, stayed relatively high. Teachers therefore used the audit results to separate two issues: a technical fault that required repair and everyday habits that required reminders and better supervision.`,
 questions:[
...
Staff also tracked the kinds of questions people asked at the help desks. Most were not about advanced features; they involved basic steps such as checking whether a payment had gone through, identifying a merchant name or finding a transaction reference. The organisers concluded that confidence often depends less on adding more digital functions than on making ordinary actions easier to understand when something does not happen as expected.`,
 questions:[
...
The pilot team also compared late departures with late arrivals. Some buses left the depot on time but became delayed near two busy junctions, while others started late because earlier trips had returned behind schedule. This distinction mattered because the same visible problem—an overdue bus—could require different remedies. The report therefore recommended separating route delays from depot delays before changing the timetable.`,
 questions:[
...
A library may also be difficult to use even when its doors are technically open. Readers who cannot locate books, find a quiet seat or ask for help may leave without using the collection effectively. Digital catalogues can improve access, but only if they are simple enough for occasional visitors as well as regular users. The same principle applies to study areas, lighting and basic guidance: a public resource creates value when people can actually navigate it.

This is why counting books alone gives an incomplete picture of library quality. Collection size matters, but so do opening hours, staff support, catalogue clarity and the number of usable study spaces. A strong library combines resources with the practical conditions that allow different kinds of readers to benefit from them.`,
 questions:[
...
A repair-first approach can also encourage better product design. If batteries, screens or common parts can be replaced without damaging the whole device, owners have more real choices when something fails. By contrast, a product that is difficult to open or uses unavailable parts may force replacement even when the underlying fault is small.

The sensible policy is therefore not to glorify repair in every case, but to preserve the option where it is safe and economical. Consumers should be able to compare the cost of repair, expected life after repair and the performance of a replacement. That comparison is more useful than assuming that either the newest product or the oldest one is automatically the better choice.`,
 questions:[
...
A school garden also needs a clear educational purpose. If students only plant once and teachers never return to the site, the area can quickly become another maintenance task. By contrast, even a small garden can support observation of insects, soil, weather, nutrition and plant growth when it is used repeatedly across lessons.

The strongest argument for such a garden is therefore not that every school needs a large green project. It is that an existing outdoor space can become more valuable when it is linked to regular learning. A modest plot that students observe, measure and care for may teach more than a larger garden that is attractive but rarely used.`,
 questions:[
...
Context should also explain whether figures are directly comparable over time. A department may improve its method, expand the number of places surveyed or change the definition of what counts as a case. In such situations, a rise or fall can partly reflect better measurement rather than a real change in the underlying condition.

Good dashboards therefore need short methodological notes, not only attractive charts. Readers should be able to see the time period, source, definition and any important break in the series. This does not make public data less accessible; it makes the simplicity of the display less likely to create a misleading conclusion.`,
 questions:[
