...
import { ENG002_CP012_STEM, generateEng002Cp012QuestionV1 } from "../chapters/sentence-improvement/ENG-002/CP012/eng-002-cp012-v1";

function assertClean(q: ReturnType<typeof generateEng002Cp012QuestionV1>) {
  ...
  assert.equal(/\b(was|were|is|are|has|have|had|will|would|can|could|may|might|must|should)\s+\1\b/i.test(q.options.slice(0, 3).join("\n")), false);
...
for (const difficulty of ["easy", "medium", "hard"] as const) {
  ...
  for (let index = 0; index < 2000; index += 1) {
    ...
    const first = generateEng002Cp012QuestionV1({ seed, difficulty });
    assertClean(first);
    assert.deepEqual(generateEng002Cp012QuestionV1({ seed, difficulty }), first);
  ...
  for (const ruleId of rulesForDifficultyCp012V1(difficulty)) {
    ...
    for (const noImprovement of [false, true]) {
      const q = generateEng002Cp012QuestionV1({ seed: `eng002-cp012-rule:${difficulty}:${ruleId}:${noImprovement}`, difficulty, ruleId, sceneId: scene.id, noImprovement });
