...
import { generateEng003Cp006QuestionV1, materializeEng003Cp006AnswerV1 } from "../chapters/grammar-fillers/ENG-003/CP006/eng-003-cp006-v1";
...
for (const difficulty of ["easy", "medium", "hard"] as const) {
  const pool = cp006ScenePoolV1(difficulty);
  const allowedRules = new Set(rulesForDifficultyCp006V1(difficulty));
  ...
  for (let index = 0; index < 2_000; index += 1) {
    const input = { difficulty, seed: `eng003-cp006-test:${difficulty}:${index}` } as const;
    const first = generateEng003Cp006QuestionV1(input);
    const replay = generateEng003Cp006QuestionV1(input);
    ...
    assert(!bannedFabrications.test(first.options.join(" ")), `${first.questionId} contains a fabricated comparison form`);
    ...
    assert(first.metadata.chapterId === "ENG-003" && first.metadata.cpId === "ENG-003-CP006", `${first.questionId} has wrong ids`);
    ...
    assert(
      materializeEng003Cp006AnswerV1(first.segments, first.blankIndex, answer) === first.correctedSentence,
  ...
  for (const ruleId of allowedRules) {
    ...
    const question = generateEng003Cp006QuestionV1({
      difficulty,
      ruleId,
      sceneId: scene.id,
      seed: `eng003-cp006-rule-contract:${difficulty}:${ruleId}`,
    });
...
console.log("ENG-003-CP006 V1 adjective/adverb/comparison filler tests passed.");
