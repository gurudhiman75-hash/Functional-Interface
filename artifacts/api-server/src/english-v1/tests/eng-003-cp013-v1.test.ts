...
import { generateEng003Cp013QuestionV1, materializeEng003Cp013AnswerV1 } from "../chapters/grammar-fillers/ENG-003/CP013/eng-003-cp013-v1";
...
for(const difficulty of ["easy","medium","hard"] as const){
  ...
  const allowedRules=new Set(rulesForDifficultyCp013V1(difficulty));
  ...
  for(let index=0;index<2000;index+=1){
    const input={difficulty,seed:`eng003-cp013-test:${difficulty}:${index}`} as const;
    const first=generateEng003Cp013QuestionV1(input);
    const replay=generateEng003Cp013QuestionV1(input);
    ...
    assert(first.explanation.startsWith("The blank needs"),`${first.questionId} explanation does not begin with answer`);
    ...
    assert(!overFormal.test(first.explanation),`${first.questionId} explanation became too formal`);
    assert(first.metadata.chapterId==="ENG-003"&&first.metadata.cpId==="ENG-003-CP013",`${first.questionId} has wrong ids`);
    ...
    for(const option of first.options){
      ...
      if(first.metadata.ruleId==="GR-USG-003"){
        assert(!/different\s+(?:to|than)\b/i.test(rendered),`${first.questionId} used a dialect-valid different variant as an exam distractor: ${rendered}`);
      }
    }

    const answer=first.options[first.correctOptionIndex]!;
    assert(materializeEng003Cp013AnswerV1(first.segments,first.blankIndex,answer)===first.correctedSentence,`${first.questionId} answer does not reconstruct approved sentence`);
  ...
  for(let option=0;option<4;option+=1){
    const share=answerCounts[option]!/2000;
  ...
  for(const ruleId of allowedRules){
    ...
    const q=generateEng003Cp013QuestionV1({difficulty,ruleId,sceneId:scene.id,seed:`eng003-cp013-rule-contract:${difficulty}:${ruleId}`});
...
console.log("ENG-003-CP013 V1 common usage filler tests passed.");
