...
const all=[...ENG007_CP001_ENTRIES_V1,...ENG007_CP002_ENTRIES_V1,...ENG007_CP003_ENTRIES_V1,...ENG007_CP004_ENTRIES_V1];
assert.equal(all.length,1020);
const correct=all.map(x=>x.correct.toLowerCase()),wrong=all.map(x=>x.misspelling.toLowerCase());
assert.equal(new Set(correct).size,1020,"duplicate canonical spellings");
assert.equal(new Set(wrong).size,1020,"duplicate misspellings");
...
console.log("ENG-007 CP001-CP004 breadth audit passed.",{canonical:correctSet.size,misspellings:wrongSet.size});
