...
import { generateEng003Cp009QuestionV1, materializeEng003Cp009AnswerV1 } from "../chapters/grammar-fillers/ENG-003/CP009/eng-003-cp009-v1";

function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(message);
}
const stable = (value: unknown) => JSON.stringify(value);
const malformed = /\b(?:to\s+to|having\s+having|used\s+to\s+to|without\s+to\s+to)\b/i;

for (const difficulty of ["easy", "medium", "hard"] as const) {
  const pool = cp009ScenePoolV1(difficulty);
  const allowedRules = new Set(rulesForDifficultyCp009V1(difficulty));
  ...
  for (let index = 0; index < 2_000; index += 1) {
    const input = { difficulty, seed: `eng003-cp009-test:${difficulty}:${index}` } as const;
    const first = generateEng003Cp009QuestionV1(input);
    const replay = generateEng003Cp009QuestionV1(input);
    ...
    assert(first.metadata.chapterId === "ENG-003" && first.metadata.cpId === "ENG-003-CP009", `${first.questionId} has wrong ids`);
    ...
    for (const option of first.options) {
      const rendered = materializeEng003Cp009AnswerV1(first.segments, first.blankIndex, option);
      assert(!malformed.test(rendered), `${first.questionId} rendered malformed option: ${rendered}`);
    }

    const answer = first.options[first.correctOptionIndex]!;
    assert(
      materializeEng003Cp009AnswerV1(first.segments, first.blankIndex, answer) === first.correctedSentence,
  ...
  for (const ruleId of allowedRules) {
    ...
    const question = generateEng003Cp009QuestionV1({
      ...
      ruleId,
      sceneId: scene.id,
      seed: `eng003-cp009-rule-contract:${difficulty}:${ruleId}`,
    });
...
console.log("ENG-003-CP009 V1 non-finite grammar filler tests passed.");
