...
const all=[...ENG007_CP001_ENTRIES_V1,...ENG007_CP002_ENTRIES_V1,...ENG007_CP003_ENTRIES_V1,...ENG007_CP004_ENTRIES_V1,...ENG007_CP005_ENTRIES_V1,...ENG007_CP006_ENTRIES_V1,...ENG007_CP007_ENTRIES_V1];
assert.equal(all.length,1445);
const correct=all.map(x=>x.correct.toLowerCase()),wrong=all.map(x=>x.misspelling.toLowerCase());
assert.equal(new Set(correct).size,1445,"duplicate canonical spellings");
assert.equal(new Set(wrong).size,1445,"duplicate misspellings");
...
console.log("ENG-007 CP001-CP007 breadth audit passed.",{canonical:correctSet.size,misspellings:wrongSet.size});
