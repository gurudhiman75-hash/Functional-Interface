...
assert.equal(ENG008_CP001_PASSAGES_V1.length,22);
assert.equal(new Set(ENG008_CP001_PASSAGES_V1.map(x=>x.id)).size,22);
assert.equal(ENG008_CP001_QUESTION_AUTHORITIES_V1.length,132);
assert.equal(new Set(ENG008_CP001_QUESTION_AUTHORITIES_V1.map(x=>x.question.id)).size,132);
...
assert.deepEqual(Object.fromEntries(genres),{narrative:11,report:11});
for(const familyId of ENG008_CP001_FAMILY_IDS_V1)assert.equal(families.get(familyId),22,`${familyId} should have 22 authorities`);
...
console.log("ENG-008 CP001 SSC foundation RC audit passed.",{passages:22,authorities:132,soak:6000});
