...
assert.equal(ENG008_CP001_PASSAGES_V1.length,10);
assert.equal(new Set(ENG008_CP001_PASSAGES_V1.map(x=>x.id)).size,10);
assert.equal(ENG008_CP001_QUESTION_AUTHORITIES_V1.length,60);
assert.equal(new Set(ENG008_CP001_QUESTION_AUTHORITIES_V1.map(x=>x.question.id)).size,60);
...
assert.deepEqual(Object.fromEntries(genres),{narrative:5,report:5});
for(const familyId of ENG008_CP001_FAMILY_IDS_V1)assert.equal(families.get(familyId),10,`${familyId} should have 10 authorities`);
...
console.log("ENG-008 CP001 SSC foundation RC audit passed.",{passages:10,authorities:60,soak:6000});
