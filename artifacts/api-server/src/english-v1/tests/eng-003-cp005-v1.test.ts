...
import { generateEng003Cp005QuestionV1, materializeEng003Cp005AnswerV1 } from "../chapters/grammar-fillers/ENG-003/CP005/eng-003-cp005-v1";
...
for (const difficulty of ["easy", "medium", "hard"] as const) {
  const pool = cp005ScenePoolV1(difficulty);
  const allowedRules = new Set(rulesForDifficultyCp005V1(difficulty));
  ...
  for (let index = 0; index < 2_000; index += 1) {
    const input = { difficulty, seed: `eng003-cp005-test:${difficulty}:${index}` } as const;
    const first = generateEng003Cp005QuestionV1(input);
    const replay = generateEng003Cp005QuestionV1(input);
    ...
    assert(first.options.every((option) => /^[A-Za-z]+$/.test(option)), `${first.questionId} has a non-preposition option`);
    ...
    assert(first.metadata.chapterId === "ENG-003" && first.metadata.cpId === "ENG-003-CP005", `${first.questionId} has wrong ids`);
    ...
    assert(
      materializeEng003Cp005AnswerV1(first.segments, first.blankIndex, answer) === first.correctedSentence,
  ...
  for (const ruleId of allowedRules) {
    ...
    const question = generateEng003Cp005QuestionV1({
      difficulty,
      ruleId,
      sceneId: scene.id,
      seed: `eng003-cp005-rule-contract:${difficulty}:${ruleId}`,
    });
...
console.log("ENG-003-CP005 V1 preposition grammar filler tests passed.");
