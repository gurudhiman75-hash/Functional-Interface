...
} from "../chapters/error-spotting/ENG-001/CP009/eng-001-cp009-v1";
import { assertValidEng001Cp009QuestionV1 } from "../chapters/error-spotting/ENG-001/CP009/eng-001-cp009-v1-validator";
import type {
  Eng001QlId,
  EnglishDifficulty,
...
for (const scene of CP009_SCENES_V1) {
  ...
  assert.equal(question.metadata.candidateId, `GIP-V1:${scene.id}`);
...
for (const difficulty of difficulties) {
  ...
  for (let index = 0; index < 3000; index += 1) {
    const seed = `cp009:${difficulty}:stress:${index}`;
    const question = generateEng001Cp009QuestionV1({ seed, difficulty });
    assert.deepEqual(question, generateEng001Cp009QuestionV1({ seed, difficulty }));
    assertValidEng001Cp009QuestionV1(question);
    assert.equal(question.metadata.cpId, "ENG-001-CP009");
  ...
  for (const ruleId of rulesForDifficultyCp009V1(difficulty)) {
    for (const qlId of qls) {
      const question = generateEng001Cp009QuestionV1({
        ...
        difficulty,
        qlId,
        ruleId: ruleId as GerundInfinitiveParticipleRuleId,
      });
      assertValidEng001Cp009QuestionV1(question);
