...
import { generateEng003Cp010QuestionV1, materializeEng003Cp010AnswerV1 } from "../chapters/grammar-fillers/ENG-003/CP010/eng-003-cp010-v1";

function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(message);
}
const stable = (value: unknown) => JSON.stringify(value);
const mechanical = /^(?:Apparently|Still|Evidently|Also),\s|\b(?:still|also)\s+with\b|\b(?:still|also)\s+(?:carrying|containing|wearing|holding|showing)\b|^Even\s+(?:The|Several|A|An|Some)\b|\bbegan being (?:had|still)\b|\b(?:which|that) was (?:carrying|containing|wearing|holding|showing)\b|\bthe \w+ly (?:procedure|documents|differences|sequence)\b/i;
const duplicatedModifier = /\b(only|almost|nearly|even|usually|rarely|normally|never|clearly|carefully|neatly)\s+\1\b/i;

for (const difficulty of ["easy", "medium", "hard"] as const) {
  const pool = cp010ScenePoolV1(difficulty);
  const allowedRules = new Set(rulesForDifficultyCp010V1(difficulty));
  ...
  for (let index = 0; index < 2_000; index += 1) {
    const input = { difficulty, seed: `eng003-cp010-test:${difficulty}:${index}` } as const;
    const first = generateEng003Cp010QuestionV1(input);
    const replay = generateEng003Cp010QuestionV1(input);
    ...
    assert(first.metadata.chapterId === "ENG-003" && first.metadata.cpId === "ENG-003-CP010", `${first.questionId} has wrong ids`);
    ...
    for (const option of first.options) {
      const rendered = materializeEng003Cp010AnswerV1(first.segments, first.blankIndex, option);
      assert(!mechanical.test(rendered), `${first.questionId} rendered a mechanical modifier option: ${rendered}`);
      assert(!duplicatedModifier.test(rendered), `${first.questionId} duplicated a modifier: ${rendered}`);
    }

    const answer = first.options[first.correctOptionIndex]!;
    assert(
      materializeEng003Cp010AnswerV1(first.segments, first.blankIndex, answer) === first.correctedSentence,
  ...
  for (const ruleId of allowedRules) {
    ...
    const question = generateEng003Cp010QuestionV1({
      difficulty,
      ruleId,
      sceneId: scene.id,
      seed: `eng003-cp010-rule-contract:${difficulty}:${ruleId}`,
    });
...
console.log("ENG-003-CP010 V1 modifier and placement filler tests passed.");
