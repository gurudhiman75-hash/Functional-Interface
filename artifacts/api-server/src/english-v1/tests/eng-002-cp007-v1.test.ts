...
import { buildEng002Cp007ReviewV1 } from "../chapters/sentence-improvement/ENG-002/CP007/eng-002-cp007-review-v1-export";
...
for (const difficulty of ["easy", "medium", "hard"] as const) {
  assert.equal(cp007ScenePoolV1(difficulty).length, 20);
  const seenDomains = new Set<string>(), seenRules = new Set<string>(); const answerCounts = [0, 0, 0, 0];
  for (let index = 0; index < 2000; index += 1) {
    const seed = `eng002-cp007-stress:${difficulty}:${index}`;
    const first = verify(seed, difficulty); assert.deepEqual(generateEng002Cp007QuestionV1({ seed, difficulty }), first);
  ...
  assert.deepEqual([...seenRules].sort(), [...rulesForDifficultyCp007V1(difficulty)].sort());
...
const review = buildEng002Cp007ReviewV1();
