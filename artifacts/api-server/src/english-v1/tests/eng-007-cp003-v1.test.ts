...
import{generateEng007CP003QuestionV1}from"../chapters/spelling-correction/ENG-007/CP003/eng-007-cp003-v1";
assert.equal(ENG007_CP003_ENTRIES_V1.length,260);
assert.deepEqual(["easy","medium","hard"].map(d=>ENG007_CP003_ENTRIES_V1.filter(x=>x.difficulty===d).length),[70,110,80]);
assert.equal(new Set(ENG007_CP003_ENTRIES_V1.map(x=>x.correct.toLowerCase())).size,260);
assert.equal(new Set(ENG007_CP003_ENTRIES_V1.map(x=>x.misspelling.toLowerCase())).size,260);
const canonicalSet=new Set(ENG007_CP003_ENTRIES_V1.map(x=>x.correct.toLowerCase()));
const misspellingSet=new Set(ENG007_CP003_ENTRIES_V1.map(x=>x.misspelling.toLowerCase()));
for(const e of ENG007_CP003_ENTRIES_V1){
 for(const mode of["correct-spelling","misspelt-word"]as const){
  const q=generateEng007CP003QuestionV1({seed:`entry:${e.id}:${mode}`,difficulty:e.difficulty,entryId:e.id,mode});
...
for(let i=0;i<18000;i++){
 ...
 const q=generateEng007CP003QuestionV1({seed:`eng007-cp003-soak:${i}`,difficulty,mode});
...
console.log("ENG-007 CP003 validation passed.",{entries:260,positions});
