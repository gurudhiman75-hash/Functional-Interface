...
assert.equal(all.length,1392);
const correct=all.map(x=>x.correct.toLowerCase()),wrong=all.map(x=>x.misspelling.toLowerCase());
assert.equal(new Set(correct).size,1392,"duplicate canonical spellings");
assert.equal(new Set(wrong).size,1392,"duplicate misspellings");
