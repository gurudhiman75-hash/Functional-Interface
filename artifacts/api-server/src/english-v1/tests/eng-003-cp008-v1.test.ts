...
import { generateEng003Cp008QuestionV1, materializeEng003Cp008AnswerV1 } from "../chapters/grammar-fillers/ENG-003/CP008/eng-003-cp008-v1";

function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(message);
}
const stable = (value: unknown) => JSON.stringify(value);
const mechanical = /\b(?:many little|many few|many amount|many number|many all|several all|one a pair|one two pair|one both pair|informations of information|one material|two material)\b/i;

for (const difficulty of ["easy", "medium", "hard"] as const) {
  const pool = cp008ScenePoolV1(difficulty);
  const allowedRules = new Set(rulesForDifficultyCp008V1(difficulty));
  ...
  for (let index = 0; index < 2_000; index += 1) {
    const input = { difficulty, seed: `eng003-cp008-test:${difficulty}:${index}` } as const;
    const first = generateEng003Cp008QuestionV1(input);
    const replay = generateEng003Cp008QuestionV1(input);
    ...
    assert(first.metadata.chapterId === "ENG-003" && first.metadata.cpId === "ENG-003-CP008", `${first.questionId} has wrong ids`);
    ...
    assert(
      materializeEng003Cp008AnswerV1(first.segments, first.blankIndex, answer) === first.correctedSentence,
  ...
  for (const ruleId of allowedRules) {
    ...
    const question = generateEng003Cp008QuestionV1({
      difficulty,
      ruleId,
      sceneId: scene.id,
      seed: `eng003-cp008-rule-contract:${difficulty}:${ruleId}`,
    });
...
console.log("ENG-003-CP008 V1 noun and quantifier filler tests passed.");
