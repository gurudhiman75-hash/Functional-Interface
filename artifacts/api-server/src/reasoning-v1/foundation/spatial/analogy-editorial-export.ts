...
export function buildSpatialAnalogyEditorialReviewExport(
  questions: readonly SpatialAnalogyProofQuestion[],
): SpatialAnalogyEditorialReviewExport {
  return {
    schemaVersion: "1.0",
    familyCode: "SPA-001",
    chapterCode: "FAN-001",
    ...
    rows: questions.map((question, index) => ({
      ...
      aSvg: renderSpatialSceneToSvg(question.aScene, {
        ariaLabel: `Question ${index + 1} figure A`,
      }),
      bSvg: renderSpatialSceneToSvg(question.bScene, {
        ariaLabel: `Question ${index + 1} figure B`,
      }),
      cSvg: renderSpatialSceneToSvg(question.cScene, {
        ariaLabel: `Question ${index + 1} figure C`,
      }),
...
export function buildSpatialAnalogyEditorialReviewHtml(
  review: SpatialAnalogyEditorialReviewExport,
): string {
      ...
      return `
        <article class="question">
          <header>
            ...
            <span>${escapeHtml(row.ruleId)}</span>
          </header>
          <p class="instruction">Choose the figure that completes A : B :: C : ?</p>
          <div class="analogy">
            <div class="figure"><b>A</b>${row.aSvg}</div>
            <div class="relation">:</div>
            <div class="figure"><b>B</b>${row.bSvg}</div>
            <div class="relation">::</div>
            <div class="figure"><b>C</b>${row.cSvg}</div>
            <div class="relation">:</div>
...
<head>
  ...
  <style>
    ...
    .analogy { display: grid; grid-template-columns: 1fr auto 1fr auto 1fr auto .5fr; gap: 10px; align-items: center; }
    .figure { min-width: 0; text-align: center; }
    .figure svg { width: 150px; max-width: 100%; height: auto; }
    .figure b { display: block; margin-bottom: 4px; }
    .relation, .unknown { font-size: 28px; font-weight: 700; text-align: center; }
    .options { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 14px; margin-top: 18px; }
    .option { position: relative; border: 1px solid #d4d4d8; border-radius: 10px; padding: 10px; text-align: center; }
    .option svg { width: 160px; max-width: 100%; height: auto; }
    ...
    @media (max-width: 760px) {
      body { padding: 12px; }
      .analogy { grid-template-columns: 1fr auto 1fr; }
      .analogy > :nth-child(4), .analogy > :nth-child(6) { display: none; }
      .analogy > :nth-child(5) { grid-column: 1; }
      .analogy > :nth-child(7) { grid-column: 3; }
...
<body>
  <main>
    <section class="summary">
      <h1>FAN-001 Figure Analogy Proof Review</h1>
      <p>${review.questionCount} deterministic questions. Answers are hidden inside each question.</p>
    </section>
    ${questions}
  </main>
</body>
