...
export const PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2 = Object.freeze({
  authorityId: "PFC-001-PERMANENT-ENGLISH-RUNTIME-V2" as const,
  ...
  remediationAuthority: PFC_001_VISUAL_TAXONOMY_REMEDIATION_AUTHORITY_V4.authorityId,
  priorEnglishFreeze: "PFC-001-ENGLISH-FREEZE-V1" as const,
  priorFreezeStatus: "SUPERSEDED_FOR_INTEGRATION_PENDING_V2_LEARNER_REVIEW" as const,
  status: "REMEDIATED_RUNTIME_IMPLEMENTED_REVIEW_REQUIRED" as const,
  questionStudioRegistered: false,
  automaticPublication: false,
} as const);

export interface PfcPermanentEnglishQuestionV2 extends PfcDiscoveryQuestionV4 {
...
const PERMANENT_SELECTIONS_V2: Readonly<Record<PfcPermanentQlIdV3, readonly number[]>> = Object.freeze({
  "SPA-QL-035": [
...
export const PFC_001_PERMANENT_V2_REVIEW_OFFSETS: Readonly<Record<PfcPermanentQlIdV3, readonly number[]>> = Object.freeze({
...
function permanentQuestionV2(
  qlId: PfcPermanentQlIdV3,
  qlIndex: number,
  discoveryIndex: number,
): PfcPermanentEnglishQuestionV2 {
  const source = generatePfcDiscoveryQuestionV4(discoveryIndex);
...
export function generatePfcPermanentEnglishQlV2(
  qlId: PfcPermanentQlIdV3,
): PfcPermanentEnglishQuestionV2[] {
  return PERMANENT_SELECTIONS_V2[qlId].map((discoveryIndex, index) =>
    permanentQuestionV2(qlId, index, discoveryIndex),
  );
}

export function generatePfcPermanentEnglishCorpusV2(): PfcPermanentEnglishQuestionV2[] {
  return (["SPA-QL-035", "SPA-QL-036", "SPA-QL-037", "SPA-QL-038"] as const)
    .flatMap((qlId) => generatePfcPermanentEnglishQlV2(qlId));
}

export function generatePfcPermanentEnglishReviewQuestionsV2(): PfcPermanentEnglishQuestionV2[] {
  return (["SPA-QL-035", "SPA-QL-036", "SPA-QL-037", "SPA-QL-038"] as const)
    .flatMap((qlId) => {
      const questions = generatePfcPermanentEnglishQlV2(qlId);
      return PFC_001_PERMANENT_V2_REVIEW_OFFSETS[qlId].map((offset) => questions[offset]);
    });
}

export function renderPfcPermanentEnglishReviewHtmlV2(
  questions: readonly PfcPermanentEnglishQuestionV2[],
): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PFC-001 Permanent English Review V2</title></head><body style="font-family:Arial,sans-serif;background:#fff;color:#111;max-width:1120px;margin:0 auto;padding:20px"><h1 style="font-size:24px">PFC-001 Permanent English Review V2</h1><p>Remediated paper-folding review. White exam surface, physically ordered fold panels, real cut/notch shapes, off-centre folds, both diagonal/corner directions, mixed cut families and fold-line cuts are intentionally represented. This runtime is review-only until a new learner approval freezes V2.</p>${cards}</body></html>`;
}
