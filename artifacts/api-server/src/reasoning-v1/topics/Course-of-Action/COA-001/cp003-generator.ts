...
import { coaEnglishAuthoritiesForQl, type CoaCp003OwnedQlId } from "./english-authorities.ts";
import type { CoaActionAuthority, CoaAnswerClass, CoaScenarioAuthority } from "./types.ts";

export const COA_CP003_CHECKPOINT_ID = "COA-CP-003" as const;
export const COA_CP003_AUTHORITY = "COA_CP003_VERIFICATION_ADMINISTRATIVE_ENGLISH_V2" as const;

export const COA_CP003_FOUR_WAY_ANSWER_OPTIONS = Object.freeze([
...
const INSTRUCTIONS = Object.freeze([
  ...
  "Which of the courses of action should be taken?",
...
function explanationFor(actions: readonly [CoaActionAuthority, CoaActionAuthority], answerClass: CoaAnswerClass): string {
  ...
  const firstVerdict = first.expectedVerdict === "FOLLOWS" ? "Course I follows" : "Course I does not follow";
  const secondVerdict = second.expectedVerdict === "FOLLOWS" ? "Course II follows" : "Course II does not follow";
  const answerText = COA_CP003_FOUR_WAY_ANSWER_OPTIONS[answerIndex(answerClass)];
  return `${firstVerdict}: ${first.explanation} ${secondVerdict}: ${second.explanation} Therefore, ${answerText.toLowerCase()}.`;
}

export type GenerateCoaCp003Input = Readonly<{
  qlId: CoaCp003OwnedQlId;
  seed: number;
}>;

export function generateCoaCp003Question(input: GenerateCoaCp003Input) {
  ...
  return Object.freeze({
    ...
    checkpointId: COA_CP003_CHECKPOINT_ID,
    authority: COA_CP003_AUTHORITY,
    qlId: input.qlId,
    ...
    answerOptions: COA_CP003_FOUR_WAY_ANSWER_OPTIONS,
    answerClass,
