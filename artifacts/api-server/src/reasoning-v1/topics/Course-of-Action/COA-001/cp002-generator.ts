...
import { coaEnglishAuthoritiesForQl, type CoaCp002OwnedQlId } from "./english-authorities.ts";
import type { CoaActionAuthority, CoaAnswerClass, CoaScenarioAuthority } from "./types.ts";

export const COA_CP002_CHECKPOINT_ID = "COA-CP-002" as const;
export const COA_CP002_AUTHORITY = "COA_CP002_DIRECT_PREVENTIVE_ENGLISH_V1" as const;

export const COA_FOUR_WAY_ANSWER_OPTIONS = Object.freeze([
...
function explanationFor(actions: readonly [CoaActionAuthority, CoaActionAuthority], answerClass: CoaAnswerClass): string {
  ...
  const answerText = COA_FOUR_WAY_ANSWER_OPTIONS[answerIndex(answerClass)];
  return `${firstVerdict}: ${first.explanation} ${secondVerdict}: ${second.explanation} Therefore, ${answerText.toLowerCase()}.`;
}

export type GenerateCoaCp002Input = Readonly<{
  qlId: CoaCp002OwnedQlId;
  seed: number;
}>;

export function generateCoaCp002Question(input: GenerateCoaCp002Input) {
  ...
  return Object.freeze({
    ...
    checkpointId: COA_CP002_CHECKPOINT_ID,
    authority: COA_CP002_AUTHORITY,
    qlId: input.qlId,
    ...
    answerOptions: COA_FOUR_WAY_ANSWER_OPTIONS,
    answerClass,
