...
import { coaEnglishAuthoritiesForQl, type CoaCp004OwnedQlId } from "./english-authorities.ts";
import type { CoaActionAuthority, CoaAnswerClass, CoaScenarioAuthority } from "./types.ts";

export const COA_CP004_CHECKPOINT_ID = "COA-CP-004" as const;
export const COA_CP004_AUTHORITY = "COA_CP004_CONSTRAINT_PROPORTIONALITY_ENGLISH_V2" as const;

export const COA_CP004_FOUR_WAY_ANSWER_OPTIONS = Object.freeze([
...
function explanationFor(actions: readonly [CoaActionAuthority, CoaActionAuthority], answerClass: CoaAnswerClass): string {
  ...
  const answerText = COA_CP004_FOUR_WAY_ANSWER_OPTIONS[answerIndex(answerClass)];
  return `${firstVerdict}: ${first.explanation} ${secondVerdict}: ${second.explanation} Therefore, ${answerText.toLowerCase()}.`;
}

export type GenerateCoaCp004Input = Readonly<{
  qlId: CoaCp004OwnedQlId;
  seed: number;
}>;

export function generateCoaCp004Question(input: GenerateCoaCp004Input) {
  ...
  return Object.freeze({
    ...
    checkpointId: COA_CP004_CHECKPOINT_ID,
    authority: COA_CP004_AUTHORITY,
    qlId: input.qlId,
    ...
    answerOptions: COA_CP004_FOUR_WAY_ANSWER_OPTIONS,
    answerClass,
