...
import { coaEnglishAuthoritiesForQl, type CoaCp007OwnedQlId } from "./english-authorities.ts";
import type { CoaActionAuthority, CoaAnswerClass, CoaScenarioAuthority } from "./types.ts";

export const COA_CP007_CHECKPOINT_ID = "COA-CP-007" as const;
export const COA_CP007_AUTHORITY = "COA_CP007_INTEGRATED_REASONING_ENGLISH_V2" as const;

export const COA_CP007_ANSWER_OPTIONS = Object.freeze([
...
const INSTRUCTIONS = Object.freeze([
  ...
  "Which course or courses of action best fit the stated situation?",
...
function explanationFor(actions: readonly [CoaActionAuthority, CoaActionAuthority], answerClass: CoaAnswerClass): string {
  ...
  return `${firstLead}: ${first.explanation} ${secondLead}: ${second.explanation} Therefore, ${COA_CP007_ANSWER_OPTIONS[answerIndex(answerClass)].toLowerCase()}.`;
}

export function generateCoaCp007Question(input: { readonly qlId: CoaCp007OwnedQlId; readonly seed: number }) {
  ...
  return Object.freeze({
    ...
    checkpointId: COA_CP007_CHECKPOINT_ID,
    authority: COA_CP007_AUTHORITY,
    qlId: input.qlId,
    ...
    answerOptions: COA_CP007_ANSWER_OPTIONS,
    answerClass,
