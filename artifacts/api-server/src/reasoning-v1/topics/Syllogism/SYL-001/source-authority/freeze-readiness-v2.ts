...
import { buildSylProfilePlanV2, SYL_PROFILE_PLAN_V2 } from "../runtime/profile-plan-v2";
...
export type SylFreezeRequirementStatusV2 = "MET" | "PARTIAL" | "BLOCKED";

export interface SylFreezeRequirementV2 {
  requirementId: string;
  status: SylFreezeRequirementStatusV2;
  evidence: string;
  unblockAction: string | null;
}

const sscPlan = buildSylProfilePlanV2("SSC", 0, 100);
const bankingPlan = buildSylProfilePlanV2("BANKING", 0, 100);
const punjabPolicePlan = buildSylProfilePlanV2("PUNJAB_POLICE", 0, 100);
const crossExamPlan = buildSylProfilePlanV2("CROSS_EXAM_PRACTICE", 0, 100);

export const SYL_FREEZE_REQUIREMENTS_V2: readonly SylFreezeRequirementV2[] = Object.freeze([
  {
    requirementId: "LEARNER_CONTENT_APPROVED_BASELINE",
    status: "MET",
    evidence: "The approved baseline learner content remains preserved; the Banking negative-modal V4 candidate is additive and still human-gated.",
    unblockAction: null,
  },
  {
  ...
  },
  {
    ...
    status: "MET",
    evidence: "Profile planner V2 preserves deterministic 100-slot expansion while distinguishing inactive candidates from unresolved remodels.",
    unblockAction: null,
  },
  {
  ...
  },
  {
    ...
    status: "PARTIAL",
    evidence: `${bankingPlan.readinessCounts.ACTIVE_CANONICAL}% active canonical and ${bankingPlan.readinessCounts.CANDIDATE_INACTIVE}% candidate-inactive. The prior remodel blocker is closed by ${SYL_PROFILE_PLAN_V2.candidateAuthority}, but no permanent QL is registered and human/source-profile review is pending.`,
  ...
  },
  {
    ...
    status: "BLOCKED",
    evidence: `Planner V2 status is ${SYL_PROFILE_PLAN_V2.status}; connectedToGenerator=${SYL_PROFILE_PLAN_V2.connectedToGenerator}.`,
...
const counts = SYL_FREEZE_REQUIREMENTS_V2.reduce<Record<SylFreezeRequirementStatusV2, number>>(
  (result, requirement) => {
...
export const SYL_FREEZE_READINESS_V2 = Object.freeze({
  authorityId: "SYL_001_FREEZE_READINESS_V2",
  status: "NOT_READY_FOR_PROFILE_OR_QL_FREEZE",
  supersedes: "SYL_001_FREEZE_READINESS_V1",
  requirementCount: SYL_FREEZE_REQUIREMENTS_V2.length,
  counts,
  ...
  nextCriticalPath: [
    "Complete human review of Banking Editorial V4 wording, localization and combined diagrams.",
