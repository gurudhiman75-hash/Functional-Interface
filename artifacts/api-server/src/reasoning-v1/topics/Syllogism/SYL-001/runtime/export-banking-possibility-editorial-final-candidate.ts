...
const outDir = resolve(process.cwd(), "dist/reasoning-v1/syl-001-bank-possibility-editorial-final-candidate");
mkdirSync(outDir, { recursive: true });
const records = seeds.flatMap((seed) => locales.map((locale) => generateBankingPossibilityEditorialFinalCandidate(seed, locale)));

function esc(value: string): string {
  return value.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#39;");
}
function card(q: ReturnType<typeof generateBankingPossibilityEditorialFinalCandidate>): string {
