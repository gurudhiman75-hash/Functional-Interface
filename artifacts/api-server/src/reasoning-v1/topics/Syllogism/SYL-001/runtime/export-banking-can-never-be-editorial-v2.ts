...
const outDir = resolve(process.cwd(), "dist/reasoning-v1/syl-001-bank-can-never-be-editorial-v2");
...
const records = seeds.flatMap((seed) =>
  locales.map((locale) => generateBankingCanNeverEditorialV2(seed, locale)));
...
for (const question of records) {
  ...
  if (!negative || !negative.surfaceKind || !negative.disposition) {
    throw new Error(`${question.seed}/${question.locale}: missing negative-modal conclusion.`);
  }
...
const summary = {
  status: "PROTOTYPE_CAN_NEVER_BE_EDITORIAL_V2_HUMAN_REVIEW_REQUIRED",
  ...
  schemaVersion: "banking-can-never-be-editorial-v2",
...
function conclusionMeta(question: (typeof records)[number], index: number): string {
  ...
  return conclusion.mode === "CAN_NEVER_BE"
    ? `${conclusion.surfaceKind} · ${conclusion.disposition} · ${conclusion.canonicalConclusion.form}`
    : `DEFINITE · ${conclusion.classification} · ${conclusion.canonicalConclusion.form}`;
}
...
<header><strong>Seed ${question.seed} · ${esc(question.locale)}</strong><span>${esc(question.scenarioId)}</span></header>
<div class="chips"><span>${esc(question.semanticAnswer)}</span><span>${esc(negative?.surfaceKind ?? "")}</span><span>${esc(negative?.disposition ?? "")}</span><span>Answer ${String.fromCharCode(65 + question.correctIndex)}</span></div>
<h3>Statements</h3><ol>${question.statements.map((entry) => `<li>${esc(entry)}</li>`).join("")}</ol>
<h3>Conclusions</h3><ol class="roman">${question.conclusions.map((entry, index) => `<li><small>${esc(conclusionMeta(question, index))}</small><br>${esc(entry.text)}</li>`).join("")}</ol>
...
<h3>Student explanation</h3>${question.explanation.map((entry) => `<p>${esc(entry)}</p>`).join("")}
</article>`;
}

const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SYL-001 Banking Can Never Be — Editorial V2 Review</title><style>
body{font-family:system-ui,-apple-system,"Segoe UI",sans-serif;background:#f1f5f9;color:#0f172a;margin:0;padding:18px}main{max-width:980px;margin:auto}.notice,.card{background:#fff;border:1px solid #cbd5e1;border-radius:14px;padding:16px;margin:0 0 18px}.notice{border-left:5px solid #d97706}.card header{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;border-bottom:1px solid #e2e8f0;padding-bottom:10px}.chips{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}.chips span{font-size:.78rem;background:#e2e8f0;border-radius:999px;padding:4px 8px}.correct{font-weight:800}.answer{padding:9px 11px;background:#f8fafc;border-radius:8px}small{color:#475569}.roman{list-style-type:upper-roman}p{line-height:1.55}@media(max-width:760px){body{padding:8px}.card{padding:12px}}
</style></head><body><main><h1>SYL-001 Banking “Can Never Be” — Editorial V2 Review</h1><div class="notice"><b>Human review required.</b> V2 naturalizes Hindi/Punjabi negative-modal wording and makes every negative-modal explanation term-specific. Semantics, answer keys and delivery locks are unchanged.</div>${records.map(card).join("\n")}</main></body></html>`;

const markdown = [
  "# SYL-001 Banking ‘Can Never Be’ — Editorial V2 Review",
  "",
  "> Human review required. V2 changes learner-facing negative-modal copy only; semantics and answer keys are unchanged.",
  "",
  ...records.flatMap((question) => [
    ...
    "",
    "### Conclusions",
    ...question.conclusions.map((entry, index) => `${index === 0 ? "I" : "II"}. [${conclusionMeta(question, index)}] ${entry.text}`),
    "",
...
writeFileSync(resolve(outDir, "SYL-001-BANKING-CAN-NEVER-BE-EDITORIAL-V2.html"), html, "utf8");
writeFileSync(resolve(outDir, "SYL-001-BANKING-CAN-NEVER-BE-EDITORIAL-V2.md"), markdown, "utf8");
