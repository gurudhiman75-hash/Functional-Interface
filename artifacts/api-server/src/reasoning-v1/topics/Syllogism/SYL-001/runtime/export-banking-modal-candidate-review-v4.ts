...
import {
  buildBankingModalCandidateReviewOverlayV4,
  type BankingModalCandidateReviewBindingV4,
} from "./banking-modal-candidate-review-overlay-v4";

const plannerSeed = 731;
const requestedSlots = 100;
const locales: readonly SylLocale[] = ["en-IN", "hi-IN", "pa-IN"];
const outDir = resolve(process.cwd(), "dist/reasoning-v1/syl-001-bank-modal-candidate-review-v4");
...
const records = locales.flatMap((locale) =>
  buildBankingModalCandidateReviewOverlayV4(plannerSeed, requestedSlots, locale));
...
for (const binding of records) {
  ...
  localizationBlockerHits += (text.match(/वर्ग वर्ग/gu) ?? []).length;
  localizationBlockerHits += (text.match(/ਵਰਗ ਵਰਗ/gu) ?? []).length;
  localizationBlockerHits += (text.match(/सभी “[^”]+” वर्ग (?:का|को)/gu) ?? []).length;
  localizationBlockerHits += (text.match(/ਸਾਰੇ “[^”]+” ਵਰਗ (?:ਦਾ|ਨੂੰ)/gu) ?? []).length;
  diagrams += 1;
}

if (localizationBlockerHits !== 0) throw new Error("Review V4 still contains known localization blockers.");

const summary = {
  status: "HUMAN_REVIEW_REQUIRED_BANKING_MODAL_CANDIDATE_PACK_V4",
...
function card(binding: BankingModalCandidateReviewBindingV4): string {
...
const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SYL-001 Banking Modal Candidate Human Review V4</title><style>body{font-family:system-ui,-apple-system,"Segoe UI",sans-serif;background:#f1f5f9;color:#0f172a;margin:0;padding:18px}main{max-width:1040px;margin:auto}.notice,.card{background:#fff;border:1px solid #cbd5e1;border-radius:14px;padding:16px;margin:0 0 18px}.notice{border-left:5px solid #d97706}.card header{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;border-bottom:1px solid #e2e8f0;padding-bottom:10px}.right{text-align:right}.authority,.caption,.meta{font-size:.9rem;color:#475569}.roman{list-style-type:upper-roman}.correct{font-weight:800}.svg-wrap{max-width:560px;margin:10px auto}.svg-wrap svg{width:100%;height:auto;display:block}.meta{font-family:ui-monospace,SFMono-Regular,Menlo,monospace}p,li{line-height:1.5}h1{margin-top:0}@media(max-width:700px){body{padding:8px}.card{padding:12px}.right{text-align:left}}</style></head><body><main><h1>SYL-001 Banking Modal Candidate — Human Review V4</h1><div class="notice"><b>Human review required; not an activation artifact.</b> These are the exact 20 inactive Banking planner slots in English, Hindi and Punjabi. Ordinary possibility uses Editorial V4; can-never uses Editorial V6. The 10/10 split is evaluation coverage, not an exam-frequency claim. No permanent QL is created and all delivery surfaces remain locked.</div>${records.map(card).join("\n")}</main></body></html>`;

const markdown = [
  "# SYL-001 Banking Modal Candidate — Human Review V4",
  "",
...
writeFileSync(resolve(outDir, "SYL-001-BANKING-MODAL-CANDIDATE-HUMAN-REVIEW-V4.html"), html, "utf8");
writeFileSync(resolve(outDir, "SYL-001-BANKING-MODAL-CANDIDATE-HUMAN-REVIEW-V4.md"), markdown, "utf8");
