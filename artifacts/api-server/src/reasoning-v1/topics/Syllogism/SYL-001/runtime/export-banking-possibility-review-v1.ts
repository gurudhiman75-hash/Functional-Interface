...
import { generateBankingPossibilityShellV1 } from "./banking-possibility-shell-v1";
...
const outputDir = process.env.SYL_BANK_POSSIBILITY_REVIEW_DIR
  ...
  : resolve(process.cwd(), "artifacts/api-server/dist/reasoning-v1/syl-001-bank-possibility-review-v1");

mkdirSync(outputDir, { recursive: true });

const questions = seeds.flatMap((seed) =>
  locales.map((locale) => generateBankingPossibilityShellV1(seed, locale)));
...
writeFileSync(
  resolve(outputDir, "syl-001-bank-possibility-review-v1.jsonl"),
  `${records.map((entry) => JSON.stringify(entry)).join("\n")}\n`,
  "utf8",
);
writeFileSync(
  resolve(outputDir, "syl-001-bank-possibility-summary-v1.json"),
  `${JSON.stringify(summary, null, 2)}\n`,
  "utf8",
);

const markdown: string[] = [
  "# SYL-001 Banking Possibility Shell — Prototype Review V1",
  "",
  `Authority: \`${summary.authority}\``,
  "",
  "This is a human-review prototype. It is not a registered QL and is not connected to profile generation or delivery.",
  "",
  "## Review boundary",
  "",
  ...
  `- Semantic outcomes: ${JSON.stringify(summary.statuses)}`,
  `- Scenario groups: ${JSON.stringify(summary.scenarioGroups)}`,
  `- Human editorial status: ${summary.humanEditorialStatus}`,
  `- Human localization status: ${summary.humanLocalizationStatus}`,
  `- Human exam-authenticity status: ${summary.humanExamAuthenticityStatus}`,
  "",
  "## Records",
  "",
];
...
for (const record of records) {
  ...
  markdown.push("");
  record.conclusions.forEach((conclusion) => {
    markdown.push(`${conclusion.label}. ${conclusion.text}`);
  });
  markdown.push("");
...
writeFileSync(
  resolve(outputDir, "SYL-001-BANKING-POSSIBILITY-PROTOTYPE-REVIEW-V1.md"),
  `${markdown.join("\n")}\n`,
  "utf8",
);
...
const cards = records.map((record) => {
  ...
  const explanations = record.explanation
    ...
    .join("");
  return `
    <article class="card" data-locale="${escapeHtml(record.locale)}" data-status="${escapeHtml(record.semanticAnswer)}" data-group="${escapeHtml(record.scenarioGroup)}">
      <header>
...
<title>SYL-001 Banking Possibility Prototype Review V1</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  select { padding: 8px; min-width: 180px; }
  .grid { display: grid; gap: 16px; grid-template-columns: repeat(auto-fit, minmax(min(100%, 330px), 1fr)); }
  .card h2 { margin: 0; font-size: 1.05rem; }
  .card header p, .card footer { color: #4b5563; overflow-wrap: anywhere; }
  ...
  .mode { font-size: 0.72rem; font-weight: 700; background: #e5e7eb; border-radius: 999px; padding: 2px 6px; }
...
<main>
  <section class="summary">
    <h1>SYL-001 Banking Possibility Shell — Prototype Review V1</h1>
    <p>72 localized records from 24 logical questions. Human editorial, localization and exam-authenticity review remain pending.</p>
...
<script>
  const filters = ["locale", "status", "group"];
  function apply() {
...
writeFileSync(
  resolve(outputDir, "SYL-001-BANKING-POSSIBILITY-PROTOTYPE-REVIEW-V1.html"),
  html,
  "utf8",
);

console.log(JSON.stringify({
  status: "SYL-001 Banking possibility prototype review exported",
  outputDir,
  ...summary,
}, null, 2));
