...
const outDir = resolve(process.cwd(), "dist/reasoning-v1/syl-001-bank-possibility-editorial-v8");
mkdirSync(outDir, { recursive: true });
const records = seeds.flatMap((seed) => locales.map((locale) => generateBankingPossibilityEditorialQuestionV8(seed, locale)));

function esc(value: string): string {
  return value.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#39;");
}

function card(q: ReturnType<typeof generateBankingPossibilityEditorialQuestionV8>): string {
...
writeFileSync(resolve(outDir,"SYL-001-BANKING-POSSIBILITY-EDITORIAL-V8-FULL.html"),html,"utf8");
writeFileSync(resolve(outDir,"syl-001-bank-possibility-editorial-v8-full.jsonl"),`${records.map((r)=>JSON.stringify(r)).join("\n")}\n`,"utf8");
writeFileSync(resolve(outDir,"syl-001-bank-possibility-editorial-v8-summary.json"),`${JSON.stringify(summary,null,2)}\n`,"utf8");
