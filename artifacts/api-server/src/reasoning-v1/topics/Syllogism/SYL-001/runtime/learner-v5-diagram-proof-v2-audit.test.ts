...
function parseWitnesses(svg: string): readonly Witness[] {
  return [...svg.matchAll(/<g data-witness="decisive"[^>]*>/gu)].map((match) => {
    ...
    const sourceText = attribute(markup, "data-source");
    assert.ok(sourceText === "PREMISE" || sourceText === "TARGET");
    return {
      ...
      source: sourceText as "PREMISE" | "TARGET",
    };
  });
}
...
for (const definition of SYL_QL_REGISTRY) {
  for (let seed = 0; seed < 80; seed += 1) {
    for (const locale of locales) {
      ...
      const expected = expectedCompleteExistentialWitnessesV5(question, presentation);
      assert.ok(expected, `${definition.qlId}/${seed}/${locale}: enabled despite too many witnesses`);
      ...
      assert.deepEqual(actualKeys, expectedKeys, `${definition.qlId}/${seed}/${locale}: wrong witness set`);
      ...
      if (definition.qlId === "SYL-QL-002" && seed === 0) {
        assert.equal(witnesses.length, 2, `${locale}: counterexample must retain premise and target witnesses`);
...
console.log(JSON.stringify({
  status: "PASS_SYL_001_V5_EVERY_DIAGRAM_EXISTENTIAL_COMPLETE",
  records,
  enabled,
  omitted,
  existentialPremises,
