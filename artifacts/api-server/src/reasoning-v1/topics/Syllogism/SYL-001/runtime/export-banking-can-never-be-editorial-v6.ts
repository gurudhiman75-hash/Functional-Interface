...
const outDir = resolve(process.cwd(), "dist/reasoning-v1/syl-001-bank-can-never-be-editorial-v6");
...
const records = seeds.flatMap((seed) => locales.map((locale) =>
  generateBankingCanNeverEditorialV6(seed, locale)));
...
let awkwardWholeClassPhrases = 0;

for (const question of records) {
  if (!question.diagram.enabled || !question.diagram.svg || question.diagram.diagramCount !== 1) {
    throw new Error(`${question.seed}/${question.locale}: V6 review pack requires one enabled diagram.`);
  }
  ...
  const text = question.explanation.join("\n");
  awkwardWholeClassPhrases += (text.match(/सभी “[^”]+” वर्ग (?:का|को)/gu) ?? []).length;
  awkwardWholeClassPhrases += (text.match(/ਸਾਰੇ “[^”]+” ਵਰਗ (?:ਦਾ|ਨੂੰ)/gu) ?? []).length;
}

if (awkwardWholeClassPhrases !== 0) throw new Error("V6 review pack still contains awkward whole-class phrases.");

const summary = {
  status: "HUMAN_REVIEW_REQUIRED_EDITORIAL_V6",
  authority: "SYL_001_BANKING_CAN_NEVER_BE_EDITORIAL_V6",
...
const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SYL-001 Banking Can-Never-Be Editorial V6</title><style>body{font-family:system-ui,-apple-system,"Segoe UI",sans-serif;background:#f1f5f9;color:#0f172a;margin:0;padding:18px}main{max-width:980px;margin:auto}.notice,.card{background:#fff;border:1px solid #cbd5e1;border-radius:14px;padding:16px;margin:0 0 18px}.notice{border-left:5px solid #d97706}.card header{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;border-bottom:1px solid #e2e8f0;padding-bottom:10px}.roman{list-style-type:upper-roman}.correct{font-weight:800}.svg-wrap{max-width:560px;margin:10px auto}.svg-wrap svg{width:100%;height:auto;display:block}.caption,.meta{font-size:.9rem;color:#475569}.meta{font-family:ui-monospace,SFMono-Regular,Menlo,monospace}p,li{line-height:1.5}h1{margin-top:0}@media(max-width:700px){body{padding:8px}.card{padding:12px}}</style></head><body><main><h1>SYL-001 Banking Can-Never-Be — Editorial V6</h1><div class="notice"><b>Human review required.</b> V6 polishes Hindi/Punjabi whole-class noun phrases only. V5 semantics, statements, conclusions, options, answers, diagram SVGs and delivery locks remain unchanged.</div>${records.map(card).join("\n")}</main></body></html>`;

const markdown = [
  "# SYL-001 Banking Can-Never-Be — Editorial V6",
  "",
  "> Human review required. Localization polish only; V5 semantics, questions, answers and diagrams remain unchanged.",
  "",
...
writeFileSync(resolve(outDir, "SYL-001-BANKING-CAN-NEVER-BE-EDITORIAL-V6.html"), html, "utf8");
writeFileSync(resolve(outDir, "SYL-001-BANKING-CAN-NEVER-BE-EDITORIAL-V6.md"), markdown, "utf8");
