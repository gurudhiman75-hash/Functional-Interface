import { CLOCK_CHECKPOINTS, CLOCK_DESIGN_AUTHORITY, CLOCK_TASK_CATALOG } from "./catalog";
...
export function buildClockEndToEndReview(input: {
  ...
  const seedPrefix = input.seedPrefix ?? "CLK-E2E-REVIEW";
  ...
  if (!Number.isInteger(questionsPerTaskPerLocale) || questionsPerTaskPerLocale < 1) {
    throw new Error("Review questions per task and locale must be a positive integer.");
  }
  ...
  const localeCounts = {
    ...
    "hi-IN": questions.filter((question) => question.locale === "hi-IN").length,
    "pa-IN": questions.filter((question) => question.locale === "pa-IN").length,
  } as const;
  return {
    schemaVersion: "CLK_REVIEW_V2",
    ...
    taskCount: CLOCK_TASK_CATALOG.length,
    localeCounts,
    questions,
  };
}
...
export function renderClockReviewHtml(bundle: ClockReviewBundle): string {
  const checkpointSections = CLOCK_CHECKPOINTS.map((checkpoint) => {
    ...
    return `<section class="checkpoint" id="${checkpoint.code}"><header class="checkpoint-header"><h2>${checkpoint.code} — ${escapeHtml(checkpoint.title)}</h2><p>${questions.length} review questions</p></header>${questions.map((question) => {
      ...
      return `<article class="question" lang="${question.locale}" data-task="${question.taskId}" data-fingerprint="${question.fingerprint}"><header><div><h3>${question.taskId}</h3><p>${question.locale} · ${question.difficulty} · seed ${escapeHtml(question.seed)}</p></div><span class="status">OPEN DISCOVERY</span></header><div class="stem">${safeGeneratedRichText(question.stem)}</div><ol class="options">${options}</ol><section class="answer"><h4>Answer and explanation</h4><p><strong>Correct option:</strong> ${String.fromCharCode(65 + question.correctOptionIndex)} — ${safeGeneratedRichText(question.answer.display)}</p><p><strong>Given:</strong> ${escapeHtml(question.explanation.given)}</p><p><strong>Rule:</strong> ${escapeHtml(question.explanation.rule)}</p><ol>${working}</ol><p><strong>Validity:</strong> ${escapeHtml(question.explanation.validityCheck)}</p><p><strong>Closest trap:</strong> ${escapeHtml(question.explanation.closestTrap)}</p><p><strong>Answer:</strong> ${escapeHtml(question.explanation.answer)}</p></section><details class="metadata"><summary>Solver and lifecycle evidence</summary><pre>${escapeHtml(JSON.stringify({ scenario: question.scenario, solveTrace: question.solveTrace, lifecycle: question.lifecycle, fingerprint: question.fingerprint }, null, 2))}</pre></details><fieldset><legend>Human review</legend><label><input type="checkbox"> Mathematical accuracy</label><label><input type="checkbox"> Exam-natural stem</label><label><input type="checkbox"> Options plausible and unique</label><label><input type="checkbox"> Explanation clear and question-specific</label><label><input type="checkbox"> Locale wording approved</label><label><input type="checkbox"> Final disposition approved</label><textarea rows="3" placeholder="Reviewer notes"></textarea></fieldset></article>`;
    }).join("")}</section>`;
  }).join("");

  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>CLK-001 End-to-End Open Discovery Review V2</title><style>body{font-family:system-ui,sans-serif;margin:0;background:#f3f5f7;color:#151719}main{max-width:1100px;margin:auto;padding:24px}.hero,.checkpoint-header,.question{background:white;border:1px solid #d8dde3;border-radius:12px;padding:18px;margin:0 0 18px}.question>header{display:flex;justify-content:space-between;gap:12px}.status{font-size:12px;font-weight:700}.stem{font-size:18px;line-height:1.55;margin:16px 0}.options{list-style:none;padding:0;display:grid;gap:10px}.options li{border:1px solid #d8dde3;border-radius:9px;padding:12px;display:grid;grid-template-columns:30px 1fr;gap:8px}.options li.correct{border-width:2px}.options details{grid-column:2}.answer{border-top:1px solid #d8dde3;margin-top:16px;padding-top:12px}.metadata pre{white-space:pre-wrap;overflow-wrap:anywhere}.question svg,.option-value svg{max-width:220px;display:block;margin:10px auto}fieldset{display:grid;gap:8px;margin-top:14px}textarea{width:100%}code{overflow-wrap:anywhere}</style></head><body><main><section class="hero"><h1>CLK-001 — Clocks End-to-End Review V2</h1><p><strong>Sole design authority:</strong> ${escapeHtml(bundle.designAuthority.file)}</p><p><strong>Authority SHA-256:</strong> <code>${bundle.designAuthority.sha256}</code></p><p>${bundle.questionCount} questions · ${bundle.taskCount} provisional task contracts · ${bundle.checkpointCount} checkpoints</p><p>Lifecycle remains locked: no permanent QLs, Question Studio registration, Question Bank writes, test eligibility or publication.</p></section>${checkpointSections}</main></body></html>`;
}
