...
export function generateCp009SaturationQuestion(input: Readonly<{ locale: CaeLocale; seed: number }>): GeneratedCaeQuestion {
  return withCae001SaturationWave2(() => {
    ...
    if (mode === "COMMON_CAUSE_RECONSTRUCTION") {
      ...
      const candidates = [...variant.semanticCandidateEvents, ...variant.semanticBridgeCandidateEvents, ...variant.semanticEffectCandidateEvents].filter((c) => c.text[input.locale] !== cause.text[input.locale]).slice(0, 3);
      const fallback = world.nodes.filter((n) => n.id !== cause.id && !visibleIds.includes(n.id)).map((n) => option(`ALT:${n.id}`, n.text[input.locale], false, "COMMON_CAUSE_CONFUSION"));
      const wrong = candidates.map((c) => option(`ALT:${c.id}`, c.text[input.locale], false, "COMMON_CAUSE_CONFUSION"));
      while (wrong.length < 3 && fallback.length) wrong.push(fallback.shift()!);
      if (wrong.length < 3) {
        wrong.push(option("ALT:partial-1", trim(effects[0]!.text[input.locale]), false, "COMMON_CAUSE_CONFUSION"));
        wrong.push(option("ALT:partial-2", trim(effects[1]!.text[input.locale]), false, "COMMON_CAUSE_CONFUSION"));
      }
      options = shuffled([option(cause.id, cause.text[input.locale], true), ...wrong.slice(0, 3)], selectionSeed);
      stem = `${copy.commonCause}\n\nObservation I: ${effects[0]!.text[input.locale]}\nObservation II: ${effects[1]!.text[input.locale]}`;
