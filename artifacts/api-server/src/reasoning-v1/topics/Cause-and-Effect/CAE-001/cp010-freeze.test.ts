...
assert.ok(ql6.some((q) => q.answerId === "FIRST_BRIDGE"));
assert.ok(ql6.some((q) => q.answerId === "FINAL_BRIDGE"));
assert.ok(new Set(ql6.map((q) => q.answerId)).size >= 6, "CP006 learner-operation breadth regressed");
