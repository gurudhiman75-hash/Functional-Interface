...
export type Cae001ReviewedEditorialSample = Readonly<{ seed: number; question: GeneratedCaeQuestion }>;
...
function selectForQl(qlId: (typeof CAE_PROVISIONAL_QL_IDS)[number]): readonly Cae001ReviewedEditorialSample[] {
  ...
  for (let seed = 0; seed < 5_000 && generated.length < 320; seed += 1) generated.push({ seed, question: generateReviewedCaeQuestion({ qlId, locale: "en-IN", seed }) });
  ...
  const canAdd = (entry: Cae001ReviewedEditorialSample) => !(saturationCapped && SATURATION_FAMILY_IDS.has(entry.question.scenarioFamilyId) && saturationSelected() >= CONTROLLED_REVIEW_SATURATION_MAX);
  ...
  const findUnseen = (predicate: (entry: Cae001ReviewedEditorialSample) => boolean) => generated.find((entry) => predicate(entry) && canAdd(entry) && !selectedCausalStates.has(entry.question.causalStateId));
  for (const difficulty of DIFFICULTY_ORDER) if (availableDifficulties.has(difficulty)) add(findUnseen((entry) => entry.question.difficulty === difficulty));
  for (const key of new Set(generated.map((entry) => operationKey(entry.question)))) add(findUnseen((entry) => operationKey(entry.question) === key));
  for (const familyId of new Set(generated.map((entry) => entry.question.scenarioFamilyId))) add(findUnseen((entry) => entry.question.scenarioFamilyId === familyId));
...
function mechanisms(question: GeneratedCaeQuestion): string {
  return question.candidateComparisons.length === 0 ? question.distractorMechanisms.join(", ") : question.candidateComparisons.map((candidate) => `${candidate.mechanism} (${candidate.editorialPlausibility})`).join(", ");
}
...
export function renderCae001ReviewedEditorialRealnessReview(): string {
  const lines = [
    ...
    "",
    "Deterministic English (`en-IN`) review-only samples. Ten distinct causal states are selected per CP/QL with difficulty, learner-operation and family breadth prioritised. Controlled saturation is capped at two of ten review samples so the human review pack mirrors the intended reviewed allocation. CP007/008/009 use dedicated calibrated Wave 3 adapters rather than raw graph projections. The frozen V3 regression pack remains separate.",
  ];
  for (const qlId of CAE_PROVISIONAL_QL_IDS) {
    ...
    for (const { seed, question } of samples) {
      lines.push("", `### ${question.difficulty} — seed ${seed}`, "", question.stem, "", ...question.options.map((option, index) => `${LETTERS[index]}. ${option}`), "", `**Answer:** ${LETTERS[question.correctIndex]}. ${question.options[question.correctIndex]}`, "", `**Explanation:** ${question.explanation}`, "", `**Family / variant:** ${question.scenarioFamilyId} / ${question.scenarioVariantId}`, `**Projection / operation:** ${question.projectionId} / ${operationKey(question)}`, `**causalStateId:** \`${question.causalStateId}\``, `**itemVariantId:** \`${question.itemVariantId}\``, `**Difficulty evidence:** ${difficultyEvidence(question)}`, `**Distractor mechanisms:** ${mechanisms(question)}`);
    }
  }
  return `${lines.join("\n")}\n`;
}
