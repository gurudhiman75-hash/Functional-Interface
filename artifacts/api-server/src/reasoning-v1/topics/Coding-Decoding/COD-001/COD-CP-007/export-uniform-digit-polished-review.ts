...
const outputDirectory = process.argv[2] ?? "cod-cp007-uniform-digit-polished-review-output";
...
writeFileSync(
  join(outputDirectory, "COD-CP-007-uniform-digit-polished-review.jsonl"),
  `${questions.map((question) => JSON.stringify(question)).join("\n")}\n`,
  "utf8",
);

const markdown: string[] = [
  "# COD-CP-007 Uniform Modular Digit Translation — Polished Prototype Review",
  "",
...
writeFileSync(
  join(outputDirectory, "COD-CP-007-uniform-digit-polished-review.md"),
  `${markdown.join("\n")}\n`,
  "utf8",
);

writeFileSync(
  join(outputDirectory, "COD-CP-007-uniform-digit-polished-summary.json"),
  `${JSON.stringify({
    ...
    editorialLayer: "task-specific-v1",
