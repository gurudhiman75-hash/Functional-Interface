...
function stripProtected(text: string, english: QuestionLike): string {
  const protectedStrings = collectStrings({ prompt: english.structuredPrompt, options: english.options });
...
const ids = [...Array.from({ length: 172 }, (_, index) => qlId(index + 1)), "COD-QL-199"];
...
for (const id of ids) {
  for (let seed = 1; seed <= seedsPerQl; seed += 1) {
    ...
    for (const locale of locales) {
      ...
      assert.doesNotMatch(unprotected, /\b(?:what|which|how|find|code|coded|decode|using|same|rule|correct|answer|given|table|condition|first|last|word|letter|digit|string|option|therefore|apply|example|move|forward|backward|position|value|result|source|target)\b/iu, `${id}/${locale}/${seed} leaks English instructional text`);
      assert.doesNotMatch(text, /\b(?:TODO|TBD|FIXME|undefined|null)\b|\[object Object\]/iu);
      if (locale === "pa-IN") assert.doesNotMatch(text, /(?:^|[\s।,:;!?])(?:ਪਦ|ਸਾਦ੍ਰਿਸ਼ਤਾ)(?=$|[\s।,:;!?])/u, `${id}/${locale}/${seed} uses banned technical Punjabi`);
