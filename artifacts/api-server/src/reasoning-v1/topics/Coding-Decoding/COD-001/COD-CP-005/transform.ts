...
export function rearrangementOrder(
  ...
  switch (ruleId) {
    ...
    case "OUTER_INNER_INTERLEAVING": {
      ...
      while (left <= right) {
        if (left === right) {
          output.push(left);
        } else if (context.startSide === "RIGHT") {
...
export function transformRearrangementWord(
  ruleId: CodCp005RuleId,
  context: CodCp005RuleContext,
  word: string,
): string {
...
export function inverseRearrangementWord(
  ruleId: CodCp005RuleId,
  context: CodCp005RuleContext,
  code: string,
): string {
...
export function rearrangementIsActive(
  ruleId: CodCp005RuleId,
  context: CodCp005RuleContext,
  word: string,
): boolean {
  try {
    ...
    return new Set(order).size === word.length && order.some((sourceIndex, codeIndex) => sourceIndex !== codeIndex) && transformRearrangementWord(ruleId, context, word) !== word;
  } catch {
    return false;
  }
}
...
export function sameRearrangementContext(left: CodCp005RuleContext, right: CodCp005RuleContext): boolean {
  const normalize = (context: CodCp005RuleContext) => JSON.stringify(Object.entries(context).sort(([a], [b]) => a.localeCompare(b)));
