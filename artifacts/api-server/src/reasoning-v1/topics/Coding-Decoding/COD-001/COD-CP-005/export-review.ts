...
import type { GeneratedCodCp005Question } from "./types";

const outputDir = resolve(process.argv[2] ?? "cod-cp005-review-output");
mkdirSync(outputDir, { recursive: true });

const questions: GeneratedCodCp005Question[] = [];
for (let seed = 1; seed <= 5; seed += 1) {
  for (const logic of COD_CP005_QUESTION_LOGICS) {
    questions.push(generateCodCp005Question(logic.qlId, seed));
  }
}
writeFileSync(
  resolve(outputDir, "cod-cp005-seeds1-5.jsonl"),
  `${questions.map((question) => JSON.stringify(question)).join("\n")}\n`,
);

const seedOne = questions.filter((question) => question.seed === 1);
const markdown = [
  "# COD-001 / COD-CP-005 — Seed 1 Editorial Review",
  "",
...
writeFileSync(resolve(outputDir, "cod-cp005-seed1-review.md"), markdown.join("\n"));

const summary = {
  checkpoint: "COD-CP-005",
  qls: COD_CP005_QUESTION_LOGICS.length,
  seeds: 5,
  renderedQuestions: questions.length,
  qlRange: ["COD-QL-113", "COD-QL-136"],
...
writeFileSync(resolve(outputDir, "cod-cp005-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`);
