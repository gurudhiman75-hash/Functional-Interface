...
const questions = BLR_CP002_PROTOTYPE_CONTRACTS.flatMap((contract) =>
  Array.from({ length: seedsPerPrototype }, (_, seed) =>
    generateBlrCp002ReviewQuestion(contract.prototypeId, seed),
  ),
);
...
const csvHeader = [
  "prototypeId", "seed", "scenarioId", "presentation", "difficulty", "stem",
  "optionA", "optionB", "optionC", "optionD", "correctIndex", "correctAnswer",
  "answerId", "assertionRoleDepth", "queryRoleDepth", "onlyConstraintCount",
  "pathLength", "selfIdentity", "fingerprint",
].join(",");
...
const cards = questions.map((question, index) => {
  ...
  const distractors = (question.explanation.distractorAnalysis ?? []).map(
    (entry) =>
...
const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BLR-CP-002 English review V2</title><style>
...
</style></head><body><main><h1>BLR-CP-002 English open-discovery review V2</h1><p>60 deterministic, editorially upgraded records across five non-permanent role-chain prototypes. No QL allocation or publication permission is implied.</p>${cards}</main></body></html>`;

const summary = {
  packageId: "BLR-001",
  checkpointId: "BLR-CP-002",
  status: "OPEN_DISCOVERY_EDITORIAL_V2",
  permanentQlCount: 0,
  ...
  editorialChecks: {
    hiddenSelfIdentityPreserved: true,
    naturalPhotographOpening: true,
    genderSpecificReflexive: true,
    conditionalOnlyTeaching: true,
  },
  release: {
...
await writeFile(
  path.join(outputDirectory, "blr-cp002-review-v2.jsonl"),
  `${questions.map((question) => JSON.stringify(question)).join("\n")}\n`,
  "utf8",
);
await writeFile(
  path.join(outputDirectory, "blr-cp002-review-v2.csv"),
  `${csvHeader}\n${csvRows.join("\n")}\n`,
  "utf8",
);
await writeFile(path.join(outputDirectory, "blr-cp002-review-v2.html"), html, "utf8");
await writeFile(
  path.join(outputDirectory, "blr-cp002-review-v2-summary.json"),
