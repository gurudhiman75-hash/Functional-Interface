...
function routeSiblingGroup(
  ...
  const labelX = (leftCardCentre + rightCardCentre) / 2;
  const labelY = routeY + SIBLING_LABEL_OFFSET;

  return `<g data-sibling-route="card-bottom-bracket"><path d="M ${numberText(leftCardCentre)} ${numberText(cardBottom)} V ${numberText(routeY)} H ${numberText(rightCardCentre)} V ${numberText(cardBottom)}" fill="none" stroke="#4f46e5" stroke-width="4" stroke-dasharray="8 6" stroke-linecap="round" stroke-linejoin="round" marker-start="url(#blr-sibling-arrow)" marker-end="url(#blr-sibling-arrow)"/><text x="${numberText(labelX)}" y="${numberText(labelY)}" text-anchor="middle" font-size="10" font-weight="700" fill="#4f46e5">SIBLINGS</text></g>`;
}
