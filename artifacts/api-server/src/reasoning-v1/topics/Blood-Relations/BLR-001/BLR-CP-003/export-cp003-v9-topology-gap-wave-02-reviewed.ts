...
const out = path.resolve(
  process.argv[2] ?? "blr-cp003-v9-topology-gap-wave-02-reviewed-output",
);
...
const records = generateBlrCp003V9TopologyGapWave02ReviewedCandidates();
...
const remediation = {
  pairGrammarRecords: records.filter(
    (record) => record.metadata.pairGrammarRemediated,
  ).length,
  passageExclusionRecords: records.filter(
    (record) => record.metadata.passageExclusionRemediated,
  ).length,
  passageExclusionGroups: new Set(
    records
      .filter((record) => record.metadata.passageExclusionRemediated)
      .map((record) => `${record.scenarioId}::${record.seed}`),
  ).size,
  unaryStatusVisualRecords: records.filter(
    (record) => record.metadata.unaryStatusVisualAligned,
  ).length,
  roleLabelDiagramRecords: records.filter((record) =>
    record.proceduralLogic.nodes.some((node) => node.roleLabel),
  ).length,
  uniqueStems: new Set(records.map((record) => record.stem)).size,
  uniqueShortcuts: new Set(
    records.map((record) => record.editorial.examShortcut),
  ).size,
  uniqueConclusions: new Set(
    records.map((record) => record.editorial.conclusion),
  ).size,
};

const coverageMatrix = [
  ["Negative and exclusion-heavy clues", "REVIEWED_CANDIDATE", "18 groups with at least four negative constraints"],
  ["Unknown spouse boundaries", "REVIEWED_CANDIDATE", "unknown remains distinct from explicit unmarried status"],
  ["Weak exclusion quality", "REMEDIATED", "two same-gender spouse exclusions replaced with meaningful parent/sibling exclusions"],
  ["Unary status visual evidence", "REMEDIATED", "24 records highlight only the answer node and render its status label"],
  ["Pair grammar", "REMEDIATED", "30 records corrected without changing answers or graphs"],
  ["Unresolved-status authority decision", "OPEN_AUDIT", "one provisional split candidate; no permanent QL"],
  ...
  ["Structural saturation", "OPEN", "nine topologies and twenty-nine prototypes are evidence, not a quota"],
] as const;
...
const summary = {
  ...
  status: "HUMAN_REVIEW_NEGATIVE_BOUNDARY_REVIEWED_CANDIDATE_PENDING",
  approvedBaseline: {
    v8Records: v8.length,
...
if (
  ...
  summary.wave02Reviewed.candidateRecords !== 72 ||
  summary.wave02Reviewed.passageGroups !== 18 ||
  summary.wave02Reviewed.topologyCount !== 3 ||
  summary.wave02Reviewed.prototypeCount !== 12 ||
  remediation.pairGrammarRecords !== 30 ||
  remediation.passageExclusionRecords !== 8 ||
  remediation.passageExclusionGroups !== 2 ||
  remediation.unaryStatusVisualRecords !== 24 ||
  remediation.roleLabelDiagramRecords !== 48 ||
  remediation.uniqueStems !== 72 ||
  remediation.uniqueShortcuts !== 72 ||
  remediation.uniqueConclusions !== 72 ||
  ...
  throw new Error(`Unexpected reviewed V9 Wave 02 inventory: ${JSON.stringify(summary)}.`);
}

const csvColumns = [
  "topologyId", "scenarioId", "seed", "prototypeId", "provisionalAuthority",
  "itemId", "sharedPrompt", "stem", "optionA", "optionB", "optionC",
  "optionD", "correctIndex", "correctAnswer", "boundaryPolicy",
  "unknownSpouseBoundaryIds", "explicitUnmarriedIds", "pairGrammarRemediated",
  "passageExclusionRemediated", "unaryStatusVisualAligned", "evidencePaths",
  "solutionPhases", "optionAnalysis", "conclusion", "shortcut",
  "semanticFingerprint",
] as const;
const csvRows = records.map((record) => {
  const row: Record<(typeof csvColumns)[number], unknown> = {
    ...
    pairGrammarRemediated: record.metadata.pairGrammarRemediated,
    passageExclusionRemediated: record.metadata.passageExclusionRemediated,
    unaryStatusVisualAligned: record.metadata.unaryStatusVisualAligned,
...
const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BLR-CP-003 V9 Wave 02 Reviewed</title><style>*{box-sizing:border-box}body{font-family:system-ui,-apple-system,sans-serif;margin:0;background:#f4f4f5;color:#18181b}main{max-width:1240px;margin:auto;padding:24px}.summary,.group{background:#fff;border:1px solid #d4d4d8;border-radius:16px;padding:24px;margin:22px 0}.warning{background:#fff7ed;border-left:5px solid #ea580c;padding:14px}.passage{line-height:1.7;background:#fafafa;border-left:4px solid #52525b;padding:14px}.boundary,.id{font-size:.8rem;color:#71717a;overflow-wrap:anywhere}.question{border-top:2px solid #e4e4e7;padding:28px 0}.correct{font-weight:750}.badge{font-size:.75rem;background:#dcfce7;border-radius:999px;padding:2px 7px}.phase{background:#f8fafc;border-left:4px solid #4f46e5;padding:10px 14px;margin:12px 0}.conclusion{background:#f0fdf4;padding:10px}.shortcut{background:#fefce8;border-left:4px solid #ca8a04;padding:12px}.svg-family-tree{overflow:hidden;min-width:0;margin:18px 0;border:1px solid #dbeafe;border-radius:14px;background:linear-gradient(135deg,#eef2ff,#fff,#f0f9ff);padding:14px}.svg-family-tree svg{display:block;width:100%;max-width:100%;height:auto;min-width:0}.svg-tree-key{display:flex;flex-wrap:wrap;gap:16px;border-top:1px solid #dbeafe;padding:10px 8px 2px;color:#475569;font-size:12px;font-weight:650}@media(max-width:640px){main{padding:10px}.summary,.group{padding:14px;border-radius:12px}.question{padding:20px 0}.svg-family-tree{padding:6px}.svg-tree-key{gap:8px;font-size:10px}}</style></head><body><main><h1>BLR-CP-003 — V9 Topology Gap Wave 02 Reviewed</h1><section class="summary"><p><strong>72 reviewed candidates</strong> cover negative constraints, unresolved spouse boundaries and mixed in-law/generation tasks.</p><p class="warning"><strong>Human review is still required.</strong> Structural saturation, permanent QLs and release remain blocked.</p><p>Remediation: <strong>30 grammar records, 2 passage groups and 24 unary status visuals</strong>.</p><p>Combined evidence: <strong>298 candidates, 102 groups, 9 topologies and 29 prototypes</strong>.</p></section>${htmlGroups}</main></body></html>`;

const coverageMarkdown = `# BLR-CP-003 V9 Wave 02 Reviewed Coverage Matrix\n\n| Dimension | Status | Evidence |\n|---|---|---|\n${coverageMatrix.map(([dimension, status, evidence]) => `| ${dimension} | ${status} | ${evidence} |`).join("\n")}\n\nStructural saturation remains open.\n`;

await mkdir(out, { recursive: true });
await writeFile(path.join(out, "blr-cp003-v9-wave02-reviewed-summary.json"), `${JSON.stringify(summary, null, 2)}\n`, "utf8");
await writeFile(path.join(out, "blr-cp003-v9-wave02-reviewed-candidates.jsonl"), `${records.map((record) => JSON.stringify(record)).join("\n")}\n`, "utf8");
await writeFile(path.join(out, "blr-cp003-v9-wave02-reviewed-candidates.csv"), `${csvColumns.join(",")}\n${csvRows.join("\n")}\n`, "utf8");
await writeFile(path.join(out, "blr-cp003-v9-wave02-reviewed-candidates.md"), `# BLR-CP-003 V9 Wave 02 Reviewed Candidate Bank\n\n${markdown}\n`, "utf8");
await writeFile(path.join(out, "blr-cp003-v9-wave02-reviewed-candidates.html"), html, "utf8");
await writeFile(path.join(out, "blr-cp003-v9-wave02-reviewed-coverage-matrix.md"), coverageMarkdown, "utf8");
