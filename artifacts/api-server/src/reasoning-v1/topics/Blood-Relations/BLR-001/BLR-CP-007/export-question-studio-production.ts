...
const summary = {
  ...
  idempotentFullCorpusSyncEnabled: true,
...
const markdown = `# BLR-CP-007 Production Lifecycle\n\n## Verdict\n\n\`${summary.verdict}\`\n\n## Corpus\n\n| Measure | Result |\n|---|---:|\n| English | ${summary.englishCount} |\n| Hindi | ${summary.hindiCount} |\n| Punjabi | ${summary.punjabiCount} |\n| Total multilingual records | ${summary.multilingualRecordCount} |\n| Unique language records | ${summary.uniqueQuestionLanguageIdCount} |\n| Valid records | ${summary.validRecordCount} |\n\n## Production flow\n\n1. Administrators preview frozen BLR questions in Question Studio.\n2. A filtered batch or all 504 records can be synchronized into audited generation runs.\n3. Imported items remain unreviewed until an administrator approves them.\n4. Approval uses the existing Question Bank converter and creates approved question versions and options.\n5. Approved questions are eligible for test assembly and the existing publication QA workflow.\n6. Nothing is automatically published to students.\n\n## Safety and reliability\n\n- Full-corpus synchronization is idempotent by question-language identifier.\n- Generation run, item versions, audit event and outbox event are written transactionally.\n- Source fingerprints, locale, QL, canonical item and freeze authority remain attached.\n- The chapter was transplanted onto current \`New-main\` without importing stale feature-branch history.\n`;
