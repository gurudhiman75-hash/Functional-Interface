...
import {
  ...
  generateBlrCp007EditorialV2Bank,
...
const bank = generateBlrCp007EditorialV2Bank();
const telemetry = buildBlrCp007EditorialV2Telemetry(bank);
...
assert(telemetry.semicolonCorrectCount > 0, "semicolon formatting must appear in correct options");
assert(telemetry.semicolonWrongCount > 0, "semicolon formatting must appear in wrong options");
...
for (const question of bank) {
  ...
  assert(question.reviewProof.questionId === question.itemId);
  assert(question.reviewProof.reviewerNote.length > 20);

  const replay = generateBlrCp007EditorialV2Question(question.sourcePrototypeId, question.seed);
  ...
  assert(question.explanation.steps.length >= 1, `${question.itemId}: missing adaptive steps`);
  assert(question.explanation.conclusion.length >= 25, `${question.itemId}: weak conclusion`);
  ...
  assert.equal(
    question.explanation.diagramProof.edges.length,
    question.explanation.familyTree.edges.length,
  );
  question.explanation.diagramProof.edges.forEach((edge) => {
    assert(edge.label.length > 0, `${question.itemId}: unlabelled diagram edge`);
  ...
  question.explanation.optionAnalysis.forEach((analysis, index) => {
    ...
    assert(analysis.explanation.length >= 25, `${question.itemId}: non-specific option analysis`);
  ...
  if (question.qlId === "BLR-QL-034") {
    const candidateIds = question.query.kind === "MISSING_PERSON"
      ? question.query.candidatePersonIds
      : [];
    assert.deepEqual(candidateIds, ["P", "Q", "R", "S"]);
    const visiblePeople = new Set(question.completedStatements.flatMap((entry) => [entry.leftId, entry.rightId]));
    candidateIds.forEach((candidateId) => {
      assert(visiblePeople.has(candidateId), `${question.itemId}: candidate ${candidateId} absent from completed graph`);
    });
    question.options.forEach((option) => {
      assert.equal(option.graphValidity, "VALID", `${question.itemId}: invalid missing-person distractor ${option.text}`);
...
for (const [prototypeId, letters] of prototypeSequences) {
  ...
  assert.equal(sequence.length, 8, `${prototypeId}: sequence length`);
...
console.log(JSON.stringify({
  ...
  verdict: "BLR-CP-007 EDITORIAL V2 IS AN EXECUTABLE HUMAN-REVIEW CANDIDATE; V1 FINAL FREEZE IS SUPERSEDED",
}, null, 2));
