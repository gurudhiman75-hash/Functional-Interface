...
for (const locale of CLS_CP003_LOCALIZED_LOCALES) {
  ...
  for (const qlId of CLS_CP003_LOCALIZED_QL_IDS) {
    for (let seed = 0; seed < 5000; seed += 1) {
      const question = generateClsCp003LocalizedQuestionV4(qlId, locale, seed);
...
const lines: string[] = [
  ...
  "",
  "Runtime: `cls-cp003-localized-runtime-v4`",
  "",
  `Questions: ${questions.length}`,
  "",
  "The review corpus contains six deterministic samples for each of the seven prototype ancestries in each native locale. QL identities and solve contracts are preserved. V4 retains the validated balanced mathematical state, natural native-language wording, gender-safe jumble explanations and correct Hindi singular/plural grammar.",
  "",
];
...
const manifest = {
  ...
  status: "LOCALIZED_REVIEW_REQUIRED",
  runtimeVersion: "cls-cp003-localized-runtime-v4",
...
console.log("CLS-CP-003 localized V4 review exported.", {
