...
import { generateClsCp003LocalizedQuestionV5 } from "./cp003-localized-runtime-v5";

const OUTPUT_DIR = join(
  process.cwd(),
  "dist/reasoning-v1/cls-001/cp003-localized-review-v5",
);
...
for (const locale of CLS_CP003_LOCALIZED_LOCALES) {
  ...
  for (const qlId of CLS_CP003_LOCALIZED_QL_IDS) {
    for (let seed = 0; seed < 5000; seed += 1) {
      const question = generateClsCp003LocalizedQuestionV5(qlId, locale, seed);
...
const lines: string[] = [
  "# CLS-CP-003 Hindi and Punjabi Localisation Review — V5",
  "",
  "Status: `LOCALIZED_REVIEW_REQUIRED`",
  "",
  "Runtime: `cls-cp003-localized-runtime-v5`",
  "",
  `Questions: ${questions.length}`,
  "",
  "The review corpus contains six deterministic samples for each of the seven prototype ancestries in each native locale. V5 preserves the validated V4 question state while presenting only the learner-useful concept and step-by-step explanation. Forced shortcut/trap boilerplate is not part of the learner surface.",
  "",
];
...
questions.forEach((question, index) => {
  ...
  lines.push("");
  lines.push(...question.explanation.coreConcept.map((text) => `- ${text}`));
...
const manifest = {
  ...
  status: "LOCALIZED_REVIEW_REQUIRED",
  runtimeVersion: "cls-cp003-localized-runtime-v5",
  ...
  editorialPolicy: {
    beginnerFirstExplanation: true,
    forcedShortcutSection: false,
    forcedTrapSection: false,
  },
  lifecycle: {
...
await writeFile(
  join(OUTPUT_DIR, "cls-cp003-hi-pa-review-v5.json"),
  `${JSON.stringify(manifest, null, 2)}\n`,
  "utf8",
);
await writeFile(
  join(OUTPUT_DIR, "cls-cp003-hi-pa-review-v5.md"),
  `${lines.join("\n")}\n`,
  "utf8",
);

console.log("CLS-CP-003 localized V5 review exported.", {
