...
Status: `CHAPTER_CLOSED__CP001_TO_CP007_MULTILINGUAL_REVIEW_AUTHORITY__CP008_ZERO_QL_CLOSED__QUESTION_STUDIO_REVIEW_ONLY`
...
- Question Studio: review-only registration enabled; read-only generation for editorial review
...
| `CLS-QL-005` | `CLS-CP-003` | Find the word whose visible spelling or structural property differs | English frozen + approved multilingual V5 review freeze |
| `CLS-QL-006` | `CLS-CP-003` | Resolve controlled jumbles, then find the semantic outlier | English frozen + approved multilingual V5 review freeze |
| `CLS-QL-007` | `CLS-CP-004` | Find the number whose conventional arithmetic or digit property differs | English frozen + approved multilingual native review freeze |
| `CLS-QL-008` | `CLS-CP-005` | Find the pair, triple or four-number tuple whose internal rule differs | Frozen multilingual runtime + learner review V2 |
| `CLS-QL-009` | `CLS-CP-005` | Select the number tuple following the same internal rule as a reference tuple | Frozen multilingual runtime + learner review V2 |
| `CLS-QL-010` | `CLS-CP-006` | Find the single letter whose bounded alphabet property differs | Frozen multilingual runtime + learner review V2 |
| `CLS-QL-011` | `CLS-CP-006` | Find the complete ordered letter-pair whose internal alphabet relation differs | Frozen multilingual runtime + learner review V2 |
| `CLS-QL-012` | `CLS-CP-007` | Find the complete three-, four- or five-letter cluster whose internal alphabet structure differs | English frozen + approved multilingual V3 review freeze |
| `CLS-QL-013` | `CLS-CP-007` | Find the complete letter-cluster pair whose position-wise transformation differs | English frozen + approved multilingual V3 review freeze |
...
| `CLS-CP-003` | Lexical, spelling and word-structure classification | `MULTILINGUAL_REVIEW_FROZEN_V5` |
| `CLS-CP-004` | Number-property classification | `MULTILINGUAL_REVIEW_FROZEN` |
| `CLS-CP-005` | Number-pair, triple and set classification | `FROZEN_MULTILINGUAL_RUNTIME__LEARNER_REVIEW_V2` |
| `CLS-CP-006` | Alphabet, letter-pair and letter-class classification | `FROZEN_MULTILINGUAL_RUNTIME__LEARNER_REVIEW_V2` |
| `CLS-CP-007` | Letter-cluster and complete cluster-pair classification | `MULTILINGUAL_REVIEW_FROZEN_V3` |
...
Question Studio exposure:          review-only; read-only
...
Source controls:                 7
Approved review locales:        en-IN, hi-IN, pa-IN
```
...
Approved learner locales:               en-IN, hi-IN, pa-IN
```
...
CP-001 through CP-007 now have approved multilingual review authority at the chapter level; CP-008 is a closed zero-allocation ownership checkpoint. CLS-001 is registered in Question Studio for read-only editorial review in English, Hindi and Punjabi. Question Bank writes, internal/public test eligibility, mock-test eligibility, student delivery, public publication and automatic release remain disabled and require a separate explicit authorization checkpoint.
