...
const OUTPUT_DIR = join(process.cwd(), "dist/reasoning-v1/cls-001/cp007-localized-review-v2");
...
const questions: Array<ReturnType<typeof generateClsCp007LocalizedClusterQuestionV2> | ReturnType<typeof generateClsCp007LocalizedPairQuestionV2>> = [];
...
for (const locale of LOCALES) {
  for (const prototypeId of PROTOTYPES) {
    for (let sample = 0; sample < SINGLE_SAMPLES_PER_PROTOTYPE; sample += 1) {
      ...
      questions.push(generateClsCp007LocalizedClusterQuestionV2(locale, prototypeId, seed, sample === 2 ? 5 : 4));
    }
  }
  for (let sample = 0; sample < PAIR_SAMPLES_PER_LOCALE; sample += 1) {
    questions.push(generateClsCp007LocalizedPairQuestionV2(locale, sample * 5, sample % 4 === 3 ? 5 : 4));
  }
}

if (questions.length !== 102) throw new Error(`Expected 102 CP007 V2 review questions, received ${questions.length}`);

const lines: string[] = [
  "# CLS-CP-007 Hindi and Punjabi Localisation Review V2",
  "",
  "Status: `LOCALIZED_REVIEW_REQUIRED`",
  "",
  "Runtime: `cls-cp007-multilingual-review-v2`",
  "",
  ...
  "",
  "V2 preserves the V1/English mathematical state and simplifies only learner wording. Forced shortcut/trap boilerplate remains absent.",
  "",
];
...
const manifest = {
  ...
  status: "LOCALIZED_REVIEW_REQUIRED",
  runtimeVersion: "cls-cp007-multilingual-review-v2",
  editorialVersion: "simple-native-language-v2",
  locales: LOCALES,
...
await writeFile(join(OUTPUT_DIR, "cls-cp007-hi-pa-review-v2.md"), `${lines.join("\n")}\n`, "utf8");
await writeFile(join(OUTPUT_DIR, "cls-cp007-hi-pa-review-v2.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");

console.log("CLS-CP-007 Hindi/Punjabi V2 review exported.", {
