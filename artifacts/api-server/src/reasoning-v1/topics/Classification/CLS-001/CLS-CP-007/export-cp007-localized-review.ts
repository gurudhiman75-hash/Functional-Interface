...
const OUTPUT_DIR = join(process.cwd(), "dist/reasoning-v1/cls-001/cp007-localized-review");
...
const questions: Array<ReturnType<typeof generateClsCp007LocalizedClusterQuestion> | ReturnType<typeof generateClsCp007LocalizedPairQuestion>> = [];
...
for (const locale of LOCALES) {
  for (const prototypeId of PROTOTYPES) {
    for (let sample = 0; sample < SINGLE_SAMPLES_PER_PROTOTYPE; sample += 1) {
      ...
      questions.push(generateClsCp007LocalizedClusterQuestion(locale, prototypeId, seed, sample === 2 ? 5 : 4));
    }
  }
  for (let sample = 0; sample < PAIR_SAMPLES_PER_LOCALE; sample += 1) {
    questions.push(generateClsCp007LocalizedPairQuestion(locale, sample * 5, sample % 4 === 3 ? 5 : 4));
  }
}

if (questions.length !== 102) {
  throw new Error(`Expected 102 CP007 localized review questions, received ${questions.length}`);
}

const lines: string[] = [
  "# CLS-CP-007 Hindi and Punjabi Localisation Review",
  "",
  "Status: `LOCALIZED_REVIEW_REQUIRED`",
  "",
  "Runtime: `cls-cp007-multilingual-review-v1`",
  "",
  ...
  "",
  "The permanent English generators remain the sole question-state authority. Native generation changes only learner wording; options, answer, rule, ambiguity proof and difficulty remain identical.",
  "",
];
...
const manifest = {
  ...
  status: "LOCALIZED_REVIEW_REQUIRED",
  runtimeVersion: "cls-cp007-multilingual-review-v1",
  locales: LOCALES,
...
await writeFile(join(OUTPUT_DIR, "cls-cp007-hi-pa-review.md"), `${lines.join("\n")}\n`, "utf8");
await writeFile(join(OUTPUT_DIR, "cls-cp007-hi-pa-review.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");

console.log("CLS-CP-007 Hindi/Punjabi review exported.", {
