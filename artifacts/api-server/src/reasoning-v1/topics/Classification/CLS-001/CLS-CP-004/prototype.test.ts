...
for (const prototype of CLS_CP004_PROTOTYPES) {
  for (const optionCount of [4, 5] as const) {
    for (let seed = 0; seed < 70; seed += 1) {
      const question = generateClsCp004Prototype(prototype.prototypeId, seed, optionCount);
      const replay = generateClsCp004Prototype(prototype.prototypeId, seed, optionCount);
...
assert.ok(fingerprints.size >= 1750, `Visible discovery diversity is too low: ${fingerprints.size}/1820`);
...
assert.throws(() => generateClsCp004Prototype("CLS-CP004-PROT-001", -1));
assert.throws(() => generateClsCp004Prototype("CLS-CP004-PROT-001", 0, 3 as never));
assert.throws(() => generateClsCp004Prototype("CLS-CP004-PROT-999" as never, 0));
