...
import { SIF_CP010_PROFILE_BY_AUTHORITY_ID } from "./cp010-data-authorities.ts";
...
function render(question: GeneratedSifQuestion, index: number): string {
  return [
    ...
    `**Conditional family:** ${SIF_CP010_PROFILE_BY_AUTHORITY_ID[question.scenarioId].family}`,
...
export function buildSifCp010ReviewMarkdown(): string {
  const authorities = listSifAuthorities("SIF-CP010");
  const review = buildSifCpReviewPack({ cpId: "SIF-CP010", locale: "en-IN", seed: 91_008 });
  const families = [...new Set(authorities.map((entry) => SIF_CP010_PROFILE_BY_AUTHORITY_ID[entry.id].family))];
  const familyRows = families.map((family) => `| ${family} | ${authorities.filter((entry) => SIF_CP010_PROFILE_BY_AUTHORITY_ID[entry.id].family === family).length} | ${review.questions.filter((entry) => SIF_CP010_PROFILE_BY_AUTHORITY_ID[entry.scenarioId].family === family).length} |`);
  const parity = [91_008, 91_009, 91_013, 91_019].flatMap((seed) => (["en-IN", "hi-IN", "pa-IN"] as const).map((locale) => generateSifQuestion({ cpId: "SIF-CP010", locale, seed })));
  return [
    ...
    `**Authority pool:** ${authorities.length} trilingual scenarios across ${families.length} conditional-inference families`,
...
if (process.argv[1]?.includes("cp010-review-export")) {
