...
function render(question: GeneratedSifQuestion, index: number): string {
  return [
    ...
    `**Synthesis family:** ${SIF_CP008_PROFILE_BY_AUTHORITY_ID[question.scenarioId].family}`,
...
export function buildSifCp008ReviewMarkdown(): string {
  const authorities = listSifAuthorities("SIF-CP008");
  const review = buildSifCpReviewPack({ cpId: "SIF-CP008", locale: "en-IN", seed: 80_000 });
  const families = [...new Set(authorities.map((entry) => SIF_CP008_PROFILE_BY_AUTHORITY_ID[entry.id].family))];
  const familyRows = families.map((family) => `| ${family} | ${authorities.filter((entry) => SIF_CP008_PROFILE_BY_AUTHORITY_ID[entry.id].family === family).length} | ${review.questions.filter((entry) => SIF_CP008_PROFILE_BY_AUTHORITY_ID[entry.scenarioId].family === family).length} |`);
  const parity = [80_000, 80_001, 80_005, 80_011].flatMap((seed) => (["en-IN", "hi-IN", "pa-IN"] as const).map((locale) => generateSifQuestion({ cpId: "SIF-CP008", locale, seed })));
  return [
    ...
    `**Authority pool:** ${authorities.length} trilingual scenarios across ${families.length} synthesis families`,
    `**Review sample:** ${review.questions.length} distinct scenarios; three from each family`,
    `**Difficulty mix:** ${review.effectiveDistribution.MEDIUM} Medium / ${review.effectiveDistribution.HARD} Hard`,
    ...
    "## Synthesis-family coverage", "", "| Family | Authorities | Review questions |", "|---|---:|---:|", ...familyRows, "",
...
if (process.argv[1]?.includes("cp008-review-export")) {
  const outputPath = process.argv[2];
  if (!outputPath) throw new Error("Output path is required.");
  writeFileSync(outputPath, buildSifCp008ReviewMarkdown(), "utf8");
}
