...
export function renderHindiStem023To035(english: R): string | null {
  ...
  switch (qlId) {
    ...
    case "DIR-QL-025":
      return `चिह्न @, #, % और & एक-से-एक व्यवस्था में उत्तर, पूर्व, दक्षिण और पश्चिम दिशाओं को दर्शाते हैं। दिए गए उदाहरण हैं: ${(s.evidence ?? []).map((e: R) => { const names = e.displayEntities ?? []; return `“${evidenceChain(e)}” के अनुसार ${nameHi(names[0])}, ${nameHi(names[names.length - 1])} के ${directionHi(e.resultDirection)} में है`; }).join("; ")}। कौन-सा चिह्न ${directionHi(s.targetDirection)} को दर्शाता है?`;
    case "DIR-QL-026":
    ...
    case "DIR-QL-034":
      return `${sunTime(s.period, s.variation ?? english.seed)} ${placeHi(s.place)} में ${nameHi(s.name)} की छाया ठीक ${sideHi(s.side)} पड़ी। इसके बाद क्रमशः ये मोड़ लिए गए: ${(s.turns ?? []).map((t: string) => turnHi(t)).join(", फिर ")}। अब मुख किस दिशा में है?`;
    case "DIR-QL-035":
