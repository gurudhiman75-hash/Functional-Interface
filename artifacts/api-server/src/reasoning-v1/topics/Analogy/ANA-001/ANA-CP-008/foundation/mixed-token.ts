export type MixedToken =
  ...
  | { kind: "NUMBER_CLUSTER"; number: number; letters: string };
...
export function numberClusterToken(number: number, letters: string): MixedToken {
  const normalized = normalizeLetters(letters);
  if (normalized.length < 2 || normalized.length > 6) {
    throw new Error(`NUMBER_CLUSTER requires 2..6 letters: ${letters}`);
  }
  return { kind: "NUMBER_CLUSTER", number: normalizeNumber(number), letters: normalized };
}
...
export function renderMixedToken(token: MixedToken): string {
  switch (token.kind) {
    ...
    case "NUMBER_CLUSTER": return `${token.number}${token.letters}`;
  }
}
...
export function mixedTokenKey(token: MixedToken): string {
  switch (token.kind) {
    ...
    case "NUMBER_CLUSTER": return `NUMBER_CLUSTER:${token.number}:${token.letters}`;
  }
}
...
export function parseMixedTokenForReview(value: string): MixedToken {
  ...
  if (new RegExp(`^${SIGNED_INTEGER_PATTERN}$`).test(normalized)) {
    return numberToken(Number(normalized));
  }

  const numberFirst = normalized.match(new RegExp(`^(${SIGNED_INTEGER_PATTERN})([A-Z]{1,6})$`));
  if (numberFirst) {
    const number = Number(numberFirst[1]);
    const letters = numberFirst[2];
    return letters.length === 1
      ? numberLetterToken(number, letters)
      : numberClusterToken(number, letters);
  }
...
export function mixedTokenLetterText(token: MixedToken): string | null {
  switch (token.kind) {
    ...
    case "CLUSTER_NUMBER":
    case "NUMBER_CLUSTER": return token.letters;
...
export function mixedTokenNumber(token: MixedToken): number | null {
  switch (token.kind) {
    ...
    case "NUMBER_LETTER":
    case "CLUSTER_NUMBER":
    case "NUMBER_CLUSTER": return token.number;
    case "LETTER":
