...
import { misCp007RuleByCandidateId } from './rule-definitions';
...
const outputDirectory = process.env.MIS_CP_007_HTML_OUTPUT_DIR || resolve(process.cwd(), 'dist/reasoning-v1/mis-001');
const outputPath = resolve(outputDirectory, 'MIS-CP-007-REVIEW.html');
mkdirSync(outputDirectory, { recursive: true });

const cards: string[] = [];
for (const candidateId of MIS_CP007_CANDIDATE_IDS) {
  const rule = misCp007RuleByCandidateId(candidateId);
  const samples: string[] = [];
  for (let sample = 1; sample <= 4; sample += 1) {
    const q = generateMisCp007Question(candidateId, 'MIS-CP-007-HTML:' + candidateId + ':S' + sample);
...
<title>MIS-CP-007 Box / Corner-Centre Review</title>
<style>
...
<body><main>
<h1>MIS-CP-007 Box / Corner-Centre Review</h1>
<p class="intro">Actual inline SVG square/rectangle figures; genuine row-column matrices remain excluded.</p>
${cards.join('')}
</main></body></html>`;
