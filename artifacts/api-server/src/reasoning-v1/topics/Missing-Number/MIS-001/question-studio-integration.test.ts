...
async function main() {
  ...
  assert.deepEqual(MIS_001_QUESTION_STUDIO_PACKAGE.cpIds, ['MIS-CP-001', 'MIS-CP-002']);
  ...
  assert.equal(MIS_001_QUESTION_STUDIO_PACKAGE.metadata?.candidateCount, 15);
  ...
  assert.ok(listed);
  assert.deepEqual(listed?.cpIds, ['MIS-CP-001', 'MIS-CP-002']);
  ...
  assert.equal(candidateIds.size, 15);
  assert.deepEqual([...checkpointIds].sort(), ['MIS-CP-001', 'MIS-CP-002']);
  ...
  const medium = await generateMis001QuestionStudioBatch({
    ...
    difficulty: 'Medium',
    count: 16,
    seed: 'MIS-QS-MEDIUM-CP001-CP002',
  });
  ...
  console.log('MIS-001 chapter Question Studio CP001-CP002 integration audit passed.');
}
