...
import { misCp005RuleByCandidateId } from './rule-definitions';
...
const outputDirectory = process.env.MIS_CP_005_HTML_OUTPUT_DIR || resolve(process.cwd(), 'dist/reasoning-v1/mis-001');
const outputPath = resolve(outputDirectory, 'MIS-CP-005-REVIEW.html');
mkdirSync(outputDirectory, { recursive: true });

const cards: string[] = [];
for (const candidateId of MIS_CP005_CANDIDATE_IDS) {
  const rule = misCp005RuleByCandidateId(candidateId);
  const samples: string[] = [];
  for (let sample = 1; sample <= 4; sample += 1) {
    const q = generateMisCp005Question(candidateId, 'MIS-CP-005-HTML:' + candidateId + ':S' + sample);
...
<title>MIS-CP-005 Triangle Review</title>
<style>
...
<body><main>
<h1>MIS-CP-005 Triangle Review</h1>
<p class="intro">Actual inline SVG triangle figures; review-only provisional candidates.</p>
${cards.join('')}
</main></body></html>`;
