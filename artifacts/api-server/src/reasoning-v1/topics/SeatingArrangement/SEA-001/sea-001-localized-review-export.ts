...
function assertNativeExportCandidate(canonical: AuditCaselet, localized: Sea001LocalizedReviewCaselet): void {
  ...
  if (localized.canonicalCaseletId !== canonical.caseletId) {
    throw new Error(`${canonical.caseletId}/${localized.locale}: export canonical caselet identity changed`);
  }
  ...
  if (latinTokens.length !== 0) {
    throw new Error(`${canonical.caseletId}/${localized.locale}: export contains Latin learner text: ${latinTokens.slice(0, 8).join(", ")}`);
  }
  for (const canonicalName of SEA001_REVIEW_CANONICAL_NAMES) {
    if (new RegExp(`\\b${canonicalName}\\b`).test(learnerSurface)) {
      throw new Error(`${canonical.caseletId}/${localized.locale}: export exposes canonical Latin name ${canonicalName}`);
    }
  }
  if (!localized.humanLanguageReviewRequired) {
    throw new Error(`${canonical.caseletId}/${localized.locale}: export must require human language review`);
  }
  if (localized.productDeliveryUnlocked || localized.productionStagingApproved) {
    throw new Error(`${canonical.caseletId}/${localized.locale}: export cannot unlock delivery or staging`);
  }
}
