...
const mechanicalHindiFragments = [
  ...
  "नवदीप पर 1 का अंतिम छोर",
  "कौन हैं ठीक अगला पड़ोसी",
  "के लिए हर बायाँ/दायाँ संकेत",
  "चलने पर 2 सीटें घड़ी की दिशा में से",
  "यदि सभी बदलता है उनका मुख-दिशा दिशा",
  "कितने व्यक्ति बीच में बैठे हैं",
] as const;
...
const mechanicalPunjabiFragments = [
  ...
  "ਨਵਦੀਪ 'ਤੇ 1 ਦਾ ਅੰਤਲੇ ਸਿਰੇ",
  "ਕੌਣ ਹਨ ਬਿਲਕੁਲ ਅਗਲਾ",
  "ਲਈ ਹਰ ਖੱਬਾ/ਸੱਜਾ ਸੰਕੇਤ",
  "ਚੱਲਣ 'ਤੇ 2 ਸੀਟਾਂ ਘੜੀ ਦੀ ਦਿਸ਼ਾ ਵਿੱਚ ਤੋਂ",
  "ਜੇ ਸਭ ਬਦਲਦਾ ਹੈ ਉਨ੍ਹਾਂ ਦਾ ਮੂੰਹ ਦੀ ਦਿਸ਼ਾ",
  "ਕਿੰਨੇ ਵਿਅਕਤੀ ਵਿਚਕਾਰ ਬੈਠੇ ਹਨ",
] as const;
...
let residualLatinCount = 0;
let mechanicalTranslationeseCount = 0;
let ordinalGrammarViolationCount = 0;
let singularGenderMarkerCount = 0;
let genericWrongOptionFallbackCount = 0;
let explanationParityCaseletCount = 0;
let sharedExplanationBlockParityCount = 0;
let caseDecisionParityCount = 0;
let optionRationaleParityCount = 0;
...
for (const locale of SEA001_TRANSLATION_TARGET_LOCALES) {
  for (const canonical of canonicalReview) {
    ...
    assert(
      localized.canonicalParityFingerprint === sea001CanonicalParityFingerprint(canonical),
      `${canonical.caseletId}: recorded canonical parity mismatch`,
    );
    assert(
      sea001CanonicalParityFingerprint(localized) === sea001CanonicalParityFingerprint(canonical),
      `${canonical.caseletId}/${locale}: semantic parity changed`,
    );
    ...
    assert(
      explanationParity.sharedEnglishBlocks === explanationParity.sharedLocalizedBlocks,
      `${canonical.caseletId}/${locale}: shared explanation block structure changed`,
    );
    assert(
      explanationParity.childExplanations === canonical.children.length,
      `${canonical.caseletId}/${locale}: correct-answer explanation coverage changed`,
    );
    const canonicalOptionCount = canonical.children.reduce((sum, child) => sum + child.options.length, 0);
    assert(
      explanationParity.optionRationales === canonicalOptionCount,
      `${canonical.caseletId}/${locale}: option-rationale coverage changed`,
    );
    explanationParityCaseletCount += 1;
    sharedExplanationBlockParityCount += 1;
    optionRationaleParityCount += 1;

    const englishCaseCounts = englishCaseDecisionCounts(canonical.sharedExplanation);
    ...
    for (let childIndex = 0; childIndex < canonical.children.length; childIndex += 1) {
      const source = canonical.children[childIndex]!;
      const candidate = localized.children[childIndex]!;
      ...
      for (let optionIndex = 0; optionIndex < 4; optionIndex += 1) {
        const a = source.options[optionIndex]!;
        const b = candidate.options[optionIndex]!;
...
assert(sharedExplanationBlockParityCount === 200, `expected shared block parity for 200 caselets, got ${sharedExplanationBlockParityCount}`);
...
console.log("explanation parity", `${explanationParityCaseletCount}/200`);
