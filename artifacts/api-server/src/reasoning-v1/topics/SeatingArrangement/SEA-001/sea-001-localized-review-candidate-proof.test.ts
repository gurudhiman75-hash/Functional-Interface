...
let localizedCaseletCount=0,localizedChildCount=0,canonicalNameLeakCount=0,residualLatinCount=0,mechanicalTranslationeseCount=0;
...
for(const locale of SEA001_TRANSLATION_TARGET_LOCALES){
  for(const canonical of canonicalReview){
    ...
    if(locale==="hi-IN"){assert(/[\u0900-\u097F]/u.test(surface),`${canonical.caseletId}: Hindi learner surface lacks Devanagari`);for(const fragment of mechanicalHindiFragments) if(surface.includes(fragment)) mechanicalTranslationeseCount+=1;}else{assert(/[\u0A00-\u0A7F]/u.test(surface),`${canonical.caseletId}: Punjabi learner surface lacks Gurmukhi`);for(const fragment of mechanicalPunjabiFragments) if(surface.includes(fragment)) mechanicalTranslationeseCount+=1;}
