...
assert.equal(decisions.filter((decision) => decision === "EXCLUDE_FROM_PERMANENT_SET_RETAIN_STRESS_ONLY").length, 2);
assert.equal(SEA002_CP008_WAVE04_AUTHORITY_STATUS.stressOnlyFamilies.includes("SEA-CP008-SIG-D"), true);
assert.equal(SEA002_CP008_WAVE04_AUTHORITY_STATUS.stressOnlyFamilies.includes("SEA-CP008-PROT-010"), true);
