...
let placementFacingPrerequisiteChecks = 0;
let auth01InferenceChecks = 0;
let auth04InferenceChecks = 0;

function arrow(value: "N" | "S") {
  return value === "N" ? "↑" : "↓";
}
...
for (const authorityKey of AUTHORITIES) {
  for (let index = 0; index < 20; index += 1) {
    ...
    const explanation = renderSea002Cp007TeacherExplanationV3(caselet);
    surfaces += 1;
    ...
    if (authorityKey === "CP007-AUTH-01") {
      ...
      assert.ok(explanation.includes("1) Settle the facing directions before using any left/right placement clue:"));
      ...
      const buildStart = explanation.indexOf(`2) Build ${reference}'s row from the position clues:`);
      const referenceRow = caselet.participants.find((p) => p.id === reference)!.seat.row;
      const placementReferences = caselet.clues
        .filter((clue) => clue.kind === "SAME_ROW_OFFSET")
        .filter((clue) => caselet.participants.find((p) => p.id === clue.reference)!.seat.row === referenceRow)
        .map((clue) => clue.kind === "SAME_ROW_OFFSET" ? clue.reference : "");
      for (const personId of new Set(placementReferences)) {
        const person = caselet.participants.find((p) => p.id === personId)!;
        assert.ok(
          explanation.slice(0, buildStart).includes(`${person.id} ${arrow(person.facing)}`),
          `${caselet.caseletId} uses ${person.id}'s facing before teaching it.`,
        );
        placementFacingPrerequisiteChecks += 1;
      }
    ...
    if (authorityKey === "CP007-AUTH-04") {
      ...
      assert.ok(explanation.includes("1) Settle the facing directions before building either row:"));
      ...
      const buildStart = explanation.indexOf("3) Build and align the two rows:");
      const placementReferences = caselet.clues
        .filter((clue) => clue.kind === "SAME_ROW_OFFSET" || clue.kind === "DIAGONAL")
        .map((clue) => clue.kind === "SAME_ROW_OFFSET" ? clue.reference : clue.kind === "DIAGONAL" ? clue.reference : "");
      for (const personId of new Set(placementReferences)) {
        const person = caselet.participants.find((p) => p.id === personId)!;
        assert.ok(
          explanation.slice(0, buildStart).includes(`${person.id} ${arrow(person.facing)}`),
...
console.log("PASS_SEA002_CP007_ENGLISH_SURFACE_V7_EXPLICIT_PLACEMENT_FACINGS");
...
console.log("placement-facing prerequisite checks", placementFacingPrerequisiteChecks);
