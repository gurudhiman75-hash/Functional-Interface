...
const thirdQueryContracts=new Set<string>();
const thirdQueryFamilies=new Set<string>();
  ...
  for(const width of WIDTHS) {
    for(let index=0;index<4;index+=1) {
      ...
      const sourceClues=caselet.clues.filter((clue)=>
        ...
        clue.kind==="SAME_ROW_EQUAL_GAP"||
        clue.kind==="NOT_ADJACENT"||
        clue.kind==="ROW_END_DISTANCE"||
        clue.kind==="FACING_REFERENT_RELATIVE");
      assert.equal(sourceClues.length,8,`${caselet.caseletId}: incomplete source-realness clue bundle`);
      ...
      assert.ok(sourceClues.some((clue)=>clue.kind==="SAME_ROW_EQUAL_GAP"),`${caselet.caseletId}: missing equal-gap semantic`);
      ...
      assert.deepEqual(caselet.children.slice(0,2).map((child)=>child.queryContractId),["SEA-QC-010","SEA-QC-003"]);
      assert.ok(caselet.children[2].queryContractId==="SEA-QC-006"||caselet.children[2].queryContractId==="SEA-QC-014",`${caselet.caseletId}: unexpected third query contract`);
      assert.ok(caselet.children[3].queryContractId==="SEA-QC-008"||caselet.children[3].queryContractId==="SEA-QC-015",`${caselet.caseletId}: unexpected fourth query contract`);
      thirdQueryContracts.add(caselet.children[2].queryContractId);
      fourthQueryContracts.add(caselet.children[3].queryContractId);
      thirdQueryFamilies.add(caselet.children[2].answerDeterminingFactFingerprint.split(":")[0]!);
      assert.equal(caselet.children[2].answerType,"PAIR");
      assert.equal(caselet.children[3].answerType,caselet.children[3].queryContractId==="SEA-QC-008"?"COUNT":"RELATION");
      ...
      assert.match(learnerSurface,/number of persons sitting between .* is the same as/i);
...
assert.deepEqual([...sourceKinds].sort(),["FACING_REFERENT_RELATIVE","NOT_ADJACENT","ROW_END_DISTANCE","SAME_ROW_EQUAL_GAP","SAME_ROW_GAP","SAME_ROW_MIN_BETWEEN"]);
assert.deepEqual([...thirdQueryContracts].sort(),["SEA-QC-006","SEA-QC-014"],"source corpus must exercise neighbour and pair-at-position query contracts");
assert.deepEqual([...thirdQueryFamilies].sort(),["FACING_PAIR","NEIGHBOURS","ROW_END_PAIR"],"source corpus must exercise neighbour, facing-pair and row-end-pair surfaces");
assert.deepEqual([...fourthQueryContracts].sort(),["SEA-QC-008","SEA-QC-015"],"source corpus must exercise linear count-between and relative-position fourth-query families");
...
assert.ok(structures.size>=32,`source-realness structure diversity too thin: ${structures.size}`);
...
console.log("third-query families",[...thirdQueryFamilies].sort().join(","));
