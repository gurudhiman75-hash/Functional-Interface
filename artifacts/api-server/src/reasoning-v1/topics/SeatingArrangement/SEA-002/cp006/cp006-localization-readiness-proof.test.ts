...
import { SEA002_CP006_APPROVED_REVIEW, buildApprovedCp006ReviewLedger } from "./review/approved-review.ts";
import { SEA002_CP006_REOPENED_ENGLISH_REVIEW } from "./review/reopened-review.ts";
import {
  SEA002_CP006_LOCALIZATION_ENGLISH_BLOCKER,
...
const expectedGlossaryConcepts = [
  "POSITION","UPPER_ROW","LOWER_ROW","NORTH","SOUTH","LEFT","RIGHT","SAME_ROW","DIFFERENT_ROW","OPPOSITE","DIAGONAL","ADJACENT","NOT_ADJACENT","BETWEEN","END_POSITION","FROM_EITHER_END","PERSON_FACING","CASE_ACCEPT","CASE_REJECT",
] as const;

assert.equal(SEA002_CP006_LOCALIZATION_READINESS.status,"BLOCKED_BY_ENGLISH_REVIEW_REOPENED");
...
assert.equal(SEA002_CP006_LOCALIZATION_READINESS.humanReviewStatus,"NOT_STARTED");
assert.deepEqual(SEA002_CP006_LOCALIZATION_READINESS.activeEditorialBlockers,[SEA002_CP006_LOCALIZATION_ENGLISH_BLOCKER]);
...
assert.equal(SEA002_CP006_LOCALIZATION_READINESS.englishFreezeActive,false);
assert.equal(SEA002_CP006_LOCALIZATION_READINESS.englishReviewCandidateFingerprint,SEA002_CP006_REOPENED_ENGLISH_REVIEW.currentReviewCandidateFingerprint);
...
const requiredProtectedFields=["checkpointId","blueprintAuthorityId","state","clues","structuralFingerprint","queryContractId","answerType","answerDeterminingFactFingerprint","answerIndex","answer","option.value","option.isCorrect","option.misconceptionId","permanentQlId"];
for(const field of requiredProtectedFields) assert.ok((SEA002_CP006_LOCALIZATION_PROTECTED_FIELDS as readonly string[]).includes(field),`missing protected field ${field}`);
...
assert.equal(fingerprint,SEA002_CP006_REOPENED_ENGLISH_REVIEW.currentReviewCandidateFingerprint);
assert.notEqual(fingerprint,SEA002_CP006_APPROVED_REVIEW.approvedReviewFingerprint);
assert.throws(()=>buildApprovedCp006ReviewLedger(corpus),/signed review is stale/);
assert.throws(()=>assertCp006LocalizationBindsToApprovedEnglish(),/localization is blocked/);
...
console.log("PASS_SEA002_CP006_LOCALIZATION_BLOCKED_BY_ENGLISH_REVIEW");
console.log("canonical locale",SEA002_CP006_LOCALIZATION_READINESS.canonicalLocale);
console.log("target locales reserved",SEA002_CP006_TRANSLATION_TARGET_LOCALES.join(","));
console.log("corrected English review fingerprint",fingerprint);
...
console.log("localization status",SEA002_CP006_LOCALIZATION_READINESS.status);
console.log("human Hindi/Punjabi review","NOT_STARTED");
