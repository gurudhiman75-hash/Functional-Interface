...
import {
  ...
  type Cp006CorrectedRationaleKind,
} from "./rationale-fidelity-polish.ts";
...
export function polishCp006LocalizedLearnerText(
  text:string,
  ...
  for(const canonical of caselet.people){
    ...
    if(locale==="hi-IN"){
      output=output
        ...
        .replace(new RegExp(`${token} ([^।\\n]+) नहीं हो सकता।`,"gu"),`${person} का स्थान $1 नहीं हो सकता।`)
        .replace(new RegExp(`${token} ([^।\\n]+) हो सकता है।`,"gu"),`${person} का स्थान $1 हो सकता है।`)
      ...
      output=output
        ...
        .replace(new RegExp(`${token} ([^।\\n]+) ਨਹੀਂ ਹੋ ਸਕਦਾ।`,"gu"),`${person} ਦਾ ਸਥਾਨ $1 ਨਹੀਂ ਹੋ ਸਕਦਾ।`)
        .replace(new RegExp(`${token} ([^।\\n]+) ਹੋ ਸਕਦਾ ਹੈ।`,"gu"),`${person} ਦਾ ਸਥਾਨ $1 ਹੋ ਸਕਦਾ ਹੈ।`)
...
export function localizeCp006PolishedReviewCaselet(
  caselet:Sea002Cp006Caselet,
  locale:Sea002Cp006TranslatedLocale,
  ...
  const children=base.children.map((child)=>Object.freeze({
    ...
    options:child.options.map((option)=>Object.freeze({
      ...option,
      explanation:polish(option.explanation),
    })) as unknown as typeof child.options,
