...
import { localizeCp006ReviewCaselet } from "./localization/candidate-localizer.ts";
...
if (englishFingerprint !== SEA002_CP006_ENGLISH_FREEZE.approvedReviewFingerprint) {
  throw new Error(`CP006 localized export source is not the frozen English review: frozen=${SEA002_CP006_ENGLISH_FREEZE.approvedReviewFingerprint}, current=${englishFingerprint}`);
}

const localized = corpus.flatMap((canonical) => SEA002_CP006_TRANSLATION_TARGET_LOCALES.map((locale) => ({
  canonical,
  localized: localizeCp006ReviewCaselet(canonical, locale),
})));
...
const outputDir = process.env.CP006_LOCALIZED_REVIEW_OUTPUT_DIR ?? "/tmp/cp006-hi-pa-review";
...
const manifest = {
  ...
  renderer: "SEA002_CP006_EXPLANATION_PARITY_POSITION_V1",
...
const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SEA-CP-006 Hindi/Punjabi Review — 200 localized caselets</title><style>body{font-family:system-ui,-apple-system,"Noto Sans Devanagari","Noto Sans Gurmukhi",sans-serif;max-width:1500px;margin:auto;padding:24px;line-height:1.45;background:#f5f6f8;color:#171717}.caselet{background:#fff;border:1px solid #ddd;border-radius:12px;padding:22px;margin:0 0 28px}.meta,.fingerprint,.note{color:#666;font-size:12px}.cols{display:grid;grid-template-columns:1fr 1fr;gap:18px}.question{border-top:1px solid #ddd;margin-top:20px;padding-top:12px}table{width:100%;border-collapse:collapse;margin:8px 0 16px}th,td{width:50%;text-align:left;vertical-align:top;border:1px solid #ddd;padding:9px}.solution pre{min-height:180px}pre{white-space:pre-wrap;background:#fafafa;padding:12px;border-radius:8px;overflow:auto}@media(max-width:850px){.cols{grid-template-columns:1fr}th,td{display:block;width:auto}}</style></head><body><h1>SEA-002 / SEA-CP-006 — Hindi/Punjabi Review Candidate</h1><p><strong>100 Hindi + 100 Punjabi localized caselets · 800 localized child questions</strong></p><p>Frozen English fingerprint: <code>${h(englishFingerprint)}</code><br>Localized review fingerprint: <code>${h(localizedReviewFingerprint)}</code></p><p>This is an executable explanation-parity review candidate. Human Hindi/Punjabi approval is required before multilingual freeze. Question Studio, Question Bank, mocks, staging and public delivery remain locked.</p>${cards}</body></html>`;
writeFileSync(join(outputDir, "cp006-hi-pa-review-200.html"), html);

console.log("CP006_LOCALIZED_REVIEW_EXPORT", outputDir);
