...
for (const probe of probes) {
  for (const seed of [1, 2, 3]) {
    ...
    assert.equal(v3.correctIndex ?? question.correctIndex, question.correctIndex);
    ...
    const roles = v3.distractors.map((entry) => entry.role);
    assert.equal(new Set(roles).size, 3);
    distinctRoleChecks += 1;
    ...
    for (const distractor of v3.distractors) {
      ...
      assert.doesNotMatch(
        ...
        /trap code|authority|taxonomy|canonical/i,
      );
    ...
    increment(taskCounts, v3.editorialTaskKind);
    increment(proofModelCounts, v3.proofModel);
    sampledReviews += 1;
  }
}
...
assert.equal(distinctRoleChecks, 420);
