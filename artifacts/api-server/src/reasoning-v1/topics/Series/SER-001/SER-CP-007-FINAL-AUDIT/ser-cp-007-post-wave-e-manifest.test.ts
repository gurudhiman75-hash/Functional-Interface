...
const sourceProbeCount =
  ...
  SER_CP007_WAVE_E_SOURCE_RULE_IDS.length;

assert.equal(sourceProbeCount, 45);
assert.equal(templateIds.length, 140);
assert.equal(new Set(templateIds).size, 140);
assert.equal(authorityIds.size, 17);
assert.equal(templateIds.length * SEEDS_PER_TEMPLATE, 16_800);
...
const expectedAuthorityCounts = {
  ...
  CYCLIC_CLUSTER_PERMUTATION: 1_080,
...
for (const id of SER_CP007_WAVE_E_TEMPORARY_TEMPLATE_IDS) {
  const question = generateSerCp007WaveEQuestion(id, 1);
  assertCommonSample(question, renderSerCp007WaveEReview(question));
}

assert.equal(sampleReviews, 140);
assert.equal(sampleQuestionIds.size, 140);
assert.deepEqual(
...
console.log(
  JSON.stringify(
    {
      status:
        "PASS_SER_CP007_POST_WAVE_E_MANIFEST_COLLISION_COMPLETE_LEDGER_PENDING",
      completedWaves: 5,
      ...
      waveEExecutableDiscovery: "COMPLETE",
      postWaveECollisionAudit: "COMPLETE",
      mathematicalSaturation: "PENDING_SOURCE_LEDGER_COMPLETION",
      traceabilityPass: "IN_PROGRESS",
      verifiedSourceRecords: 26,
