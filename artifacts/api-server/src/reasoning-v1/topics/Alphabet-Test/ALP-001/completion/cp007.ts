...
const RULES = [
  ...
  { d: { en: "replace odd-ranked letters by the next letter and keep even-ranked letters unchanged", hi: "विषम वर्णमाला-स्थान वाले अक्षर को अगले अक्षर से बदलें और सम स्थान वाले अक्षर को न बदलें", pa: "ਟਾਂਕ ਵਰਣਮਾਲਾ-ਥਾਂ ਵਾਲੇ ਅੱਖਰ ਨੂੰ ਅਗਲੇ ਅੱਖਰ ਨਾਲ ਬਦਲੋ ਅਤੇ ਜਿਸਤ ਥਾਂ ਵਾਲੇ ਅੱਖਰ ਨੂੰ ਨਾ ਬਦਲੋ" }, f: (token: string) => rank(token) % 2 ? shift(token, 1) : token },
] as const;
...
export function buildCp007(ql: AlpQuestionLogic, seed: number): C {
  ...
  const shortcut: L = {
    ...
    pa: "ਪਹਿਲਾਂ ਹਰ ਅੱਖਰ ਦੀ ਸ਼੍ਰੇਣੀ ਨਿਸ਼ਾਨ ਲਗਾਓ, ਇੱਕ ਦਿੱਤਾ ਬਦਲਾਅ ਕਰੋ ਅਤੇ ਉਸ ਤੋਂ ਬਾਅਦ ਹੀ ਛਾਂਟਣ ਜਾਂ ਉਲਟਣ ਕਰੋ।",
  };
  ...
  if (ql.solveMode === "CLASS_SHIFT_TRANSFORMED_WORD") {
    ...
    query = { en: `sort the changed letters and read left position ${position}`, hi: `बदले अक्षर क्रमबद्ध करके बायाँ स्थान ${position} पढ़ें`, pa: `ਬਦਲੇ ਅੱਖਰ ਕ੍ਰਮਬੱਧ ਕਰਕੇ ਖੱਬੀ ਥਾਂ ${position} ਪੜ੍ਹੋ` };
    ...
    query = { en: `sort the changed letters and find the first ${target}`, hi: `बदले अक्षर क्रमबद्ध करके पहला ${target} खोजें`, pa: `ਬਦਲੇ ਅੱਖਰ ਕ੍ਰਮਬੱਧ ਕਰਕੇ ਪਹਿਲਾ ${target} ਲੱਭੋ` };
    ...
    query = { en: `replace vowels by opposite letters and read position ${position}`, hi: `स्वरों को विपरीत अक्षरों से बदलकर स्थान ${position} पढ़ें`, pa: `ਸਵਰਾਂ ਨੂੰ ਉਲਟ ਅੱਖਰਾਂ ਨਾਲ ਬਦਲ ਕੇ ਥਾਂ ${position} ਪੜ੍ਹੋ` };
