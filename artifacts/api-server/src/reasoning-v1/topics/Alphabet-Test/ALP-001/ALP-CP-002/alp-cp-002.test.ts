...
import { generateAlpCp002Question } from "./generator";
import { ALP_CP002_TASK_REGISTRY } from "./task-registry";

function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(message);
}

assert(ALP_CP002_TASK_REGISTRY.questionLogics.length === 18, "ALP-CP-002 QL count mismatch.");
let generated = 0;
for (const ql of ALP_CP002_TASK_REGISTRY.questionLogics) {
  for (let seed = 0; seed < 40; seed += 1) {
    const data = generateAlpInstance(ql, seed);
    const solved = solveAlpInstance(ql, data);
    const question = generateAlpCp002Question(ql.qlId, seed, "en-IN");
...
console.log("ALP-CP-002 checkpoint audit passed.", { generated });
