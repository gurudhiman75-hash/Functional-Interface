...
function transformationText(transformation: WorBankingTransformation, locale: WorLocale): string {
  const english: Record<WorBankingTransformation, string> = {
    ...
    SHIFT_FIRST_PREVIOUS: "Replace the first letter of every group with the preceding letter of the alphabet.",
    SHIFT_FIRST_NEXT: "Replace the first letter of every group with the following letter of the alphabet.",
  };
...
function englishMoveText(offset: number): string {
  if (offset === 0) return "";
  const distance = Math.abs(offset);
  const unit = distance === 1 ? "place" : "places";
  const direction = offset > 0 ? "forward" : "backward";
  return ` Then move ${distance} ${unit} ${direction} in the alphabet.`;
}
...
function renderBankingStem(trace: WorBankingTrace, locale: WorLocale): string {
  ...
  switch (trace.taskKind) {
    ...
    case "BANK_SORT_CONCAT_CHAR":
      return `Arrange the five groups in ${direction} and join them without spaces. What is character ${trace.globalCharacterIndex} from the ${sideText(trace.globalCharacterSide!, locale)} in the resulting letter string?`;
    case "BANK_SORT_LOCAL_CHAR": {
      const offset = trace.alphabetOffset ?? 0;
      return `Arrange the five groups in ${direction}. Take the group at position ${trace.wordRank} from the ${sideText(trace.wordRankSide!, locale)}, then take character ${trace.characterIndex} from the ${sideText(trace.characterSide!, locale)} of that group.${englishMoveText(offset)} What is the final letter?`;
    }
    case "BANK_TRANSFORM_SORT_POSITION":
      return `${transformation} Arrange the resulting groups in ${direction}. Select the ${trace.answerMode === "ORIGINAL" ? "original group corresponding to" : "transformed group at"} position ${trace.wordRank} from the ${sideText(trace.wordRankSide!, locale)}.`;
    case "BANK_TRANSFORM_SORT_LOCAL_CHAR":
      return `${transformation} Arrange the resulting groups in ${direction}. From the group at position ${trace.wordRank} from the ${sideText(trace.wordRankSide!, locale)}, take character ${trace.characterIndex} from the ${sideText(trace.characterSide!, locale)}. Which letter do you get?`;
  }
}
...
function renderBankingExplanation(trace: WorBankingTrace, answer: string, locale: WorLocale): string {
  ...
  if (trace.taskKind === "BANK_SORT_LOCAL_CHAR") {
    ...
    const base = selected[charIndex]!;
    const offset = trace.alphabetOffset ?? 0;
    if (offset === 0) return `${start} The order is ${ordered}. The selected group is ${selected}; character ${trace.characterIndex} from the ${trace.characterSide!.toLowerCase()} is ${base}. Therefore, the answer is ${answer}.`;
    return `${start} The order is ${ordered}. The selected group is ${selected}; character ${trace.characterIndex} from the ${trace.characterSide!.toLowerCase()} is ${base}.${englishMoveText(offset)} This gives ${answer}.`;
  }
  ...
  const charIndex = indexFromSide(selected.length, trace.characterIndex!, trace.characterSide!);
  const base = selected[charIndex]!;
  return `${start}${transform} The transformed order is ${ordered}. The selected group is ${selected}; character ${trace.characterIndex} from the ${trace.characterSide!.toLowerCase()} is ${base}. Therefore, the answer is ${answer}.`;
}
