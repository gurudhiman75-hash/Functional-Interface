...
for (const caselet of caselets) {
  ...
  for (const time of caselet.labels.times) assert.ok(caselet.questionSetup.includes(time));
  assert.match(caselet.questionSetup, /six chronological slots/u);
  assert.doesNotMatch(caselet.questionSetup, /associated|centre|city|slot setting|\.\./u);
...
console.log("LP-010 proof passed: 100 day-time scheduling caselets, 400 standalone children, unique clue sets, progressive explanations and balanced answer positions.");
