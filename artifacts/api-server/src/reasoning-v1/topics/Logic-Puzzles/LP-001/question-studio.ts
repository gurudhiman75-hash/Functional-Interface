...
import {
  LP_010_MULTILINGUAL_QUESTION_STUDIO_V1,
  normalizeLp010QuestionStudioLanguage,
} from "./lp-010-question-studio-multilingual-v1.ts";
...
export function isLogicPuzzleQuestionStudioRequest(request: LogicPuzzleQuestionStudioRequest) {
  ...
  return packageId === "lp 001" || packageId === "lp 002" || packageId === "lp 003" || packageId === "lp 004" || packageId === "lp 005" || packageId === "lp 006" || packageId === "lp 007" || packageId === "lp 008" || packageId === "lp 009" || packageId === "lp 010" || packageId === "logic puzzles" || cp.startsWith("lp ") || subtopic === "logic puzzles" || (topic === "reasoning" && subtopic === "puzzles");
}
...
function isLp010Request(request: LogicPuzzleQuestionStudioRequest) {
  const packageId = normalize(request.packageId ?? request.archetypeId);
  const selector = normalize(request.cpId ?? request.canonicalProblemId ?? request.patternId);
  return packageId === "lp 010" || selector === "lp cp 010" || selector.startsWith("lp ql 037") || selector.startsWith("lp ql 038") || selector.startsWith("lp ql 039") || selector.startsWith("lp ql 040");
}
...
export function listLogicPuzzleQuestionStudioPackages() {
  const capability = (pkg: typeof LP_001_REVIEW_PACKAGE | typeof LP_002_REVIEW_PACKAGE | typeof LP_003_REVIEW_PACKAGE | typeof LP_004_REVIEW_PACKAGE | typeof LP_005_REVIEW_PACKAGE | typeof LP_006_REVIEW_PACKAGE | typeof LP_007_REVIEW_PACKAGE | typeof LP_008_REVIEW_PACKAGE | typeof LP_009_REVIEW_PACKAGE | typeof LP_010_REVIEW_PACKAGE) => {
    ...
    if (pkg.packageId === "LP-010") {
      return {
        ...base,
        supportedLanguages: [...LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.supportedLanguages],
        releaseFreezeStatus: "LOCALIZATION_FROZEN_V6",
        localizationFreezeStatus: LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.localizationFreezeStatus,
        questionStudioLanguageActivation: LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.questionStudioLanguageActivation,
        permanentQlCount: LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.permanentQlCount,
        permanentQlIds: [...LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.permanentQlIds],
        permanentQlAllocationStatus: LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.permanentQlAllocationStatus,
      };
    }
    return base;
  };
  return [capability(LP_001_REVIEW_PACKAGE), capability(LP_002_REVIEW_PACKAGE), capability(LP_003_REVIEW_PACKAGE), capability(LP_004_REVIEW_PACKAGE), capability(LP_005_REVIEW_PACKAGE), capability(LP_006_REVIEW_PACKAGE), capability(LP_007_REVIEW_PACKAGE), capability(LP_008_REVIEW_PACKAGE), capability(LP_009_REVIEW_PACKAGE), capability(LP_010_REVIEW_PACKAGE)];
}
...
export async function generateLogicPuzzleQuestionStudioBatch(request: LogicPuzzleQuestionStudioRequest = {}) {
  ...
  const lp002 = isLp002Request(request); const lp003 = isLp003Request(request); const lp004 = isLp004Request(request); const lp005 = isLp005Request(request); const lp006 = isLp006Request(request); const lp007 = isLp007Request(request); const lp008 = isLp008Request(request); const lp009 = isLp009Request(request); const lp010 = isLp010Request(request);
  const language = lp010 ? normalizeLp010QuestionStudioLanguage(request.language) : lp009 ? normalizeLp009QuestionStudioLanguage(request.language) : String(request.language ?? "en").trim().toLowerCase();
  if (!lp009 && !lp010 && language !== "en") throw new Error("LP-001 through LP-008 localization is not enabled; English review is the current allowed surface.");
  const seed = String(request.seed || `question-studio:${lp010 ? "LP-010" : lp009 ? "LP-009" : lp008 ? "LP-008" : lp007 ? "LP-007" : lp006 ? "LP-006" : lp005 ? "LP-005" : lp004 ? "LP-004" : lp003 ? "LP-003" : lp002 ? "LP-002" : "LP-001"}`);
  if (lp010) {
    const caselets = language === "en" ? generateLp010Batch(seed, count) : generateLp010LocalizedBatchV6(language, seed, count);
    const questions = caselets.flatMap((caselet, index) => caselet.children.map((child, childIndex) => ({
      ...
      difficulty: child.difficultyBand, difficultyLabel: child.difficultyBand, patternId: child.qlId, section: "Reasoning", topic: "Puzzles", subtopic: "Logic Puzzles", generationBackend: "reasoning-v1", packageId: LP_010_REVIEW_PACKAGE.packageId, canonicalProblemId: child.qlId, questionLanguageId: `${child.qlId}-${language.toUpperCase()}`, questionId: child.questionId,
      language, seed, questionIndex: index * 4 + childIndex, questionCount: caselets.length * 4, runtimeMode: LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.runtimeMode, reviewStatus: "REVIEW_ONLY", questionBankStatus: "NOT_STORED", questionBankWritable: false, testEligibility: "INELIGIBLE", testEligible: false, mockTestEligible: false, publiclyPublishable: false,
      traceability: { checkpointId: LP_010_REVIEW_PACKAGE.checkpointId, qlId: child.qlId, caseletId: caselet.caseletId, sourceStatus: "FROZEN_LOCALIZATION_V6" }, metadata: { packageId: LP_010_REVIEW_PACKAGE.packageId, checkpointId: LP_010_REVIEW_PACKAGE.checkpointId, qlId: child.qlId, caseletId: caselet.caseletId, language, localizationAuthorityId: language === "en" ? "LP_010_ENGLISH_FREEZE_V1" : "LP_010_HI_PA_LOCALIZATION_FREEZE_V6" }, logic: { people: caselet.people, slots: caselet.slots, labels: caselet.labels, clues: caselet.clues, assignment: caselet.assignment },
    })));
    return { generationContext: { generationDomain: "reasoning-v1", packageId: "LP-010", chapterId: "REAS-PUZ", seed, runtimeMode: "REVIEW_ONLY", lifecycleStatus: "REVIEW_ONLY", permanentQlCount: LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.permanentQlCount, permanentQlIds: [...LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.permanentQlIds], permanentQlAllocationStatus: LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.permanentQlAllocationStatus, localizationFreezeStatus: LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.localizationFreezeStatus, questionStudioLanguageActivation: LP_010_MULTILINGUAL_QUESTION_STUDIO_V1.questionStudioLanguageActivation, questionBankStatus: "NOT_STORED", questionBankWritable: false, testEligibility: "INELIGIBLE", testEligible: false, mockTestEligible: false, publiclyPublishable: false, automaticStudentPublication: false, language, checkpointId: LP_010_REVIEW_PACKAGE.checkpointId }, questionPackages: questions, questions };
  }
  if (lp009) {
