...
export function isLogicPuzzleQuestionStudioRequest(request: LogicPuzzleQuestionStudioRequest) {
  ...
  return packageId === "lp 001" || packageId === "lp 002" || packageId === "lp 003" || packageId === "lp 004" || packageId === "lp 005" || packageId === "lp 006" || packageId === "lp 007" || packageId === "lp 008" || packageId === "logic puzzles" || cp.startsWith("lp ") || subtopic === "logic puzzles" || (topic === "reasoning" && subtopic === "puzzles");
}
...
export function listLogicPuzzleQuestionStudioPackages() {
  const capability = (pkg: typeof LP_001_REVIEW_PACKAGE | typeof LP_002_REVIEW_PACKAGE | typeof LP_003_REVIEW_PACKAGE | typeof LP_004_REVIEW_PACKAGE | typeof LP_005_REVIEW_PACKAGE | typeof LP_006_REVIEW_PACKAGE | typeof LP_007_REVIEW_PACKAGE | typeof LP_008_REVIEW_PACKAGE) => ({ id: pkg.packageId, packageId: pkg.packageId, type: "reasoning-v1", section: "Reasoning", domain: "reasoning", subject: "Reasoning", topic: "Puzzles", subtopic: "Logic Puzzles", name: pkg.label, label: pkg.label, generationDomain: "reasoning-v1", cpIds: [pkg.checkpointId], canonicalProblems: pkg.qlIds.map((id) => ({ id, label: id, checkpointId: pkg.checkpointId })), patternIds: [...pkg.qlIds], supportedDifficulties: [...pkg.supportedDifficulties], supportedLanguages: [...pkg.supportedLanguages], enabled: true, runtimeMode: pkg.runtimeMode, supportedRuntimeModes: [pkg.runtimeMode], reviewStatus: "REVIEW_ONLY", releaseFreezeStatus: "NOT_FROZEN", reviewOnly: true, permanentQlCount: 0, permanentQlIds: [], permanentQlAllocationStatus: "UNALLOCATED", questionBankStatus: "NOT_STORED", questionBankWritable: false, testEligibility: "INELIGIBLE", testEligible: false, mockTestEligible: false, publiclyPublishable: false, automaticStudentPublication: false });
  return [capability(LP_001_REVIEW_PACKAGE), capability(LP_002_REVIEW_PACKAGE), capability(LP_003_REVIEW_PACKAGE), capability(LP_004_REVIEW_PACKAGE), capability(LP_005_REVIEW_PACKAGE), capability(LP_006_REVIEW_PACKAGE), capability(LP_007_REVIEW_PACKAGE), capability(LP_008_REVIEW_PACKAGE)];
}

export async function generateLogicPuzzleQuestionStudioBatch(request: LogicPuzzleQuestionStudioRequest = {}) {
  if (normalize(request.language || "en") !== "en") throw new Error("LP-001 through LP-008 localization is not started; English review is the current allowed surface.");
  const count = Math.min(12, Math.max(1, Math.floor(Number(request.count ?? 1) || 1)));
  const lp002 = isLp002Request(request); const lp003 = isLp003Request(request); const lp004 = isLp004Request(request); const lp005 = isLp005Request(request); const lp006 = isLp006Request(request); const lp007 = isLp007Request(request); const lp008 = isLp008Request(request); const seed = String(request.seed || `question-studio:${lp008 ? "LP-008" : lp007 ? "LP-007" : lp006 ? "LP-006" : lp005 ? "LP-005" : lp004 ? "LP-004" : lp003 ? "LP-003" : lp002 ? "LP-002" : "LP-001"}`);
  if (lp008) {
