...
export async function generateLogicPuzzleQuestionStudioBatch(request: LogicPuzzleQuestionStudioRequest = {}) {
  if (normalize(request.language || "en") !== "en") throw new Error("LP-001 through LP-009 localization is not started; English review is the current allowed surface.");
  const count = Math.min(12, Math.max(1, Math.floor(Number(request.count ?? 1) || 1)));
  const lp002 = isLp002Request(request); const lp003 = isLp003Request(request); const lp004 = isLp004Request(request); const lp005 = isLp005Request(request); const lp006 = isLp006Request(request); const lp007 = isLp007Request(request); const lp008 = isLp008Request(request); const lp009 = isLp009Request(request); const seed = String(request.seed || `question-studio:${lp009 ? "LP-009" : lp008 ? "LP-008" : lp007 ? "LP-007" : lp006 ? "LP-006" : lp005 ? "LP-005" : lp004 ? "LP-004" : lp003 ? "LP-003" : lp002 ? "LP-002" : "LP-001"}`);
  if (lp009) {
    const caselets = generateLp009Batch(seed, count);
    const questions = caselets.flatMap((caselet, index) => caselet.children.map((child, childIndex) => ({
      ...
      language: "en", seed, questionIndex: index * 4 + childIndex, questionCount: caselets.length * 4, runtimeMode: LP_009_REVIEW_PACKAGE.runtimeMode, reviewStatus: "REVIEW_ONLY", questionBankStatus: "NOT_STORED", questionBankWritable: false, testEligibility: "INELIGIBLE", testEligible: false, mockTestEligible: false, publiclyPublishable: false,
      traceability: { checkpointId: LP_009_REVIEW_PACKAGE.checkpointId, qlId: child.qlId, caseletId: caselet.caseletId, sourceStatus: "SOURCE_SATURATION_REVIEW_REQUIRED" }, metadata: { packageId: LP_009_REVIEW_PACKAGE.packageId, checkpointId: LP_009_REVIEW_PACKAGE.checkpointId, qlId: child.qlId, caseletId: caselet.caseletId, language: "en" }, logic: { mode: caselet.mode, people: caselet.people, values: caselet.values, labels: caselet.labels, clues: caselet.clues, assignment: caselet.assignment },
    })));
    return { generationContext: { generationDomain: "reasoning-v1", packageId: "LP-009", chapterId: "REAS-PUZ", seed, runtimeMode: "REVIEW_ONLY", lifecycleStatus: "REVIEW_ONLY", permanentQlCount: 0, permanentQlIds: [], permanentQlAllocationStatus: "UNALLOCATED", questionBankStatus: "NOT_STORED", questionBankWritable: false, testEligibility: "INELIGIBLE", testEligible: false, publiclyPublishable: false, automaticStudentPublication: false, language: "en", checkpointId: LP_009_REVIEW_PACKAGE.checkpointId }, questionPackages: questions, questions };
  }
  if (lp008) {
