...
const lines: string[] = [
  ...
  "",
  "The same seed is used for both languages so assignment, QL, difficulty, correct option index and 2/4/5/6-time layout semantics remain identical. V6 adds unique localized names, native day-time grammar, readable six-slot domains and profile-specific exam wording.",
  "",
];

for (const language of ["hi", "pa"] as const) {
  const label = language === "hi" ? "Hindi" : "Punjabi";
  const caselets = generateLp010LocalizedBatchV6(language, "lp-010-hi-pa-human-review-v6", 6);
  lines.push(`# ${label}`, "");
...
writeFileSync(resolve(outputDirectory, "LP-010-HI-PA-LOCALIZATION-REVIEW-V6.md"), `${lines.join("\n")}\n`, "utf8");
console.log(`Wrote LP-010 Hindi/Punjabi V6 review pack to ${outputDirectory}/LP-010-HI-PA-LOCALIZATION-REVIEW-V6.md`);
