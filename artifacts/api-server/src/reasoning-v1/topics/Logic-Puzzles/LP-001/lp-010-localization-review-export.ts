...
import { generateLp010LocalizedBatchV3 } from "./lp-010-localization-v3.ts";
...
const lines: string[] = [
  ...
  "",
  "The same seed is used for both languages so assignment, QL, difficulty, correct option index and 2/4/5/6-time layout semantics remain identical. V3 also enforces unique localized display names and native day-time grammar.",
  "",
];

for (const language of ["hi", "pa"] as const) {
  const label = language === "hi" ? "Hindi" : "Punjabi";
  const caselets = generateLp010LocalizedBatchV3(language, "lp-010-hi-pa-human-review-v3", 6);
  lines.push(`# ${label}`, "");
...
writeFileSync(resolve(outputDirectory, "LP-010-HI-PA-LOCALIZATION-REVIEW-V3.md"), `${lines.join("\n")}\n`, "utf8");
console.log(`Wrote LP-010 Hindi/Punjabi V3 review pack to ${outputDirectory}/LP-010-HI-PA-LOCALIZATION-REVIEW-V3.md`);
