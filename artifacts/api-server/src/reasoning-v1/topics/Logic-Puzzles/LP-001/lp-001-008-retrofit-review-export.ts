...
const packages: readonly [string, Generator][] = [
  ["LP-001", generateLp001BatchStabilizedV4 as Generator],
  ["LP-002", generateLp002BatchStabilizedV4 as Generator],
  ["LP-003", generateLp003BatchStabilizedV4 as Generator],
  ["LP-004", generateLp004BatchStabilizedV4 as Generator],
  ["LP-005", generateLp005BatchStabilizedV4 as Generator],
  ["LP-006", generateLp006BatchStabilizedV4 as Generator],
  ["LP-007", generateLp007BatchStabilizedV4 as Generator],
  ["LP-008", generateLp008BatchStabilizedV4 as Generator],
];
...
const lines: string[] = [
  "# Logic Puzzles LP-001 → LP-008 — Stabilized English Review V4",
  "",
  "Status: **human review candidate**",
  "",
  "V4 fixes two review issues. First, the setup now explicitly names every domain used by the puzzle: all people/items and all groups, days, locations, positions, duties, study areas, cities, values, months/dates, etc. Second, the explanation no longer follows the displayed clue order mechanically.",
  "",
  "The explanation planner chooses the strongest anchor first, then prefers connected clues that immediately narrow or fill the same working table. If a clue does not visibly change the table by itself, it is kept with the next connected clue instead of producing a useless unchanged-table step.",
  "",
  "Working cells show a fixed value when forced, a narrowed candidate set when useful, and `?` only while the full domain is still open. When exactly two complete arrangements remain, both are shown as Case 1 / Case 2 tables and the discriminating clue removes the invalid case.",
  "",
  "Displayed clue order in the question is unchanged. Puzzle assignments, QLs, difficulty, option-integrity repairs, answers and correct-option positions remain unchanged from V2.",
  "",
];

for (const [packageId, generate] of packages) {
  // Keep the same underlying samples as V3.3 so review differences are editorial, not seed changes.
