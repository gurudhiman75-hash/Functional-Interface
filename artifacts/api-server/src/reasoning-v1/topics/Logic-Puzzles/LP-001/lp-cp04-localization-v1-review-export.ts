...
const seed = process.argv[2] || "lp-cp04-localization-v1-review";
const perLanguage = Math.min(12, Math.max(3, Number(process.argv[3] || 9)));
const outputPath = process.argv[4] || "lp-cp04-localization-v1-review.md";

const lines: string[] = [
  "# LP-QL-047 — Hindi / Punjabi Localization Review V1",
  "",
  `Authority: ${LP_CP04_HI_PA_LOCALIZATION_REVIEW_V1.authorityId}`,
  "Status: HUMAN REVIEW CANDIDATE — REVIEW ONLY",
...
for (const language of ["hi", "pa"] as const) {
  ...
  lines.push(`# ${label}`);
  lines.push("");
  const caselets = generateLpCp04LocalizedBatchV1(language, `${seed}:${language}`, perLanguage);
