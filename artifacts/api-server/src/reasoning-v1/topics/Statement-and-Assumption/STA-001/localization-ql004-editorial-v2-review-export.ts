...
const OUTPUT_DIR = process.env.STA_QL004_LOCALIZATION_V2_REVIEW_OUTPUT_DIR ?? "/tmp/sta-001-ql004-hi-pa-review-v2";
const EXAMPLES_PER_AUTHORITY = Number(process.env.STA_QL004_LOCALIZATION_V2_EXAMPLES_PER_AUTHORITY ?? 2);
...
function collect(locale: StaLocalizedLocale): StaQl004LocalizedQuestionV2[] {
  const expectedIds = STA_ENGLISH_CORPUS_BY_QL["STA-QL-004"].map((scenario) => scenario.scenarioId);
  const baseByScenario = new Map<string, StaQl004LocalizedQuestionV2>();
  for (let index = 0; index < MAX_SEARCH && baseByScenario.size < expectedIds.length; index += 1) {
    const question = generateStaQl004LocalizedQuestionV2(`sta-ql004-native-review-v2:${locale}:${index}`, locale);
  ...
  return expectedIds.flatMap((scenarioId) => {
    ...
    if (!base) throw new Error(`${locale}:${scenarioId}: no deterministic QL004 V2 review seed found`);
    ...
    return copy.statementVariants.slice(0, EXAMPLES_PER_AUTHORITY).map((statement) => ({
      ...base,
      statement: editorializeStaQl004LocalizedText(locale, statement),
    }));
  });
}
...
function questionHtml(question: StaQl004LocalizedQuestionV2, index: number): string {
...
function page(title: string, rows: readonly StaQl004LocalizedQuestionV2[]): string {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>body{font-family:system-ui,-apple-system,"Noto Sans Devanagari","Noto Sans Gurmukhi",sans-serif;max-width:980px;margin:0 auto;padding:24px;line-height:1.55;background:#fff;color:#111}article{border:1px solid #ddd;border-radius:10px;padding:18px;margin:18px 0}.meta{font-size:.8rem;color:#666}.stem{font-weight:650}.assumptions,.options{padding-left:28px}.correct{font-weight:750}details{margin-top:12px}.explanation{margin-top:10px}.lock{background:#f4f4f4;padding:10px;border-radius:8px}</style></head><body><h1>${escapeHtml(title)}</h1><p class="lock">QL004 Hindi/Punjabi V2 native-editorial review candidate. QL001, QL002 and QL003 are FROZEN_V2. Multilingual chapter freeze and all downstream product locks remain false.</p>${rows.map(questionHtml).join("\n")}</body></html>`;
}

mkdirSync(OUTPUT_DIR, { recursive: true });
const hindi = collect("hi-IN");
const punjabi = collect("pa-IN");
const combined = { editorialVersion: STA_QL004_LOCALIZATION_EDITORIAL_VERSION, lifecycle: hindi[0]!.lifecycle, hindi, punjabi };

writeFileSync(join(OUTPUT_DIR, "STA-QL004-HINDI-REVIEW-V2.json"), `${JSON.stringify(hindi, null, 2)}\n`, "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL004-PUNJABI-REVIEW-V2.json"), `${JSON.stringify(punjabi, null, 2)}\n`, "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL004-HINDI-REVIEW-V2.html"), page("STA-QL-004 Hindi Native Editorial Review V2", hindi), "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL004-PUNJABI-REVIEW-V2.html"), page("STA-QL-004 Punjabi Native Editorial Review V2", punjabi), "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL004-HI-PA-REVIEW-V2.json"), `${JSON.stringify(combined, null, 2)}\n`, "utf8");
