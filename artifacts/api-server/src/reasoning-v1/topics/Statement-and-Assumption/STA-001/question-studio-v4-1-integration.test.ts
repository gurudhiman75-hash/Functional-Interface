...
const ALL_QLS = ["STA-QL-001", "STA-QL-002", "STA-QL-003", "STA-QL-004", "STA-QL-005", "STA-QL-006"] as const;

assert.equal(STA_001_QUESTION_STUDIO_REVIEW_PACKAGE.permanentQlCount, 6);
assert.deepEqual(STA_001_QUESTION_STUDIO_REVIEW_PACKAGE.permanentQlIds, ALL_QLS);
assert.equal(STA_001_QUESTION_STUDIO_REVIEW_PACKAGE.candidateQlCount, 6);
assert.deepEqual(STA_001_QUESTION_STUDIO_REVIEW_PACKAGE.candidateQlIds, ALL_QLS);
assert.deepEqual(STA_001_HISTORICAL_PERMANENT_QL_IDS, ["STA-QL-001", "STA-QL-002", "STA-QL-003", "STA-QL-004"]);
assert.equal(STA_001_QUESTION_STUDIO_REVIEW_PACKAGE.permanentQlAllocationStatus, "V4_1_SIX_QL_FROZEN");
assert.equal(STA_001_QUESTION_STUDIO_REVIEW_PACKAGE.candidateQlAllocationStatus, "V4_1_FROZEN_AUTHORITY");
assert.ok(STA_001_QUESTION_STUDIO_REVIEW_PACKAGE.qls.every((entry) => entry.status === "V4_1_FROZEN"));
...
function assertLockedPayload(payload: Record<string, any>): void {
  ...
  assert.equal(payload.multilingualChapterFrozen, true);
  assert.equal(payload.permanentQlAllocationStatus, "V4_1_SIX_QL_FROZEN");
  assert.equal(payload.candidateQlAllocationStatus, "V4_1_FROZEN_AUTHORITY");
  assert.equal(payload.humanContentReviewStatus, "V4_1_APPROVED_FROZEN");
  assert.equal(payload.nativeHumanSignoffStatus, "V4_1_APPROVED_FROZEN");
...
for (const language of ["en", "hi", "pa"] as const) {
  ...
  assert.deepEqual(generated.generationContext.permanentQlIds, ALL_QLS);
  assert.equal(generated.generationContext.candidateQlCount, 6);
  assert.deepEqual(generated.generationContext.candidateQlIds, ALL_QLS);
...
}), /review-only|delivery remains locked/u);

console.log("PASS_STA_001_QUESTION_STUDIO_V4_1_FROZEN_INTEGRATION");
