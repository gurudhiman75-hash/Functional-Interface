...
for (const locale of ["hi-IN", "pa-IN"] as const) {
  ...
  for (const sourceScenario of expectedScenarios) {
    ...
    for (const statement of copy.statementVariants) {
      ...
      for (const opening of BLOCKED_OPENINGS[locale]) {
        assert.equal(statement.trimStart().startsWith(opening), false, `${locale}/${sourceScenario.scenarioId}: unnecessary opening label returned: ${opening}`);
      }
    ...
    for (const sourceCandidate of sourceScenario.candidates) {
      ...
      for (const value of [...localized.textVariants, localized.rationale]) {
        for (const opening of BLOCKED_OPENINGS[locale]) {
          assert.equal(value.trimStart().startsWith(opening), false, `${locale}/${sourceScenario.scenarioId}/${sourceCandidate.candidateId}: unnecessary opening label in authored copy`);
        }
        authoredCopyChecks += 1;
      }
    }
...
for (const locale of ["hi-IN", "pa-IN"] as const) {
  for (let index = 0; index < CASES_PER_LOCALE; index += 1) {
    ...
    for (const opening of BLOCKED_OPENINGS[locale]) {
      assert.equal(question.statement.trimStart().startsWith(opening), false, `${locale}/${seed}: unnecessary stem opening returned`);
    }
    ...
    assert.deepEqual(
      question.options.map((option) => ({ semanticAnswerSet: option.semanticAnswerSet, isCorrect: option.isCorrect })),
      question.options.map((option) => ({ semanticAnswerSet: option.semanticAnswerSet, isCorrect: option.isCorrect })),
    );
...
console.log(JSON.stringify({
  ...
  frozenEnglishQl004Authorities: expectedScenarios.length,
  authoredCopyChecks,
  localizedQuestions,
