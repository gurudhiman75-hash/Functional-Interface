...
export interface StaQl004LocalizedLifecycle {
  ...
  readonly ql002HindiPunjabiStatus: "FROZEN_V2";
  readonly ql002FreezeId: "STA-001-QL002-HI-PA-v2-frozen";
  readonly ql003HindiPunjabiStatus: "FROZEN_V2";
  readonly ql003FreezeId: "STA-001-QL003-HI-PA-v2-frozen";
  readonly ql004HindiPunjabiStatus: "REVIEW_CANDIDATE_V1";
  readonly localizedQlIds: readonly ["STA-QL-001", "STA-QL-002", "STA-QL-003", "STA-QL-004"];
...
export interface StaQl004LocalizedQuestion extends Omit<StaQuestion, "locale" | "statement" | "candidates" | "options" | "explanation" | "lifecycle"> {
  ...
  readonly lifecycle: StaQl004LocalizedLifecycle;
}

export const STA_QL004_LOCALIZATION_LIFECYCLE: StaQl004LocalizedLifecycle = {
  ...
  ql003FreezeId: STA_QL003_HI_PA_FREEZE_V2_MANIFEST.freezeId,
  ql004HindiPunjabiStatus: "REVIEW_CANDIDATE_V1",
  localizedQlIds: ["STA-QL-001", "STA-QL-002", "STA-QL-003", "STA-QL-004"],
...
function localizedExplanation(
  ...
  candidates: StaQl004LocalizedQuestion["candidates"],
...
export function generateStaQl004LocalizedQuestion(seed: string, locale: StaLocalizedLocale): StaQl004LocalizedQuestion {
  const english = generateStaQuestionFromPool(
    seed,
    "STA-QL-004",
    STA_ENGLISH_CORPUS_BY_QL as unknown as StaScenarioPoolByQl,
  );
  const sourceScenario = STA_ENGLISH_CORPUS_BY_QL["STA-QL-004"].find((scenario) => scenario.scenarioId === english.scenarioId);
  if (!sourceScenario) throw new Error(`${english.scenarioId}: missing frozen English QL004 scenario`);

  const copy = bundleFor(locale)[english.scenarioId];
  if (!copy) throw new Error(`${english.scenarioId}: missing ${locale} QL004 localization copy`);
  ...
  }) as unknown as StaQl004LocalizedQuestion["candidates"];
  ...
  return {
    ...
    lifecycle: STA_QL004_LOCALIZATION_LIFECYCLE,
  };
}
