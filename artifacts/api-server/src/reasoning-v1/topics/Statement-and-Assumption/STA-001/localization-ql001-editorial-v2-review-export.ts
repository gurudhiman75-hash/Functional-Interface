...
const OUTPUT_DIR = process.env.STA_QL001_LOCALIZATION_V2_REVIEW_OUTPUT_DIR ?? "/tmp/sta-001-ql001-hi-pa-review-v2";
const EXAMPLES_PER_AUTHORITY = Number(process.env.STA_QL001_LOCALIZATION_V2_EXAMPLES_PER_AUTHORITY ?? 2);
...
function collect(locale: StaLocalizedLocale): StaLocalizedQuestion[] {
  ...
  return expectedIds.flatMap((scenarioId) => {
    ...
    if (!base) throw new Error(`${locale}:${scenarioId}: no deterministic V2 review seed found`);
...
function page(title: string, rows: readonly StaLocalizedQuestion[]): string {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>body{font-family:system-ui,-apple-system,"Noto Sans Devanagari","Noto Sans Gurmukhi",sans-serif;max-width:980px;margin:0 auto;padding:24px;line-height:1.55;background:#fff;color:#111}article{border:1px solid #ddd;border-radius:10px;padding:18px;margin:18px 0}.meta{font-size:.8rem;color:#666}.stem{font-weight:650}.assumptions,.options{padding-left:28px}.correct{font-weight:750}details{margin-top:12px}.explanation{margin-top:10px}.lock{background:#f4f4f4;padding:10px;border-radius:8px}</style></head><body><h1>${escapeHtml(title)}</h1><p class="lock">${STA_QL001_LOCALIZATION_EDITORIAL_VERSION}. Native-review candidate only; English V2 and all downstream locks remain unchanged.</p>${rows.map(questionHtml).join("\n")}</body></html>`;
}

mkdirSync(OUTPUT_DIR, { recursive: true });
const hindi = collect("hi-IN");
const punjabi = collect("pa-IN");
const combined = { editorialVersion: STA_QL001_LOCALIZATION_EDITORIAL_VERSION, lifecycle: hindi[0]!.lifecycle, hindi, punjabi };

writeFileSync(join(OUTPUT_DIR, "STA-QL001-HINDI-REVIEW-V2.json"), `${JSON.stringify(hindi, null, 2)}\n`, "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL001-PUNJABI-REVIEW-V2.json"), `${JSON.stringify(punjabi, null, 2)}\n`, "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL001-HINDI-REVIEW-V2.html"), page("STA-QL-001 Hindi Native Editorial V2", hindi), "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL001-PUNJABI-REVIEW-V2.html"), page("STA-QL-001 Punjabi Native Editorial V2", punjabi), "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL001-HI-PA-REVIEW-V2.json"), `${JSON.stringify(combined, null, 2)}\n`, "utf8");
