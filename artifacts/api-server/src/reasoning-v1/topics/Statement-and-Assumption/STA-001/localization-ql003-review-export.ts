...
import { generateStaQl003LocalizedQuestion, type StaQl003LocalizedQuestion } from "./localization-ql003.ts";
import type { StaLocalizedLocale, StaLocalizationBundle } from "./localization-types.ts";

const OUTPUT_DIR = process.env.STA_QL003_LOCALIZATION_REVIEW_OUTPUT_DIR ?? "/tmp/sta-001-ql003-hi-pa-review-v1";
const EXAMPLES_PER_AUTHORITY = Number(process.env.STA_QL003_LOCALIZATION_EXAMPLES_PER_AUTHORITY ?? 2);
...
function collect(locale: StaLocalizedLocale): StaQl003LocalizedQuestion[] {
  const expectedIds = STA_ENGLISH_CORPUS_BY_QL["STA-QL-003"].map((scenario) => scenario.scenarioId);
  const baseByScenario = new Map<string, StaQl003LocalizedQuestion>();

  for (let index = 0; index < MAX_SEARCH && baseByScenario.size < expectedIds.length; index += 1) {
    const question = generateStaQl003LocalizedQuestion(`sta-ql003-native-review-v1:${locale}:${index}`, locale);
  ...
  return expectedIds.flatMap((scenarioId) => {
    ...
    if (!base) throw new Error(`${locale}:${scenarioId}: no deterministic QL003 review seed found`);
    const copy = bundle[scenarioId];
    if (!copy) throw new Error(`${locale}:${scenarioId}: missing QL003 localization copy`);
...
function questionHtml(question: StaQl003LocalizedQuestion, index: number): string {
...
function page(title: string, rows: readonly StaQl003LocalizedQuestion[]): string {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>body{font-family:system-ui,-apple-system,"Noto Sans Devanagari","Noto Sans Gurmukhi",sans-serif;max-width:980px;margin:0 auto;padding:24px;line-height:1.55;background:#fff;color:#111}article{border:1px solid #ddd;border-radius:10px;padding:18px;margin:18px 0}.meta{font-size:.8rem;color:#666}.stem{font-weight:650}.assumptions,.options{padding-left:28px}.correct{font-weight:750}details{margin-top:12px}.explanation{margin-top:10px}.lock{background:#f4f4f4;padding:10px;border-radius:8px}</style></head><body><h1>${escapeHtml(title)}</h1><p class="lock">QL003 Hindi/Punjabi Review Candidate V1. QL001 and QL002 Hindi/Punjabi remain FROZEN_V2; English V2 and all downstream product locks remain unchanged.</p>${rows.map(questionHtml).join("\n")}</body></html>`;
}

mkdirSync(OUTPUT_DIR, { recursive: true });
const hindi = collect("hi-IN");
const punjabi = collect("pa-IN");
const combined = { version: "QL003_REVIEW_CANDIDATE_V1", lifecycle: hindi[0]!.lifecycle, hindi, punjabi };

writeFileSync(join(OUTPUT_DIR, "STA-QL003-HINDI-REVIEW-V1.json"), `${JSON.stringify(hindi, null, 2)}\n`, "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL003-PUNJABI-REVIEW-V1.json"), `${JSON.stringify(punjabi, null, 2)}\n`, "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL003-HINDI-REVIEW-V1.html"), page("STA-QL-003 Hindi Review Candidate V1", hindi), "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL003-PUNJABI-REVIEW-V1.html"), page("STA-QL-003 Punjabi Review Candidate V1", punjabi), "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL003-HI-PA-REVIEW-V1.json"), `${JSON.stringify(combined, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  outputDir: OUTPUT_DIR,
  authoritiesPerLanguage: STA_ENGLISH_CORPUS_BY_QL["STA-QL-003"].length,
