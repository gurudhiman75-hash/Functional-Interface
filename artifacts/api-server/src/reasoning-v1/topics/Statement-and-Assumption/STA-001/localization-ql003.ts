...
export interface StaQl003LocalizedLifecycle {
  ...
  readonly ql003HindiPunjabiStatus: "REVIEW_CANDIDATE_V1";
  readonly localizedQlIds: readonly ["STA-QL-001", "STA-QL-002", "STA-QL-003"];
...
export interface StaQl003LocalizedQuestion extends Omit<StaQuestion, "locale" | "statement" | "candidates" | "options" | "explanation" | "lifecycle"> {
  ...
  readonly lifecycle: StaQl003LocalizedLifecycle;
}

export const STA_QL003_LOCALIZATION_LIFECYCLE: StaQl003LocalizedLifecycle = {
  ...
  localizedQlIds: ["STA-QL-001", "STA-QL-002", "STA-QL-003"],
...
function localizedExplanation(
  ...
  candidates: StaQl003LocalizedQuestion["candidates"],
...
export function generateStaQl003LocalizedQuestion(seed: string, locale: StaLocalizedLocale): StaQl003LocalizedQuestion {
  const english = generateStaQuestionFromPool(
    seed,
    "STA-QL-003",
    STA_ENGLISH_CORPUS_BY_QL as unknown as StaScenarioPoolByQl,
  );
  const sourceScenario = STA_ENGLISH_CORPUS_BY_QL["STA-QL-003"].find((scenario) => scenario.scenarioId === english.scenarioId);
  if (!sourceScenario) throw new Error(`${english.scenarioId}: missing frozen English QL003 scenario`);

  const copy = bundleFor(locale)[english.scenarioId];
  if (!copy) throw new Error(`${english.scenarioId}: missing ${locale} QL003 localization copy`);
  ...
  }) as unknown as StaQl003LocalizedQuestion["candidates"];
  ...
  return {
    ...
    lifecycle: STA_QL003_LOCALIZATION_LIFECYCLE,
  };
}
