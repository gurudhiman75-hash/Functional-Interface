...
const examplesPerAuthority = Number(process.env.STA_ENGLISH_REVIEW_EXAMPLES_PER_AUTHORITY ?? 2);
const qlIds: readonly StaQlId[] = ["STA-QL-001", "STA-QL-002", "STA-QL-003", "STA-QL-004"];
mkdirSync(outputDir, { recursive: true });

const questions = STA_ENGLISH_CORPUS_V2.flatMap((authority) =>
  Array.from({ length: examplesPerAuthority }, (_, index) => {
    const singleAuthorityPool = {
      ...STA_ENGLISH_CORPUS_BY_QL,
      [authority.proposedQlId]: [authority],
    };
    return generateStaQuestionFromPool(
      `STA-EN-V2-REVIEW-${authority.scenarioId}-${String(index).padStart(2, "0")}`,
      authority.proposedQlId,
      singleAuthorityPool,
    );
  }),
);
...
const exportRows = questions.map((question) => {
  const authority = STA_ENGLISH_CORPUS_V2.find((scenario) => scenario.scenarioId === question.scenarioId);
...
writeFileSync(join(outputDir, "sta-001-english-corpus-v2-review.json"), JSON.stringify(exportRows, null, 2), "utf8");
writeFileSync(join(outputDir, "sta-001-english-corpus-v2-coverage.json"), JSON.stringify(getStaEnglishCorpusCoverage(), null, 2), "utf8");
...
const cards = exportRows.map((question) => {
  ...
  return `<article data-ql="${question.qlId}" data-domain="${question.domain}" data-scenario="${esc(question.scenarioId)}">
    <h2>${question.qlId} · ${esc(question.domain)} · ${esc(question.corpusFamilyId)}</h2>
    <p class="meta">${esc(question.sourceProfile)} · ${esc(question.difficulty)} · ${esc(question.discourseAct)} · ${question.candidates.length} assumptions · ${esc(question.semanticShape)} · ${esc(question.scenarioId)}</p>
...
<title>STA-001 English Corpus V2 Review</title><style>
...
</style></head><body><h1>STA-001 — English Corpus V2 Freeze-Candidate Review</h1>
<div class="summary"><strong>Corpus status:</strong> V2_FREEZE_CANDIDATE_NOT_FROZEN<br>
...
<strong>Review coverage:</strong> ${examplesPerAuthority} generated example(s) for every semantic authority · ${exportRows.length} total questions<br>
Question Studio remains closed.</div>${cards}</body></html>`;

writeFileSync(join(outputDir, "sta-001-english-corpus-v2-review.html"), html, "utf8");
console.log(`WROTE_STA_001_ENGLISH_CORPUS_V2_REVIEW ${exportRows.length} questions / ${STA_ENGLISH_CORPUS_V2.length} authorities -> ${outputDir}`);
