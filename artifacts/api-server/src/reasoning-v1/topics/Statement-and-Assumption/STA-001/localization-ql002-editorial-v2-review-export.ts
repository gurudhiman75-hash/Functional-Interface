...
const OUTPUT_DIR = process.env.STA_QL002_LOCALIZATION_V2_REVIEW_OUTPUT_DIR ?? "/tmp/sta-001-ql002-hi-pa-review-v2";
const EXAMPLES_PER_AUTHORITY = Number(process.env.STA_QL002_LOCALIZATION_V2_EXAMPLES_PER_AUTHORITY ?? 2);
...
function collect(locale: StaLocalizedLocale): StaQl002LocalizedQuestionV2[] {
  ...
  return expectedIds.flatMap((scenarioId) => {
    ...
    if (!base) throw new Error(`${locale}:${scenarioId}: no deterministic QL002 V2 review seed found`);
...
function questionHtml(question: StaQl002LocalizedQuestionV2, index: number): string {
...
writeFileSync(join(OUTPUT_DIR, "STA-QL002-HINDI-REVIEW-V2.json"), `${JSON.stringify(hindi, null, 2)}\n`, "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL002-PUNJABI-REVIEW-V2.json"), `${JSON.stringify(punjabi, null, 2)}\n`, "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL002-HINDI-REVIEW-V2.html"), page("STA-QL-002 Hindi Native Editorial V2", hindi), "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL002-PUNJABI-REVIEW-V2.html"), page("STA-QL-002 Punjabi Native Editorial V2", punjabi), "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL002-HI-PA-REVIEW-V2.json"), `${JSON.stringify(combined, null, 2)}\n`, "utf8");
