...
import { generateStaQl004LocalizedQuestion, type StaQl004LocalizedQuestion } from "./localization-ql004.ts";
import type { StaLocalizedLocale, StaLocalizationBundle } from "./localization-types.ts";

const OUTPUT_DIR = process.env.STA_QL004_LOCALIZATION_REVIEW_OUTPUT_DIR ?? "/tmp/sta-001-ql004-hi-pa-review-v1";
const EXAMPLES_PER_AUTHORITY = Number(process.env.STA_QL004_LOCALIZATION_EXAMPLES_PER_AUTHORITY ?? 2);
...
function collect(locale: StaLocalizedLocale): StaQl004LocalizedQuestion[] {
  const expectedIds = STA_ENGLISH_CORPUS_BY_QL["STA-QL-004"].map((scenario) => scenario.scenarioId);
  const baseByScenario = new Map<string, StaQl004LocalizedQuestion>();

  for (let index = 0; index < MAX_SEARCH && baseByScenario.size < expectedIds.length; index += 1) {
    const question = generateStaQl004LocalizedQuestion(`sta-ql004-native-review-v1:${locale}:${index}`, locale);
  ...
  return expectedIds.flatMap((scenarioId) => {
    ...
    if (!base) throw new Error(`${locale}:${scenarioId}: no deterministic QL004 review seed found`);
...
function questionHtml(question: StaQl004LocalizedQuestion, index: number): string {
...
writeFileSync(join(OUTPUT_DIR, "STA-QL004-HINDI-REVIEW-V1.json"), `${JSON.stringify(hindi, null, 2)}\n`, "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL004-PUNJABI-REVIEW-V1.json"), `${JSON.stringify(punjabi, null, 2)}\n`, "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL004-HINDI-REVIEW-V1.html"), page("STA-QL-004 Hindi Review Candidate V1", hindi), "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL004-PUNJABI-REVIEW-V1.html"), page("STA-QL-004 Punjabi Review Candidate V1", punjabi), "utf8");
writeFileSync(join(OUTPUT_DIR, "STA-QL004-HI-PA-REVIEW-V1.json"), `${JSON.stringify(combined, null, 2)}\n`, "utf8");
...
console.log(JSON.stringify({
  ...
  ql004HindiPunjabiStatus: "REVIEW_CANDIDATE_V1",
