...
for (const [relativePath, expectedSha] of Object.entries(STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.sourceBlobLocks)) {
  assert.equal(gitBlobSha(relativePath), expectedSha, `${relativePath}: QL004 V3 review-locked learner source changed`);
}
...
function collect(locale: StaLocalizedLocale): StaQl004LocalizedQuestionV3[] {
  ...
  return expectedIds.flatMap((scenarioId) => {
    ...
    assert.ok(base, `${locale}:${scenarioId}: canonical review seed unavailable`);
    const copy = bundle[scenarioId];
    assert.ok(copy, `${locale}:${scenarioId}: QL004 localization copy unavailable`);
    assert.ok(copy.statementVariants.length >= 2, `${locale}:${scenarioId}: two reviewed stem variants required`);
...
assert.equal(STA_QL004_EXAM_REALNESS_EDITORIAL_VERSION, STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.editorialVersion);
assert.equal(hindi.length, STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.canonicalQuestionsPerLocale);
assert.equal(punjabi.length, STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.canonicalQuestionsPerLocale);
assert.equal(new Set(hindi.map((question) => question.scenarioId)).size, STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.authorityCountPerLocale);
assert.equal(new Set(punjabi.map((question) => question.scenarioId)).size, STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.authorityCountPerLocale);
assert.equal(new Set(hindi.map((question) => question.statement)).size, STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.canonicalQuestionsPerLocale);
assert.equal(new Set(punjabi.map((question) => question.statement)).size, STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.canonicalQuestionsPerLocale);
assert.equal(hindi.length + punjabi.length, STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.canonicalCombinedQuestionCount);
...
assert.equal(
  ...
  "QL004 Hindi/Punjabi learner content changed from the certified V3 review artifact",
);

assert.equal(STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.review.exactArtifactInspected, true);
...
assert.equal(STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.review.exactCandidateApproved, false);
assert.equal(STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.lifecycle.ql004HindiPunjabiStatus, "REVIEW_LOCKED_V3");
assert.equal(STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.lifecycle.multilingualChapterFrozen, false);
assert.equal(STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.lifecycle.questionStudioDiscoverable, false);
assert.equal(STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.lifecycle.questionBankWritable, false);
assert.equal(STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.lifecycle.testEligible, false);
assert.equal(STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.lifecycle.publiclyPublishable, false);

console.log("PASS_STA_QL004_HI_PA_REVIEW_LOCK_V3");
console.log(`review lock ${STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.reviewLockId}`);
console.log(`certified source head ${STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.certifiedSourceHead}`);
console.log(`review run ${STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.sourceReviewWorkflowRunId}`);
console.log(`review artifact ${STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.sourceReviewArtifactId}`);
console.log(`locked localization blobs ${Object.keys(STA_QL004_HI_PA_REVIEW_LOCK_V3_MANIFEST.sourceBlobLocks).length}`);
console.log(`canonical learner questions ${hindi.length + punjabi.length}`);
console.log(`learner content digest ${learnerDigest}`);
console.log("QL004 Hindi/Punjabi REVIEW_LOCKED_V3");
console.log("native/product approval false");
