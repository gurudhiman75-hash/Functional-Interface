...
const OUTPUT = process.argv[2] ?? "RNK-CP-007-HI-PA-LOCALIZATION-REVIEW-V2-64Q.md";
...
function balancedSample(
  ...
  const bank = buildRnkCp007LocalizedReviewBankV2(locale);
  const selected: RnkCp007LocalizedReviewQuestionV2[] = [];
  for (const mode of MODES) {
    for (const style of STYLES) {
      ...
      if (bucket.length < 2) {
        throw new Error(`Insufficient CP007 V2 review bucket ${locale}/${mode}/${style}`);
      }
      selected.push(bucket[0]!, bucket[1]!);
    }
  }
  if (selected.length !== 32) {
    throw new Error(`Expected 32 ${locale} V2 review questions, found ${selected.length}`);
  }
  return selected;
}
...
const lines: string[] = [];
lines.push("# RNK-CP-007 — Hindi/Punjabi Native Editorial Human Review Pack V2");
lines.push("");
lines.push("Status: **machine-proved native-editorial review candidate — not multilingual frozen**.");
lines.push("");
lines.push("V2 supersedes V1 for human-language review. It fixes native oblique/plural agreement around postpositions and the incomplete COMPACT_RATIO rank sentence while preserving the frozen English mathematics exactly.");
lines.push("");
lines.push("```text");
...
lines.push("- correct oblique/plural forms before में/में से and ਵਿੱਚ/ਵਿੱਚੋਂ;");
lines.push("- COMPACT_RATIO rank sentence includes स्थान पर / ਸਥਾਨ 'ਤੇ;");
lines.push("- correct आगे/पीछे and ਅੱਗੇ/ਪਿੱਛੇ semantics relative to the named person;");
...
lines.push("- no residual English learner-facing prose except one-letter symbolic markers inside localized labels.");
