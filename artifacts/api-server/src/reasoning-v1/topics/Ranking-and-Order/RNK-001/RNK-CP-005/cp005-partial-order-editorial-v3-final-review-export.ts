...
import {
  ...
  generateRnkCp005EditorialV3FinalQuestion,
...
function renderAnswer(answer: string, context: RnkCp005Context): string {
  const general = answer.match(/^(.+?) ranks above (.+)$/i);
  if (general) {
    return relationSurface(general[1]!.trim(), general[2]!.trim(), context);
  }
...
function chooseReviewSet(
  ...
  const candidates = Array.from({ length: 24 }, (_, ordinal) =>
    generateRnkCp005EditorialV3FinalQuestion(sourceForm, ordinal),
  );
  ...
  if (!visit(0)) {
    throw new Error(`${sourceForm}: unable to build four-question V3 final review set`);
  }
  return [...selected];
}
...
if (new Set(pairReview.map((question) => question.pairStatusMode)).size !== 3) {
  throw new Error("Pair-status review set must show all three answer modes");
}

const output: string[] = [
  "# RNK-CP-005 Partial Order and Ranking Uncertainty",
  "",
  "## Editorial V3 Final Human Review — 28 Questions",
  "",
  "This pack contains four questions from each of seven source forms. The forms consolidate into three provisional authorities. No permanent QL has been allocated.",
  "",
  ...
  "- could-be-true wrong options require a multi-step contradiction and cannot merely reverse a displayed clue;",
  "- cannot-be-true wrong options are all possible-but-not-compulsory;",
  "- named-pair questions include first-above, second-above and indeterminate outcomes;",
  "- rank bounds require at least three compulsory people plus branch integration;",
  "- boundary explanations prove both the limit and attainability;",
  "- exact-rank explanations use structural proof or two counterexample rankings;",
  "- every source form contributes four different graph topologies to this review pack;",
  "- difficulty depends on proof depth rather than entity count.",
  "",
  "---",
  "",
  "# Part A — Questions",
...
const destination = resolve(
  process.cwd(),
  "artifacts/api-server/src/reasoning-v1/topics/Ranking-and-Order/RNK-001/RNK-CP-005/generated/RNK-CP-005-PARTIAL-ORDER-EDITORIAL-V3-FINAL-REVIEW-28Q.md",
);
