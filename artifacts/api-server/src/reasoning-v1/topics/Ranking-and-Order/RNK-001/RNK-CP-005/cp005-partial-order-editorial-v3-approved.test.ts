...
import {
  buildRnkCp005EditorialV3ApprovedCorpus,
  ...
  RNK_CP005_PARTIAL_ORDER_EDITORIAL_V3_APPROVED_VERSION,
...
const corpus = buildRnkCp005EditorialV3ApprovedCorpus(24);
assert.equal(corpus.length, 168);
assert.equal(
  RNK_CP005_PARTIAL_ORDER_EDITORIAL_V3_APPROVED_VERSION,
  "RNK_CP005_PARTIAL_ORDER_EDITORIAL_V3_APPROVED",
);
...
function validateGenericRelation(
  ...
  assert.equal(
    ...
    4,
    `${question.discoveryId}: repeated ordered pair`,
  );
  assert.equal(
    new Set(relations.map((item) => [item.first, item.second].sort().join("|"))).size,
    4,
    `${question.discoveryId}: repeated unordered pair`,
  );
  ...
  assert.ok(personCounts.size >= 4, `${question.discoveryId}: too few option people`);
  assert.ok(
  ...
  if (question.prototypeId === "DEFINITELY_TRUE_RELATION") {
    ...
    assert.ok(
      ...
      `${question.discoveryId}: must-be-true distractors too obvious`,
    );
    ...
    for (const item of wrong) {
      ...
      assert.ok(
        ...
        `${question.discoveryId}: could distractor lacks multi-step contradiction`,
      );
      assert.equal(
        directEdge(state, item.second, item.first),
        false,
        `${question.discoveryId}: could distractor directly reverses a clue`,
      );
    }
  } else {
    ...
    assert.ok(
      ...
      `${question.discoveryId}: cannot-be-true distractors must all be plausible`,
    );
  }
}
...
function validatePairStatus(
  ...
  assert.ok(pair, `${question.discoveryId}: pair stem did not parse`);
  const mode = question.pairStatusMode;
  assert.ok(mode, `${question.discoveryId}: missing pair-status mode`);
  pairModes[mode] += 1;
  ...
  if (mode === "FIRST_ABOVE") {
    ...
    assert.ok(
      state.validOrders.some(
        (order) => order.indexOf(pair.first) < order.indexOf(pair.second),
      ),
    );
    assert.ok(
      state.validOrders.some(
        (order) => order.indexOf(pair.second) < order.indexOf(pair.first),
      ),
    );
  }
}
...
function validateRankBound(
  ...
  assert.ok(
    compulsory.length >= 3,
    `${question.discoveryId}: bound has fewer than three compulsory people`,
  );
  assert.equal(
    containsIncomparablePair(state, compulsory),
    true,
    `${question.discoveryId}: bound lacks branch integration`,
  );
  ...
  assert.equal(
    /must all rank/i.test(question.explanation.join(" ")),
    false,
    `${question.discoveryId}: learner grammar regressed to 'must all rank'`,
  );
  assert.match(
    question.explanation.at(-1) ?? "",
    /attainable/i,
    `${question.discoveryId}: bound explanation lacks attainability proof`,
  );
}
...
for (const question of corpus) {
  ...
  assert.equal(
    fingerprints.has(question.mathematicalFingerprint),
    false,
    `${question.discoveryId}: duplicate fingerprint`,
  );
...
for (const sourceForm of RNK_CP005_EDITORIAL_V3_SOURCE_FORMS) {
  ...
  assert.ok(
    ...
    `${sourceForm}: must span at least five topology families`,
  );
}
...
console.log(
  JSON.stringify(
    {
      status: "PASS",
      editorialVersion: RNK_CP005_PARTIAL_ORDER_EDITORIAL_V3_APPROVED_VERSION,
