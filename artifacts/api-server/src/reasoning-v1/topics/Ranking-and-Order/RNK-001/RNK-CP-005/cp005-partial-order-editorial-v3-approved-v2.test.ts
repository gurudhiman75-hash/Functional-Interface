...
assert.equal(
  RNK_CP005_PARTIAL_ORDER_EDITORIAL_V3_APPROVED_VERSION,
  "RNK_CP005_PARTIAL_ORDER_EDITORIAL_V3_APPROVED_V2",
);
...
function validateGenericRelation(
  ...
  assert.ok([...people.values()].every((count) => count <= 2));
...
function validatePairStatus(
  ...
  const gapOption = question.options.find((option) =>
    /must be consecutive|Exactly \d+ (?:person|people) must be ranked between/i.test(
      option.label,
    ),
  );
  assert.ok(gapOption, `${question.discoveryId}: missing varied gap distractor`);
  assert.equal(gapOption.truth, false);
  if (/consecutive/i.test(gapOption.label)) pairGapKinds.add("CONSECUTIVE");
  else if (/Exactly 1 person/i.test(gapOption.label)) pairGapKinds.add("ONE_BETWEEN");
  else if (/Exactly 2 people/i.test(gapOption.label)) pairGapKinds.add("TWO_BETWEEN");
...
function validateRankBound(
  ...
  assert.equal(
    ...
    true,
    `${question.discoveryId}: bound still reducible to direct predecessor counting`,
  );
...
function validateExactRank(
  ...
  assert.equal(
    ...
    true,
    `${question.discoveryId}: definite exact rank is only direct comparison counting`,
  );
}
...
assert.ok(pairGapKinds.size >= 3, "pair questions still use one repeated gap distractor");
