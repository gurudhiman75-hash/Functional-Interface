...
function relationSurface(first: string, second: string, context: RnkCp005Context): string {
  switch (context) {
...
function mustSurface(first: string, second: string, context: RnkCp005Context): string {
  switch (context) {
...
function renderLabel(label: string, context: RnkCp005Context): string {
  ...
  if (relation) return `${relationSurface(relation[1]!.trim(), relation[2]!.trim(), context)}.`;
  const must = label.match(/^(.+?) must rank above (.+?)\.$/i);
  if (must) return `${mustSurface(must[1]!.trim(), must[2]!.trim(), context)}.`;
  return label;
}
...
function chooseReviewSet(
  ...
  const candidates = Array.from({ length: 24 }, (_, ordinal) =>
    generateRnkCp005EditorialV3ApprovedQuestion(sourceForm, ordinal),
  );
  ...
  if (!visit(0)) {
    throw new Error(`${sourceForm}: unable to select four topology-diverse approved questions`);
  }
  return [...selected];
}

function renderQuestion(question: RnkCp005EditorialV3Question, number: number): string[] {
  const metadata = [
  ...
  if (question.pairStatusMode) metadata.push(`**Pair-status mode:** ${question.pairStatusMode}`);
  return [
    `## Question ${number}`,
    "",
    ...metadata,
...
function renderAnswerBlock(question: RnkCp005EditorialV3Question, number: number): string[] {
  return [
...
for (const sourceForm of RNK_CP005_EDITORIAL_V3_SOURCE_FORMS) {
  const family = questions.filter((question) => question.prototypeId === sourceForm);
  if (family.length !== 4) throw new Error(`${sourceForm}: expected four questions`);
  ...
  if (new Set(family.map((question) => question.v3Topology)).size !== 4) {
    throw new Error(`${sourceForm}: expected four distinct topologies`);
  }
...
const pairReview = questions.filter(
  (question) => question.prototypeId === "PAIR_RELATION_CANNOT_BE_DETERMINED",
);
if (new Set(pairReview.map((question) => question.pairStatusMode)).size !== 3) {
  throw new Error("Approved pair-status review must display all three answer modes");
}
...
const output: string[] = [
  ...
  "",
  "This pack contains four questions from each of seven source forms, consolidated into three provisional authorities. No permanent QL has been allocated.",
  "",
  ...
  "### V3 approved gates",
  "",
  "- four distinct generic relation pairs; no person appears in more than two options;",
  "- must-be-true uses at least two possible-but-not-compulsory distractors;",
  "- could-be-true wrong options require multi-step contradiction and never direct clue reversal;",
  ...
  "- bound solutions prove the limit and show an attainable witness order;",
  "- definite exact ranks use structural proof; indeterminate exact ranks use two valid witnesses;",
  "- four distinct graph topologies appear within every source-form review sample;",
  ...
  JSON.stringify(
    {
      ...
      authorityCandidates: Object.fromEntries(
        RNK_CP005_V3_AUTHORITY_CANDIDATE_IDS.map((authorityId) => [
          authorityId,
          questions.filter((question) => question.authorityCandidateId === authorityId).length,
        ]),
      ),
      ...
      pairStatusModes: Object.fromEntries(
        ["FIRST_ABOVE", "SECOND_ABOVE", "INDETERMINATE"].map((mode) => [
          mode,
          pairReview.filter((question) => question.pairStatusMode === mode).length,
        ]),
      ),
