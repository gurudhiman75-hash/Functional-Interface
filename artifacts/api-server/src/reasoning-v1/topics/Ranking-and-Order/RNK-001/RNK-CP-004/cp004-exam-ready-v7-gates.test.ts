...
function correctRole(question: RnkCp004ExamReadyQuestion): string {
  return question.reviewMetadata.optionRoleMetadata[question.correctIndex].role;
}

const outputDirectory = process.argv[2] ?? 'rnk-cp004-exam-ready-v7-output';
mkdirSync(outputDirectory, { recursive: true });

const runtime = RNK_CP004_REMODEL_V7_PROTOTYPE_IDS.flatMap((prototypeId) =>
  Array.from({ length: 240 }, (_, seed) =>
...
let directionQuestionCount = 0;
let exactDistanceQuestionCount = 0;
let grammarRepairCount = 0;
let compactMissingHelpCount = 0;

for (const question of runtime) {
  const regenerated = generateRnkCp004ExamReadyQuestion(question.prototypeId, question.seed);
  assert(JSON.stringify(regenerated) === JSON.stringify(question), `Non-deterministic V7 output at ${question.prototypeId}:${question.seed}`);
  ...
  assert(question.reviewMetadata.editorialDiversityProfile.distractorCount === question.options.length - 1, `Distractor metadata mismatch at ${question.prototypeId}:${question.seed}`);
  assert(question.reviewMetadata.editorialDiversityProfile.optionCountInvariantSatisfied, `Option invariant false at ${question.prototypeId}:${question.seed}`);
  assert(question.reviewMetadata.optionRoleMetadata.length === question.options.length, `Option-role count mismatch at ${question.prototypeId}:${question.seed}`);
  ...
  assert(question.reviewMetadata.difficultyModel.score === componentTotal(question), `Difficulty score not reproducible at ${question.prototypeId}:${question.seed}`);
  ...
  assert(question.reviewMetadata.difficultyModel.reasons.some((reason) => reason === '3 distractors across 4 options'), `Correct distractor reason missing at ${question.prototypeId}:${question.seed}`);
  ...
  assert(!text.includes('rank positions above'), `Unnatural rank-position wording at ${question.prototypeId}:${question.seed}`);
  assert(!text.includes('1 places above'), `Singular place wording defect at ${question.prototypeId}:${question.seed}`);
  if (text.includes('There is 1 person between them')) grammarRepairCount += 1;
  ...
  if (query.kind === 'RELATIVE_ORDER_OF_PAIR'
    ...
    directionQuestionCount += 1;
    assert(editorial.optionSemanticMode === 'DIRECTION_ONLY', `Pair option mode drift at ${question.prototypeId}:${question.seed}`);
    const roles = new Set(question.reviewMetadata.optionRoleMetadata.map((record) => record.role));
    assert(roles.has('CORRECT'), `Direction correct role missing at ${question.seed}`);
    assert(roles.has('REVERSE_DIRECTION'), `Direction reverse role missing at ${question.seed}`);
    assert(roles.has('SAME_RANK_CONTRADICTION'), `Same-rank role missing at ${question.seed}`);
    assert(roles.has('CANNOT_DETERMINE_CONTRADICTION'), `Cannot-determine role missing at ${question.seed}`);
    assert(question.options.every((item) => !item.answerKey.startsWith('GAP:')), `Mixed distance semantics remain at ${question.seed}`);
  ...
  if (question.prototypeId === RNK_CP004_EXACT_RANK_DIFFERENCE_PROTOTYPE_ID) {
    exactDistanceQuestionCount += 1;
    assert(editorial.optionSemanticMode === 'EXACT_DISTANCE', `Exact-distance mode drift at ${question.seed}`);
    assert(question.options.every((item) => /\bplace(?:s)? above\b/.test(item.label)), `Exact-distance option is not naturally phrased at ${question.seed}`);
    assert(correctRole(question) === 'CORRECT', `Exact-distance correct role mismatch at ${question.seed}`);
    const features = question.reviewMetadata.reasoningFeatures;
    const language = question.reviewMetadata.languageProfile;
    const confirmatory = question.reviewMetadata.clueRoleProfile.confirmatory;
    const expected = features.entityCount >= 8 && language.reversedClueCount >= 2 && confirmatory > 0 ? 'HARD' : 'MEDIUM';
    assert(question.difficulty === expected, `Exact-distance difficulty overstatement at ${question.seed}`);
  }

  if (query.kind === 'MIDDLE_ENTITY' && question.displayedEvidence.entities.length === 5) {
    assert(question.difficulty === 'EASY', `Five-person middle question is not Easy at ${question.seed}`);
  }
  if (query.kind === 'RANK_OF_NAMED_ENTITY') {
    assert(question.difficulty !== 'HARD', `Named-rank question remains overstated at ${question.seed}`);
  }
  if (query.kind === 'IMMEDIATE_NEIGHBOUR') {
    assert(question.difficulty !== 'HARD', `Immediate-neighbour question remains overstated at ${question.seed}`);
  }
  if (query.kind === 'VALID_RANK_STATEMENT') {
    ...
    assert(question.reviewMetadata.editorialDiversityProfile.distractorCount === 3, `Definitely-true distractor count defect at ${question.seed}`);
  }
  if (query.kind === 'MISSING_COMPARISON') {
    for (const line of question.visibleExplanation.optionAnalysis ?? []) {
      assert(line.includes('contradicts') || line.includes('permits'), `Missing-comparison help lacks compact proof at ${question.seed}`);
      assert(wordCount(line) <= 35, `Missing-comparison mock help is too long at ${question.seed}`);
      if (line.includes('permits')) compactMissingHelpCount += 1;
    }
  }
...
assert(directionQuestionCount === 240, `Expected 240 direction questions, found ${directionQuestionCount}`);
assert(exactDistanceQuestionCount === 240, `Expected 240 exact-distance questions, found ${exactDistanceQuestionCount}`);
assert(compactMissingHelpCount > 0, 'Compact missing-comparison help was not exercised');
...
const reviewPack = buildRnkCp004ReviewPackV7();
...
const reviewConfirmatory = new Set<number>();
...
reviewPack.forEach((question, index) => {
  ...
  const perPrototype = contextByPrototype.get(question.prototypeId) ?? new Map<string, number>();
  perPrototype.set(context, (perPrototype.get(context) ?? 0) + 1);
  contextByPrototype.set(question.prototypeId, perPrototype);
  ...
  reviewConfirmatory.add(question.reviewMetadata.clueRoleProfile.confirmatory);

  const batchStart = Math.floor(index / RNK_CP004_REMODEL_V7_PROTOTYPE_IDS.length)
    * RNK_CP004_REMODEL_V7_PROTOTYPE_IDS.length;
  const batch = reviewPack.slice(batchStart, batchStart + RNK_CP004_REMODEL_V7_PROTOTYPE_IDS.length);
  if (batch.length === RNK_CP004_REMODEL_V7_PROTOTYPE_IDS.length) {
    assert(new Set(batch.map((item) => item.prototypeId)).size === RNK_CP004_REMODEL_V7_PROTOTYPE_IDS.length, `Mixed batch repeats an authority at batch ${Math.floor(index / 11) + 1}`);
  }
});

for (const prototypeId of RNK_CP004_REMODEL_V7_PROTOTYPE_IDS) {
  assert(prototypeCounts.get(prototypeId) === 12, `Expected 12 review records for ${prototypeId}`);
  assert(shapesByPrototype.get(prototypeId)?.size === 12, `Expected 12 independent structures for ${prototypeId}`);
  const contexts = contextByPrototype.get(prototypeId);
  assert(contexts?.size === 6, `Expected six contexts for ${prototypeId}`);
  for (const count of contexts?.values() ?? []) assert(count === 2, `Context is not represented twice for ${prototypeId}`);
}
for (const count of contextCounts.values()) assert(count === 22, `Context distribution is not 22 each: ${JSON.stringify(Object.fromEntries(contextCounts))}`);
assert(reviewConfirmatory.has(0), 'Review evidence lacks zero-confirmatory records');
assert(reviewConfirmatory.has(1), 'Review evidence lacks one-confirmatory records');
assert(reviewConfirmatory.has(2), 'Review evidence lacks two-confirmatory records');
...
assert(markdown.includes('Targeted SSC/Banking English Remodel V7'), 'V7 title missing');
assert(markdown.includes('Mixed review batch 12'), 'Mixed-batch evidence incomplete');
assert(!markdown.includes('There are 1 people'), 'Grammar defect remains in review pack');
assert(!markdown.includes('rank positions above'), 'Unnatural option wording remains in review pack');
...
const averageVisibleWords = Number((reviewPack.reduce(
  (total, question) => total + wordCount(question.visibleExplanation.lines.join(' ')),
  0,
) / reviewPack.length).toFixed(2));
const averageAllWords = Number((reviewPack.reduce(
  (total, question) => total + wordCount([
    ...question.visibleExplanation.lines,
    ...(question.visibleExplanation.optionAnalysis ?? []),
  ].join(' ')),
  0,
...
const report = {
  ...
  correctedDefects: {
    ...
    optionDistractorInvariant: 'PASS',
  ...
  editorialDiversity: {
    ...
    confirmatoryCountsRepresented: [...reviewConfirmatory].sort(),
  },
  difficulty: {
...
writeFileSync(
  join(outputDirectory, 'RNK-CP-004-Questions-and-Explanations-Remodeled-V7.md'),
  markdown,
  'utf8',
);
