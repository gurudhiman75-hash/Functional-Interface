...
for (const qlId of STC_QL_IDS) {
  ...
  assert.deepEqual(answerCounts, EXPECTED_ANSWER_COUNTS, `${qlId} answer classes must be 2/2/2/2`);
  ...
  const generated = Array.from({ length: 8 }, (_, seed) => generateStcV2EditorialQuestion({ qlId, locale: "en-IN", seed }));
  assert.equal(new Set(generated.map((question) => question.scenarioId)).size, 8, `${qlId} seeds 0..7 must cover all eight authorities`);
  assert.equal(new Set(generated.map((question) => question.surfaceArchetype)).size, 8, `${qlId} seeds 0..7 must cover all eight surfaces`);

  for (const question of generated) {
    assert.equal(question.stem, pool.find((entry) => entry.id === question.scenarioId)!.statement);
...
console.log("PASS_STC_001_V2_EDITORIAL_SURFACE_DIVERSITY");
