...
import type { GeneratedStcQuestion, StcAnswerClass, StcLocale, StcLogicalDefect, StcModalDefect } from "./types.ts";
...
const LOGICAL_DEFECT_TEXT: Record<StcLogicalDefect, Record<StcLocale, string>> = {
  POLARITY_FLIP: { "en-IN": "it reverses the stated fact", "hi-IN": "यह कथन की बात को उलट देता है", "pa-IN": "ਇਹ ਕਥਨ ਦੀ ਗੱਲ ਨੂੰ ਉਲਟ ਦਿੰਦਾ ਹੈ" },
  UNSUPPORTED_EXTRA: { "en-IN": "it adds information the statement never gives", "hi-IN": "यह ऐसी अतिरिक्त बात जोड़ता है जो कथन में दी ही नहीं गई", "pa-IN": "ਇਹ ਐਸੀ ਵਾਧੂ ਗੱਲ ਜੋੜਦਾ ਹੈ ਜੋ ਕਥਨ ਵਿੱਚ ਦਿੱਤੀ ਹੀ ਨਹੀਂ ਗਈ" },
  OVERCLAIM: { "en-IN": "it claims more than the statement proves", "hi-IN": "यह कथन से सिद्ध होने वाली बात से अधिक दावा करता है", "pa-IN": "ਇਹ ਕਥਨ ਤੋਂ ਸਾਬਤ ਹੋਣ ਵਾਲੀ ਗੱਲ ਨਾਲੋਂ ਵੱਧ ਦਾਅਵਾ ਕਰਦਾ ਹੈ" },
  INVALID_COMBINATION: { "en-IN": "it treats an alternative as if every part were compulsory", "hi-IN": "यह विकल्प को ऐसे मानता है जैसे हर भाग अनिवार्य हो", "pa-IN": "ਇਹ ਵਿਕਲਪ ਨੂੰ ਇਸ ਤਰ੍ਹਾਂ ਮੰਨਦਾ ਹੈ ਜਿਵੇਂ ਹਰ ਹਿੱਸਾ ਲਾਜ਼ਮੀ ਹੋਵੇ" },
  CONVERSE: { "en-IN": "it reverses the direction of the given if-then rule", "hi-IN": "यह दी गई यदि-तो शर्त की दिशा उलट देता है", "pa-IN": "ਇਹ ਦਿੱਤੀ ਜੇ-ਤਾਂ ਸ਼ਰਤ ਦੀ ਦਿਸ਼ਾ ਉਲਟ ਦਿੰਦਾ ਹੈ" },
  INVERSE: { "en-IN": "the rule does not say that failure of the condition guarantees the opposite result", "hi-IN": "शर्त पूरी न होने पर विपरीत परिणाम निश्चित होगा, ऐसा कथन नहीं कहता", "pa-IN": "ਸ਼ਰਤ ਪੂਰੀ ਨਾ ਹੋਣ ਤੇ ਉਲਟ ਨਤੀਜਾ ਲਾਜ਼ਮੀ ਹੋਵੇਗਾ, ਕਥਨ ਇਹ ਨਹੀਂ ਕਹਿੰਦਾ" },
  DENYING_ANTECEDENT: { "en-IN": "it wrongly fixes the result just because the stated condition is absent", "hi-IN": "यह केवल शर्त के न होने से परिणाम को गलत रूप से निश्चित मानता है", "pa-IN": "ਇਹ ਕੇਵਲ ਸ਼ਰਤ ਨਾ ਹੋਣ ਕਰਕੇ ਨਤੀਜੇ ਨੂੰ ਗਲਤ ਤਰੀਕੇ ਨਾਲ ਨਿਸ਼ਚਿਤ ਮੰਨਦਾ ਹੈ" },
};

const MODAL_DEFECT_TEXT: Record<StcModalDefect, Record<StcLocale, string>> = {
  STRONGER_MODALITY: { "en-IN": "the statement gives only a possibility, not certainty", "hi-IN": "कथन केवल संभावना बताता है, निश्चितता नहीं", "pa-IN": "ਕਥਨ ਸਿਰਫ਼ ਸੰਭਾਵਨਾ ਦੱਸਦਾ ਹੈ, ਨਿਸ਼ਚਿਤਤਾ ਨਹੀਂ" },
  POLARITY_FLIP: { "en-IN": "it says the opposite of the stated possibility or certainty", "hi-IN": "यह बताई गई संभावना या निश्चितता के विपरीत बात कहता है", "pa-IN": "ਇਹ ਦੱਸੀ ਸੰਭਾਵਨਾ ਜਾਂ ਨਿਸ਼ਚਿਤਤਾ ਦੇ ਉਲਟ ਗੱਲ ਕਹਿੰਦਾ ਹੈ" },
  UNSUPPORTED_EXTRA: { "en-IN": "it introduces an event the statement never mentions", "hi-IN": "यह ऐसी घटना जोड़ता है जिसका कथन में उल्लेख नहीं है", "pa-IN": "ਇਹ ਐਸੀ ਘਟਨਾ ਜੋੜਦਾ ਹੈ ਜਿਸਦਾ ਕਥਨ ਵਿੱਚ ਜ਼ਿਕਰ ਨਹੀਂ ਹੈ" },
};

const CONDITIONAL_FOLLOWS: Record<StcLocale, string> = {
  "en-IN": "the stated condition or condition-chain leads to this conclusion",
  "hi-IN": "दी गई शर्त या शर्तों की श्रृंखला से यह निष्कर्ष निकलता है",
  "pa-IN": "ਦਿੱਤੀ ਸ਼ਰਤ ਜਾਂ ਸ਼ਰਤਾਂ ਦੀ ਲੜੀ ਤੋਂ ਇਹ ਨਤੀਜਾ ਨਿਕਲਦਾ ਹੈ",
};
const MODAL_FOLLOWS: Record<StcLocale, string> = {
  "en-IN": "this stays within the degree of certainty stated in the question",
  "hi-IN": "यह निष्कर्ष कथन में दी गई निश्चितता की सीमा के भीतर रहता है",
  "pa-IN": "ਇਹ ਨਤੀਜਾ ਕਥਨ ਵਿੱਚ ਦਿੱਤੀ ਨਿਸ਼ਚਿਤਤਾ ਦੀ ਹੱਦ ਅੰਦਰ ਰਹਿੰਦਾ ਹੈ",
};
...
export function generateStcCp002Question(input: {
  ...
  if (input.qlId === "STC-QL-003") {
    const scenario = CONDITIONAL_AUTHORITIES[base % CONDITIONAL_AUTHORITIES.length]!;
    ...
    const reason = (follows: boolean, defect: StcLogicalDefect | undefined) => follows ? CONDITIONAL_FOLLOWS[input.locale] : defect ? LOGICAL_DEFECT_TEXT[defect][input.locale] : FALLBACK[input.locale];
    return {
      chapterId: "STC-001", checkpointId: "STC-CP-002", qlId: input.qlId, scenarioId: scenario.id,
  ...
  const scenario = MODAL_AUTHORITIES[base % MODAL_AUTHORITIES.length]!;
  const [firstIndex, secondIndex] = choosePair(base);
  const first = scenario.candidates[firstIndex]!;
  const second = scenario.candidates[secondIndex]!;
  ...
  const cls = answerClass(firstFollows, secondFollows);
  const reason = (follows: boolean, defect: StcModalDefect | undefined) => follows ? MODAL_FOLLOWS[input.locale] : defect ? MODAL_DEFECT_TEXT[defect][input.locale] : FALLBACK[input.locale];
  return {
    chapterId: "STC-001", checkpointId: "STC-CP-002", qlId: input.qlId, scenarioId: scenario.id,
    locale: input.locale, seed: input.seed, difficulty: scenario.difficulty,
    stem: `${STEM_PREFIX[input.locale]}\n\n${scenario.statement[input.locale]}`,
    ...
    correctIndex: indexForAnswer(cls), answerClass: cls,
    explanation: `I: ${first.text[input.locale]} — ${reason(firstFollows, first.defectIfNotEntailed)}. II: ${second.text[input.locale]} — ${reason(secondFollows, second.defectIfNotEntailed)}.`,
    metadata: { solver: "MODAL_STRENGTH_ENTAILMENT_V1", reviewOnly: true, questionBankWritable: false, testEligible: false, mockEligible: false, publicEligible: false },
  };
}
