...
assert(
  stcOrderEntails(
    [orderClaim("rank", "a", "b"), orderClaim("rank", "b", "c")],
    orderClaim("rank", "a", "c"),
  ),
  "strict-order transitivity must hold",
);
assert(
  !stcOrderEntails(
    [orderClaim("rank", "a", "b"), orderClaim("rank", "b", "c")],
  ...
  "reversed strict order must fail",
);
assert(
  !stcOrderEntails([orderClaim("rank", "a", "b")], orderClaim("age", "a", "b")),
  "unrelated relation must fail",
);

assert(
  stcTemporalEntails(
    [beforeClaim("a", "b"), beforeClaim("b", "c")],
    beforeClaim("a", "c"),
  ),
  "temporal transitivity must hold",
);
assert(
  !stcTemporalEntails(
    [beforeClaim("a", "b"), beforeClaim("b", "c")],
  ...
  "reversed time must fail",
);
assert(
  stcTemporalEntails(
    [trendClaim("m", "jan", "feb", "INCREASED"), trendClaim("m", "feb", "mar", "INCREASED")],
    trendClaim("m", "jan", "mar", "INCREASED"),
  ),
  "increase trend must compose transitively",
);
assert(
  stcTemporalEntails(
    [trendClaim("m", "jan", "feb", "DECREASED"), trendClaim("m", "feb", "mar", "DECREASED")],
    trendClaim("m", "jan", "mar", "DECREASED"),
  ),
  "decrease trend must compose transitively",
);
assert(
  !stcTemporalEntails(
    [trendClaim("m", "jan", "feb", "INCREASED"), trendClaim("m", "feb", "mar", "INCREASED")],
  ...
  "reversed trend must fail",
);

for (const scenario of STC_CP003_ORDER_AUTHORITIES) {
...
for (const scenario of STC_CP003_TEMPORAL_AUTHORITIES) {
  assert(scenario.candidates.length === 4, `${scenario.id}: expected four candidates`);
  assert(new Set(scenario.candidates.map((candidate) => candidate.text["en-IN"])).size === 4, `${scenario.id}: duplicate English candidate`);
  for (const candidate of scenario.candidates) {
...
for (const qlId of QLS) {
  ...
  const expected = qlId === "STC-QL-005" ? STC_CP003_ORDER_AUTHORITIES.length : STC_CP003_TEMPORAL_AUTHORITIES.length;
  assert(scenarios.size === expected, `${qlId}: not all curated scenarios reached`);
}

console.log(`STC-CP-003 proof passed: ${STC_CP003_ORDER_AUTHORITIES.length + STC_CP003_TEMPORAL_AUTHORITIES.length} authorities, 2 QLs, 3 locales.`);
