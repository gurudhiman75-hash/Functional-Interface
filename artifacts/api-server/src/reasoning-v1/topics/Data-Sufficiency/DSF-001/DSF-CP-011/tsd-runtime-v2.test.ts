...
assert.deepEqual(new Set(questions.map((question) => question.targetKind)), new Set(["DISTANCE", "SPEED", "TIME", "TRAIN_CLEAR_TIME", "TRAIN_CROSS_TIME", "BOAT_TRAVEL_TIME"]));
...
const classCounts = Object.fromEntries(SUFFICIENCY_CLASSES.map((semanticClass) => [semanticClass, questions.filter((question) => question.canonicalAnswer === semanticClass).length]));
for (const semanticClass of SUFFICIENCY_CLASSES) assert.equal(classCounts[semanticClass], 70);
const solveModeCounts = Object.fromEntries(DSF_CP011_TSD_SOLVE_MODES.map((solveMode) => [solveMode, questions.filter((question) => question.solveModeId === solveMode).length]));
...
assert(normalizedStems.size >= 30, `Expected at least 30 perceptually different TSD/train/boat stem surfaces, found ${normalizedStems.size}`);
const structuralFingerprints = new Set(questions.map((question) => question.studentSurfaceFingerprint));
assert(structuralFingerprints.size >= 100, `Expected at least 100 distinct TSD/train/boat student-facing structural fingerprints, found ${structuralFingerprints.size}`);
...
assert(largestFingerprintCluster <= 10, `A single TSD/train/boat student-facing structure repeated ${largestFingerprintCluster} times in a 350-question audit`);

console.log(JSON.stringify({
  status: "PASS_DSF_CP011_TSD_TRAINS_BOATS_BREADTH_AND_REALNESS_AUDIT_V2",
