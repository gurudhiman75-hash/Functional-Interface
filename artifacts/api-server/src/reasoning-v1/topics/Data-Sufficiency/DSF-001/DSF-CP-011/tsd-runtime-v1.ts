...
export const DSF_CP011_TSD_RUNTIME_VERSION = "DSF_CP011_TSD_RUNTIME_V1" as const;
...
type StatementFamily =
  ...
  | "TRAIN_FULL_DATA"
  | "TRAIN_BOUND"
  | "SECOND_TRAIN_LENGTH_EXACT"
  | "SECOND_TRAIN_SPEED_EXACT"
  | "RELATIVE_SPEED_EXACT"
  | "CROSS_TIME_EXACT"
  | "LENGTH_PAIR"
  | "SPEED_PAIR"
  | "CROSSING_DATA_PAIR"
...
function hashSeed(seed: number, salt: string): number {
  let hash = 2166136261;
  const text = `${DSF_CP011_TSD_RUNTIME_VERSION}:${seed}:${salt}`;
...
function enumerateCoreWorlds(): readonly CoreWorld[] {
  ...
  const speeds = [5, 6, 8, 10, 12, 15, 18, 20] as const;
  const times = [20, 24, 30, 36, 40, 45, 50, 60] as const;
...
function enumerateFixedTrainWorlds(): readonly FixedTrainWorld[] {
  ...
  const trainLengths = [90, 120, 150, 180, 210, 240] as const;
  const objectLengths = [0, 60, 90, 120, 150] as const;
  const speeds = [10, 12, 15, 18, 20, 24] as const;
  for (const trainLength of trainLengths) {
    for (const objectLength of objectLengths) {
      for (const speed of speeds) {
        ...
        worlds.push({
          ...
          objectLengthMetres,
          speedMps,
          totalLengthMetres: rational(trainLength + objectLength),
          clearTimeSeconds,
        });
      }
    }
...
function enumerateTwoTrainWorlds(): readonly TwoTrainWorld[] {
  ...
  const firstLengths = [100, 120, 150, 180] as const;
  const secondLengths = [90, 120, 160, 200] as const;
  const firstSpeeds = [10, 12, 15, 18] as const;
  const secondSpeeds = [8, 10, 12, 15] as const;
  for (const firstLength of firstLengths) {
    for (const secondLength of secondLengths) {
      for (const firstSpeed of firstSpeeds) {
        for (const secondSpeed of secondSpeeds) {
          ...
          const crossingTimeSeconds = twoTrainCompleteCrossingTime(
            firstLengthMetres,
            secondLengthMetres,
            firstSpeedMps,
            rational(-secondSpeed),
          );
          if (!crossingTimeSeconds) continue;
          worlds.push({
            kind: "TRAIN_TWO",
            firstLengthMetres,
            secondLengthMetres,
            firstSpeedMps,
            secondSpeedMps,
            totalLengthMetres: rational(firstLength + secondLength),
            relativeSpeedMps: rational(firstSpeed + secondSpeed),
            crossingTimeSeconds,
          });
        }
      }
...
function enumerateBoatWorlds(kind: BoatWorld["kind"]): readonly BoatWorld[] {
  ...
  const stillSpeeds = [6, 8, 10, 12, 15, 18] as const;
  const streamSpeeds = [1, 2, 3, 4] as const;
  const distances = [600, 720, 900, 1080, 1200, 1440] as const;
  for (const still of stillSpeeds) {
    for (const stream of streamSpeeds) {
      if (still <= stream) continue;
      const stillWaterSpeedMps = rational(still);
      const streamSpeedMps = rational(stream);
      const direction = kind === "BOAT_DOWNSTREAM" ? 1 : -1;
      const groundSpeedMps = groundSpeedInMedium(stillWaterSpeedMps, direction, streamSpeedMps);
      if (groundSpeedMps.numerator <= 0n) continue;
      for (const distance of distances) {
        const distanceMetres = rational(distance);
        const travelTimeSeconds = durationForUniformMotion(distanceMetres, groundSpeedMps);
        worlds.push({ kind, distanceMetres, stillWaterSpeedMps, streamSpeedMps, groundSpeedMps, travelTimeSeconds });
      }
    }
  }
  return worlds;
...
function sourceAnswer(problem: TsdProblem, world: TsdWorld): string {
  ...
  switch (problem.solveMode) {
    case "DSF-SM-TSD-DISTANCE": {
      ...
      value = scalarValue(solveCp001({ solveMode: "distanceFromSpeedAndTime", speedMps: world.speedMps, durationSeconds: world.timeSeconds }));
      unit = "metres";
      break;
    }
    case "DSF-SM-TSD-SPEED": {
      if (world.kind !== "CORE") throw new Error("Core speed mode received incompatible world.");
      value = scalarValue(solveCp001({ solveMode: "speedFromDistanceAndTime", distanceMetres: world.distanceMetres, durationSeconds: world.timeSeconds }));
      unit = "m/s";
      break;
    }
    case "DSF-SM-TSD-TIME": {
      if (world.kind !== "CORE") throw new Error("Core time mode received incompatible world.");
      value = scalarValue(solveCp001({ solveMode: "timeFromDistanceAndSpeed", distanceMetres: world.distanceMetres, speedMps: world.speedMps }));
      unit = "seconds";
      break;
    }
    case "DSF-SM-TRAIN-FIXED-CLEAR-TIME": {
      if (world.kind !== "TRAIN_FIXED") throw new Error("Fixed-train mode received incompatible world.");
      value = trainClearTimeAgainstFixedObject(world.trainLengthMetres, world.objectLengthMetres, world.speedMps);
      unit = "seconds";
      break;
    }
    ...
    case "DSF-SM-BOAT-DOWNSTREAM-TIME": {
      ...
      const direction = expectedKind === "BOAT_DOWNSTREAM" ? 1 : -1;
      const ground = groundSpeedInMedium(world.stillWaterSpeedMps, direction, world.streamSpeedMps);
      value = durationForUniformMotion(world.distanceMetres, ground);
      unit = "seconds";
      break;
    }
  }
...
const adapter = {
  adapterId: "DSF-CP011-TSD-001-SOURCE-BOUND",
...
function buildCoreStatements(anchor: CoreWorld): readonly TsdStatement[] {
  ...
  return [
    statement(`D_EQ_${d}`, "DISTANCE_EXACT", 1, `The distance travelled is ${d} metres.`, (w) => w.kind === "CORE" && eq(w.distanceMetres, anchor.distanceMetres)),
    statement(`S_EQ_${s}`, "SPEED_EXACT", 1, `The speed is ${s} metres per second.`, (w) => w.kind === "CORE" && eq(w.speedMps, anchor.speedMps)),
    statement(`T_EQ_${t}`, "TIME_EXACT", 1, `The travel time is ${t} seconds.`, (w) => w.kind === "CORE" && eq(w.timeSeconds, anchor.timeSeconds)),
    ...
    statement(`S_LE_${s}`, "CORE_BOUND", 2, `The speed does not exceed ${s} m/s.`, (w) => w.kind === "CORE" && n(w.speedMps) <= s),
    statement(`S_GE_${s}`, "CORE_BOUND", 2, `The speed is at least ${s} m/s.`, (w) => w.kind === "CORE" && n(w.speedMps) >= s),
    statement(`T_LE_${t}`, "CORE_BOUND", 2, `The journey takes at most ${t} seconds.`, (w) => w.kind === "CORE" && n(w.timeSeconds) <= t),
    statement(`T_GE_${t}`, "CORE_BOUND", 2, `The journey takes at least ${t} seconds.`, (w) => w.kind === "CORE" && n(w.timeSeconds) >= t),
    statement(`S_PAR_${s % 2}`, "CORE_PARITY", 2, `The speed in m/s is ${s % 2 === 0 ? "even" : "odd"}.`, (w) => w.kind === "CORE" && Math.trunc(n(w.speedMps)) % 2 === s % 2),
  ];
}

function buildFixedTrainStatements(anchor: FixedTrainWorld): readonly TsdStatement[] {
  const l = n(anchor.trainLengthMetres), o = n(anchor.objectLengthMetres), s = n(anchor.speedMps), total = n(anchor.totalLengthMetres), time = n(anchor.clearTimeSeconds);
  return [
    ...
    statement(`TOTAL_${total}`, "TOTAL_LENGTH_EXACT", 2, `The train must cover ${total} metres from the instant its front reaches the object until its rear clears it.`, (w) => w.kind === "TRAIN_FIXED" && eq(w.totalLengthMetres, anchor.totalLengthMetres)),
    statement(`CT_${toCanonicalString(anchor.clearTimeSeconds)}`, "CLEAR_TIME_EXACT", 1, `The train completely clears the object in ${toMixedString(anchor.clearTimeSeconds)} seconds.`, (w) => w.kind === "TRAIN_FIXED" && eq(w.clearTimeSeconds, anchor.clearTimeSeconds)),
    statement(`TOTAL_SPEED_${total}_${s}`, "TOTAL_SPEED_PAIR", 2, `During complete clearance, the train covers ${total} metres at ${s} m/s.`, (w) => w.kind === "TRAIN_FIXED" && eq(w.totalLengthMetres, anchor.totalLengthMetres) && eq(w.speedMps, anchor.speedMps)),
    statement(`FULL_${l}_${o}_${s}`, "TRAIN_FULL_DATA", 3, `The train is ${l} m long, the object is ${o} m long, and the train moves at ${s} m/s.`, (w) => w.kind === "TRAIN_FIXED" && eq(w.trainLengthMetres, anchor.trainLengthMetres) && eq(w.objectLengthMetres, anchor.objectLengthMetres) && eq(w.speedMps, anchor.speedMps)),
    statement(`TL_LE_${l}`, "TRAIN_BOUND", 2, `The train length is at most ${l} metres.`, (w) => w.kind === "TRAIN_FIXED" && n(w.trainLengthMetres) <= l),
    statement(`S_GE_${s}`, "TRAIN_BOUND", 2, `The train speed is at least ${s} m/s.`, (w) => w.kind === "TRAIN_FIXED" && n(w.speedMps) >= s),
  ];
}
...
function buildTwoTrainStatements(anchor: TwoTrainWorld): readonly TsdStatement[] {
  ...
  return [
    ...
    statement(`TOTAL_${total}`, "TOTAL_LENGTH_EXACT", 2, `The sum of the two train lengths is ${total} metres.`, (w) => w.kind === "TRAIN_TWO" && eq(w.totalLengthMetres, anchor.totalLengthMetres)),
    statement(`REL_${rel}`, "RELATIVE_SPEED_EXACT", 2, `Their relative speed while approaching is ${rel} m/s.`, (w) => w.kind === "TRAIN_TWO" && eq(w.relativeSpeedMps, anchor.relativeSpeedMps)),
    statement(`CROSS_${toCanonicalString(anchor.crossingTimeSeconds)}`, "CROSS_TIME_EXACT", 1, `They completely cross each other in ${toMixedString(anchor.crossingTimeSeconds)} seconds.`, (w) => w.kind === "TRAIN_TWO" && eq(w.crossingTimeSeconds, anchor.crossingTimeSeconds)),
    statement(`LP_${l1}_${l2}`, "LENGTH_PAIR", 2, `The train lengths are ${l1} metres and ${l2} metres respectively.`, (w) => w.kind === "TRAIN_TWO" && eq(w.firstLengthMetres, anchor.firstLengthMetres) && eq(w.secondLengthMetres, anchor.secondLengthMetres)),
    statement(`SP_${s1}_${s2}`, "SPEED_PAIR", 2, `Their speeds are ${s1} m/s and ${s2} m/s respectively, in opposite directions.`, (w) => w.kind === "TRAIN_TWO" && eq(w.firstSpeedMps, anchor.firstSpeedMps) && eq(w.secondSpeedMps, anchor.secondSpeedMps)),
    statement(`TR_${total}_${rel}`, "CROSSING_DATA_PAIR", 2, `Their total length is ${total} metres and their relative speed is ${rel} m/s.`, (w) => w.kind === "TRAIN_TWO" && eq(w.totalLengthMetres, anchor.totalLengthMetres) && eq(w.relativeSpeedMps, anchor.relativeSpeedMps)),
    statement(`FULL_${l1}_${l2}_${s1}_${s2}`, "TWO_TRAIN_FULL_DATA", 3, `The trains are ${l1} m and ${l2} m long and move at ${s1} m/s and ${s2} m/s in opposite directions.`, (w) => w.kind === "TRAIN_TWO" && eq(w.firstLengthMetres, anchor.firstLengthMetres) && eq(w.secondLengthMetres, anchor.secondLengthMetres) && eq(w.firstSpeedMps, anchor.firstSpeedMps) && eq(w.secondSpeedMps, anchor.secondSpeedMps)),
    statement(`TOTAL_LE_${total}`, "TRAIN_BOUND", 2, `Their combined length does not exceed ${total} metres.`, (w) => w.kind === "TRAIN_TWO" && n(w.totalLengthMetres) <= total),
    statement(`REL_GE_${rel}`, "TRAIN_BOUND", 2, `Their relative speed is at least ${rel} m/s.`, (w) => w.kind === "TRAIN_TWO" && n(w.relativeSpeedMps) >= rel),
  ];
}
...
function buildBoatStatements(anchor: BoatWorld): readonly TsdStatement[] {
  ...
  return [
    statement(`BD_${d}`, "BOAT_DISTANCE_EXACT", 1, `The ${directionText} distance is ${d} metres.`, (w) => w.kind === anchor.kind && eq(w.distanceMetres, anchor.distanceMetres)),
    statement(`BS_${still}`, "STILL_SPEED_EXACT", 1, `The boat's speed in still water is ${still} m/s.`, (w) => w.kind === anchor.kind && eq(w.stillWaterSpeedMps, anchor.stillWaterSpeedMps)),
    statement(`SS_${stream}`, "STREAM_SPEED_EXACT", 1, `The stream speed is ${stream} m/s.`, (w) => w.kind === anchor.kind && eq(w.streamSpeedMps, anchor.streamSpeedMps)),
    statement(`GS_${ground}`, "GROUND_SPEED_EXACT", 2, `The boat's effective ${directionText} speed is ${ground} m/s.`, (w) => w.kind === anchor.kind && eq(w.groundSpeedMps, anchor.groundSpeedMps)),
    statement(`BT_${toCanonicalString(anchor.travelTimeSeconds)}`, "BOAT_TIME_EXACT", 1, `The ${directionText} journey takes ${toMixedString(anchor.travelTimeSeconds)} seconds.`, (w) => w.kind === anchor.kind && eq(w.travelTimeSeconds, anchor.travelTimeSeconds)),
    statement(`DG_${d}_${ground}`, "BOAT_DISTANCE_GROUND_PAIR", 2, `The ${directionText} distance is ${d} metres and the effective speed is ${ground} m/s.`, (w) => w.kind === anchor.kind && eq(w.distanceMetres, anchor.distanceMetres) && eq(w.groundSpeedMps, anchor.groundSpeedMps)),
    statement(`SSP_${still}_${stream}`, "BOAT_SPEED_PAIR", 2, `The still-water speed is ${still} m/s and the stream speed is ${stream} m/s.`, (w) => w.kind === anchor.kind && eq(w.stillWaterSpeedMps, anchor.stillWaterSpeedMps) && eq(w.streamSpeedMps, anchor.streamSpeedMps)),
    statement(`FULL_${d}_${still}_${stream}`, "BOAT_FULL_TRIP_DATA", 3, `The ${directionText} distance is ${d} m, the still-water speed is ${still} m/s, and the stream speed is ${stream} m/s.`, (w) => w.kind === anchor.kind && eq(w.distanceMetres, anchor.distanceMetres) && eq(w.stillWaterSpeedMps, anchor.stillWaterSpeedMps) && eq(w.streamSpeedMps, anchor.streamSpeedMps)),
    statement(`D_LE_${d}`, "BOAT_BOUND", 2, `The ${directionText} distance is at most ${d} metres.`, (w) => w.kind === anchor.kind && n(w.distanceMetres) <= d),
    statement(`STILL_GE_${still}`, "BOAT_BOUND", 2, `The still-water speed is at least ${still} m/s.`, (w) => w.kind === anchor.kind && n(w.stillWaterSpeedMps) >= still),
  ];
}
...
function pairQuality(first: TsdStatement, second: TsdStatement, evaluation: TwoStatementSufficiencyEvaluation<string>): number {
  ...
  const breadth = Math.min(evaluation.statementI.worldCount, 60) + Math.min(evaluation.statementII.worldCount, 60);
  return familyBonus + classBonus + Math.floor(breadth / 25) - first.complexity - second.complexity;
}
  ...
  for (const statementI of statements) {
    for (const statementII of statements) {
      ...
      try {
        ...
        // Inconsistent conjunctions are rejected generation candidates.
      }
    }
  }
  if (candidates.length === 0) throw new Error(`No TSD-001 pair for ${problem.solveMode}/${targetClass}`);
...
function buildProblem(seed: number, attempt: number): TsdProblem {
  const solveMode = DSF_CP011_TSD_SOLVE_MODES[Math.abs(seed) % DSF_CP011_TSD_SOLVE_MODES.length]!;
  ...
  if (solveMode === "DSF-SM-TSD-DISTANCE" || solveMode === "DSF-SM-TSD-SPEED" || solveMode === "DSF-SM-TSD-TIME") {
    const context = pick(random, CORE_CONTEXTS);
    return { solveMode, targetKind: solveMode === "DSF-SM-TSD-DISTANCE" ? "DISTANCE" : solveMode === "DSF-SM-TSD-SPEED" ? "SPEED" : "TIME", anchor: pick(random, CORE_WORLDS), contextId: context.id, intro: pick(random, context.intros) };
  }
  if (solveMode === "DSF-SM-TRAIN-FIXED-CLEAR-TIME") {
    const context = pick(random, FIXED_TRAIN_CONTEXTS);
    return { solveMode, targetKind: "TRAIN_CLEAR_TIME", anchor: pick(random, FIXED_TRAIN_WORLDS), contextId: context.id, intro: pick(random, context.intros) };
  }
  if (solveMode === "DSF-SM-TRAIN-TWO-CROSS-TIME") {
    const context = pick(random, TWO_TRAIN_CONTEXTS);
    return { solveMode, targetKind: "TRAIN_CROSS_TIME", anchor: pick(random, TWO_TRAIN_WORLDS), contextId: context.id, intro: pick(random, context.intros) };
  }
  const context = pick(random, BOAT_CONTEXTS);
  return { solveMode, targetKind: "BOAT_TRAVEL_TIME", anchor: pick(random, solveMode === "DSF-SM-BOAT-UPSTREAM-TIME" ? UPSTREAM_WORLDS : DOWNSTREAM_WORLDS), contextId: context.id, intro: pick(random, context.intros) };
}
...
function generationIdentity(seed: number, problem: TsdProblem, pair: SynthesizedPair): string {
  return createHash("sha256")
    .update(`${DSF_CP011_TSD_RUNTIME_VERSION}|${seed}|${problem.solveMode}|${problem.contextId}|${pair.statementI.id}|${pair.statementII.id}`)
    .digest("hex")
    .slice(0, 24);
}
...
export function generateDsfCp011TsdQuestion(seed: number) {
  ...
  if (!problem || !pair) throw lastError instanceof Error ? lastError : new Error(`Unable to synthesize TSD-001 DS question for seed ${seed}`);
