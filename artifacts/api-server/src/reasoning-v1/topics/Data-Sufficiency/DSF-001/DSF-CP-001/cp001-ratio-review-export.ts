...
function renderQuestion(question: DsfCp001RatioQuestion & { readonly editorialVersion: string }, ordinal: number): string {
  return `
...
const ratioCount = questions.filter((question) => question.targetKind === "RATIO_AB").length;
const comparisonCount = questions.length - ratioCount;

const html = `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>DSF CP-001 Ratio Review</title>
<style>
...
<section class="top"><h1>Data Sufficiency · CP-001 Ratio & Proportion Review</h1><p>English review candidate only. Publication remains locked.</p><div class="summary"><span>Questions: ${questions.length}</span><span>Ratio targets: ${ratioCount}</span><span>Comparison targets: ${comparisonCount}</span>${SUFFICIENCY_CLASSES.map((semanticClass) => `<span>${escapeHtml(semanticClass)}: ${classCounts[semanticClass]}</span>`).join("")}</div></section>
...
const htmlPath = resolve(outputDirectory, "dsf-cp001-ratio-review.html");
const jsonPath = resolve(outputDirectory, "dsf-cp001-ratio-review.json");
...
console.log(JSON.stringify({
  status: "PASS_DSF_CP_001_RATIO_REVIEW_EXPORT",
  htmlPath,
  jsonPath,
