...
assert(DSF_QL_BOUNDARY_CANDIDATES.every((candidate) => candidate.permanentQlId === null));
...
assert.equal(twoStatement.status, "INITIAL_ALLOCATION_READY");
...
assert.equal(DSF_INITIAL_QL_ALLOCATION_PLAN.permanentIdsAllocated, false);

console.log(JSON.stringify({
  status: "PASS_DSF_CP_000_OWNERSHIP_QL_BOUNDARY",
  ownership: dsfOwnershipSummary(),
  qlBoundary: {
    initialCandidate: twoStatement.candidateId,
