...
type StatementFamily =
  | "TARGET_EXACT" | "AB_EXACT" | "BC_EXACT" | "CD_EXACT" | "AC_EXACT" | "BD_EXACT" | "AD_EXACT"
  | "AB_BD_CHAIN" | "AC_CD_CHAIN" | "AB_BC_CHAIN" | "BC_CD_CHAIN"
  | "AB_NONLESS" | "BC_NONLESS" | "CD_NONLESS" | "A_D_EQUALITY_STATUS";

type InequalityWorld = Readonly<{ A: number; B: number; C: number; D: number; facts: readonly InequalityFact[] }>;
type InequalityProblem = Readonly<{ solveMode: DsfCp012InequalitySolveMode; anchor: InequalityWorld; contextId: ContextId; intro: string }>;
type InequalityStatement = Readonly<{ id: string; family: StatementFamily; complexity: 1 | 2 | 3; text: string; test: (world: InequalityWorld) => boolean }>;
type Pair = Readonly<{ statementI: InequalityStatement; statementII: InequalityStatement; evaluation: TwoStatementSufficiencyEvaluation<string>; quality: number }>;
...
function factForValues(left: SymbolId, right: SymbolId, leftValue: number, rightValue: number): InequalityFact {
  ...
  return leftValue > rightValue ? { left, relation: ">", right } : { left: right, relation: ">", right: left };
}
function completeFacts(values: Readonly<Record<SymbolId, number>>): readonly InequalityFact[] {
  const facts: InequalityFact[] = [];
  for (let first = 0; first < SYMBOLS.length; first += 1) for (let second = first + 1; second < SYMBOLS.length; second += 1) {
...
function enumerateWorlds(): readonly InequalityWorld[] {
  ...
  for (let A = 0; A <= 3; A += 1) for (let B = 0; B <= 3; B += 1) for (let C = 0; C <= 3; C += 1) for (let D = 0; D <= 3; D += 1) {
...
const adapter = {
  adapterId: "DSF-CP012-INEQUALITY-SOURCE-BOUND-V1", domainFamily: "REASONING" as const, sourceChapterId: "REAS-INEQ",
...
function hashSeed(seed: number, salt: string): number {
  let hash = 2166136261;
  for (const character of `${DSF_CP012_INEQUALITY_RUNTIME_VERSION}:${seed}:${salt}`) { hash ^= character.charCodeAt(0); hash = Math.imul(hash, 16777619); }
  return hash >>> 0;
}
...
function modeForSeed(seed: number): DsfCp012InequalitySolveMode { return DSF_CP012_INEQUALITY_SOLVE_MODES[Math.abs(seed) % DSF_CP012_INEQUALITY_SOLVE_MODES.length]!; }
...
function statement(id: string, family: StatementFamily, complexity: 1 | 2 | 3, text: string, test: (world: InequalityWorld) => boolean): InequalityStatement { return { id, family, complexity, text, test }; }
function relationText(left: SymbolId, relation: InequalityRelation, right: SymbolId): string { return `${left} ${relation} ${right}`; }
function relationPredicate(left: SymbolId, relation: InequalityRelation, right: SymbolId) { return (world: InequalityWorld) => directRelation(world, left, right) === relation; }
function nonLessPredicate(left: SymbolId, right: SymbolId) { return (world: InequalityWorld) => directRelation(world, left, right) !== "<"; }
function targetLabel(mode: DsfCp012InequalitySolveMode): string { const [left, right] = queryPair(mode); return `definite relation between ${left} and ${right}`; }
function promptFor(mode: DsfCp012InequalitySolveMode): string { const [left, right] = queryPair(mode); return `What is the definite relation between ${left} and ${right}?`; }
...
function buildStatementPool(problem: InequalityProblem): readonly InequalityStatement[] {
  ...
  const ab = directRelation(a, "A", "B"), bc = directRelation(a, "B", "C"), cd = directRelation(a, "C", "D");
  const ac = directRelation(a, "A", "C"), bd = directRelation(a, "B", "D"), ad = directRelation(a, "A", "D");
  return [
...
function normalizeSurface(text: string): string { return text.toLowerCase().replace(/[^a-z]+/g, " ").trim().replace(/\s+/g, " "); }
export function normalizeDsfCp012InequalitySurface(text: string): string { return normalizeSurface(text); }
...
export function generateDsfCp012InequalityQuestion(seed: number) {
  ...
  for (let attempt = 0; attempt < 32; attempt += 1) {
    ...
    try { pair = synthesizePair(candidate, seed + attempt * 104729, desiredClass); problem = candidate; break; }
  ...
  const together = !evaluation.statementI.sufficient && !evaluation.statementII.sufficient
    ...
    : undefined;
  const generationIdentity = createHash("sha256").update(`${DSF_CP012_INEQUALITY_RUNTIME_VERSION}|${seed}|${problem.solveMode}|${problem.contextId}|${pair.statementI.id}|${pair.statementII.id}`).digest("hex").slice(0, 24);
  return Object.freeze({
    packageId: "DSF-001" as const, checkpointId: "DSF-CP-012" as const, qlId: "DSF-QL-001" as const,
    runtimeVersion: DSF_CP012_INEQUALITY_RUNTIME_VERSION, seed, locale: "en-IN" as const, difficulty: difficultyFor(pair),
    domainFamily: "REASONING" as const, sourceChapterId: "REAS-INEQ" as const,
    ...
    proof: {
      baseWorldCount: INEQUALITY_WORLDS.length, statementIWorldCount: evaluation.statementI.worldCount, statementIIWorldCount: evaluation.statementII.worldCount,
      togetherWorldCount: evaluation.together.worldCount, statementITargetAnswers: evaluation.statementI.normalizedTargetAnswers,
      statementIITargetAnswers: evaluation.statementII.normalizedTargetAnswers, togetherTargetAnswers: evaluation.together.normalizedTargetAnswers,
...
export function generateDsfCp012InequalityBatch(seeds: readonly number[]) { return seeds.map(generateDsfCp012InequalityQuestion); }
