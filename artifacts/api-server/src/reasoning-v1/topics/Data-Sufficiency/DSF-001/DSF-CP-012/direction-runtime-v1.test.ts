...
import {
  DSF_CP012_DIRECTION_SOLVE_MODES,
  generateDsfCp012DirectionBatch,
  normalizeDsfCp012DirectionSurface,
} from "./direction-runtime-v1.ts";

const questions = generateDsfCp012DirectionBatch(Array.from({ length: 300 }, (_, seed) => seed));
...
assert.deepEqual(new Set(questions.map((question) => question.solveModeId)), new Set(DSF_CP012_DIRECTION_SOLVE_MODES));
...
assert.deepEqual(
  ...
  new Set(["WALKING_ROUTE", "DELIVERY_ROUTE", "CAMPUS_PATH", "PATROL_ROUTE", "WAREHOUSE_ROUTE", "FIELD_ROUTE"]),
);

for (const mode of DSF_CP012_DIRECTION_SOLVE_MODES) {
...
for (const question of questions) {
  ...
  assert.equal(question.sourceChapterId, "REAS-DIR");
  assert(question.sourceCapabilities.includes("lib/reasoning/spatial-reasoning::VectorPathState"));
  assert.equal(question.options.length, 5);
  assert.equal(question.options.filter((option) => option.isCorrect).length, 1);
  assert.equal(question.options[question.correctIndex]?.semanticClass, question.canonicalAnswer);
  assert.equal(question.proof.baseWorldCount, 432);
...
assert.equal(identities.size, questions.length, "Direction generation identities must remain unique");

const normalizedStems = new Set(questions.map((question) => normalizeDsfCp012DirectionSurface(question.stem)));
assert(normalizedStems.size >= 45, `Expected at least 45 normalized Direction stem surfaces, found ${normalizedStems.size}`);

const structuralFingerprints = new Set(questions.map((question) => question.studentSurfaceFingerprint));
assert(structuralFingerprints.size >= 100, `Expected at least 100 Direction structural fingerprints, found ${structuralFingerprints.size}`);
...
assert(largestFingerprintCluster <= 8, `A Direction structure repeated ${largestFingerprintCluster} times in the 300-question audit`);

console.log(JSON.stringify({
  status: "PASS_DSF_CP012_DIRECTION_SOURCE_BOUND_BREADTH_AUDIT",
  auditedQuestions: questions.length,
  solveModeCounts: Object.fromEntries(DSF_CP012_DIRECTION_SOLVE_MODES.map((mode) => [mode, questions.filter((question) => question.solveModeId === mode).length])),
