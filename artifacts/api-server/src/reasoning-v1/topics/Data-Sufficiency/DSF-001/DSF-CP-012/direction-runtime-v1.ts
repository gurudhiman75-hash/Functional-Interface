...
type StatementFamily =
  ...
  | "SECOND_TURN_EXACT"
  | "TURN_PAIR"
  | "FIRST_DISTANCE_EXACT"
  | "SECOND_DISTANCE_EXACT"
  | "THIRD_DISTANCE_EXACT"
  | "DISTANCE_PAIR"
  | "FINAL_X_EXACT"
  | "FINAL_Y_EXACT"
...
type DirectionProblem = Readonly<{
  solveMode: DsfCp012DirectionSolveMode;
  anchor: DirectionWorld;
  contextId: ContextId;
  intro: string;
}>;

type DirectionStatement = Readonly<{
  id: string;
  family: StatementFamily;
  complexity: 1 | 2 | 3;
  text: string;
  test: (world: DirectionWorld) => boolean;
}>;

type Pair = Readonly<{
  statementI: DirectionStatement;
  statementII: DirectionStatement;
...
function enumerateWorlds(): readonly DirectionWorld[] {
  ...
  for (const startFacing of facings) {
    for (const firstDistance of distances) {
      for (const firstTurn of turns) {
        for (const secondDistance of distances) {
          for (const secondTurn of turns) {
            for (const thirdDistance of distances) {
              ...
              worlds.push(Object.freeze({
                startFacing,
                startDegrees: FACING_TO_DEGREES[startFacing],
                firstDistance,
                firstTurn,
                secondDistance,
                secondTurn,
                ...
                shortestDistance: path.shortestDistance(),
                totalPath: firstDistance + secondDistance + thirdDistance,
              }));
            }
          }
        }
...
function signLabel(value: number): "positive" | "negative" | "zero" {
...
export function normalizeDsfCp012DirectionSurface(text: string): string {
...
export function generateDsfCp012DirectionQuestion(seed: number) {
  ...
  return Object.freeze({
    ...
    runtimeVersion: DSF_CP012_DIRECTION_RUNTIME_VERSION,
    seed,
    ...
    proof: {
      baseWorldCount: DIRECTION_WORLDS.length,
...
export function generateDsfCp012DirectionBatch(seeds: readonly number[]) {
  return seeds.map(generateDsfCp012DirectionQuestion);
}
