...
import {
  DSF_CP013_CODING_SOLVE_MODES,
  DSF_CP013_CODING_WORLD_COUNT,
  generateDsfCp013CodingBatch,
  normalizeDsfCp013CodingSurface,
} from "./coding-runtime-v1.ts";

assert.equal(DSF_CP013_CODING_WORLD_COUNT, 24, "COD-CP-001 direct-map universe must remain the complete 4! bijection domain");

const questions = generateDsfCp013CodingBatch(Array.from({ length: 300 }, (_, seed) => seed));
...
for (const question of questions) {
  ...
  assert.equal(question.sourceChapterId, "COD-001");
  assert.equal(question.sourceCheckpointId, "COD-CP-001");
  assert.equal(question.sourceWorldCount, 24);
  assert(question.sourceCapabilities.includes("COD-CP-001/independent-solver::solveCodCp001"));
  assert.equal(question.options.length, 5);
  assert.equal(question.options.filter((option) => option.isCorrect).length, 1);
  assert.equal(question.options[question.correctIndex]?.semanticClass, question.canonicalAnswer);
  assert.equal(question.proof.baseWorldCount, 24);
...
assert.equal(identities.size, questions.length, "Coding generation identities must remain unique");

const normalizedStems = new Set(questions.map((question) => normalizeDsfCp013CodingSurface(question.stem)));
assert(normalizedStems.size >= 45, `Expected at least 45 normalized Coding stem surfaces, found ${normalizedStems.size}`);

const structuralFingerprints = new Set(questions.map((question) => question.studentSurfaceFingerprint));
assert(structuralFingerprints.size >= 100, `Expected at least 100 Coding structural fingerprints, found ${structuralFingerprints.size}`);
...
assert(largestFingerprintCluster <= 8, `A Coding structure repeated ${largestFingerprintCluster} times in the 300-question audit`);

console.log(JSON.stringify({
  status: "PASS_DSF_CP013_CODING_SOURCE_BOUND_BREADTH_AUDIT",
