...
function provePrototype(prototypeId: FigureCompletionPrototypeV1): PrototypeProofResult {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS_PER_PROTOTYPE && accepted.length < TARGET_PER_PROTOTYPE; attempt += 1) {
    attempts += 1;
    const seed = `FGC-DISCOVERY:${prototypeId}:${String(attempt).padStart(4, "0")}`;
    ...
    const optionValidation = validateSpatialOptionUniqueness(question.options.map((option) => option.scene));
    assert(optionValidation.ok, `${prototypeId}: semantic option uniqueness failed.`);
    const perceptualValidation = validateSpatialPerceptualOptionUniquenessV2(question.options.map((option) => option.scene));
    assert(perceptualValidation.ok, `${prototypeId}: perceptual option uniqueness failed.`);
...
assert(allAccepted.length === FGC_001_PROTOTYPES_V1.length * TARGET_PER_PROTOTYPE, "FGC-001 total accepted count mismatch.");
assert(new Set(allAccepted.map((question) => question.contentFingerprint)).size === allAccepted.length, "FGC-001 cross-prototype content collision detected.");

const reviewQuestions = results.flatMap((result) => result.accepted.slice(0, REVIEW_PER_PROTOTYPE));
const proof = {
  version: "FGC-001-DISCOVERY-PROOF-V1",
  chapterCode: "FGC-001",
  status: "PASS_FGC_001_EXECUTABLE_DISCOVERY_V1",
  permanentQlCount: 0,
  sourceScope: {
...
const jsonPath = resolve(outputDir, "spa-fgc-001-discovery-v1-review.json");
const evidencePath = resolve(outputDir, "spa-fgc-001-discovery-v1-evidence.json");
const htmlPath = resolve(outputDir, "spa-fgc-001-discovery-v1-review.html");
