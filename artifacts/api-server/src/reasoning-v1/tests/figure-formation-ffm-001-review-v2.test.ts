...
import { generateFigureFormationReviewQuestionV2 } from "../foundation/spatial/figure-formation-review-runtime-v2";

const languages = ["en", "hi", "pa"] as const;
const qlIds = ["SPA-QL-051", "SPA-QL-052"] as const;
const seeds = Array.from({ length: 40 }, (_, index) => `ffm-review-v2-${String(index + 1).padStart(2, "0")}`);
...
for (const qlId of qlIds) {
  for (const seed of seeds) {
    ...
    for (const language of languages) {
      const question = generateFigureFormationReviewQuestionV2({ qlId, seed, language }) as any;
      ...
      const replay = generateFigureFormationReviewQuestionV2({ qlId, seed, language }) as any;
...
assert.ok(coverage.ql051TwoPieces && coverage.ql051ThreePieces, "QL051 must cover both 2-piece and 3-piece assembly.");
...
const reviewSeeds = Array.from({ length: 12 }, (_, index) => `ffm-visual-${String(index + 1).padStart(2, "0")}`);
const reviewQuestions = qlIds.flatMap((qlId) => reviewSeeds.map((seed) => generateFigureFormationReviewQuestionV2({ qlId, seed, language: "en" }) as any));
function card(question: any) {
  ...
  return `<article><header><strong>${question.qlId}</strong> · ${question.answerSurface}${question.targetKind ? ` · ${question.targetKind}` : ""} · ${question.difficultyBand}<br><small>${question.seed}</small></header><p>${question.stem}</p><div class="stimuli">${stimuli}</div><div class="options">${options}</div><p class="answer">Answer: ${question.answer}</p></article>`;
}
const html = `<!doctype html><html><head><meta charset="utf-8"><title>FFM-001 V2 visual review</title><style>body{font-family:Arial,sans-serif;margin:24px;background:#f5f6f8;color:#111827}h1{margin:0 0 20px}main{display:grid;grid-template-columns:repeat(auto-fit,minmax(620px,1fr));gap:18px}article{background:white;border:1px solid #d1d5db;border-radius:10px;padding:16px}.stimuli{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.stimulus svg{max-width:390px;height:auto}.options{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px}.option{border:1px solid #d1d5db;border-radius:8px;padding:8px;display:flex;align-items:center;gap:8px}.option svg{max-width:190px;height:auto}.answer{font-weight:700}small{color:#4b5563}</style></head><body><h1>FFM-001 — 24-question visual review V2</h1><main>${reviewQuestions.map(card).join("")}</main></body></html>`;

const outDir = resolve(process.cwd(), "dist/reasoning-v1/spatial");
mkdirSync(outDir, { recursive: true });
writeFileSync(resolve(outDir, "ffm-001-visual-review-v2.html"), html, "utf8");
const evidence = {
  status: "PASS_FFM_001_TWO_QL_REVIEW_V2",
...
writeFileSync(resolve(outDir, "ffm-001-review-v2-evidence.json"), `${JSON.stringify(evidence, null, 2)}\n`, "utf8");
