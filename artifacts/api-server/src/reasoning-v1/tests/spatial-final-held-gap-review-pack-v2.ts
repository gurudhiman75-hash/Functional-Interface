...
import { generateSpatialFinalHeldGapReviewQuestionV2 } from "../foundation/spatial/spatial-final-held-gap-review-runtime-v2";
import type {
...
const rows = qls.flatMap((qlId) => languages.flatMap((language) =>
  Array.from({ length: 4 }, (_, index) => generateSpatialFinalHeldGapReviewQuestionV2({
    qlId,
    language,
    seed: `spa-final-held-gap-review-v2:${qlId}:${index}`,
  })),
));
