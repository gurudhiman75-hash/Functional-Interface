...
const learnerExplanationText=review.pqls.flatMap((pql:any)=>pql.questions.flatMap((q:any)=>[
  q.learnerExplanation.observation,
  q.learnerExplanation.rule,
  q.learnerExplanation.application,
  q.learnerExplanation.check,
])).join("\n");
const reviewerMetaPatterns:RegExp[]=[
  /broad visible-feature audit/i,
  /\bparameter of the same\b/i,
  /\bnot a new question type\b/i,
  /\blearner method\b/i,
  /\bsource slice\b/i,
  /\bscale proof\b/i,
  /\bcoverage gate\b/i,
  /\bsemantic fingerprint\b/i,
  /\bperceptual fingerprint\b/i,
  /\bprototype id\b/i,
];
if(/\bOption [1-4]\b/.test(serialized)) throw new Error("Numeric option language remains in final human review JSON.");
for(const pattern of reviewerMetaPatterns){
  if(pattern.test(learnerExplanationText)) throw new Error(`Reviewer/meta language leaked into final learner explanations: ${pattern}.`);
}
if(/<div class="cap">Option [1-4]<\/div>/.test(html)) throw new Error("Numeric option captions remain in final human review HTML.");
const renderedLetteredOptionCaptions=(html.match(/<div class="cap">Option [A-D]<\/div>/g)??[]).length;
if(renderedLetteredOptionCaptions!==review.pqls.length*4*4) throw new Error(`Expected ${review.pqls.length*4*4} A-D option captions, found ${renderedLetteredOptionCaptions}.`);
console.log(JSON.stringify({status:"PASS_SPA_FND_001_PROPOSED_QL_HUMAN_REVIEW_EDITORIAL_CLEANUP_V1",numericOptionLanguageRemoved:true,reviewerJargonRemoved:true,learnerMetaLanguageRejected:true,renderedOptionCaptionsLettered:true},null,2));
