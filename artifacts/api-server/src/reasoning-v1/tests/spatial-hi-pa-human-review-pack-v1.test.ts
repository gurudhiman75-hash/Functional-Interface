...
function plainLanguageGate(question: ReturnType<typeof generateSpatialProductionStudioQuestionV1>) {
  if (question.language === "en") return;
  const text = learnerText(question);
  const hardTerms = question.language === "hi"
    ? ["रूपांतरण", "सादृश्य", "अंतर्वेशन", "अभिविन्यास", "संयोजकता", "प्रतिस्थापन", "सापेक्ष", "दृश्य-अवस्था", "निर्णायक", "घटक"]
    : ["ਰੂਪਾਂਤਰਨ", "ਸਾਦ੍ਰਿਸ਼ਤਾ", "ਸਾਪੇਖ", "ਸਥਾਨਾਂਤਰਨ", "ਪ੍ਰਤੀਸਥਾਪਨ", "ਨਿਰਣਾਇਕ", "ਘਟਕ"];
  const leaked = hardTerms.filter((term) => text.includes(term));
  assert(leaked.length === 0, `${question.qlId}/${question.language}: difficult learner jargon leaked: ${leaked.join(", ")}.`);
}
...
const samples = SPATIAL_PERMANENT_QL_ALLOCATIONS_V1.map((allocation) => {
  ...
  scriptGate(hi);
  scriptGate(pa);
  plainLanguageGate(hi);
  plainLanguageGate(pa);
  ...
  return {
    ...
    languages: {
      en: { questionLanguageId: en.questionLanguageId, qlName: en.qlName, stem: en.stem, explanation: en.explanation },
      hi: { questionLanguageId: hi.questionLanguageId, qlName: hi.qlName, stem: hi.stem, explanation: hi.explanation },
      pa: { questionLanguageId: pa.questionLanguageId, qlName: pa.qlName, stem: pa.stem, explanation: pa.explanation },
    },
  };
});
...
const sections = samples.map((sample, index) => {
  ...
  return `<section class="ql" id="${esc(sample.permanentQlId)}"><div class="ql-head"><div><div class="eyebrow">${index + 1} / 30 · ${esc(sample.chapterCode)} · ${esc(sample.difficulty)}</div><h2>${esc(sample.permanentQlId)} — ${esc(en.qlName)}</h2><div class="meta">Mode: ${esc(sample.mode)} · ${esc(sample.contentFingerprint)}</div></div><div class="answer-pill">Correct: ${esc(sample.correctOption)}</div></div>${sample.stimulusSvgs.length ? `<h3>Shared stimulus</h3>${figureStrip(sample.stimulusSvgs, "Figure")}` : ""}<h3>Shared options</h3>${figureStrip(sample.optionSvgs, "Option", sample.optionLabels)}<div class="language-grid">${languagePanel("English", { ...sample, ...en, language: "en", explanation: en.explanation } as any, { observation: "Observe", rule: "Rule", application: "Apply", check: "Check" })}${languagePanel("हिन्दी", { ...sample, ...hi, language: "hi", explanation: hi.explanation } as any, { observation: "क्या देखें", rule: "नियम", application: "कैसे लगाएँ", check: "जाँच" })}${languagePanel("ਪੰਜਾਬੀ", { ...sample, ...pa, language: "pa", explanation: pa.explanation } as any, { observation: "ਕੀ ਵੇਖਣਾ", rule: "ਨਿਯਮ", application: "ਕਿਵੇਂ ਲਗਾਉਣਾ", check: "ਜਾਂਚ" })}</div><div class="review-box"><strong>Human editorial review</strong><div class="checks"><label><input type="checkbox"> Meaning matches English exactly</label><label><input type="checkbox"> Hindi is easy on first read</label><label><input type="checkbox"> Punjabi is easy on first read</label><label><input type="checkbox"> Common exam-prep words are used</label><label><input type="checkbox"> Explanation matches this generated rule</label><label><input type="checkbox"> No formal/literal translation</label></div><div class="notes">Reviewer notes: ________________________________________________</div></div></section>`;
}).join("\n");
...
const reviewJson = {
  ...
  plainLanguagePolicy: {
    studentFirst: true,
    formalJargonBlocked: true,
    shortDirectInstructions: true,
  },
...
const evidence = {
  ...
  checks: {
    ...
    difficultJargonGate: true,
    studentFirstLanguagePolicy: true,
