...
import {
  PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2,
  generatePfcPermanentEnglishCorpusV2,
  generatePfcPermanentEnglishQlV2,
  generatePfcPermanentEnglishReviewQuestionsV2,
  renderPfcPermanentEnglishReviewHtmlV2,
} from "../foundation/spatial/paper-folding-permanent-english-runtime-v2";
import { renderPfcDiscoveryStimulusSvgV4 } from "../foundation/spatial/paper-folding-visual-taxonomy-remediation-v4";

const qlIds = ["SPA-QL-035", "SPA-QL-036", "SPA-QL-037", "SPA-QL-038"] as const;
const corpus = generatePfcPermanentEnglishCorpusV2();
assert.equal(corpus.length, 320);
assert.equal(PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2.questionsPerQl, 80);
assert.equal(PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2.totalQuestions, 320);
assert.equal(PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2.priorFreezeStatus, "SUPERSEDED_FOR_INTEGRATION_PENDING_V2_LEARNER_REVIEW");
...
const reviewQuestions = generatePfcPermanentEnglishReviewQuestionsV2();
...
for (const expected of Object.values(expectedCoverageByQl).flat()) {
  assert.ok(reviewCoverage.has(expected), `V2 review pack missing ${expected}`);
}
const reviewHtml = renderPfcPermanentEnglishReviewHtmlV2(reviewQuestions);
assert.ok(reviewHtml.includes("PFC-001 Permanent English Review V2"));
assert.ok(reviewHtml.includes("background:#fff"));
assert.ok(!reviewHtml.includes("background:#f5f5f5"));
assert.ok(reviewHtml.includes("TRIANGLE_CUT") || reviewHtml.includes("MIXED_HOLE_TRIANGLE_CUT"));
...
const evidence = {
  authority: PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2,
  status: "PASS_PFC_001_PERMANENT_ENGLISH_RUNTIME_V2_REVIEW_CANDIDATE",
  corpus: {
  ...
  review: {
    ...
    optionPixels: 124,
    stimulusPixels: 640,
    file: "spa-pfc-001-permanent-english-runtime-v2-review.html",
  },
  governance: {
    priorEnglishFreezeSupersededForIntegration: true,
    v2LearnerReviewRequired: true,
    v2Frozen: false,
...
writeFileSync(
  "dist/reasoning-v1/spatial/spa-pfc-001-permanent-english-runtime-v2-evidence.json",
  `${JSON.stringify(evidence, null, 2)}\n`,
  "utf8",
);
writeFileSync(
  "dist/reasoning-v1/spatial/spa-pfc-001-permanent-english-runtime-v2-review.html",
  reviewHtml,
  "utf8",
);
