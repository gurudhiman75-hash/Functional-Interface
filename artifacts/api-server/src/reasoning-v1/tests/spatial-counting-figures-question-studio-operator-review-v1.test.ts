...
import {
  ...
  type CountingFiguresStudioLanguageV1,
...
type ReviewPoolItem = Readonly<{
  seed: string;
  inputMode: "EXPLICIT_TARGET" | "IMPLICIT_TARGET";
  targetShape?: CountingFigureTargetShapeV1;
  studio: ReturnType<typeof generateCountingFiguresQuestionStudioSeededV1>;
  source: ReturnType<typeof generateCountingFiguresPermanentEnglishQuestionV1>;
}>;

function explicitItem(index: number): ReviewPoolItem {
  const targetShape = TARGETS[index % TARGETS.length]!;
  const seed = `FCT-QS-OPERATOR-EXPLICIT-${index}`;
  const studio = generateCountingFiguresQuestionStudioSeededV1({ seed, language: "en", targetShape });
  ...
  return Object.freeze({ seed, inputMode: "EXPLICIT_TARGET", targetShape, studio, source });
}

function implicitItem(index: number): ReviewPoolItem {
  const seed = `FCT-QS-OPERATOR-IMPLICIT-${index}`;
  ...
  assert.equal(studio.geometryFingerprint, source.geometryFingerprint);
  assert.equal(studio.contentFingerprint, source.contentFingerprint);
  return Object.freeze({ seed, inputMode: "IMPLICIT_TARGET", studio, source });
}

const explicitPool = Object.freeze(Array.from({ length: 480 }, (_, index) => explicitItem(index)));
const implicitPool = Object.freeze(Array.from({ length: 240 }, (_, index) => implicitItem(index)));
const pool = Object.freeze([...explicitPool, ...implicitPool]);

assert.equal(new Set(explicitPool.map((item) => item.source.motifFamily)).size, 11, "Explicit-target review corpus must expose all 11 FCT motif families.");
assert.equal(new Set(explicitPool.map((item) => item.source.targetShape)).size, 4);
assert.equal(new Set(pool.map((item) => item.source.difficulty)).size, 3);
assert.equal(new Set(pool.map((item) => item.source.stemVariant)).size, 8);
assert.equal(new Set(pool.map((item) => item.source.correctIndex)).size, 4);

const required = new Set<string>();
for (const motif of new Set(explicitPool.map((item) => item.source.motifFamily))) required.add(`motif:${motif}`);
...
function coverageTokens(item: ReviewPoolItem): readonly string[] {
  return [
...
const selected: ReviewPoolItem[] = [];
const selectedSeeds = new Set<string>();
const uncovered = new Set(required);
const add = (item: ReviewPoolItem | undefined) => {
...
while (uncovered.size) {
  let best: ReviewPoolItem | undefined;
  let score = -1;
  for (const item of explicitPool) {
...
for (const motif of new Set(explicitPool.map((item) => item.source.motifFamily))) {
  while (selected.filter((item) => item.source.motifFamily === motif).length < 2) {
    add(explicitPool.find((item) => item.source.motifFamily === motif && !selectedSeeds.has(item.seed)));
  }
}
for (const targetShape of TARGETS) {
  while (selected.filter((item) => item.source.targetShape === targetShape).length < 4) {
    add(explicitPool.find((item) => item.source.targetShape === targetShape && !selectedSeeds.has(item.seed)));
  }
}
for (const item of implicitPool) {
  if (selected.filter((candidate) => candidate.inputMode === "IMPLICIT_TARGET").length >= 6) break;
  add(item);
}
for (const item of explicitPool) {
...
assert.ok(selected.length >= 28 && selected.length <= 34, `Expected 28..34 operator-review questions, got ${selected.length}.`);
...
assert.equal(new Set(selected.map((item) => item.source.geometryFingerprint)).size, selected.length);
assert.ok(selected.filter((item) => item.inputMode === "IMPLICIT_TARGET").length >= 6);

function studioSurface(item: ReviewPoolItem, language: CountingFiguresStudioLanguageV1) {
  return item.inputMode === "IMPLICIT_TARGET"
    ? generateCountingFiguresQuestionStudioSeededV1({ seed: item.seed, language })
    : generateCountingFiguresQuestionStudioSeededV1({ seed: item.seed, language, targetShape: item.targetShape });
}
...
const cards = selected.map((item, index) => {
  ...
  const en = studioSurface(item, "en");
  const hi = studioSurface(item, "hi");
  const pa = studioSurface(item, "pa");
...
const evidence = {
  ...
  corpus: {
    explicitTargetPoolCount: explicitPool.length,
    implicitTargetPoolCount: implicitPool.length,
    explicitTargetMotifFamilyCount: new Set(explicitPool.map((item) => item.source.motifFamily)).size,
    implicitTargetMotifFamilyCount: new Set(implicitPool.map((item) => item.source.motifFamily)).size,
  },
  coverage: {
    ...
    explicitTargetQuestionCount: selected.filter((item) => item.inputMode === "EXPLICIT_TARGET").length,
    implicitTargetQuestionCount: selected.filter((item) => item.inputMode === "IMPLICIT_TARGET").length,
  },
  governance: {
