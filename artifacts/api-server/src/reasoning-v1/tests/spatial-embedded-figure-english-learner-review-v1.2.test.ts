...
function reviewOverlaySvg(graph: EmbeddedGraphV1, matchedEdgeIds: ReadonlySet<string>): string {
  ...
  const renderEdge = (edge: EmbeddedEdgeV1, highlighted: boolean): string => {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120" role="img" aria-label="review exact-path overlay"><rect width="120" height="120" fill="white"/>${graph.edges.filter((edge) => !matchedEdgeIds.has(edge.id)).map((edge) => renderEdge(edge, false)).join("")}${graph.edges.filter((edge) => matchedEdgeIds.has(edge.id)).map((edge) => renderEdge(edge, true)).join("")}</svg>`;
}

const candidates = Object.freeze(Array.from({ length: 720 }, (_, index) => generateEmbeddedFigureWholeOptionConnectivityQuestionV1(`EMB-WOC-REVIEW-${index}`)));
const selected: EmbeddedWholeOptionConnectivityQuestionV1[] = [];
const selectedSeeds = new Set<string>();
const add = (question: EmbeddedWholeOptionConnectivityQuestionV1 | undefined) => {
  assert(question, "Unable to satisfy EMB-001 V1.2 learner-review coverage from deterministic pool.");
...
add(candidates.find((question) => question.connectivityValidation.sourceComponentCounts.some((count) => count > 1)));
...
const connectivityRemediationExamples = reviewQuestions.filter((question) => question.connectivityValidation.sourceComponentCounts.some((count) => count > 1));
...
assert(connectivityRemediationExamples.length > 0, "Review pack does not exercise whole-option connectivity remediation.");

let overlayChecks = 0;
let wholeOptionConnectivityChecks = 0;
let bridgePathExamples = 0;
const cards = reviewQuestions.map((question, reviewIndex) => {
  ...
  overlayChecks += 1;
  wholeOptionConnectivityChecks += question.connectivityValidation.finalComponentCounts.filter((count) => count === 1).length;
  bridgePathExamples += question.connectivityValidation.bridgePathCounts.reduce((sum, count) => sum + count, 0);
  ...
  return `<article class="card">
    ...
    <div class="target-row"><div><h3>Question figure</h3><div class="figure target">${question.targetSvg}</div></div><div class="meta">Motif: ${escapeHtml(question.motifId)}<br/>Policy: ${question.equivalencePolicy}<br/>Source components A–D: ${question.connectivityValidation.sourceComponentCounts.join(" / ")}<br/>Final components A–D: ${question.connectivityValidation.finalComponentCounts.join(" / ")}<br/>Bridge paths A–D: ${question.connectivityValidation.bridgePathCounts.join(" / ")}</div></div>
    <h3>Options</h3><div class="options">${options}</div>
    <details><summary>Reviewer answer and exact-path proof</summary><div class="solution"><div><div class="figure overlay">${overlay}</div><p>Teal = exact target path in option ${question.answer}; grey = concealment/connectivity geometry. Review evidence only.</p></div><div><p><strong>Answer:</strong> ${question.answer}</p><p><strong>Observe:</strong> ${escapeHtml(question.explanation.observation)}</p><p><strong>Rule:</strong> ${escapeHtml(question.explanation.rule)}</p><p><strong>Apply:</strong> ${escapeHtml(question.explanation.application)}</p><p><strong>Check:</strong> ${escapeHtml(question.explanation.check)}</p></div></div></details>
  </article>`;
}).join("\n");

assert(wholeOptionConnectivityChecks === reviewQuestions.length * 4, `Expected ${reviewQuestions.length * 4} connected-option checks, got ${wholeOptionConnectivityChecks}.`);
assert(bridgePathExamples > 0, "Review pack contains no visible connectivity-remediation examples.");

const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>EMB-001 English Learner Review V1.2</title><style>
...
</style></head><body><main><section class="intro"><h1>EMB-001 · English Learner Review V1.2</h1><p class="muted">Whole-option connectivity remediation candidate. ${reviewQuestions.length} deterministic questions cover all 8 motif families × L1/L2/L3, all 8 stems, all answer positions and all five misconception-owned distractor families. Every option is one connected figure. Detached source islands, when present, are repaired with bent two-edge paths and then re-solved. Permanent QLs remain zero.</p></section>${cards}</main></body></html>`;

mkdirSync("dist/reasoning-v1/spatial", { recursive: true });
writeFileSync("dist/reasoning-v1/spatial/spa-emb-001-english-learner-review-v1.2.html", html);
const evidence = {
  status: "PASS_EMB_001_ENGLISH_LEARNER_REVIEW_V1_2_CANDIDATE",
  authorityId: "EMB-001-ENGLISH-LEARNER-REVIEW-V1.2-CANDIDATE",
  ...
  overlayChecks,
  wholeOptionConnectivityChecks,
  connectivityRemediationQuestionCount: connectivityRemediationExamples.length,
  bridgePathExamples,
  governance: {
    candidateOnly: true,
    englishFrozen: false,
  ...
  nextGate: "DIRECT_VISUAL_REVIEW_OF_ALL_V1_2_ITEMS_THEN_ENGLISH_FREEZE_AND_PERMANENT_QL_PROPOSAL",
};
writeFileSync("dist/reasoning-v1/spatial/spa-emb-001-english-learner-review-v1.2-evidence.json", JSON.stringify(evidence, null, 2));
