...
function provePrototype(prototypeId: FigureCompletionPrototypeV1): PrototypeProofResult {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS_PER_PROTOTYPE && accepted.length < TARGET_PER_PROTOTYPE; attempt += 1) {
    attempts += 1;
    const seed = `FGC-DISCOVERY-V2-HARDENED:${prototypeId}:${String(attempt).padStart(4, "0")}`;
    ...
    question.options.forEach((option, optionIndex) => renderSpatialSceneToSvg(option.scene, { ariaLabel: `Figure completion option ${optionIndex + 1}` }));
...
assert(allAccepted.length === FGC_001_PROTOTYPES_V1.length * TARGET_PER_PROTOTYPE, "FGC-001 hardened V2 total accepted count mismatch.");
assert(new Set(allAccepted.map((question) => question.contentFingerprint)).size === allAccepted.length, "FGC-001 hardened V2 cross-prototype content collision detected.");

const reviewQuestions = results.flatMap((result) => result.accepted.slice(0, REVIEW_PER_PROTOTYPE));
const proof = {
  version: "FGC-001-DISCOVERY-PROOF-V2-HARDENED",
  chapterCode: "FGC-001",
  status: "PASS_FGC_001_EXECUTABLE_DISCOVERY_V2_HARDENED",
  permanentQlCount: 0,
  remediation: {
    centeredMissingWindow: true,
    boundaryAnglesContinuous: true,
    bentPathBoundarySeparationMinUnits: 4,
    junctionArmsCollinear: true,
    markerPatternEqualSpacing: true,
    markerOmittedDistractorActuallyOmitsMarker: true,
  },
  sourceScope: { SSC: "INITIAL_ANCHOR_ESTABLISHED", Banking: "NOT_SATURATED", PunjabState: "NOT_ESTABLISHED" },
