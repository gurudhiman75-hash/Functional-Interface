...
const cards = questions.map((q, i) => {
  ...
  return `<section class="question">
    <div class="meta"><strong>${i + 1}. ${q.qlId}</strong><span>${q.familyLabel}</span><span>${q.solveFacts.sourceVariant.replaceAll("_", " ")}</span><span>${q.difficulty}</span><span>${q.matrixSize}×${q.matrixSize}</span>${retryBadge}</div>
