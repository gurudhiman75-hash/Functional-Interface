...
for (const taskId of taskIds) {
  ...
  if (disposition.disposition === "HOLD_FOR_ADVANCED_SOURCE_CONFIRMATION") {
    if (source.evidenceLevel === "DIRECT_SOURCE" || source.evidenceLevel === "DIRECT_MULTI_SOURCE") assert.equal(gap.status, "UNRESOLVED_SOURCE_BACKED_HOLD");
    else assert.equal(gap.status, "INTENTIONAL_ADVANCED_HOLD");
  }
}
...
assert.equal(summary.declaredSourceRegistrySaturationComplete, true);
assert.equal(summary.sourceSaturationComplete, false);
assert.equal(summary.semanticDifficultyAuditComplete, true);
assert.equal(summary.itemLevelDifficultyModelImplemented, true);
assert.equal(summary.difficultyAuditComplete, false);
assert.equal(summary.multilingualRiskAuditComplete, true);
assert.deepEqual(summary.blockingGates, ["SOURCE_SATURATION_SIGN_OFF", "ITEM_LEVEL_DIFFICULTY_HUMAN_CALIBRATION"]);
...
assert.equal(CLOCK_EFFECTIVE_DISCOVERY_GATE_POLICY.status, "TECHNICAL_DISCOVERY_AUDITS_COMPLETE__HUMAN_GATES_OPEN");
...
assert.equal(CLOCK_EFFECTIVE_DISCOVERY_GATE_POLICY.declaredSourceRegistrySaturationComplete, true);
assert.equal(CLOCK_EFFECTIVE_DISCOVERY_GATE_POLICY.sourceSaturationComplete, false);
assert.equal(CLOCK_EFFECTIVE_DISCOVERY_GATE_POLICY.semanticDifficultyAuditComplete, true);
assert.equal(CLOCK_EFFECTIVE_DISCOVERY_GATE_POLICY.itemLevelDifficultyModelImplemented, true);
assert.equal(CLOCK_EFFECTIVE_DISCOVERY_GATE_POLICY.difficultyHumanCalibrationRequired, true);
...
assert.equal(CLOCK_EFFECTIVE_DISCOVERY_GATE_POLICY.noUnexplainedDeclaredSourceFamily, true);
...
console.log(JSON.stringify({
  status: "PASS_CLK_001_TECHNICAL_DISCOVERY_AUDITS__HUMAN_GATES_OPEN",
  sourceCandidateRows: taskIds.length,
  effectiveAuthorityClusters: authorityClusters.length,
  declaredSourceRegistrySaturationComplete: summary.declaredSourceRegistrySaturationComplete,
  sourceSaturationComplete: summary.sourceSaturationComplete,
  semanticDifficultyAuditComplete: summary.semanticDifficultyAuditComplete,
  itemLevelDifficultyModelImplemented: summary.itemLevelDifficultyModelImplemented,
