...
for (const qlId of qlIds) {
  for (const seed of seeds) {
    const generationSeed = `FFM-V4:${qlId}:${seed}`;
    const question = generateFigureFormationReviewQuestionV4Final({ qlId, seed: generationSeed, language: "en" }) as any;
    ...
    manifest.push({
      ...
      geometryScalePolicy: question.renderer.reviewGeometryScalePolicy,
      illustratedExplanation: question.renderer.reviewIllustratedExplanation,
      learnerContentFrozen: question.review.learnerContentFrozen,
    });
  }
}

const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>FFM-001 Visual Review V4</title><style>
*{box-sizing:border-box}body{margin:0;background:#f3f4f6;color:#111827;font-family:Arial,sans-serif}main{max-width:1500px;margin:0 auto;padding:22px}.hero{background:white;border:1px solid #d1d5db;border-radius:10px;padding:16px 18px;margin-bottom:20px}.hero h1{margin:0 0 8px;font-size:24px}.hero p{margin:5px 0;color:#4b5563;line-height:1.5}.warning{font-weight:700;color:#111827}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,620px),1fr));gap:20px}.card{background:white;border:1px solid #cbd5e1;border-radius:10px;padding:17px;overflow:hidden}header{display:flex;justify-content:space-between;border-bottom:1px solid #e5e7eb;padding-bottom:9px}.stem{font-size:15px;line-height:1.5;min-height:44px}.stimulus{display:flex;justify-content:center;align-items:center;min-height:150px;margin:8px 0 16px;overflow:auto;padding:4px}svg{height:auto;max-width:100%}.options{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.option{position:relative;border:1px solid #cbd5e1;border-radius:8px;padding:25px 8px 10px;min-height:145px;display:flex;align-items:center;justify-content:center;background:white;overflow:auto}.option.correct{outline:2px solid #6b7280}.label{position:absolute;top:6px;left:8px;font-size:12px;font-weight:700}.explanation{margin-top:16px;border-top:1px solid #d1d5db;padding-top:14px}.explanation h3{font-size:14px;margin:0 0 9px}.solution-visual{display:flex;justify-content:center;overflow:auto;background:#fff;border:1px solid #e5e7eb;border-radius:7px;padding:6px}.explanation ol{margin:12px 0 8px;padding-left:22px}.explanation li{font-size:13px;line-height:1.5;margin:5px 0}.answer-check{font-size:13px;line-height:1.5;margin:8px 0 0;background:#f9fafb;border-left:3px solid #9ca3af;padding:8px 10px}@media(max-width:700px){main{padding:12px}.options{grid-template-columns:1fr}.option{min-height:140px}.grid{grid-template-columns:1fr}.card{padding:13px}}
</style></head><body><main><section class="hero"><h1>FFM-001 Figure Formation — Visual Review V4</h1><p class="warning">V3 rejected: independent auto-scaling created proportion ambiguity and explanations lacked usable illustrations.</p><p>V4 hard-locks one geometric unit per question, removes independent piece stretching, keeps 1.35px exam strokes on white, and adds a visible illustrated solution for every review card. This remains review-only: no Question Studio, Question Bank, Test Builder or student activation.</p></section><div class="grid">${cards.join("\n")}</div></main></body></html>`;

const outDir = resolve(process.cwd(), "dist/reasoning-v1/spatial");
mkdirSync(outDir, { recursive: true });
writeFileSync(resolve(outDir, "spa-ffm-001-visual-review-v4.html"), html, "utf8");
writeFileSync(resolve(outDir, "spa-ffm-001-visual-review-v4.json"), `${JSON.stringify({ cards: manifest, cardCount: manifest.length, svgCount }, null, 2)}\n`, "utf8");
console.log(JSON.stringify({ status: "PASS_FFM_001_VISUAL_REVIEW_V4_PACK_GENERATED", cardCount: manifest.length, svgCount }, null, 2));
