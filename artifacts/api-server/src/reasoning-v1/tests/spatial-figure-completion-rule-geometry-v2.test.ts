...
function isRetryableGenerationReject(error: unknown): boolean {
  ...
  return error.message.includes("semantically equivalent completion options") || error.message.includes("perceptually equivalent completion options");
}

function samePoint(left: SpatialPoint, right: SpatialPoint): boolean {
  return Math.abs(left.x - right.x) <= EPSILON && Math.abs(left.y - right.y) <= EPSILON;
}
function vector(from: SpatialPoint, to: SpatialPoint): SpatialPoint { return { x: to.x - from.x, y: to.y - from.y }; }
function cross(left: SpatialPoint, right: SpatialPoint): number { return left.x * right.y - left.y * right.x; }
function dot(left: SpatialPoint, right: SpatialPoint): number { return left.x * right.x + left.y * right.y; }
function translated(point: SpatialPoint): SpatialPoint { return { x: point.x + PATCH_ORIGIN.x, y: point.y + PATCH_ORIGIN.y }; }
...
interface BoundaryTangent { boundary: SpatialPoint; interior: SpatialPoint; }
...
function fragmentBoundaryTangents(node: SpatialNode): BoundaryTangent[] {
  ...
  if (node.kind === "polyline") {
    ...
    const result: BoundaryTangent[] = [];
    if (points.length >= 2 && onPatchBoundary(points[0]!)) result.push({ boundary: points[0]!, interior: points[1]! });
    const last = points.length - 1;
    if (points.length >= 2 && onPatchBoundary(points[last]!)) result.push({ boundary: points[last]!, interior: points[last - 1]! });
    return result;
  }
  return [];
}
...
function contextBoundaryTangents(node: SpatialNode): BoundaryTangent[] {
  ...
  if (node.kind === "polyline") {
    const result: BoundaryTangent[] = [];
    if (node.points.length >= 2 && onPatchBoundary(node.points[0]!)) result.push({ boundary: node.points[0]!, interior: node.points[1]! });
    const last = node.points.length - 1;
    if (node.points.length >= 2 && onPatchBoundary(node.points[last]!)) result.push({ boundary: node.points[last]!, interior: node.points[last - 1]! });
    return result;
  }
  return [];
}
...
function assertMarkerPattern(question: FigureCompletionQuestionV1): void {
  ...
  const markers = [...visibleMarkers.map((node) => (node as Extract<SpatialNode, { kind: "circle" }>).center), hidden].sort((left, right) => left.x - right.x);
  ...
  for (const marker of markers) assert(pointLineResidual(marker, globalStart, globalEnd) <= EPSILON, `${question.seed}: marker is not on the continued path.`);
...
for (const prototypeId of FGC_001_PROTOTYPES_V1) {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS_PER_PROTOTYPE && accepted < ACCEPTED_PER_PROTOTYPE; attempt += 1) {
    const seed = `FGC-RULE-GEOMETRY-HARDENED:${prototypeId}:${String(attempt).padStart(4, "0")}`;
    ...
    try {
      ...
      if (isRetryableGenerationReject(error)) { generationRejects += 1; continue; }
      throw error;
    }
...
console.log(JSON.stringify({
  status: "PASS_FGC_001_RULE_GEOMETRY_V2_HARDENED",
  ...
  guarantees: [
    ...
    "bent-path boundary cues remain separated rather than collapsing into a false junction",
