...
import {
  PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2_1,
  generatePfcPermanentEnglishCorpusV2_1,
  generatePfcPermanentEnglishQlV2_1,
  generatePfcPermanentEnglishReviewQuestionsV2_1,
  renderPfcPermanentEnglishReviewHtmlV2_1,
} from "../foundation/spatial/paper-folding-permanent-english-runtime-v2-1";
import { renderPfcDiscoveryStimulusSvgV4 } from "../foundation/spatial/paper-folding-visual-taxonomy-remediation-v4-1";

const qlIds = ["SPA-QL-035", "SPA-QL-036", "SPA-QL-037", "SPA-QL-038"] as const;
const corpus = generatePfcPermanentEnglishCorpusV2_1();
assert.equal(corpus.length, 320);
assert.equal(PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2_1.questionsPerQl, 80);
assert.equal(PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2_1.totalQuestions, 320);
assert.equal(PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2_1.remediationAuthority, "PFC-001-VISUAL-TAXONOMY-REMEDIATION-V4.1");
assert.equal(PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2_1.priorFreezeStatus, "SUPERSEDED_FOR_INTEGRATION_PENDING_V2_1_LEARNER_REVIEW");
...
const reviewQuestions = generatePfcPermanentEnglishReviewQuestionsV2_1();
...
for (const expected of Object.values(expectedCoverageByQl).flat()) {
  assert.ok(reviewCoverage.has(expected), `V2.1 review pack missing ${expected}`);
}
const reviewHtml = renderPfcPermanentEnglishReviewHtmlV2_1(reviewQuestions);
assert.ok(reviewHtml.includes("PFC-001 Permanent English Review V2.1"));
...
const evidence = {
  authority: PFC_001_PERMANENT_ENGLISH_RUNTIME_AUTHORITY_V2_1,
  status: "PASS_PFC_001_PERMANENT_ENGLISH_RUNTIME_V2_1_REVIEW_CANDIDATE",
  corpus: {
  ...
  review: {
    ...
    optionPixels: 124,
    stimulusPixels: 640,
    file: "spa-pfc-001-permanent-english-runtime-v2-1-review.html",
  },
  governance: {
    priorEnglishFreezeSupersededForIntegration: true,
    v2_1LearnerReviewRequired: true,
    v2_1Frozen: false,
...
writeFileSync(
  "dist/reasoning-v1/spatial/spa-pfc-001-permanent-english-runtime-v2-1-evidence.json",
  `${JSON.stringify(evidence, null, 2)}\n`,
  "utf8",
);
writeFileSync(
  "dist/reasoning-v1/spatial/spa-pfc-001-permanent-english-runtime-v2-1-review.html",
  reviewHtml,
  "utf8",
);
