...
const CURRENT_MAIN_BASE = "c2f25062aa534ec52d90cd491eb2e2123e2e933c" as const;
...
assert.equal(
  EMBEDDED_FIGURE_QUESTION_STUDIO_PRODUCT_OWNER_APPROVAL_V1.governance.standardQuestionStudioRegistrationAuthorized,
  true,
);
assert.equal(SPATIAL_PERMANENT_QL_ALLOCATION_AUTHORITY_V5.permanentQlCount, 41);
assert.equal(SPATIAL_QUESTION_STUDIO_PACKAGE_V2.permanentQlCount, 40, "Current-main inherited V2 package drifted.");
...
assert.ok(
  ...
  "Current-main Spatial admin route is not wired to integration V3.",
);
assert.ok(
  routeSource.includes('from "../reasoning-v1/foundation/spatial/spatial-question-studio-production-v3"'),
  "Current-main Spatial admin route is not wired to production V3.",
);
...
for (let index = 0; index < 24; index += 1) {
  const seed = `EMB-CURRENT-MAIN-${index}`;
  const source = generateEmbeddedFigurePermanentEnglishQuestionV1(seed);
  const questions = LANGUAGES.map((language) =>
    generateSpatialProductionStudioQuestionV3({ qlId: EMB_QL, seed, language }),
  );
  const [en, hi, pa] = questions;
  assert.ok(en && hi && pa);

  const solved = source.optionGraphs.map((option) =>
    matchEmbeddedGraphV1(source.targetGraph, option, "FIXED_ORIENTATION"),
  );
  const solvedIndices = solved
    .map((result, optionIndex) => (result.matched ? optionIndex : -1))
    .filter((optionIndex) => optionIndex >= 0);
  assert.deepEqual(solvedIndices, [source.correctIndex], `${seed}: exact solver found a non-unique answer.`);
  solverOptionChecks += 4;

  assert.deepEqual(projection(hi), projection(en), `${seed}: Hindi geometry/answer parity drifted.`);
  assert.deepEqual(projection(pa), projection(en), `${seed}: Punjabi geometry/answer parity drifted.`);
  ...
  assert.equal(getGeneratedQuestionBankEligibilityIssue(payload), null, `${seed}: QB eligibility contract drifted.`);
  ...
  const normalized = normalizeGeneratedQuestionPayload(payload, {
    itemId: seed,
    generationRunCode: "EMB-CURRENT-MAIN-INTEGRATION",
  });
  ...
  assert.deepEqual(
    generateSpatialProductionStudioQuestionV3({ qlId: EMB_QL, seed, language: "pa" }),
    pa,
    `${seed}: deterministic replay failed.`,
  );
}
...
const evidence = {
  status: "PASS_EMB_001_CURRENT_NEW_MAIN_INTEGRATION_V1",
  ...
  nextGate: "EMB_001_CURRENT_MAIN_INTEGRATION_DECISION",
};

mkdirSync("dist/reasoning-v1/spatial", { recursive: true });
writeFileSync(
  "dist/reasoning-v1/spatial/spa-emb-001-current-main-integration-v1-evidence.json",
