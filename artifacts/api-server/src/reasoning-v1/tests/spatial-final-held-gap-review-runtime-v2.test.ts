...
} from "../foundation/spatial/spatial-final-held-gap-review-runtime-v2";
import type {
...
assert.equal(SPATIAL_FINAL_HELD_GAP_REVIEW_RUNTIME_AUTHORITY_V2.rendererPolicy.stroke, "#111827");
assert.equal(SPATIAL_FINAL_HELD_GAP_REVIEW_RUNTIME_AUTHORITY_V2.rendererPolicy.strokeWidth, 1.35);
assert.equal(SPATIAL_FINAL_HELD_GAP_REVIEW_RUNTIME_AUTHORITY_V2.rendererPolicy.legacyEmbeddedStrokeSuppressed, true);
assert.equal(SPATIAL_FINAL_HELD_GAP_REVIEW_RUNTIME_AUTHORITY_V2.lifecycle.questionStudioDiscoverable, false);
assert.equal(SPATIAL_FINAL_HELD_GAP_REVIEW_RUNTIME_AUTHORITY_V2.lifecycle.questionBankWritable, false);
assert.equal(SPATIAL_FINAL_HELD_GAP_REVIEW_RUNTIME_AUTHORITY_V2.lifecycle.testEligible, false);
assert.equal(SPATIAL_FINAL_HELD_GAP_REVIEW_RUNTIME_AUTHORITY_V2.lifecycle.automaticStudentPublication, false);
...
function assertCopy(question: ReturnType<typeof generateSpatialFinalHeldGapReviewQuestionV2>, label: string): void {
  ...
  for (const term of ["solver-attested", "renderer authority", "runtime proof", "geometry fingerprint", "canonical fingerprint"]) {
...
function assertLocked(question: ReturnType<typeof generateSpatialFinalHeldGapReviewQuestionV2>, label: string): void {
...
for (const qlId of qls) {
  for (let index = 0; index < 48; index += 1) {
    const seed = `spa-final-v2:${qlId}:${index}`;
    const question = generateSpatialFinalHeldGapReviewQuestionV2({ qlId, seed, language: "en" });
    const replay = generateSpatialFinalHeldGapReviewQuestionV2({ qlId, seed, language: "en" });
  ...
  for (const language of languages) {
    for (let index = 0; index < 6; index += 1) {
      const question = generateSpatialFinalHeldGapReviewQuestionV2({
        qlId,
        seed: `spa-final-v2-locale:${qlId}:${index}`,
        language,
      });
...
assert.deepEqual([...metrics.ql048Motifs].sort(), ["CROSSHATCH", "ORTHOGONAL_GRID", "RADIAL_DIAMETERS"]);
assert(metrics.ql048Counts.size >= 6, "QL048 count diversity is too thin.");
assert.deepEqual([...metrics.ql049Targets].sort(), ["CIRCLE", "SEMICIRCLE"]);
assert.deepEqual([...metrics.ql049Motifs].sort(), ["CIRCLE_ARRAY", "CONCENTRIC_RINGS", "NESTED_SEMICIRCLES", "SEMICIRCLE_ARRAY"]);
assert(metrics.ql049Counts.size >= 6, "QL049 count diversity is too thin.");
...
const evidence = {
  status: "PASS_SPA_FINAL_HELD_GAP_REVIEW_RUNTIME_V2",
  ...
  runtimeAuthorityId: SPATIAL_FINAL_HELD_GAP_REVIEW_RUNTIME_AUTHORITY_V2.authorityId,
  ...
  metrics: {
    ...
    ql048Motifs: [...metrics.ql048Motifs],
    ql048Counts: [...metrics.ql048Counts].sort((a, b) => a - b),
    ql049Motifs: [...metrics.ql049Motifs],
    ql049Targets: [...metrics.ql049Targets],
  ...
  lifecycle: SPATIAL_FINAL_HELD_GAP_REVIEW_RUNTIME_AUTHORITY_V2.lifecycle,
};

const outDir = resolve(process.cwd(), "dist/reasoning-v1/spatial");
mkdirSync(outDir, { recursive: true });
writeFileSync(resolve(outDir, "spa-final-held-gap-review-runtime-v2-evidence.json"), JSON.stringify(evidence, null, 2));
