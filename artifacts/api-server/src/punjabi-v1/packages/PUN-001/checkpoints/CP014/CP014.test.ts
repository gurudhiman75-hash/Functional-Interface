...
console.log("Starting CP014 Reading Comprehension & Translation Tests...");

// 1. Definition check
assert.equal(PUN_001_CP014_DEFINITION.cpId, "PUN-001-CP014");
assert.equal(PUN_001_CP014_DEFINITION.families.length, 4);

// 2. Determinism check
const q1 = generateCP014Question(42, "Medium", "F01");
const q2 = generateCP014Question(42, "Medium", "F01");
assert.deepEqual(q1, q2, "CP014 generation must be 100% deterministic");
...
for (const diff of difficulties) {
  for (const fam of families) {
    const q = generateCP014Question(7777, diff, fam);
    const validation = validatePunjabiQuestion(q);
    assert.equal(
      validation.isValid,
      true,
      `CP014 validation failed for ${fam} [${diff}]: ${validation.errors.join("; ")}`
    );
...
console.log("Running 200-seed stress test for CP014...");
for (let seed = 14000; seed < 14200; seed++) {
  const diff = difficulties[seed % 3]!;
  const q = generateCP014Question(seed, diff);
...
console.log("Generating 60-question CP014 golden review batch...");
const batch = generateCP014ReviewBatch(60, 15000);
...
console.log("All CP014 Reading Comprehension & Translation tests passed successfully!");
