...
const strides:Record<string,number>={F01:3,F02:5,F03:7,F04:9,F05:11,F06:31,F07:23,F08:29};
const out:string[]=[
 "# PUN-001 CP011 Review — ਮੁਹਾਵਰੇ",
 "",
 "Status: HUMAN REVIEW PENDING",
 "",
 "The review is stratified across all eight audited families. Easy questions test direct meaning; Medium uses authored context, figurative precision and mapping checks; Hard requires two independent idiom judgments.",
 ""
];
...
for(const family of CP011_FAMILIES)if((familyCounts.get(family.familyId)??0)===0)throw new Error(`${family.familyId} missing from review pack`);
if(f08Verdicts.size!==4)throw new Error(`F08 review must expose all four truth outcomes; got ${[...f08Verdicts].join(", ")}`);
writeFileSync("PUN-001-CP011-REVIEW.md",out.join("\n"),"utf8");
console.log(`Wrote PUN-001-CP011-REVIEW.md with ${fingerprints.size} unique questions; all ${CP011_FAMILIES.length} families represented; ${f08Verdicts.size} F08 truth outcomes`);
