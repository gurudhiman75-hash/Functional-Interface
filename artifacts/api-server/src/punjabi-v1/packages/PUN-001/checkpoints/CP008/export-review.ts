...
import { generateCP008ReviewBatch } from "./generator";

const batch = generateCP008ReviewBatch(120, 12000);
const outPath = resolve(process.cwd(), "dist/PUN-001-CP008-V4.1-REVIEW-120.md");
mkdirSync(dirname(outPath), { recursive: true });

const letters = ["A", "B", "C", "D"];
const lines: string[] = [];
lines.push("# PUN-001 CP008 V4.1 — Review File (120 Questions)");
lines.push("");
lines.push("**Topic:** ਅਗੇਤਰ, ਪਿਛੇਤਰ ਅਤੇ ਸ਼ਬਦ-ਰਚਨਾ  ");
lines.push("**Engine revision:** 4.1.0  ");
lines.push("**Review seed:** 12000  ");
lines.push("**Distribution:** 40 Easy · 40 Medium · 40 Hard  ");
lines.push("**Families:** F01–F08, 5 questions per family per difficulty  ");
lines.push("");
lines.push("---");
lines.push("");
