...
import { CP008_FAMILIES } from "./engine";

const plan:Record<PunjabiDifficulty,readonly [string,number][]>={
 Easy:[["F01",14],["F02",14],["F05",12]],
 Medium:[["F03",15],["F04",10],["F06",15]],
 Hard:[["F07",20],["F08",20]],
};
const strides:Record<string,number>={F01:5,F02:7,F03:5,F04:1,F05:5,F06:7,F07:29,F08:29};
const out:string[]=[
 "# PUN-001 CP008 Review — ਅਗੇਤਰ, ਪਿਛੇਤਰ ਅਤੇ ਸ਼ਬਦ-ਰਚਨਾ",
 "",
 "Status: HUMAN REVIEW PENDING",
 "",
 "Review selection is stratified across all eight audited families. Easy questions test direct recognition, Medium questions require root/function/formation analysis, and Hard questions combine two independent morphology decisions.",
 ""
];
...
for(const difficulty of ["Easy","Medium","Hard"] as PunjabiDifficulty[]){
 ...
 for(const [familyId,count] of plan[difficulty]){
  const family=CP008_FAMILIES.find(f=>f.familyId===familyId)!;
...
for(const family of CP008_FAMILIES)if((familyCounts.get(family.familyId)??0)===0)throw new Error(`${family.familyId} missing from review pack`);
if(f08Verdicts.size!==4)throw new Error(`F08 review must expose all four truth outcomes; got ${[...f08Verdicts].join(", ")}`);
writeFileSync("PUN-001-CP008-REVIEW.md",out.join("\n"),"utf8");
console.log(`Wrote PUN-001-CP008-REVIEW.md with ${fingerprints.size} unique questions; all ${CP008_FAMILIES.length} families represented; ${f08Verdicts.size} F08 truth outcomes`);
