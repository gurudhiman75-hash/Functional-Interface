...
console.log("Starting CP002 Spelling Precision Tests...");

// 1. Checkpoint definition check
assert.equal(PUN_001_CP002_DEFINITION.cpId, "PUN-001-CP002");
assert.equal(PUN_001_CP002_DEFINITION.families.length, 3);

// 2. Determinism check: Same seed must produce identical output
const q1 = generateCP002Question(42, "Medium", "F01");
const q2 = generateCP002Question(42, "Medium", "F01");
assert.deepEqual(q1, q2, "CP002 question generation must be 100% deterministic");
...
for (const diff of difficulties) {
  for (const fam of families) {
    const q = generateCP002Question(999, diff, fam);
    const validation = validatePunjabiQuestion(q);
    assert.equal(
      validation.isValid,
      true,
      `CP002 validation failed for ${fam} [${diff}]: ${validation.errors.join("; ")}`
    );
...
console.log("Running 200-seed stress test for CP002...");
for (let seed = 2000; seed < 2200; seed++) {
  const diff = difficulties[seed % 3]!;
  const q = generateCP002Question(seed, diff);
...
console.log("Generating 60-question CP002 golden review batch...");
const batch = generateCP002ReviewBatch(60, 6000);
...
console.log("All CP002 Spelling Precision tests passed successfully!");
