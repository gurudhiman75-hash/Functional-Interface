...
console.log("Starting CP013 Syntax & Sentence Diagnostics Tests...");

// 1. Definition check
assert.equal(PUN_001_CP013_DEFINITION.cpId, "PUN-001-CP013");
assert.equal(PUN_001_CP013_DEFINITION.families.length, 3);

// 2. Determinism check
const q1 = generateCP013Question(42, "Medium", "F01");
const q2 = generateCP013Question(42, "Medium", "F01");
assert.deepEqual(q1, q2, "CP013 generation must be 100% deterministic");
...
for (const diff of difficulties) {
  for (const fam of families) {
    const q = generateCP013Question(5555, diff, fam);
    const validation = validatePunjabiQuestion(q);
    assert.equal(
      validation.isValid,
      true,
      `CP013 validation failed for ${fam} [${diff}]: ${validation.errors.join("; ")}`
    );
...
console.log("Running 200-seed stress test for CP013...");
for (let seed = 13000; seed < 13200; seed++) {
  const diff = difficulties[seed % 3]!;
  const q = generateCP013Question(seed, diff);
...
console.log("Generating 60-question CP013 golden review batch...");
const batch = generateCP013ReviewBatch(60, 14000);
...
console.log("All CP013 Syntax & Sentence Diagnostics tests passed successfully!");
