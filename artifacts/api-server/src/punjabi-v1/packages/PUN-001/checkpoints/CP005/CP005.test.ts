...
console.log("Starting CP005 Adjectives & Adverbs Tests...");
...
const q2 = generateCP005Question(42, "Medium", "F01");
assert.deepEqual(q1, q2, "CP005 generation must be 100% deterministic");
...
for (const diff of difficulties) {
  for (const fam of families) {
    const q = generateCP005Question(4321, diff, fam);
    const validation = validatePunjabiQuestion(q);
    assert.equal(
      validation.isValid,
      true,
      `CP005 validation failed for ${fam} [${diff}]: ${validation.errors.join("; ")}`
    );
...
console.log("Running 200-seed stress test for CP005...");
for (let seed = 5000; seed < 5200; seed++) {
  const diff = difficulties[seed % 3]!;
  const q = generateCP005Question(seed, diff);
...
console.log("Generating 60-question CP005 golden review batch...");
const batch = generateCP005ReviewBatch(60, 14000);
...
console.log("All CP005 Adjectives & Adverbs tests passed successfully!");
