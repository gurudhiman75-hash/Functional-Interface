...
console.log("Starting CP003 Noun & Pronoun Grammar Tests...");

// 1. Definition test
assert.equal(PUN_001_CP003_DEFINITION.cpId, "PUN-001-CP003");
assert.equal(PUN_001_CP003_DEFINITION.families.length, 2);

// 2. Determinism check
const q1 = generateCP003Question(42, "Medium", "F01");
const q2 = generateCP003Question(42, "Medium", "F01");
assert.deepEqual(q1, q2, "CP003 generation must be 100% deterministic");
...
for (const diff of difficulties) {
  for (const fam of families) {
    const q = generateCP003Question(777, diff, fam);
    const validation = validatePunjabiQuestion(q);
    assert.equal(
      validation.isValid,
      true,
      `CP003 validation failed for ${fam} [${diff}]: ${validation.errors.join("; ")}`
    );
...
console.log("Running 200-seed stress test for CP003...");
for (let seed = 3000; seed < 3200; seed++) {
  const diff = difficulties[seed % 3]!;
  const q = generateCP003Question(seed, diff);
...
console.log("Generating 60-question CP003 golden review batch...");
const batch = generateCP003ReviewBatch(60, 7000);
...
console.log("All CP003 Noun & Pronoun Grammar tests passed successfully!");
