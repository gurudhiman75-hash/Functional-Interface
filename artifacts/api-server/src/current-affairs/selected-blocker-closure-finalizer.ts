...
export async function finalizeSelectedBlockerClosure(args: {
  ...
  // CP-068: the canonical draft generated by the selected-processing workflow
  // must contain exactly the distinct events reached from the admin-selected
  // headline set. The broad verified/include-recommended preview is not allowed
  // to leak unselected events into this final selected-processing artifact.
  ...
  return {
    ...
    packPreviewNote: "CP-068 canonical draft membership is exactly the distinct event set reached from admin-selected target-date headlines; unselected verified events cannot leak into this selected-processing pack.",
    blockerClosure: {
