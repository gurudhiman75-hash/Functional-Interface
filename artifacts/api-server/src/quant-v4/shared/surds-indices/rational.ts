...
export const NEGATIVE_ONE = rational(-1n);
...
export function powRationalInteger(value: Rational, exponent: number): Rational {
  if (!Number.isInteger(exponent)) throw new Error("Integer exponent required");
  if (exponent === 0) return ONE;
  if (exponent < 0) return powRationalInteger(reciprocalRational(value), -exponent);
  let base = value;
  let power = exponent;
  let result = ONE;
  while (power > 0) {
...
export function serializeRational(value: Rational): { numerator: string; denominator: string } {
  return { numerator: value.numerator.toString(), denominator: value.denominator.toString() };
}

export function parseRational(serialized: { numerator: string; denominator: string }): Rational {
  return rational(BigInt(serialized.numerator), BigInt(serialized.denominator));
}
