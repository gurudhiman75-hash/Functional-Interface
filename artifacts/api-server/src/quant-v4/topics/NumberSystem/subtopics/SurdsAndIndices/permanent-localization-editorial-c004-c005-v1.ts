...
export function localizeSriEditorialC004C005SurfaceV1(
  ...
  // C004-D: a^p / a^q supplied relation pair -> a^(p±q).
  match = text.match(/^If (\d+\^p = \d+) and (\d+\^q = \d+), find (\d+\^\(p[+-]q\))\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^The supplied condition is (\d+\^p = \d+) and (\d+\^q = \d+)\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^Let (X = \d+\^\d+) and (Y = \d+\^\d+)\. If (Y = X\^n), find n\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^For (X=\d+\^\d+) and (Y=\d+\^\d+), determine n when (Y=X\^n)\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^Given (X=\d+=\d+\^\d+) and (Y=\d+=\d+\^\d+), find the exponent n in (Y=X\^n)\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^The supplied relations are (X = \d+\^\d+) and (Y = \d+\^\d+); (Y = X\^n)\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^The supplied relation is (X=\d+\^\d+) and (Y=\d+\^\d+)\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^The supplied condition is (X=\d+=\d+\^\d+) and (Y=\d+=\d+\^\d+)\.$/u);
  if (match) {
    return locale === "hi-IN"
      ? `दिए गए संबंध ${match[1]} और ${match[2]} हैं।`
      : `ਦਿੱਤੇ ਸੰਬੰਧ ${match[1]} ਅਤੇ ${match[2]} ਹਨ।`;
  ...
  // C004-F: recover k from a^x and a^(x+k).
  match = text.match(/^If (\d+\^x = \d+) and (\d+\^\(x\+k\) = \d+), find k\.$/u);
  if (match) {
    return locale === "hi-IN"
      ? `यदि ${match[1]} और ${match[2]}, तो k ज्ञात कीजिए।`
      : `ਜੇ ${match[1]} ਅਤੇ ${match[2]}, ਤਾਂ k ਪਤਾ ਕਰੋ।`;
  }

  match = text.match(/^Given (\d+\^x=\d+) while (\d+\^\(x\+k\)=\d+), determine k\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^The values (\d+\^x=\d+) and (\d+\^\(x\+k\)=\d+) are known\. What is k\?$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^The supplied values are (\d+\^x=\d+) and (\d+\^\(x\+k\)=\d+)\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^The supplied condition is (\d+\^x = \d+) and (\d+\^\(x\+k\) = \d+)\.$/u);
  if (match) {
    return locale === "hi-IN"
      ? `दिए गए संबंध ${match[1]} और ${match[2]} हैं।`
      : `ਦਿੱਤੇ ਸੰਬੰਧ ${match[1]} ਅਤੇ ${match[2]} ਹਨ।`;
  ...
  // C004-G: same pair, then evaluate a^(x+2k).
  match = text.match(/^If (\d+\^x=\d+) and (\d+\^\(x\+k\)=\d+), find (\d+\^\(x\+2k\))\.$/u);
  if (match) {
    return locale === "hi-IN"
      ? `यदि ${match[1]} और ${match[2]}, तो ${match[3]} ज्ञात कीजिए।`
      : `ਜੇ ${match[1]} ਅਤੇ ${match[2]}, ਤਾਂ ${match[3]} ਪਤਾ ਕਰੋ।`;
  ...
  match = text.match(/^Given (\d+\^x=\d+) and (\d+\^\(x\+k\)=\d+), evaluate (\d+\^\(x\+2k\))\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^Using the two relations (\d+\^x=\d+) and (\d+\^\(x\+k\)=\d+), determine (\d+\^\(x\+2k\))\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^The supplied relation is the two relations (\d+\^x=\d+) and (\d+\^\(x\+k\)=\d+)\.$/u);
  if (match) {
    return locale === "hi-IN"
  ...
  match = text.match(/^The given equation is (\d+\^\(.+\)=\d+\^\d+) and then evaluate (\d+\^\(x\+\d+\))\.$/u);
  if (match) {
    return locale === "hi-IN"
