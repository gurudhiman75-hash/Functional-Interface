...
function generatePackage(
  ...
  for (const [cpId, cpEntries] of [...byCp.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    reviewEntries.forEach((entry, index) => {
      const seed = `${packageId}:exam-review-v2:${cpId}:${index}`;
  ...
  return { packageId, questions: rows.length - 1, averageExplanationWords: Math.round((explanationWords / (rows.length - 1)) * 10) / 10, target };
}
...
console.log(JSON.stringify({ reports, totalQuestions, averageExplanationWords: Math.round((weightedWords / totalQuestions) * 10) / 10 }));
