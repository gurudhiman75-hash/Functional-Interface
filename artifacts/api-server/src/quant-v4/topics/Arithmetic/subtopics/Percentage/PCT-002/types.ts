...
export type Pct002TaskKind =
  ...
  | "repeatedReplacement"
  | "iterativeDilution"
  | "tripleInclusionExclusion"
  | "multiTierPiecewiseRate"
  | "reversePiecewiseRate"
  | "variableReplacement"
  | "electionMargin"
  | "multiStageAttrition"
  | "shiftedBaseChain";
...
export interface Pct002TaskRegistryEntry {
  ...
  supportsReverseReasoning?: boolean;
  supportsDifferenceQuestions?: boolean;
  difficultyBands?: Pct002DifficultyBand[];
}
...
export interface Pct002SemanticEntity {
  id: string;
  en: string;
  hi: string;
  pa: string;
...
export interface Pct002SemanticContext {
  scenario: string;
  entities: Record<string, Pct002SemanticEntity>;
}
...
export interface Pct002Parameters {
  ...
  semanticContext?: Pct002SemanticContext;
  sourceTrace: {
