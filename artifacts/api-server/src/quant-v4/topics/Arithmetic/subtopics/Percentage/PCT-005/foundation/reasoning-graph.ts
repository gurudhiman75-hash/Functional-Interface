import type { Pct005Parameters, Pct005ReasoningGraph, Pct005ReasoningNode, Pct005SolverResult } from "./types";

export function buildPct005ReasoningGraph(parameters: Pct005Parameters, solver: Pct005SolverResult): Pct005ReasoningGraph {
  const evidenceNodes: Pct005ReasoningNode[] = Object.entries(solver.evidence).map(([key, value]) => ({
    id: `evidence:${key}`,
    label: `Evidence ${key}`,
    value,
  }));

  return {
    graphId: `${parameters.questionId}:graph`,
    nodes: [
      { id: "inputs", label: "Captured inputs", value: parameters.variables },
      { id: "task", label: "Selected task kind", value: parameters.taskKind },
      { id: "answerType", label: "Declared answer type", value: parameters.answerType },
      ...evidenceNodes,
      { id: "mathJax", label: "MathJax evidence", value: solver.mathJax },
      { id: "answer", label: "Final answer", value: solver.answer },
    ],
  };
}
