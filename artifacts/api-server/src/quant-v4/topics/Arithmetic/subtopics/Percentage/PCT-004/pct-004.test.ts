...
const hiPkg004 = runPct004Pipeline("PCT-CP-001", {
  ...
  seed: "pct-004-hi-localization",
});
assert.equal(hiPkg004.parameters.language, "hi");
assert.ok(!/\b(First|Now|Therefore|So the|required|increase|decrease)\b/.test(hiPkg004.explanation.lines.join(" ")));

const paPkg004 = runPct004Pipeline("PCT-CP-006", {
  language: "pa",
  questionLanguageId: "PCT-QL-012",
  seed: "pct-004-pa-localization",
});
assert.equal(paPkg004.parameters.language, "pa");
assert.ok(!/\b(First|Now|Therefore|So the|required|increase|decrease)\b/.test(paPkg004.explanation.lines.join(" ")));
