  ...
  switch (parameters.taskKind) {
    ...
    case "percentageDecreaseBridge":
      lines.push(
        ...
        ...sentenceWithMath(localizedText(parameters, `So the ${wholeLabel} after ${formatNumber(periodCount)} periods is ${displayValue(parameters, solver.numericAnswer ?? 0)}.`, `अतः ${formatNumber(periodCount)} अवधियों के बाद ${wholeLabel} ${displayValue(parameters, solver.numericAnswer ?? 0)} है।`, `ਇਸ ਲਈ ${formatNumber(periodCount)} ਮਿਆਦਾਂ ਤੋਂ ਬਾਅਦ ${wholeLabel} ${displayValue(parameters, solver.numericAnswer ?? 0)} ਹੈ।`), parameters.language === "hi" ? `\\text{अंतिम मान}=${renderedAnswer}` : `\\text{ਅੰਤਿਮ ਮਾਨ}=${renderedAnswer}`),
      );
      break;
  }
