...
function q(value: number | bigint): Rational { return rational(value); }
...
function absDiff(a: Rational, b: Rational): Rational { return absRational(subtract(a, b)); }
function mean(values: readonly Rational[]): Rational { return divide(sum(values), q(values.length)); }
function minR(values: readonly Rational[]): Rational { return values.reduce((best, value) => compare(value, best) < 0 ? value : best); }
function maxR(values: readonly Rational[]): Rational { return values.reduce((best, value) => compare(value, best) > 0 ? value : best); }
...
function positiveOrZero(value: Rational): boolean { return compare(value, ZERO) >= 0; }
function pickScalar(correct: Rational, candidates: readonly TsdCp012ScalarDistractor[]): readonly TsdCp012ScalarDistractor[] {
  ...
  for (const candidate of candidates) {
    if (!positiveOrZero(candidate.value)) continue;
...
function movementBlocks(distance: Rational, speed: Rational, blockDuration: Rational): bigint {
  return ceilRational(divide(distance, multiply(speed, blockDuration)));
}
function ratioSplitCandidates(total: Rational, speedA: Rational, speedB: Rational): readonly TsdCp012ScalarDistractor[] {
  ...
  return Object.freeze([
    c("SPEED_RATIO_USED_AS_DISTANCE_RATIO_A", "Split total distance directly in the speed ratio and assign the larger share to speed A.", divide(multiply(total, speedA), speedSum)),
    c("SPEED_RATIO_USED_AS_DISTANCE_RATIO_B", "Split total distance directly in the speed ratio and assign the smaller share to speed A.", divide(multiply(total, speedB), speedSum)),
    c("EQUAL_DISTANCE_SPLIT_ASSUMED", "Ignore the time condition and split the route equally between the two speeds.", divide(total, TWO)),
    c("SQUARED_SPEED_RATIO_SPLIT", "Use a squared-speed ratio instead of the reciprocal-time relation.", divide(multiply(total, squareA), add(squareA, squareB))),
  ]);
}
function feasibleValues(input: Extract<TsdCp012ReviewInput, { authorityKey: "feasibleParameterSetState" }>, mode: "CORRECT" | "STRICT" | "IGNORE_DELAY" | "ALL_ALLOWED" | "EXTEND_UPPER"): readonly Rational[] {
  const minimum = input.minimumCandidate;
  ...
  for (let candidate = minimum; candidate <= maximum; candidate += 1) {
    ...
    const elapsed = add(delay, divide(input.distance, q(candidate)));
...
function feasibleCandidates(input: Extract<TsdCp012ReviewInput, { authorityKey: "feasibleParameterSetState" }>): readonly TsdCp012SetDistractor[] {
  return Object.freeze([
    ...
    s("FIXED_DELAY_IGNORED", "Check travel time against the deadline but omit the fixed non-travel delay.", feasibleValues(input, "IGNORE_DELAY")),
    s("UPPER_BOUND_OFF_BY_ONE", "Apply the feasibility inequality correctly but admit one speed above the allowed maximum.", feasibleValues(input, "EXTEND_UPPER")),
  ...
  switch (input.authorityKey) {
    case "discreteSpeedProgramState": {
      if (input.target === "TOTAL_DISTANCE") {
        ...
        candidates = [
          c("STAGE_PAIRING_LOST", "Multiply the sum of all speeds by the sum of all durations instead of pairing each stage.", multiply(sum(speeds), sum(durations))),
          c("UNWEIGHTED_AVERAGE_SPEED", "Use the simple average of the listed speeds for the whole elapsed time.", multiply(mean(speeds), sum(durations))),
          c("FINAL_STAGE_OMITTED", "Add the travelled distances but forget the final stage.", sum(input.stages.slice(0, -1).map(timedDistance))),
          c("FINAL_STAGE_ONLY", "Use only the last speed-duration block as the journey distance.", timedDistance(input.stages.at(-1)!)),
        ];
        break;
      }
      if (input.target === "TOTAL_TIME") {
        ...
        const speeds = input.stages.map((stage) => stage.speed);
        candidates = [
          c("SPEEDS_ADDED_BEFORE_TIME", "Divide the total route distance by the sum of segment speeds.", divide(totalDistance, sum(speeds))),
      ...
      if (input.target === "UNKNOWN_FINAL_SPEED") {
        ...
        candidates = [
          ...
          c("PRIOR_TIME_ADDED_TO_FINAL_DENOMINATOR", "Use the remaining distance but divide by all earlier time plus the final duration.", divide(remaining, add(priorDuration, input.finalDuration))),
      ...
      if (input.target === "EXACT_TIME_TO_DISTANCE_IN_REPEATING_CYCLE") {
        ...
        candidates = [
          ...
          c("PARTIAL_CYCLE_IGNORED", "Count only completed cycles and ignore the final partial stage needed to reach the target.", multiply(q(floorRational(divide(input.distance, cycleDistance))), cycleTime)),
    ...
    case "periodicTravelRestProgramState": {
      ...
      if (input.target === "COMPLETION_TIME") {
        candidates = [
          ...
          c("REST_AFTER_ARRIVAL_CHARGED", "Charge one rest after every movement block, including the arrival block.", add(movingTime, multiply(q(blocks), input.restDuration))),
          c("FINAL_PARTIAL_BLOCK_TREATED_AS_FULL", "Charge full movement duration for every block before adding the correct number of rests.", add(multiply(q(blocks), input.travelDurationPerBlock), multiply(q(rests), input.restDuration))),
          c("FULL_BLOCK_AND_FINAL_REST_BOTH_CHARGED", "Treat all movement blocks as full and also charge a rest after arrival.", multiply(q(blocks), add(input.travelDurationPerBlock, input.restDuration))),
        ];
        ...
        candidates = [
          ...
          c("FLOOR_BLOCK_COUNT_USED", "Use completed full block count directly as the number of rests.", q(floorRational(divide(input.distance, multiply(input.travelSpeed, input.travelDurationPerBlock))))),
        ];
      } else {
        ...
        candidates = [
          ...
          c("MOVING_TIME_NOT_REMOVED", "Divide the full elapsed time by the number of rests.", divide(input.totalElapsedTime, q(rests))),
    ...
    case "terminalConstraintProgramState": {
      if (input.target === "REQUIRED_FINAL_SPEED") {
        ...
        candidates = [
          c("COMPLETED_DISTANCE_NOT_REMOVED", "Use the full route distance with only the remaining time.", divide(input.totalDistance, remainingTime)),
          ...
          c("PAST_AVERAGE_SPEED_REUSED", "Reuse the speed achieved in the completed part for the remaining journey.", divide(input.completedDistance, input.elapsedTime)),
        ];
        ...
        candidates = [
          c("DURATIONS_SUBTRACTED_AS_DISTANCE", "Subtract listed durations directly from total distance without multiplying by speed.", subtract(input.totalDistance, sum(input.completedStages.map((stage) => stage.duration)))),
    ...
    case "routeProfileProgramState": {
      if (input.target === "TOTAL_TIME") {
        ...
        candidates = [
          ...
          c("FINAL_ROUTE_SEGMENT_OMITTED", "Add route times but forget the last segment.", sum(input.segments.slice(0, -1).map(stageTime))),
        ];
    ...
    case "motionReconstructionProgramState": {
      if (input.target === "MISSING_DISTANCE") {
        ...
        candidates = [
          ...
          c("LAST_KNOWN_TIME_COPIED", "Copy the last recorded stage time into the missing row.", input.knownTimes.at(-1)!),
        ];
    ...
    case "trainScheduleSynthesisState": {
      ...
      if (input.target === "MEETING_TIME_FROM_FIRST_DEPARTURE") {
        ...
        candidates = [
          ...
          c("DELAY_DISTANCE_SUBTRACTED", "Subtract the second train's delayed-start distance from the station gap instead of adding it to the closure equation.", divide(absRational(subtract(input.stationDistance, delayedTravel)), relativeToward)),
          ...
          c("TIME_MEASURED_FROM_SECOND_DEPARTURE", "Report only the catch-up interval after Train B starts.", subtract(solution.answer, input.delayB)),
        ];
        ...
        candidates = [
          c("TRAIN_LENGTHS_IGNORED", "Use only the initial nose-to-nose gap with the combined speed.", divide(input.initialGap, relativeToward)),
        ...
        const numerator = subtract(multiply(relativeToward, input.meetingTimeFromFirstDeparture), input.stationDistance);
        candidates = [
          c("DELAY_DIVIDED_BY_FIRST_TRAIN_SPEED", "Use Train A's speed instead of Train B's speed when converting the delayed closure into time.", divide(numerator, input.speedA)),
          ...
          c("NAIVE_TIME_DIFFERENCE", "Subtract the simultaneous-start meeting time from the observed meeting time.", subtract(input.meetingTimeFromFirstDeparture, divide(input.stationDistance, relativeToward))),
          c("STATION_DISTANCE_DIVIDED_BY_SECOND_SPEED", "Treat Train B's solo time across the station gap as the departure delay.", divide(input.stationDistance, input.speedB)),
        ];
      }
      break;
    }
    ...
    case "mediumPursuitSynthesisState": {
      const boat = input.boatStillWaterSpeed;
      const delay = input.boatStartDelay;
      if (input.target === "RAFT_CATCH_TIME_FROM_RAFT_START") {
        candidates = [
          c("BOAT_START_TO_CATCH_INTERVAL_ONLY", "Compute only the interval after the delayed boat starts and forget the raft's head-start time.", divide(multiply(input.currentSpeed, delay), boat)),
          c("UPSTREAM_RELATIVE_SPEED_USED", "Use boat speed minus current as if the chase were upstream.", divide(multiply(add(boat, input.currentSpeed), delay), subtract(boat, input.currentSpeed))),
          c("CURRENT_IGNORED_IN_BOAT_GROUND_SPEED", "Use still-water boat speed for both closing and ground-motion parts of the chase.", divide(multiply(boat, delay), subtract(boat, input.currentSpeed))),
          c("DELAY_REPORTED_AS_CATCH_TIME", "Report the boat's starting delay itself as the total catch time.", delay),
        ];
      } else if (input.target === "RAFT_CATCH_DISTANCE_FROM_START") {
        candidates = [
          c("RAFT_POSITION_AT_BOAT_START", "Report the raft's distance when the boat starts, not when it is caught.", multiply(input.currentSpeed, delay)),
          c("STILL_WATER_BOAT_DISTANCE_DURING_DELAY", "Multiply the boat's still-water speed by the delay and report it as the catch distance.", multiply(boat, delay)),
          c("DOWNSTREAM_BOAT_DISTANCE_DURING_DELAY", "Use downstream boat speed times the starting delay as the catch distance.", multiply(add(boat, input.currentSpeed), delay)),
          c("CATCH_TIME_REPORTED_AS_DISTANCE", "Carry the catch-time magnitude into the distance answer without multiplying by current speed.", divide(multiply(add(boat, input.currentSpeed), delay), boat)),
        ];
      } else {
        const catchTime = input.catchTimeFromRaftStart;
        const extra = subtract(catchTime, delay);
        candidates = [
          c("TOTAL_CATCH_TIME_USED_IN_NUMERATOR", "Use total raft-to-catch time instead of only the interval beyond the boat's delay.", divide(multiply(boat, catchTime), delay)),
          c("DELAY_OVER_TOTAL_TIME_RATIO", "Reverse the time ratio when inferring the current speed.", divide(multiply(boat, delay), catchTime)),
          c("EXTRA_TIME_DIVIDED_BY_TOTAL_TIME", "Scale boat speed by extra catch time over total catch time rather than over the start delay.", divide(multiply(boat, extra), catchTime)),
    ...
    case "closedTrackRaceSynthesisState": {
      ...
      if (input.target === "TRACK_GAP_AT_FASTER_FINISH") {
        ...
        candidates = [
          ...
          c("LAP_WRAP_IGNORED", "Use the straight-line distance difference without reducing position modulo the track length.", absRational(subtract(raceDistance, add(input.slowerHeadStart, slowerTravel)))),
        ];
      } else if (input.target === "HEAD_START_FOR_DEAD_HEAT") {
        const correctHead = solution.answer;
        candidates = [
          c("SLOWER_SPEED_IN_DENOMINATOR", "Scale the race distance by the speed difference over the slower speed.", divide(multiply(raceDistance, subtract(input.fasterSpeed, input.slowerSpeed)), input.slowerSpeed)),
          c("SUM_SPEED_DENOMINATOR", "Use speed difference over speed sum instead of the faster-speed race time.", divide(multiply(raceDistance, subtract(input.fasterSpeed, input.slowerSpeed)), add(input.fasterSpeed, input.slowerSpeed))),
          c("HEAD_START_COMPLEMENT", "Report the remaining race distance after the correct head start as the head start itself.", subtract(raceDistance, correctHead)),
          c("ONE_LAP_COMPLEMENT", "Use the complement within one track lap regardless of race length.", absRational(subtract(input.trackLength, correctHead))),
        ];
      } else {
        ...
        candidates = [
          ...
          c("FULL_LAP_GAP_USED", "Ignore the stated head start and use a complete lap as the closing gap.", divide(input.trackLength, speedDifference)),
    ...
    case "movingSurfaceScheduleSynthesisState": {
      ...
      if (input.target === "TIME_WITH_STOP_AFTER") {
        ...
        candidates = [
          c("SURFACE_ACTIVE_FOR_WHOLE_TRIP", "Assume the walkway keeps assisting for the entire length.", divide(input.length, assisted)),
          ...
          c("ASSISTED_RATE_USED_AFTER_STOP", "Split at the stopping time but keep using the assisted rate after the walkway stops.", add(input.surfaceActiveTime, divide(remaining, assisted))),
        ...
        candidates = [
          ...
          c("DELAY_OMITTED_AFTER_SPLIT", "Use only the assisted time for the post-activation remaining distance.", divide(remaining, assisted)),
        ];
      } else {
    ...
    case "twoEngineInverseState": {
      ...
      if (input.target === "X") {
        candidates = [
          ...
          c("DETERMINANT_SIGN_REVERSED", "Reverse the determinant sign while keeping the x numerator unchanged.", divide(subtract(multiply(input.c1, input.b2), multiply(input.c2, input.b1)), subtract(ZERO, determinant))),
        ];
      } else {
        candidates = [
          ...
          c("DETERMINANT_SIGN_REVERSED", "Reverse the determinant sign while keeping the y numerator unchanged.", divide(subtract(multiply(input.a1, input.c2), multiply(input.a2, input.c1)), subtract(ZERO, determinant))),
        ];
      }
      break;
    }
    ...
    case "feasibleParameterSetState": {
      ...
      const wrongSets = feasibleCandidates(input);
      candidates = wrongSets.map((candidate) => c(candidate.misconceptionId, candidate.calculation, q(candidate.values.length)));
      break;
    }
  }
