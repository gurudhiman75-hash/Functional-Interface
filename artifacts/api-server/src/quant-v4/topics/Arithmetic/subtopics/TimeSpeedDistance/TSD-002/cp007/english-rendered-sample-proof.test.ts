...
for (const qlId of TSD_CP007_PERMANENT_QL_IDS) {
  ...
  assert(new Set(qlSamples.map((sample) => sample.scene)).size === 6, `${qlId}: rendered review pack lost scene diversity`);
  assert(new Set(qlSamples.map((sample) => sample.representation)).size === 6, `${qlId}: rendered review pack lost representation diversity`);
