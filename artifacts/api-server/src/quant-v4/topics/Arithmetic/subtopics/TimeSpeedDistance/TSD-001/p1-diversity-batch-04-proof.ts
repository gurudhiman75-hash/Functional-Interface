...
function hasConsecutiveConclusionLines(steps: readonly string[]): boolean {
  return steps.some((line, index) =>
    line.startsWith("Therefore,")
    && steps[index + 1]?.startsWith("Therefore,"),
  );
}
...
const supplementalRows = TARGETS.map(([mode, seed, expectedAnswer]) => {
  ...
  assert(!hasConsecutiveConclusionLines(row.explanation.steps), `${mode}: duplicated consecutive conclusion lines`);
