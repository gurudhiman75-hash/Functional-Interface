...
for (const language of ["hi", "pa"] as const) {
  ...
  assert(ql119.every((q) => language === "hi" ? /(एक ही समय|बराबरी)/.test(q.stem) : /(ਇੱਕੋ ਸਮੇਂ|ਬਰਾਬਰੀ)/.test(q.stem)), `${language}/QL119: dead-heat equality must be explicit`);
  assert(ql119.every((q) => language === "hi" ? /(शुरुआती|समय शून्य)/.test(q.stem) : /(ਸ਼ੁਰੂਆਤੀ|ਸਮਾਂ ਸਿਫ਼ਰ)/.test(q.stem)), `${language}/QL119: start advantage/delay must be explicit`);
  ...
  assert(ql122.every((q) => language === "hi" ? /(शुरुआती|आगे से शुरू)/.test(q.stem) : /(ਸ਼ੁਰੂਆਤੀ|ਅੱਗੇ ਤੋਂ ਸ਼ੁਰੂ)/.test(q.stem)), `${language}/QL122: second-race head start must be explicit`);
