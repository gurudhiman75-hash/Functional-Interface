...
for (const [language, questions] of Object.entries(surfaces) as [TsdCp012StudioLanguage, typeof TSD_CP012_ENGLISH_REVIEW_FINAL][]) {
  assert(questions.length === 66, `${language}: expected 66 calibrated review families, found ${questions.length}`);
  ...
  assert(medium === 44, `${language}: expected 44 MEDIUM families, found ${medium}`);
  ...
  const allPreview = previewTsdCp012StudioCandidate({ language, count: 66, seed: "cp012-difficulty-contract-all" });
  ...
  const mediumPreview = previewTsdCp012StudioCandidate({ language, difficulty: "MEDIUM", count: 44, seed: "cp012-difficulty-contract-medium" });
  assert(mediumPreview.availableCombinationsUnderFilters === 44, `${language}: MEDIUM filter must expose exactly 44 reviewed combinations`);
...
console.log(JSON.stringify({
  ...
  perLocale: { total: 66, easy: 22, medium: 44, hard: 0 },
  multilingual: { total: 198, easy: 66, medium: 132, hard: 0 },
