...
function rotateOptions(
  ...
  if (wrongEntries.some((entry) => entry.text === answerText)) throw new Error(`${mode}: distractor duplicates the correct answer ${answerText}`);
  if (new Set(wrongEntries.map((entry) => entry.text)).size !== 3) throw new Error(`${mode}: duplicate distractors`);
...
function validationErrors(question: Omit<TsdCp002GeneratedQuestion, "validation">): string[] {
  ...
  if (question.lifecycle.englishFreezeStatus !== "FROZEN") errors.push("English authority is not frozen");
  ...
  if (!/^TSD-QL-0(2[4-9]|3[0-7])$/.test(question.permanentQlId)) errors.push("CP-002 permanent QL is outside TSD-QL-024..037");
...
export function generateCp002Candidate(
  ...
  if (frozenAuthority.provisionalAuthorityId !== authority.provisionalId) throw new Error(`${mode}: frozen/discovery authority mismatch`);
  ...
  const base = {
    ...
    explanation: Object.freeze({
      ...
      examSpeedShortcut: CP002_SHORTCUT[mode],
      optionAnalysis: optionPackage.analysis,
      conclusion: `Answer: ${answerText}.`,
    }),
    mathematicalFingerprint: `${frozenAuthority.permanentQlId}|${mode}|${definition.caseId}|${stableStringify(definition.input)}`,
  ...
  return Object.freeze({ ...base, validation: Object.freeze({ valid: errors.length === 0, errors: Object.freeze(errors) }) });
}
