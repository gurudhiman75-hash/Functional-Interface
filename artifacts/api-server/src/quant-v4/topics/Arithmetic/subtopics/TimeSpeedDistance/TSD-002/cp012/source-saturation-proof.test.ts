import { TSD_CP012_LEARNER_AUTHORITIES, TSD_CP012_SOURCE_CANDIDATES, TSD_CP012_SOURCE_SUMMARY } from "./source-saturation";
...
assert(TSD_CP012_SOURCE_SUMMARY.learnerAuthorities === 7, "expected seven provisional mathematical authorities");
...
for (const authorityKey of TSD_CP012_LEARNER_AUTHORITIES) {
  ...
  assert(owned.length >= 2, `${authorityKey}: authority is under-evidenced`);
}
...
assert(transferred.every((x) => x.authorityKey === "composedMotionSystemState"), "CP011 state-change holds must land in composed synthesis");
...
console.log("TSD-CP-012 SOURCE SATURATION PROOF: PASS");
console.log(JSON.stringify(TSD_CP012_SOURCE_SUMMARY, null, 2));
