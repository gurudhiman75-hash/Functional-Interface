...
function numericTokens(text: string): readonly string[] { return Object.freeze(text.match(/\d+(?:\.\d+)?/g) ?? []); }
...
const rows = TSD_CP005_NATIVE_EDITORIAL_REVIEW_V4;
const hi = rows.filter((row) => row.presentation.language === "hi");
const pa = rows.filter((row) => row.presentation.language === "pa");

assert(TSD_CP005_ENGLISH_FREEZE_ID === "TSD-CP-005-EN-v13-frozen", "native V4 is not based on certified English V13 freeze");
assert(TSD_CP005_APPROVED_ENGLISH_FROZEN_78Q.length === 78, "English frozen authority must contain 78 rows");
assert(rows.length === 156 && hi.length === 78 && pa.length === 78, "native V4 must contain 78 Hindi + 78 Punjabi rows");

for (const languageRows of [hi, pa]) {
  assert(new Set(languageRows.map((row) => row.source.permanentQlId)).size === 13, "native V4 lost QL coverage");
  assert(new Set(languageRows.map((row) => row.source.solveMode)).size === 20, "native V4 lost solve-mode coverage");
  assert(new Set(languageRows.map((row) => row.presentation.stem)).size === 78, "native V4 stems are not unique");
  assert(new Set(languageRows.map((row) => row.source.objectContextId)).size === 44, "native V4 changed object-context coverage");
  assert(new Set(languageRows.map((row) => row.source.objectFamily)).size === 25, "native V4 changed object-family coverage");
  assert(new Set(languageRows.map((row) => row.source.endpointFamily)).size === 18, "native V4 changed endpoint-family coverage");
...
for (let index = 0; index < 78; index += 1) {
  ...
  assert(nativeRows.every((row) => row.source === source), `row ${index + 1}: native presentation detached from frozen source`);
  ...
  for (const native of nativeRows) {
    ...
    for (let optionIndex = 0; optionIndex < source.options.length; optionIndex += 1) {
      assert(numericTokens(native.presentation.options[optionIndex]!).join("|") === numericTokens(source.options[optionIndex]!).join("|"), `row ${index + 1}/${lang} option ${optionIndex + 1}: numeric option identity changed`);
    }
    const nativeStemNumbers = numericTokens(native.presentation.stem);
    for (const token of numericTokens(source.stem)) assert(nativeStemNumbers.includes(token), `row ${index + 1}/${lang}: frozen stem number ${token} missing`);
    ...
    assert(!/समय सीमा \d+(?:\.\d+)? घंटे है(?!ं)/u.test(native.presentation.stem), `row ${index + 1}: awkward Hindi time-limit agreement remains`);
    assert(!/ਸਮਾਂ ਸੀਮਾ \d+(?:\.\d+)? ਘੰਟੇ ਹੈ/u.test(native.presentation.stem), `row ${index + 1}: awkward Punjabi time-limit agreement remains`);
...
console.log(JSON.stringify({
  status: "PASS", phase: "TSD_CP005_HI_PA_NATIVE_EDITORIAL_REVIEW_V4", englishAuthority: TSD_CP005_ENGLISH_FREEZE_ID,
  nativeRows: rows.length, hindiRows: hi.length, punjabiRows: pa.length, permanentQlRange: "TSD-QL-058..TSD-QL-070",
  ...
  stemsPerQlPerLanguage: 6, explanationStepsPerQuestion: 2, strictScriptAndEnglishLeakGate: true,
  frozenGivenNumericParityGate: true, nativeVehicleAgreementGate: true, nativeSpeedPhrasingGate: true,
  nativeTimeLimitGrammarGate: true, englishSourceMutation: false, multilingualFreezeStatus: "UNFROZEN",
  productOwnerApprovalRecorded: false, questionStudioEnabled: false, questionBankStatus: "NOT_STORED",
  testEligibility: "INELIGIBLE", publiclyPublishable: false,
}, null, 2));
