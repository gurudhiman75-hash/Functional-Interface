...
function expectedAnswer(input: TsdCp010ExecutableInput): { answer: Rational; unit: TsdCp010ExecutableSolution["unit"] } {
  switch (input.authorityKey) {
    case "finishDistanceLeadState":
      return { answer: finishDistanceLead(input.raceDistance, input.winnerSpeed, input.loserSpeed), unit: "METRE" };
    case "finishTimeLeadState":
      return { answer: finishTimeLead(input.raceDistance, input.winnerSpeed, input.loserSpeed), unit: "SECOND" };
    case "raceSpeedRatioState":
      return { answer: divide(input.raceDistance, subtract(input.raceDistance, input.distanceLead)), unit: "RATIO" };
    case "raceLengthFromLeadEvidence":
      return {
        answer: divide(multiply(input.distanceLead, input.winnerSpeed), subtract(input.winnerSpeed, input.loserSpeed)),
        unit: "METRE",
      };
    ...
    case "transitiveRaceComparison": {
      const cAtAFinish = divide(
        multiply(
          subtract(input.raceDistance, input.aBeatsBBy),
          subtract(input.raceDistance, input.bBeatsCBy),
        ),
        input.raceDistance,
      );
    ...
    case "multiOutcomeRaceComparison": {
      ...
      const traveled = multiply(ratio, input.secondRaceDistance);
      return {
        answer: subtract(subtract(input.secondRaceDistance, input.secondRaceHeadStartForLoser), traveled),
        unit: "METRE",
      };
    }
    case "changedRaceOutcomeState": {
      if (input.mode === "FASTER_SPEED_CHANGE") {
        return { answer: finishDistanceLead(input.raceDistance, input.changedFasterSpeed!, input.slowerSpeed), unit: "METRE" };
      }
    ...
    case "runnerStateFromTwoRaceOutcomes": {
      ...
      const faster = divide(
        multiply(input.secondRaceDistance, subtract(rational(1), ratio)),
        multiply(ratio, input.secondRaceTimeLead),
      );
      return {
        answer: input.target === "FASTER_SPEED" ? faster : multiply(ratio, faster),
...
export function verifyTsdCp010(input: TsdCp010ExecutableInput, solution: TsdCp010ExecutableSolution) {
  ...
  return Object.freeze({
    accepted:
      solution.authorityKey === input.authorityKey &&
      solution.unit === expected.unit &&
      equals(solution.answer, expected.answer),
    expectedAnswer: expected.answer,
    expectedUnit: expected.unit,
    invariant: `Independent race-state reconstruction for ${input.authorityKey}`,
  });
}