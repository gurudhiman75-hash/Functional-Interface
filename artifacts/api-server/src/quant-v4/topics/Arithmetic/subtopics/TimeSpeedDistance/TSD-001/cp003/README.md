...
**Current phase:** `POST_OVERLAP_AUTHORITY_CANDIDATE_REVIEW`  
**Permanent QLs:** `0`  
**English:** `UNFROZEN`  
...
Executable discovery remains intentionally broad so source coverage is not lost:

- 35 authoritative source candidates;
- 24 provisional mathematical authorities;
- 22 learner-facing discovery modes;
- 2 internal QA modes;
- exact rational solver and independent verifier coverage;
- deterministic parameter generation, exam-style stems, method-derived distractors and six-step explanations;
- 40-seed runtime proof per learner discovery mode;
- three-row editorial sample per learner discovery mode (66 rows total).

## Cross-checkpoint overlap result

The post-generation overlap audit now separates mathematical authority ownership from representation coverage:
...
The next work is chapter-quality closure for this post-overlap pool:
