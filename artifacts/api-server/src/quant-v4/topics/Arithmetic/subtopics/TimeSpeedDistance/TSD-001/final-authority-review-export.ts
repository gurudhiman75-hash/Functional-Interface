...
const cards = coverage.map((entry) => `<article>
  ...
  <p class="ok">Final authority ownership and required representation coverage are present.</p>
</article>`).join("\n");
...
body{font-family:system-ui,sans-serif;max-width:1000px;margin:24px auto;padding:0 14px;background:#f5f6f8;color:#1c2430}article{background:#fff;border:1px solid #d7dce3;border-radius:10px;padding:16px;margin:12px 0}h1{line-height:1.2}h2{font-size:17px;margin:0 0 8px}.ok{color:#166534;font-weight:600}code{background:#eef1f5;padding:2px 5px;border-radius:4px}@media(max-width:600px){body{margin:8px auto}article{padding:12px}}
</style></head><body>
<h1>TSD CP-001 and CP-002 — Final Authority Ownership and Pool Review</h1>
<p><b>38 learner authorities</b> · <b>116 remapped and supplemental records</b> · <b>0 permanent QL IDs assigned</b>.</p>
<p>All authority-level representation gaps identified by the cross-QL audit are closed. English remains <code>UNFROZEN</code>; Question Bank, tests and public delivery remain disabled.</p>
${cards}
</body></html>`, "utf8");
...
console.log(JSON.stringify({
  status: "PASS",
  outputDir,
  remappedAndSupplementalReviewRows: rows.length,
