...
import {
  ...
  TSD_CP006_PUNJABI_ACTOR_ROOTS_V7,
...
const v6 = generateCp006NativeReviewV6();
const rows = generateCp006NativeReviewV7();
assert(TSD_CP006_ENGLISH_FREEZE_ID === "TSD-CP-006-EN-v5-frozen", "V7 lost English V5 freeze authority");
assert(rows.length===156,"V7 must contain 156 rows");
assert(rows.filter(r=>r.presentation.language==="hi").length===78,"V7 Hindi count changed");
assert(rows.filter(r=>r.presentation.language==="pa").length===78,"V7 Punjabi count changed");
assert(new Set(rows.map(r=>r.source.permanentQlId)).size===13,"V7 QL coverage changed");

let verifierChecks=0;
let grammarChangedRows=0;
for(let i=0;i<rows.length;i+=1){
  const row=rows[i]!; const prior=v6[i]!; const {source,presentation:p}=row;
  const label=`${source.seed}/${p.language}`; const learner=`${p.stem} ${p.explanation.steps.join(" ")}`;
  ...
  assert(p.lifecycle.nativeReviewStatus===TSD_CP006_NATIVE_REVIEW_STATUS_V7,`${label}: V7 status missing`);
  ...
  assert(JSON.stringify(p.options)===JSON.stringify(prior.presentation.options),`${label}: V7 changed option text/order`);
  assert(p.correctIndex===prior.presentation.correctIndex&&p.answerText===prior.presentation.answerText&&p.options[p.correctIndex]===p.answerText,`${label}: answer identity changed`);
  ...
  if(p.language==="hi"){
    assert(p.stem===prior.presentation.stem,`${label}: Hindi stem changed in Punjabi-only V7`);
    assert(JSON.stringify(p.explanation.steps)===JSON.stringify(prior.presentation.explanation.steps),`${label}: Hindi explanation changed in Punjabi-only V7`);
    ...
    const expected=TSD_CP006_PUNJABI_ACTOR_ROOTS_V7[source.objectFamily];
    assert(Boolean(expected)&&p.stem.includes(expected!),`${label}: V7 Punjabi actor family is not visibly localized`);
    assert(!/ਧਾਵਕ|ਪ੍ਰਸ਼ਿਕਸ਼ੂ|ਐਥਲੀਟ|ਜੌਗਰ|ਵਾਕਰ|ਰੇਸਰ/u.test(learner),`${label}: legacy Hindi/English-style Punjabi actor word remains`);
    assert(!/ਹੌਲੀ ਦੌੜਨ ਵਾਲਾ|ਕਸਰਤ ਲਈ ਤੁਰਨ ਵਾਲਾ|(?<!ਹੌਲੇ )ਤੁਰਨ ਵਾਲਾ/u.test(learner),`${label}: inflection-prone Punjabi actor phrase remains`);
    assert(!/(?:ਤੁਰਨ ਵਾਲਾ|ਦੌੜਨ ਵਾਲਾ) [ABC] ਦੀ/u.test(learner),`${label}: Punjabi actor oblique-case defect remains`);
    if(p.stem!==prior.presentation.stem||JSON.stringify(p.explanation.steps)!==JSON.stringify(prior.presentation.explanation.steps)) grammarChangedRows+=1;
  }
  assert(!/[A-Za-z]{2,}/.test(stripAllowedLatin(learner)),`${label}: unresolved English remains`);
  assert(!/ਅਗਲੀ ਪਹਿਲੀ|ਪਹਿਲੀ ਅਗਲੀ|ਮਾਪਿਆਂ|ਕਿੰਨੇ ਮੁਲਾਕਾਤਾਂ|ਮੁਲਾਕਾਤਾਂ ਕਿੰਨੇ|ਪੂਰੇ ਹੋਣ ਵਾਲੇ ਮੁਲਾਕਾਤਾਂ|\sand\s/.test(learner),`${label}: known editorial defect remains`);
...
assert(grammarChangedRows>0,"V7 made no Punjabi grammar improvement");
console.log(JSON.stringify({status:"PASS",phase:"TSD_CP006_HI_PA_PUNJABI_LEXICON_GRAMMAR_REVIEW_V7",englishAuthority:TSD_CP006_ENGLISH_FREEZE_ID,nativeRows:156,hindiRows:78,punjabiRows:78,permanentQlRange:"TSD-QL-071..TSD-QL-083",rowsPerQlPerLanguage:6,selectedObjectFamiliesPerLanguage:18,selectedRouteFamiliesPerLanguage:6,independentVerifierChecks:verifierChecks,punjabiGrammarChangedRows:grammarChangedRows,hindiLearnerContentUnchangedFromV6:true,frozenStemNumericMultisetParityGate:true,optionAndAnswerByteIdentityFromV6:true,localizedPunjabiActorFamilyGate:true,legacyPunjabiActorWordGate:true,punjabiActorCaseGrammarGate:true,nativeScriptLeakGate:true,englishLeakGate:true,exactReviewSurfaceValidated:"V7",explanationStepsPerQuestion:2,multilingualFreezeStatus:"UNFROZEN",productOwnerApprovalRecorded:false,questionStudioEnabled:false,questionBankStatus:"NOT_STORED",testEligibility:"INELIGIBLE",publiclyPublishable:false,nextGate:"MANUAL_PUNJABI_V7_ARTIFACT_AUDIT_BEFORE_PRODUCT_OWNER_REVIEW"},null,2));
