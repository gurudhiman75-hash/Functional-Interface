...
function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`TSD-CP-012 permanent QL allocation proof failed: ${message}`);
}

assert(TSD_CP012_QL_ALLOCATION_STATUS === "PERMANENT_FROZEN", "allocation status must be permanent and frozen");
assert(TSD_CP012_QL_ALLOCATION.length === 11, "expected one permanent QL for each of eleven mathematical authorities");
assert(TSD_CP012_PERMANENT_QL_IDS.length === 11, "expected eleven permanent QL IDs");
assert(new Set(TSD_CP012_PERMANENT_QL_IDS).size === 11, "permanent QL IDs must be unique");
assert(new Set(TSD_CP012_QL_ALLOCATION.map((x) => x.authorityKey)).size === 11, "each authority must receive exactly one permanent QL");
assert(TSD_CP012_NEXT_PERMANENT_QL === "TSD-QL-143", "next permanent QL ID must remain TSD-QL-143");

const expectedIds = Array.from({ length: 11 }, (_, index) => `TSD-QL-${132 + index}`);
assert(expectedIds.every((id, index) => TSD_CP012_PERMANENT_QL_IDS[index] === id), "allocation must be contiguous TSD-QL-132..TSD-QL-142");
...
assert(TSD_CP012_SOURCE_SUMMARY.frozen === true, "source authority must reflect the frozen checkpoint");
assert(TSD_CP012_SOURCE_SUMMARY.questionStudioRegistered === false, "content freeze must not register Studio");
assert(TSD_CP012_SOURCE_SUMMARY.bankWritable === false, "content freeze must not enable Bank writes");
assert(TSD_CP012_SOURCE_SUMMARY.testEligible === false, "content freeze must not enable tests");
assert(TSD_CP012_SOURCE_SUMMARY.publiclyPublishable === false, "content freeze must not enable publishing");

assert(TSD_CP012_QL_LIFECYCLE.productOwnerApproved === true, "approved lifecycle must record product-owner approval");
assert(TSD_CP012_QL_LIFECYCLE.frozen === true, "permanent QL allocation must be frozen");
...
console.log("TSD-CP-012 PERMANENT FROZEN QL ALLOCATION + LIFECYCLE PROOF: PASS");
console.log(JSON.stringify({
  allocationStatus: TSD_CP012_QL_ALLOCATION_STATUS,
  permanentQlIds: TSD_CP012_PERMANENT_QL_IDS,
