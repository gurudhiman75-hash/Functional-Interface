...
for (const allocation of TSD_CP012_QL_ALLOCATION) {
  ...
  assert(owned.length >= 8, `${allocation.qlId}: executable evidence is too thin for ${allocation.authorityKey}`);
  assert(new Set(owned.map((x) => x.input.target)).size >= 2, `${allocation.qlId}: executable target variety is too thin`);
}
assert(TSD_CP012_TWO_ENGINE_PROVENANCE.length === 8, "TSD-QL-141 must retain eight cross-authority provenance rows");
