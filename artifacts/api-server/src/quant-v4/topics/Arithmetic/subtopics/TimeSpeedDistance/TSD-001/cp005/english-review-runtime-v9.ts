...
function sameStart(i: TsdCp005EnglishReviewQuestion["input"]): string {
  return `Travellers A and B start together from P towards Q on a ${km(required(i.routeDistance, "routeDistance"))} route. A travels at ${kmph(required(i.speedA, "speedA"))} and B at ${kmph(required(i.speedB, "speedB"))}.`;
}
...
function renderExamStem(question: TsdCp005EnglishReviewQuestion): string {
  ...
  switch (question.solveMode) {
    ...
    case "findMeetingAfterBothTurnAtEndpoints":
      return `${oppositeStart(i)} Both continue moving and reverse immediately at each endpoint. After their first meeting, at what time from the start will they meet again?`;
    case "findNthMeetingTimeOnLine": {
      const meeting = i.nthMeeting;
      if (!meeting) throw new Error("CP005 V9 nth meeting missing");
      return `${oppositeStart(i)} Both reverse immediately at each endpoint. How long after the start will their ${ordinal(meeting)} meeting occur?`;
    }
    ...
    case "findNthMeetingPointOnLine": {
      ...
      return `${oppositeStart(i)} Both reverse immediately at each endpoint. How far from P is their ${ordinal(meeting)} meeting point?`;
    }
    ...
    case "findShuttleMeetingTime":
      return `${sameStart(i)} A reaches Q, turns back immediately and travels towards B. Find the elapsed time until the returning A meets B.`;
    case "findPassThenCatchAfterTurnaround":
      return `${sameStart(i)} A reaches Q before B, reverses immediately and then meets B while returning. Find the total time from the start to this meeting.`;
