...
const hindi99 = TSD_CP008_FROZEN_HINDI_LOCALIZATION.qls.find((ql) => ql.qlId === "TSD-QL-099");
const punjabi99 = TSD_CP008_FROZEN_PUNJABI_LOCALIZATION.qls.find((ql) => ql.qlId === "TSD-QL-099");
...
assert(TSD_CP008_LOCALIZATION_FREEZE_APPROVAL.finalLocalizationLayer === "CP008_FINAL_LOCALIZATION_WITH_QL099_UNIQUENESS_GUARD", "final semantic localization layer changed");
...
assert(hindi99?.families.every((family) => family.stem.includes("समान दिशा वाली स्थिति में पहली ट्रेन को तेज माना जाए।")), "Hindi QL099 faster-train uniqueness guard missing");
assert(punjabi99?.families.every((family) => family.stem.includes("ਇੱਕੋ ਦਿਸ਼ਾ ਵਾਲੀ ਸਥਿਤੀ ਵਿੱਚ ਪਹਿਲੀ ਰੇਲਗੱਡੀ ਨੂੰ ਤੇਜ਼ ਮੰਨਿਆ ਜਾਵੇ।")), "Punjabi QL099 faster-train uniqueness guard missing");
