...
function localizeTimelineEvent(locale: TsdCp007Locale, value: string): string {
  if (value === "the engine passes the fixed marker") return locale === "hi-IN" ? "इंजन स्थिर चिन्ह को पार करता है" : "ਇੰਜਣ ਨਿਰਧਾਰਤ ਨਿਸ਼ਾਨ ਨੂੰ ਪਾਰ ਕਰਦਾ ਹੈ";
  if (value === "the rear passes the same fixed marker") return locale === "hi-IN" ? "ट्रेन का पिछला सिरा उसी स्थिर चिन्ह को पार करता है" : "ਟ੍ਰੇਨ ਦਾ ਪਿਛਲਾ ਸਿਰਾ ਉਸੇ ਨਿਰਧਾਰਤ ਨਿਸ਼ਾਨ ਨੂੰ ਪਾਰ ਕਰਦਾ ਹੈ";
  ...
  if (value.startsWith("the front enters the ")) return locale === "hi-IN" ? `ट्रेन का अगला सिरा ${localized} में प्रवेश करता है` : `ਟ੍ਰੇਨ ਦਾ ਅਗਲਾ ਸਿਰਾ ${localized} ਵਿੱਚ ਦਾਖਲ ਹੁੰਦਾ ਹੈ`;
  if (value.startsWith("the rear leaves the far end of the ")) return locale === "hi-IN" ? `ट्रेन का पिछला सिरा ${localized} के दूसरे छोर से बाहर निकलता है` : `ਟ੍ਰੇਨ ਦਾ ਪਿਛਲਾ ਸਿਰਾ ${localized} ਦੇ ਦੂਜੇ ਸਿਰੇ ਤੋਂ ਬਾਹਰ ਨਿਕਲਦਾ ਹੈ`;
  if (value.startsWith("the rear enters the ")) return locale === "hi-IN" ? `ट्रेन का पिछला सिरा ${localized} में प्रवेश करता है` : `ਟ੍ਰੇਨ ਦਾ ਪਿਛਲਾ ਸਿਰਾ ${localized} ਵਿੱਚ ਦਾਖਲ ਹੁੰਦਾ ਹੈ`;
  if (value.startsWith("the front reaches the far end of the ")) return locale === "hi-IN" ? `ट्रेन का अगला सिरा ${localized} के दूसरे छोर तक पहुंचता है` : `ਟ੍ਰੇਨ ਦਾ ਅਗਲਾ ਸਿਰਾ ${localized} ਦੇ ਦੂਜੇ ਸਿਰੇ ਤੱਕ ਪਹੁੰਚਦਾ ਹੈ`;
  return value;
}
...
function registryFor(locale: TsdCp007Locale): readonly TsdCp007LocalizedQlSpec[] {
  return locale === "hi-IN" ? TSD_CP007_HINDI_LOCALIZATION : TSD_CP007_PUNJABI_LOCALIZATION;
}
...
export function renderCp007LocalizedReviewQuestions(): readonly TsdCp007RenderedLocalizedQuestion[] {
  ...
  const frozenEnglishFamilies = new Map(TSD_CP007_FROZEN_ENGLISH_REGISTRY.flatMap((ql) => ql.stemFamilies.map((family) => [family.familyId, family] as const)));
