...
function firstMeetingStem(english: TsdCp004Question, language: TsdCp004NativeLanguage): string | null {
  ...
  if (language === "hi") {
    ...
    if (s.variant === 1) return `${B}, ${A} से ${gap} km आगे है और ${b} km/h की गति से उसी दिशा में चलता है। यदि ${A} उसी समय पीछे से ${a} km/h की गति से चले, तो वह ${B} को कितने मिनट बाद पकड़ेगा?`;
  ...
  if (s.variant === 1) return `${B}, ${A} ਤੋਂ ${gap} km ਅੱਗੇ ਹੈ ਅਤੇ ${b} km/h ਦੀ ਰਫ਼ਤਾਰ ਨਾਲ ਉਸੇ ਦਿਸ਼ਾ ਵਿੱਚ ਚੱਲਦਾ ਹੈ। ਜੇ ${A} ਉਸੇ ਸਮੇਂ ਪਿੱਛੋਂ ${a} km/h ਦੀ ਰਫ਼ਤਾਰ ਨਾਲ ਚੱਲੇ, ਤਾਂ ਉਹ ${B} ਨੂੰ ਕਿੰਨੇ ਮਿੰਟ ਬਾਅਦ ਫੜੇਗਾ?`;
...
export function renderCp004EditorialV5NativeQuestion(english: TsdCp004Question, language: TsdCp004NativeLanguage): TsdCp004FinalNativeQuestion {
  const base = renderCp004EditorialV4NativeQuestion(english, language);
  return Object.freeze({ ...base, stem: firstMeetingStem(english, language) ?? base.stem });
}
