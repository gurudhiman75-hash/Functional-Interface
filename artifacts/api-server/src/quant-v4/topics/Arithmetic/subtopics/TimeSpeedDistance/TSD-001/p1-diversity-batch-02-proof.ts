...
assert(rows.length === 125, "Batch 02 compatibility gate expects the Batch 03-expanded 125-record pool");
...
assert(
  ...
  "Distance-share supplement input drifted",
);
...
assert(
  ...
  "Distance-ratio supplement input drifted",
);

const printedNumbers = [distanceShareSupplement, distanceRatioSupplement]
  .flatMap((row) => row?.stem.match(/\d+(?:\.\d+)?/g) ?? []);
