...
export function generateCp004EnglishQuestion(authorityId: TsdCp004AuthorityId, seed: string): TsdCp004Question {
  ...
  const question: TsdCp004Question = Object.freeze({
    ...
    state,
    stem: renderCp004EnglishStem(state),
  ...
  if (errors.length) throw new Error(`Invalid CP004 question ${authorityId}/${seed}: ${errors.join("; ")}`);
  return question;
}

export function generateCp004EnglishCorpus(seedsPerAuthority = 50): readonly TsdCp004Question[] {
  ...
  for (const authority of TSD_CP004_AUTHORITIES) {
    for (let i = 0; i < seedsPerAuthority; i += 1) {
      questions.push(generateCp004EnglishQuestion(authority.authorityId, `cp004-${authority.authorityId.toLowerCase()}-${String(i + 1).padStart(3, "0")}`));
    }
  }
  return Object.freeze(questions);
}

export function generateCp004EnglishReviewCorpus(): readonly TsdCp004Question[] {
  const questions: TsdCp004Question[] = [];
  for (const authority of TSD_CP004_AUTHORITIES) {
    for (let variant = 0; variant < 3; variant += 1) {
      // Search a bounded deterministic seed window for the requested stem variant.
      let found: TsdCp004Question | null = null;
      for (let i = variant; i < 300; i += 3) {
        const candidate = generateCp004EnglishQuestion(authority.authorityId, `cp004-review-${authority.authorityId.toLowerCase()}-${i}`);
        if (candidate.state.variant === variant) {
          found = candidate;
          break;
        }
      }
      if (!found) throw new Error(`Unable to find review variant ${variant} for ${authority.authorityId}`);
...
export function validateCp004EnglishCorpus(corpus: readonly TsdCp004Question[]): {
  readonly questions: number;
  readonly authorities: number;
  readonly uniqueStems: number;
  readonly uniqueMathFingerprints: number;
  readonly answerPositions: readonly [number, number, number, number];
  readonly difficulties: Readonly<Record<"Easy" | "Medium" | "Hard", number>>;
} {
  ...
  for (const question of corpus) {
    const errors = validateQuestion(question);
    if (errors.length) throw new Error(`CP004 corpus validation failed: ${errors.join("; ")}`);
