...
import { generateCp005EnglishAuditPoolV3, generateCp005ReviewSetV3 } from "./english-review-runtime-v3";
...
const audit = generateCp005EnglishAuditPoolV3(30);
const selected = generateCp005ReviewSetV3(6);
...
assert(audit.every((row) => row.validation.valid), "CP005 English V3 audit contains invalid questions");
assert(selected.every((row) => row.validation.valid), "CP005 English V3 review set contains invalid questions");
assert(new Set(selected.map((row) => row.stem)).size === selected.length, "CP005 V3 selected stems are not globally unique");
assert(new Set(selected.map((row) => row.mathematicalFingerprint)).size === selected.length, "CP005 V3 selected mathematical fingerprints are not globally unique");
assert(new Set(audit.map((row) => row.mathematicalFingerprint)).size === audit.length, "CP005 V3 audit mathematical fingerprints are not globally unique");
assert(new Set(selected.map((row) => row.permanentQlId)).size === 13, "CP005 V3 review does not cover all permanent QLs");
assert(new Set(selected.map((row) => row.solveMode)).size === 20, `CP005 V3 must cover 20 retained/merged learner solve modes, covered ${new Set(selected.map((row) => row.solveMode)).size}`);

for (const ql of TSD_CP005_PERMANENT_QL_IDS) {
  const rows = selected.filter((row) => row.permanentQlId === ql);
  assert(rows.length === 6, `${ql}: expected six V3 English review questions`);
...
for (const authority of TSD_CP005_APPROVED_LEARNER_AUTHORITIES) {
  ...
  assert(rows.length === 30, `${authority.authorityKey}: expected thirty V3 audit questions`);
  assert(new Set(rows.map((row) => row.mathematicalFingerprint)).size === 30, `${authority.authorityKey}: V3 mathematical diversity too weak`);
}
...
assert(selected.every((row) => !heldModes.has(row.solveMode)), "cross-checkpoint held mode leaked into V3 learner review");
assert(selected.every((row) => !qaModes.has(row.solveMode)), "internal-QA mode leaked into V3 learner review");
assert(audit.every((row) => row.options.length === 4 && new Set(row.options).size === 4), "CP005 V3 option uniqueness failed");
assert(audit.every((row) => row.internalOptionAudit.filter((entry) => !entry.isCorrect && entry.wrongWorking).length === 3), "CP005 V3 question missing three misconception-owned wrong workings");
assert(audit.every((row) => row.explanation.steps.length >= 2 && row.explanation.finalAnswer.includes(row.answerText)), "CP005 V3 learner explanation contract failed");
assert(audit.every((row) => (row.explanation as unknown as Record<string, unknown>).optionAnalysis === undefined), "CP005 V3 public option analysis leaked");
assert(audit.every((row) => row.lifecycle.englishFreezeStatus === "UNFROZEN" && !row.lifecycle.questionStudioEnabled && row.lifecycle.questionBankStatus === "NOT_STORED" && row.lifecycle.testEligibility === "INELIGIBLE" && !row.lifecycle.publiclyPublishable), "CP005 V3 downstream lifecycle lock violated");

const correctOptionPositions = [0, 1, 2, 3].map((position) => audit.filter((row) => row.correctIndex === position).length);
assert(correctOptionPositions.every((count) => count >= 75), `CP005 V3 answer-position balance too weak: ${correctOptionPositions.join(",")}`);
...
assert(difficulty.EASY === 24 && difficulty.MEDIUM === 36 && difficulty.HARD === 18, `unexpected CP005 V3 difficulty mix ${JSON.stringify(difficulty)}`);
...
assert(exactWrongWorkings === 1170, `expected 1170 CP005 V3 exact wrong workings, received ${exactWrongWorkings}`);
...
console.log(JSON.stringify({
  status: "PASS",
  phase: "TSD_CP005_ENGLISH_REVIEW_CANDIDATE_V3",
  ...
  difficulty,
  distractorContract: "THREE_DISTINCT_SEMANTIC_PATHS_WITH_TARGETED_V3_INVERSE_FIX",
