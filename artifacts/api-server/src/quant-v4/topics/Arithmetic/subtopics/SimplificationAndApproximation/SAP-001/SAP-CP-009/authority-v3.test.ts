...
function basics(pkg: SapCp009Package): void {
  ...
  const visible = `${pkg.stem} ${pkg.explanation.coreConcept} ${pkg.explanation.steps.join(" ")} ${pkg.explanation.verification.join(" ")}`;
...
for (const prototypeId of SAP_CP009_PROTOTYPE_IDS) {
  ...
  for (let seed = 1; seed <= 100; seed += 1) {
    ...
    payloads.add(pkg.canonicalPayloadKey); identities.add(pkg.generationIdentity); localStems.add(pkg.stem);
    positions[pkg.correctIndex]! += 1; qls.add(pkg.proposedPermanentQlId); total += 1;
...
assert.equal(payloads.size, 1900);
assert.equal(identities.size, 1900);
assert.equal(SAP_CP009_CATALOGUE.length, 19);
assert.deepEqual([...qls].sort(), Array.from({ length: 19 }, (_, i) => `SAP-QL-${String(147 + i).padStart(3, "0")}`));
assert.deepEqual(positions, [475, 475, 475, 475]);
...
console.log("SAP-CP-009 authority passed: 1,900 independently verified unique cases across SAP-QL-147..165, 100 stems per identity, all ratio relations, both over/under classes, 475 A/B/C/D positions each, short student explanations and inactive lifecycle.");
