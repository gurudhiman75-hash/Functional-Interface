...
const markdown: string[] = [
  ...
  "**Candidate coordinates:** SAP-QL-092 through SAP-QL-111  ",
  "**Coverage:** 20 solve modes × 15 unique questions = 300 questions  ",
...
const prototypeOptions = SAP_CP006_FULL_REVIEW_CATALOGUE.map((item) =>
...
</style></head><body><header><div class="toolbar"><input id="search" placeholder="Search question, QL, value…"><select id="prototype"><option value="">All QLs</option>${prototypeOptions}</select><select id="difficulty"><option value="">All difficulties</option><option>MEDIUM</option><option>HARD</option></select><select id="direction"><option value="">All directions</option><option>INVERSE</option><option>COMPARISON</option><option>ORDERING</option><option>SYNTHESIS</option><option>VERIFICATION</option></select><button id="expand">Expand visible</button><button id="collapse">Collapse all</button></div></header><main><section class="intro"><h1>SAP-CP-006 — 300-Question English Review Candidate</h1><p><strong>Missing Values, Equality, Comparison and Exact Synthesis</strong></p><p class="warning"><b>Review boundary:</b> SAP-QL-092..SAP-QL-111 are candidate coordinates only. This artifact is for semantic and exam-readiness review; all runtime lifecycle surfaces remain inactive.</p></section><div class="stats"><span class="pill"><span id="visible">300</span> visible</span><span class="pill">20 solve modes</span><span class="pill">15 questions per mode</span><span class="pill">75 answers per A/B/C/D position</span></div><section class="list">${cards}</section><div id="empty" class="empty">No questions match the current filters.</div></main><script>
...
console.log(JSON.stringify({
  status: "WROTE_SAP_CP006_FULL_ENGLISH_REVIEW",
  questionCount: records.length,
  admittedModes: 20,
  questionsPerMode: 15,
  candidateQlRange: "SAP-QL-092..SAP-QL-111",
  markdownPath,
  htmlPath,
  jsonPath,
