...
const accuracyByPosition = [new Set<number>(), new Set<number>(), new Set<number>(), new Set<number>()];
...
for (let seed = 1; seed <= 100; seed += 1) {
  ...
  const expectedNumber = d1 < d2 ? 1 : 2;
  assert.ok(accuracy.canonicalAnswer.startsWith(expected), `accuracy/${seed}: wrong better estimate`);
  accuracyByPosition[accuracy.correctIndex]!.add(expectedNumber);
  expectedNumber === 1 ? estimate1++ : estimate2++;
...
for (let i = 0; i < 4; i += 1) assert.deepEqual([...accuracyByPosition[i]!].sort(), [1, 2], `answer position ${i} must contain both accuracy outcomes`);
assert.deepEqual([...classes].sort(), ["Impossible", "Multiple", "Unique"]);
console.log(JSON.stringify({authority:"SAP-E2-R6-EXAM-READINESS",states:400,estimateAccuracy:{estimate1,estimate2,allAnswerPositionsContainBothOutcomes:true},outcomeClasses:[...classes].sort(),difficultyCalibration:{count:"MEDIUM",outcome:"MEDIUM",roundedOperand:"MEDIUM"},roundedOperandStem:"UNAMBIGUOUS_EXACT_INTERVAL",canonicalPayloadStemBinding:true,lifecycle:"INACTIVE"}));
