...
function passesVariety(prototypeId: SapCp008PrototypeId, accepted: number, pkg: SapCp008Package): boolean {
  ...
  if ([
    ...
    return Number(d.unit) === UNITS[accepted % 2];
  }
  if (prototypeId === "SAP-CP008-PROT-COMPARE-ADDITIVE-ESTIMATES") {
    return pkg.canonicalAnswer === RELATIONS[accepted % 3];
  }
  if (prototypeId === "SAP-CP008-PROT-OVER-UNDER-CLASS") {
    return pkg.canonicalAnswer === OVER_UNDER[accepted % 2];
  }
  return true;
}
