...
function solveOracle(oracle: SapCp004Oracle): string {
  ...
  switch (oracle.kind) {
    case "POWER_EXPRESSION": {
      const p = power(BigInt(d.base!), d.exponent!);
      if (mode === 1) return (p - BigInt(d.add!)).toString();
      if (mode === 2) return (BigInt(d.multiplier!) * p + BigInt(d.add!)).toString();
      if (mode === 3) return ((p + BigInt(d.adjustment!)) / BigInt(d.divisor!)).toString();
      return (p + BigInt(d.add!)).toString();
    }
...
function transitions(sequence: readonly number[], predicate: (previous: number, current: number) => boolean): number {
  let count = 0;
  for (let index = 1; index < sequence.length; index += 1) if (predicate(sequence[index - 1]!, sequence[index]!)) count += 1;
  return count;
}
...
function fourGrams(sequence: readonly number[]): ReadonlyMap<string, number> {
...
for (const pkg of sweep) {
  ...
  assert.doesNotMatch([pkg.stem, pkg.explanation.coreConcept, ...pkg.explanation.steps, pkg.explanation.finalAnswer, ...pkg.options.map((option) => option.analysis)].join("\n"), BANNED);
  ...
  if (pkg.prototypeId === "SAP-CP004-PROT-NEGATIVE-BASE-PARITY") {
    assert.ok(pkg.options.some((option) => option.misconceptionId?.includes("PARITY")));
  }
  if (pkg.prototypeId === "SAP-CP004-PROT-BOUNDED-NTH-ROOT" && pkg.oracle.data.index === 4) assert.match(pkg.stem, /principal|positive|non-negative/i);
  ...
  if (pkg.taskDirection === "DIAGNOSIS") {
    const answers = diagnosisAnswers.get(pkg.prototypeId) ?? new Set<string>();
    answers.add(pkg.canonicalAnswer);
    diagnosisAnswers.set(pkg.prototypeId, answers);
  }
}
...
for (const prototypeId of SAP_CP004_PROTOTYPE_IDS) {
  ...
  if (DIVERSIFIED.has(prototypeId)) assert.equal(framesByPrototype.get(prototypeId)?.size, 4, `${prototypeId} must expose four genuine structures.`);
}
assert.deepEqual([...comparisonAnswers].sort(), ["A < B", "A = B", "A > B"]);
for (const answers of diagnosisAnswers.values()) assert.deepEqual([...answers].sort(), ["No error", "Step 1", "Step 2", "Step 3"]);
...
const gramCounts = fourGrams(sequence);
const maximumGramFrequency = Math.max(...gramCounts.values());
const same = transitions(sequence, (previous, current) => previous === current);
const forward = transitions(sequence, (previous, current) => current === (previous + 1) % 4);
assert.deepEqual(distribution, [75, 75, 75, 75]);
assert.equal(maximumRun(sequence), 2);
assert.ok(same >= 35 && same <= 90);
assert.ok(forward <= 110);
assert.ok(cyclicFourWindows(sequence) <= 20);
assert.ok(gramCounts.size >= 130);
assert.ok(maximumGramFrequency <= 7);

console.log(JSON.stringify({
  status: "PASS_SAP_CP004_DIVERSIFIED_ENGLISH_CANDIDATE_AUTHORITY",
  packagesTested: sweep.length,
  uniqueGenerationIdentities: identities.size,
  prototypeCount: countsByPrototype.size,
  diversifiedPrototypeCount: DIVERSIFIED.size,
  ...
  samePositionTransitions: same,
  forwardCycleTransitions: forward,
  cyclicFourAnswerWindows: cyclicFourWindows(sequence),
  distinctFourAnswerPatterns: gramCounts.size,
