...
function solveOracle(oracle: SapCp004Oracle): string {
  ...
  switch (oracle.kind) {
    ...
    case "POWER_ROOT_CANCELLATION": return (exactRoot(power(BigInt(d.base!), 2), 2) + BigInt(d.add!)).toString();
    case "NESTED_ROOT": {
...
for (const pkg of sweep) {
  ...
  if (pkg.prototypeId === "SAP-CP004-PROT-COMPARE-POWER-ROOT-EXPRESSIONS") comparisonAnswers.add(pkg.canonicalAnswer);
  if (pkg.prototypeId === "SAP-CP004-PROT-FIRST-INCORRECT-POWER-ROOT-STEP") powerDiagnosisAnswers.add(pkg.canonicalAnswer);
  if (pkg.prototypeId === "SAP-CP004-PROT-FIRST-INCORRECT-FACTORIAL-STEP") factorialDiagnosisAnswers.add(pkg.canonicalAnswer);
}
...
assert.ok(maximumRun(sequence) <= 2);
assert.ok(forwardTransitions(sequence) <= 130);
assert.ok(cyclicFourWindows(sequence) <= 45);
assert.ok(grams.size >= 70);
assert.ok(maximumGramFrequency <= 12);

console.log(JSON.stringify({
  status: "PASS_SAP_CP004_COMPLETE_ENGLISH_CANDIDATE_AUTHORITY",
