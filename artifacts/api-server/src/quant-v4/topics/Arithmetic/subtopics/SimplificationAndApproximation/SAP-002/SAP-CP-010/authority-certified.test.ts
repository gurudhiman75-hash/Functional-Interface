...
function verifyMath(q: SapCp010Package): void {
  ...
  switch (q.prototypeId) {
    ...
    case "SAP-CP010-PROT-PERCENT-POWER-FACTOR": {
      ...
      assert.ok(Math.abs(Number(q.canonicalAnswer) - expected) < 1e-9);
      break;
    }
    ...
    case "SAP-CP010-PROT-MISSING-RADICAND":
      ...
      assert.equal(Number(q.canonicalAnswer), Number(d.correctN));
      break;
...
assert.deepEqual(
  ...
  Array.from({ length: 17 }, (_, i) => `SAP-QL-${String(166 + i).padStart(3, "0")}`),
);
...
for (const prototypeId of SAP_CP010_PROTOTYPE_IDS) {
  ...
  for (let seed = 1; seed <= 100; seed += 1) {
    ...
    assert.doesNotMatch(visible, /-?\d+\.\d{6,}/, `${prototypeId}:${seed}: raw floating-point display`);
    assert.doesNotMatch(q.stem, /using\s+-?\d+(?:\.\d+)?\s+for\s+-?\d+(?:\.\d+)?/i, `${prototypeId}:${seed}: replacement value supplied in stem`);
    ...
    assert.ok(!identities.has(q.generationIdentity), `${prototypeId}:${seed}: duplicate generation identity`);
    stems.add(q.stem);
    ...
    if (prototypeId === SAP_CP010_PROTOTYPE_IDS[14]) kinds.add(String(q.oracle.data.kind));
...
console.log("SAP-CP-010 certified authority passed: 1,700 independently verified release states across 17 identities, 100 unique stems per identity, exact 425/425/425/425 answer balance, strict lifecycle locks and no unsupported numerical methods.");
