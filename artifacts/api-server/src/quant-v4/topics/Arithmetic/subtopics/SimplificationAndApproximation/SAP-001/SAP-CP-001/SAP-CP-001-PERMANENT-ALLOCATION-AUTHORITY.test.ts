...
const registryIds = SAP_PERMANENT_QL_REGISTRY.map((entry) => entry.permanentQlId);
const registryTemplates = SAP_PERMANENT_QL_REGISTRY.map((entry) => entry.templateId);
...
assert.equal(SAP_PERMANENT_QL_REGISTRY.length, 16);
assert.equal(new Set(permanentIds).size, 16, "Permanent QL IDs must be unique");
assert.equal(new Set(registryIds).size, 16, "Registry IDs must be unique");
assert.equal(new Set(registryTemplates).size, 16, "Every template must be allocated once");
assert.deepEqual(registryIds, permanentIds, "Registry order must equal the approved contiguous ID order");
assert.deepEqual(registryTemplates, [...SAP_CP001_ENGLISH_TEMPLATE_IDS]);

for (let index = 0; index < permanentIds.length; index += 1) {
  const expected = `SAP-QL-${String(index + 1).padStart(3, "0")}`;
  assert.equal(permanentIds[index], expected, "The allocated range must remain contiguous and gap-free");
}
...
assert.equal(SAP_PERMANENT_QL_REGISTRY_STATE.registryVersion, 1);
assert.equal(SAP_PERMANENT_QL_REGISTRY_STATE.allocatedTemplateCount, 16);
assert.equal(SAP_PERMANENT_QL_REGISTRY_STATE.firstAllocatedId, "SAP-QL-001");
assert.equal(SAP_PERMANENT_QL_REGISTRY_STATE.lastAllocatedId, "SAP-QL-016");
assert.equal(SAP_PERMANENT_QL_REGISTRY_STATE.nextAvailableId, "SAP-QL-017");
...
for (const pkg of packages) {
  ...
  assert.equal(pkg.registryEntry.packageId, "SAP-001");
  ...
  assert.equal(pkg.technicalDetails.discoveryValidation.ok, true);
  assert.ok(pkg.technicalDetails.independentEvidence.length >= 1);
...
for (const permanentQlId of SAP_CP001_PERMANENT_QL_IDS) {
  const registryEntry = SAP_PERMANENT_QL_BY_ID[permanentQlId];
  const expectedPackageCount = registryEntry.prototypeAncestry.length * 100;
  assert.equal(packageCountByQl.get(permanentQlId), expectedPackageCount);
  ...
  assert.deepEqual(
    [...prototypesByQl.get(permanentQlId)!].sort(),
    [...registryEntry.prototypeAncestry].sort(),
  );
}

for (const prototypeId of SAP_CP001_ALL_PROTOTYPE_IDS) {
  for (const seed of [1, 2, 3]) {
    const frozenCandidate = generateSapCp001EnglishCandidate(prototypeId, seed);
    assert.equal(frozenCandidate.permanentQlId, null,
      "Historical English-freeze evidence must remain ID-free and immutable");
  }
}

console.log(JSON.stringify({
  status: "PASS_SAP_CP001_PERMANENT_ALLOCATION_AUTHORITY",
  allocatedRange: SAP_PERMANENT_QL_REGISTRY_STATE.allocatedRange,
  ...
  qlPackageCounts: Object.fromEntries(packageCountByQl),
  activeQlCount: 0,
