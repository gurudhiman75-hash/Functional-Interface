...
function parseFixed(value: string, dp: number): bigint {
  const negative = value.startsWith("-");
  const unsigned = negative ? value.slice(1) : value;
  const [whole, fraction = ""] = unsigned.split(".");
  const padded = fraction.padEnd(dp, "0");
  assert.ok(padded.length === dp, `Cannot parse ${value} at ${dp} decimal places.`);
  const scaled = BigInt(whole || "0") * pow10(dp) + BigInt(padded || "0");
  ...
  switch (pkg.prototypeId) {
    ...
    case "SAP-CP007-PROT-NEGATIVE-HALFWAY-EXPLICIT-RULE": {
      ...
      if (pkg.prototypeId === "SAP-CP007-PROT-NEGATIVE-HALFWAY-EXPLICIT-RULE") {
        ...
        const absolute = BigInt(d.scaled as number) < 0n ? -BigInt(d.scaled as number) : BigInt(d.scaled as number);
...
console.log("SAP-CP-007 foundation authority passed: 1,200 deterministic cases across 12 rounding identities with exact fixed-point proof, reverse-interval reconstruction, 300 A/B/C/D positions each, and inactive candidate lifecycle.");
