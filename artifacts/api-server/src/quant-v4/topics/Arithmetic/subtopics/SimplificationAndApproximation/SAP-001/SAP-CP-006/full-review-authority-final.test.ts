...
assert.equal(SAP_CP006_FULL_REVIEW_PROTOTYPE_IDS.reduce((sum, id) => sum + sapCp006FullReviewCountForPrototype(id), 0), 300);
...
for (const [index, record] of records.entries()) {
  ...
  assert.ok(record.stem.length >= 20 && record.stem.length <= maxStemLength, `${record.questionId}: stem outside review boundary.`);
  ...
  if (foundationModes.has(record.prototypeId)) {
    if (examOfFoundationModes.has(record.prototypeId)) {
      assert.match(record.stem, /\bof\b/);
    ...
    if (integerFoundationModes.has(record.prototypeId)) {
      for (const option of record.options) assert.match(option.value, /^\d+$/);
    }
  ...
  if (wave2Modes.has(record.prototypeId)) {
    ...
    assert.equal(pkg.options.filter((option) => wave2SubstitutionHolds(pkg, Number(option.value))).length, 1, `${record.questionId}: substitution ambiguity.`);
  }
...
assert.ok(tableWrapperCount >= 3, `Expected multiple table variants, found ${tableWrapperCount}.`);
...
for (const prototypeId of SAP_CP006_FULL_REVIEW_PROTOTYPE_IDS) {
  assert.equal(counts.get(prototypeId), sapCp006FullReviewCountForPrototype(prototypeId));
  const minimumAnswers = minimumAnswerDiversity[prototypeId] ?? 4;
  assert.ok((answersByPrototype.get(prototypeId)?.size ?? 0) >= minimumAnswers, `${prototypeId}: insufficient answer diversity.`);
}
