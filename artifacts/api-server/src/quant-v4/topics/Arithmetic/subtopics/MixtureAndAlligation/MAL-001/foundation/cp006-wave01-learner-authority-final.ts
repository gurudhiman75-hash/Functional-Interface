...
function finalTextPolish(text: string): string {
  ...
  return text
    ...
    .replace(new RegExp(`${amount} is transferred`, "giu"), "$1 are transferred")
    ...
    .replace(/\b([0-9]+(?:\.[0-9]+)?(?:\s+[0-9]+\/[0-9]+)? litres) goes\b/giu, "$1 are transferred")
...
function surfaceIsClean(question: MalCp006DiscoveryQuestion): boolean {
  ...
  if (/\blitres is transferred\b/iu.test(learnerText)) return false;
