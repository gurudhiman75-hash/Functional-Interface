...
import { MAL_001_QUESTION_STUDIO_CP_IDS } from "./question-studio-adapter";
...
function assertQuestionSurface(question: any, expectedQl: string, expectedCanonicalProblem: string): void {
  ...
  assert(
    question.questionLanguageId === expectedQl,
    `${expectedQl}: Question Studio QL identity drift.`,
  );
  ...
  assert(
    ...
    `${expectedQl}: answer/index mismatch.`,
  );
  assert(String(question.text ?? "").trim().length > 0, `${expectedQl}: empty stem.`);
  assert(String(question.explanation ?? "").trim().length > 0, `${expectedQl}: empty Question Studio explanation.`);
...
assert(
  Array.isArray(packageCard.supportedLanguages) && packageCard.supportedLanguages.length === 1 && packageCard.supportedLanguages[0] === "en",
  "MAL-001 Question Studio language boundary drifted.",
);
...
for (let number = 1; number <= 67; number += 1) {
  ...
  assert(
    result.generationContext?.runtimeMode === "QUESTION_STUDIO_ACTIVE",
    `${questionLanguageId}: batch runtime is not Question Studio active.`,
  );
  assert(
    ...
    `${questionLanguageId}: conservative Question Studio batch lifecycle drifted.`,
  );
  ...
  if (canonicalProblemId === "MAL-CP-006") {
    ...
    assert(pkg.questionStudioDiscoverable === true, `${questionLanguageId}: CP006 is not Question Studio discoverable.`);
    ...
    assert(preview.questionBankWritable === false, `${questionLanguageId}: CP006 preview write lock missing.`);
    assert(preview.testEligibility === "INELIGIBLE", `${questionLanguageId}: CP006 preview test lock missing.`);
    assert(preview.publiclyPublishable === false, `${questionLanguageId}: CP006 preview public lock missing.`);
...
for (const difficulty of ["Easy", "Medium", "Hard"] as const) {
  ...
  for (const question of result.questions as any[]) {
    ...
    assert(
      MAL_001_QUESTION_STUDIO_CP_IDS.includes(question.canonicalProblemId),
      `${difficulty}: unknown MAL canonical problem ${question.canonicalProblemId}.`,
    );
