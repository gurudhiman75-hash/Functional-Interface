...
function inverseStem(q: MalCp006Wave02LearnerQuestion, seed: string): MalCp006Wave02LearnerQuestion {
  ...
  const stems = [
    ...
    `After a transfer-and-return operation, vessel A has milk and water in the ratio ${milkPart}:${waterPart}. Initially, A contained ${volume} litres of pure milk and B contained ${volume} litres of pure water. The same quantity was transferred from A to B and then from the mixed liquid in B back to A. Find that quantity.`,
    ...
    `Initially, vessel A has only ${volume} litres of milk and vessel B has only ${volume} litres of water. A transfer is made from A to B, followed by an equal-volume return from the mixed contents of B to A. The resulting milk-to-water ratio in A is ${milkPart}:${waterPart}. Calculate the volume of each transfer.`,
  ] as const;
...
function chainStem(q: MalCp006Wave02LearnerQuestion, seed: string): MalCp006Wave02LearnerQuestion {
  ...
  const stems = [
    ...
    `A contains pure milk, B contains ${waterB} litres of pure water, and C is empty. First, A transfers ${firstFactor}x litres of milk to B. Next, after mixing B well, ${secondFactor}x litres of B's contents is transferred to C. C finally has water and milk in the ratio ${waterPart}:${milkPart}. Find the quantity of milk left in B.`,
    ...
    `Initially, B has ${waterB} litres of water and C is empty. In step 1, vessel A supplies ${firstFactor}x litres of pure milk to B. In step 2, B is mixed and then supplies ${secondFactor}x litres of the mixture to C. Given that C has water and milk in the ratio ${waterPart}:${milkPart}, determine the milk remaining in B.`,
    ...
    `B starts with ${waterB} litres of pure water. A adds ${firstFactor}x litres of pure milk to B. The contents of B are mixed, after which B sends ${secondFactor}x litres to empty vessel C. The mixture received by C has water and milk in the ratio ${waterPart}:${milkPart}. Calculate the amount of milk that remains in B.`,
