...
import {
  MAL_CP004_PRODUCT_REVIEW_RUNTIME_V6,
  malCp004ProductReviewV6Stable,
  runMalCp004EnglishProductReviewV6Pipeline,
} from "./foundation/cp004-product-review-runtime-v6";
import {
...
function validate(question: MalCp004ProductReviewQuestion): void {
  ...
  assert(
    ...
    `${id}: candidate claims delivery permission.`,
  );
...
for (const allocation of MAL_CP004_PERMANENT_ALLOCATION) {
  ...
  for (let index = 0; index < 200; index += 1) {
    const seed = `mal-cp004-product-review-v6:${allocation.qlId}:${index}`;
    const first = runMalCp004EnglishProductReviewV6Pipeline({
      questionLanguageId: allocation.qlId,
      seed,
      language: "en",
    });
    const second = runMalCp004EnglishProductReviewV6Pipeline({
      questionLanguageId: allocation.qlId,
      seed,
      language: "en",
    });
    assert(
      malCp004ProductReviewV6Stable(first) ===
        malCp004ProductReviewV6Stable(second),
    ...
    assert(
      malCp004ProductReviewV6Stable(studio) ===
        malCp004ProductReviewV6Stable(first),
...
assert(v4Questions === 800, `Expected 800 V4 questions, received ${v4Questions}.`);
...
writeFileSync(
  resolve(outputDir, "mal-cp004-product-review-v6-review.json"),
  `${stable({
    candidate: MAL_CP004_PRODUCT_REVIEW_CANDIDATE,
    runtimeId: MAL_CP004_PRODUCT_REVIEW_RUNTIME_V6,
    answerPositions,
    review,
  })}\n`,
  "utf8",
...
console.log(
  JSON.stringify(
    {
      status: "PASS_MAL_CP004_PRODUCT_REVIEW_RUNTIME_V6",
      generated,
      deterministic,
      sourceParity,
      studioParity,
