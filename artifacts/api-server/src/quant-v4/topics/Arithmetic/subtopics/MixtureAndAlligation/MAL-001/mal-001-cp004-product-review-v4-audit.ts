...
import {
  MAL_CP004_PRODUCT_REVIEW_RUNTIME_V4,
  malCp004ProductReviewV4Stable,
  runMalCp004EnglishProductReviewV4Pipeline,
} from "./foundation/cp004-product-review-runtime-v4";
import {
...
function check(question: MalCp004ProductReviewQuestion): void {
  ...
  assert(
    ...
    `${id}: dishonest presentation lifecycle.`,
  );
  assert(
    !question.active &&
  ...
  assert(question.questionStudioDiscoverable, `${id}: review preview is unavailable.`);
  ...
  assert(
    ...
    `${id}: correct position does not contain the answer.`,
  );
  assert(question.optionAudit.length === 4, `${id}: option audit is incomplete.`);
  assert(
    question.optionAudit.filter((entry) => entry.isCorrect).length === 1,
    `${id}: option audit does not contain exactly one correct entry.`,
  );
  ...
  if (alternative) {
    assert(
      ...
      `${id}: shared responsive visual directive is missing.`,
    );
...
assert(
  ...
  "Candidate falsely claims product approval.",
);
...
for (const allocation of MAL_CP004_PERMANENT_ALLOCATION) {
  ...
  for (let index = 0; index < seedsPerQl; index += 1) {
    const seed = `mal-cp004-product-review-v4:${allocation.qlId}:${index}`;
    const first = runMalCp004EnglishProductReviewV4Pipeline({
      questionLanguageId: allocation.qlId,
      seed,
      language: "en",
    });
    const second = runMalCp004EnglishProductReviewV4Pipeline({
      questionLanguageId: allocation.qlId,
      seed,
      language: "en",
    });
    assert(
      malCp004ProductReviewV4Stable(first) ===
        malCp004ProductReviewV4Stable(second),
    ...
    assert(
      ...
      `${allocation.qlId}/${seed}: selected source state changed mathematics.`,
    );
    sourceParity += 1;
    ...
    assert(
      malCp004ProductReviewV4Stable(studio) ===
        malCp004ProductReviewV4Stable(first),
      `${allocation.qlId}/${seed}: Question Studio differs from the candidate runtime.`,
    );
    studioParity += 1;
    ...
    if (
      (first.parameters as Record<string, unknown>).productReviewRuntimeId ===
      MAL_CP004_PRODUCT_REVIEW_RUNTIME_V4
    ) {
      v4Overrides += 1;
    }
...
assert(deterministic === 2_000, "Determinism proof is incomplete.");
assert(sourceParity === 2_000, "Selected-state parity proof is incomplete.");
assert(studioParity === 2_000, "Question Studio parity proof is incomplete.");
...
assert(v4Overrides === 800, `Expected 800 V4 edge-state overrides, received ${v4Overrides}.`);
...
writeFileSync(
  resolve(outputDir, "mal-cp004-product-review-v4-review.json"),
  `${stable({
    candidate: MAL_CP004_PRODUCT_REVIEW_CANDIDATE,
    runtimeId: MAL_CP004_PRODUCT_REVIEW_RUNTIME_V4,
    answerPositions,
    review,
  })}\n`,
  "utf8",
...
console.log(
  JSON.stringify(
    {
      status: "PASS_MAL_CP004_PRODUCT_REVIEW_RUNTIME_V4",
      generated,
      deterministic,
      sourceParity,
      studioParity,
