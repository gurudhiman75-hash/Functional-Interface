...
function normalizeReleaseStem(text: string): string {
  const normalized = normalizeCostPriceText(text).trim();
  if (normalized.endsWith("?")) return normalized;
  const interrogative = normalized.replace(
    /Find the total profit\.$/u,
    "What is the total profit?",
  );
  if (!interrogative.endsWith("?")) {
    throw new Error(`Permanent CP-005 release stem is not interrogative: ${normalized}`);
  }
...
export function generateMalCp005PermanentQuestion(
  ...
  const stem = normalizeReleaseStem(source.stem);
  ...
  if (!stem.endsWith("?")) {
    throw new Error(`${qlId}/${seed}: permanent release stem is not interrogative.`);
  }
  ...
  const checks = [
    ...
    {
      name: "INTERROGATIVE_RELEASE_STEM",
      passed: true as const,
      message: "The permanent learner-facing stem ends as an explicit question.",
    },
    {
