...
import {
  MAL_CP004_PRODUCT_REVIEW_RUNTIME_V5,
  malCp004ProductReviewV5Stable,
  runMalCp004EnglishProductReviewV5Pipeline,
} from "./foundation/cp004-product-review-runtime-v5";
import {
...
function check(question: MalCp004ProductReviewQuestion): void {
  ...
  assert(question.questionStudioDiscoverable, `${id}: Question Studio preview is unavailable.`);
  ...
  assert(
    ...
    `${id}: correct option does not match the answer.`,
  );
  assert(question.optionAudit.length === 4, `${id}: option audit incomplete.`);
  assert(
  ...
  if (alternative) {
    assert(
      ...
      `${id}: shared responsive visual is missing.`,
    );
...
for (const allocation of MAL_CP004_PERMANENT_ALLOCATION) {
  ...
  for (let index = 0; index < seedsPerQl; index += 1) {
    const seed = `mal-cp004-product-review-v5:${allocation.qlId}:${index}`;
    const first = runMalCp004EnglishProductReviewV5Pipeline({
      questionLanguageId: allocation.qlId,
      seed,
      language: "en",
    });
    const second = runMalCp004EnglishProductReviewV5Pipeline({
      questionLanguageId: allocation.qlId,
      seed,
      language: "en",
    });
    assert(
      malCp004ProductReviewV5Stable(first) ===
        malCp004ProductReviewV5Stable(second),
    ...
    assert(
      malCp004ProductReviewV5Stable(studio) ===
        malCp004ProductReviewV5Stable(first),
      `${allocation.qlId}/${seed}: Question Studio differs from V5.`,
    );
    studioParity += 1;
...
assert(sourceParity === 2_000, "Selected-state parity proof incomplete.");
...
assert(svgCrosses === 400, `Expected 400 shared SVG crosses, received ${svgCrosses}.`);
assert(v4Questions === 800, `Expected 800 V4-owned questions, received ${v4Questions}.`);
assert(v5Questions === 1_200, `Expected 1,200 V5-owned questions, received ${v5Questions}.`);
...
writeFileSync(
  resolve(outputDir, "mal-cp004-product-review-v5-review.json"),
  `${stable({
    candidate: MAL_CP004_PRODUCT_REVIEW_CANDIDATE,
    runtimeId: MAL_CP004_PRODUCT_REVIEW_RUNTIME_V5,
    answerPositions,
    review,
  })}\n`,
  "utf8",
...
console.log(
  JSON.stringify(
    {
      status: "PASS_MAL_CP004_PRODUCT_REVIEW_RUNTIME_V5",
      generated,
      deterministic,
      sourceParity,
      studioParity,
