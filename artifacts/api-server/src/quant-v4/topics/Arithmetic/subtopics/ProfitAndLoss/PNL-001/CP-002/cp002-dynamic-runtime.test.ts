...
assert.deepEqual(qlIds, Array.from({ length: 34 }, (_, index) => `PNL-QL-${String(index + 37).padStart(3, "0")}`));

const seeds = Array.from({ length: 24 }, (_, index) => `cp002-proof-seed-${index + 1}`);
...
for (const qlId of qlIds) {
  ...
  for (const seed of seeds) {
    ...
    assert.equal(pkg.traceability.misconceptionLabels.filter((label) => label !== "CORRECT").length, 3);
    assert.ok(pkg.stem.length > 30, `${qlId}: stem is unexpectedly short.`);
    assert.ok(pkg.explanation.lines.length >= 4, `${qlId}: explanation is unexpectedly short.`);

    const prose = `${pkg.stem}\n${pkg.explanation.lines.join("\n")}`
      .replace(/\\\[[\s\S]*?\\\]/g, "")
      .replace(/\\\([\s\S]*?\\\)/g, "");
    assert.doesNotMatch(prose, /\{[a-z][A-Za-z0-9_]*\}/, `${qlId}: unresolved prose placeholder.`);
    ...
    assert.equal(replay.stem, pkg.stem, `${qlId}: same seed must reproduce the stem.`);
    assert.equal(replay.answer, pkg.answer, `${qlId}: same seed must reproduce the answer.`);
    assert.deepEqual(replay.options, pkg.options, `${qlId}: same seed must reproduce option order.`);
  }

  assert.ok(stems.size >= 2, `${qlId}: seed sweep did not vary the generated stem.`);
  if (!["PNL-QL-067", "PNL-QL-070"].includes(qlId)) {
    assert.ok(answers.size >= 2, `${qlId}: seed sweep did not vary the generated answer.`);
  }
}
