...
for (const qlId of qlIds) {
  ...
  for (const seed of seeds) {
    ...
    if (qlId === "PNL-QL-079") {
      assert.match(pkg.answer, /^\d+$/, "Unknown inventory quantity must be a whole number.");
    }
...
assert.match(lotsTable.stem, /\| Group \| Quantity or cost \| Selling condition \|/);
...
assert.match(inventoryTable.stem, /\| Group \| Quantity and unit cost \| Selling condition \|/);
