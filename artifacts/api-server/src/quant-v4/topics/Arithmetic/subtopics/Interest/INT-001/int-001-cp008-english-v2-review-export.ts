import { generateIntCp008EnglishQuestion } from "./cp008-instalment-english-v2";
import { INT_CP008_QL_IDS } from "./cp008-instalment-runtime-v1-final";

const lines: string[] = [
  "# INT-CP-008 English V2 Review Pack",
  "",
  "Status: English learner-content review only. Permanent QL identities are frozen; learner content is not frozen.",
  "",
  "V2 supersedes V1 only to repair the QL120 malformed MathJax delimiter. This export shows all six stem families for every permanent QL.",
  "",
];
...
for (const qlId of INT_CP008_QL_IDS) {
  ...
  for (let index = 0; index < 500 && selected.size < 6; index += 1) {
    const seed = `int-cp008-en-v2-review:${qlId}:${index}`;
