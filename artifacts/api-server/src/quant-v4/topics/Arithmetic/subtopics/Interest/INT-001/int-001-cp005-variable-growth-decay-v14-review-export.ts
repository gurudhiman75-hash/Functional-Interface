...
const OUT_DIR = resolve(process.cwd(), "dist/quant-v4/int-cp005-review-v14");
mkdirSync(OUT_DIR, { recursive: true });

function seedForPosition(qlId: IntCp005QlId, target: number): string {
  for (let attempt = 0; attempt < 14000; attempt += 1) {
    const seed = `int-cp005-v14-review-${qlId}-pos-${target}-${attempt}`;
    if (generateIntCp005QuestionV14(qlId, seed, "en-IN").correctIndex === target) return seed;
  }
  throw new Error(`${qlId}: no review seed for answer position ${target}`);
}

function render(question: IntCp005QuestionV14, ordinal: number): string {
...
const summary: Record<string, unknown> = { runtimeVersion: INT_CP005_RUNTIME_VERSION_V14, qlRange: "INT-QL-086..INT-QL-095", qlCount: 10, perLocaleQuestions: 40, totalQuestions: 120 };

for (const locale of LOCALES) {
  const questions: IntCp005QuestionV14[] = [];
  ...
  for (const qlId of INT_CP005_QL_IDS) {
    for (let target = 0; target < 4; target += 1) {
      ...
      const question = generateIntCp005QuestionV14(qlId, seed, locale);
      ...
      semantics.set(question.answerSemantic, (semantics.get(question.answerSemantic) ?? 0) + 1);
  ...
  const markdown = [
    `# INT-CP-005 Variable Rates, Growth & Decay — ${locale} Review Pack V14`,
    "",
    `Runtime: ${INT_CP005_RUNTIME_VERSION_V14}`,
    `Questions: ${questions.length}`,
    `Answer positions: ${positions.join(" / ")}`,
    `Contexts: ${JSON.stringify(Object.fromEntries(contexts))}`,
    `Answer semantics: ${JSON.stringify(Object.fromEntries(semantics))}`,
  ...
  writeFileSync(resolve(OUT_DIR, `INT-CP-005-V14-REVIEW-${locale}.md`), markdown, "utf8");
  summary[locale] = { questions: 40, answerPositions: positions, contexts: Object.fromEntries(contexts), answerSemantics: Object.fromEntries(semantics), representations: Object.fromEntries(representations), duplicateStems: 0 };
}

writeFileSync(resolve(OUT_DIR, "summary.json"), `${JSON.stringify(summary, null, 2)}\n`, "utf8");
console.log(JSON.stringify(summary, null, 2));
console.log("PASS_INT_CP005_VARIABLE_GROWTH_DECAY_V14_REVIEW_EXPORT");
