...
function assertSurface(question: IntCp005QuestionV10): void {
  ...
  if (question.qlId === "INT-QL-089") {
    ...
    assert(!question.explanation.steps[0]?.includes("known factors"), `${question.qlId}/${question.seed}/${question.locale}: English text inside formula`);
    assert(question.explanation.steps.some((step) => /\\\(K\\\)/u.test(step)), `${question.qlId}/${question.seed}/${question.locale}: K definition wrapper missing`);
    editorialChecks += 3;
  }
  ...
  assert(/\\\([^\n]+\\\)/u.test(question.explanation.steps[0] ?? ""), `${question.qlId}/${question.seed}/${question.locale}: first math wrapper missing`);
...
for (const qlId of INT_CP005_QL_IDS) {
  ...
  for (let index = 0; index < 100; index += 1) {
    const seed = `int-cp005-v10-audit-${qlId}-${index}`;
    ...
    if (english.mathematicalState.qlId === "INT-QL-093") {
      thresholdProfiles.add(`${english.mathematicalState.direction}:${english.mathematicalState.rate.numerator}/${english.mathematicalState.rate.denominator}:${english.mathematicalState.targetYear}:${english.mathematicalState.initial.numerator}/${english.mathematicalState.initial.denominator}`);
    ...
    for (const locale of LOCALES) {
      ...
      questions += 1;
      assert.equal(question.runtimeVersion, INT_CP005_RUNTIME_VERSION_V10);
      ...
      assert.equal(optionKey(question), canonicalOptions, `${qlId}/${seed}/${locale}: locale option ownership drift`);
      localeParityChecks += 4;
      ...
      if (qlId !== "INT-QL-095") {
        ...
        assert.equal(optionKey(question), optionKey(v9 as IntCp005QuestionV10), `${qlId}/${seed}/${locale}: V10 changed non-plan option ownership`);
...
assert(thresholdProfiles.size >= 50, `threshold state diversity regressed: ${thresholdProfiles.size}`);
...
for (const locale of LOCALES) {
  const regression = generateIntCp005QuestionV10("INT-QL-095", REGRESSION_PLAN_SEED, locale);
  assert(verifyIntCp005Answer(regression.mathematicalState, regression.solution), `regression seed/${locale}: solution invalid`);
  assert.equal(regression.options.length, 4);
  regression.options.forEach((option) => assert.equal(option.value.denominator, 1n, `regression seed/${locale}: fractional plan option`));
  assertSurface(regression);
  knownFallbackChecks += 1;
}

for (let index = 0; index < 500; index += 1) {
  const seed = `int-cp005-v10-plan-smoke-${index}`;
  const question = generateIntCp005QuestionV10("INT-QL-095", seed, "en-IN");
  assert(verifyIntCp005Answer(question.mathematicalState, question.solution), `${seed}: plan smoke solution invalid`);
  assert.equal(question.options.length, 4);
  assert.equal(new Set(question.options.map((option) => option.text)).size, 4, `${seed}: duplicate plan smoke options`);
  question.options.forEach((option, optionIndex) => {
    assert.equal(option.value.denominator, 1n, `${seed}: fractional option`);
    assert.equal(verifyIntCp005Answer(question.mathematicalState, option.value), optionIndex === question.correctIndex, `${seed}: option ownership`);
  });
  arbitraryPlanSmokeChecks += 1;
}

console.log(JSON.stringify({ runtimeVersion: INT_CP005_RUNTIME_VERSION_V10, qls: 10, questions, perLocale: questions / 3, verifierChecks, replayChecks, optionChecks, localeParityChecks, lifecycleChecks, wrapperChecks, editorialChecks, sourceParityChecks, knownFallbackChecks, arbitraryPlanSmokeChecks, thresholdStates: thresholdProfiles.size, thresholdYears: [...thresholdYears].sort(), planSchedulePairs: planPairs.size, planInitialValues: planInitials.size }, null, 2));
console.log("PASS_INT_CP005_VARIABLE_GROWTH_DECAY_V10");
