...
let contextGroundingChecks = 0;

function learnerStrings(question: IntCp005QuestionV3): string[] {
  return [
...
function optionOwnershipKey(question: IntCp005QuestionV3): string {
...
function assertContextGrounding(question: IntCp005QuestionV3): void {
  const stem = question.presentation.markdown;
  if (question.qlId === "INT-QL-088") {
    if (question.mathematicalState.context === "SALARY") {
      assert(question.locale === "en-IN" ? /salary/iu.test(stem) : question.locale === "hi-IN" ? /वेतन/u.test(stem) : /ਤਨਖਾਹ/u.test(stem), `${question.qlId}/${question.seed}/${question.locale}: salary context is hidden`);
      contextGroundingChecks += 1;
    }
    if (question.mathematicalState.context === "POPULATION") {
      assert(question.locale === "en-IN" ? /population/iu.test(stem) : question.locale === "hi-IN" ? /जनसंख्या/u.test(stem) : /ਆਬਾਦੀ/u.test(stem), `${question.qlId}/${question.seed}/${question.locale}: population context is hidden`);
      contextGroundingChecks += 1;
    }
  }
}

function assertLearnerSurface(question: IntCp005QuestionV3): void {
  ...
  if (question.locale === "pa-IN") {
    ...
    if (question.qlId === "INT-QL-086" && ["POPULATION", "SALARY", "PRODUCTION"].includes(question.mathematicalState.context)) {
      assert(/ਵਾਧਾ ਹੁੰਦਾ ਹੈ/u.test(question.presentation.markdown), `${question.qlId}/${question.seed}: Punjabi growth noun missing`);
    }
  }
  if (question.locale === "hi-IN" && question.qlId === "INT-QL-092") {
    assert(!/इसके मूल्य में क्रमशः [^।]+ होता है।/u.test(question.presentation.markdown), `${question.qlId}/${question.seed}: mechanical Hindi mixed-change sentence survived`);
  }
  if (question.locale === "pa-IN" && question.qlId === "INT-QL-092") {
    assert(!/ਇਸ ਦੇ ਮੁੱਲ ਵਿੱਚ ਕ੍ਰਮਵਾਰ [^।]+ ਹੁੰਦਾ ਹੈ।/u.test(question.presentation.markdown), `${question.qlId}/${question.seed}: mechanical Punjabi mixed-change sentence survived`);
  }
  ...
  assertContextGrounding(question);
}

assert.equal(INT_CP005_RUNTIME_VERSION_V3, "INT-CP-005-VARIABLE-GROWTH-DECAY-v3");
...
for (const qlId of INT_CP005_QL_IDS) {
  ...
  for (let index = 0; index < 100; index += 1) {
    const seed = `int-cp005-v3-audit-${qlId}-${index}`;
    const english = generateIntCp005QuestionV3(qlId, seed, "en-IN");
    const replay = generateIntCp005QuestionV3(qlId, seed, "en-IN");
    ...
    english.options.forEach((option, optionIndex) => {
      assert.equal(verifyIntCp005Answer(english.mathematicalState, option.value), optionIndex === english.correctIndex, `${qlId}/${seed}: option verifier ownership mismatch at ${optionIndex}`);
      optionChecks += 1;
    });
    ...
    for (const locale of LOCALES) {
      const question = locale === "en-IN" ? english : generateIntCp005QuestionV3(qlId, seed, locale);
      questions += 1;
      assert.equal(question.runtimeVersion, INT_CP005_RUNTIME_VERSION_V3);
...
assert(![...representations].some((entry) => entry.endsWith(":RATE_TABLE")));
...
console.log("PASS_INT_CP005_VARIABLE_GROWTH_DECAY_V3");
