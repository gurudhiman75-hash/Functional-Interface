...
for (const prototypeId of INT_CP001_PROTOTYPE_IDS) {
  ...
  for (let index = 0; index < 80; index += 1) {
    ...
    for (const option of item.options) {
      if (/^₹-?\d+ \d+\/\d+$/u.test(option) || /^₹-?\d+\/\d+$/u.test(option)) {
        fractionalMoneyOptions += 1;
      }
    }
    ...
    for (const option of item.optionAudit) {
      ...
      misconceptionCounts.set(option.misconceptionId, (misconceptionCounts.get(option.misconceptionId) ?? 0) + 1);
    }
    ...
    contextCounts.set(item.parameters.context.scenarioId, (contextCounts.get(item.parameters.context.scenarioId) ?? 0) + 1);
  ...
  if (stable([...positions].sort()) !== stable([0, 1, 2, 3])) fail(`${prototypeId} lacks answer-position coverage.`);
...
if (misconceptionCounts.size < 18) fail(`Chapter-wide misconception coverage is too narrow: ${misconceptionCounts.size}.`);
