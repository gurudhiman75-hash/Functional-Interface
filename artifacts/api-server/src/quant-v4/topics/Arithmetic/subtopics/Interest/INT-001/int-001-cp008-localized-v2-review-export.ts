import { generateIntCp008LocalizedReviewQuestion } from "./cp008-instalment-localized-v2";
import { INT_CP008_QL_IDS } from "./cp008-instalment-runtime-v1-final";

const LOCALES = Object.freeze(["hi-IN", "pa-IN"] as const);
const lines: string[] = [
  "# INT-CP-008 Hindi/Punjabi V2 Review Pack",
  "",
  "Status: multilingual math-parity review candidate. English V6 source is frozen; localized learner content is not frozen.",
  "",
  "V2 preserves V1 localized stems and protected payload while restoring the missing frozen-English QL120 MathJax segment.",
  "",
];
...
for (const qlId of INT_CP008_QL_IDS) {
  ...
  for (const locale of LOCALES) {
    ...
    for (let index = 0; index < 1000 && selected.size < 6; index += 1) {
      const seed = `int-cp008-localized-v2-review:${qlId}:${index}`;
    ...
    if (selected.size !== 6) throw new Error(`${qlId}/${locale}: could not collect all six V2 stem families`);
...
if (reviewQuestions !== 108) throw new Error(`expected 108 V2 review questions, got ${reviewQuestions}`);
lines.push("---", "", `Review questions: **${reviewQuestions}**`, `Permanent QLs: **${INT_CP008_QL_IDS.length}**`, "Locales: **2**", "Stem families per QL per locale: **6**", "Source English content frozen: **true**", "Localized learner content frozen: **false**", "Delivery authorized: **false**", "");
