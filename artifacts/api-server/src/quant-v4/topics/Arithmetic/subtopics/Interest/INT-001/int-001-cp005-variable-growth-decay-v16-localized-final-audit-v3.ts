...
for (const locale of INT_CP005_V16_LOCALIZED_LOCALES) {
  ...
  for (const qlId of INT_CP005_V16_QL_IDS) {
    for (let index = 0; index < 160; index += 1) {
      const seed = `int-cp005-v16-hi-pa-v2-audit-${qlId}-${index}`;
      const en = generateIntCp005QuestionV16EditorialFinal(qlId, seed, "en-IN");
      const loc = generateIntCp005QuestionV16LocalizedV2(qlId, seed, locale);
      const replay = generateIntCp005QuestionV16LocalizedV2(qlId, seed, locale);
      ...
      assert(!/क्रमशः वार्षिक मूल्यह्रास|ਕ੍ਰਮਵਾਰ ਸਾਲਾਨਾ ਘਟਾਓ/u.test(loc.presentation.markdown), `${locale}/${qlId}/${seed}: mechanical depreciation phrasing returned`);
      languageChecks += 6;
      if (locale === "pa-IN") {
        assert(!DEVANAGARI_LETTER_OR_SIGN.test(learner), `${qlId}/${seed}: Devanagari letter/sign leaked into Punjabi learner text`);
        ...
        assert(!GURMUKHI_LETTER_OR_SIGN.test(learner), `${qlId}/${seed}: Gurmukhi leaked into Hindi learner text`);
...
for (const locale of INT_CP005_V16_LOCALIZED_LOCALES) {
  let rejected = false;
  try { generateIntCp005QuestionV16LocalizedV2("INT-QL-094", "must-reject", locale); } catch { rejected = true; }
  assert(rejected, `${locale}: QL094 did not remain rejected`);
}
console.log(JSON.stringify({ localizedEditorialVersion: INT_CP005_V16_LOCALIZED_EDITORIAL_VERSION, qls: INT_CP005_V16_QL_IDS.length, locales: INT_CP005_V16_LOCALIZED_LOCALES, questions, parityChecks, optionChecks, lifecycleChecks, languageChecks, uniqueStems: Object.fromEntries([...uniqueStems].map(([locale, stems]) => [locale, stems.size])), ql094Rejected: true }, null, 2));
console.log("PASS_INT_CP005_V16_HI_PA_NATIVE_EDITORIAL_V2");
