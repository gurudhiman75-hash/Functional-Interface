...
const EXPECTED_QL_LANGUAGE_SURFACES = 399;
const EXPECTED_GENERATED = EXPECTED_QL_LANGUAGE_SURFACES * SEEDS_PER_QL_LANGUAGE;
...
function languages(pkg: any): readonly string[] {
  assert.ok(Array.isArray(pkg.supportedLanguages), "Question Studio package is missing supported languages.");
  const values = Object.freeze(pkg.supportedLanguages.map(String).sort());
  assert.deepEqual(values, [...LANGUAGES].sort(), `${pkg.checkpointId ?? pkg.canonicalProblemId ?? pkg.name}: Question Studio surface is not fully trilingual.`);
  return values;
}
...
assert.equal(surfaces.reduce((sum, surface) => sum + surface.qlIds.length * surface.languages.length, 0), EXPECTED_QL_LANGUAGE_SURFACES, "Interest Question Studio is missing a permanent QL-language surface.");
...
function permanentQlMarker(question: any): string | null {
  const candidates = [question?.qlId, question?.permanentQlId, question?.questionLanguageId, question?.traceability?.permanentQlId, question?.parameters?.qlId, question?.canonicalItemId];
  ...
  for (const qlId of surface.qlIds) {
    for (const language of surface.languages) {
      ...
      for (let index = 0; index < SEEDS_PER_QL_LANGUAGE; index += 1) {
        const seed = `INT-001-399-SURFACE-SOAK:${surface.id}:${qlId}:${language}:${index}`;
...
assert.equal(distinctStems.size, EXPECTED_QL_LANGUAGE_SURFACES, "Not all 399 QL-language surfaces were exercised.");
assert.equal(generated, EXPECTED_GENERATED, `Expected exactly ${EXPECTED_GENERATED} generated questions.`);
assert.equal(deterministicChecks, EXPECTED_GENERATED);
assert.equal(explicitQlOwnershipChecks, EXPECTED_GENERATED, "Every generated Question Studio item must expose its permanent QL explicitly.");
assert.equal(legacyImplicitOwnershipCases, 0, "Legacy implicit QL ownership remains in Question Studio output.");
assert.equal(answerOwnershipChecks, EXPECTED_GENERATED * 5);
assert.equal(lifecycleChecks, EXPECTED_GENERATED * 5);
...
assert.equal(thinSurfaces.length, 0, "A QL-language surface produced fewer than two distinct stems across the soak.");
const positionCoverage = Object.fromEntries([...answerPositions.entries()].map(([key, values]) => [key, [...values].sort()]));

console.log(JSON.stringify({
  version: "INT-001-399-SURFACE-QUESTION-STUDIO-SOAK-v2",
  ...
  policy: {
    all133PermanentQlsTrilingual: true,
    qlLanguageSurfaceCount: "399_REQUIRED",
    explicitPayloadQlMarker: "REQUIRED_FOR_EVERY_QUESTION",
    legacyImplicitOwnership: "ZERO_REQUIRED",
...
console.log("PASS_INT_001_399_SURFACE_QUESTION_STUDIO_SOAK_V2_AUDIT");