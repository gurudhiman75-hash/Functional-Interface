...
import { validateIntCp001StemCashFlow } from "./cp001-cash-flow-direction";
...
function validateEnglishDirection(stem: string, direction: string): string[] {
  const errors: string[] = [];
  if (direction === "BORROWER_PAYS") {
    if (/\b(?:invested|earned as interest|will be earned as interest)\b/iu.test(stem)) {
      errors.push("English borrowing stem uses investment/earning language.");
    }
    if (!/\b(?:loan|borrowed|business advance|interest payable|paid as interest|must be paid as interest|total amount payable)\b/iu.test(stem)) {
      errors.push("English borrowing stem lacks an explicit borrower/payment marker.");
    }
  }
  if (direction === "INVESTOR_EARNS") {
    if (/\b(?:total amount payable|must be paid as interest|paid as interest|repaid)\b/iu.test(stem)) {
      errors.push("English investment stem uses borrower/payment language.");
    }
    if (!/\b(?:invested|deposit|certificate|investment|earned as interest|will be earned as interest|amount)\b/iu.test(stem)) {
      errors.push("English investment stem lacks an explicit investment/earning marker.");
    }
  }
  return errors;
}
...
for (const entry of INT_CP001_FINAL_REGISTRY) {
  for (let index = 0; index < 80; index += 1) {
    ...
    const approvedEnglish = generateIntCp001FinalEditorialV3Question(entry.qlId, seed);
    const candidates = {
    ...
    for (const language of languages) {
      ...
      const directionErrors = language === "en"
        ? validateEnglishDirection(item.stem, cashFlow.direction)
        : [
            ...validateIntCp001ContextLeadV2(item.stem, language, cashFlow),
            ...validateIntCp001StemCashFlow(item.stem, item.solveContract, language, cashFlow.direction),
            ...validateIntCp001LoanAmountWordingV2(item.stem, item.solveContract, language, cashFlow.direction),
          ];
      if (directionErrors.length) fail(`${entry.qlId}/${seed}/${language} cash-flow failure: ${directionErrors.join(" | ")}`);
      cashFlowChecks += 1;
      ...
      localeStats.candidateWords += wordCount(item.stem);
      localeStats.approvedCharacters += previous.stem.length;
      localeStats.candidateCharacters += item.stem.length;
      localeStats.shorterOrEqual += item.stem.length <= previous.stem.length ? 1 : 0;
...
for (const language of languages) {
  ...
  if (localeStats.candidateWords > Math.ceil(localeStats.approvedWords * 1.05)) {
    fail(`${language} readable stems increased aggregate word load by more than 5%.`);
  }
}
if (crossLocaleCollisions !== 0) fail(`Hindi/Punjabi exact stem collisions: ${crossLocaleCollisions}.`);

console.log(JSON.stringify({
  status: "PASS_INT_CP001_READABLE_STEM_CANDIDATES",
  cpId: "INT-CP-001",
  ...
  locales: Object.fromEntries(languages.map((language) => [language, {
    ...
    shorterOrEqual: stats[language].shorterOrEqual,
