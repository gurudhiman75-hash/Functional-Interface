...
function findSeeds(qlId: IntCp007QlId): readonly string[] {
  ...
  for (let index = 0; index < 10000; index += 1) {
    const seed = `int-cp007-en-v6-review-${qlId}-${index}`;
  ...
  if (seeds.some((seed) => !seed) || !extra) throw new Error(`${qlId}: failed to collect complete V6 review seed set`);
...
const output: string[] = [];
output.push("# INT-CP-007 — English Product Review V6 — Exact LaTeX Presentation");
output.push("");
output.push(`Current English candidate: **${INT_CP007_ENGLISH_VERSION}**`);
output.push(`Supersedes: **${INT_CP007_ENGLISH_V6_SUPERSEDES}**.`);
output.push("");
output.push("V6 follows the V5 LaTeX remediation and removes its last display defect: accumulation factors that do not terminate within the short decimal display are now shown exactly as LaTeX fractions instead of producing an awkward equals-approximation sequence. All calculation formulas remain MathJax-wrapped. Permanent mathematics, QL identity, stems, options, correct answers and lifecycle controls remain unchanged.");
output.push("");
output.push("Permanent QLs: **INT-QL-109..INT-QL-115 (7 contracts)**. This file samples all three authored stem families for every QL plus one extra deterministic state, for **28 complete learner-facing questions**.");
output.push("");
...
const outputPath = path.join(outputDirectory, "INT-CP-007-ENGLISH-V6-LATEX-EXACT-REVIEW.md");
fs.writeFileSync(outputPath, `${output.join("\n")}\n`, "utf8");
console.log(JSON.stringify({ output: outputPath, qls: INT_CP007_QL_IDS.length, reviewQuestions }, null, 2));
console.log("PASS_INT_CP007_ENGLISH_V6_LATEX_EXACT_REVIEW_EXPORT");
