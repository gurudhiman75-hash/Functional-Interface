import { INT_CP005_V16_QL_IDS, generateIntCp005QuestionV16Editorial } from "./cp005-variable-growth-decay-runtime-v16-editorial";
...
for (const qlId of INT_CP005_V16_QL_IDS) {
  for (let index = 0; index < 160; index += 1) {
    const seed = `int-cp005-v16-editorial-audit-${qlId}-${index}`;
    const question = generateIntCp005QuestionV16Editorial(qlId, seed);
    const replay = generateIntCp005QuestionV16Editorial(qlId, seed);
    ...
    if (["INT-QL-086", "INT-QL-088", "INT-QL-091", "INT-QL-092"].includes(qlId)) {
      for (const option of question.options) {
        assert(option.value.denominator === 1n, `${qlId}/${seed}: non-integral distractor survived`);
        assert(option.value.numerator > 0n, `${qlId}/${seed}: non-positive distractor survived`);
        assert(option.value.numerator <= 300000n, `${qlId}/${seed}: distractor exceeds ₹3 lakh`);
        if (!option.isCorrect) {
...
console.log("PASS_INT_CP005_V16_EDITORIAL_DISTRACTORS");
