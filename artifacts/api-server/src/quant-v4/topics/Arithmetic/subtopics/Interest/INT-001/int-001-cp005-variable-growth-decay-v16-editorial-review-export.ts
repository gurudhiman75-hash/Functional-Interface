...
import { INT_CP005_V16_QL_IDS, INT_CP005_RUNTIME_VERSION_V16, generateIntCp005QuestionV16Editorial } from "./cp005-variable-growth-decay-runtime-v16-editorial";

function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(message);
}

const selected: ReturnType<typeof generateIntCp005QuestionV16Editorial>[] = [];
for (const qlId of INT_CP005_V16_QL_IDS) {
  const positions = new Map<number, ReturnType<typeof generateIntCp005QuestionV16Editorial>>();
  const stems = new Set<string>();
  for (let index = 0; index < 1800 && positions.size < 4; index += 1) {
    const seed = `int-cp005-v16-editorial-review-${qlId}-${index}`;
    const question = generateIntCp005QuestionV16Editorial(qlId, seed);
  ...
  assert(positions.size === 4, `${qlId}: could not find A/B/C/D review questions`);
...
assert(selected.length === 36, `expected 36 questions, received ${selected.length}`);
assert(answerCounts.every((count) => count === 9), `unbalanced answer positions ${answerCounts.join("/")}`);
assert(new Set(selected.map((question) => question.presentation.markdown)).size === 36, "duplicate review stems");
...
lines.push("");
lines.push("**Scope correction:** `INT-QL-094` is excluded because percentage growth plus fixed migration/event-order arithmetic has no recovered Interest-family authority. Production and salary contexts are excluded. Ordinary questions use smaller hand-calculable values and 2–3 year durations.");
lines.push("");
...
lines.push("- QL-095 permutation-only plans: rejected");
lines.push("- Synthetic rounded reverse/fallback distractors: removed");
...
console.log("PASS_INT_CP005_V16_EDITORIAL_REVIEW_EXPORT");
