import { generateIntCp008LocalizedReviewQuestion } from "./cp008-instalment-localized-v3";
import { INT_CP008_QL_IDS } from "./cp008-instalment-runtime-v1-final";

const LOCALES = Object.freeze(["hi-IN", "pa-IN"] as const);
const lines: string[] = [
  "# INT-CP-008 Hindi/Punjabi V3 Review Pack",
  "",
  "Status: multilingual editorial review candidate. English V6 source is frozen; localized learner content is not frozen.",
  "",
  "V3 is a prose-only overlay over V2. It preserves frozen-English mathematics, option values/order, correct answers and MathJax while repairing learner-language defects found in the full V2 manual review.",
  "",
];
...
for (const qlId of INT_CP008_QL_IDS) {
  ...
  for (const locale of LOCALES) {
    ...
    for (let index = 0; index < 1000 && selected.size < 6; index += 1) {
      const seed = `int-cp008-localized-v3-review:${qlId}:${index}`;
    ...
    if (selected.size !== 6) throw new Error(`${qlId}/${locale}: could not collect all six V3 stem families`);
...
if (reviewQuestions !== 108) throw new Error(`expected 108 V3 review questions, got ${reviewQuestions}`);
lines.push(
  "---", "",
