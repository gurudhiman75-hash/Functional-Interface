...
import { generateIntCp008LocalizedReviewQuestion as generateV2 } from "./cp008-instalment-localized-v2";
import {
  INT_CP008_LOCALIZED_VERSION,
  INT_CP008_LOCALIZED_V3_SUPERSEDES,
...
function v2NonProsePayload(q: any) {
  return {
    id: q.id,
    runtimeVersion: q.runtimeVersion,
...
let questions = 0, deterministicChecks = 0, preservationChecks = 0, v2PayloadChecks = 0;
...
for (const locale of LOCALES) {
  for (const qlId of INT_CP008_QL_IDS) {
    ...
    for (let index = 0; index < 200; index += 1) {
      const seed = `int-cp008-localized-v3:${qlId}:${index}`;
      const english = generateIntCp008EnglishFrozenQuestion(qlId, seed) as any;
      const v2 = generateV2(qlId, seed, locale) as any;
      const localized = generateV3(qlId, seed, locale) as any;
      const replay = generateV3(qlId, seed, locale) as any;
      questions += 1;

      assert.equal(stable(localized), stable(replay), `${key}/${seed}: nondeterministic V3 replay`); deterministicChecks += 1;
      ...
      assert.equal(stable(v2NonProsePayload(localized)), stable(v2NonProsePayload(v2)), `${key}/${seed}: V2 non-prose payload drift`);
      assert.equal(localized.localizedVersion, INT_CP008_LOCALIZED_VERSION);
      assert.equal(INT_CP008_LOCALIZED_V3_SUPERSEDES, "INT-CP-008-HI-PA-v2-math-parity-review");
      ...
      preservationChecks += 5; v2PayloadChecks += 1;
      ...
      const v2Learner = learnerText(v2), v3Learner = learnerText(localized);
      if (v2Learner !== v3Learner) {
        changedQuestions += 1;
      ...
      if (qlId === "INT-QL-119") {
        assert.ok(!/पहले 1 (?:अवधि-अंत )?भुगतान/u.test(text), `${key}/${seed}: Hindi singular QL119 form remains`);
        assert.ok(!/ਪਹਿਲੀਆਂ 1 (?:ਮਿਆਦ-ਅੰਤ )?ਅਦਾਇਗੀਆਂ/u.test(text), `${key}/${seed}: Punjabi singular QL119 form remains`);
        targetedRepairChecks += 2;
      }
...
assert.ok(changedQuestions > 0, "V3 did not change any localized learner prose");
for (const qlId of ["INT-QL-118", "INT-QL-119", "INT-QL-123", "INT-QL-124"] as const) {
  assert.ok((changedByQl.get(qlId) ?? 0) > 0, `${qlId}: expected V3 editorial repairs were not exercised`);
}

console.log(JSON.stringify({
  localizedVersion: INT_CP008_LOCALIZED_VERSION,
  supersedes: INT_CP008_LOCALIZED_V3_SUPERSEDES,
  questions, deterministicChecks, preservationChecks, v2PayloadChecks, mathChecks, scriptChecks,
...
console.log("PASS_INT_CP008_LOCALIZED_V3_AUDIT");
