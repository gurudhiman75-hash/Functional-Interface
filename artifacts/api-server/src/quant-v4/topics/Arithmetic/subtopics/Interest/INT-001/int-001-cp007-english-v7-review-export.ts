...
function contentKey(question: ReturnType<typeof generateIntCp007EnglishQuestion>): string {
...
function findSeeds(qlId: IntCp007QlId): readonly string[] {
  ...
  const selectedKeys = new Set(selected.map((seed) => contentKey(generateIntCp007EnglishQuestion(qlId, seed))));
  let extra: string | undefined;
  for (let index = 0; index < 20000; index += 1) {
    const seed = `int-cp007-en-v7-review-${qlId}-${index}`;
    if (selected.includes(seed)) continue;
    const key = contentKey(generateIntCp007EnglishQuestion(qlId, seed));
    if (!selectedKeys.has(key)) {
      extra = seed;
      break;
    }
  }
  if (!extra) throw new Error(`${qlId}: failed to find a fourth meaningfully distinct V7 learner-content surface`);
...
output.push("");
output.push("V7 is the self-audit remediation over V6. It closes the last eight unwrapped numeric-result occurrences inside calculation steps: four final annual-rate statements in QL111 and four final principal-ratio statements in QL113. The exporter also requires the fourth sample for every QL to differ in learner content, not merely in option order.");
output.push("");
output.push("Permanent QLs: **INT-QL-109..INT-QL-115 (7 contracts)**. The review includes all three authored stem families for every QL plus one additional meaningfully distinct learner-content surface, for **28 complete review questions with 28 distinct stem/answer/explanation surfaces**.");
output.push("");
...
for (const qlId of INT_CP007_QL_IDS) {
  ...
  seeds.forEach((seed, index) => {
    ...
    if (globalContent.has(key)) throw new Error(`${qlId}/${seed}: duplicate stem/answer/explanation surface entered V7 review`);
    globalContent.add(key);
    output.push(renderQuestion(qlId, seed, index + 1));
    reviewQuestions += 1;
  });
}

if (reviewQuestions !== 28 || globalContent.size !== 28) {
  throw new Error(`expected 28 meaningfully distinct V7 review surfaces, got questions=${reviewQuestions}, uniqueContent=${globalContent.size}`);
}
...
console.log(JSON.stringify({ output: outputPath, qls: INT_CP007_QL_IDS.length, reviewQuestions, uniqueContentSurfaces: globalContent.size }, null, 2));
