...
import { generateIntCp008EnglishQuestion as generateV1 } from "./cp008-instalment-english-v1";
import {
...
let v1ToV2PayloadChecks = 0;
let changedQuestions = 0;
let ql120RepairChecks = 0;
let answerChecks = 0;
let optionChecks = 0;
...
for (const qlId of INT_CP008_QL_IDS) {
  ...
  for (let index = 0; index < SEEDS_PER_QL; index += 1) {
    ...
    const v1 = generateV1(qlId, seed);
    questions += 1;

    assert(stable(question) === stable(replay), `${qlId}/${seed}: deterministic English V2 replay mismatch`);
    ...
    assert(stable(question.mathematicalState) === stable(v1.mathematicalState), `${qlId}/${seed}: V2 changed V1 mathematical state`);
    assert(stable(question.presentation) === stable(v1.presentation), `${qlId}/${seed}: V2 changed V1 presentation`);
    assert(stable(question.options) === stable(v1.options), `${qlId}/${seed}: V2 changed V1 options`);
    assert(question.correctIndex === v1.correctIndex, `${qlId}/${seed}: V2 changed correct index`);
    assert(question.correctAnswer === v1.correctAnswer, `${qlId}/${seed}: V2 changed correct answer`);
    assert(question.explanation.keyIdea === v1.explanation.keyIdea, `${qlId}/${seed}: V2 changed key idea`);
    assert(question.explanation.finalAnswer === v1.explanation.finalAnswer, `${qlId}/${seed}: V2 changed explanation final answer`);
    assert(question.explanation.commonMistake === v1.explanation.commonMistake, `${qlId}/${seed}: V2 changed common mistake`);
    v1ToV2PayloadChecks += 8;

    const stepsChanged = stable(question.explanation.steps) !== stable(v1.explanation.steps);
    if (qlId === "INT-QL-120") {
      assert(stepsChanged, `${qlId}/${seed}: V2 failed to repair QL120 step`);
      assert(v1.explanation.steps[2]!.endsWith("$."), `${qlId}/${seed}: expected V1 malformed lineage not found`);
      assert(!question.explanation.steps[2]!.endsWith("$."), `${qlId}/${seed}: stray V1 delimiter remains`);
      for (let stepIndex = 0; stepIndex < question.explanation.steps.length; stepIndex += 1) {
        if (stepIndex === 2) continue;
        assert(question.explanation.steps[stepIndex] === v1.explanation.steps[stepIndex], `${qlId}/${seed}: V2 changed unrelated explanation step ${stepIndex + 1}`);
      }
      ql120RepairChecks += 4;
      changedQuestions += 1;
    } else {
      assert(!stepsChanged, `${qlId}/${seed}: V2 changed non-QL120 explanation`);
      ql120RepairChecks += 1;
    }
...
assert(changedQuestions === 200, `CP008 V2 must change exactly the 200 QL120 audit questions, got ${changedQuestions}`);
...
console.log("PASS_INT_CP008_ENGLISH_V2_AUDIT");
