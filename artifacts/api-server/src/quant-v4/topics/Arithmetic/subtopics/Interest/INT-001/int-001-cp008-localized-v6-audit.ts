...
import { generateIntCp008LocalizedReviewQuestion as generateV5 } from "./cp008-instalment-localized-v5";
import {
  INT_CP008_LOCALIZED_VERSION,
  INT_CP008_LOCALIZED_V6_SUPERSEDES,
  generateIntCp008LocalizedReviewQuestion as generateV6,
...
function v5NonProsePayload(q: any) {
  return {
...
let questions = 0, deterministicChecks = 0, preservationChecks = 0, v5PayloadChecks = 0;
...
for (const locale of LOCALES) {
  for (const qlId of INT_CP008_QL_IDS) {
    ...
    for (let index = 0; index < 200; index += 1) {
      const seed = `int-cp008-localized-v6:${qlId}:${index}`;
      const english = generateIntCp008EnglishFrozenQuestion(qlId, seed) as any;
      const v5 = generateV5(qlId, seed, locale) as any;
      const localized = generateV6(qlId, seed, locale) as any;
      const replay = generateV6(qlId, seed, locale) as any;
      questions += 1;

      assert.equal(stable(localized), stable(replay), `${key}/${seed}: nondeterministic V6 replay`); deterministicChecks += 1;
      ...
      assert.equal(stable(v5NonProsePayload(localized)), stable(v5NonProsePayload(v5)), `${key}/${seed}: V5 non-prose payload drift`);
      assert.equal(localized.localizedVersion, INT_CP008_LOCALIZED_VERSION);
      assert.equal(INT_CP008_LOCALIZED_V6_SUPERSEDES, "INT-CP-008-HI-PA-v5-final-editorial-review");
      ...
      preservationChecks += 5; v5PayloadChecks += 1;
      ...
      if (learnerText(v5) !== learnerText(localized)) {
        changedQuestions += 1;
      ...
      const banned = [
        "मौद्रिक उत्तर निकटतम पैसे तक दें।", "ਰਕਮ ਵਾਲਾ ਉੱਤਰ ਸਭ ਤੋਂ ਨੇੜਲੇ ਪੈਸੇ ਤੱਕ ਦਿਓ।",
        "बकाया अपडेट करें", "ਬਕਾਇਆ ਅਪਡੇਟ ਕਰੋ", "एकल शुरुआती राशि", "ਇਕੱਲੀ ਸ਼ੁਰੂਆਤੀ ਰਕਮ",
        "सटीक पुनर्भुगतान पुनरावृत्ति", "ਸਹੀ ਵਾਪਸੀ ਪੁਨਰਾਵਰਤੀ", "पुनर्भुगतान कार्यक्रम", "ਵਾਪਸੀ ਕਾਰਜਕ੍ਰਮ",
        "कार्यक्रम में", "ਕਾਰਜਕ੍ਰਮ ਵਿੱਚ", "1 ਅਦਾਇਗੀਆਂ", "ਚੱਕਰਵੱਧੀ",
      ];
      for (const phrase of banned) assert.ok(!text.includes(phrase), `${key}/${seed}: blocked phrase remains: ${phrase}`);
      targetedRepairChecks += banned.length;

      if (qlId === "INT-QL-118") {
        assert.ok(!/भुगतान संख्या [12] के बाद/u.test(localized.presentation.prompt), `${key}/${seed}: Hindi payment-number phrasing remains`);
        assert.ok(!/ਅਦਾਇਗੀ ਨੰਬਰ [12] ਤੋਂ ਬਾਅਦ/u.test(localized.presentation.prompt), `${key}/${seed}: Punjabi payment-number phrasing remains`);
        targetedRepairChecks += 2;
      }
      if (qlId === "INT-QL-119" && localized.mathematicalState.contractState.periods - 1 === 1) {
        assert.ok(!localized.explanation.steps[1].includes("इन नियमित भुगतानों के लिए"), `${key}/${seed}: Hindi plural QL119 explanation remains`);
        assert.ok(!localized.explanation.steps[1].includes("ਇਨ੍ਹਾਂ ਨਿਯਮਿਤ ਅਦਾਇਗੀਆਂ ਲਈ"), `${key}/${seed}: Punjabi plural QL119 explanation remains`);
        targetedRepairChecks += 2;
      }
      if (qlId === "INT-QL-123") {
        const growthPeriods = localized.mathematicalState.contractState.periods - localized.mathematicalState.contractState.missedPaymentNumber;
        if (growthPeriods > 1) {
          assert.ok(!new RegExp(`${growthPeriods} पूरी ब्याज अवधि है`, "u").test(text), `${key}/${seed}: Hindi plural interest-period grammar remains`);
          assert.ok(!new RegExp(`${growthPeriods} ਪੂਰੀ ਵਿਆਜ ਮਿਆਦ ਹੈ`, "u").test(text), `${key}/${seed}: Punjabi plural interest-period grammar remains`);
          if (locale === "hi-IN") assert.ok(localized.explanation.steps[1].includes(`${growthPeriods} पूरी ब्याज अवधियाँ हैं`), `${key}/${seed}: Hindi plural repair missing`);
          else assert.ok(localized.explanation.steps[1].includes(`${growthPeriods} ਪੂਰੀਆਂ ਵਿਆਜ ਮਿਆਦਾਂ ਹਨ`), `${key}/${seed}: Punjabi plural repair missing`);
...
assert.ok(changedQuestions > 0, "V6 did not change learner prose");
for (const qlId of ["INT-QL-117", "INT-QL-118", "INT-QL-119", "INT-QL-123"] as const) assert.ok((changedByQl.get(qlId) ?? 0) > 0, `${qlId}: expected V6 repair not exercised`);
for (const qlId of ["INT-QL-116", "INT-QL-120", "INT-QL-121", "INT-QL-122", "INT-QL-124"] as const) assert.ok(!changedByQl.has(qlId), `${qlId}: V6 changed out-of-scope learner prose`);

console.log(JSON.stringify({
  localizedVersion: INT_CP008_LOCALIZED_VERSION, supersedes: INT_CP008_LOCALIZED_V6_SUPERSEDES,
  questions, deterministicChecks, preservationChecks, v5PayloadChecks, mathChecks, scriptChecks,
...
console.log("PASS_INT_CP008_LOCALIZED_V6_AUDIT");
