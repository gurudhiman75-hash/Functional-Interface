...
import {
  INT_CP004_ENGLISH_EXAM_FRIENDLY_REVIEW_V12,
  generateIntCp004EnglishExamFriendlyReviewV12,
} from "./cp004-english-exam-friendly-review-v12";
...
let ql078ExactProofChecks = 0;
const answerPositions = [0, 0, 0, 0];
const stemFamilies = new Map<string, Set<string>>();
const uniqueStates = new Map<string, Set<string>>();

assert.equal(INT_CP004_ENGLISH_EXAM_FRIENDLY_REVIEW_V12.approved, false);
assert.equal(INT_CP004_ENGLISH_EXAM_FRIENDLY_REVIEW_V12.permanentIdentityChanges, false);
assert.equal(INT_CP004_ENGLISH_EXAM_FRIENDLY_REVIEW_V12.questionStudioActivationAuthorized, false);
assert.equal(INT_CP004_ENGLISH_EXAM_FRIENDLY_REVIEW_V12.ql078ExactFactorProof, true);
...
for (const qlId of INT_CP004_QL_IDS) {
  ...
  for (let index = 0; index < SAMPLES_PER_QL; index += 1) {
    const seed = `int-cp004-english-parity-v12:${qlId}:${index}`;
    const first = generateIntCp004EnglishExamFriendlyReviewV12(qlId, seed);
    const replay = generateIntCp004EnglishExamFriendlyReviewV12(qlId, seed);
    ...
    assert.equal(first.v12Remediation.mathematicalStateChanged, false);
    ...
    if (qlId === "INT-QL-078") {
      ...
      assert.ok(joined.includes("exact growth factor"), `${qlId}/${index}: exact factor comparison missing`);
      assert.ok(joined.includes("does not match") || joined.includes("which matches"), `${qlId}/${index}: schedule comparison proof missing`);
      assert.equal(first.v12Remediation.ql078ExactFactorProofApplied, true);
      ql078ExactProofChecks += 3;
    }
    ...
    assert.equal(first.v12Remediation.approvalGranted, false);
    lifecycleChecks += 11;
...
console.log("PASS_INT_CP004_ENGLISH_PARITY_V12_AUDIT");
