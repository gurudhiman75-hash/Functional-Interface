...
for (const authorityId of INT_CP010_PRODUCTION_CANDIDATE_AUTHORITIES) {
  ...
  for (let index = 0; index < 800 && seen.size < 8; index += 1) {
    const q = generateIntCp010ProductionCandidateV2(authorityId, `cp010:prod-v2-review:${authorityId}:${index}`) as any;
    if (seen.has(q.stemFamilyId)) continue;
    seen.add(q.stemFamilyId);
    selected.push(q);
  }
  if (seen.size !== 8) throw new Error(`${authorityId}: unable to recover all eight V2 stem families`);
}

console.log("# INT-CP-010 — Production Authoring Candidate V2 English Review\n");
console.log("Status: **PRE-ALLOCATION / ID-FREE / REALISM-GATED**\n");
console.log("One question from each of the 16 authority/stem families. P001/P002 source-hold prototypes are deliberately absent.\n");
...
selected.forEach((q, index) => {
  ...
  console.log(`Requested seed: \`${q.seed}\`  `);
  console.log(`Selected candidate seed: \`${q.underlyingCandidateSeed}\`  `);
  console.log(`Realism attempts: ${q.realismSelectionAttempts}\n`);
  console.log(q.stem);
  console.log("");
