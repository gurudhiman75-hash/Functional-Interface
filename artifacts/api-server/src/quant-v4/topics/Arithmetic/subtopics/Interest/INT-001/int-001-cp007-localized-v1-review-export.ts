...
const output: string[] = [
  "# INT-CP-007 — Hindi/Punjabi Native Review V1",
  "",
  `Localized candidate: **${INT_CP007_LOCALIZED_VERSION}**`,
  "",
  "Source authority: **INT-CP-007-EN-v8-frozen**.",
  "",
  "Punjabi Compound Interest authority: **ਮਿਸ਼ਰਤ ਵਿਆਜ**. The deprecated learner-facing term is rejected by audit.",
  "",
...
for (const qlId of INT_CP007_QL_IDS) {
  ...
  for (let index = 0; index < 200 && selected.length < 4; index += 1) {
    const seed = `int-cp007-localized-v1-review-${qlId}-${index}`;
  ...
  if (selected.length !== 4 || stemFamilies.size < 3) throw new Error(`${qlId}: failed to select four review surfaces covering all three stem families`);
...
const destination = path.resolve("dist/quant-v4/INT-CP-007-HI-PA-V1-NATIVE-REVIEW.md");
...
console.log("PASS_INT_CP007_LOCALIZED_V1_REVIEW_EXPORT");
