...
const OUT_DIR = resolve(process.cwd(), "dist/quant-v4/int-cp005-review-v2");
mkdirSync(OUT_DIR, { recursive: true });

function seedForPosition(qlId: IntCp005QlId, target: number): string {
  for (let attempt = 0; attempt < 2000; attempt += 1) {
    const seed = `int-cp005-v2-review-${qlId}-pos-${target}-${attempt}`;
    if (generateIntCp005QuestionV2(qlId, seed, "en-IN").correctIndex === target) return seed;
  }
  throw new Error(`${qlId}: no review seed found for answer position ${target}`);
}

function render(question: IntCp005QuestionV2, ordinal: number): string {
...
for (const locale of LOCALES) {
  const questions: IntCp005QuestionV2[] = [];
  ...
  for (const qlId of INT_CP005_QL_IDS) {
    for (let target = 0; target < 4; target += 1) {
      ...
      const question = generateIntCp005QuestionV2(qlId, seed, locale);
  ...
  const markdown = [
    `# INT-CP-005 Variable Rates, Growth & Decay — ${locale} Review Pack V2`,
    "",
    `Runtime: ${INT_CP005_RUNTIME_VERSION_V2}`,
  ...
  writeFileSync(resolve(OUT_DIR, `INT-CP-005-V2-REVIEW-${locale}.md`), markdown, "utf8");
  summary[locale] = {
    questions: questions.length,
    answerPositions,
...
console.log("PASS_INT_CP005_VARIABLE_GROWTH_DECAY_V2_REVIEW_EXPORT");
