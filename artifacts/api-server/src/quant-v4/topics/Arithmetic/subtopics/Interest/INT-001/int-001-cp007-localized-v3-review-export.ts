...
const output: string[] = [
  "# INT-CP-007 — Hindi/Punjabi Exam-Editorial Review V3",
  "",
...
for (const qlId of INT_CP007_QL_IDS) {
  ...
  for (let index = 0; index < 400 && selected.length < 4; index += 1) {
    const seed = `int-cp007-localized-v3-review-${qlId}-${index}`;
  ...
  if (selected.length !== 4 || stemFamilies.size < 3) throw new Error(`${qlId}: failed to select four meaningful V3 review surfaces across all stem families`);
...
const destination = path.resolve("dist/quant-v4/INT-CP-007-HI-PA-V3-EXAM-EDITORIAL-REVIEW.md");
...
console.log("PASS_INT_CP007_LOCALIZED_V3_REVIEW_EXPORT");
