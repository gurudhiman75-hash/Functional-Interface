...
const lines: string[] = [];
lines.push("# INT-CP-009 — Dated Cash-Flow Discovery Wave 01 Balanced Review");
lines.push("");
...
for (const prototypeId of INT_CP009_PROTOTYPE_IDS) {
  const found = new Map<string, ReturnType<typeof buildIntCp009BalancedDiscoveryPackage>>();
  for (let index = 0; index < 500 && found.size < 3; index += 1) {
    const seed = `int-cp009-review:${prototypeId}:${index}`;
    const question = buildIntCp009BalancedDiscoveryPackage(prototypeId, seed);
