...
import { INT_CP004_QL_IDS, verifyCp004Answer } from "./cp004-frequency-math";
import { selectIntCp004ExamFriendlyFrozenSourceV9 } from "./cp004-exam-friendly-source-v9";
import {
  INT_CP004_ENGLISH_EXAM_FRIENDLY_REVIEW_V13,
  generateIntCp004EnglishExamFriendlyReviewV13,
} from "./cp004-english-exam-friendly-review-v13";
...
let mixedFrequencyProofChecks = 0;
const answerPositions = [0, 0, 0, 0];
const stemFamilies = new Map<string, Set<string>>();
const uniqueStates = new Map<string, Set<string>>();

assert.equal(INT_CP004_ENGLISH_EXAM_FRIENDLY_REVIEW_V13.approved, false);
assert.equal(INT_CP004_ENGLISH_EXAM_FRIENDLY_REVIEW_V13.permanentIdentityChanges, false);
assert.equal(INT_CP004_ENGLISH_EXAM_FRIENDLY_REVIEW_V13.questionStudioActivationAuthorized, false);
assert.equal(INT_CP004_ENGLISH_EXAM_FRIENDLY_REVIEW_V13.mixedFrequencyExactFactorProof, true);
...
for (const qlId of INT_CP004_QL_IDS) {
  ...
  for (let index = 0; index < SAMPLES_PER_QL; index += 1) {
    const seed = `int-cp004-english-parity-v13:${qlId}:${index}`;
    const first = generateIntCp004EnglishExamFriendlyReviewV13(qlId, seed);
    const replay = generateIntCp004EnglishExamFriendlyReviewV13(qlId, seed);
    ...
    assert.equal(first.v13Remediation.mathematicalStateChanged, false);
    ...
    if (qlId === "INT-QL-084" || qlId === "INT-QL-085") {
      const joined = first.explanation.steps.join("\n");
      assert.ok(joined.includes("exact stage factor"), `${qlId}/${index}: exact stage-factor proof missing`);
      assert.ok(joined.includes("Combined growth factor"), `${qlId}/${index}: combined factor proof missing`);
      assert.equal(first.v13Remediation.mixedFrequencyExactFactorProofApplied, true);
      mixedFrequencyProofChecks += 3;
    }
    ...
    assert.equal(first.v13Remediation.approvalGranted, false);
    lifecycleChecks += 11;
...
console.log("PASS_INT_CP004_ENGLISH_PARITY_V13_AUDIT");
