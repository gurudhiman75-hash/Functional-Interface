...
const OUT_DIR = resolve(process.cwd(), "dist/quant-v4/int-cp005-review-v5");
mkdirSync(OUT_DIR, { recursive: true });

function seedForPosition(qlId: IntCp005QlId, target: number): string {
  for (let attempt = 0; attempt < 3000; attempt += 1) {
    const seed = `int-cp005-v5-review-${qlId}-pos-${target}-${attempt}`;
    if (generateIntCp005QuestionV5(qlId, seed, "en-IN").correctIndex === target) return seed;
  }
  throw new Error(`${qlId}: no review seed for answer position ${target}`);
}

function render(question: IntCp005QuestionV5, ordinal: number): string {
...
for (const locale of LOCALES) {
  const questions: IntCp005QuestionV5[] = [];
  ...
  for (const qlId of INT_CP005_QL_IDS) {
    for (let target = 0; target < 4; target += 1) {
      ...
      const question = generateIntCp005QuestionV5(qlId, seed, locale);
  ...
  const markdown = [
    `# INT-CP-005 Variable Rates, Growth & Decay — ${locale} Review Pack V5`,
    "",
    `Runtime: ${INT_CP005_RUNTIME_VERSION_V5}`,
  ...
  writeFileSync(resolve(OUT_DIR, `INT-CP-005-V5-REVIEW-${locale}.md`), markdown, "utf8");
...
console.log("PASS_INT_CP005_VARIABLE_GROWTH_DECAY_V5_REVIEW_EXPORT");
