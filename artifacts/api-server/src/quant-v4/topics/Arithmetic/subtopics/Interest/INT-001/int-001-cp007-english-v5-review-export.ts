...
function findSeeds(qlId: IntCp007QlId): readonly string[] {
  ...
  for (let index = 0; index < 10000; index += 1) {
    const seed = `int-cp007-en-v5-review-${qlId}-${index}`;
  ...
  if (seeds.some((seed) => !seed) || !extra) throw new Error(`${qlId}: failed to collect complete V5 review seed set`);
...
const output: string[] = [];
output.push("# INT-CP-007 — English Product Review V5 — LaTeX Presentation");
output.push("");
output.push(`Current English candidate: **${INT_CP007_ENGLISH_VERSION}**`);
output.push(`Supersedes: **${INT_CP007_ENGLISH_V5_SUPERSEDES}**.`);
output.push("");
output.push("V5 is a learner-presentation remediation after direct review of V4. Mathematical calculations are now explicitly MathJax/LaTeX wrapped with `$...$`, using structured fractions, multiplication, powers, subscripts and comparison operators. Currency symbols remain outside math segments. Permanent mathematics, QL identity, stems, options, correct answers and lifecycle controls are unchanged from V4.");
output.push("");
...
const outputPath = path.join(outputDirectory, "INT-CP-007-ENGLISH-V5-LATEX-REVIEW.md");
fs.writeFileSync(outputPath, `${output.join("\n")}\n`, "utf8");
console.log(JSON.stringify({ output: outputPath, qls: INT_CP007_QL_IDS.length, reviewQuestions }, null, 2));
console.log("PASS_INT_CP007_ENGLISH_V5_LATEX_REVIEW_EXPORT");
