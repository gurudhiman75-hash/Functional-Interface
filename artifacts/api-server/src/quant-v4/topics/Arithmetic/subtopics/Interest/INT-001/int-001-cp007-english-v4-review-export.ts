...
function findSeeds(qlId: IntCp007QlId): readonly string[] {
  ...
  for (let index = 0; index < 10000; index += 1) {
    const seed = `int-cp007-en-v4-review-${qlId}-${index}`;
  ...
  if (seeds.some((seed) => !seed) || !extra) throw new Error(`${qlId}: failed to collect complete V4 review seed set`);
...
const output: string[] = [];
output.push("# INT-CP-007 — English Product Review V4");
output.push("");
output.push(`Current English candidate: **${INT_CP007_ENGLISH_VERSION}**`);
output.push(`Supersedes: **${INT_CP007_ENGLISH_V4_SUPERSEDES}**.`);
output.push("");
output.push("V4 is the final teacher-style editorial pass after human review of the green V3 artifact. It removes one-year algebra clutter in QL111, reduces QL113 factor ratios directly to the final fraction, and rewrites QL114 stems so continuing annual schemes are not misleadingly described as one-year-only schemes. Permanent mathematics, identities, answer keys and lifecycle controls are unchanged.");
output.push("");
...
const outputPath = path.join(outputDirectory, "INT-CP-007-ENGLISH-V4-REVIEW.md");
fs.writeFileSync(outputPath, `${output.join("\n")}\n`, "utf8");
console.log(JSON.stringify({ output: outputPath, qls: INT_CP007_QL_IDS.length, reviewQuestions }, null, 2));
console.log("PASS_INT_CP007_ENGLISH_V4_REVIEW_EXPORT");
