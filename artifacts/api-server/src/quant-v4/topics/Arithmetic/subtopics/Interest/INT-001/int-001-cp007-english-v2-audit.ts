...
assert(INT_CP007_ENGLISH_VERSION === "INT-CP-007-EN-v2-review", "CP007 English V2 version drift");
assert(INT_CP007_ENGLISH_V2_SUPERSEDES === "INT-CP-007-EN-v1-review", "CP007 English V2 supersession drift");
assert(INT_CP007_RUNTIME_VERSION === "INT-CP-007-v3-permanent-allocation", "CP007 permanent runtime drift under English V2");
...
let ql113DistinctDistractorChecks = 0;
...
for (const qlId of INT_CP007_QL_IDS) {
  ...
  for (let index = 0; index < 200; index += 1) {
    const seed = `int-cp007-en-v2-${qlId}-${index}`;
    ...
    assert(question.englishVersion === INT_CP007_ENGLISH_VERSION, `${qlId}/${seed}: English V2 version drift`);
    ...
    if (qlId === "INT-QL-113") {
      const distractors = question.options.filter((_option, optionIndex) => optionIndex !== question.correctIndex);
      assert(distractors.length === 3, `${qlId}/${seed}: QL113 requires three distractors`);
      assert(new Set(distractors.map((option) => option.text)).size === 3, `${qlId}/${seed}: QL113 distractor text collision`);
      assert(new Set(distractors.map((option) => option.misconceptionId)).size === 3, `${qlId}/${seed}: QL113 misconception collision`);
      ql113DistinctDistractorChecks += 3;
    }
...
assert(questions === 1400, `expected 1400 English V2 review questions, got ${questions}`);
assert(correctIndexCoverage.size === 4, "all four correct-answer positions were not exercised");
assert(ql113DistinctDistractorChecks === 600, `expected 600 QL113 V2 distractor checks, got ${ql113DistinctDistractorChecks}`);
assert(INT_CP007_PERMANENT_ALLOCATION.permanentIdentityFrozen, "permanent allocation authority drift");
assert(!INT_CP007_PERMANENT_ALLOCATION.learnerContentFrozen, "English V2 review unexpectedly frozen");

console.log(JSON.stringify({
  englishVersion: INT_CP007_ENGLISH_VERSION,
  supersedes: INT_CP007_ENGLISH_V2_SUPERSEDES,
...
console.log("PASS_INT_CP007_ENGLISH_V2_AUDIT");
