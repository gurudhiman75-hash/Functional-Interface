...
import { INT_CP005_V16_QL_IDS, INT_CP005_RUNTIME_VERSION_V16, generateIntCp005QuestionV16EditorialFinal } from "./cp005-variable-growth-decay-runtime-v16-editorial-final";

function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(message);
}

const selected: ReturnType<typeof generateIntCp005QuestionV16EditorialFinal>[] = [];
for (const qlId of INT_CP005_V16_QL_IDS) {
  const positions = new Map<number, ReturnType<typeof generateIntCp005QuestionV16EditorialFinal>>();
...
lines.push("- Rounded/synthetic money distractors: removed; audited option cap ₹3 lakh");
...
console.log("PASS_INT_CP005_V16_EDITORIAL_FINAL_REVIEW_EXPORT");
