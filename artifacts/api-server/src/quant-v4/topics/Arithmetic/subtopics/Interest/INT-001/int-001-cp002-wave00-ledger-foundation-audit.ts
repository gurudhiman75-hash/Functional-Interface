...
function assertRational(actual: Rational, expected: Rational, label: string): void {
  assert(equalsRational(actual, expected), `${label}: expected ${expected.numerator}/${expected.denominator}, received ${actual.numerator}/${actual.denominator}`);
}
...
let exactLedgerChecks = 0;
let inverseChecks = 0;
let verifierChecks = 0;
let tamperRejectionChecks = 0;
let invalidStateChecks = 0;
let dayCountChecks = 0;
let eventOrderChecks = 0;

const piecewiseContributions: IntCp002Contribution[] = [
  contribution({
    id: "piecewise-first",
...
const piecewiseState: IntCp002LedgerState = {
  contributions: piecewiseContributions,
  dayCountBasis: "NOT_APPLICABLE",
  totalInterest: rational(1800),
};
const piecewiseResult = calculateIntCp002Ledger(piecewiseState);
assertRational(piecewiseResult.contributions[0]!.interest, rational(1000), "piecewise first interval");
assertRational(piecewiseResult.contributions[1]!.interest, rational(800), "piecewise second interval");
assertRational(piecewiseResult.totalInterest, rational(1800), "piecewise total");
...
const multipleDepositResult = calculateIntCp002Ledger(multipleDepositState);
assertRational(multipleDepositResult.contributions[0]!.interest, rational(600), "deposit A interest");
assertRational(multipleDepositResult.contributions[1]!.interest, rational(900), "deposit B interest");
assertRational(multipleDepositResult.totalInterest, rational(1500), "multiple-deposit total");
assert(verifyIntCp002LedgerCandidate(multipleDepositState, rational(1500)).ok, "multiple-deposit verification failed");
exactLedgerChecks += 3;
verifierChecks += 1;

const commonRate = solveIntCp002CommonRatePercent(rational(1200), [
  { principal: rational(5000), durationYears: rational(2) },
  { principal: rational(10000), durationYears: rational(1) },
]);
assertRational(commonRate, rational(6), "common rate recovery");
inverseChecks += 1;
...
const principalVerification = verifyIntCp002UnknownContributionCandidate({
...
assert(principalVerification.ok, "missing principal independent verification failed");
inverseChecks += 1;
verifierChecks += 1;
...
assert(
  verifyIntCp002LedgerCandidate({
    contributions: [...knownForDuration, durationContribution],
    dayCountBasis: "NOT_APPLICABLE",
    totalInterest: rational(1500),
  }, rational(1500)).ok,
  "missing duration independent verification failed",
);
inverseChecks += 1;
verifierChecks += 1;
...
const comparison = compareIntCp002Ledgers(
  piecewiseState,
  multipleDepositState,
);
...
const outstandingContributions = buildIntCp002OutstandingBalanceContributions({
...
assert(outstandingContributions.length === 2, "outstanding-balance segment count mismatch");
assertRational(outstandingContributions[0]!.principal, rational(10000), "first outstanding balance");
assertRational(outstandingContributions[1]!.principal, rational(6000), "second outstanding balance");
const outstandingState: IntCp002LedgerState = {
  contributions: outstandingContributions,
  dayCountBasis: "NOT_APPLICABLE",
...
const tamperedLedger = verifyIntCp002LedgerCandidate(piecewiseState, rational(1801));
assert(!tamperedLedger.ok, "tampered ledger total was accepted");
assert(!verifyIntCp002LedgerDifferenceCandidate({
  left: piecewiseState,
  right: multipleDepositState,
