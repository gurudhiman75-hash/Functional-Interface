...
function findSeeds(qlId: IntCp007QlId): readonly string[] {
  ...
  for (let index = 0; index < 10000; index += 1) {
    const seed = `int-cp007-en-v3-review-${qlId}-${index}`;
  ...
  if (seeds.some((seed) => !seed) || !extra) throw new Error(`${qlId}: failed to collect complete V3 review seed set`);
...
const output: string[] = [];
output.push("# INT-CP-007 — English Product Review V3");
output.push("");
output.push(`Current English candidate: **${INT_CP007_ENGLISH_VERSION}**`);
output.push(`Supersedes technically green but editorially superseded candidate: **${INT_CP007_ENGLISH_V3_SUPERSEDES}**.`);
output.push("");
output.push("V3 is a narrow human-QA remediation. Permanent mathematics, QL identities and stems are unchanged. QL111 now carries the missing-rate algebra through to r; QL114 uses learner-friendly decimal factor displays instead of raw long rational fractions; QL115 uses clean rupee/paise distractors instead of fraction-form currency.");
output.push("");
...
const outputPath = path.join(outputDirectory, "INT-CP-007-ENGLISH-V3-REVIEW.md");
fs.writeFileSync(outputPath, `${output.join("\n")}\n`, "utf8");
console.log(JSON.stringify({ output: outputPath, qls: INT_CP007_QL_IDS.length, reviewQuestions }, null, 2));
console.log("PASS_INT_CP007_ENGLISH_V3_REVIEW_EXPORT");
