...
const lines: string[] = [];
lines.push("# INT-CP-009 — Exam-Readiness V2 Review");
lines.push("");
...
for (const prototypeId of INT_CP009_PROTOTYPE_IDS) {
  const found = new Map<string, ReturnType<typeof buildIntCp009ExamReadyDiscoveryPackage>>();
  for (let index = 0; index < 600 && found.size < 3; index += 1) {
    const seed = `int-cp009-exam-review:${prototypeId}:${index}`;
    const question = buildIntCp009ExamReadyDiscoveryPackage(prototypeId, seed);
    found.set(question.presentation.stemFamilyId, question);
  }
  if (found.size !== 3) throw new Error(`${prototypeId}: could not recover all three exam-ready stem families`);
...
if (exported !== 24) throw new Error(`Expected 24 CP009 V2 review questions, got ${exported}`);
