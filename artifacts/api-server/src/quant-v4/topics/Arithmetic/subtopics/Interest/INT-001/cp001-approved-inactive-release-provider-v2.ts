...
export type IntCp001ApprovedInactiveLanguage = "en" | "hi" | "pa";
export type IntCp001ApprovedInactiveQuestion =
  | IntCp001ApprovedCloseDistractorEnglishQuestion
  | IntCp001ApprovedCloseDistractorLocalizedQuestion;

export const INT_CP001_APPROVED_INACTIVE_PROVIDER_V2 = Object.freeze({
  providerId: "INT-001:INT-CP-001:APPROVED-INACTIVE-V2",
  packageId: "INT-001",
  ...
  runtimeMode: "APPROVED_CLOSE_DISTRACTOR_INACTIVE_RELEASE_PROOF",
...
export interface IntCp001ApprovedInactiveRequest {
  qlId: IntCp001FinalQlId;
  language: IntCp001ApprovedInactiveLanguage;
  seed: string;
}

export interface IntCp001ApprovedInactiveEnvelope {
  provider: typeof INT_CP001_APPROVED_INACTIVE_PROVIDER_V2;
  question: IntCp001ApprovedInactiveQuestion;
  trace: {
    providerId: typeof INT_CP001_APPROVED_INACTIVE_PROVIDER_V2.providerId;
    packageId: "INT-001";
    canonicalProblemId: "INT-CP-001";
    qlId: IntCp001FinalQlId;
    language: IntCp001ApprovedInactiveLanguage;
...
function assertSeed(seed: string): string {
  ...
  if (!normalized) {
    throw new Error("INT-CP-001 approved inactive provider requires an explicit deterministic seed.");
  }
  return normalized;
}
...
function assertLanguage(language: string): asserts language is IntCp001ApprovedInactiveLanguage {
  if (!(INT_CP001_APPROVED_INACTIVE_PROVIDER_V2.supportedLanguages as readonly string[]).includes(language)) {
    throw new Error(`Unsupported INT-CP-001 approved release language '${language}'.`);
  }
}

export function generateIntCp001ApprovedInactiveEnvelope(
  request: IntCp001ApprovedInactiveRequest,
): IntCp001ApprovedInactiveEnvelope {
  ...
  const question = request.language === "en"
    ? generateIntCp001ApprovedCloseDistractorEnglishQuestion(request.qlId, seed)
    : generateIntCp001ApprovedCloseDistractorLocalizedQuestion(request.qlId, seed, request.language);

  if (!question.validation.ok) {
    throw new Error(`${request.qlId}/${seed}/${request.language}: ${question.validation.errors.join(" | ")}`);
  }

  const expectedRelease = INT_CP001_APPROVED_INACTIVE_PROVIDER_V2.releaseIds[request.language];
...
function explanationText(question: IntCp001ApprovedInactiveQuestion): string {
...
export function toIntCp001ApprovedInactivePreview(
  envelope: IntCp001ApprovedInactiveEnvelope,
  ...
  return {
    ...
    topic: "Arithmetic",
    subtopic: "Simple Interest",
    generationBackend: "quant-v4-approved-inactive-proof-v2",
    debugSource: "int-cp001-approved-close-distractor-inactive-provider-v2",
    canonicalProblemId: "INT-CP-001",
...
export function generateIntCp001ApprovedInactiveBatch(request: {
  language: IntCp001ApprovedInactiveLanguage;
  seed: string;
  count?: number;
  qlId?: IntCp001FinalQlId;
  ...
  const qlOrder = request.qlId
    ? [request.qlId]
    : shuffled(INT_CP001_FINAL_QL_IDS, `${batchSeed}:INT-001:approved-v2:ql-order`);

  const envelopes = Array.from({ length: count }, (_unused, index) => {
    const qlId = qlOrder[index % qlOrder.length]!;
    return generateIntCp001ApprovedInactiveEnvelope({
      qlId,
      language: request.language,
  ...
  return {
    ...
    questions: envelopes.map((item, index) =>
      toIntCp001ApprovedInactivePreview(item, {
