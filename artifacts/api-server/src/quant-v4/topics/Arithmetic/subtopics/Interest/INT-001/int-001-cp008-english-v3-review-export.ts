import { generateIntCp008EnglishQuestion } from "./cp008-instalment-english-v3";
import { INT_CP008_QL_IDS } from "./cp008-instalment-runtime-v1-final";

const lines: string[] = [
  "# INT-CP-008 English V3 Review Pack",
  "",
  "Status: English learner-content review only. Permanent QL identities are frozen; learner content is not frozen.",
  "",
  "V3 preserves exact underlying Rational mathematics, repairs the QL120 MathJax defect, expands QL122 recurring-fund steps, and renders non-paise exact money to the nearest paise with an explicit instruction.",
  "",
];
...
for (const qlId of INT_CP008_QL_IDS) {
  ...
  for (let index = 0; index < 800 && selected.size < 6; index += 1) {
    const seed = `int-cp008-en-v3-review:${qlId}:${index}`;
  ...
  if (selected.size !== 6) throw new Error(`${qlId}: could not collect all six stem families for V3 review export`);
