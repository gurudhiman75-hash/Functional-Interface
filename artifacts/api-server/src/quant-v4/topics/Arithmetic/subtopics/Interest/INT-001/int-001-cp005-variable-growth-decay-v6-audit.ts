...
function learnerStrings(question: IntCp005QuestionV6): readonly string[] {
...
function optionKey(question: IntCp005QuestionV6): string {
...
function assertSurface(question: IntCp005QuestionV6): void {
...
assert.equal(INT_CP005_RUNTIME_VERSION_V6, "INT-CP-005-VARIABLE-GROWTH-DECAY-v6");
...
for (const qlId of INT_CP005_QL_IDS) {
  ...
  for (let index = 0; index < 100; index += 1) {
    const seed = `int-cp005-v6-audit-${qlId}-${index}`;
    const english = generateIntCp005QuestionV6(qlId, seed, "en-IN");
    assert.deepEqual(generateIntCp005QuestionV6(qlId, seed, "en-IN"), english, `${qlId}/${seed}: replay drift`);
    replayChecks += 1;
    ...
    for (const locale of LOCALES) {
      const question = locale === "en-IN" ? english : generateIntCp005QuestionV6(qlId, seed, locale);
      questions += 1;
      assert.equal(question.runtimeVersion, INT_CP005_RUNTIME_VERSION_V6);
...
console.log("PASS_INT_CP005_VARIABLE_GROWTH_DECAY_V6");
