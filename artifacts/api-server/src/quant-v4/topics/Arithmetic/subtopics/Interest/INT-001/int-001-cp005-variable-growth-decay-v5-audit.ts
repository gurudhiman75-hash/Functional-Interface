...
function learnerStrings(question: IntCp005QuestionV5): readonly string[] {
...
function optionKey(question: IntCp005QuestionV5): string {
...
function assertSurface(question: IntCp005QuestionV5): void {
  ...
  assert(question.explanation.steps[0]?.startsWith(prefix));
  assert(/\\\([^\n]+\\\)/u.test(question.explanation.steps[0] ?? ""));
...
assert.equal(INT_CP005_RUNTIME_VERSION_V5, "INT-CP-005-VARIABLE-GROWTH-DECAY-v5");
...
for (const qlId of INT_CP005_QL_IDS) {
  ...
  for (let index = 0; index < 100; index += 1) {
    const seed = `int-cp005-v5-audit-${qlId}-${index}`;
    const english = generateIntCp005QuestionV5(qlId, seed, "en-IN");
    assert.deepEqual(generateIntCp005QuestionV5(qlId, seed, "en-IN"), english, `${qlId}/${seed}: replay drift`);
    replayChecks += 1;
    ...
    for (const locale of LOCALES) {
      const question = locale === "en-IN" ? english : generateIntCp005QuestionV5(qlId, seed, locale);
      questions += 1;
      assert.equal(question.runtimeVersion, INT_CP005_RUNTIME_VERSION_V5);
  ...
  const minimum = qlId === "INT-QL-093" ? 16 : qlId === "INT-QL-095" ? 60 : 20;
...
assert(thresholdRates.size >= 8, `threshold rates ${thresholdRates.size} < 8`);
...
console.log("PASS_INT_CP005_VARIABLE_GROWTH_DECAY_V5");
