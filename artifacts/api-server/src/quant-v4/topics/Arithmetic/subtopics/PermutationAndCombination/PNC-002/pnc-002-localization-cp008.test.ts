...
import {
  buildPnc002Cp008LocalizedPresentation,
  PNC_002_CP008_LOCALIZATION_CANDIDATE,
} from "./foundation/localization-cp008";
...
assert.equal(PNC_002_CP008_LOCALIZATION_CANDIDATE.publiclyPublishable, false);
assert.equal(PNC_002_CP008_LOCALIZATION_CANDIDATE.editorialStatus, "PENDING");
assert.equal(PNC_002_CP008_LOCALIZATION_CANDIDATE.status, "MANUAL_REVIEW");
assert.equal(PNC_002_CP008_LOCALIZATION_CANDIDATE.qlCount, 23);
    ...
    for (const seedName of seeds) {
      try {
        const source = runPnc002Pipeline({
          ...
          seed: `pnc-cp008-localization:${locale}:${seedName}:${entry.qlId}`,
        });
        ...
        const localized = buildPnc002Cp008LocalizedPresentation(source, locale);
        auditedPackages += 1;

        assert.equal(localized.questionLanguageId, entry.qlId);
        assert.equal(localized.canonicalProblemId, "PNC-CP-008");
        ...
        assert.doesNotMatch(proseOnly, /In how many|Core Concept|Step-by-Step|Exam Speed Shortcut|Common Trap|Don't fall for Option|Interpret the condition|Count the next stage|Final answer/i);
        assert.doesNotMatch(proseOnly, /\{[A-Za-z][A-Za-z0-9_]{1,}\}/);
        assert.doesNotMatch(proseOnly, /रैखिक|ਸਿੱਧੇ ਕ੍ਰਮ|linear arrangement|relative order chain/i);
...
assert.equal(exportRows.length, 46);
for (const locale of locales) {
...
const report = {
  releaseId: PNC_002_CP008_LOCALIZATION_CANDIDATE.releaseId,
  packageId: "PNC-002",
...
const directory = resolve(process.cwd(), "dist/quant-v4/pnc-002-localization-cp008");
mkdirSync(directory, { recursive: true });
writeFileSync(resolve(directory, "pnc-002-cp008-hi-pa-review.json"), `${JSON.stringify(exportRows, null, 2)}\n`, "utf8");
writeFileSync(resolve(directory, "pnc-002-cp008-hi-pa-review.csv"), `${csvLines.join("\n")}\n`, "utf8");
writeFileSync(resolve(directory, "pnc-002-cp008-hi-pa-report.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
