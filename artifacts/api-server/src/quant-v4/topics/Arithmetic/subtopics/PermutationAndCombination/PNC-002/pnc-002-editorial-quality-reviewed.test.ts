...
const allEntries: EditorialEntry[] = [
  ...baseEntries.map((entry) => ({
    ...
    run: () => runPnc002Pipeline({ questionLanguageId: entry.qlId, seed: `pnc-editorial-reviewed:${entry.qlId}` }),
  })),
  ...cp011Grouping.map((entry) => ({
    ...
    difficulty: entry.difficulty,
    solveMode: entry.solveMode,
    run: () => runPnc002Cp011GroupingPipeline({ questionLanguageId: entry.qlId, seed: `pnc-editorial-reviewed:${entry.qlId}` }),
  })),
  ...cp011Distribution1.map((entry) => ({
    ...
    difficulty: entry.difficulty,
    solveMode: entry.solveMode,
    run: () => runPnc002Cp011DistributionWave1Pipeline({ questionLanguageId: entry.qlId, seed: `pnc-editorial-reviewed:${entry.qlId}` }),
  })),
  ...cp011Distribution2.map((entry) => ({
    ...
    difficulty: entry.difficulty,
    solveMode: entry.solveMode,
    run: () => runPnc002Cp011DistributionWave2Pipeline({ questionLanguageId: entry.qlId, seed: `pnc-editorial-reviewed:${entry.qlId}` }),
  })),
  ...cp011Inverse.map((entry) => ({
    ...
    difficulty: entry.difficulty,
    solveMode: entry.solveMode,
    run: () => runPnc002Cp011InversePipeline({ questionLanguageId: entry.qlId, seed: `pnc-editorial-reviewed:${entry.qlId}` }),
  })),
  ...cp012Entries.map((entry) => ({
    ...
    difficulty: entry.difficulty,
    solveMode: entry.solveMode,
    run: () => runPnc002Cp012Pipeline({ questionLanguageId: entry.qlId, seed: `pnc-editorial-reviewed:${entry.qlId}` }),
  })),
];
...
for (const entry of allEntries) {
  try {
    ...
    assert.ok(!roboticPhrases.some((phrase) => presentation.stem.toLowerCase().includes(phrase)), `${entry.qlId}: robotic phrase remains`);
    ...
    assert.ok(steps.length >= 4, `${entry.qlId}: step-by-step section is too short`);
    assert.ok(steps.every((line, index) => line.startsWith(`${index + 1}. `)), `${entry.qlId}: solution steps must be numbered`);
    assert.ok(steps.some((line) => line.includes(`$$${source.solver.mathJax}$$`)), `${entry.qlId}: display MathJax calculation is missing`);
    assert.ok(steps.join(" ").includes(presentation.answerLabel), `${entry.qlId}: final labelled answer is missing`);
...
assert.equal(byQl.get("PNC-QL-107")?.optionUnit, "arrangements");
assert.equal(byQl.get("PNC-QL-219")?.optionUnit, "distributions");
assert.equal(byQl.get("PNC-QL-253")?.optionUnit, "permutations");
assert.equal(byQl.get("PNC-QL-258")?.optionUnit, "paths");
assert.equal(byQl.get("PNC-QL-269")?.optionUnit, "ways");
assert.ok(byQl.get("PNC-QL-219")?.upgradedStem.startsWith("A records office is filing documents into labelled storage locations."));
...
const report = {
  packageId: "PNC-002",
  editorialStandard: "REVIEWED_HUMANISED_STEMS_LABELLED_OPTIONS_FOUR_TIER_EXPLANATIONS",
...
const outputDirectory = resolve(process.cwd(), "dist/quant-v4/pnc-002-editorial-quality-reviewed");
mkdirSync(outputDirectory, { recursive: true });
writeFileSync(resolve(outputDirectory, "pnc-002-editorial-quality-reviewed.json"), `${JSON.stringify(outputRows, null, 2)}\n`, "utf8");
writeFileSync(resolve(outputDirectory, "pnc-002-editorial-quality-reviewed.csv"), `${csvLines.join("\n")}\n`, "utf8");
writeFileSync(resolve(outputDirectory, "pnc-002-editorial-quality-reviewed-report.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
