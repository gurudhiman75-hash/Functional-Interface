...
import { getPnc002QuestionEntries, getPnc002QuestionLanguageIds } from "./foundation/library";
...
const solveModeCounts = Object.fromEntries(
  [...new Set(entries.map((entry) => entry.solveMode))].map((solveMode) => [
    solveMode,
    entries.filter((entry) => entry.solveMode === solveMode).length,
  ]),
);
...
assert.equal(
  (mixedRestriction.solver.evidence.validUnitArrangementCount ?? 0)
    + (mixedRestriction.solver.evidence.adjacentExternalPairCount ?? 0),
...
for (const entry of entries) {
  for (let seedIndex = 0; seedIndex < 12; seedIndex += 1) {
    const seed = `pnc-002-proof:${entry.qlId}:${seedIndex}`;
...
console.log(JSON.stringify({
  ...
  activeSolveModeCount: Object.keys(solveModeCounts).length,
  generatedCases,
