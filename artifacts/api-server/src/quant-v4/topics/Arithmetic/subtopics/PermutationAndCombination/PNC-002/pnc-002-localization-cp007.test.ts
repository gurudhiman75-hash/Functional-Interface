...
import { buildPnc002Cp007LocalizedPresentation, PNC_002_CP007_LOCALIZATION_PILOT } from "./foundation/localization-cp007";
...
const expectedIds = Array.from({ length: 18 }, (_, index) => `PNC-QL-${String(index + 107).padStart(3, "0")}`);
const locales: PncStudentLocale[] = ["hi-IN", "pa-IN"];
const seeds = ["review-a", "review-b", "review-c"];

assert.equal(PNC_002_CP007_LOCALIZATION_PILOT.publiclyPublishable, false);
assert.equal(PNC_002_CP007_LOCALIZATION_PILOT.editorialStatus, "PENDING");
assert.equal(PNC_002_CP007_LOCALIZATION_PILOT.status, "MANUAL_REVIEW");
assert.equal(PNC_002_CP007_LOCALIZATION_PILOT.qlCount, 18);
    ...
    for (const seedName of seeds) {
      try {
        const source = runPnc002Pipeline({
          ...
          seed: `pnc-cp007-localization:${locale}:${seedName}:${entry.qlId}`,
        });
        ...
        const localized = buildPnc002Cp007LocalizedPresentation(source, locale);
        auditedPackages += 1;

        assert.equal(localized.questionLanguageId, entry.qlId);
        assert.equal(localized.canonicalProblemId, "PNC-CP-007");
        ...
        assert.doesNotMatch(studentText, /In how many|Core Concept|Step-by-Step|Exam Speed Shortcut|Common Trap|Don't fall for Option|Interpret the condition|Count the next stage|Final answer/i);
        assert.doesNotMatch(studentText.replace(/\$\$[\s\S]*?\$\$|\$[^$\n]+\$/g, " "), /\{[A-Za-z][A-Za-z0-9_]{1,}\}/);
        ...
        assert.ok(localizedCore.lines.length >= 1 && localizedCore.lines.length <= 2);
        ...
        assert.ok(localizedShortcut.lines[0]!.includes("$"));
        assert.deepEqual(mathTokens(localizedShortcut.lines[0]!), mathTokens(getSection(english, "examSpeedShortcut").lines[0]!));
...
assert.equal(auditedPackages, 18 * 2 * 3);
assert.equal(exportRows.length, 36);
for (const locale of locales) {
...
const csvLines = [
  ...
  ...exportRows.map((row) => [
    row.qlId, row.cpId, row.difficulty, row.solveMode, row.locale, row.englishStem, row.localizedStem, row.optionUnit,
    ...
    row.answerLabel, row.coreHeading, row.coreConcept, row.stepHeading, row.stepByStep, row.shortcutHeading,
    row.examSpeedShortcut, row.trapHeading, row.commonTrapWarning, row.runtimeValidation, row.editorialStatus, row.publiclyPublishable,
  ].map(csvCell).join(",")),
];

const report = {
  releaseId: PNC_002_CP007_LOCALIZATION_PILOT.releaseId,
  packageId: "PNC-002",
  canonicalProblemId: "PNC-CP-007",
  qlRange: ["PNC-QL-107", "PNC-QL-124"],
  qlCount: 18,
  locales,
...
const directory = resolve(process.cwd(), "dist/quant-v4/pnc-002-localization-cp007");
mkdirSync(directory, { recursive: true });
writeFileSync(resolve(directory, "pnc-002-cp007-hi-pa-review.json"), `${JSON.stringify(exportRows, null, 2)}\n`, "utf8");
writeFileSync(resolve(directory, "pnc-002-cp007-hi-pa-review.csv"), `${csvLines.join("\n")}\n`, "utf8");
writeFileSync(resolve(directory, "pnc-002-cp007-hi-pa-report.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
