...
export function auditPnc002Coverage(): Pnc002CoverageAudit {
  ...
  const expectedQlIds = Array.from({ length: 18 }, (_, index) => `PNC-QL-${String(index + 107).padStart(3, "0")}`);
  ...
  return { passed, activeQlCount: entries.length, expectedQlCount: expectedQlIds.length, missingQlIds, duplicateQlIds, exactDuplicateTemplateGroups, difficultyCounts, solveModeCounts, invalidRuntimeSamples };
}
