...
export function formatPnc001LatexText(value: string): string {
  ...
  output = output.replace(
    /\b(\d+)C(\d+)\s*×\s*(\d+)P(\d+)(?:\s*=\s*(\d+))?/g,
    (_match: string, n: string, s: string, selected: string, roles: string, answer?: string) =>
      wrap(`\\binom{${n}}{${s}} \\times {}^{${selected}}P_{${roles}}${answer ? ` = ${answer}` : ""}`),
  );

  output = output.replace(
    /\b(\d+)P(\d+)(?:\s*=\s*(\d+))?/g,
    (_match: string, n: string, r: string, answer?: string) =>
      wrap(`{}^{${n}}P_{${r}}${answer ? ` = ${answer}` : ""}`),
  );
  output = output.replace(
    /\b(\d+)C(\d+)(?:\s*=\s*(\d+))?/g,
    (_match: string, n: string, r: string, answer?: string) =>
      wrap(`\\binom{${n}}{${r}}${answer ? ` = ${answer}` : ""}`),
  );

  output = output.replace(
    /\bC\(\s*([A-Za-z])\s*,\s*([A-Za-z])\s*\)/g,
    (_match: string, n: string, r: string) => wrap(`\\binom{${n}}{${r}}`),
  );
  output = output.replace(
    /\bP\(\s*([A-Za-z])\s*,\s*([A-Za-z])\s*\)/g,
    (_match: string, n: string, r: string) => wrap(`{}^{${n}}P_{${r}}`),
  );
  output = output.replace(/\bnCr\b/gi, () => wrap("\\binom{n}{r}"));
  output = output.replace(/\bnPr\b/gi, () => wrap("{}^nP_r"));
  output = output.replace(
    /\b([nrs])C([nrs])\b/g,
    (_match: string, n: string, r: string) => wrap(`\\binom{${n}}{${r}}`),
  );
...
export function validatePnc001LatexContract(pkg: Pnc001QuestionPackage): Pnc001ValidationCheck[] {
  ...
  const balanced = visibleText.every((value) =>
    delimiterCount(value, "\\(") === delimiterCount(value, "\\)")
    && delimiterCount(value, "\\[") === delimiterCount(value, "\\]"),
  );
