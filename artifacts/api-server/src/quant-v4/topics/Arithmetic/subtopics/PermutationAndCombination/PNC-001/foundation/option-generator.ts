import { combinationExact, createSeededRandom, factorialExact, multisetPermutationExact, permutationExact, powerExact, productExact, shuffleSeeded, sumExact } from "./math";
...
export function buildPnc001Options(parameters:Pnc001Parameters,solver:Pnc001SolverResult):{options:string[];correctIndex:number}{
 ...
 switch(parameters.solveMode){
  ...
  case"formNumbersWithoutRepetitionNoZero":{const symbols=e.symbolCount??2,length=e.digitLength??2;candidates=[powerExact(symbols,length),permutationExact(symbols,Math.max(1,length-1)),factorialExact(length),sumExact(e.positionChoices??[])];break;}
  case"formNumbersWithoutRepetitionWithZero":{const symbols=e.symbolCount??3,length=e.digitLength??2,maximum=e.digitMaximum??symbols-1;candidates=[permutationExact(symbols,length),powerExact(symbols,length),permutationExact(maximum,length),Math.max(1,correct-permutationExact(maximum,Math.max(1,length-1)))];break;}
  case"formCodesWithRepetition":{const symbols=e.symbolCount??2,length=e.digitLength??2;candidates=[permutationExact(symbols,length),powerExact(Math.max(1,symbols-1),length),symbols*length,factorialExact(length)];break;}
  case"formNumbersWithRepetitionAndZero":{const symbols=e.symbolCount??3,length=e.digitLength??2,first=e.firstPositionChoices??symbols-1;candidates=[powerExact(symbols,length),powerExact(first,length),permutationExact(symbols,length),first*length];break;}
  case"formParityNumbersWithoutRepetition":{const cases=e.caseCounts??[],symbols=e.symbolCount??4,length=e.digitLength??2;candidates=[permutationExact(symbols,length),cases[0]?.count??correct+1,cases[1]?.count??Math.max(1,Math.floor(correct/2)),Math.max(1,correct-(e.eligibleLastDigits?.length??1))];break;}
  case"formDivisibleByFiveNumbersWithoutRepetition":{const cases=e.caseCounts??[],symbols=e.symbolCount??6,length=e.digitLength??2;candidates=[permutationExact(symbols,length),cases[0]?.count??correct+1,cases[1]?.count??correct+2,Math.max(1,2*permutationExact(Math.max(1,symbols-1),Math.max(1,length-1)))];break;}
  case"formNumbersAboveLeadingThreshold":{const qualifying=e.qualifyingFirstDigits?.length??1,suffix=e.suffixArrangementCount??1,symbols=e.symbolCount??5;candidates=[suffix,(symbols-1)*suffix,Math.max(1,(qualifying-1)*suffix),qualifying*powerExact(Math.max(1,symbols-1),3)];break;}
  case"formAlphanumericCodes":{const letterStage=e.letterStageCount??1,digitStage=e.digitStageCount??1;candidates=[letterStage+digitStage,letterStage,digitStage,Math.max(1,letterStage*digitStage-(e.letterChoices??1))];break;}
  case"recoverSymbolCountForCode":{const target=e.codeTarget??correct,length=e.digitLength??2;candidates=[Math.max(1,correct-1),correct+1,length,Math.max(1,Math.round(Math.pow(target,1/Math.max(1,length-1))))];break;}
  case"formCodesWithExactlyOnePair":{const symbols=e.symbolCount??4,repeated=e.repeatedSymbolChoices??symbols,others=e.otherSymbolSelectionCount??1,pattern=e.patternArrangementCount??12;candidates=[permutationExact(symbols,4),repeated*others,others*pattern,combinationExact(symbols,2)*pattern];break;}
