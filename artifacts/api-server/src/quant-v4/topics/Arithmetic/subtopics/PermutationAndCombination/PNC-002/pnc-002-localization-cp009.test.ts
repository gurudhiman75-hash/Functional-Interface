...
import {
  buildPnc002Cp009LocalizedPresentation,
  PNC_002_CP009_LOCALIZATION_CANDIDATE,
} from "./foundation/localization-cp009";
...
assert.equal(PNC_002_CP009_LOCALIZATION_CANDIDATE.publiclyPublishable, false);
assert.equal(PNC_002_CP009_LOCALIZATION_CANDIDATE.editorialStatus, "PENDING");
assert.equal(PNC_002_CP009_LOCALIZATION_CANDIDATE.status, "MANUAL_REVIEW");
assert.equal(PNC_002_CP009_LOCALIZATION_CANDIDATE.qlCount, 29);
    ...
    for (const seedName of seeds) {
      try {
        const source = runPnc002Pipeline({
          ...
          seed: `pnc-cp009-localization:${locale}:${seedName}:${entry.qlId}`,
        });
        ...
        const localized = buildPnc002Cp009LocalizedPresentation(source, locale);
        auditedPackages += 1;

        assert.equal(localized.questionLanguageId, entry.qlId);
        assert.equal(localized.canonicalProblemId, "PNC-CP-009");
        ...
        assert.deepEqual(
          localized.displayOptions.map(optionNumber),
          source.options.map((option) => option.replace(/,/g, "")),
        );
        ...
        assert.doesNotMatch(
          proseOnly,
          /In how many|How many committees|How many teams|Core Concept|Step-by-Step|Exam Speed Shortcut|Common Trap|Don't fall for Option|Final answer|Category A|Category B/i,
        );
        ...
        assert.match(
          localizedCore.heading,
          locale === "hi-IN" ? /^📌 मूल अवधारणा — / : /^📌 ਮੁੱਖ ਵਿਚਾਰ — /,
        );
        assert.ok(localizedCore.lines.length >= 1 && localizedCore.lines.length <= 2);
        assert.equal(
          localizedSteps.heading,
          locale === "hi-IN" ? "📝 चरण-दर-चरण हल" : "📝 ਕਦਮ-ਦਰ-ਕਦਮ ਹੱਲ",
        );
        ...
        assert.deepEqual(
          mathTokens(localizedSteps.lines.join("\n")),
          mathTokens(englishSteps.lines.join("\n")),
        );

        assert.equal(
          localizedShortcut.heading,
          locale === "hi-IN" ? "⚡ परीक्षा में तेज़ तरीका" : "⚡ ਪੇਪਰ ਵਿੱਚ ਤੇਜ਼ ਤਰੀਕਾ",
        );
        assert.equal(localizedShortcut.lines.length, 1);
        assert.deepEqual(
          mathTokens(localizedShortcut.lines.join("\n")),
          mathTokens(getSection(english, "examSpeedShortcut").lines.join("\n")),
        );

        assert.equal(
          localizedTraps.heading,
          locale === "hi-IN" ? "⚠️ सामान्य गलतियाँ" : "⚠️ ਆਮ ਗਲਤੀਆਂ",
        );
        assert.equal(localizedTraps.lines.length, 3);
        assert.ok(localizedTraps.lines.every((line) =>
          locale === "hi-IN" ? /^विकल्प [A-D] \(/.test(line) : /^ਚੋਣ [A-D] \(/.test(line)));
        assert.deepEqual(
          mathTokens(localizedTraps.lines.join("\n")),
        ...
        invalid.push(
          `${entry.qlId}/${locale}/${seedName}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  }
...
assert.equal(exportRows.length, 58);
for (const locale of locales) {
...
const report = {
  releaseId: PNC_002_CP009_LOCALIZATION_CANDIDATE.releaseId,
  packageId: "PNC-002",
...
const directory = resolve(process.cwd(), "dist/quant-v4/pnc-002-localization-cp009");
mkdirSync(directory, { recursive: true });
writeFileSync(
  resolve(directory, "pnc-002-cp009-hi-pa-review.json"),
  `${JSON.stringify(exportRows, null, 2)}\n`,
  "utf8",
);
writeFileSync(
  resolve(directory, "pnc-002-cp009-hi-pa-review.csv"),
  `${csvLines.join("\n")}\n`,
  "utf8",
);
writeFileSync(
  resolve(directory, "pnc-002-cp009-hi-pa-report.json"),
