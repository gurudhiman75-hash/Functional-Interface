...
function fillToSix(selected: NumCp008Wave04Package[], prototypeId: NumCp008Wave04PrototypeId): void {
  const usedSeeds = new Set(selected.map((q) => q.seed));
  for (let seed = 1; seed <= 120 && selected.length < 6; seed += 1) {
    if (usedSeeds.has(seed)) continue;
    selected.push(generateNumCp008Wave04Reviewed(prototypeId, seed));
    usedSeeds.add(seed);
  }
}
...
function selectMultiplicity(): NumCp008Wave04Package[] {
  ...
  for (let seed = 1; seed <= 120; seed += 1) {
    ...
    if (seenAnswers.size === 3 && seenDifficulties.size >= 2 && seenStemModes.size === 3) break;
  }

  fillToSix(selected, "NUM-CP008-PROT-025");
...
function selectTripleSet(): NumCp008Wave04Package[] {
  ...
  for (let seed = 1; seed <= 120; seed += 1) {
    ...
    seenDifficulties.add(q.difficulty);
    seenStemModes.add(stemMode);
    if (seenDifficulties.size >= 2 && seenStemModes.size === 3) break;
  }

  fillToSix(selected, "NUM-CP008-PROT-026");
