...
import { generateNumCp010LocalizedHumanFinal } from "./localization/runtime-human-final.ts";
import type { NumCp010LocalizedLanguage } from "./localization/types.ts";

export const NUM_CP010_QUESTION_STUDIO_PACKAGE_ID = "NUM-002" as const;
export const NUM_CP010_QUESTION_STUDIO_CHECKPOINT_ID = "NUM-CP-010" as const;
export const NUM_CP010_QUESTION_STUDIO_LANGUAGES = ["en", "hi", "pa"] as const;
export const NUM_CP010_QUESTION_STUDIO_QL_IDS = Object.freeze(
  NUM_CP010_PERMANENT_ALLOCATION.map((entry) => entry.qlId as NumCp010PermanentQlId),
);

export type NumCp010QuestionStudioLanguage = typeof NUM_CP010_QUESTION_STUDIO_LANGUAGES[number];
export type NumCp010QuestionStudioDifficulty = "Easy" | "Medium" | "Hard";

export type NumCp010QuestionStudioRequest = Readonly<{
  packageId?: string;
  archetypeId?: string;
  patternId?: string;
  topic?: string;
...
const RELEASE_ID = "NUM-CP-010-QS-MULTILINGUAL-FROZEN-V1" as const;
const DIFFICULTY_SEARCH_WINDOW = 360;
...
function isCp010Ql(value: unknown): value is NumCp010PermanentQlId {
  return NUM_CP010_QUESTION_STUDIO_QL_IDS.includes(String(value ?? "") as NumCp010PermanentQlId);
}

/**
 * CP010 never claims a package-only NUM-002 request. CP008 keeps the historical package
 * fallback; CP009 and CP010 are selected only by explicit checkpoint/pattern/QL identity.
 */
export function isNumCp010QuestionStudioRequest(request: NumCp010QuestionStudioRequest) {
  const patternId = normalizeSelector(request.patternId);
  const checkpointId = String(request.canonicalProblemId ?? request.cpId ?? "");
  return patternId.includes("num cp 010")
    || checkpointId === NUM_CP010_QUESTION_STUDIO_CHECKPOINT_ID
    || isCp010Ql(request.questionLanguageId);
}

function normalizeLanguage(value: unknown): NumCp010QuestionStudioLanguage {
  const language = String(value ?? "en").trim().toLowerCase();
  if (language === "en" || language === "hi" || language === "pa") return language;
  throw new Error(`NUM-CP-010 does not support Question Studio language ${language}.`);
}

function normalizeDifficulty(value: unknown): NumCp010QuestionStudioDifficulty | undefined {
...
function titleDifficulty(value: "EASY" | "MEDIUM" | "HARD"): NumCp010QuestionStudioDifficulty {
...
function generateAuthority(
  ...
  seed: number,
  language: NumCp010QuestionStudioLanguage,
) {
  return language === "en"
    ? generateNumCp010Permanent(qlId, seed)
    : generateNumCp010LocalizedHumanFinal(qlId, seed, language as NumCp010LocalizedLanguage);
}
...
function tryGenerateMatchingDifficulty(
  ...
  initialSeed: number,
  language: NumCp010QuestionStudioLanguage,
  difficulty?: NumCp010QuestionStudioDifficulty,
) {
...
function normalizedPackage(
  ...
  return Object.freeze({
    packageId: NUM_CP010_QUESTION_STUDIO_PACKAGE_ID,
    canonicalProblemId: NUM_CP010_QUESTION_STUDIO_CHECKPOINT_ID,
    questionLanguageId: pkg.permanentQlId,
    explanationId: `${pkg.permanentQlId}-EXP-${language.toUpperCase()}`,
    questionId: `NUM-CP010-${pkg.permanentQlId.slice(-3)}-${language.toUpperCase()}-${identity}`,
    stem: pkg.stem,
...
function toPreview(pkg: ReturnType<typeof normalizedPackage>, index: number, count: number) {
  ...
  return Object.freeze({
    ...
    patternId: NUM_CP010_QUESTION_STUDIO_PACKAGE_ID,
    section: "Quant",
    topic: "Arithmetic",
    subtopic: "Number System",
    generationBackend: "quant-v4",
    debugSource: "quant-v4-num-cp010-frozen-multilingual-runtime",
    packageSource: "quant-v4-num-cp010-frozen-multilingual-runtime",
    packageId: NUM_CP010_QUESTION_STUDIO_PACKAGE_ID,
    canonicalProblemId: NUM_CP010_QUESTION_STUDIO_CHECKPOINT_ID,
...
export function listNumCp010QuestionStudioPackages() {
  return [Object.freeze({
    id: NUM_CP010_QUESTION_STUDIO_PACKAGE_ID,
    packageId: NUM_CP010_QUESTION_STUDIO_PACKAGE_ID,
    type: "quant-v4",
    section: "Quant",
    domain: "quant",
    ...
    topic: "Arithmetic",
    subtopic: "Number System",
    name: "NUM-002 Number System — Digit Structure & Number Reconstruction",
    label: "Number System — Digit Structure & Number Reconstruction",
    generationDomain: "quant-v4",
    cpIds: Object.freeze([NUM_CP010_QUESTION_STUDIO_CHECKPOINT_ID]),
    canonicalProblems: Object.freeze([Object.freeze({
      id: NUM_CP010_QUESTION_STUDIO_CHECKPOINT_ID,
      label: "Digit structure, place value and number reconstruction",
    })]),
    permanentQlCount: NUM_CP010_QUESTION_STUDIO_QL_IDS.length,
    permanentQlIds: NUM_CP010_QUESTION_STUDIO_QL_IDS,
    supportedDifficulties: Object.freeze(["Easy", "Medium", "Hard"]),
    supportedLanguages: NUM_CP010_QUESTION_STUDIO_LANGUAGES,
    enabled: true,
...
export async function generateNumCp010QuestionStudioBatch(request: NumCp010QuestionStudioRequest = {}) {
  ...
  if (explicitQl && !isCp010Ql(explicitQl)) throw new Error(`${explicitQl} is not owned by NUM-CP-010.`);

  const explicitCp = String(request.canonicalProblemId ?? request.cpId ?? "") || undefined;
  if (explicitCp && explicitCp !== NUM_CP010_QUESTION_STUDIO_CHECKPOINT_ID) {
    throw new Error(`NUM-CP-010 integration cannot serve canonical problem ${explicitCp}.`);
  }

  const batchSeed = request.seed?.trim()
    || `question-studio:NUM-002:NUM-CP-010:${language}:${Date.now()}:${Math.random().toString(36).slice(2)}`;
  const qlOffset = stablePositiveSeed(`${batchSeed}:ql-offset`) % NUM_CP010_QUESTION_STUDIO_QL_IDS.length;
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const qlCandidates = explicitQl
      ? [explicitQl as NumCp010PermanentQlId]
      : Array.from({ length: NUM_CP010_QUESTION_STUDIO_QL_IDS.length }, (_, scan) =>
          NUM_CP010_QUESTION_STUDIO_QL_IDS[(qlOffset + index + scan) % NUM_CP010_QUESTION_STUDIO_QL_IDS.length]!,
        );
    ...
    if (!selectedFrozen) {
      ...
      throw new Error(`No NUM-CP-010 frozen QL can satisfy requested ${String(difficulty ?? "difficulty")} filter.`);
    }
