export function buildQl137Editorial(frozen: any, seed: number) {
  ...
  if (difficulty === "EASY") {
    ...
    answer = "n must be even";
    steps = [`For odd/even behaviour, ${math("3n^{2}+4n+1")} has the same parity as ${math("n^{2}+1")}.`, `For this to be odd, ${math("n^{2}")} must be even, so ${math("n")} is even.`];
  } else {
    condition = `${math("5n^{2}+2n+1")} is even`;
    answer = "n must be odd";
    steps = [`For odd/even behaviour, ${math("5n^{2}+2n+1")} has the same parity as ${math("n^{2}+1")}.`, `For this to be even, ${math("n^{2}")} must be odd, so ${math("n")} is odd.`];
  }
  ...
  return { stem: `What must be true about the integer ${math("n")} if ${condition}?`, options: Object.freeze(options), correctIndex, answer, concept: "Track only whether each part is odd or even, then work backward.", steps };
}
