...
} from "../wave04/direct-edge-hardening.ts";
...
for (const allocation of NUM_CP007_PERMANENT_ALLOCATION) {
  ...
  for (let seed = 1; seed <= seedsPerQl; seed += 1) {
    const first = runNumCp007PermanentPipeline({ questionLanguageId: qlId, seed });
    const replay = runNumCp007PermanentPipeline({ questionLanguageId: qlId, seed });
...
for (let seed = 1; seed <= 120; seed += 1) {
  ...
  quotientZeroEdgeChecks += 1;
  assert(verifyQuotientZeroEdgeCase(edge), `quotient-zero edge ${seed}: invalid division state`);
}

let unsupportedLanguageRejected = false;
try {
  runNumCp007PermanentPipeline({ language: "hi" as never });
} catch {
