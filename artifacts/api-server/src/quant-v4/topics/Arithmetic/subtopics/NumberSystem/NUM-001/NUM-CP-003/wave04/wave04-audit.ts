...
const forbidden = [/NUM-CP/iu, /W4-PROT/iu, /undefined/iu, /NaN/iu, /Infinity/iu, /\{\{/u, /\}\}/u, /[\u0009\u000d]/u];
...
for (const id of NUM_CP003_WAVE04_IDS) {
  for (let index = 0; index < 60; index += 1) {
    const question = generateNumCp003Wave04(id, `audit-${index}`);
    const learnerText = [
      question.stem,
      ...question.options,
    ...
    ok(question.publiclyPublishable === false && question.questionStudioDiscoverable === false, `${id}: publication leak`);
...
ok(misconceptions.size >= 20, `misconceptions ${misconceptions.size}`);
