...
export const NUM_CP012_QUESTION_STUDIO_LANGUAGES = ["en", "hi", "pa"] as const;
export const NUM_CP012_QUESTION_STUDIO_QL_IDS = Object.freeze(
  NUM_CP012_PERMANENT_ALLOCATION.map((entry) => entry.qlId as NumCp012PermanentQlId),
);

export type NumCp012QuestionStudioLanguage = typeof NUM_CP012_QUESTION_STUDIO_LANGUAGES[number];
export type NumCp012QuestionStudioDifficulty = "Easy" | "Medium" | "Hard";

export type NumCp012QuestionStudioRequest = Readonly<{
  packageId?: string;
  archetypeId?: string;
  patternId?: string;
  topic?: string;
...
const RELEASE_ID = "NUM-CP-012-QS-MULTILINGUAL-FROZEN-V1" as const;
...
function isCp012Ql(value: unknown): value is NumCp012PermanentQlId {
  return NUM_CP012_QUESTION_STUDIO_QL_IDS.includes(String(value ?? "") as NumCp012PermanentQlId);
}

/** CP012 claims only explicit checkpoint, pattern or permanent-QL requests. */
export function isNumCp012QuestionStudioRequest(request: NumCp012QuestionStudioRequest) {
  const patternId = normalizeSelector(request.patternId);
  const checkpointId = String(request.canonicalProblemId ?? request.cpId ?? "");
  return patternId.includes("num cp 012")
    || checkpointId === NUM_CP012_QUESTION_STUDIO_CHECKPOINT_ID
    || isCp012Ql(request.questionLanguageId);
}

function normalizeLanguage(value: unknown): NumCp012QuestionStudioLanguage {
  const language = String(value ?? "en").trim().toLowerCase();
  if (language === "en" || language === "hi" || language === "pa") return language;
  throw new Error(`NUM-CP-012 does not support Question Studio language ${language}.`);
}

function normalizeDifficulty(value: unknown): NumCp012QuestionStudioDifficulty | undefined {
...
function titleDifficulty(value: "EASY" | "MEDIUM" | "HARD"): NumCp012QuestionStudioDifficulty {
...
function tryGenerateMatchingDifficulty(
  ...
  initialSeed: number,
  language: NumCp012QuestionStudioLanguage,
  difficulty?: NumCp012QuestionStudioDifficulty,
) {
...
function normalizedPackage(
  ...
  return Object.freeze({
    packageId: NUM_CP012_QUESTION_STUDIO_PACKAGE_ID,
    canonicalProblemId: NUM_CP012_QUESTION_STUDIO_CHECKPOINT_ID,
    questionLanguageId: pkg.permanentQlId,
    explanationId: `${pkg.permanentQlId}-EXP-${language.toUpperCase()}`,
    questionId: `NUM-CP012-${pkg.permanentQlId.slice(-3)}-${language.toUpperCase()}-${identity}`,
    stem: pkg.stem,
...
function toPreview(pkg: ReturnType<typeof normalizedPackage>, index: number, count: number) {
  ...
  return Object.freeze({
    ...
    debugSource: "quant-v4-num-cp012-frozen-multilingual-runtime",
    packageSource: "quant-v4-num-cp012-frozen-multilingual-runtime",
    packageId: NUM_CP012_QUESTION_STUDIO_PACKAGE_ID,
    canonicalProblemId: NUM_CP012_QUESTION_STUDIO_CHECKPOINT_ID,
...
export function listNumCp012QuestionStudioPackages() {
  return [Object.freeze({
    id: NUM_CP012_QUESTION_STUDIO_PACKAGE_ID,
    packageId: NUM_CP012_QUESTION_STUDIO_PACKAGE_ID,
    type: "quant-v4",
    section: "Quant",
    domain: "quant",
    ...
    topic: "Arithmetic",
    subtopic: "Number System",
    name: "NUM-002 Number System — Perfect Squares, Cubes & General Perfect Powers",
    label: "Number System — Perfect Squares, Cubes & General Perfect Powers",
    generationDomain: "quant-v4",
    cpIds: Object.freeze([NUM_CP012_QUESTION_STUDIO_CHECKPOINT_ID]),
    canonicalProblems: Object.freeze([Object.freeze({
      id: NUM_CP012_QUESTION_STUDIO_CHECKPOINT_ID,
      label: "Perfect squares, cubes and general perfect powers",
    })]),
    permanentQlCount: NUM_CP012_QUESTION_STUDIO_QL_IDS.length,
    permanentQlIds: NUM_CP012_QUESTION_STUDIO_QL_IDS,
    supportedDifficulties: Object.freeze(["Easy", "Medium", "Hard"]),
    supportedLanguages: NUM_CP012_QUESTION_STUDIO_LANGUAGES,
    enabled: true,
...
export async function generateNumCp012QuestionStudioBatch(request: NumCp012QuestionStudioRequest = {}) {
  ...
  if (explicitQl && !isCp012Ql(explicitQl)) throw new Error(`${explicitQl} is not owned by NUM-CP-012.`);

  const explicitCp = String(request.canonicalProblemId ?? request.cpId ?? "") || undefined;
  if (explicitCp && explicitCp !== NUM_CP012_QUESTION_STUDIO_CHECKPOINT_ID) {
    throw new Error(`NUM-CP-012 integration cannot serve canonical problem ${explicitCp}.`);
  }

  const batchSeed = request.seed?.trim()
    || `question-studio:NUM-002:NUM-CP-012:${language}:${Date.now()}:${Math.random().toString(36).slice(2)}`;
  const qlOffset = stablePositiveSeed(`${batchSeed}:ql-offset`) % NUM_CP012_QUESTION_STUDIO_QL_IDS.length;
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const qlCandidates = explicitQl
      ? [explicitQl as NumCp012PermanentQlId]
      : Array.from({ length: NUM_CP012_QUESTION_STUDIO_QL_IDS.length }, (_, scan) =>
          NUM_CP012_QUESTION_STUDIO_QL_IDS[(qlOffset + index + scan) % NUM_CP012_QUESTION_STUDIO_QL_IDS.length]!,
        );
    ...
    if (!selectedFrozen) {
      ...
      throw new Error(`No NUM-CP-012 frozen QL can satisfy requested ${String(difficulty ?? "difficulty")} filter.`);
    }
  ...
  return Object.freeze({
    generationContext: Object.freeze({
      generationDomain: "quant-v4",
      packageId: NUM_CP012_QUESTION_STUDIO_PACKAGE_ID,
      chapterId: NUM_CP012_QUESTION_STUDIO_CHECKPOINT_ID,
      canonicalProblemId: NUM_CP012_QUESTION_STUDIO_CHECKPOINT_ID,
      seed: batchSeed,
      timestamp: Date.now(),
      runtimeMode: "QUESTION_STUDIO_ACTIVE",
