...
const forbidden = [/NUM-CP/iu, /W2-PROT/iu, /undefined/iu, /NaN/iu, /Infinity/iu, /\{\{/u, /\}\}/u, /[\u0009\u000d]/u];
...
for (const id of NUM_CP003_WAVE02_IDS) {
  for (let index = 0; index < 60; index += 1) {
    const question = generateNumCp003Wave02(id, `audit-${index}`);
    const learnerText = [
      question.stem,
      ...question.options,
    ...
    ok(question.optionAudit.length === 4, `${id}: option audit count`);
    ok(question.optionAudit.every((row) => row.diagnostic.length >= 16), `${id}: option diagnostic`);
    ok(question.optionAudit.filter((row) => row.misconceptionId === "CORRECT").length === 1, `${id}: correct diagnostic count`);
    ...
    ok(question.permanentQlId === null, `${id}: permanent QL leak`);
    ok(question.questionBankStatus === "NOT_STORED", `${id}: Question Bank leak`);
    ok(question.testEligibility === "INELIGIBLE", `${id}: test eligibility leak`);
    ok(question.publiclyPublishable === false, `${id}: publication leak`);
    ok(question.questionStudioDiscoverable === false, `${id}: Question Studio leak`);
...
ok(hiddenKinds.size === 9, `hidden kinds ${[...hiddenKinds]}`);
ok(misconceptions.size >= 16, `misconceptions ${misconceptions.size}`);
