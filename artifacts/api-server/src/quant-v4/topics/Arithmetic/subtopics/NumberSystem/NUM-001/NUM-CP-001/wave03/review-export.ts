...
const rows = NUM_CP001_WAVE03_PROTOTYPE_IDS.flatMap((prototypeId) =>
  [1, 2, 3, 4].map((seed) => {
...
fs.writeFileSync(path.join(outDir, "num-001-cp001-wave03-review.json"), JSON.stringify(rows, null, 2));
...
fs.writeFileSync(path.join(outDir, "num-001-cp001-wave03-review.md"), md);
console.log(JSON.stringify({ status: "PASS_NUM_CP001_WAVE03_REVIEW_EXPORT", questions: rows.length }, null, 2));
