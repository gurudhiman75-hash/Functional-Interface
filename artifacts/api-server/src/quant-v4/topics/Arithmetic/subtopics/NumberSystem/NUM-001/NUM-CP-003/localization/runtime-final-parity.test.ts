...
const HINDI_REJECTED = /(?:पूर्ण हैं|उसका \\?\([^)]*\\?\) और \\?\([^)]*\\?\), दोनों से विभाज्य होना जरूरी है|वैकल्पिक स्थानों के अंकों|डेटा-पर्याप्तता|बंद अंतराल)/u;
const PUNJABI_REJECTED = /(?:ਪੂਰੇ ਹਨ|ਉਸਦਾ \\?\([^)]*\\?\) ਅਤੇ \\?\([^)]*\\?\), ਦੋਵਾਂ ਨਾਲ ਭਾਗਯੋਗ ਹੋਣਾ ਜ਼ਰੂਰੀ ਹੈ|ਇੱਕ ਛੱਡ ਕੇ ਅੰਕਾਂ ਦੇ|ਡਾਟਾ-ਪਰਯਾਪਤਾ)/u;

function learnerText(q: ReturnType<typeof runNumCp003LocalizedFinalForQl>): string {
...
function requiresRuleFirst(kind: string): boolean {
  return [
    "DIRECT_DIVISIBILITY",
...
const reviewRows: Array<ReturnType<typeof runNumCp003LocalizedFinalForQl>> = [];
...
for (const qlId of NUM_CP003_PERMANENT_QL_IDS) {
  for (let seed = 1; seed <= 120; seed += 1) {
    ...
    for (const language of LANGUAGES) {
      const q = runNumCp003LocalizedFinalForQl(qlId, seedText, language);
      const replay = runNumCp003LocalizedFinalForQl(qlId, seedText, language);
      const label = `${qlId}/${seed}/${language}`;
      audited += 1;

      assert(json(q) === json(replay), `${label}: final localized replay changed`);
      replayChecks += 1;
      ...
      assert(q.fingerprint === canonical.fingerprint, `${label}: mathematical fingerprint changed`);
      assert(json(q.hiddenState) === json(canonical.hiddenState), `${label}: mathematical state changed`);
      ...
      assert(script.test(q.stem), `${label}: final stem lacks target script`);
      assert(script.test(q.explanation.concept), `${label}: final concept lacks target script`);
      assert(q.explanation.solution.every((line) => script.test(line)), `${label}: final solution line lacks target script`);
      ...
      assert(!(language === "hi" ? HINDI_REJECTED : PUNJABI_REJECTED).test(learner), `${label}: rejected literal/awkward wording leaked`);
      linguisticChecks += 1;

      assert(q.explanation.solution.length >= 2 && q.explanation.solution.length <= 4, `${label}: final solution must contain 2-4 lines`);
      if (requiresRuleFirst(q.hiddenState.kind)) {
...
assert(audited === 17 * 120 * 2, `Unexpected final audit count ${audited}`);
assert(reviewRows.length === 136, `Unexpected final review count ${reviewRows.length}`);
...
writeFileSync(reviewMarkdown, [
  "# NUM-CP-003 Hindi/Punjabi Final Rule-First Localization Review",
  "",
  "Final learner-facing editorial surface. English mathematical authority remains frozen and all delivery gates remain closed.",
  "",
...
const audit = {
  status: "PASS_NUM_CP003_HI_PA_FINAL_RULE_FIRST_EDITORIAL_AUDIT",
  qlCount: 17,
  audited,
  replayChecks,
