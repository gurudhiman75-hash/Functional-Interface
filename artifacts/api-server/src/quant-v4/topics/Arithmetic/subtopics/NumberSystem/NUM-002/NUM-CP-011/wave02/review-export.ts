...
const titles: Readonly<Record<string, string>> = Object.freeze({
  ...
  "NUM-CP011-PROT-008": "Exact valuation / exact decimal-zero preimage",
  "NUM-CP011-PROT-009": "Least factorial reaching a general-base zero threshold",
  "NUM-CP011-PROT-010": "Least factorial divisible by a composite integer",
  "NUM-CP011-PROT-011": "Recover a missing product exponent from valuation",
});

const seeds = [6, 17, 28] as const;
const lines: string[] = [
  "# ExamTree — NUM-CP-011 Wave 02 Review",
  "",
  "**Checkpoint:** Factorials, Prime Valuations and Trailing Zeroes",
  "",
  "**Status:** inverse discovery only; no permanent QL or Question Studio exposure",
  "",
  `**Temporary prototypes:** ${NUM_CP011_WAVE02_PROTOTYPE_IDS.length}`,
  "",
  `**Review questions:** ${NUM_CP011_WAVE02_PROTOTYPE_IDS.length * seeds.length}`,
  "",
  "---",
  "",
];
...
const output = resolve(process.cwd(), "artifacts/api-server/dist/quant-v4/num-cp011-wave02-review.md");
mkdirSync(dirname(output), { recursive: true });
writeFileSync(output, `${lines.join("\n")}\n`, "utf8");

console.log(JSON.stringify({
  status: "PASS_NUM_CP011_WAVE02_REVIEW_EXPORT",
  prototypes: NUM_CP011_WAVE02_PROTOTYPE_IDS.length,
  questions: NUM_CP011_WAVE02_PROTOTYPE_IDS.length * seeds.length,
  output,
}, null, 2));
