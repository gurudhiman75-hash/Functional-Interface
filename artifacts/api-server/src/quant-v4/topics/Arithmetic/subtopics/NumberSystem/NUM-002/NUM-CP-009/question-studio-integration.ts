...
import { generateNumCp009LocalizedHumanFinal } from "./localization/runtime-human-final.ts";
import type { NumCp009LocalizedLanguage } from "./localization/types.ts";

export const NUM_CP009_QUESTION_STUDIO_PACKAGE_ID = "NUM-002" as const;
export const NUM_CP009_QUESTION_STUDIO_CHECKPOINT_ID = "NUM-CP-009" as const;
export const NUM_CP009_QUESTION_STUDIO_LANGUAGES = ["en", "hi", "pa"] as const;
export const NUM_CP009_QUESTION_STUDIO_QL_IDS = Object.freeze(
  NUM_CP009_PERMANENT_ALLOCATION.map((entry) => entry.qlId as NumCp009PermanentQlId),
);

export type NumCp009QuestionStudioLanguage = typeof NUM_CP009_QUESTION_STUDIO_LANGUAGES[number];
export type NumCp009QuestionStudioDifficulty = "Easy" | "Medium" | "Hard";

export type NumCp009QuestionStudioRequest = Readonly<{
  packageId?: string;
  archetypeId?: string;
  patternId?: string;
  topic?: string;
...
const RELEASE_ID = "NUM-CP-009-QS-MULTILINGUAL-FROZEN-V1" as const;
const DIFFICULTY_SEARCH_WINDOW = 320;
...
function isCp009Ql(value: unknown): value is NumCp009PermanentQlId {
  return NUM_CP009_QUESTION_STUDIO_QL_IDS.includes(String(value ?? "") as NumCp009PermanentQlId);
}

/**
 * CP009 intentionally does not claim package-only NUM-002 requests. CP008 already owns
 * that backwards-compatible fallback in the shared facade. CP009 is selected by its
 * explicit checkpoint, pattern, or permanent QL identity.
 */
export function isNumCp009QuestionStudioRequest(request: NumCp009QuestionStudioRequest) {
  const patternId = normalizeSelector(request.patternId);
  const checkpointId = String(request.canonicalProblemId ?? request.cpId ?? "");
  return patternId.includes("num cp 009")
    || checkpointId === NUM_CP009_QUESTION_STUDIO_CHECKPOINT_ID
    || isCp009Ql(request.questionLanguageId);
}

function normalizeLanguage(value: unknown): NumCp009QuestionStudioLanguage {
  const language = String(value ?? "en").trim().toLowerCase();
  if (language === "en" || language === "hi" || language === "pa") return language;
  throw new Error(`NUM-CP-009 does not support Question Studio language ${language}.`);
}

function normalizeDifficulty(value: unknown): NumCp009QuestionStudioDifficulty | undefined {
...
function titleDifficulty(value: "EASY" | "MEDIUM" | "HARD"): NumCp009QuestionStudioDifficulty {
...
function generateAuthority(
  ...
  seed: number,
  language: NumCp009QuestionStudioLanguage,
) {
  return language === "en"
    ? generateNumCp009Permanent(qlId, seed)
    : generateNumCp009LocalizedHumanFinal(qlId, seed, language as NumCp009LocalizedLanguage);
}
...
function tryGenerateMatchingDifficulty(
  ...
  initialSeed: number,
  language: NumCp009QuestionStudioLanguage,
  difficulty?: NumCp009QuestionStudioDifficulty,
) {
...
function normalizedPackage(
  ...
  return Object.freeze({
    packageId: NUM_CP009_QUESTION_STUDIO_PACKAGE_ID,
    canonicalProblemId: NUM_CP009_QUESTION_STUDIO_CHECKPOINT_ID,
    questionLanguageId: pkg.permanentQlId,
    explanationId: `${pkg.permanentQlId}-EXP-${language.toUpperCase()}`,
    questionId: `NUM-CP009-${pkg.permanentQlId.slice(-3)}-${language.toUpperCase()}-${identity}`,
    stem: pkg.stem,
...
function toPreview(pkg: ReturnType<typeof normalizedPackage>, index: number, count: number) {
  ...
  return Object.freeze({
    ...
    patternId: NUM_CP009_QUESTION_STUDIO_PACKAGE_ID,
    section: "Quant",
    topic: "Arithmetic",
    subtopic: "Number System",
    generationBackend: "quant-v4",
    debugSource: "quant-v4-num-cp009-frozen-multilingual-runtime",
    packageSource: "quant-v4-num-cp009-frozen-multilingual-runtime",
    packageId: NUM_CP009_QUESTION_STUDIO_PACKAGE_ID,
    canonicalProblemId: NUM_CP009_QUESTION_STUDIO_CHECKPOINT_ID,
...
export function listNumCp009QuestionStudioPackages() {
  return [Object.freeze({
    id: NUM_CP009_QUESTION_STUDIO_PACKAGE_ID,
    packageId: NUM_CP009_QUESTION_STUDIO_PACKAGE_ID,
    type: "quant-v4",
    section: "Quant",
    domain: "quant",
    ...
    topic: "Arithmetic",
    subtopic: "Number System",
    name: "NUM-002 Number System — Cyclicity & Terminal Digits",
    label: "Number System — Cyclicity & Terminal Digits",
    generationDomain: "quant-v4",
    cpIds: Object.freeze([NUM_CP009_QUESTION_STUDIO_CHECKPOINT_ID]),
    canonicalProblems: Object.freeze([Object.freeze({
      id: NUM_CP009_QUESTION_STUDIO_CHECKPOINT_ID,
      label: "Cyclicity, terminal digits and power patterns",
    })]),
    permanentQlCount: NUM_CP009_QUESTION_STUDIO_QL_IDS.length,
    permanentQlIds: NUM_CP009_QUESTION_STUDIO_QL_IDS,
    supportedDifficulties: Object.freeze(["Easy", "Medium", "Hard"]),
    supportedLanguages: NUM_CP009_QUESTION_STUDIO_LANGUAGES,
    enabled: true,
...
export async function generateNumCp009QuestionStudioBatch(request: NumCp009QuestionStudioRequest = {}) {
  ...
  if (explicitQl && !isCp009Ql(explicitQl)) throw new Error(`${explicitQl} is not owned by NUM-CP-009.`);

  const explicitCp = String(request.canonicalProblemId ?? request.cpId ?? "") || undefined;
  if (explicitCp && explicitCp !== NUM_CP009_QUESTION_STUDIO_CHECKPOINT_ID) {
    throw new Error(`NUM-CP-009 integration cannot serve canonical problem ${explicitCp}.`);
  }

  const batchSeed = request.seed?.trim()
    || `question-studio:NUM-002:NUM-CP-009:${language}:${Date.now()}:${Math.random().toString(36).slice(2)}`;
  const qlOffset = stablePositiveSeed(`${batchSeed}:ql-offset`) % NUM_CP009_QUESTION_STUDIO_QL_IDS.length;
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const qlCandidates = explicitQl
      ? [explicitQl as NumCp009PermanentQlId]
      : Array.from({ length: NUM_CP009_QUESTION_STUDIO_QL_IDS.length }, (_, scan) =>
          NUM_CP009_QUESTION_STUDIO_QL_IDS[(qlOffset + index + scan) % NUM_CP009_QUESTION_STUDIO_QL_IDS.length]!,
        );
    ...
    if (!selectedFrozen) {
      ...
      throw new Error(`No NUM-CP-009 frozen QL can satisfy requested ${String(difficulty ?? "difficulty")} filter.`);
    }
