...
let inRangeDistractorChecks = 0;
let nearMissDistractorChecks = 0;
...
for (const prototypeId of NUM_CP008_WAVE04_PROTOTYPE_IDS) {
  for (let seed = 1; seed <= 120; seed += 1) {
    ...
    assert(reviewed.options[reviewed.correctIndex]?.value === source.options[source.correctIndex]?.value, `${label}: correct-option value drift`);
    if (prototypeId === "NUM-CP008-PROT-025") {
      assert(JSON.stringify(reviewed.options) === JSON.stringify(source.options), `${label}: P025 option drift`);
    }
    parityChecks += 1;
    ...
    if (prototypeId === "NUM-CP008-PROT-025") {
      ...
      if (merged) {
        assert(/Write \$x=/.test(working) && /\^\{-1\}/.test(working) && /Taking the least non-negative/.test(working) && /final class/.test(working), `${label}: compatible CRT inverse working hidden`);
      } else {
      ...
      assert(/Write \$x=/.test(working) && /\^\{-1\}/.test(working) && /Taking the least non-negative/.test(working) && /all the required integers/.test(working), `${label}: complete triple-set CRT inverse work hidden`);

      const state = reviewed.hiddenState as Record<string, unknown>;
      const lower = Number(state.lower);
      const upper = Number(state.upper);
      const constraints = state.constraints as Array<{ residue: number; modulus: number }>;
      const nearMissOptions = reviewed.options.filter((option) => option.misconceptionId === "SATISFIES_ONLY_TWO_CONGRUENCES");
      assert(nearMissOptions.length === 1, `${label}: expected one two-of-three near-miss distractor`);
      for (const option of reviewed.options) {
        const values = (option.value.match(/-?\d+/g) ?? []).map(Number);
        assert(values.length > 0, `${label}: set option has no values`);
        for (const value of values) {
          assert(value >= lower && value <= upper, `${label}: option value ${value} outside [${lower},${upper}]`);
          inRangeDistractorChecks += 1;
        }
      }
      const nearMissValues = (nearMissOptions[0]!.value.match(/-?\d+/g) ?? []).map(Number);
      const sourceSolutions = state.canonicalSolutions as number[];
      const altered = nearMissValues.filter((value) => !sourceSolutions.includes(value));
      assert(altered.length === 1, `${label}: near-miss option must replace exactly one valid solution`);
      const score = constraints.filter((constraint) => mod(altered[0]!, constraint.modulus) === mod(constraint.residue, constraint.modulus)).length;
      assert(score === 2, `${label}: near-miss candidate must satisfy exactly two congruences, got ${score}`);
      nearMissDistractorChecks += 1;
    }
...
assert(nearMissDistractorChecks === 120, `Expected 120 near-miss distractor checks, got ${nearMissDistractorChecks}`);
for (const prototypeId of NUM_CP008_WAVE04_PROTOTYPE_IDS) {
...
console.log(JSON.stringify({
  ...
  answerBindingChecks,
  workedReviewChecks,
  inRangeDistractorChecks,
  nearMissDistractorChecks,
