...
assert.equal(packages.length, NUM_CP004_WAVE01_PROTOTYPE_IDS.length * SEEDS_PER_PROTOTYPE);
...
for (const pkg of packages) {
  ...
  assert.ok(!fingerprints.has(pkg.mathematicalFingerprint), `Duplicate mathematical fingerprint: ${pkg.mathematicalFingerprint}`);
  ...
  const difficultySet = difficulties.get(pkg.temporaryPrototypeId) ?? new Set<NumCp004Difficulty>();
  difficultySet.add(pkg.difficulty);
  difficulties.set(pkg.temporaryPrototypeId, difficultySet);

  semantics.add(pkg.answerSemantic);
  if (pkg.temporaryPrototypeId === "NUM-CP004-PROT-008") p8Classes.add(pkg.canonicalAnswer);
...
assert.ok(semantics.has("PRIME_CLASS"));
assert.ok(semantics.has("PRIME_SET"));
assert.ok(semantics.has("COUNT"));
assert.ok(semantics.has("FACTORISATION"));
assert.ok(semantics.has("PRIME_FACTOR"));
assert.ok(semantics.has("PRIME"));
assert.ok(semantics.has("PRIME_EXPONENT"));
assert.ok(semantics.has("PAIR"));
assert.ok(semantics.has("COPRIME_CLASS"));
...
console.log(JSON.stringify({
  status: "PASS_NUM_CP004_WAVE01_RUNTIME",
