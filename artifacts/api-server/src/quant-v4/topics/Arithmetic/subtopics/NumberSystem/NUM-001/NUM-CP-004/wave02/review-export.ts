...
const rows = NUM_CP004_WAVE02_PROTOTYPE_IDS.flatMap((prototypeId) =>
  [1, 2, 3].map((seed) => generateNumCp004Wave02Package(prototypeId, seed)),
);

const jsonPath = resolve(outputDirectory, "num-001-cp004-wave02-review.json");
const markdownPath = resolve(outputDirectory, "num-001-cp004-wave02-review.md");

writeFileSync(jsonPath, `${JSON.stringify({
  status: "NUM_CP004_WAVE02_REVIEW_CANDIDATES",
  permanentQlCount: 0,
...
const markdown = [
  "# NUM-CP-004 Wave 02 — English Review Pack",
  "",
...
console.log(JSON.stringify({
  status: "PASS_NUM_CP004_WAVE02_REVIEW_EXPORT",
  jsonPath,
  markdownPath,
  reviewQuestionCount: rows.length,
  temporaryPrototypeCount: NUM_CP004_WAVE02_PROTOTYPE_IDS.length,
