...
for (const qlId of NUM_CP012_PERMANENT_QL_IDS) sourceCoverage.set(qlId, new Set());
...
for (const qlId of NUM_CP012_PERMANENT_QL_IDS) {
  ...
  for (const language of languages) {
    for (let seed = 1; seed <= 50; seed += 1) {
      ...
      const localized = generateNumCp012Localized(qlId, seed, language);
      const replay = generateNumCp012Localized(qlId, seed, language);
...
console.log(JSON.stringify({
  status: "PASS_NUM_CP012_HI_PA_PARITY",
