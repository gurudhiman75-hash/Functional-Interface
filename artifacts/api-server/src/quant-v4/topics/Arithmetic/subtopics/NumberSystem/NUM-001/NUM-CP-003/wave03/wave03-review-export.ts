...
const records = NUM_CP003_WAVE03_IDS.flatMap((prototypeId) =>
  ["review-0", "review-1", "review-2"].map((seed) => generateNumCp003Wave03(prototypeId, seed)),
);

const jsonPath = resolve(outputDirectory, "num-001-cp003-gap-wave-03-review.json");
const markdownPath = resolve(outputDirectory, "num-001-cp003-gap-wave-03-review.md");
...
const markdown = [
  "# NUM-CP-003 Gap Wave 03 — English Review Pack",
  "",
  `Questions: ${records.length}`,
  `Temporary prototypes: ${NUM_CP003_WAVE03_IDS.length}`,
  "Permanent QLs: 0",
  "",
