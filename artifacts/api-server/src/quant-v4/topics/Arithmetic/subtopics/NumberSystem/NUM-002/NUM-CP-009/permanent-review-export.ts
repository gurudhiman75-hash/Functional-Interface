...
const outputDir = path.resolve("dist/quant-v4/num-cp009-permanent-english-review");
fs.mkdirSync(outputDir, { recursive: true });

const reviewSeeds = [7, 41, 113] as const;
const samples = NUM_CP009_PERMANENT_ALLOCATION.flatMap((allocation) =>
  reviewSeeds.map((seed) => {
    const item = generateNumCp009Permanent(allocation.qlId as NumCp009PermanentQlId, seed);
...
const audit = {
  status: "PASS_NUM_CP009_PERMANENT_ENGLISH_REVIEW_EXPORT",
  authorities: NUM_CP009_PERMANENT_ALLOCATION.length,
...
const markdown = [
  "# NUM-CP-009 Permanent English Review",
  "",
  "This artifact samples every approved permanent authority after allocation. It is learner-facing review evidence only; downstream lifecycle gates remain closed.",
  "",
  ...sections,
  "",
].join("\n");
