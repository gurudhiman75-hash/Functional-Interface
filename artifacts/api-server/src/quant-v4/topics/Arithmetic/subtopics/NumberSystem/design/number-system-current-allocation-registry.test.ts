...
assert(NUMBER_SYSTEM_COMPLETED_CHECKPOINT_ALLOCATIONS.length === 2, "completed checkpoint count");
assert(NUM_CP003_PERMANENT_QL_IDS.length === 17, "CP-003 QL count");
assert(NUM_CP004_PERMANENT_QL_IDS.length === 28, "CP-004 QL count");

const allQlIds = [...NUM_CP003_PERMANENT_QL_IDS, ...NUM_CP004_PERMANENT_QL_IDS];
assert(allQlIds.length === 45, "chapter permanent QL count");
...
assert(NUMBER_SYSTEM_CURRENT_PERMANENT_QL_RANGE.last === 45, "chapter last QL");
assert(NUMBER_SYSTEM_CURRENT_PERMANENT_QL_RANGE.count === 45, "chapter QL count");
assert(NUMBER_SYSTEM_NEXT_PERMANENT_QL_NUMBER_CURRENT === 46, "next chapter QL identity");
