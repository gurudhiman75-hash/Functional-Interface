...
import { generateNumCp005Wave04Package } from "./runtime-proven";
import { NUM_CP005_WAVE04_PROTOTYPE_IDS } from "./types";

const samplesPerPrototype = 3;
const records = NUM_CP005_WAVE04_PROTOTYPE_IDS.flatMap((prototypeId, prototypeIndex) =>
  Array.from({ length: samplesPerPrototype }, (_unused, sampleIndex) =>
    generateNumCp005Wave04Package(prototypeId, 1 + sampleIndex + prototypeIndex * 7)),
);

const outputDirectory = resolve(process.cwd(), "dist/quant-v4");
mkdirSync(outputDirectory, { recursive: true });
const jsonPath = resolve(outputDirectory, "num-001-cp005-wave04-review.json");
const markdownPath = resolve(outputDirectory, "num-001-cp005-wave04-review.md");

writeFileSync(jsonPath, `${JSON.stringify({
  status: "NUM_CP005_WAVE04_REVIEW_READY",
...
const markdown = [
  "# NUM-CP-005 Wave 04 — Data Sufficiency and Solution Topology Review",
  "",
  "**Status:** executable discovery only; permanent QLs remain zero.",
  "",
  `**Temporary prototypes:** ${NUM_CP005_WAVE04_PROTOTYPE_IDS.length}`,
  "",
...
console.log(JSON.stringify({
  status: "PASS_NUM_CP005_WAVE04_REVIEW_EXPORT",
  temporaryPrototypeCount: NUM_CP005_WAVE04_PROTOTYPE_IDS.length,
  reviewQuestionCount: records.length,
  permanentQlCount: 0,
  jsonPath,
