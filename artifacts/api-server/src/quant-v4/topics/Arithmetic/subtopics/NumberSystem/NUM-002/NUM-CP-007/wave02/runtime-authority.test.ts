...
for (const prototypeId of NUM_CP007_WAVE02_PROTOTYPE_IDS) {
  ...
  for (let seed = 1; seed <= 120; seed++) {
    ...
    const replay = generateNumCp007Wave02Package(prototypeId, seed);
    generatedPackages++;

    assert.deepEqual(replay, pkg, `${prototypeId} seed ${seed} is not deterministic.`);
    deterministicReplayChecks++;

    const verified = verifyNumCp007Wave02Package(pkg);
...
assert.equal(lifecycleViolations, 0, "Wave 02 lifecycle must remain closed.");

console.log(JSON.stringify({
  status: "PASS_NUM_CP007_WAVE02_AUTHORITY",
