...
function pairStem(pkg: Avg001QuestionPackage, language: Avg001Cp004PilotLanguage) {
  ...
  if (pkg.solveMode === "findCombinedAverageOfTwoGroups") {
    ...
    return language === "hi"
      ? `${count1} ${words.first} हैं और ${words.firstMeasure} का औसत ${average1} है। ${count2} ${words.second} हैं और ${words.secondMeasure} का औसत ${average2} है। ${words.result} ज्ञात कीजिए।`
      : `${count1} ${words.first} ਹਨ ਅਤੇ ${words.firstMeasure} ਦੀ ਔਸਤ ${average1} ਹੈ। ${count2} ${words.second} ਹਨ ਅਤੇ ${words.secondMeasure} ਦੀ ਔਸਤ ${average2} ਹੈ। ${words.result} ਪਤਾ ਕਰੋ।`;
  }
  if (pkg.solveMode === "findGroupCountFromCombinedAverage") {
    ...
    return language === "hi"
      ? `${knownCount} ${words.first} हैं और ${words.firstMeasure} का औसत ${knownAverage} है। ${words.secondMeasure} का औसत ${unknownAverage} तथा दोनों समूहों का संयुक्त औसत ${combinedAverage} है। ${words.secondCount} ज्ञात कीजिए।`
      : `${knownCount} ${words.first} ਹਨ ਅਤੇ ${words.firstMeasure} ਦੀ ਔਸਤ ${knownAverage} ਹੈ। ${words.secondMeasure} ਦੀ ਔਸਤ ${unknownAverage} ਅਤੇ ਦੋਵਾਂ ਸਮੂਹਾਂ ਦੀ ਸੰਯੁਕਤ ਔਸਤ ${combinedAverage} ਹੈ। ${words.secondCount} ਪਤਾ ਕਰੋ।`;
  }
  const count1 = shown(pkg, "count1");
  const count2 = shown(pkg, "count2");
  const average1 = display(shown(pkg, "average1"), unit, language);
  const combinedAverage = display(shown(pkg, "combinedAverage"), unit, language);
  return language === "hi"
    ? `${count1} ${words.first} हैं और ${words.firstMeasure} का औसत ${average1} है। ${count2} ${words.second} को मिलाने पर संयुक्त औसत ${combinedAverage} हो जाता है। ${words.secondMeasure} का औसत ज्ञात कीजिए।`
    : `${count1} ${words.first} ਹਨ ਅਤੇ ${words.firstMeasure} ਦੀ ਔਸਤ ${average1} ਹੈ। ${count2} ${words.second} ਨੂੰ ਮਿਲਾਉਣ ਉੱਤੇ ਸੰਯੁਕਤ ਔਸਤ ${combinedAverage} ਹੋ ਜਾਂਦੀ ਹੈ। ${words.secondMeasure} ਦੀ ਔਸਤ ਪਤਾ ਕਰੋ।`;
}
...
function speedStem(pkg: Avg001QuestionPackage, language: Avg001Cp004PilotLanguage) {
  const words = avg001Cp004SpeedLexicon(variant(pkg), language);
  ...
  return language === "hi"
    ? `${words.subject} समान समय तक क्रमशः ${speed1} और ${speed2} की दर से कार्य करती है। ${words.result} ज्ञात कीजिए।`
    : `${words.subject} ਬਰਾਬਰ ਸਮੇਂ ਲਈ ਕ੍ਰਮਵਾਰ ${speed1} ਅਤੇ ${speed2} ਦੀ ਦਰ ਨਾਲ ਕੰਮ ਕਰਦੀ ਹੈ। ${words.result} ਪਤਾ ਕਰੋ।`;
}
