...
import { add, divide, equals, formatRational, gcd, rational } from "./math";
...
function optionSet(answer: Rational, answerType: string, seed: string) {
  ...
  const wrong = answerType === "RATIO"
    ? [
        ...
        `${answer.numerator + 1}:${answer.denominator}`,
        `${answer.numerator}:${answer.denominator + 1}`,
        `${answer.numerator + 1}:${answer.denominator + 1}`,
        `${Math.max(1, answer.numerator - 1)}:${answer.denominator + 1}`,
      ]
    : [String(answer.numerator + 2), String(Math.max(1, answer.numerator - 2)), String(answer.numerator + 4), String(answer.numerator + 6)];
  const unique = [...new Set(wrong)].filter((item) => item !== correct).slice(0, 3);
  if (unique.length !== 3) throw new Error(`Unable to build three distinct distractors for ${correct}`);
...
function explanation(mode: string, s: GapState, answer: string) {
  const n = (key: string) => Number(s[key]);
  if (mode === "findAverageAfterUniformTransformation") {
    if (n("factor") === 1) return ["Every observation increases by the same amount, so the average increases by that amount too.", `$$New average = ${n("oldAverage")} + ${n("change")}$$`, `$$New average = ${answer}$$`, "The number of observations remains unchanged.", `So the new average is ${answer}.`];
    if (n("change") === 0) return ["Every observation is multiplied by the same factor, so multiply the average by that factor.", `$$New average = ${n("oldAverage")} × ${n("factor")}$$`, `$$New average = ${answer}$$`, "The number of observations remains unchanged.", `So the new average is ${answer}.`];
    return ["Apply the same two operations to the average that are applied to every observation.", `$$New average = ${n("oldAverage")} × ${n("factor")} + ${n("change")}$$`, `$$New average = ${answer}$$`, "The number of observations remains unchanged.", `So the new average is ${answer}.`];
  }
...
export function runAvg001GapExpansionPipeline(input: { questionLanguageId: string; seed: string; language: Avg001Language }): Avg001QuestionPackage {
  ...
  const renderVariables: Record<string, string | number> = {
    ...
    averageChange: n("shift"), newAverage: n("newAverage"), unit: entry.unitKind === "marks" ? "marks" : entry.unitKind === "runs" ? "runs" : "units",
