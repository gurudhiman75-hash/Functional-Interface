...
for (const entry of getAvg001QuestionEntries().filter((item) => item.cpId === "AVG-CP-003")) {
  const seed = `avg-cp003-review:${entry.qlId}:0`;
  const pkg = runAvg001Pipeline({ questionLanguageId: entry.qlId, seed });
  rows.push([
    pkg.packageId,pkg.canonicalProblemId,pkg.questionLanguageId,pkg.taskKind,pkg.solveMode,pkg.difficultyBand,pkg.parameters.answerType,pkg.questionId,pkg.seed,pkg.mathematicalFingerprint,pkg.stem,
...
const output = path.resolve("src/quant-v4/topics/Arithmetic/subtopics/Average/AVG-001/avg-001-cp003-human-review-en.csv");
fs.writeFileSync(output, `${rows.join("\n")}\n`, "utf8");
console.log(JSON.stringify({ rows: rows.length - 1, output }, null, 2));
if (rows.length - 1 !== 14) throw new Error(`Expected 14 CP-003 proof review rows; got ${rows.length - 1}`);
