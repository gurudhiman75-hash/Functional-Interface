...
const output = path.resolve(
  "src/quant-v4/topics/Arithmetic/subtopics/Average/AVG-001/avg-001-cp003-human-review-en.csv",
);
fs.writeFileSync(output, `${rows.join("\n")}\n`, "utf8");
console.log(JSON.stringify({ rows: rows.length - 1, output }, null, 2));
if (rows.length - 1 !== 86) {
  throw new Error(`Expected 86 CP-003 review rows; got ${rows.length - 1}`);
}
