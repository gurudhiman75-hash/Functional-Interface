...
for (const entry of getAvg001QuestionEntries().filter(
  ...
  const seed = `avg-cp001-review:${entry.qlId}:0`;
...
const output = path.resolve(
  "src/quant-v4/topics/Arithmetic/subtopics/Average/AVG-001/avg-001-cp001-human-review-en.csv",
);
fs.writeFileSync(output, `${rows.join("\n")}\n`, "utf8");
console.log(JSON.stringify({ rows: rows.length - 1, output }, null, 2));
if (rows.length - 1 !== 72) {
  throw new Error(`Expected 72 CP-001 review rows; got ${rows.length - 1}`);
}
