...
const records = entries.flatMap((entry) => languages.map((language) => {
  const seed = `avg-001-natural-language-v3-2:${language}:${entry.qlId}`;
  ...
  assert.equal(new Set(question.options.map(numericalKey)).size, 4, `${entry.qlId}:${language} has equivalent numerical options`);
  assert.equal(question.options[question.correctIndex], question.answer);
  assert.equal(question.explanation.lines.length, 4);
  assert.equal(question.maturity, "MANUAL_REVIEW");
  assert.equal(question.publiclyPublishable, false);
  assert.equal(question.validation.valid, true, `${entry.qlId}:${language} failed V3.2 validation`);
  ...
  if (language === "en" && ["AVG-CP-001", "AVG-CP-002", "AVG-CP-003"].includes(entry.cpId) && !hasUnitCue(question.stem)) {
...
const forbiddenGeneric = ["contains a small arithmetic error", "गणना में छोटी गलती करता है", "ਗਣਨਾ ਵਿੱਚ ਛੋਟੀ ਗਲਤੀ ਕਰਦਾ ਹੈ"];
const genericReasonOccurrences = records.reduce((total, record) => total + forbiddenGeneric.reduce((subtotal, phrase) => subtotal + record.explanation.split(phrase).length - 1, 0), 0);
...
writeFileSync(resolve(out, "avg-001-natural-language-v3-summary.json"), JSON.stringify({
  packageId: "AVG-001", reviewCandidate: AVG_001_NATURAL_LANGUAGE_V3_REVIEW, status: "PASS",
  sourceReleases: ["AVG-001-EN-v2", "AVG-001-HI-v1", "AVG-001-PA-v1"], sourceReleasesUnchanged: true,
  qlCountPerLanguage: 425, languageCount: 3, totalReviewRows: records.length, solveModeCount: 45,
  ...
  validation: {
    ...
    localizedMathLabelsRequired: true, bareMathJaxOperatorsRejected: true, publiclyPublishable: false,
  },
}, null, 2), "utf8");
