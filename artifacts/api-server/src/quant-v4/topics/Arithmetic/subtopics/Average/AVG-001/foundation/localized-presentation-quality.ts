...
import { polishAvg001LocalizedStemVariation } from "./localized-stem-variation-polish";
...
const STEM_VARIATION_POLISH = "AVG-001 localized stem variation polish v1";
...
function refreshValidation(pkg: Avg001QuestionPackage, language: PilotLanguage) {
  const replaced = new Set([
    ...
    "localized-stem-variation-polish",
  ...
  checks.push(
    ...
    {
      name: "localized-stem-variation-polish",
      passed: pkg.traceability.localizedStemVariationPolish === STEM_VARIATION_POLISH,
      message: "Localized stem variation passed final Hindi/Punjabi grammar polishing",
    },
    {
...
export function applyAvg001LocalizedPresentationQuality(
  ...
  const variationPolishedStem = polishAvg001LocalizedStemVariation(variedStem, language);
  const humanized = applyAvg001HumanAuthoredExplanation(variationPolishedStem);
