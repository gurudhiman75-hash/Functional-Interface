...
import "./avg-001-cp006-multilingual-pilot-audit";
...
const CP006_AUTHORSHIP = "AVG-CP-006 context-authored localization v1";
const LOCALIZED_CP_IDS = ["AVG-CP-001", "AVG-CP-002", "AVG-CP-003", "AVG-CP-004", "AVG-CP-005", "AVG-CP-006"];
const CP003_MANUALLY_DIFFERENTIATED = new Set([
  "AVG-QL-156", "AVG-QL-160", "AVG-QL-394", "AVG-QL-395", "AVG-QL-398",
  "AVG-QL-399", "AVG-QL-400", "AVG-QL-401", "AVG-QL-404", "AVG-QL-405",
]);
...
function answerToken(pkg: Avg001QuestionPackage) {
  const match = String(pkg.answer).replaceAll(",", "").match(/-?\d+(?:\.\d+)?(?::-?\d+(?:\.\d+)?)?/);
...
function checkAuthorship(pkg: Avg001QuestionPackage, scope: string) {
  if (pkg.traceability.explanationAuthorship !== AUTHORSHIP) fail(`${scope}: explanation authorship marker missing`);
  if (typeof pkg.traceability.explanationOpeningVariant !== "number") fail(`${scope}: explanation opening variant missing`);
  if (typeof pkg.traceability.explanationConclusionVariant !== "number") fail(`${scope}: explanation conclusion variant missing`);
  if (pkg.canonicalProblemId === "AVG-CP-003" && pkg.traceability.cp003ExplanationAuthorship !== CP003_AUTHORSHIP) {
    fail(`${scope}: CP-003 context-authored explanation marker missing`);
  }
  if (CP003_MANUALLY_DIFFERENTIATED.has(pkg.questionLanguageId) && pkg.traceability.cp003ExplanationFinalPolish !== CP003_FINAL_POLISH) {
    fail(`${scope}: CP-003 manually differentiated prose marker missing`);
  }
  if (pkg.language !== "en" && pkg.canonicalProblemId === "AVG-CP-004" && pkg.traceability.cp004LocalizationAuthorship !== CP004_AUTHORSHIP) {
    fail(`${scope}: CP-004 context-authored localization marker missing`);
  }
  if (pkg.language !== "en" && pkg.canonicalProblemId === "AVG-CP-005" && pkg.traceability.cp005LocalizationAuthorship !== CP005_AUTHORSHIP) {
    fail(`${scope}: CP-005 context-authored localization marker missing`);
  }
  if (pkg.language !== "en" && pkg.canonicalProblemId === "AVG-CP-006" && pkg.traceability.cp006LocalizationAuthorship !== CP006_AUTHORSHIP) {
    fail(`${scope}: CP-006 context-authored localization marker missing`);
  }
}
function checkExplanation(pkg: Avg001QuestionPackage, scope: string) {
  if (pkg.explanation.lines.length < 4 || pkg.explanation.lines.length > 8) fail(`${scope}: explanation has ${pkg.explanation.lines.length} lines`);
  const token = answerToken(pkg);
  if (!pkg.explanation.lines.some((line) => line.replaceAll(",", "").replaceAll("₹", "").includes(token))) fail(`${scope}: explanation omits answer evidence`);
  if (!pkg.explanation.lines.some(arithmeticLine)) fail(`${scope}: explanation omits substituted arithmetic`);
  if (!containsCalculatedAnswer(pkg)) fail(`${scope}: no arithmetic line calculates the displayed answer`);
}
...
for (const entry of entries) {
  ...
  if (JSON.stringify(english.explanation) !== JSON.stringify(repeatedEnglish.explanation)) fail(`${entry.qlId}: English explanation is not deterministic`);
  ...
  if (LOCALIZED_CP_IDS.includes(entry.cpId)) {
    ...
    for (const language of ["hi", "pa"] as const) {
      ...
      if (localized.traceability.localizedStemVariationFinalizer !== STEM_VARIATION) fail(`${scope}: localized stem variation marker missing`);
      checkAuthorship(localized, scope);
      checkExplanation(localized, scope);
      if (localized.answer !== english.answer || localized.correctIndex !== english.correctIndex) fail(`${scope}: answer or correct index changed`);
      if (JSON.stringify(localized.options) !== JSON.stringify(english.options)) fail(`${scope}: options changed`);
      if (JSON.stringify(localized.parameters.values) !== JSON.stringify(english.parameters.values)) fail(`${scope}: mathematical parameters changed`);
      if (localized.mathematicalFingerprint !== english.mathematicalFingerprint) fail(`${scope}: mathematical fingerprint changed`);
      if (JSON.stringify(localized.explanation) !== JSON.stringify(repeated.explanation) || localized.stem !== repeated.stem) fail(`${scope}: localized presentation is not deterministic`);
...
console.log(JSON.stringify({
  ...
  cp006Authorship: CP006_AUTHORSHIP,
  ...
  verdict: failures.length ? "FAIL" : "PASS — UNIQUE LOCALIZED STEMS, UNIQUE PROSE AND DECISIVE ARITHMETIC",
}, null, 2));
