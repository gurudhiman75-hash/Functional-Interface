...
for (const entry of getAvg001QuestionEntries().filter((item) => item.cpId === "AVG-CP-006")) {
  const seed = `avg-cp006-review:${entry.qlId}:0`;
...
const output = path.resolve("src/quant-v4/topics/Arithmetic/subtopics/Average/AVG-001/avg-001-cp006-human-review-en.csv");
fs.writeFileSync(output, `${rows.join("\n")}\n`, "utf8");
console.log(JSON.stringify({ rows: rows.length - 1, output }, null, 2));
if (rows.length - 1 !== 44) throw new Error(`Expected 44 CP-006 review rows; got ${rows.length - 1}`);
