...
for (const entry of getAvg001QuestionEntries().filter((item) => item.cpId === "AVG-CP-004")) {
  const seed = `avg-cp004-review:${entry.qlId}:0`;
...
const output = path.resolve(
  "src/quant-v4/topics/Arithmetic/subtopics/Average/AVG-001/avg-001-cp004-human-review-en.csv",
);
fs.writeFileSync(output, `${rows.join("\n")}\n`, "utf8");
console.log(JSON.stringify({ rows: rows.length - 1, output }, null, 2));
if (rows.length - 1 !== 65) {
  throw new Error(`Expected 65 CP-004 review rows; got ${rows.length - 1}`);
}
