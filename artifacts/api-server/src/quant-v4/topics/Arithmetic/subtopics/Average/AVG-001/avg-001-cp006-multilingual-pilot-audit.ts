...
import {
  AVG_001_CP006_MULTILINGUAL_PILOT,
  getAvg001Cp006LocalizedQlIds,
  runAvg001Cp006LocalizationPilot,
} from "./foundation/cp006-localization-quality-runtime";
import { runAvg001Pipeline } from "./foundation/pipeline";

const failures: string[] = [];
const qlIds = getAvg001Cp006LocalizedQlIds();
const expectedQlIds = Array.from({ length: 44 }, (_, index) => `AVG-QL-${String(330 + index).padStart(3, "0")}`);
...
const forbiddenGrammar = {
  hi: /(?:स्कोर|अंकों का औसत अंक|औसत अंकों का औसत|वेतन का औसत वेतन|आयु का औसत आयु|उत्पादन का औसत उत्पादन)/,
  pa: /(?:ਸਕੋਰ|ਅੰਕਾਂ ਦੀ ਔਸਤ ਅੰਕ|ਔਸਤ ਅੰਕਾਂ ਦੀ ਔਸਤ|ਤਨਖਾਹ ਦੀ ਔਸਤ ਤਨਖਾਹ|ਉਮਰ ਦੀ ਔਸਤ ਉਮਰ|ਉਤਪਾਦਨ ਦੀ ਔਸਤ ਉਤਪਾਦਨ)/,
};

for (const qlId of qlIds) {
  for (const language of AVG_001_CP006_MULTILINGUAL_PILOT.languages) {
    for (let seedIndex = 0; seedIndex < 2; seedIndex += 1) {
      const seed = `avg-cp006-localization:${qlId}:${seedIndex}`;
      const english = runAvg001Pipeline({ questionLanguageId: qlId, seed, language: "en" });
      const localized = runAvg001Cp006LocalizationPilot({ questionLanguageId: qlId, seed, language });
      const repeated = runAvg001Cp006LocalizationPilot({ questionLanguageId: qlId, seed, language });
      ...
      if (localized.traceability.cp006LocalizationAuthorship !== "AVG-CP-006 context-authored localization v1") {
        failures.push(`${scope}: CP-006 authorship marker missing`);
      }
      if (english.parameters.scenarioVariant.startsWith("schoolSections")) {
...
if (qlIds.length !== AVG_001_CP006_MULTILINGUAL_PILOT.qlCount) {
  failures.push(`inventory mismatch: expected ${AVG_001_CP006_MULTILINGUAL_PILOT.qlCount}, found ${qlIds.length}`);
}
if (JSON.stringify(qlIds) !== JSON.stringify(expectedQlIds)) {
  failures.push("QL inventory differs from AVG-QL-330..AVG-QL-373");
}

console.log(JSON.stringify({
  releaseId: AVG_001_CP006_MULTILINGUAL_PILOT.releaseId,
  qlCount: qlIds.length,
  qlRange: "AVG-QL-330..AVG-QL-373",
  languages: AVG_001_CP006_MULTILINGUAL_PILOT.languages,
  generated,
  normalizedStemGroups: stemGroups.size,
  normalizedExplanationGroups: explanationGroups.size,
  failureCount: failures.length,
  failures: failures.slice(0, 200),
  verdict: failures.length ? "FAIL" : "PASS — EXHAUSTIVE CP-006 HINDI/PUNJABI RUNTIME PROOF",
}, null, 2));
