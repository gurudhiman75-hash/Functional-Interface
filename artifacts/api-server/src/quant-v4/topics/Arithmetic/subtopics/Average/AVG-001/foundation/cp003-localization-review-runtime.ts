...
type Role = { nominative: string; oblique: string };
...
function isAgeQuestion(pkg: Avg001QuestionPackage) {
  ...
  return pkg.parameters.contextDomain === "Family" || /age|teacher|child|newborn|afteryears|elapsedyears|retir/i.test(pkg.parameters.scenarioVariant);
}
...
function ageGroup(pkg: Avg001QuestionPackage, language: PilotLanguage) {
  ...
  if (language === "hi") {
    ...
    if (pkg.parameters.contextDomain === "Workplace") return /worker/i.test(variant) ? "कर्मियों" : "कर्मचारियों";
  ...
  if (pkg.parameters.contextDomain === "Workplace") return /worker/i.test(variant) ? "ਕਾਮਿਆਂ" : "ਕਰਮਚਾਰੀਆਂ";
...
function ageStem(pkg: Avg001QuestionPackage, language: PilotLanguage) {
  ...
  const after = years ? (language === "hi" ? `${years} वर्ष बाद, ` : `${years} ਸਾਲ ਬਾਅਦ, `) : "";
  ...
  if (language === "hi") {
    ...
    switch (pkg.solveMode) {
      case "findNewAverageAfterAddition":
        return newborn ? `${lead} ${after}एक नवजात शिशु जन्म लेता है। नई औसत आयु ज्ञात कीजिए।` : `${lead} ${after}${added} वर्ष का एक ${role.nominative} समूह में शामिल होता है। नई औसत आयु ज्ञात कीजिए।`;
      ...
      case "findAddedMemberValueFromShift":
        return `${lead} ${after}एक ${role.oblique} के जुड़ने पर औसत आयु ${newAverage} वर्ष हो जाती है। जुड़ने वाले ${role.oblique} की आयु ज्ञात कीजिए।`;
      case "findRemovedMemberValueFromShift":
        return `${lead} ${after}एक ${role.oblique} के जाने पर औसत आयु ${newAverage} वर्ष हो जाती है। जाने वाले ${role.oblique} की आयु ज्ञात कीजिए।`;
      case "findReplacementValueFromShift":
        return target === "old" ? `${lead} ${after}एक ${role.oblique} के स्थान पर ${newValue} वर्ष का ${role.nominative} आने से औसत आयु ${newAverage} वर्ष हो जाती है। पुराने ${role.oblique} की आयु ज्ञात कीजिए।` : `${lead} ${after}${oldValue} वर्ष के ${role.oblique} को बदलने पर औसत आयु ${newAverage} वर्ष हो जाती है। नए ${role.oblique} की आयु ज्ञात कीजिए।`;
      default:
        return pkg.stem;
    }
  }
  ...
  switch (pkg.solveMode) {
    case "findNewAverageAfterAddition":
      return newborn ? `${lead} ${after}ਇੱਕ ਨਵਜੰਮਿਆ ਬੱਚਾ ਜਨਮ ਲੈਂਦਾ ਹੈ। ਨਵੀਂ ਔਸਤ ਉਮਰ ਪਤਾ ਕਰੋ।` : `${lead} ${after}${added} ਸਾਲ ਦਾ ਇੱਕ ${role.nominative} ਸਮੂਹ ਵਿੱਚ ਸ਼ਾਮਲ ਹੁੰਦਾ ਹੈ। ਨਵੀਂ ਔਸਤ ਉਮਰ ਪਤਾ ਕਰੋ।`;
    case "findNewAverageAfterRemoval":
    ...
    case "findAddedMemberValueFromShift":
      return `${lead} ${after}ਇੱਕ ${role.oblique} ਦੇ ਸ਼ਾਮਲ ਹੋਣ ਉੱਤੇ ਔਸਤ ਉਮਰ ${newAverage} ਸਾਲ ਹੋ ਜਾਂਦੀ ਹੈ। ਸ਼ਾਮਲ ਹੋਏ ${role.oblique} ਦੀ ਉਮਰ ਪਤਾ ਕਰੋ।`;
    case "findRemovedMemberValueFromShift":
      return `${lead} ${after}ਇੱਕ ${role.oblique} ਦੇ ਜਾਣ ਉੱਤੇ ਔਸਤ ਉਮਰ ${newAverage} ਸਾਲ ਹੋ ਜਾਂਦੀ ਹੈ। ਜਾਣ ਵਾਲੇ ${role.oblique} ਦੀ ਉਮਰ ਪਤਾ ਕਰੋ।`;
    case "findReplacementValueFromShift":
      return target === "old" ? `${lead} ${after}ਇੱਕ ${role.oblique} ਦੀ ਥਾਂ ${newValue} ਸਾਲ ਦਾ ${role.nominative} ਆਉਣ ਨਾਲ ਔਸਤ ਉਮਰ ${newAverage} ਸਾਲ ਹੋ ਜਾਂਦੀ ਹੈ। ਪੁਰਾਣੇ ${role.oblique} ਦੀ ਉਮਰ ਪਤਾ ਕਰੋ।` : `${lead} ${after}${oldValue} ਸਾਲ ਦੇ ${role.oblique} ਨੂੰ ਬਦਲਣ ਉੱਤੇ ਔਸਤ ਉਮਰ ${newAverage} ਸਾਲ ਹੋ ਜਾਂਦੀ ਹੈ। ਨਵੇਂ ${role.oblique} ਦੀ ਉਮਰ ਪਤਾ ਕਰੋ।`;
    default:
      return pkg.stem;
  }
}
...
function nonAgeReplacementStem(pkg: Avg001QuestionPackage, language: PilotLanguage) {
  ...
  if (language === "hi") {
    if (pkg.solveMode === "findNewAverageAfterReplacement") return `${count} ${quantity.subject} ${oldAverage} है। ${oldValue} के स्थान पर ${newValue} रखने पर नया औसत ज्ञात कीजिए।`;
    return target === "old" ? `${count} ${quantity.subject} ${oldAverage} था। एक अज्ञात मान के स्थान पर ${newValue} रखने से औसत ${newAverage} हो गया। पुराना मान ज्ञात कीजिए।` : `${count} ${quantity.subject} ${oldAverage} था। ${oldValue} के स्थान पर अज्ञात मान रखने से औसत ${newAverage} हो गया। नया मान ज्ञात कीजिए।`;
  }
  if (pkg.solveMode === "findNewAverageAfterReplacement") return `${count} ${quantity.subject} ${oldAverage} ਹੈ। ${oldValue} ਦੀ ਥਾਂ ${newValue} ਰੱਖਣ ਉੱਤੇ ਨਵੀਂ ਔਸਤ ਪਤਾ ਕਰੋ।`;
  return target === "old" ? `${count} ${quantity.subject} ${oldAverage} ਸੀ। ਇੱਕ ਅਣਜਾਣ ਮੁੱਲ ਦੀ ਥਾਂ ${newValue} ਰੱਖਣ ਨਾਲ ਔਸਤ ${newAverage} ਹੋ ਗਈ। ਪੁਰਾਣਾ ਮੁੱਲ ਪਤਾ ਕਰੋ।` : `${count} ${quantity.subject} ${oldAverage} ਸੀ। ${oldValue} ਦੀ ਥਾਂ ਅਣਜਾਣ ਮੁੱਲ ਰੱਖਣ ਨਾਲ ਔਸਤ ${newAverage} ਹੋ ਗਈ। ਨਵਾਂ ਮੁੱਲ ਪਤਾ ਕਰੋ।`;
}
...
function polishCountStem(pkg: Avg001QuestionPackage, language: PilotLanguage) {
  ...
  let stem = pkg.stem;
  if (language === "hi") {
    stem = stem.replaceAll("विद्यार्थी की संख्या", "विद्यार्थियों की संख्या").replaceAll("खिलाड़ी की संख्या", "खिलाड़ियों की संख्या").replaceAll("कर्मी की संख्या", "कर्मियों की संख्या").replaceAll("मशीनें की संख्या", "मशीनों की संख्या").replaceAll("एक नया मशीन", "एक नई मशीन");
    if (variant === 1) stem = stem.replace("प्रारंभ में", "परिवर्तन से पहले");
    if (variant === 2) stem = stem.replace("प्रारंभ में", "मूल समूह में");
    return stem;
  }
  stem = stem.replaceAll("ਵਿਦਿਆਰਥੀ ਦੀ ਗਿਣਤੀ", "ਵਿਦਿਆਰਥੀਆਂ ਦੀ ਗਿਣਤੀ").replaceAll("ਖਿਡਾਰੀ ਦੀ ਗਿਣਤੀ", "ਖਿਡਾਰੀਆਂ ਦੀ ਗਿਣਤੀ").replaceAll("ਕਾਮੇ ਦੀ ਗਿਣਤੀ", "ਕਾਮਿਆਂ ਦੀ ਗਿਣਤੀ").replaceAll("ਇੱਕ ਨਵਾਂ ਮਸ਼ੀਨ", "ਇੱਕ ਨਵੀਂ ਮਸ਼ੀਨ");
  if (variant === 1) stem = stem.replace("ਸ਼ੁਰੂ ਵਿੱਚ", "ਬਦਲਾਅ ਤੋਂ ਪਹਿਲਾਂ");
  if (variant === 2) stem = stem.replace("ਸ਼ੁਰੂ ਵਿੱਚ", "ਮੂਲ ਸਮੂਹ ਵਿੱਚ");
  return stem;
}

function polishExplanation(pkg: Avg001QuestionPackage, language: PilotLanguage, ageQuestion: boolean) {
  const lines = pkg.explanation.lines.map((line) => line.replaceAll(";quad", "; ").replaceAll("\\quad", " "));
  if (!ageQuestion) return { lines };

  const role = roleFor(pkg, language);
  const answer = pkg.answer;
  if (language === "hi") {
    if (pkg.solveMode === "findNewAverageAfterAddition" || pkg.solveMode === "findNewAverageAfterRemoval" || pkg.solveMode === "findNewAverageAfterReplacement") lines[lines.length - 1] = `अतः नई औसत आयु ${answer} वर्ष है।`;
    else if (pkg.solveMode === "findAddedMemberValueFromShift") lines[lines.length - 1] = `अतः जुड़ने वाले ${role.oblique} की आयु ${answer} वर्ष है।`;
    else if (pkg.solveMode === "findRemovedMemberValueFromShift") lines[lines.length - 1] = `अतः जाने वाले ${role.oblique} की आयु ${answer} वर्ष है।`;
  ...
  } else if (pkg.solveMode === "findNewAverageAfterAddition" || pkg.solveMode === "findNewAverageAfterRemoval" || pkg.solveMode === "findNewAverageAfterReplacement") lines[lines.length - 1] = `ਇਸ ਲਈ ਨਵੀਂ ਔਸਤ ਉਮਰ ${answer} ਸਾਲ ਹੈ।`;
  else if (pkg.solveMode === "findAddedMemberValueFromShift") lines[lines.length - 1] = `ਇਸ ਲਈ ਸ਼ਾਮਲ ਹੋਏ ${role.oblique} ਦੀ ਉਮਰ ${answer} ਸਾਲ ਹੈ।`;
  else if (pkg.solveMode === "findRemovedMemberValueFromShift") lines[lines.length - 1] = `ਇਸ ਲਈ ਜਾਣ ਵਾਲੇ ${role.oblique} ਦੀ ਉਮਰ ${answer} ਸਾਲ ਹੈ।`;
  else if (pkg.solveMode === "findReplacementValueFromShift") {
    const target = String(pkg.parameters.values.replacementTarget ?? "new");
...
export function runAvg001Cp003LocalizationPilot(input: { questionLanguageId: string; seed: string; language: PilotLanguage }): Avg001QuestionPackage {
  const pkg = runBasePilot(input);
  const ageQuestion = isAgeQuestion(pkg);
  let stem = pkg.stem;
  if (ageQuestion) stem = ageStem(pkg, input.language);
  else if (pkg.solveMode === "findNewAverageAfterReplacement" || pkg.solveMode === "findReplacementValueFromShift") stem = nonAgeReplacementStem(pkg, input.language);
  else if (pkg.solveMode === "findOriginalCountFromJoiningMemberShift" || pkg.solveMode === "findOriginalCountFromLeavingMemberShift") stem = polishCountStem(pkg, input.language);
  return { ...pkg, stem, explanation: polishExplanation(pkg, input.language, ageQuestion) };
}
