...
import {
  BTD_001_BREADTH_REMEDIATION_V6,
  BTD_001_DISCOVERY_PROTOTYPE_IDS_V6,
  buildBtdDiscoveryQuestionV6,
  solveBtdPrototype006V5,
  verifyBtdPrototype006V5,
  type BtdDiscoveryPrototypeIdV6,
} from "./btd-cp001-breadth-remediation-v6";

const SEEDS_PER_PROTOTYPE = 200;
const MAX_ALLOWED_PACKAGING_ATTEMPTS = 8;
const MIN_UNIQUE_STATES = 70;
...
function solveAny(prototypeId: BtdDiscoveryPrototypeIdV6, state: any): Rational {
...
function verifyAny(prototypeId: BtdDiscoveryPrototypeIdV6, state: any, answer: Rational): boolean {
...
const stateSets = new Map<BtdDiscoveryPrototypeIdV6, Set<string>>();
const stemFamilies = new Map<BtdDiscoveryPrototypeIdV6, Set<string>>();
const exactStems = new Map<string, Set<BtdDiscoveryPrototypeIdV6>>();
const skeletons = new Map<BtdDiscoveryPrototypeIdV6, Set<string>>();
const contexts = new Map<BtdDiscoveryPrototypeIdV6, Set<string>>();
const misconceptionIds = new Map<BtdDiscoveryPrototypeIdV6, Set<string>>();
const remediatedByPrototype = new Map<BtdDiscoveryPrototypeIdV6, number>();
const maxAttemptsByPrototype = new Map<BtdDiscoveryPrototypeIdV6, number>();
...
for (const prototypeId of BTD_001_DISCOVERY_PROTOTYPE_IDS_V6) {
  ...
  for (let index = 0; index < SEEDS_PER_PROTOTYPE; index += 1) {
    const seed = `BTD-001:V6:${prototypeId}:${index}`;
    const first = buildBtdDiscoveryQuestionV6(prototypeId, seed) as any;
    const second = buildBtdDiscoveryQuestionV6(prototypeId, seed) as any;
    assert.equal(stableJson(first), stableJson(second), `${prototypeId}/${index}: deterministic replay drift`);
    assert.equal(first.requestedSeed, seed);
    assert.ok(first.packagingResolutionAttempts >= 1 && first.packagingResolutionAttempts <= MAX_ALLOWED_PACKAGING_ATTEMPTS, `${prototypeId}/${index}: packaging attempts ${first.packagingResolutionAttempts}`);
    ...
    assert.equal(first.chapterId, "BTD-001"); assert.equal(first.checkpointId, "BTD-CP-001"); assert.equal(first.prototypeId, prototypeId); assert.equal(first.packagingVersion, BTD_001_BREADTH_REMEDIATION_V6); assert.equal(first.sourceBoundary.officialPaperProvenanceRecovered, true);
...
assert.deepEqual(crossPrototypeExactStemCollisions, [], `Cross-prototype exact stems: ${JSON.stringify(crossPrototypeExactStemCollisions.slice(0, 5).map(([stem, owners]) => [stem, [...owners]]))}`);
assert.deepEqual(answerPositions.map((count) => count > 0), [true, true, true, true]);
for (const prototypeId of BTD_001_DISCOVERY_PROTOTYPE_IDS_V6) {
  assert.equal(stemFamilies.get(prototypeId)!.size, 3, `${prototypeId}: stem-family reach`);
  assert.ok(stateSets.get(prototypeId)!.size >= MIN_UNIQUE_STATES, `${prototypeId}: state pool too thin (${stateSets.get(prototypeId)!.size})`);
  assert.ok(misconceptionIds.get(prototypeId)!.size >= 4, `${prototypeId}: misconception vocabulary too thin`);
  assert.ok(skeletons.get(prototypeId)!.size >= 3, `${prototypeId}: presentation skeletons too thin`);
}
...
console.log(JSON.stringify({
  auditVersion: "BTD-001-CP001-BREADTH-REMEDIATION-AUDIT-v5",
  chapterId: "BTD-001",
  checkpointId: "BTD-CP-001",
  packagingVersion: BTD_001_BREADTH_REMEDIATION_V6,
  officialPaperProvenanceRecovered: true,
  ...
  prototypeCount: BTD_001_DISCOVERY_PROTOTYPE_IDS_V6.length,
  ...
  answerPositions,
  remediatedPackages,
  remediationRate: remediatedPackages / questions,
  totalPackagingAttempts,
  maximumPackagingAttempts,
  maxAllowedPackagingAttempts: MAX_ALLOWED_PACKAGING_ATTEMPTS,
  perPrototype: Object.fromEntries(BTD_001_DISCOVERY_PROTOTYPE_IDS_V6.map((prototypeId) => [prototypeId, {
    ...
    remediatedPackages: remediatedByPrototype.get(prototypeId),
    maximumPackagingAttempts: maxAttemptsByPrototype.get(prototypeId),
  }])),
  permanentQlAllocationAuthorized: false,
  questionStudioDiscoverable: false,
  downstreamDeliveryOpened: false,
}, null, 2));
console.log("PASS_BTD_001_CP001_BREADTH_REMEDIATION_AUDIT_V5");
