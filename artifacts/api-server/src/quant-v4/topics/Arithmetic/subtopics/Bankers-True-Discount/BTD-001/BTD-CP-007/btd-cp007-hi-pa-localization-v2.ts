...
function num(value: number, digits = 4) { return value.toFixed(digits).replace(/0+$/u, "").replace(/\.$/u, ""); }
function indian(value: number) {
  const raw = num(value, 2);
...
function dateText(iso: string, language: BtdCp007LanguageV2) {
  const d = new Date(`${iso}T00:00:00.000Z`);
  const months = language === "hi" ? HI_MONTHS : PA_MONTHS;
...
function family(stemFamilyId: string) {
  const match = stemFamilyId.match(/(?:T|STEM-)([123])$/u);
  if (!match) throw new Error(`BTD CP007 v2 cannot resolve stem family ${stemFamilyId}`);
  return Number(match[1]);
}
function raw(entry: (typeof BTD_PERMANENT_QL_REGISTRY)[number], seed: string): R {
  return entry.origin === "BTD-CP-001"
    ? buildBtdDiscoveryQuestionV6(entry.sourceAuthorityId as any, seed) as R
    : buildBtdCp002CandidateQuestion(entry.sourceAuthorityId as any, seed) as R;
}
function target(answerSemantic: string, language: BtdCp007LanguageV2) {
  const map: Record<string, readonly [string, string]> = {
    PRESENT_WORTH: ["वर्तमान मूल्य", "ਮੌਜੂਦਾ ਮੁੱਲ"],
    TRUE_DISCOUNT: ["सच्चा बट्टा", "ਸੱਚਾ ਬੱਟਾ"],
    BANKERS_DISCOUNT: ["बैंकर बट्टा", "ਬੈਂਕਰ ਬੱਟਾ"],
    BANKERS_GAIN: ["बैंकर लाभ", "ਬੈਂਕਰ ਲਾਭ"],
    RATE_PERCENT: ["वार्षिक ब्याज दर", "ਸਾਲਾਨਾ ਵਿਆਜ ਦਰ"],
    FACE_VALUE: ["अंकित मूल्य", "ਅੰਕਿਤ ਮੁੱਲ"],
    TIME_MONTHS: ["अवधि", "ਮਿਆਦ"],
    FACE_VALUE_DIFFERENCE: ["दोनों अंकित मूल्यों का अंतर", "ਦੋਵੇਂ ਅੰਕਿਤ ਮੁੱਲਾਂ ਦਾ ਅੰਤਰ"],
  };
...
function given(sourceId: string, s: R, language: BtdCp007LanguageV2): string {
  ...
  switch (sourceId) {
    case "BTD-PROT-001": case "BTD-PROT-002": case "BTD-PROT-003": case "BTD-PROT-004":
      return tr(language, `${c} का अंकित मूल्य ${money(s.faceValue)}, वार्षिक दर ${s.ratePercent}% और शेष अवधि ${duration(s.months, language)} है।`, `${c} ਦਾ ਅੰਕਿਤ ਮੁੱਲ ${money(s.faceValue)}, ਸਾਲਾਨਾ ਦਰ ${s.ratePercent}% ਅਤੇ ਬਾਕੀ ਮਿਆਦ ${duration(s.months, language)} ਹੈ।`);
...
function stem(g: string, askTarget: string, familyId: number, language: BtdCp007LanguageV2) {
  if (familyId === 1) return `${g} ${tr(language, `${askTarget} ज्ञात कीजिए।`, `${askTarget} ਕੱਢੋ।`)}`;
...
function keyIdea(sourceId: string, language: BtdCp007LanguageV2) {
  ...
  const pair = ideas[sourceId];
  if (!pair) throw new Error(`BTD CP007 v2 missing key idea for ${sourceId}`);
  return language === "hi" ? pair[0] : pair[1];
}
function calculations(sourceId: string, s: R, answer: string, language: BtdCp007LanguageV2): readonly string[] {
  ...
  switch (sourceId) {
    case "BTD-PROT-001": return [tr(language, `ब्याज गुणक x = ${s.ratePercent} × ${s.months}/1200 = ${num(x!)}।`, `ਵਿਆਜ ਗੁਣਕ x = ${s.ratePercent} × ${s.months}/1200 = ${num(x!)}।`), `PW = ${money(s.faceValue)}/(1+x) = ${answer}।`];
    case "BTD-PROT-002": { const pw = numeric(s.faceValue) / (1 + x!); return [`PW = ${money(s.faceValue)}/(1+${num(x!)}) = ${money(pw)}।`, tr(language, `TD = अंकित मूल्य − PW = ${money(s.faceValue)} − ${money(pw)} = ${answer}।`, `TD = ਅੰਕਿਤ ਮੁੱਲ − PW = ${money(s.faceValue)} − ${money(pw)} = ${answer}।`)]; }
    ...
    case "BTD-PROT-004": { const face = numeric(s.faceValue); const pw = face / (1 + x!); const bd = face * x!; const td = face - pw; return [`BD = ${money(bd)}; TD = ${money(td)}।`, `BG = BD − TD = ${money(bd)} − ${money(td)} = ${answer}।`]; }
    case "BTD-PROT-005": { const pw = numeric(s.faceValue) - numeric(s.trueDiscount); return [`PW = ${money(s.faceValue)} − ${money(s.trueDiscount)} = ${money(pw)}।`, `BD = TD × Face/PW = ${money(s.trueDiscount)} × ${money(s.faceValue)}/${money(pw)} = ${answer}।`]; }
    case "BTD-PROT-006": { const factor = numeric(s.bdToTdRatio) - 1; return [`x = BD/TD − 1 = ${ratio(s.bdToTdRatio)} − 1 = ${num(factor)}।`, tr(language, `दर = x × 1200/${s.months} = ${answer}।`, `ਦਰ = x × 1200/${s.months} = ${answer}।`)]; }
    case "BTD-PROT-007": return [`x = ${s.ratePercent} × ${s.months}/1200 = ${num(x!)}।`, `PW = BG/x² = ${money(s.bankersGain)}/${num(x!)}² = ${answer}।`];
    ...
    case "BTD-PROT-009": { const delta = numeric(s.bdToTdRatio) - 1; return [`BD/TD − 1 = ${ratio(s.bdToTdRatio)} − 1 = ${num(delta)}।`, `R² = 100 × ${s.rateEqualsYearsMultiplier} × ${num(delta)}; R = ${answer}।`]; }
    case "BTD-CAND-010": return [`TD = √(PW × BG) = √(${money(s.presentWorth)} × ${money(s.bankersGain)})।`, tr(language, `अतः TD = ${answer}।`, `ਇਸ ਲਈ TD = ${answer}।`)];
    case "BTD-CAND-011": { const total = numeric(s.totalFaceValue); const discount = numeric(s.totalBankersDiscount); const x1 = s.ratePercent * s.firstMonths / 1200; const x2 = s.ratePercent * s.secondMonths / 1200; const f1 = (discount - total * x2) / (x1 - x2); const f2 = total - f1; return [`F₁ + F₂ = ${money(total)}; ${num(x1)}F₁ + ${num(x2)}F₂ = ${money(discount)}।`, tr(language, `हल करने पर F₁ = ${money(f1)}, F₂ = ${money(f2)}; अंतर = ${answer}।`, `ਹੱਲ ਕਰਨ 'ਤੇ F₁ = ${money(f1)}, F₂ = ${money(f2)}; ਅੰਤਰ = ${answer}।`)]; }
    ...
    case "BTD-CAND-013": return [`x = ${s.ratePercent} × ${s.months}/1200 = ${num(x!)}।`, `TD = BD/(1+x) = ${money(s.bankersDiscount)}/(1+${num(x!)}) = ${answer}।`];
    case "BTD-CAND-014": { const factor = numeric(s.bdToTdRatio) - 1; return [`x = BD/TD − 1 = ${num(factor)}।`, tr(language, `अवधि = x × 1200/${s.ratePercent} = ${answer}।`, `ਮਿਆਦ = x × 1200/${s.ratePercent} = ${answer}।`)]; }
    case "BTD-CAND-015": return [`x = ${s.ratePercent} × ${s.months}/1200 = ${num(x!)}।`, `TD = BG/x = ${money(s.bankersGain)}/${num(x!)} = ${answer}।`];
    ...
    case "BTD-CAND-018": { const factor = numeric(s.trueDiscountFace) / numeric(s.bankersDiscountFace) - 1; return [`1 + x = ${money(s.trueDiscountFace)}/${money(s.bankersDiscountFace)}; x = ${num(factor)}।`, tr(language, `अवधि = x × 1200/${s.ratePercent} = ${answer}।`, `ਮਿਆਦ = x × 1200/${s.ratePercent} = ${answer}।`)]; }
    case "BTD-CAND-019": { const factor = numeric(s.bankersDiscount) / numeric(s.trueDiscount) - 1; return [`x = BD/TD − 1 = ${money(s.bankersDiscount)}/${money(s.trueDiscount)} − 1 = ${num(factor)}।`, tr(language, `दर = x × 1200/${s.months} = ${answer}।`, `ਦਰ = x × 1200/${s.months} = ${answer}।`)]; }
    case "BTD-CAND-020": return [`x = ${s.ratePercent} × ${s.months}/1200 = ${num(x!)}।`, `BD = TD(1+x) = ${money(s.trueDiscount)} × (1+${num(x!)}) = ${answer}।`];
...
export function buildBtdLocalizedQuestionV2(qlId: BtdPermanentQlId, seed: string, language: BtdCp007LanguageV2) {
  ...
  const frozen = buildBtdFrozenEnglishQuestionV1(qlId, seed) as R;
  const authority = raw(entry, seed);
  ...
  const askTarget = target(entry.answerSemantic, language);
  const whatGiven = given(entry.sourceAuthorityId, authority.state, language);
  const familyId = family(frozen.presentation.stemFamilyId);
  const localizedStem = stem(whatGiven, askTarget, familyId, language);
  const steps = calculations(entry.sourceAuthorityId, authority.state, frozen.correctAnswer, language);
  const explanation = Object.freeze({
    whatGiven,
    whatAsked: tr(language, `${askTarget} ज्ञात कीजिए।`, `${askTarget} ਕੱਢੋ।`),
    keyIdea: keyIdea(entry.sourceAuthorityId, language),
    steps: Object.freeze([...steps]),
    finalAnswer: tr(language, `अतः सही उत्तर ${frozen.correctAnswer} है।`, `ਇਸ ਲਈ ਸਹੀ ਉੱਤਰ ${frozen.correctAnswer} ਹੈ।`),
  });
  const options = Object.freeze(frozen.options.map((option: R) => Object.freeze({
  ...
  const payload = Object.freeze({
    ...
    sourceStateFingerprint: observedStateFingerprint,
    englishContentFingerprint: frozen.contentFingerprint,
    presentation: Object.freeze({ stemFamilyId: `${frozen.presentation.stemFamilyId}-${language.toUpperCase()}`, stem: localizedStem }),
    options,
  ...
  return deepFreeze({
    ...
    lifecycle: BTD_CP007_LOCALIZATION_BOUNDARY_V2,
  });
}
