...
import {
  BTD_001_DISCOVERY_PACKAGING_V3,
...
const remediatedByPrototype = new Map<BtdPrototypeId, number>();
const maxAttemptsByPrototype = new Map<BtdPrototypeId, number>();
...
for (const prototypeId of BTD_001_PROTOTYPE_IDS) {
  ...
  remediatedByPrototype.set(prototypeId, 0);
  maxAttemptsByPrototype.set(prototypeId, 0);

  for (let index = 0; index < SEEDS_PER_PROTOTYPE; index += 1) {
    const seed = `BTD-001:DISCOVERY:${prototypeId}:${index}`;
    const first = buildBtdDiscoveryQuestionV3(prototypeId, seed) as any;
    const second = buildBtdDiscoveryQuestionV3(prototypeId, seed) as any;
    ...
    assert.ok(first.packagingResolutionAttempts >= 1, `${prototypeId}/${index}: invalid packaging attempt count`);
    assert.ok(first.packagingResolutionAttempts <= MAX_ALLOWED_PACKAGING_ATTEMPTS, `${prototypeId}/${index}: packaging needed ${first.packagingResolutionAttempts} attempts; state pool is too brittle`);
    if (first.packagingResolutionAttempts === 1) {
      assert.equal(first.effectiveSeed, seed, `${prototypeId}/${index}: unremediated package changed effective seed`);
      assert.equal(first.packagingRemediation, "NONE_REQUIRED");
    } else {
      assert.notEqual(first.effectiveSeed, seed, `${prototypeId}/${index}: remediated package retained failing seed`);
      assert.equal(first.packagingRemediation, "DETERMINISTIC_OPTION_SAFE_STATE_RESELECTION_WITHIN_SAME_PROTOTYPE_AUTHORITY");
      remediatedPackages += 1;
      remediatedByPrototype.set(prototypeId, remediatedByPrototype.get(prototypeId)! + 1);
    }
    ...
    assert.equal(first.packagingVersion, BTD_001_DISCOVERY_PACKAGING_V3);
...
assert.ok(remediatedPackages <= Math.ceil(questions * 0.1), `Option-safe state reselection is too frequent: ${remediatedPackages}/${questions}`);

console.log(JSON.stringify({
  auditVersion: "BTD-001-CP001-SOURCE-BOUND-DISCOVERY-AUDIT-v2",
  chapterId: "BTD-001",
...
console.log("PASS_BTD_001_CP001_SOURCE_BOUND_DISCOVERY_AUDIT_V2");
