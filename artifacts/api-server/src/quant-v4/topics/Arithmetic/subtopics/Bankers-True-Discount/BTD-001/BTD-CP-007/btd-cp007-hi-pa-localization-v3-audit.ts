...
function familyKey(stemFamilyId: string) {
  const match = stemFamilyId.match(/T([123])-(HI|PA)$/u);
...
for (const entry of BTD_PERMANENT_QL_REGISTRY) {
  for (const language of BTD_CP007_LANGUAGES_V3) {
    ...
    assert.deepEqual([...familyCoverage.get(coverageKey)!].sort(), ["1", "2", "3"], `${coverageKey}: all three localized stem families were not reached`);
  }
}
...
function renderReview() {
  const lines: string[] = [
    "# BTD-001 CP007 Hindi/Punjabi Localization Review v3",
    "",
...
writeFileSync("btd-cp007-hi-pa-review-v3.md", review, "utf8");

console.log(JSON.stringify({
  auditVersion: "BTD-001-CP007-HI-PA-LOCALIZATION-AUDIT-v3",
...
console.log("PASS_BTD_001_CP007_HI_PA_LOCALIZATION_AUDIT_V3");
