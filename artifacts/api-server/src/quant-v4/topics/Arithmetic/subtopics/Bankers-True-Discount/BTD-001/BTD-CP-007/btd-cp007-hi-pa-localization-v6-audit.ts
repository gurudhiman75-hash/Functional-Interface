...
import {
  ...
  BTD_CP007_LOCALIZATION_V4,
  buildBtdLocalizedQuestionV4,
  localizeBtdCp007DisplayValueV4,
} from "./btd-cp007-hi-pa-localization-v4";

const AUDIT_VERSION = "BTD-001-CP007-HI-PA-LOCALIZATION-AUDIT-v6" as const;
...
const CONTENT_FINGERPRINT = /^[a-f0-9]{64}$/u;
...
assert.equal(BTD_CP007_LOCALIZATION_V4, "BTD-001-CP007-HI-PA-LOCALIZATION-v4");
assert.deepEqual(BTD_CP007_LANGUAGES_V4, ["hi", "pa"]);
assert.equal(BTD_CP007_LOCALIZATION_BOUNDARY_V4.localizationStatus, "HI_PA_REVIEW_CANDIDATE");
assert.equal(BTD_CP007_LOCALIZATION_BOUNDARY_V4.englishAuthorityFrozen, true);
assert.equal(BTD_CP007_LOCALIZATION_BOUNDARY_V4.multilingualFrozen, false);
assert.equal(BTD_CP007_LOCALIZATION_BOUNDARY_V4.questionStudioDiscoverable, false);
assert.equal(BTD_CP007_LOCALIZATION_BOUNDARY_V4.questionStudioGenerationEnabled, false);
assert.equal(BTD_CP007_LOCALIZATION_BOUNDARY_V4.questionBankWritable, false);
assert.equal(BTD_CP007_LOCALIZATION_BOUNDARY_V4.testEligible, false);
assert.equal(BTD_CP007_LOCALIZATION_BOUNDARY_V4.mockTestEligible, false);
assert.equal(BTD_CP007_LOCALIZATION_BOUNDARY_V4.publiclyPublishable, false);
...
let jsonChecks = 0;
let contentFingerprintChecks = 0;
let diversityChecks = 0;
let displayLocalizationChecks = 0;
...
for (const entry of BTD_PERMANENT_QL_REGISTRY) {
  for (const language of BTD_CP007_LANGUAGES_V4) {
    ...
    for (let index = 0; index < 100; index += 1) {
      ...
      const localized = buildBtdLocalizedQuestionV4(entry.qlId, seed, language) as any;
      const replay = buildBtdLocalizedQuestionV4(entry.qlId, seed, language) as any;
      ...
      const expectedLocalizedOptions = frozen.options.map((option: any) => localizeBtdCp007DisplayValueV4(String(option.text), language));
      const expectedLocalizedAnswer = localizeBtdCp007DisplayValueV4(String(frozen.correctAnswer), language);
      assert.equal(localized.options.length, 4, `${coverageKey}/${seed}: expected four options`);
      assert.deepEqual(localized.options.map((option: any) => option.text), expectedLocalizedOptions, `${coverageKey}/${seed}: localized option text/order parity drift`);
      ...
      assert.equal(localized.correctAnswer, expectedLocalizedAnswer, `${coverageKey}/${seed}: localized correct-answer display drift`);
      assert.equal(localized.options[localized.correctIndex].text, localized.correctAnswer, `${coverageKey}/${seed}: correct option does not own localized answer`);
      ...
      assert.ok(localized.explanation.finalAnswer.includes(localized.correctAnswer), `${coverageKey}/${seed}: final answer does not own localized canonical answer`);
      ...
      if (language === "hi") {
        assert.equal(/इन जानकारियों से .+ कितना होगा\?/u.test(localized.presentation.stem), false, `${coverageKey}/${seed}: gendered generic Hindi question survived polish`);
        naturalnessChecks += 1;
      } else {
        assert.equal(/ਇਨ੍ਹਾਂ ਜਾਣਕਾਰੀਆਂ ਤੋਂ .+ ਕਿੰਨਾ ਹੋਵੇਗਾ\?/u.test(localized.presentation.stem), false, `${coverageKey}/${seed}: gendered generic Punjabi question survived polish`);
        assert.equal(/ਲੋੜੀਂਦਾ .+ ਨਿਰਧਾਰਤ ਕਰੋ।/u.test(localized.presentation.stem), false, `${coverageKey}/${seed}: gendered Punjabi determiner survived polish`);
        naturalnessChecks += 2;
      }
      ...
      assert.match(localized.localizationFingerprint, CONTENT_FINGERPRINT, `${coverageKey}/${seed}: malformed localization content fingerprint`);
      assert.equal(replay.localizationFingerprint, localized.localizationFingerprint, `${coverageKey}/${seed}: content fingerprint is not deterministic`);
      observedContentFingerprints.add(localized.localizationFingerprint);
      contentFingerprintChecks += 2;
...
assert.ok(observedContentFingerprints.size >= MIN_UNIQUE_STEMS_PER_100, "localized content fingerprint pool unexpectedly collapsed");

function renderReview() {
  const lines: string[] = [
    "# BTD-001 CP007 Hindi/Punjabi Localization Review v6",
    "",
...
writeFileSync("btd-cp007-hi-pa-review-v6.md", review, "utf8");
writeFileSync("btd-cp007-hi-pa-diversity-v6.json", JSON.stringify({
  thresholds: {
  ...
  observed: {
    ...
    uniqueContentFingerprints: observedContentFingerprints.size,
  },
...
console.log(JSON.stringify({
  ...
  answerPositions,
  localizationStatus: BTD_CP007_LOCALIZATION_BOUNDARY_V4.localizationStatus,
  multilingualFrozen: BTD_CP007_LOCALIZATION_BOUNDARY_V4.multilingualFrozen,
  questionStudioDiscoverable: BTD_CP007_LOCALIZATION_BOUNDARY_V4.questionStudioDiscoverable,
  downstreamDeliveryOpened: false,
}, null, 2));
console.log("PASS_BTD_001_CP007_HI_PA_LOCALIZATION_AUDIT_V6");
