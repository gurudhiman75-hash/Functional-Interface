...
function polishLearnerText(text: string) {
  return text
    ...
    .replace(/BD\/TD = Face\/PW = 1 \+ RT\. With time known, the annual rate follows directly\./gu, "Let x = rate × months/1200. Then BD/TD = Face/PW = 1 + x; with time known, the annual rate follows directly.")
    .replace(/If x = RT, then/gu, "If x = rate × months/1200, then")
    .replace(/, RT =/gu, ", x =")
    .replace(/R = RT ×/gu, "rate = x ×")
...
function cp002Explanation(sourceAuthorityId: string, state: any, correctAnswer: string) {
  switch (sourceAuthorityId) {
    ...
    case "BTD-CAND-014": return { whatAsked: "Find the unexpired time.", keyIdea: "Let x be the simple-interest factor for the unexpired term. Then BD/TD = 1 + x.", steps: [`x = BD/TD − 1 = ${fraction(state.bdToTdRatio)} − 1.`, `Time in months = (BD/TD − 1) × 1200/rate = (${fraction(state.bdToTdRatio)} − 1) × 1200/${state.ratePercent} = ${correctAnswer}.`] };
    ...
    case "BTD-CAND-018": return { whatAsked: "Find the common time.", keyIdea: "If BD on the first face equals TD on the second, then second face/first face = 1 + x, where x is the common interest factor.", steps: [`x = ${money(state.trueDiscountFace)}/${money(state.bankersDiscountFace)} − 1.`, `Time in months = (${money(state.trueDiscountFace)}/${money(state.bankersDiscountFace)} − 1) × 1200/${state.ratePercent} = ${correctAnswer}.`] };
    case "BTD-CAND-019": return { whatAsked: "Find the annual rate.", keyIdea: "For one bill, BD/TD = 1 + x, so BD and TD reveal the simple-interest factor x.", steps: [`x = BD/TD − 1 = ${money(state.bankersDiscount)}/${money(state.trueDiscount)} − 1.`, `Rate = (BD/TD − 1) × 1200/months = (${money(state.bankersDiscount)}/${money(state.trueDiscount)} − 1) × 1200/${state.months} = ${correctAnswer}.`] };
