...
const rows = TMW_CP002_REGISTRY.flatMap((entry) => languages.map((language) => {
  const seed = `tmw-cp002-localization-review:${entry.qlId}`;
...
writeFileSync("dist/quant-v4/tmw-001-cp002-localization-review.json", JSON.stringify(output, null, 2));
