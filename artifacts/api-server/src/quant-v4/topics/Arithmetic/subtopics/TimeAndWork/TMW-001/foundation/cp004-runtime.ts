...
import { buildTmwCp004CommonTrap, buildTmwCp004Shortcut, buildTmwCp004WorkingLatex } from "./cp004-learning";
...
export function runTmwCp004Pipeline(input:{questionLanguageId:string;seed:string;language?:"en"|"hi"|"pa"}):TmwCp004GeneratedQuestion{
 ...
 const explanation={opening:tmwCp004ExplanationOpening(entry),formula,steps,shortcut,commonTrap,conclusion:tmwCp004Conclusion(entry,parameters,solution.answerText)};
 const errors:string[]=[];
 const learnerText=[stem,...optionSet.options.map(x=>x.text),solution.answerText,explanation.opening,explanation.formula,...explanation.steps,explanation.shortcut.title,...explanation.shortcut.steps,explanation.commonTrap.optionLabel,explanation.commonTrap.optionText,explanation.commonTrap.explanation,explanation.conclusion].join(" ");
 ...
 return {archetypeId:"TMW-001",canonicalProblemId:"TMW-CP-004",questionLanguageId:entry.qlId,solveMode:entry.solveMode,language:"en",seed:input.seed,stem,parameters,solution,options:optionSet.options.map(x=>x.text),optionAudit:optionSet.options,correctIndex:optionSet.correctIndex,explanation,mathematicalFingerprint:fingerprint(parameters,entry.solveMode),validation:{valid:errors.length===0,errors},publiclyPublishable:false};
}
