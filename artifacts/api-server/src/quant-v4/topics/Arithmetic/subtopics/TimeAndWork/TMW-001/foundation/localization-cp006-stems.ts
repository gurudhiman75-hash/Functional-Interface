import { divide, equals, rational } from "./rational";
...
function question(language: TmwLocalizedLanguage, hi: string, pa: string): string {
  return language === "hi" ? hi : pa;
}

export function renderTmwCp006LocalizedStem(
  ...
  switch (source.solveMode) {
    ...
    case "findEquivalentResourceTime": {
      const machine = p.context.resourceTimeUnit.endsWith("hours");
      return question(
        language,
        `${cp006Resource(p, a.resources, language)} ${machine ? cp006Hours(a.days, language) : cp006Days(a.days, language)} तक कार्यरत हैं। इसके बराबर कुल ${cp006Copy(p.context.resourceTimeUnit, language)} कितना है?`,
        `${cp006Resource(p, a.resources, language)} ${machine ? cp006Hours(a.days, language) : cp006Days(a.days, language)} ਤੱਕ ਕੰਮ ਕਰ ਰਹੇ ਹਨ। ਇਸ ਦੇ ਬਰਾਬਰ ਕੁੱਲ ${cp006Copy(p.context.resourceTimeUnit, language)} ਕਿੰਨਾ ਹੈ?`,
      );
    }
  }
}
