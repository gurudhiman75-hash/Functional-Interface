...
function fingerprint(p:TmwCp008Parameters):string{const role=(x:TmwCp008Parameters["context"]["roles"][number])=>[x.count,x.efficiency,x.days,x.hoursPerDay,x.output,x.baselineOutput,x.defectiveOutput].map(rationalKey).join(":");return[p.context.setting,p.context.task,...p.context.roles.map(role),rationalKey(p.totalPayment),String(p.targetIndex??"-"),(p.selectedIndices??[]).join(","),(p.contributionWeights??[]).map(rationalKey).join(","),(p.reportedPayments??[]).map(rationalKey).join(","),(p.knownPaymentIndices??[]).join(","),p.eventKind??"-",p.factorTarget??"-",p.pieceRate?rationalKey(p.pieceRate):"-",p.bonusPool?rationalKey(p.bonusPool):"-"].join("|");}
...
export function runTmwCp008Pipeline(input:{questionLanguageId:string;seed:string;language?:"en"|"hi"|"pa"}):TmwCp008GeneratedQuestion{
 ...
 if(moneyAnswer(entry.answerType)&&/[₹$€]/.test(learnerText))errors.push("Generic-money learner text uses an inconsistent currency");
 ...
 if(!commonTrap.explanation.startsWith(`Do not choose ${commonTrap.optionLabel}`))errors.push("Common trap is not option-specific and learner-friendly");
