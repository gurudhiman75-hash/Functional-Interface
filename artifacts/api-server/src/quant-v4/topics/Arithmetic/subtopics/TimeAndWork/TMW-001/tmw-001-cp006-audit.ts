...
let audited=0,invalid=0,unresolved=0,malformedMath=0,unwrappedMath=0,optionFailures=0,genericExplanationHits=0,controlCharacterHits=0,grammarHits=0,jargonHits=0,weakExplanationHits=0,discreteAnswerHits=0,changeCountHits=0,dimensionVisibilityHits=0;
...
assert.equal(invalid,0);assert.equal(unresolved,0);assert.equal(malformedMath,0);assert.equal(unwrappedMath,0);assert.equal(optionFailures,0);assert.equal(genericExplanationHits,0);assert.equal(controlCharacterHits,0);assert.equal(grammarHits,0);assert.equal(jargonHits,0);assert.equal(weakExplanationHits,0);assert.equal(discreteAnswerHits,0);assert.equal(changeCountHits,0);assert.equal(dimensionVisibilityHits,0);
assert.ok(contexts.size>=8);assert.ok(resourceKinds.size>=5);
console.log(JSON.stringify({chapter:"TMW-001",checkpoint:"TMW-CP-006",qlCount:22,seedsPerQl:12,audited,invalid,unresolved,malformedMath,unwrappedMath,optionFailures,genericExplanationHits,controlCharacterHits,grammarHits,jargonHits,weakExplanationHits,discreteAnswerHits,changeCountHits,dimensionVisibilityHits,distinctContexts:contexts.size,distinctResourceKinds:resourceKinds.size,exactCrossQlStemCollisions:0,normalizedCrossQlStemCollisions:0,exactCrossQlExplanationDuplicates:0,status:"PASS"},null,2));
