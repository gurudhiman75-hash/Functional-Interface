...
export function renderTmwCp006Stem(entry:TmwCp006RegistryEntry,p:TmwCp006Parameters):string{
  ...
  switch(entry.solveMode){
    ...
    case "findDimensionalWorkRatio":return `The original ${c.jobPhrase} has ${dimensions(required(p.dimensionsA,"dimensionsA"),required(p.dimensionLabels,"dimensionLabels"))}. A second one has ${dimensions(required(p.dimensionsB,"dimensionsB"),required(p.dimensionLabels,"dimensionLabels"))}. Assuming work is proportional to the product of these dimensions, find the ratio of the second work quantity to the first.`;
    case "findWorkersForChangedDimensions":return `${resourceCount(p,a.resources)} complete a ${c.jobPhrase} with ${dimensions(required(p.dimensionsA,"dimensionsA"),required(p.dimensionLabels,"dimensionLabels"))} in ${days(a.days)}. How many ${c.resourcePlural} are needed for one with ${dimensions(required(p.dimensionsB,"dimensionsB"),required(p.dimensionLabels,"dimensionLabels"))} in ${days(b.days)}, at the same daily hours and efficiency?`;
    case "findDaysForChangedDimensions":return `${resourceCount(p,a.resources)} complete a ${c.jobPhrase} with ${dimensions(required(p.dimensionsA,"dimensionsA"),required(p.dimensionLabels,"dimensionLabels"))} in ${days(a.days)}. In how many days will ${resourceCount(p,b.resources)} complete one with ${dimensions(required(p.dimensionsB,"dimensionsB"),required(p.dimensionLabels,"dimensionLabels"))}, at the same daily hours and efficiency?`;
