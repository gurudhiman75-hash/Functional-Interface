...
assert.deepEqual(TMW_CP004_REGISTRY.map(entry=>entry.qlId),Array.from({length:24},(_,index)=>`TMW-QL-${String(index+58).padStart(3,"0")}`));
...
for(const entry of TMW_CP004_REGISTRY){
  ...
  for(let i=0;i<50;i++){
    const seed=`tmw-cp004-proof:${entry.qlId}:${i}`;
...
assert.throws(()=>runTmwCp004Pipeline({questionLanguageId:"TMW-QL-058",seed:"unsupported",language:"hi"}),/English only/);
assert.throws(()=>runTmwCp004Pipeline({questionLanguageId:"TMW-QL-999",seed:"unknown"}),/Unknown TMW-CP-004/);
