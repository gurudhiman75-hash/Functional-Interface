...
const rows = TMW_CP005_REGISTRY.flatMap((entry) => languages.map((language) => {
  const seed = `tmw-cp005-localization-review:${entry.qlId}`;
...
writeFileSync("dist/quant-v4/tmw-001-cp005-localization-review.json", JSON.stringify(output, null, 2));
