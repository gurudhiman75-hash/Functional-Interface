...
import { runTmwCp011ChapterSafePipeline } from "./cp011-chapter-safe-runtime";
...
function finishLocalized(
  ...
  const wave02 = applyTmw001MultilingualStemRemediationWave02(
    wave01,
    questionLanguageId,
    language,
  );
  const sanitized = sanitizeTmw001LocalizedPresentation(wave02);
  const naturalized = naturalizeTmw001LocalizedExplanation(
    sanitized,
    questionLanguageId,
    language,
  );
  ...
  const cp002Reviewed = applyTmwCp002EditorialReviewRemediation(
    cp001Cleaned,
    questionLanguageId,
    language,
  );
  const cp003Reviewed = applyTmwCp003EditorialReviewRemediation(
    cp002Reviewed,
    questionLanguageId,
    language,
  );
  const cp003Cleaned = applyTmwCp003EditorialFieldCleanup(
    cp003Reviewed,
    questionLanguageId,
    language,
  );
...
export function runTmw001ChapterPipeline(input: Tmw001ChapterRequest): any {
  ...
  if (ordinal <= 211) {
    const safe = runTmwCp011ChapterSafePipeline(input);
    return input.language === "en"
      ? finishEnglish(safe, input.questionLanguageId)
      : finishLocalized(safe, input.questionLanguageId, input.language);
  }
