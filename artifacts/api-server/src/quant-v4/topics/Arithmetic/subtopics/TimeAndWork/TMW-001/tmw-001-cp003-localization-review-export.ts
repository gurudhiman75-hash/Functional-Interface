...
const rows = TMW_CP003_REGISTRY.flatMap((entry) => languages.map((language) => {
  const seed = `tmw-cp003-localization-review:${entry.qlId}`;
...
writeFileSync("dist/quant-v4/tmw-001-cp003-localization-review.json", JSON.stringify(output, null, 2));
