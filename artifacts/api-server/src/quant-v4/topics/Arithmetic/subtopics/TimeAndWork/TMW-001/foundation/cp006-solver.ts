...
export function solveTmwCp006(entry:TmwCp006RegistryEntry,p:TmwCp006Parameters):TmwCp006Solution{
  ...
  switch(entry.solveMode){
    case "findRequiredResourceCount":{
      answer=requiredResources(a,b);
      formulaLatex="N_2=\\frac{N_1D_1H_1E_1}{D_2H_2E_2}\\times\\frac{W_2}{W_1}";
    ...
    case "findRequiredDays":{
      answer=requiredDays(a,b);
      formulaLatex="D_2=\\frac{N_1D_1H_1E_1}{N_2H_2E_2}\\times\\frac{W_2}{W_1}";
    ...
    case "findRequiredDailyHours":{
      answer=requiredHours(a,b);
      formulaLatex="H_2=\\frac{N_1D_1H_1E_1}{N_2D_2E_2}\\times\\frac{W_2}{W_1}";
    ...
    case "findRelativeEfficiency":{
      answer=requiredEfficiency(a,b);
      formulaLatex="E_2=\\frac{N_1D_1H_1E_1}{N_2D_2H_2}\\times\\frac{W_2}{W_1}";
    ...
    case "findWorkQuantity":{
      answer=multiply(a.work,workRatio(a,b));
      formulaLatex="W_2=W_1\\times\\frac{N_2D_2H_2E_2}{N_1D_1H_1E_1}";
    ...
    case "findWorkQuantityRatio":{
      answer=workRatio(a,b);
      formulaLatex="\\frac{W_2}{W_1}=\\frac{N_2D_2H_2E_2}{N_1D_1H_1E_1}";
    ...
    case "findAdditionalWorkersForDeadline":{
      const total=requiredResources(a,b);answer=subtract(total,a.resources);
      formulaLatex="N_{add}=N_{required}-N_{current}";
    ...
    case "findWorkersRemovedForDelay":{
      const retained=requiredResources(a,b);answer=subtract(a.resources,retained);
      formulaLatex="N_{removed}=N_{original}-N_{retained}";
    ...
    case "findOriginalWorkforceFromChangedSchedule":{
      answer=requiredResources(b,a);
      formulaLatex="N_{original}=\\frac{N_{changed}D_{changed}H_{changed}E_{changed}}{D_{planned}H_{planned}E_{planned}}";
    ...
    case "findRemainingDaysFromActualProgress":{
      const elapsed=required(p.elapsedDays,"elapsedDays"),done=required(p.completedFraction,"completedFraction"),daily=divide(done,elapsed),remaining=subtract(rational(1),done);answer=divide(remaining,daily);
      formulaLatex="D_{remaining}=\\frac{1-W_{done}}{W_{done}/D_{elapsed}}";
    ...
    case "findPercentWorkCompletedFromResourceHours":{
      const fraction=divide(capacity(b),capacity(a));answer=multiply(fraction,rational(100));
      formulaLatex="\\%W=\\frac{N_2D_2H_2E_2}{N_1D_1H_1E_1}\\times100";
    ...
    case "findPercentScheduleDelay":{
      const changed=requiredDays(a,b),delay=subtract(changed,a.days);answer=multiply(divide(delay,a.days),rational(100));
      formulaLatex="\\%\\,delay=\\frac{D_{changed}-D_{planned}}{D_{planned}}\\times100";
    ...
    case "findOvertimeHoursForDeadline":{
      const requiredTotal=requiredHours(a,b);answer=subtract(requiredTotal,a.hoursPerDay);
      formulaLatex="H_{overtime}=H_{required}-H_{regular}";
    ...
    case "findShiftCountForProductionTarget":{
      const perShift=multiply(multiply(b.resources,b.hoursPerDay),b.efficiency);answer=divide(b.work,perShift);
      formulaLatex="S=\\frac{Q}{Nq}";
    ...
    case "findDimensionalWorkRatio":{
      const da=product(required(p.dimensionsA,"dimensionsA")),db=product(required(p.dimensionsB,"dimensionsB"));answer=divide(db,da);
      formulaLatex="\\frac{W_2}{W_1}=\\frac{\\prod d_{2i}}{\\prod d_{1i}}";
    ...
    case "findWorkersForChangedDimensions":{
      const da=product(required(p.dimensionsA,"dimensionsA")),db=product(required(p.dimensionsB,"dimensionsB")),ratio=divide(db,da);answer=requiredResources(a,{...b,work:ratio});
      formulaLatex="N_2=N_1\\times\\frac{D_1H_1}{D_2H_2}\\times\\frac{W_2}{W_1}";
    ...
    case "findDaysForChangedDimensions":{
      const da=product(required(p.dimensionsA,"dimensionsA")),db=product(required(p.dimensionsB,"dimensionsB")),ratio=divide(db,da);answer=requiredDays(a,{...b,work:ratio});
      formulaLatex="D_2=D_1\\times\\frac{N_1H_1}{N_2H_2}\\times\\frac{W_2}{W_1}";
    ...
    case "findResourceDurationAfterPopulationChange":{
      const p1=required(p.initialPopulation,"initialPopulation"),p2=required(p.changedPopulation,"changedPopulation"),elapsed=required(p.elapsedBeforePopulationChange,"elapsedBeforePopulationChange"),remainingStock=multiply(p1,subtract(a.days,elapsed));answer=divide(remainingStock,p2);
      formulaLatex="D_{remaining}=\\frac{P_1(D_1-D_{elapsed})}{P_2}";
    ...
    case "findCompletionTimeAfterAbsenteeism":{
      const absent=required(p.absentPercent,"absentPercent"),active=multiply(a.resources,subtract(rational(1),divide(absent,rational(100))));answer=divide(multiply(a.resources,a.days),active);
      formulaLatex="D_{actual}=\\frac{N_{planned}D_{planned}}{N_{active}}";
    ...
    case "findCompletionWithBatchWorkerAdditions":{
      const initial=required(p.initialBatchResources,"initialBatchResources"),addition=required(p.batchAddition,"batchAddition"),total=multiply(a.resources,a.days);answer=solveBatchDays(p);
      formulaLatex="W=\\frac{n}{2}\\left[2a+(n-1)b\\right]";
    ...
    case "findEquivalentResourceTime":{
      answer=capacity(a);
      formulaLatex="R_{equivalent}=NDH";
      workedLatex=[`R_{equivalent}=${toLatex(a.resources)}\\times${toLatex(a.days)}\\times${toLatex(a.hoursPerDay)}=${toLatex(answer)}`];break;
    }
  }
...
export function verifyTmwCp006(entry:TmwCp006RegistryEntry,p:TmwCp006Parameters,solution:TmwCp006Solution):boolean{
  ...
  switch(entry.solveMode){
    ...
    case "findEquivalentResourceTime":return equals(x,capacity(a));
  }
}
