...
import type { TmwCp010GeneratedQuestion, TmwCp010Parameters } from './cp010-types';
...
export function runTmwCp010Pipeline(input:{questionLanguageId:string;seed:string;language?:'en'|'hi'|'pa'}):TmwCp010GeneratedQuestion{
 ...
 const entry=getTmwCp010Entry(input.questionLanguageId),parameters=buildTmwCp010Parameters(entry,input.seed),solution=solveTmwCp010(entry,parameters),optionSet=buildTmwCp010Options(entry,parameters,solution,input.seed),stem=renderTmwCp010Stem(entry,parameters),formula=inline(solution.formulaLatex),steps=solution.workedLatex.map(inline),opening=tmwCp010Opening(entry),givens=tmwCp010Givens(entry,parameters),shortcut=tmwCp010Shortcut(entry,parameters,solution),commonTrap=tmwCp010CommonTrap(optionSet.options,optionSet.correctIndex),conclusion=tmwCp010Conclusion(entry,parameters,solution.answerText),errors:string[]=[];
