...
for (const entry of TMW_CP004_REGISTRY) {
  for (let index = 0; index < 12; index += 1) {
    ...
    for (const language of languages) {
      ...
      assert.equal(/भागीदारी (?:शुरू|समाप्त)|ਭਾਗੀਦਾਰੀ (?:ਸ਼ੁਰੂ|ਖਤਮ)/.test(learnerText), false);
      assert.equal(/सक्रिय सदस्य|ठीक शेष काम|ਸਰਗਰਮ ਮੈਂਬਰ|ਸਹੀ ਬਾਕੀ ਕੰਮ/.test(learnerText), false);
      assert.equal(/बदली हुई चरणबद्ध स्थिति|संदर्भ स्थिति|ਬਦਲੀ ਹੋਈ ਪੜਾਅਵਾਰ ਸਥਿਤੀ|ਹਵਾਲਾ ਸਥਿਤੀ/.test(learnerText), false);

      if (entry.qlId === "TMW-QL-059") {
        assert.equal(/कार्य-दल बदल|ਟੀਮ ਬਦਲ/.test(question.stem), false);
        assert.match(question.explanation.opening, language === "hi" ? /दिए समय में दोनों की संयुक्त दर/ : /ਦਿੱਤੇ ਸਮੇਂ ਵਿੱਚ ਦੋਵਾਂ ਦੀ ਸਾਂਝੀ ਦਰ/);
      }
      if (["TMW-QL-061", "TMW-QL-064", "TMW-QL-067", "TMW-QL-069", "TMW-QL-080"].includes(entry.qlId)) {
        assert.match(question.stem, language === "hi" ? /काम से हटा/ : /ਕੰਮ ਤੋਂ ਹਟਾ/);
      }
      if (["TMW-QL-062", "TMW-QL-063", "TMW-QL-065", "TMW-QL-066", "TMW-QL-081"].includes(entry.qlId)) {
        assert.match(question.stem, language === "hi" ? /काम में लगा|काम में लगाया/ : /ਕੰਮ ਵਿੱਚ ਲਾ|ਕੰਮ ਵਿੱਚ ਲਾਇਆ/);
      }
      if (entry.qlId === "TMW-QL-066") {
        assert.match(question.explanation.opening, language === "hi" ? /जुड़ने का समय x/ : /ਜੁੜਨ ਦਾ ਸਮਾਂ x/);
        assert.match(question.explanation.conclusion, language === "hi" ? /काम में लगाया गया/ : /ਕੰਮ ਵਿੱਚ ਲਾਇਆ ਗਿਆ/);
      }
      if (entry.qlId === "TMW-QL-067") {
        assert.match(question.explanation.opening, language === "hi" ? /हटाने का समय x/ : /ਹਟਾਉਣ ਦਾ ਸਮਾਂ x/);
      ...
      if (entry.qlId === "TMW-QL-069") {
        assert.match(question.explanation.opening, language === "hi" ? /अंतिम चरण का समय/ : /ਆਖ਼ਰੀ ਪੜਾਅ ਦਾ ਸਮਾਂ/);
      }
      ...
      if (entry.qlId === "TMW-QL-076") {
        assert.match(question.explanation.opening, language === "hi" ? /लक्षित काम के भाग/ : /ਟੀਚੇ ਵਾਲੇ ਕੰਮ ਦੇ ਹਿੱਸੇ/);
      }
      ...
      if (["TMW-QL-078", "TMW-QL-079"].includes(entry.qlId)) {
        assert.match(question.explanation.opening, language === "hi" ? /आवश्यक कुल कर्मचारी/ : /ਲੋੜੀਂਦੇ ਕੁੱਲ ਕਰਮਚਾਰੀ/);
      }
