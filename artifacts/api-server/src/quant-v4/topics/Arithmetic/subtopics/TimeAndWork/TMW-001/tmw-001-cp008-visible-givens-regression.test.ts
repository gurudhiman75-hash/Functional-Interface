import { equals, formatRational, toLatex } from "./foundation/rational";
...
for (const language of languages) for (const namespace of namespaces) for (const suffix of seeds) {
  ...
  if (equals(a.efficiency, b.efficiency)) {
    ...
    unequalEfficiencyCases += 1;
    const rateA = formatRational(a.efficiency);
    const rateB = formatRational(b.efficiency);
    assert(q.stem.includes(rateA) && q.stem.includes(rateB), `${label}: unequal rate values ${rateA} and ${rateB} are not both visible in the stem`);
  }
}
...
assert(unequalEfficiencyCases > 0, "Expected at least one unequal-efficiency QL148 case");
console.log(JSON.stringify({ chapter:"TMW-001", checkpoint:"TMW-CP-008", ql:"TMW-QL-148", checked, equalEfficiencyCases, unequalEfficiencyCases, publicationLocked:true, verdict:"PASS" }, null, 2));
