...
export function renderTmwCp010Stem(entry:TmwCp010RegistryEntry,p:TmwCp010Parameters):string{
 ...
 switch(entry.solveMode){
  case'findCompletionAfterDelayedActivation':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${stageCapabilities(p)} Staged schedule: for ${timeText(required(stages[0].duration,'delay'))}, ${compactArrangement(stages[0].pipes)} operates. Then ${activationPhrase(stages)}, and ${compactArrangement(stages[1].pipes)} continues until the tank is ${target(p)}. Total time from the start?`;
  case'findCompletionAfterDelayedDeactivation':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${stageCapabilities(p)} Staged schedule: for ${timeText(required(stages[0].duration,'delay'))}, ${compactArrangement(stages[0].pipes)} operates. Then ${deactivationPhrase(stages)}, and ${compactArrangement(stages[1].pipes)} continues until the tank is ${target(p)}. Total time from the start?`;
  case'findCompletionWithMultipleStaggeredEvents':return `${c.tankLabel} starts empty. ${stageCapabilities(p)} Schedule: ${compactStages(stages)} Continue the final arrangement until the tank is full. Total time?`;
  case'findCompletionWithInterruptedFlow':return `${c.tankLabel} starts empty. ${stageCapabilities(p)} Schedule: ${compactArrangement(stages[0].pipes)} for ${timeText(required(stages[0].duration,'first duration'))}; an interruption stops all flow for ${timeText(required(stages[1].duration,'idle duration'))}; then ${compactArrangement(stages[2].pipes)} resumes until the tank is full. Total time?`;
  case'findCompletionFromPartialLevelAndStages':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${stageCapabilities(p)} Schedule: ${compactArrangement(stages[0].pipes)} for ${timeText(required(stages[0].duration,'duration'))}; then ${compactArrangement(stages[1].pipes)} until the tank is full. Total time?`;
  ...
  case'findCompletionAfterThresholdSwitch':return `${c.tankLabel} starts empty. ${stageCapabilities(p)} ${compactArrangement(stages[0].pipes)} operates until level ${levelText(required(p.thresholdLevel,'threshold'))}; a level sensor then switches to ${compactArrangement(stages[1].pipes)} until the tank is full. Total time?`;
  ...
  case'findCapacityFromStagedPhysicalFlows':return `An empty ${c.tankLabel} follows this physical-flow schedule: ${required(p.physicalStages,'physical stages').map((stage,index)=>`${index+1}) ${formatRational(stage.netFlowLitresPerHour)} litres/hour for ${timeText(stage.duration)}`).join('; ')}. The tank is exactly full at the end. Capacity?`;
  case'findCompletionWithAlternatingPipes':return `${c.tankLabel} starts empty. ${cycleCapabilities(p)} Repeating cycle: ${compactCycle(cycle)}. Start with part 1. Time until the tank is full?`;
  case'findCompletionWithPeriodicSchedule':return `${c.tankLabel} starts empty. ${cycleCapabilities(p)} Repeating cycle: ${compactCycle(cycle)}. It repeats without a gap. Time until the tank is full?`;
  ...
  case'findCompletionFromArbitraryCyclePhase':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${cycleCapabilities(p)} Repeating cycle: ${compactCycle(cycle)}. Start at part ${(p.startingCycleIndex??0)+1}, then follow normal order. Time until the tank is full?`;
  ...
  case'findBoundaryEventTimeUnderSchedule':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${cycleCapabilities(p)} Repeating cycle: ${compactCycle(cycle)}. When does the tank first become ${target(p)}?`;
