...
export function renderTmwCp010Stem(entry:TmwCp010RegistryEntry,p:TmwCp010Parameters):string{
 ...
 switch(entry.solveMode){
  case'findCompletionAfterDelayedActivation':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${stageCapabilities(p)} For ${timeText(required(stages[0].duration,'delay'))}, ${compactArrangement(stages[0].pipes)} operates. Then ${activationPhrase(stages)}, and ${compactArrangement(stages[1].pipes)} continues until ${target(p)}. Total time from the start?`;
  case'findCompletionAfterDelayedDeactivation':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${stageCapabilities(p)} For ${timeText(required(stages[0].duration,'delay'))}, ${compactArrangement(stages[0].pipes)} operates. Then ${deactivationPhrase(stages)}, and ${compactArrangement(stages[1].pipes)} continues until ${target(p)}. Total time from the start?`;
  case'findCompletionWithMultipleStaggeredEvents':return `${c.tankLabel} starts empty. ${stageCapabilities(p)} Schedule: ${compactStages(stages)} Continue the final arrangement until full. Total time?`;
  ...
  case'findCompletionFromPartialLevelAndStages':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${stageCapabilities(p)} Schedule: ${compactArrangement(stages[0].pipes)} for ${timeText(required(stages[0].duration,'duration'))}; then ${compactArrangement(stages[1].pipes)} until full. Total time?`;
  case'findFinalLevelAfterStagedSchedule':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${stageCapabilities(p)} Schedule: ${compactStages(stages)} What fraction is full at the end?`;
  case'findCompletionAfterThresholdSwitch':return `${c.tankLabel} starts empty. ${stageCapabilities(p)} ${compactArrangement(stages[0].pipes)} operates until level ${levelText(required(p.thresholdLevel,'threshold'))}; a level sensor then switches to ${compactArrangement(stages[1].pipes)} until full. Total time?`;
  ...
  case'findRequiredFinalStageRate':return `An empty ${c.tankLabel} must be full in ${timeText(required(p.knownCompletionTime,'completion time'))}. ${capabilitySentences(uniquePipes([stages[0].pipes]),c.tankLabel)} Staged schedule: ${compactArrangement(stages[0].pipes)} for ${timeText(required(stages[0].duration,'first duration'))}; then a final inlet works alone. What fraction per hour must it fill?`;
  case'findCapacityFromStagedPhysicalFlows':return `An empty ${c.tankLabel} follows this physical-flow schedule: ${required(p.physicalStages,'physical stages').map((stage,index)=>`${index+1}) ${formatRational(stage.netFlowLitresPerHour)} litres/hour for ${timeText(stage.duration)}`).join('; ')}. It is exactly full at the end. Capacity?`;
  case'findCompletionWithAlternatingPipes':return `${c.tankLabel} starts empty. ${cycleCapabilities(p)} Repeating cycle: ${compactCycle(cycle)}. Start with part 1. Time until full?`;
  case'findCompletionWithPeriodicSchedule':return `${c.tankLabel} starts empty. ${cycleCapabilities(p)} Repeating cycle: ${compactCycle(cycle)}. It repeats without a gap. Time until full?`;
  case'findAutomaticLevelControlCompletion':{
   ...
   return `A controller keeps ${c.tankLabel} between ${levelText(lc.lower)} and ${levelText(lc.upper)}. ${compactCapabilities(pipes)} Starting at the upper mark, ${compactArrangement(lc.offPipes)} runs to the lower mark; then ${compactArrangement(lc.onPipes)} returns it to the upper mark. Time to reach the upper mark for the ${lc.targetUpperHits}${lc.targetUpperHits===2?'nd':'rd'} subsequent time?`;
  }
  case'findCompletionFromArbitraryCyclePhase':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${cycleCapabilities(p)} Repeating cycle: ${compactCycle(cycle)}. Start at part ${(p.startingCycleIndex??0)+1}, then follow normal order. Time until full?`;
  case'findFullCycleCountToBoundary':return `${c.tankLabel} starts empty. ${cycleCapabilities(p)} Repeating cycle: ${compactCycle(cycle)}. How many complete cycles finish before the tank fills during the terminal cycle?`;
  case'findTerminalActiveSegment':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${cycleCapabilities(p)} Repeating cycle: ${compactCycle(cycle)}. In which segment does it first become full?`;
  case'findBoundaryEventTimeUnderSchedule':return `${c.tankLabel} starts ${levelText(p.initialLevel)}. ${cycleCapabilities(p)} Repeating cycle: ${compactCycle(cycle)}. When does it first become ${target(p)}?`;
