...
const rows = TMW_CP006_REGISTRY.flatMap((entry) => languages.map((language) => {
  const seed = `tmw-cp006-localization-review:${entry.qlId}`;
...
writeFileSync("dist/quant-v4/tmw-001-cp006-localization-review.json", JSON.stringify(output, null, 2));
