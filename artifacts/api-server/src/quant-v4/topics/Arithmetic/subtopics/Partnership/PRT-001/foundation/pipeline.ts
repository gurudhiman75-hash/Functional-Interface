...
import { generatePrt001E13Parameters, isPrt001E13QuestionLanguageId } from "./e13-parameter-generator";
...
import { renderPrt001E13Explanation } from "./e13-explanation-renderer";
...
import { independentlySolvePrt001E13Task, solvePrt001E13Task } from "./e13-task-solver";
...
export function runPrt001PilotPipeline(input: { questionLanguageId?: string; seed?: string; language?: Prt001Language } = {}): Prt001QuestionPackage {
  ...
  const isE13 = isPrt001E13QuestionLanguageId(questionLanguageId);
  const isE8 = !isE13 && isPrt001E8QuestionLanguageId(questionLanguageId);
  const isE5 = !isE13 && !isE8 && isPrt001E5SolveMode(entry.solveMode);
  const isE4 = !isE13 && !isE8 && !isE5 && isPrt001E4SolveMode(entry.solveMode);
  const isE3A = !isE13 && !isE8 && !isE5 && !isE4 && isPrt001E3ASolveMode(entry.solveMode);
  const isE3B = !isE13 && !isE8 && !isE5 && !isE4 && !isE3A && isPrt001E3BSolveMode(entry.solveMode);
  const isE3 = isE3A || isE3B;
  const isE2 = !isE13 && !isE8 && !isE5 && !isE4 && !isE3 && isPrt001E2SolveMode(entry.solveMode);
  const isE1 = !isE13 && !isE8 && !isE5 && !isE4 && !isE3 && !isE2 && isPrt001E1SolveMode(entry.solveMode);
  const generatedParameters = isE13
    ? generatePrt001E13Parameters({ questionLanguageId, seed, entry, language })
    : isE8
  ...
  const taskAnswer = isE13
    ? solvePrt001E13Task(parameters, solution)
    : isE8
  ...
  const independentTaskAnswer = isE13
    ? independentlySolvePrt001E13Task(parameters, verification)
    : isE8
  ...
  const rawExplanationLines = isE13
    ? renderPrt001E13Explanation({ parameters, solution, answer: taskAnswer })
    : isE8
  ...
  const expansionWave = isE13 ? "E13" : isE8 ? "E8" : isE5 ? "E5" : isE4 ? "E4" : isE3 ? "E3" : isE2 ? "E2" : isE1 ? "E1" : "BASELINE";
