...
import { GEO_PHASE4_TEMPORARY_PROTOTYPES } from "./phase4-registry";

function jsonStringify(value: unknown): string {
  return JSON.stringify(value, (_key, item) => typeof item === "bigint" ? item.toString() : item, 2);
}

const outputDirectory = resolve(process.cwd(), "dist/quant-v4/geometry-phase4-review");
mkdirSync(outputDirectory, { recursive: true });
const questions = GEO_PHASE4_TEMPORARY_PROTOTYPES.flatMap((prototype) =>
  Array.from({ length: 3 }, (_, index) => prototype.generate(`geo-phase4-review:${prototype.temporaryPrototypeId}:${index}`)),
);

writeFileSync(resolve(outputDirectory, "geometry-phase4-review.json"), `${jsonStringify({
  status: "DISCOVERY_REVIEW_CANDIDATE_CIRCLE_WAVE1",
...
const markdown = [
  "# ExamTree Geometry — Phase 4 Circle Temporary Prototype Review",
  "",
  "**Status:** executable discovery Circle Wave 1; external source audit remains open.",
  "",
  "**Permanent QLs:** 0",
  "",
...
writeFileSync(resolve(outputDirectory, "geometry-phase4-review.md"), `${markdown}\n`, "utf8");

console.log(JSON.stringify({
  status: "EXPORTED_GEO_PHASE4_REVIEW",
  temporaryPrototypeCount: GEO_PHASE4_TEMPORARY_PROTOTYPES.length,
  questionCount: questions.length,
  outputDirectory,
}));
