...
import type {
  DiagramDisposition,
  GapWave2CheckpointId,
  GapWave2Question,
  GapWave2VerifierResult,
} from "./wave2-types";
...
function fingerprint(parts: readonly string[]): string {
  return `GEO-GAP-W2-${hashText(parts.join("|")).toString(16).padStart(8, "0")}`;
}
...
export function remediationVerifier(
  oracle: GapWave2VerifierResult["oracle"],
  passed: boolean,
...
export function finalizeGapWave2Question(input: Readonly<{
  cpId: GapWave2CheckpointId;
  temporaryPrototypeId: string;
  sourceGapId: string;
  sourceEvidenceIds: readonly GapWave2SourceEvidenceId[];
  solveMode: string;
  ...
  proofEvents: GapWave2Question["proofEvents"];
  displayedClueIds: readonly string[];
  minimalityProof: ClueMinimalityProof;
  independentVerifierResult: GapWave2VerifierResult;
  ...
  return Object.freeze({
    ...
    sourceStatus: "EXTERNAL_SOURCE_AUDIT_WAVE2__GAP_REMEDIATION" as const,
