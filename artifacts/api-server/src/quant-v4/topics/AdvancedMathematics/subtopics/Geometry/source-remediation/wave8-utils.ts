...
import type { GapWave8Question, GapWave8VerifierResult } from "./wave8-types";
...
function fingerprint(parts: readonly string[]): string {
  return `GEO-GAP-W8-${hashText(parts.join("|")).toString(16).padStart(8, "0")}`;
}

export function wave8Verifier(passed: boolean, checks: readonly string[]): GapWave8VerifierResult {
  return Object.freeze({ oracle: "INDEPENDENT_DEFINITION_CHECK", passed, checks: Object.freeze([...checks]) });
}

export function finalizeGapWave8Question(input: Readonly<{
  temporaryPrototypeId: string;
  sourceGapId: string;
  sourceEvidenceIds: readonly GapWave8SourceEvidenceId[];
  solveMode: string;
  ...
  independentVerifierResult: GapWave8VerifierResult;
}>): GapWave8Question {
  ...
  return Object.freeze({
    ...
    sourceStatus: "EXTERNAL_SOURCE_AUDIT_WAVE8__GAP_REMEDIATION",
