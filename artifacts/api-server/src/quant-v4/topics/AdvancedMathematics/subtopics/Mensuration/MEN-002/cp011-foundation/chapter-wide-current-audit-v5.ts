...
export const MEN_CP011_CURRENT_CHAPTER_AUDIT_AUTHORITY =
  "MEN-CP011-CHAPTER-WIDE-COVERAGE-ENGLISH-AUDIT-V5" as const;
...
const BLOCKERS = [
  "ADDITIONAL_AXIS_DISCOVERY_PENDING",
...
const waves = [
  {
    ...
    records: generateMenCp011ReviewBatch(
      "men-cp011-current-audit-foundation-v5",
      12,
    ).records as Question[],
  },
  {
    ...
    records: generateMenCp011SurfaceReviewBatch(
      "men-cp011-current-audit-surface-v5",
      12,
    ).records as Question[],
  },
  {
    ...
    records: generateMenCp011OpenContainerReviewBatch(
      "men-cp011-current-audit-open-container-v5",
      16,
    ).records as Question[],
  },
...
const unresolvedInitialPrototypes = initialStatusEntries
...
const audit = {
  ...
  implementedOrOwnedInitialDiscoveryPrototypeCount:
    initialStatusEntries.filter(([, status]) => status !== "MISSING").length,
  unresolvedInitialDiscoveryPrototypeCount: unresolvedInitialPrototypes.length,
  unresolvedInitialPrototypes,
  initialDiscoveryStatus: INITIAL_DISCOVERY_STATUS,
  initialArchitectureListComplete: unresolvedInitialPrototypes.length === 0,
...
assert.equal(audit.generatedReviewRecordCount, 360);
assert.equal(audit.runtimePrototypeCount, 24);
assert.equal(new Set(runtimePrototypeIds).size, 24);
...
assert.equal(audit.uniqueExactStemCount, 360);
assert.equal(audit.uniqueQuestionOptionPackageCount, 360);
assert.deepEqual(answerPositionCounts, { A: 90, B: 90, C: 90, D: 90 });
assert.equal(audit.technicallyCleanLearnerRecords, 360);
...
const reviewRows = records.map((question, index) => ({
  reviewId: `MEN-CP011-CURRENT-AUDIT-V5-${String(index + 1).padStart(3, "0")}`,
...
const reviewMarkdown = `# MEN-CP-011 Current Chapter-Wide Coverage and English Audit V5\n\n` +
  `## Verdict\n\n` +
  `The initial 20-candidate MEN-CP-011 architecture list is now fully implemented, covered by equivalent runtime authorities, or explicitly reassigned. The live system contains **${audit.runtimePrototypeCount} direct runtime families** and **${audit.generatedReviewRecordCount} current review records**. Chapter freeze is still not granted because additional discovery axes, direct source normalisation and human English review remain pending.\n\n` +
  `## Current metrics\n\n\`\`\`text\n` +
  ...
  `Initial candidates unresolved:      ${audit.unresolvedInitialDiscoveryPrototypeCount}\n` +
  `Permanent QLs:                      0\n` +
  `Publication eligible:               false\n` +
  `\`\`\`\n\n` +
  `## Implemented runtime families\n\n${prototypeSummary}\n\n` +
  `## Remaining work beyond the initial list\n\n` +
  `- Conical-shell ownership and executable discovery.\n` +
...
console.log(JSON.stringify({
  ...
  unresolvedInitialDiscoveryPrototypeCount:
    audit.unresolvedInitialDiscoveryPrototypeCount,
  initialArchitectureListComplete: audit.initialArchitectureListComplete,
  ratioPercentSlice: audit.sliceAudits.ratioPercentAnswerPositions,
