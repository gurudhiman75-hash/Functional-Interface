...
export const MEN_CP011_CURRENT_CHAPTER_AUDIT_AUTHORITY =
  "MEN-CP011-CHAPTER-WIDE-COVERAGE-ENGLISH-AUDIT-V2" as const;
...
interface WaveRecords {
  wave: string;
  records: Question[];
}

const foundationBatch = generateMenCp011ReviewBatch(
  "men-cp011-current-audit-foundation-v2",
  12,
);
const surfaceBatch = generateMenCp011SurfaceReviewBatch(
  "men-cp011-current-audit-surface-v2",
  12,
);
const openContainerBatch = generateMenCp011OpenContainerReviewBatch(
  "men-cp011-current-audit-open-container-v2",
  16,
);
const inverseBatch = generateMenCp011InverseReviewBatch();
const hollowBoxBatch = generateMenCp011HollowBoxReviewBatch();
const shellBatch = generateMenCp011ShellReviewBatch();
const shellAudit = auditMenCp011ShellBatch(shellBatch.records);

const waves: WaveRecords[] = [
  { wave: "PIPE_MATERIAL_AND_INVERSE_CORE", records: foundationBatch.records as Question[] },
  { wave: "PIPE_SURFACE_EXPOSURE", records: surfaceBatch.records as Question[] },
  { wave: "OPEN_CONTAINER_EXPOSURE", records: openContainerBatch.records as Question[] },
  { wave: "INVERSE_THICKNESS_AND_LENGTH", records: inverseBatch.records as Question[] },
  { wave: "HOLLOW_CUBE_AND_CUBOID", records: hollowBoxBatch.records as Question[] },
  { wave: "SPHERICAL_AND_HEMISPHERICAL_SHELLS", records: shellBatch.records as Question[] },
];
...
function technicallyClean(question: Question) {
  ...
  return !text.includes("\\pih") &&
    ...
    !/\[(?:USED_|CALCULATED_|ADDED_|OMITTED_|RETURNED_|COPIED_|MEN-CP011-PROT-|FALLBACK_|UNCLASSIFIED)/.test(text) &&
...
function physicalStateSignature(question: Question) {
  ...
  const materialEntries = Object.entries(state)
...
const audit = {
  ...
  coverageAxes: {
    ...
    costLiningRateCountRatioPercentApplications: "MISSING",
...
assert.equal(audit.generatedReviewRecordCount, 264);
assert.equal(audit.runtimePrototypeCount, 18);
assert.equal(new Set(runtimePrototypeIds).size, 18);
...
assert.equal(audit.uniqueExactStemCount, 264);
assert.equal(audit.uniqueQuestionOptionPackageCount, 264);
assert.deepEqual(answerPositionCounts, { A: 66, B: 66, C: 66, D: 66 });
assert.equal(audit.technicallyCleanLearnerRecords, 264);
assert.equal(audit.initialDiscoveryPrototypeCount, 20);
assert.equal(audit.implementedOrOwnedInitialDiscoveryPrototypeCount, 14);
assert.equal(audit.missingInitialDiscoveryPrototypeCount, 6);
...
assert.ok(stemWordCounts.every((count) => count >= 8 && count <= 120));
assert.ok(solutionWordCounts.every((count) => count >= 15 && count <= 280));
...
const reviewRows = records.map((question, index) => ({
  reviewId: `MEN-CP011-CURRENT-AUDIT-${String(index + 1).padStart(3, "0")}`,
...
const reviewMarkdown = `# MEN-CP-011 Current Chapter-Wide Coverage and English Audit\n\n` +
  `## Verdict\n\n` +
  `The current executable MEN-CP-011 system contains **${audit.runtimePrototypeCount} direct runtime families** and **${audit.generatedReviewRecordCount} current review records**. Automated mathematical, option, lifecycle and technical-English checks pass across the complete current corpus. Chapter coverage and English remain unfrozen because six initial application/exposure candidates, conical shells, direct source normalisation and human review remain outstanding.\n\n` +
  `## Current metrics\n\n\`\`\`text\n` +
  ...
  `## Coverage notes\n\n` +
  `- Open-cylinder exposure, inverse pipe thickness/length, hollow cube/cuboid, spherical shell and hemispherical shell families are now executable.\n` +
  `- Direct open-top cuboid sheet area remains correctly owned by the existing MEN-CP-007 authority rather than duplicated.\n` +
  `- Declared \`π = 3.14\` is now proved exactly as \`157/50\` in the shell slice.\n` +
  `- Joined/placed-solid exposure, cost/lining, ratio and percentage applications remain missing.\n` +
  `- Conical shells remain an additional uncovered axis outside the initial 20-candidate list.\n` +
...
console.log(JSON.stringify({
  ...
  shellPiPolicyCounts: audit.shellSlicePiPolicyAudit,
