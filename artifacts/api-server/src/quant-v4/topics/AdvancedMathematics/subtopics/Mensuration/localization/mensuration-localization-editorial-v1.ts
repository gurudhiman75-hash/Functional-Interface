...
export function repairMensurationLocalizedEditorialProse(text: string, language: MensurationLocalizedLanguage) {
  ...
  if (language === "hi") {
    return out
      .replace(/एक\s+(त्रिभुजाकार\s+[^.?!]+?)\s+है\s+एक\s+आधार\s+का\s+(.+?)\s+और\s+एक\s+लम्बवत\s+ऊँचाई\s+का\s+(.+?)(?=[.?!])/g, "एक $1 का आधार $2 और लम्बवत ऊँचाई $3 है")
      .replace(/आधार\s+और\s+संबंधित\s+ऊँचाई\s+का\s+एक\s+त्रिभुजाकार\s+बैनर\s+हैं\s+(.+?)\s+और\s+(.+?)\s+क्रमशः\.\s*ज्ञात करें\s+क्षेत्रफल\s+का\s+बैनर\./g, "एक त्रिभुजाकार बैनर का आधार $1 और संबंधित ऊँचाई $2 है. बैनर का क्षेत्रफल ज्ञात कीजिए.")
      .replace(/एक\s+त्रिभुजाकार\s+काँच\s+की\s+पट्टी\s+का\s+क्षेत्रफल\s+(.+?)\s+और\s+आधार\s+(.+?)\.\s*क्या\s+है\s+उसका\s+लम्बवत\s+ऊँचाई\?/g, "एक त्रिभुजाकार काँच की पट्टी का क्षेत्रफल $1 और आधार $2 है. उसकी लम्बवत ऊँचाई ज्ञात कीजिए.")
      .replace(/एक\s+आयताकार\s+([^.?!]+?)\s+है\s+(.+?)\s+लंबा\s+और\s+(.+?)\s+चौड़ा(?=[.?!])/g, "एक आयताकार $1 की लंबाई $2 और चौड़ाई $3 है")
      .replace(/क्षेत्रफल\s+का\s+एक\s+(समबाहु\s+त्रिभुजाकार\s+[^.?!]+?)\s+है\s+(.+?)(?=[.?!])/g, "एक $1 का क्षेत्रफल $2 है")
      .replace(/है\s+गया\s+प्रयोग किया/g, "प्रयोग किया गया है")
      .replace(/है\s+गया\s+हटाया/g, "हटा दिया गया है")
      .replace(/है\s+गया\s+लगाया/g, "लगाया गया है")
      .replace(/है\s+गया\s+बदला/g, "बदला गया है")
      .replace(/है\s+गया/g, "गया है")
      .replace(/हैं\s+गया/g, "गए हैं")
      .replace(/\s{2,}/g, " ");
  }
  return out
    .replace(/ਇੱਕ\s+(ਤਿਕੋਣਾ\s+[^.?!]+?)\s+ਹੈ\s+ਇੱਕ\s+ਆਧਾਰ\s+ਦਾ\s+(.+?)\s+ਅਤੇ\s+ਇੱਕ\s+ਲੰਬ\s+ਉਚਾਈ\s+ਦਾ\s+(.+?)(?=[.?!])/g, "ਇੱਕ $1 ਦਾ ਆਧਾਰ $2 ਅਤੇ ਲੰਬ ਉਚਾਈ $3 ਹੈ")
    .replace(/ਆਧਾਰ\s+ਅਤੇ\s+ਸੰਬੰਧਤ\s+ਉਚਾਈ\s+ਦਾ\s+ਇੱਕ\s+ਤਿਕੋਣਾ\s+ਬੈਨਰ\s+ਹਨ\s+(.+?)\s+ਅਤੇ\s+(.+?)\s+ਕ੍ਰਮਵਾਰ\.\s*ਪਤਾ ਕਰੋ\s+ਖੇਤਰਫਲ\s+ਦਾ\s+ਬੈਨਰ\./g, "ਇੱਕ ਤਿਕੋਣਾ ਬੈਨਰ ਦਾ ਆਧਾਰ $1 ਅਤੇ ਸੰਬੰਧਤ ਉਚਾਈ $2 ਹੈ. ਬੈਨਰ ਦਾ ਖੇਤਰਫਲ ਪਤਾ ਕਰੋ.")
    .replace(/ਇੱਕ\s+ਤਿਕੋਣੀ\s+ਕੱਚ\s+ਦੀ\s+ਪੱਟੀ\s+ਦਾ\s+ਖੇਤਰਫਲ\s+(.+?)\s+ਅਤੇ\s+ਆਧਾਰ\s+(.+?)\.\s*ਕੀ\s+ਹੈ\s+ਇਸ\s+ਦਾ\s+ਲੰਬ\s+ਉਚਾਈ\?/g, "ਇੱਕ ਤਿਕੋਣੀ ਕੱਚ ਦੀ ਪੱਟੀ ਦਾ ਖੇਤਰਫਲ $1 ਅਤੇ ਆਧਾਰ $2 ਹੈ. ਇਸ ਦੀ ਲੰਬ ਉਚਾਈ ਪਤਾ ਕਰੋ.")
    .replace(/ਇੱਕ\s+ਆਇਤਾਕਾਰ\s+([^.?!]+?)\s+ਹੈ\s+(.+?)\s+ਲੰਮਾ\s+ਅਤੇ\s+(.+?)\s+ਚੌੜਾ(?=[.?!])/g, "ਇੱਕ ਆਇਤਾਕਾਰ $1 ਦੀ ਲੰਬਾਈ $2 ਅਤੇ ਚੌੜਾਈ $3 ਹੈ")
    .replace(/ਖੇਤਰਫਲ\s+ਦਾ\s+ਇੱਕ\s+(ਸਮਭੁਜ\s+ਤਿਕੋਣਾ\s+[^.?!]+?)\s+ਹੈ\s+(.+?)(?=[.?!])/g, "ਇੱਕ $1 ਦਾ ਖੇਤਰਫਲ $2 ਹੈ")
    .replace(/ਹੈ\s+ਗਿਆ\s+ਵਰਤਿਆ/g, "ਵਰਤਿਆ ਗਿਆ ਹੈ")
    .replace(/ਹੈ\s+ਗਿਆ\s+ਹਟਾਇਆ/g, "ਹਟਾਇਆ ਗਿਆ ਹੈ")
    .replace(/ਹੈ\s+ਗਿਆ\s+ਲਗਾਇਆ/g, "ਲਗਾਇਆ ਗਿਆ ਹੈ")
    .replace(/ਹੈ\s+ਗਿਆ\s+ਬਦਲਿਆ/g, "ਬਦਲਿਆ ਗਿਆ ਹੈ")
    .replace(/ਹੈ\s+ਗਿਆ/g, "ਗਿਆ ਹੈ")
    .replace(/ਹਨ\s+ਗਿਆ/g, "ਗਏ ਹਨ")
    .replace(/\s{2,}/g, " ");
}

/** Translate the only currently observed residual learner option noun. */
export function polishMensurationLocalizedOption(text: string, language: MensurationLocalizedLanguage) {
  const protectedMath = protectMathSpans(text);
  const words = language === "hi"
    ? { coin: "सिक्का", coins: "सिक्के" }
    : { coin: "ਸਿੱਕਾ", coins: "ਸਿੱਕੇ" };
  const localized = protectedMath.text.replace(/\b(?:coin|coins)\b/gi, (word) =>
    words[word.toLowerCase() as "coin" | "coins"],
  );
  return protectedMath.restore(localized);
}
