...
export const MEN_CP011_CURRENT_CHAPTER_AUDIT_AUTHORITY =
  "MEN-CP011-CHAPTER-WIDE-COVERAGE-ENGLISH-AUDIT-V4" as const;
...
const waves = [
  {
    ...
    records: generateMenCp011ReviewBatch(
      "men-cp011-current-audit-foundation-v4",
      12,
    ).records as Question[],
  },
  {
    ...
    records: generateMenCp011SurfaceReviewBatch(
      "men-cp011-current-audit-surface-v4",
      12,
    ).records as Question[],
  },
  {
    ...
    records: generateMenCp011OpenContainerReviewBatch(
      "men-cp011-current-audit-open-container-v4",
      16,
    ).records as Question[],
  },
  {
...
const audit = {
  ...
  coverageAxes: {
    ...
    openContainerSheetCostAndInnerLiningCost: "COVERED",
    exactPiAndDeclared22Over7: "COVERED",
    declaredPi314: "COVERED_IN_SHELL_AND_COST_SLICES",
  ...
  costSliceAudit: {
    records: costAudit.recordCount,
    answerPositionCounts: costAudit.answerPositionCounts,
    piPolicyCounts: costAudit.piPolicyCounts,
  },
...
assert.equal(audit.generatedReviewRecordCount, 328);
assert.equal(audit.runtimePrototypeCount, 22);
assert.equal(new Set(runtimePrototypeIds).size, 22);
...
assert.equal(audit.uniqueExactStemCount, 328);
assert.equal(audit.uniqueQuestionOptionPackageCount, 328);
assert.deepEqual(answerPositionCounts, { A: 82, B: 82, C: 82, D: 82 });
assert.equal(audit.technicallyCleanLearnerRecords, 328);
assert.equal(audit.initialDiscoveryPrototypeCount, 20);
assert.equal(audit.implementedOrOwnedInitialDiscoveryPrototypeCount, 18);
assert.equal(audit.missingInitialDiscoveryPrototypeCount, 2);
...
assert.deepEqual(costAudit.answerPositionCounts, {
  A: 8,
  B: 8,
  C: 8,
  D: 8,
});
assert.deepEqual(costAudit.piPolicyCounts, {
  PI_22_OVER_7: 16,
  PI_3_14: 16,
});
...
assert.ok(stemWordCounts.every((count) => count >= 8 && count <= 140));
assert.ok(solutionWordCounts.every((count) => count >= 15 && count <= 340));
...
const reviewRows = records.map((question, index) => ({
  reviewId: `MEN-CP011-CURRENT-AUDIT-V4-${String(index + 1).padStart(3, "0")}`,
...
const reviewMarkdown = `# MEN-CP-011 Current Chapter-Wide Coverage and English Audit V4\n\n` +
  `## Verdict\n\n` +
  `The current executable MEN-CP-011 system contains **${audit.runtimePrototypeCount} direct runtime families** and **${audit.generatedReviewRecordCount} current review records**. Automated mathematical, option, lifecycle and technical-English checks pass across the complete current corpus. Chapter coverage and English remain unfrozen because the ratio and percentage-change candidates, conical shells, direct source normalisation and human review remain outstanding.\n\n` +
  `## Current metrics\n\n\`\`\`text\n` +
  ...
  `## Coverage notes\n\n` +
  `- Open-container sheet cost and inner-lining cost are now executable through exact area-rate ledgers.\n` +
  `- Direct open-top cuboid sheet area remains correctly owned by MEN-CP-007 rather than duplicated.\n` +
  `- Declared \`π = 3.14\` is represented exactly as \`157/50\` in shell and cost slices.\n` +
  `- Material-volume ratio and percentage-change applications remain the only missing initial candidates.\n` +
...
console.log(JSON.stringify({
  ...
  costSliceAudit: audit.costSliceAudit,
