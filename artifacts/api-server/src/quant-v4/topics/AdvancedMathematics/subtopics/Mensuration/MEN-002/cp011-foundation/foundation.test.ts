...
for (const definition of MEN_CP_011_FOUNDATION_PROTOTYPES) {
  ...
  for (let index = 0; index < 80; index += 1) {
    ...
    if (
      definition.prototypeId === "MEN-CP011-PROT-HOLLOW-CYLINDER-MATERIAL-VOLUME" &&
      first.piPolicy === "PI_22_OVER_7" &&
      first.state.outerRadius === 5n &&
      first.state.innerRadius === 3n &&
      first.state.height === 14n
    ) {
      exemplarFound = true;
      assert.equal(first.answer.includes("704"), true);
      assert.ok(first.explanation.shortcut.includes("(R-r)(R+r)"));
    }
...
if (!exemplarFound) {
  for (let index = 80; index < 1000 && !exemplarFound; index += 1) {
    const question = generateMenCp011FoundationPrototype(
      "MEN-CP011-PROT-HOLLOW-CYLINDER-MATERIAL-VOLUME",
      `men-002-cp011-exemplar-search:${index}`,
    );
    if (
      question.piPolicy === "PI_22_OVER_7" &&
      question.state.outerRadius === 5n &&
      question.state.innerRadius === 3n &&
      question.state.height === 14n
    ) {
      exemplarFound = true;
      assert.equal(question.answer.includes("704"), true);
      assert.ok(question.explanation.shortcut.includes("(R-r)(R+r)"));
    }
  }
}

assert.equal(exemplarFound, true, "The 5 cm / 3 cm / 14 cm / pi=22/7 pipe exemplar must remain generatable.");
