...
type Question = any;
const foundation = generateMenCp011ReviewBatch("men-cp011-chapter-audit-foundation-v1", 12);
const surface = generateMenCp011SurfaceReviewBatch("men-cp011-chapter-audit-surface-v1", 12);
const foundationRecords = foundation.records as Question[];
const surfaceRecords = surface.records as Question[];
...
function packageKey(question: Question) {
...
const exactPackages = records.map(packageKey);
...
const profileCounts = Object.fromEntries(profiles.map((profile) => [profile.id, 0])) as Record<string, number>;
...
for (const question of records) {
  const label = question.options[question.correctIndex].label as "A" | "B" | "C" | "D";
...
const solutionWordCounts = records.map((question) => words([
  question.learnerSolution.formula,
  ...question.learnerSolution.steps,
  question.learnerSolution.shortcut,
  ...question.learnerSolution.wrongOptionAnalysis,
...
const implementedInitialCount = Object.values(DISCOVERY_STATUS)
...
const audit = {
  ...
  foundationDistinctGeneratedStateCount: new Set(foundationRecords.map(stateKey)).size,
  surfaceDistinctGeneratedStateCount: new Set(surfaceRecords.map(stateKey)).size,
  combinedDistinctGeneratedStateCount: new Set(records.map(stateKey)).size,
...
assert.equal(audit.combinedDistinctGeneratedStateCount, 120);
...
assert.ok(records.every((question) => question.validation.valid && question.verification.valid));
...
assert.ok(records.every((question) => !question.renderSurfaces.attempt.exposesInternalCodes));
assert.ok(records.every((question) => !question.renderSurfaces.practice.exposesInternalCodes));
assert.ok(records.every((question) => !question.renderSurfaces.solution.exposesInternalCodes));
assert.ok(records.every((question) => question.renderSurfaces.admin.exposesInternalCodes));
...
assert.ok(records.every((question) => !question.publiclyPublishable));
assert.ok(records.every((question) => !question.questionStudioDiscoverable));
...
const reviewRows = records.map((question, index) => ({
  ...
  dimensions: {
    ...
    thickness: thickness(question).toString(),
...
const samples = prototypeIds.map((prototypeId) => {
  ...
  return [
    ...
    sample.stem,
    "",
    ...sample.options.map((option: any) => `- ${option.label}. ${option.display}`),
    "",
