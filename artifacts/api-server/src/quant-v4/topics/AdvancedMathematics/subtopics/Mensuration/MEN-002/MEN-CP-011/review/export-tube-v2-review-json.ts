...
const SOURCE_AUTHORITY = "New-main@663ba2fed9f3cf37c4086479967367e4ed2ca07e";
...
const records = MEN_CP_011_FOUNDATION_PROTOTYPES.flatMap((definition) =>
  Array.from({ length: SAMPLE_COUNT_PER_PROTOTYPE }, (_, index) => {
    ...
    const seed = `tube-v2-owner-review:${definition.prototypeId}:${sampleNumber}`;
...
const payload = {
  meta: {
    ...
    diagramAuthority: "TUBE_ORTHOGRAPHIC_V2",
...
if (!records.every((record) => record.question.permanentQlId === null)) {
  throw new Error("The corrected foundation review may not allocate permanent QLs.");
}
...
if (!records.every((record) => {
  ...
  return record.question.diagram.notToScale &&
    svg.includes('data-diagram-version="TUBE_ORTHOGRAPHIC_V2"') &&
    svg.includes('data-view="end-cross-section"') &&
    svg.includes('data-view="longitudinal-section"') &&
    svg.includes('data-region="annular-material"') &&
    svg.includes('data-region="open-inner-void"') &&
    svg.includes('data-region="continuous-inner-void"') &&
    svg.includes('data-dimension="pipe-length"') &&
    !svg.includes("M135 65 L395 65") &&
    !svg.includes("L455 110 L195 110 Z");
})) {
  throw new Error("Every review record must retain the corrected orthographic hollow-tube topology.");
}
if (!records.every((record) => {
  const { representation, innerRadius } = record.question.state;
  const svg = record.question.diagram.svg;
  if (representation === "DIAMETERS") {
    return svg.includes('data-dimension="outer-diameter"') &&
  ...
  if (representation === "OUTER_RADIUS_AND_THICKNESS") {
    return svg.includes('data-dimension="outer-radius"') &&
      svg.includes('data-dimension="inner-radius"') &&
      svg.includes('data-dimension="wall-thickness"');
  }
  ...
  throw new Error("The corrected diagram must use representation-correct dimension arrows and preserve inverse unknowns.");
}

const outputPath = resolve("dist/review/MEN-CP-011-Tube-V2-Review-Source.json");
mkdirSync(dirname(outputPath), { recursive: true });
writeFileSync(outputPath, JSON.stringify(payload, null, 2), "utf8");
console.log(`Wrote ${outputPath}`);
console.log(`Exported ${payload.meta.recordCount} corrected review records from ${payload.meta.sourceAuthority}.`);
