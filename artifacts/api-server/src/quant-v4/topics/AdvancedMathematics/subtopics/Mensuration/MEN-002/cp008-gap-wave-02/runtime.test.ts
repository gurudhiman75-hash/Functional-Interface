...
import {
  getMenCp008Wave02PrototypeIds,
  MEN_CP_008_WAVE_02_PROTOTYPES,
} from "./registry";
import {
  classifyMenCp008Wave02Difficulty,
  generateMenCp008Wave02Prototype,
} from "./runtime";

const prototypeIds = getMenCp008Wave02PrototypeIds();
assert.equal(prototypeIds.length, 16, "Wave 02 contains the second proven gap set, not a frozen inventory.");
...
const seenExactKinds = new Set<string>();
let generated = 0;
let ratioQuestions = 0;
let surdQuestions = 0;
let piSurdQuestions = 0;
let threePointFourteenQuestions = 0;
...
for (const prototypeId of prototypeIds) {
  ...
  for (let index = 0; index < 80; index += 1) {
    const seed = `men-002-cp008-wave02:${prototypeId}:${index}`;
    const first = generateMenCp008Wave02Prototype(prototypeId, seed);
    const second = generateMenCp008Wave02Prototype(prototypeId, seed);
    ...
    assert.equal(first.difficulty, classifyMenCp008Wave02Difficulty(first.state));
    ...
    assert.equal(first.waveId, "MEN-CP-008-GAP-WAVE-02");
    ...
    assert.equal(/Common mistake:|represents exactly two|turn the given|receive the same multiplier|follow the whole rectangle/i.test(learnerText), false, `${prototypeId} contains blocked robotic language.`);
    ...
    assert.equal(/MEN-CP008|W2-PROT|misconceptionId|FALLBACK_DERIVED/.test(JSON.stringify(first.explanation)), false, `${prototypeId} leaks internal taxonomy.`);
    ...
    if (first.exactAnswer.kind === "SURD") surdQuestions += 1;
    if (first.exactAnswer.kind === "PI_SURD") piSurdQuestions += 1;
    if (first.piPolicy === "PI_3_14") {
      threePointFourteenQuestions += 1;
    ...
    seenExactKinds.add(first.exactAnswer.kind);
    generated += 1;
  }
...
assert.ok(seenTargets.has("CAPACITY"));
assert.ok(seenTargets.has("LENGTH"));
assert.ok(seenTargets.has("RATIO"));
assert.ok(seenTargets.has("SURFACE_AREA"));
...
assert.ok(seenExactKinds.has("RATIONAL"));
assert.ok(seenExactKinds.has("SURD"));
assert.ok(seenExactKinds.has("PI"));
assert.ok(seenExactKinds.has("PI_SURD"));
assert.ok(ratioQuestions > 0);
assert.ok(surdQuestions > 0);
assert.ok(piSurdQuestions > 0);
assert.ok(threePointFourteenQuestions > 0);
assert.ok(rollerQuestions > 0);

const dispositionCounts = MEN_CP_008_WAVE_02_PROTOTYPES.reduce<Record<string, number>>((counts, item) => {
  counts[item.disposition] = (counts[item.disposition] ?? 0) + 1;
  return counts;
}, {});

console.log(
  `MEN-CP-008 gap wave 02 passed for ${generated} deterministic English packages across ${prototypeIds.length} temporary contracts. ` +
