  ...
  for (let seed = 1; seed <= 50; seed += 1) {
    ...
    if (first.answer.kind === "ORDERED_PAIR") {
      ...
      if (solved.kind === "UNIQUE") {
        ...
        assert(verifyLinearSystemSolution(first.system, first.answer.x, first.answer.y), `${candidate.candidateId} seed ${seed} substitution failed`);
      }
    }
    ...
    if (first.answer.kind === "RATIONAL") {
      ...
      if (solved.kind === "UNIQUE") {
        ...
        assert(verifyLinearSystemSolution(first.system, solved.x, solved.y), `${candidate.candidateId} seed ${seed} solved pair does not verify`);
      }
    }
    ...
    if (first.answer.kind === "PARAMETER_VALUE") {
      ...
      assert(equalsRational(first.answer.value, first.system.a2), `${candidate.candidateId} seed ${seed} hidden coefficient mismatch`);
