...
export const ALGEBRA_QUESTION_STUDIO_DELIVERY_V3_AUTHORITY =
  "ALGEBRA-FROZEN-QUESTION-STUDIO-DELIVERY-V3" as const;

export type AlgebraQuestionStudioQuestionV3 = AlgebraQuestionStudioQuestion & {
  readonly deliveryAuthority: typeof ALGEBRA_QUESTION_STUDIO_DELIVERY_V3_AUTHORITY;
};

export const ALGEBRA_QUESTION_STUDIO_PACKAGE_V3 = Object.freeze({
  ...ALGEBRA_QUESTION_STUDIO_PACKAGE_V2,
  label: "Algebra · Frozen Full Chapter · Delivery V3",
  deliveryAuthority: ALGEBRA_QUESTION_STUDIO_DELIVERY_V3_AUTHORITY,
  reviewStatus: "QUESTION_STUDIO_REVIEW_CONNECTED_FULL_ANSWER_MATRIX" as const,
});
...
function buildOptions(answer: any, language: AlgebraStudioLanguage, seed: string) {
  ...
  if (!correct || wrongs.length !== 3) {
    throw new Error(`Algebra Question Studio V3 option coverage failed for ${answer?.kind ?? typeof answer}: '${correct}'`);
  }
  ...
  const optionDetails = options.map((text, index) => ({
    ...
    misconceptionId: index === correctIndex ? null : `ALG-DIST-V3-M${++misconceptionIndex}`,
  }));
...
export function generateAlgebraStudioBatchV3(input: {
  language?: AlgebraStudioLanguage;
  examProfile?: AlgebraStudioExamProfile;
  ...
  const questions = Array.from({ length: count }, (_unused, index) => generateAlgebraStudioQuestionV3({
