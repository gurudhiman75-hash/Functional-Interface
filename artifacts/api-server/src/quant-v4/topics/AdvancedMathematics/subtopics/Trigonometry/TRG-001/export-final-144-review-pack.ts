...
const cards = records.map((record) => {
  const options = record.options
    .map((option) => `<li class="${option.isCorrect ? "correct" : ""}"><b>${esc(option.label)}.</b> ${esc(option.display)}${option.isCorrect ? " ✓" : ""}${option.misconceptionId ? `<span class="misconception">${esc(option.misconceptionId)}</span>` : ""}</li>`)
    .join("");
...
console.log(`TRG001_HUMAN_REVIEW_EXPORT_PASS count=${records.length} humanReview=PENDING`);
