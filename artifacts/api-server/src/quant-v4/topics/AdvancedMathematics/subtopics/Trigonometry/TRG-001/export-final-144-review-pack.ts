...
const records = TRG_001_AUTHORITY_ALIGNED_IDS.map((qlId, index) => {
  ...
  if (question.qlId !== qlId) throw new Error(`${qlId}: permanent ID drift during frozen review export.`);
  if (question.validation?.valid !== true || question.verification?.valid !== true) {
    throw new Error(`${qlId}: invalid final runtime state during frozen review export.`);
  }
  if (question.options?.length !== 4 || question.options.filter((o: any) => o.isCorrect).length !== 1) {
    throw new Error(`${qlId}: option integrity failed during frozen review export.`);
  }
  if (question.humanReviewStatus !== "APPROVED" || question.reviewStatus !== "HUMAN_APPROVED") {
    throw new Error(`${qlId}: human approval metadata is missing from the frozen export.`);
  }
  if (question.freezeEligible !== true || question.frozen !== true || question.freezeStatus !== "FROZEN") {
    throw new Error(`${qlId}: approved question is not frozen.`);
  }
  if (question.freeze?.approvedContentFingerprint !== TRG_001_HUMAN_APPROVAL.approvedContentFingerprint) {
    throw new Error(`${qlId}: freeze is not bound to the approved content fingerprint.`);
  }
  if (
    ...
    throw new Error(`${qlId}: activation lock failed during frozen review export.`);
  }
  ...
  return {
    ...
    frozen: question.frozen,
    freezeStatus: question.freezeStatus,
    frozenAt: question.frozenAt,
    humanReview: question.humanReview,
    freeze: question.freeze,
    activationLocks: {
...
if (records.length !== 144) throw new Error(`Expected 144 frozen review records; found ${records.length}.`);
...
const cards = records.map((record) => {
  ...
  return `<article class="question-card" id="${esc(record.qlId)}">
    ...
    <section class="review"><h3>Human review and freeze decision</h3><p><b>Decision:</b> ☒ APPROVED</p><p><b>Freeze status:</b> FROZEN · <b>Frozen at:</b> ${esc(TRG_001_FREEZE.frozenAt)}</p><p><b>Approved by:</b> ${esc(TRG_001_HUMAN_APPROVAL.approvedBy)} · <b>Approved at:</b> ${esc(TRG_001_HUMAN_APPROVAL.approvedAt)}</p></section>
  </article>`;
}).join("\n");

const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TRG-001 Final 144-QL Frozen Review Pack</title><style>
...
</style></head><body><main class="page"><section class="summary"><h1>TRG-001 Final 144-QL Frozen Review Pack</h1><p>Generated from the human-approved frozen overlay after the exact reviewed content was fingerprint-bound to the approval and freeze records.</p><p><b>Executing source head:</b> <code>${esc(sourceSha)}</code></p><p><b>Approved content source head:</b> <code>${esc(TRG_001_HUMAN_APPROVAL.approvedContentSourceHead)}</code></p><p><b>Scope:</b> 144/144 permanent English QLs. <b>AI editorial:</b> PASS. <b>Human review:</b> APPROVED. <b>Freeze status:</b> FROZEN.</p><p>Freeze does not authorize Question Studio, Question Bank, Test Builder, public publication, localization runtime, or PR merge. Those remain separate actions.</p></section>${cards}</main></body></html>`;
...
console.log(`TRG001_HUMAN_REVIEW_EXPORT_PASS count=${records.length} sourceSha=${sourceSha} humanReview=APPROVED freeze=FROZEN`);
