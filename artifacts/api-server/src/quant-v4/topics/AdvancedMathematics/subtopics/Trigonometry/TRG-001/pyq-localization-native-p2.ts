...
function languageFor(locale: Trg001LocalizedLocale): NativeLanguage {
  return locale === "hi-IN" ? "hi" : "pa";
}

function native(locale: Trg001LocalizedLocale, hi: string, pa: string) {
  return locale === "hi-IN" ? hi : pa;
}
...
function finish(question: AnyQuestion, locale: Trg001LocalizedLocale, stem: string, explanation: AnyQuestion) {
  return Object.freeze({
    ...
    stem,
    explanation,
    localizationP2: Object.freeze({
      version: TRG_001_PYQ_LOCALIZATION_NATIVE_P2.version,
      locale,
      nativeRemediation: true as const,
      humanLanguageReviewRequired: true as const,
      productionActivationChanged: false as const,
    }),
...
function localizeIntervalComparison(question: AnyQuestion, locale: Trg001LocalizedLocale) {
  ...
  const stem = native(
    locale,
    `यदि ${interval}, तो sin θ और cos θ के बीच सही संबंध चुनिए।`,
    `ਜੇ ${interval}, ਤਾਂ sin θ ਅਤੇ cos θ ਵਿਚਲਾ ਸਹੀ ਸੰਬੰਧ ਚੁਣੋ।`,
  );
  const explanation = {
    keyRule: native(
      locale,
    ...
    steps: [
      ...
      },
      {
        ...
        body: below45
          ? native(locale, "θ, 45° से कम है, इसलिए sin θ < cos θ।", "θ, 45° ਤੋਂ ਘੱਟ ਹੈ, ਇਸ ਲਈ sin θ < cos θ।")
          : native(locale, "θ, 45° से अधिक है, इसलिए sin θ > cos θ।", "θ, 45° ਤੋਂ ਵੱਧ ਹੈ, ਇਸ ਲਈ sin θ > cos θ।"),
      },
    ],
    shortcut: question?.explanation?.shortcut,
    traps: question?.explanation?.traps,
  };
  return finish(question, locale, stem, explanation);
}
...
function localizeHigherPowerRelation(question: AnyQuestion, locale: Trg001LocalizedLocale) {
  ...
  const stem = native(
    locale,
    `यदि ${given} θ + ${given}² θ = 1, तो ${target}⁴ θ + ${target}⁶ θ का मान ज्ञात कीजिए।`,
    `ਜੇ ${given} θ + ${given}² θ = 1, ਤਾਂ ${target}⁴ θ + ${target}⁶ θ ਦਾ ਮਾਨ ਕੱਢੋ।`,
  );
  const explanation = {
    keyRule: native(
      locale,
    ...
    steps: [
      {
        ...
        body: native(
          locale,
          `${given}θ+${given}²θ=1 से 1−${given}²θ=${given}θ। इसलिए ${target}²θ=${given}θ।`,
          `${given}θ+${given}²θ=1 ਤੋਂ 1−${given}²θ=${given}θ। ਇਸ ਲਈ ${target}²θ=${given}θ।`,
        ),
      },
      {
        title: native(locale, "चरण 2", "ਕਦਮ 2"),
        body: `${target}⁴θ+${target}⁶θ=${given}²θ+${given}³θ=${given}²θ(1+${given}θ).`,
      },
      {
        title: native(locale, "उत्तर", "ਉੱਤਰ"),
        body: native(
...
function localizeCubicFactorization(question: AnyQuestion, locale: Trg001LocalizedLocale) {
  const operation = String(question?.canonicalState?.operation ?? "DIFFERENCE_OF_CUBES");
...
export function localizePyqRemediatedTrg001QuestionP2(question: AnyQuestion, locale: Trg001LocalizedLocale) {
  if (question.qlId === "TRG-001-QL-024" && question.solveMode === "compareSinCosFromAcuteInterval") {
    return localizeIntervalComparison(question, locale);
  }
  if (question.qlId === "TRG-001-QL-126" && question.solveMode === "deriveHigherPowerFromTrigQuadraticRelation") {
    return localizeHigherPowerRelation(question, locale);
  }
  if (question.qlId === "TRG-001-QL-143" && question.solveMode === "simplifyTrigCubicFactorization") {
    return localizeCubicFactorization(question, locale);
  }
  ...
  return Object.freeze({
    ...
    localizationP2: Object.freeze({
      version: TRG_001_PYQ_LOCALIZATION_NATIVE_P2.version,
      locale,
      nativeRemediation: false as const,
      delegatedToExistingV5Pedagogic: true as const,
      humanLanguageReviewRequired: true as const,
      productionActivationChanged: false as const,
    }),
    questionStudioDiscoverable: false as const,
