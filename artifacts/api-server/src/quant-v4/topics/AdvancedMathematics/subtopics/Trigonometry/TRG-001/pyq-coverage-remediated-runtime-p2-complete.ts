...
function buildPowerRelationPyqSibling(seed: string) {
  ...
  return {
    ...
    canonicalState: {
      givenFunction: givenFn.toUpperCase(),
      targetFunction: targetFn.toUpperCase(),
      relation: `${givenFn}(theta)+${givenFn}^2(theta)=1`,
      pyqAnchor: "SSC-CGL-2023-07-26-S1-Q62",
      exactPyqVariant: !sinGiven,
      legacySiblingRetained: true,
    },
    verification: {
      valid: true,
      method: "TRIG_QUADRATIC_RELATION_HIGHER_POWER_SYMBOLIC_CHECK",
    ...
    authorityAlignment: {
      ...
      family: "MIXED_IDENTITY_EXPRESSION" as const,
      source: "REAL_EXAM_COVERAGE_AUDIT_P2" as const,
      broadensRoleAtPermanentId: MIXED_POWER_QL_ID,
      legacyRoleRetained: true as const,
    },
  };
}

export function generateCompletePyqCoverageRemediatedTrg001Question(qlId: string, seed: string) {
  if (qlId === MIXED_POWER_QL_ID && hash(`${seed}|${MIXED_POWER_QL_ID}|legacy-or-pyq`) % 2 === 1) {
    return buildPowerRelationPyqSibling(seed);
  }
  return generatePyqCoverageRemediatedTrg001Question(qlId, seed);
}

export const TRG_001_COMPLETE_PYQ_REMEDIATED_QL_IDS = Object.freeze([
  "TRG-001-QL-024",
  MIXED_POWER_QL_ID,
  "TRG-001-QL-143",
]);
