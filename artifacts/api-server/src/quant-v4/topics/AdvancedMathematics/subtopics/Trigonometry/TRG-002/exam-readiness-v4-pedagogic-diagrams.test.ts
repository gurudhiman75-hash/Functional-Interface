...
if (teachingDimensions < 20) throw new Error(`Expected broad teaching-dimension coverage, got ${teachingDimensions}.`);
if (diagramsWithTeachingFacts < 20) throw new Error(`Expected at least 20 diagrams with explicit explanation facts, got ${diagramsWithTeachingFacts}.`);
...
// Same-side two-position algebra must map x and x+d to the actual ground segments.
if (!hasDimension("TRG-002-QL-049", "x", "object-base", "near-ground")) throw new Error("QL049: near distance x missing.");
...
if (!hasDimension("TRG-002-QL-061", "x", "object-base", "near-ground")) throw new Error("QL061: original near distance x missing.");
...
for (const [id, expected] of [["TRG-002-QL-073", "rise = 20 m"], ["TRG-002-QL-076", "rise = 15 m"], ["TRG-002-QL-087", "rise = 24 m"]] as const) {
...
// Opposite-side systems must show the split used in the equations.
if (!hasDimension("TRG-002-QL-082", "y")) throw new Error("QL082: 60° distance y missing.");
...
console.log(`TRG002_V4_PEDAGOGIC_ALIGNMENT_PASS qls=96 helperPoints=${helperPoints} teachingDimensions=${teachingDimensions} requestedRealignments=${requestedRealignments} diagramsWithTeachingFacts=${diagramsWithTeachingFacts}`);
