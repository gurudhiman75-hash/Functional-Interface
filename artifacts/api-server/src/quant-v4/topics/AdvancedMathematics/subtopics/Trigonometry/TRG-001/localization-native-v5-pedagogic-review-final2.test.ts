...
for (const qlId of TRG_001_LOCALIZATION_QL_IDS) {
  for (const locale of TRG_001_LOCALIZATION_LOCALES) {
    for (let index = 1; index <= seedsPerQl; index += 1) {
      const seed = `trg001-review-final2-${String(index).padStart(2, "0")}-${qlId}-${locale}`;
      const source = generateHumanApprovedTrg001Question(qlId, seed) as any;
      const localized = generateLocalizedTrg001QuestionNativeReviewFinal2(qlId, seed, locale) as any;
      const repeated = generateLocalizedTrg001QuestionNativeReviewFinal2(qlId, seed, locale) as any;
      ...
      assert.equal(localized.reviewStatus, "LOCALIZATION_NATIVE_REVIEW_CANDIDATE_V5_PEDAGOGIC_REVIEW_FINAL2", `${id}: review status drift.`);
      assert.equal(localized.localizationProof?.finalNativeReviewOverlay2, true, `${id}: final native review V2 proof missing.`);
...
const inventory = {
  status: failures.length ? "TRG001_NATIVE_REVIEW_FINAL2_DEFECT_INVENTORY" : "TRG001_NATIVE_REVIEW_FINAL2_ENGINEERING_PASS",
...
assert.equal(failures.length, 0, `Final native editorial sweep V2 has ${failures.length} failures.`);
