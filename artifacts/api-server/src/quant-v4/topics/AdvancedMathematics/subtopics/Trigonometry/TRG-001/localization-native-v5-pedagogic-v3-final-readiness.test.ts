...
const QUADRANTS = [
  { label: "II", english: /\bquadrant\s+II\b/iu, hi: /(?:द्वितीय|दूसरे)\s+चतुर्थांश/u, pa: /(?:ਦੂਜਾ|ਦੂਜੇ)\s+ਚਤੁਰਭਾਗ/u },
  { label: "III", english: /\bquadrant\s+III\b/iu, hi: /(?:तृतीय|तीसरे)\s+चतुर्थांश/u, pa: /(?:ਤੀਜਾ|ਤੀਜੇ)\s+ਚਤੁਰਭਾਗ/u },
  { label: "IV", english: /\bquadrant\s+IV\b/iu, hi: /(?:चतुर्थ|चौथे)\s+चतुर्थांश/u, pa: /(?:ਚੌਥਾ|ਚੌਥੇ)\s+ਚਤੁਰਭਾਗ/u },
] as const;
...
function editorialArtifacts(text: string, locale: Locale) {
  const common = [/^A\s/u, /\ba\s+270°/iu, /\bsin-cos\b/iu];
...
for (const qlId of TRG_001_LOCALIZATION_QL_IDS) {
  for (const locale of TRG_001_LOCALIZATION_LOCALES) {
    for (let index = 1; index <= seedsPerQl; index += 1) {
      const seed = `trg001-final-readiness-${String(index).padStart(2, "0")}-${qlId}-${locale}`;
      const source = generateHumanApprovedTrg001Question(qlId, seed) as any;
      const localized = generateLocalizedTrg001QuestionNativePedagogicV3Final(qlId, seed, locale) as any;
      const repeated = generateLocalizedTrg001QuestionNativePedagogicV3Final(qlId, seed, locale) as any;
      ...
      for (const [field, sourceText] of sourceAligned) {
        ...
        for (const atom of notationAtoms(sourceText)) {
          if (!normalizedLocalized.includes(atom.replace(/\s+/gu, ""))) {
      ...
      if (anyLocalizedQuadrant.test(localizedKeyRule)) {
        ...
        if (sourceQuadrant) {
          ...
          if (!expected.test(localizedKeyRule)) {
            failures.push({ id, field: "keyRule", issue: `key-rule-quadrant-contradiction:${sourceQuadrant.label}`, source: sourceKeyRule, localized: localizedKeyRule });
          }
        }
      }
      ...
      assert.equal(localized.reviewStatus, "LOCALIZATION_NATIVE_REVIEW_CANDIDATE_V5_PEDAGOGIC_V3_FINAL", `${id}: review status drift.`);
      assert.equal(localized.localizationProof?.v5PedagogicV3FinalOverlay, true, `${id}: V3 final overlay proof missing.`);
...
const inventory = {
  status: failures.length
    ? "TRG001_LOCALIZATION_V3_FINAL_REVIEW_READINESS_DEFECT_INVENTORY"
    : "TRG001_LOCALIZATION_V3_FINAL_REVIEW_READY_ENGINEERING_PASS",
...
assert.equal(failures.length, 0, `V3 final review-readiness gate has ${failures.length} failures.`);
