...
assert(q005.stem.includes("angle of elevation of the top of a tower"), "QL-005 must identify what the elevation angle refers to.");
...
assert(q009.stem.includes("From a point on level ground") && q009.stem.includes("angle of elevation of the top of a"), "QL-009 must use explicit exam-style observation wording.");
...
assert(q020.stem.includes("angle of depression of the top") && q020.stem.includes("45°"), "QL-020 must use standard depression wording.");
const q024: any = generateFinalEditorialTrg002Mvp48Question("TRG-002-QL-024", wordingSeed);
assert(q024.stem.startsWith("From a point on level ground") && !q024.stem.includes("a 8 m building"), "QL-024 must use grammatically natural exam-style height wording.");
...
assert(q049.stem.includes("B is nearer the tower") && q049.stem.includes("30° from A") && q049.stem.includes("60° from B"), "QL-049 must assign each angle to the correct observation point explicitly.");
...
assert(q071.stem.includes("P and the feet of the towers are collinear"), "QL-071 must state the collinearity needed to subtract tower distances.");
...
assert(/\d+\.5 m high building/.test(q076.stem) && !/\d+\/2 m high building/.test(q076.stem), "QL-076 must present half-metre building heights as natural decimals.");
...
assert(q083.stem.includes("horizontal distance between their feet"), "QL-083 must state the horizontal building separation explicitly.");
