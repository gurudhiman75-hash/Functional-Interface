...
const records = TRG_002_CP008_LOCALIZATION_QL_IDS.flatMap((qlId, qlIndex) => locales.map((locale) => {
  ...
  const question: any = generateLocalizedTrg002Cp008Question(qlId, seed, locale);
  return {
...
console.log(`TRG002_CP008_LOCALIZATION_REVIEW_EXPORT_PASS records=${records.length} qls=24 locales=2 human=PENDING freeze=NO activation=OFF`);
