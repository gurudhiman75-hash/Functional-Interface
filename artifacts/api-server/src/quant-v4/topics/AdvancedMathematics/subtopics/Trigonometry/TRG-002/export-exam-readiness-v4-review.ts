...
import { generateExamRealLocalizedTrg002Question } from "./localization-exam-realness-v2";
...
const records = qlIds.map((qlId, index) => {
  ...
  const hi: any = generateExamRealLocalizedTrg002Question(qlId, seed, "hi-IN");
  const pa: any = generateExamRealLocalizedTrg002Question(qlId, seed, "pa-IN");
  return {
    qlId,
    cpId: en.cpId,
    ...
    hindi: { stem: hi.stem, options: hi.options, answer: hi.answer, explanation: hi.explanation },
    punjabi: { stem: pa.stem, options: pa.options, answer: pa.answer, explanation: pa.explanation },
...
console.log(`TRG002_V4_REVIEW_EXPORT_PASS qls=${records.length} languages=3 diagrams=INCLUDED freeze=OFF activation=OFF`);
