...
for (const locale of ["hi-IN", "pa-IN"] as const) {
  ...
  const normalizedMathText = text.replace(/\s+/gu, "");
  assert(!text.includes("√1.54"), `${locale}: historical √1.54 corruption must be absent in V4.`);
  assert(normalizedMathText.includes("8√3/24"), `${locale}: QL013 must preserve the exact ratio 8√3/24 independent of display spacing.`);
