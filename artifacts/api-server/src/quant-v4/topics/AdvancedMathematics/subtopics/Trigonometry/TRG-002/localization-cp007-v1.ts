...
function absoluteExactDifference(left: ExactTrigNumber, right: ExactTrigNumber) {
  return exactToNumber(left) >= exactToNumber(right)
    ? subtractExact(left, right)
    : subtractExact(right, left);
}

function exactText(value: ExactTrigNumber) {
...
function targetObjectFor(state: State, observation: AnyQuestion) {
  const found = state.verticalObjects.find((item: AnyQuestion) => item.topPointId === observation.targetPointId);
  if (!found) throw new Error(`TRG-002 localization: missing target object for ${observation.targetPointId}.`);
  return found;
}
...
function cp007Geometry(question: AnyQuestion) {
  ...
  return {
    ...
    verticalDelta,
    sightLine,
    angleText: degreesText(observation.angle),
  };
}
...
function localizedExplanation(question: AnyQuestion, locale: Trg002LocalizedLocale) {
  ...
  if (question.qlId === "TRG-002-QL-023") {
    ...
    bodies = [
      native(locale, `दृष्टि-रेखा L=${sight} m और कोण ${angle} है।`, `ਦ੍ਰਿਸ਼ਟੀ-ਰੇਖਾ L=${sight} m ਅਤੇ ਕੋਣ ${angle} ਹੈ।`),
      native(locale, `h=L×sin${angle}।`, `h=L×sin${angle}।`),
      native(locale, `अतः h=${answer}।`, `ਇਸ ਲਈ h=${answer}।`),
    ];
    ...
    bodies = [
      native(locale, `ऊँचाई=${height} m और कोण ${angle} है।`, `ਉਚਾਈ=${height} m ਅਤੇ ਕੋਣ ${angle} ਹੈ।`),
      native(locale, `L=${height}/sin${angle}।`, `L=${height}/sin${angle}।`),
      native(locale, `अतः L=${answer}।`, `ਇਸ ਲਈ L=${answer}।`),
    ];
    ...
    bodies = [
      native(locale, `क्षैतिज दूरी=${horizontal} m और कोण ${angle} है।`, `ਖਿਤਿਜੀ ਦੂਰੀ=${horizontal} m ਅਤੇ ਕੋਣ ${angle} ਹੈ।`),
      native(locale, `h=${horizontal}×tan${angle}।`, `h=${horizontal}×tan${angle}।`),
      native(locale, `अतः h=${answer}।`, `ਇਸ ਲਈ h=${answer}।`),
    ];
    ...
    bodies = [
      native(locale, `ऊँचाई=${height} m और कोण ${angle} है।`, `ਉਚਾਈ=${height} m ਅਤੇ ਕੋਣ ${angle} ਹੈ।`),
      native(locale, `d=${height}/tan${angle}।`, `d=${height}/tan${angle}।`),
      native(locale, `अतः d=${answer}।`, `ਇਸ ਲਈ d=${answer}।`),
    ];
    ...
    bodies = [
      native(locale, `tanθ=${height}/${horizontal}।`, `tanθ=${height}/${horizontal}।`),
      native(locale, "इस अनुपात को मानक त्रिकोणमितीय मान से मिलाएँ।", "ਇਸ ਅਨੁਪਾਤ ਨੂੰ ਮਿਆਰੀ ਤ੍ਰਿਕੋਣਮਿਤੀ ਮੁੱਲ ਨਾਲ ਮਿਲਾਓ।"),
      native(locale, `अतः θ=${answer}।`, `ਇਸ ਲਈ θ=${answer}।`),
    ];
    ...
    bodies = [
      native(locale, `ऊर्ध्वाधर अंतर=${horizontal}×tan${angle}=${drop} m।`, `ਲੰਬਕਾਰੀ ਫਰਕ=${horizontal}×tan${angle}=${drop} m।`),
      native(locale, `लक्ष्य की ऊँचाई=${observerHeight}−${drop} m।`, `ਟੀਚੇ ਦੀ ਉਚਾਈ=${observerHeight}−${drop} m।`),
      native(locale, `अतः ऊँचाई=${answer}।`, `ਇਸ ਲਈ ਉਚਾਈ=${answer}।`),
    ];
    ...
    bodies = [
      native(locale, `ऊर्ध्वाधर स्तर-अंतर=${drop} m।`, `ਲੰਬਕਾਰੀ ਪੱਧਰ-ਫਰਕ=${drop} m।`),
      native(locale, `d=${drop}/tan${angle}।`, `d=${drop}/tan${angle}।`),
      native(locale, `अतः d=${answer}।`, `ਇਸ ਲਈ d=${answer}।`),
    ];
  } else {
  ...
  return {
    ...
    steps: bodies.map((body, index) => ({
      title: index === bodies.length - 1
        ...
        : native(locale, `चरण ${index + 1}`, `ਕਦਮ ${index + 1}`),
      body,
    })),
    shortcut,
    traps: [trap],
...
export function trg002Cp007CanonicalSemanticFingerprint(question: AnyQuestion) {
  return sha256({
    ...
    options: question.options.map((option: AnyQuestion) => ({
      value: option.value,
    ...
    diagramEvidence: question.diagramEvidence,
  });
}

export function localizeFrozenTrg002Cp007Question(
  canonicalQuestion: AnyQuestion,
  locale: Trg002LocalizedLocale,
) {
  if (!TRG_002_CP007_LOCALIZATION_QL_IDS.includes(canonicalQuestion.qlId)) {
    throw new Error(`${canonicalQuestion.qlId}: outside TRG-CP-007 localization scope.`);
  }
...
export function generateLocalizedTrg002Cp007Question(
  qlId: string,
  seed: string,
  locale: Trg002LocalizedLocale,
) {
  if (!TRG_002_CP007_LOCALIZATION_QL_IDS.includes(qlId)) {
    throw new Error(`${qlId}: outside TRG-CP-007 localization scope.`);
  }
  const canonical = generateFrozenTrg002Production96Question(qlId, seed) as AnyQuestion;
  return localizeFrozenTrg002Cp007Question(canonical, locale);
}

export function buildTrg002Cp007LocalizedReviewBank(
  locale: Trg002LocalizedLocale,
  seedsPerQl = 12,
) {
  return TRG_002_CP007_LOCALIZATION_QL_IDS.flatMap((qlId) =>
    Array.from({ length: seedsPerQl }, (_, index) =>
