...
function angleSvg(angle: any, points: Map<string, any>) {
  ...
  const x = v.x + (dx / len) * 45;
  const y = v.y + (dy / len) * 45 - 8;
  return `<text x="${x}" y="${y}" text-anchor="middle" class="angle">${esc(angle.label)}</text>`;
}
