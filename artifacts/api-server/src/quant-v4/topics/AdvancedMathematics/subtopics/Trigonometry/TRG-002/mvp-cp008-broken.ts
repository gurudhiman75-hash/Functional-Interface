...
import { addExact, assertDefined, divideExact, exactInteger, formatExactPlain, multiplyExact } from "../foundation/exact";
...
function ql041(seed: string) {
  ...
  return buildTrg002MvpQuestion({ qlId: "TRG-002-QL-041", cpId: "TRG-CP-008", lockedFamily: "BROKEN_TREE_TOUCHING_GROUND", solveMode: "findFallenPartLength", seed, difficulty: "Medium", target: "LENGTH",
    stem: `A tree breaks ${formatExactPlain(stump)} m above the ground. Its upper part touches the ground and makes a 30° angle with it. Find the length of the fallen part.`, state: b.state, correct: mvpNumberAnswer(b.part),
...
function ql043(seed: string) {
  ...
  return buildTrg002MvpQuestion({ qlId: "TRG-002-QL-043", cpId: "TRG-CP-008", lockedFamily: "BROKEN_TREE_TOUCHING_GROUND", solveMode: "findGroundTouchDistance", seed, difficulty: "Medium", target: "LENGTH",
    stem: `A pole breaks ${formatExactPlain(stump)} m above the ground. The upper part touches the ground at 45°. How far from the foot of the pole does it touch the ground?`, state: b.state, correct: mvpNumberAnswer(b.run),
    wrong: [{ value: mvpNumberAnswer(b.part), misconceptionId: "RETURNED_FALLEN_PART" }, { value: mvpNumberAnswer(multiplyExact(stump, exactInteger(2))), misconceptionId: "DOUBLED_STUMP" }, { value: mvpNumberAnswer(div(stump, exactInteger(2))), misconceptionId: "HALVED_EQUAL_LEGS" }],
    explanation: mvpExplanation("At 45°, the stump and horizontal touch distance are equal legs.", [`tan45°=${formatExactPlain(stump)}/d=1.`, `Hence d=${formatExactPlain(b.run)} m.`], "The sloping upper part is not the requested horizontal distance.") });
}
