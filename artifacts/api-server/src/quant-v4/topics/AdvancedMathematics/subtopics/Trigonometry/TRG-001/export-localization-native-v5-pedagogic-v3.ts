...
const output = resolve(
  process.cwd(),
  "artifacts/api-server/dist/quant-v4/trg-001-localization-native-v5-pedagogic-v3-review.json",
);
mkdirSync(dirname(output), { recursive: true });

const reviewRows = TRG_001_LOCALIZATION_QL_IDS.map((qlId) => {
  const seed = `trg001-pedagogic-v3-review-${qlId}`;
  const en: any = generateHumanApprovedTrg001Question(qlId, seed);
  const hi: any = generateLocalizedTrg001QuestionNativePedagogicV3(qlId, seed, "hi-IN");
  const pa: any = generateLocalizedTrg001QuestionNativePedagogicV3(qlId, seed, "pa-IN");
  ...
  return {
    ...
    hindi: {
      ...
      v3Overlay: hi.localizationProof.v5PedagogicV3Overlay,
    },
    punjabi: {
      stem: pa.stem,
      ...
      v3Overlay: pa.localizationProof.v5PedagogicV3Overlay,
    },
    governance: {
...
writeFileSync(output, `${JSON.stringify({
  packageId: "TRG-001",
  status: "LOCALIZATION_NATIVE_REVIEW_CANDIDATE_V5_PEDAGOGIC_V3",
...
console.log(
  `TRG001_PEDAGOGIC_V3_REVIEW_PACK_WRITTEN rows=${reviewRows.length} surfaces=${reviewRows.length * 2} output=${output}`,
);
