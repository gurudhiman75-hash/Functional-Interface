...
type Segment = { fromPointId?: string; toPointId?: string };
...
function estimateLabelWidth(label: string, fontSize = 18) {
  const wide = Array.from(label).reduce((sum, char) => sum + (/[^\x00-\x7F]/.test(char) ? 0.95 : /[MW@#%]/.test(char) ? 0.9 : /[il1.,' ]/.test(char) ? 0.35 : 0.58), 0);
  return Math.max(28, wide * fontSize + 18);
}

function renderLabelBox(args: {
  label: string;
  x: number;
  y: number;
  ...
  const width = estimateLabelWidth(args.label, fontSize);
  const height = fontSize + 14;
  const rectX = anchor === "middle" ? args.x - width / 2 : anchor === "end" ? args.x - width + 4 : args.x - 4;
  const rectY = args.y - height / 2;
  return `<g class="diagram-label" data-label-box="true"><rect class="label-bg" x="${rectX.toFixed(2)}" y="${rectY.toFixed(2)}" width="${width.toFixed(2)}" height="${height.toFixed(2)}" rx="7" fill="#ffffff" fill-opacity="0.96" stroke="#e2e8f0" stroke-width="1"/><text x="${args.x.toFixed(2)}" y="${args.y.toFixed(2)}" text-anchor="${anchor}" dominant-baseline="middle" class="${args.className}" font-size="${fontSize}"${args.fill ? ` fill="${args.fill}"` : ""}>${esc(args.label)}</text></g>`;
}

function pointLabelPlacement(point: Point, width: number, height: number, rawSegments: Segment[], points: Map<string, Point>): LabelPlacement {
  const candidates: Array<{ dx: number; dy: number; anchor: LabelPlacement["anchor"] }> = [
    { dx: 24, dy: -24, anchor: "start" },
    { dx: -24, dy: -24, anchor: "end" },
    { dx: 24, dy: 28, anchor: "start" },
    { dx: -24, dy: 28, anchor: "end" },
    { dx: 0, dy: -32, anchor: "middle" },
    { dx: 0, dy: 34, anchor: "middle" },
  ];

  const incidentVectors = rawSegments.flatMap((segment) => {
    let other: Point | undefined;
    if (segment.fromPointId === point.id) other = points.get(String(segment.toPointId));
    else if (segment.toPointId === point.id) other = points.get(String(segment.fromPointId));
    if (!other) return [];
    const dx = other.x - point.x;
    const dy = other.y - point.y;
    const length = Math.hypot(dx, dy);
    return length > 0 ? [{ x: dx / length, y: dy / length }] : [];
  });

  let best = candidates[0]!;
  let bestScore = Number.POSITIVE_INFINITY;
  for (const candidate of candidates) {
    const cx = point.x + candidate.dx;
    const cy = point.y + candidate.dy;
    const length = Math.hypot(candidate.dx, candidate.dy) || 1;
    const ux = candidate.dx / length;
    const uy = candidate.dy / length;
    let score = 0;
    for (const vector of incidentVectors) {
      const alignment = Math.abs(ux * vector.x + uy * vector.y);
      score += alignment * alignment * 12;
    }
    if (cx < 55 || cx > width - 55) score += 30;
    if (cy < 35 || cy > height - 35) score += 30;
    if (candidate.dy > 0 && point.y > height * 0.78) score += 8;
    if (candidate.dy < 0 && point.y < height * 0.18) score += 8;
    if (score < bestScore) {
      bestScore = score;
      best = candidate;
    }
  }
  return { x: point.x + best.dx, y: point.y + best.dy, anchor: best.anchor };
}
...
function renderAngle(angle: any, points: Map<string, Point>) {
  ...
  const renderedLabel = label ? renderLabelBox({ label, x: lx, y: ly, className: "angle-label", fontSize: 20, fill: "#6d28d9" }) : "";
...
function renderMeasurementArrow(arrow: any, points: Map<string, Point>, markerId: string) {
  ...
  const labelX = (ax1 + ax2) / 2 + nx * 26;
  const labelY = (ay1 + ay2) / 2 + ny * 26;
  const label = String(arrow.label ?? "");
  const renderedLabel = label ? renderLabelBox({ label, x: labelX, y: labelY, className: "measurement-label", fontSize: 18, fill: "#0f766e" }) : "";
...
export function renderTrg002SolutionDiagramSvg(diagram: AnyDiagram) {
  ...
  const rawSegments: Segment[] = Array.isArray(diagram.segments) ? diagram.segments : [];
  ...
  const pointLabels = pointsArray.map((point) => {
    ...
    seenLabels.add(key);
    const placement = pointLabelPlacement(point, width, height, rawSegments, points);
    const label = renderLabelBox({ label: String(point.label), x: placement.x, y: placement.y, anchor: placement.anchor, className: "point-label", fontSize: 18, fill: "#111827" });
    return `<g class="diagram-point" data-point-id="${esc(point.id)}"><circle cx="${point.x.toFixed(2)}" cy="${point.y.toFixed(2)}" r="4.5" fill="#111827"/>${label}</g>`;
  }).join("");
