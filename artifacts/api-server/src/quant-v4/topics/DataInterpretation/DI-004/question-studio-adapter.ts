...
import {
  DI004_PERMANENT_OWNERSHIP,
  DI004_PERMANENT_QLS,
  DI004_PERMANENT_RELEASE_ID,
  getDi004PermanentQl,
...
export function isDi004QuestionStudioRequest(request: Di004QuestionStudioRequest) {
...
function normalizeProfile(value: unknown): Di004V2ExamProfile {
  ...
  throw new Error(`DI-004 does not support exam profile '${String(value ?? "")}' in the permanent English review checkpoint.`);
}

function stableOrder(items: readonly Di004PermanentQlDescriptor[], seed: string) {
  return [...items]
...
function eligibleDescriptors(profile: Di004V2ExamProfile, difficulty?: Di004V2Difficulty) {
  const filtered = DI004_PERMANENT_QLS.filter(
    (descriptor) => descriptor.supportedProfiles.includes(profile) && (!difficulty || descriptor.difficulty === difficulty),
  );
  if (!filtered.length) throw new Error(`DI-004 has no permanent English review QL for ${profile}${difficulty ? ` at ${difficulty} difficulty` : ""}.`);
  return filtered;
}

function resolveExplicitQl(request: Di004QuestionStudioRequest, profile: Di004V2ExamProfile, difficulty?: Di004V2Difficulty) {
  const qlId = String(request.questionLanguageId ?? "").trim().toUpperCase();
  if (!qlId) return undefined;
  const descriptor = getDi004PermanentQl(qlId);
  if (!descriptor) throw new Error(`Unknown DI-004 permanent QL '${qlId}'.`);
...
export async function generateDi004QuestionStudioBatch(request: Di004QuestionStudioRequest = {}) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const source = generateDi004PermanentQuestion({ seed, examProfile: profile, taskKind: descriptor.taskKind });
...
export function di004QuestionStudioPackageCard() {
  return {
    id: "DI-004",
    packageId: "DI-004",
    type: "quant-v4",
    ...
    questionStudioMode: DI004_PERMANENT_OWNERSHIP.lifecycle.questionStudioMode,
    questionBankStatus: "NOT_STORED",
