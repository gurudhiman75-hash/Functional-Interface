...
const taskKinds: readonly Di004TaskKind[] = [
...
const overtakeCoverage = new Set<number>();
const closestCoverage = new Set<number>();
const increaseIntervalCoverage = new Set<number>();
const averageWindowCoverage = new Set<number>();
let setCount = 0;
let questionCount = 0;
...
for (let seedIndex = 1; seedIndex <= 100; seedIndex += 1) {
  ...
  const byProfile = new Map<Di004ExamProfile, ReturnType<typeof generateDi004LineSet>>();

  for (const profile of profiles) {
    const first = generateDi004LineSet({ seed, examProfile: profile });
    const replay = generateDi004LineSet({ seed, examProfile: profile });

    assert(stable(first) === stable(replay), `${profile} ${seed} is not deterministic.`);
    deterministicReplayChecks += 1;

    assert(first.validation.valid, `${profile} ${seed} failed internal line-chart validation.`);
    assert(independentlyVerifyDi004QuestionSet(first), `${profile} ${seed} failed independent line-chart verification.`);
    independentVerificationChecks += first.questions.length;

    assert(first.stimulus.kind === "LINE", `${profile} ${seed} lost line-chart semantics.`);
    assert(first.stimulus.points.length === 6, `${profile} ${seed} must contain six plotted periods.`);
    assert(first.stimulus.series.length === 2, `${profile} ${seed} must contain two visible line series.`);
    assert(first.stimulus.points.every((point) => point.seriesA > 0 && point.seriesB > 0), `${profile} ${seed} contains a non-positive plotted value.`);
    assert(first.stimulus.points.some((point) => point.seriesA < point.seriesB), `${profile} ${seed} never has Region B above Region A.`);
    assert(first.stimulus.points.some((point) => point.seriesA > point.seriesB), `${profile} ${seed} never has Region A above Region B.`);
    ...
    assert(new Set(first.questions.map((question) => question.kind)).size === 5, `${profile} ${seed} repeated a line-chart task kind.`);
    assert(taskKinds.every((taskKind) => first.questions.some((question) => question.kind === taskKind)), `${profile} ${seed} is missing a required line-chart task.`);
    ...
    first.questions.forEach((question) => {
      ...
      if (question.kind === "FIRST_OVERTAKE_PERIOD") overtakeCoverage.add(question.evidence.overtakeIndex!);
      if (question.kind === "CLOSEST_LINES_PERIOD") closestCoverage.add(question.evidence.closestIndex!);
      if (question.kind === "CONSECUTIVE_PERCENT_INCREASE_A") increaseIntervalCoverage.add(question.evidence.fromIndex!);
      if (question.kind === "THREE_PERIOD_AVERAGE_B") averageWindowCoverage.add(question.evidence.startIndex!);
  ...
  assert(stable(ssc.stimulus) === stable(banking.stimulus), `Exam profile changed the line stimulus for ${seed}.`);
...
assert(stimulusFingerprints.size >= 95, `DI-004 produced only ${stimulusFingerprints.size} distinct line stimuli across 100 seeds.`);
assert(overtakeCoverage.size === 4, `DI-004 overtake task did not reach all four supported change points: ${[...overtakeCoverage].sort()}.`);
assert(closestCoverage.size === 6, `DI-004 closest-lines task did not reach all six periods: ${[...closestCoverage].sort()}.`);
assert(increaseIntervalCoverage.size >= 4, `DI-004 percentage-increase task reached only ${increaseIntervalCoverage.size} interval starts.`);
assert(averageWindowCoverage.size === 4, `DI-004 average task did not reach all four three-period windows: ${[...averageWindowCoverage].sort()}.`);
assert(bankingFiveOptionChecks === 500, `DI-004 completed only ${bankingFiveOptionChecks}/500 Banking five-option child checks.`);
...
for (const file of walkFiles(scope).filter((path) => path.endsWith(".ts"))) {
  ...
  assert(!source.includes(forbiddenRandomToken), `Non-deterministic random source is prohibited in DI-004: ${file}`);
}

console.log(JSON.stringify({
  status: "PASS_DI_004_LINE_PHASE3",
  sets: setCount,
  ...
  overtakeCoverage: [...overtakeCoverage].sort(),
  closestCoverage: [...closestCoverage].sort(),
  increaseIntervalCoverage: [...increaseIntervalCoverage].sort(),
  averageWindowCoverage: [...averageWindowCoverage].sort(),
