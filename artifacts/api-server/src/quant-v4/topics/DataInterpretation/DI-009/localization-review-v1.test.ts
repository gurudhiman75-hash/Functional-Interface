...
for (const locale of locales) {
  for (const descriptor of DI009_PERMANENT_QLS) {
    ...
    for (let sample = 1; sample <= 30; sample += 1) {
      ...
      assert(localized.traceability.questionStudioDiscoverable === false, `${key} became Question Studio discoverable before localization approval.`);
...
console.log(JSON.stringify({
  status: "PASS_DI_009_HI_PA_LOCALIZATION_REVIEW_V1",
