...
import { DI005_V2_TASK_KINDS } from "./pie-set-v2";
...
function selectReviewSets(): Di005V2QuestionSet[] {
  ...
  for (let index = 1; index <= 200 && (uncovered.size > 0 || selected.length < 6); index += 1) {
    ...
    const set = generateDi005V2ReviewSet({ seed: `DI005-V2-REVIEW-${String(index).padStart(3, "0")}`, examProfile: profile });
...
function visibleLabels(set: Di005V2QuestionSet) {
  const lines = ["| Category | Label shown on pie |", "|---|---:|"];
  for (const slice of set.stimulus.slices) {
    lines.push(`| ${slice.category} | ${slice.displayPercent === "?" ? "?" : `${slice.displayPercent}%`} |`);
  }
  return lines.join("\n");
}
...
const lines: string[] = [
  ...
  "- The true value of the hidden sector is intentionally not printed in this Markdown stimulus summary. Use the companion HTML review for the actual pie geometry.",
  "",
];
...
sets.forEach((set, setIndex) => {
  ...
  lines.push("");
  lines.push(visibleLabels(set));
  lines.push("");
  set.questions.forEach((_, questionIndex) => lines.push(questionBlock(set, questionIndex)));
});

writeFileSync(output, `${lines.join("\n")}\n`, "utf8");
console.log(JSON.stringify({ status: "PASS_DI_005_REVIEW_V2_CLEAN_MD", output, sets: sets.length, questions: sets.length * 5, taskFamilies: familyCoverage.size }));
