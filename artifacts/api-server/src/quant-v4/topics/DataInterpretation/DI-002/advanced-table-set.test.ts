...
const profiles: readonly Di002ExamProfile[] = ["SSC_CGL_TIER_I", "BANKING_PRELIMS"];
const taskKinds: readonly Di002TaskKind[] = [
...
const hiddenApplicantPositions = new Set<number>();
...
for (let seedIndex = 1; seedIndex <= 100; seedIndex += 1) {
  const seed = `DI-002-PHASE1-${seedIndex}`;
  const byProfile = new Map<Di002ExamProfile, ReturnType<typeof generateDi002AdvancedTableSet>>();

  for (const profile of profiles) {
    const first = generateDi002AdvancedTableSet({ seed, examProfile: profile });
    const replay = generateDi002AdvancedTableSet({ seed, examProfile: profile });

    assert(stable(first) === stable(replay), `${profile} ${seed} is not deterministic.`);
    deterministicReplayChecks += 1;

    assert(first.validation.valid, `${profile} ${seed} failed internal DI-002 validation.`);
    assert(independentlyVerifyDi002QuestionSet(first), `${profile} ${seed} failed independent set verification.`);
    ...
    assert(first.stimulus.rows.length === 5, `${profile} ${seed} did not create a five-row table.`);
    assert(first.stimulus.rows.filter((row) => row.applicants === "?").length === 1, `${profile} ${seed} does not contain exactly one hidden Applicants cell.`);
    assert(first.stimulus.rows[first.stimulus.hiddenApplicantIndex]?.applicants === "?", `${profile} ${seed} hidden Applicants index is inconsistent.`);
    assert(new Set(first.questions.map((question) => question.setId)).size === 1, `${profile} ${seed} child questions lost shared set identity.`);
    assert(new Set(first.questions.map((question) => question.questionId)).size === 5, `${profile} ${seed} repeated a child question ID.`);
    assert(new Set(first.questions.map((question) => question.kind)).size === 5, `${profile} ${seed} repeated an advanced task kind.`);
    assert(taskKinds.every((taskKind) => first.questions.some((question) => question.kind === taskKind)), `${profile} ${seed} is missing a required advanced task kind.`);
    ...
    first.questions.forEach((question) => {
      ...
      assert(question.options[question.correctIndex] === question.answer, `${question.questionId} answer does not match correct option.`);
      assert(question.options.filter((option) => option === question.answer).length === 1, `${question.questionId} displays its answer more than once.`);
      ...
      assert(question.explanation.keyIdea.length >= 30, `${question.questionId} key idea is too thin.`);
    ...
    hiddenApplicantPositions.add(first.stimulus.hiddenApplicantIndex);
  ...
  assert(stable(ssc.stimulus) === stable(banking.stimulus), `Exam profile changed the mathematical stimulus for ${seed}.`);
...
assert(hiddenApplicantPositions.size === 5, `DI-002 did not reach all five hidden Applicants positions: ${[...hiddenApplicantPositions].sort()}.`);
assert(stimulusFingerprints.size >= 80, `DI-002 produced only ${stimulusFingerprints.size} distinct stimuli across 100 seeds.`);
...
for (const file of walkFiles(scope).filter((path) => path.endsWith(".ts"))) {
  ...
  assert(!source.includes(forbiddenRandomToken), `Non-deterministic random source is prohibited in DI-002: ${file}`);
}

console.log(JSON.stringify({
  status: "PASS_DI_002_ADVANCED_TABLE_PHASE1",
  sets: setCount,
  ...
  hiddenApplicantPositions: [...hiddenApplicantPositions].sort(),
