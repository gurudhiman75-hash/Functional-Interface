...
import {
  DI002_PERMANENT_OWNERSHIP,
  DI002_PERMANENT_QLS,
  DI002_PERMANENT_RELEASE_ID,
  getDi002PermanentQl,
...
export function isDi002QuestionStudioRequest(request: Di002QuestionStudioRequest) {
  ...
  return packageId === "di 002"
    || patternId === "di 002"
    || Boolean(getDi002PermanentQl(qlId))
    || cpId === DI002_QUESTION_STUDIO_CANONICAL_PROBLEM_ID
    || (topic === "data interpretation" && ["advanced table", "advanced table di", "advanced tabular data"].includes(subtopic));
}

function normalizeDifficulty(value: unknown): Di002V2Difficulty | undefined {
...
function normalizeProfile(value: unknown): Di002V2ExamProfile {
...
function eligibleDescriptors(profile: Di002V2ExamProfile, difficulty?: Di002V2Difficulty) {
...
function resolveExplicitQl(request: Di002QuestionStudioRequest, profile: Di002V2ExamProfile, difficulty?: Di002V2Difficulty) {
  const qlId = String(request.questionLanguageId ?? "").trim().toUpperCase();
  if (!qlId) return undefined;
  const descriptor = getDi002PermanentQl(qlId);
  if (!descriptor) throw new Error(`Unknown DI-002 permanent QL '${qlId}'.`);
...
export async function generateDi002QuestionStudioBatch(request: Di002QuestionStudioRequest = {}) {
  ...
  const batchSeed = String(request.seed ?? "").trim() || `quant-v4:DI-002:${profile}:${difficulty ?? "mixed"}:${Date.now()}`;
  const pool = explicit ? [explicit] : stableOrder(eligibleDescriptors(profile, difficulty), `${batchSeed}:ql-order`);
  const questionPackages: ReturnType<typeof generateDi002PermanentQuestion>[] = [];
  const questions: any[] = [];
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const source = generateDi002PermanentQuestion({ seed, examProfile: profile, taskKind: descriptor.taskKind });
...
export function di002QuestionStudioPackageCard() {
  return {
    id: "DI-002",
    packageId: "DI-002",
    type: "quant-v4",
    ...
    topic: "Data Interpretation",
    subtopic: "Advanced Table",
    name: "DI-002 Advanced Table",
    label: "Advanced Table",
    generationDomain: "quant-v4",
    cpIds: [DI002_QUESTION_STUDIO_CANONICAL_PROBLEM_ID],
    canonicalProblems: [{ id: DI002_QUESTION_STUDIO_CANONICAL_PROBLEM_ID, label: "Advanced Table" }],
    permanentQlIds: DI002_PERMANENT_QLS.map((descriptor) => descriptor.qlId),
    permanentQlCount: DI002_PERMANENT_QLS.length,
