...
function toQuestionStudioPreview(
  ...
  descriptor: Di003PermanentQlDescriptor,
  context: { seed: string; index: number; count: number },
  language: "en" | "hi" | "pa",
) {
  const localized = language === "en" ? undefined : localizeDi003Question(source, language === "hi" ? "hi-IN" : "pa-IN");
  ...
  const releaseId = language === "en" ? DI003_PERMANENT_RELEASE_ID : DI003_LOCALIZATION_RELEASE_ID;
  return {
    text: question.stem,
    stem: question.stem,
    stimulus,
    ...
    debugSource: language === "en" ? "quant-v4-di003-permanent-english-review" : "quant-v4-di003-multilingual-frozen-review",
...
export async function generateDi003QuestionStudioBatch(request: Di003QuestionStudioRequest = {}) {
  ...
  return {
    generationContext: {
      ...
      releaseId: language === "en" ? DI003_PERMANENT_RELEASE_ID : DI003_LOCALIZATION_RELEASE_ID,
