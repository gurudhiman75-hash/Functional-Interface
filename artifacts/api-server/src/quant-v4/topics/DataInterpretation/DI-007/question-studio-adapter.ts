...
import {
  DI007_PERMANENT_OWNERSHIP,
  DI007_PERMANENT_QLS,
  DI007_PERMANENT_RELEASE_ID,
  getDi007PermanentQl,
  type Di007PermanentQlDescriptor,
...
export function isDi007QuestionStudioRequest(request: Di007QuestionStudioRequest) {
...
function normalizeDifficulty(value: unknown): Di007V2Difficulty | undefined {
...
function normalizeProfile(value: unknown): Di007V2ExamProfile {
  ...
  throw new Error(`DI-007 supports Banking Prelims and Banking Mains only in the permanent English review checkpoint; received '${String(value ?? "")}'.`);
}

function stableOrder(items: readonly Di007PermanentQlDescriptor[], seed: string) {
  return [...items]
...
function eligibleDescriptors(profile: Di007V2ExamProfile, difficulty?: Di007V2Difficulty) {
  const filtered = DI007_PERMANENT_QLS.filter(
    (descriptor) => descriptor.supportedProfiles.includes(profile) && (!difficulty || descriptor.difficulty === difficulty),
  );
  if (!filtered.length) throw new Error(`DI-007 has no permanent English review QL for ${profile}${difficulty ? ` at ${difficulty} difficulty` : ""}.`);
  return filtered;
}

function resolveExplicitQl(request: Di007QuestionStudioRequest, profile: Di007V2ExamProfile, difficulty?: Di007V2Difficulty) {
  const qlId = String(request.questionLanguageId ?? "").trim().toUpperCase();
  if (!qlId) return undefined;
  const descriptor = getDi007PermanentQl(qlId);
  if (!descriptor) throw new Error(`Unknown DI-007 permanent QL '${qlId}'.`);
...
function toQuestionStudioPreview(
  ...
  descriptor: Di007PermanentQlDescriptor,
  ...
  return {
    ...
    debugSource: "quant-v4-di007-permanent-english-review",
...
export async function generateDi007QuestionStudioBatch(request: Di007QuestionStudioRequest = {}) {
  const cpId = String(request.canonicalProblemId ?? request.cpId ?? "").trim().toUpperCase();
  if (cpId && cpId !== DI007_QUESTION_STUDIO_CANONICAL_PROBLEM_ID) throw new Error(`Unknown canonical problem '${cpId}' for package DI-007.`);
  const language = String(request.language ?? "en").trim().toLowerCase();
  if (language !== "en") throw new Error("DI-007 permanent Question Studio review is English-only; localization has not started.");
  ...
  const batchSeed = String(request.seed ?? "").trim() || `quant-v4:DI-007:${profile}:${difficulty ?? "mixed"}:${Date.now()}`;
  const pool = explicit ? [explicit] : stableOrder(eligibleDescriptors(profile, difficulty), `${batchSeed}:ql-order`);
  const questionPackages: ReturnType<typeof generateDi007PermanentQuestion>[] = [];
  const questions: any[] = [];
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const source = generateDi007PermanentQuestion({ seed, examProfile: profile, taskKind: descriptor.taskKind });
    if (source.question.kind !== descriptor.taskKind || source.question.difficulty !== descriptor.difficulty) {
      throw new Error(`${descriptor.qlId} semantic ownership drifted from its approved DI-007 V2 contract.`);
    }
...
export function di007QuestionStudioPackageCard() {
  return {
    id: "DI-007",
    packageId: "DI-007",
    type: "quant-v4",
    ...
    topic: "Data Interpretation",
    subtopic: "Missing Data",
    name: "DI-007 Missing Data Interpretation",
    label: "Missing Data Interpretation",
    generationDomain: "quant-v4",
    cpIds: [DI007_QUESTION_STUDIO_CANONICAL_PROBLEM_ID],
    canonicalProblems: [{ id: DI007_QUESTION_STUDIO_CANONICAL_PROBLEM_ID, label: "Missing Data Interpretation" }],
    permanentQlIds: DI007_PERMANENT_QLS.map((descriptor) => descriptor.qlId),
    permanentQlCount: DI007_PERMANENT_QLS.length,
    qls: DI007_PERMANENT_QLS.map((descriptor) => ({ id: descriptor.qlId, label: descriptor.label, difficulty: descriptor.difficulty, taskKind: descriptor.taskKind })),
    supportedDifficulties: ["easy", "medium", "hard"],
    supportedLanguages: ["en"],
    supportedExamProfiles: ["BANKING_PRELIMS", "BANKING_MAINS"],
    enabled: true,
    runtimeMode: DI007_QUESTION_STUDIO_RUNTIME_MODE,
    supportedRuntimeModes: [DI007_QUESTION_STUDIO_RUNTIME_MODE],
    reviewStatus: "ENGLISH_REVIEW_APPROVED",
    releaseId: DI007_PERMANENT_RELEASE_ID,
    questionStudioDiscoverable: DI007_PERMANENT_OWNERSHIP.lifecycle.questionStudioDiscoverable,
    questionStudioMode: DI007_PERMANENT_OWNERSHIP.lifecycle.questionStudioMode,
    questionBankStatus: "NOT_STORED",
