 ...
 * Question generators should store semantic chart data, not SVG payloads.
 */
export function renderDiHistogramSvg(model: DiHistogramVisualModel): string {
  ...
  if (!model.bins.every((bin) => Number.isFinite(bin.frequency) && bin.frequency >= 0)) {
    throw new Error("DI histogram renderer received an invalid frequency.");
  }
  ...
  const boundaryPositions = Array.from(
    { length: model.bins.length + 1 },
    (_, index) => Number((left + (plotWidth * index) / model.bins.length).toFixed(3)),
  );
  const safeTitle = escapeSvgText(model.title);
  const safeXAxis = escapeSvgText(model.xAxisLabel);
