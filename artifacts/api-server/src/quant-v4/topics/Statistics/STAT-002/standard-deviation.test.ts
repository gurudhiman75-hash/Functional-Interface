...
import type { Stat002ContractId, Stat002Difficulty, Stat002ExamProfile } from "./types";
...
const expectedDifficulty: Record<Stat002ContractId, Stat002Difficulty> = {
  "STAT-002-TEMP-001-RAW-SD": "Easy",
  "STAT-002-TEMP-002-MEAN-SQUARES-SD": "Medium",
  "STAT-002-TEMP-003-TRANSLATION-INVARIANCE": "Easy",
  "STAT-002-TEMP-004-SCALE-TRANSFORMATION": "Medium",
  "STAT-002-TEMP-005-REVERSE-SCALE": "Medium",
  "STAT-002-TEMP-006-AFFINE-FROM-MOMENTS": "Hard",
};
...
for (const profile of profiles) {
  for (const contractId of STAT002_CONTRACTS) {
    ...
    for (let seedIndex = 1; seedIndex <= 100; seedIndex += 1) {
      ...
      assert(question.difficulty === expectedDifficulty[contractId], `${key} seed ${seedIndex} has an incorrect difficulty label.`);
      ...
      if (contractId === "STAT-002-TEMP-006-AFFINE-FROM-MOMENTS") {
        assert(question.state.kind === "AFFINE_FROM_MOMENTS", `${key} did not preserve the compound hard-state contract.`);
        assert(question.explanation.steps.length >= 3, `${key} hard questions must show both source-SD recovery and affine transformation.`);
      }
...
const expectedQuestions = profiles.length * STAT002_CONTRACTS.length * 100;
assert(questionCount === expectedQuestions, `Expected ${expectedQuestions} questions, found ${questionCount}.`);
assert(replayCount === expectedQuestions, "Deterministic replay proof count is incomplete.");
assert(independentVerificationCount === expectedQuestions, "Independent verification proof count is incomplete.");
assert(optionCheckCount === expectedQuestions * 4, "Option proof count is incomplete.");
