...
const CONTRACT_META: Record<Sta001ContractId, { solveMode: Sta001SolveMode; difficulty: Sta001Difficulty }> = {
  "STA-TEMP-001-SIMPLE-MEAN": { solveMode: "DIRECT_MEAN", difficulty: "Easy" },
  "STA-TEMP-002-MISSING-OBSERVATION": { solveMode: "REVERSE_MEAN_TOTAL", difficulty: "Medium" },
  "STA-TEMP-003-CORRECTED-MEAN": { solveMode: "MEAN_CORRECTION", difficulty: "Medium" },
  "STA-TEMP-004-COMBINED-MEAN": { solveMode: "WEIGHTED_GROUP_MEAN", difficulty: "Hard" },
  "STA-TEMP-005-MEDIAN-RAW": { solveMode: "ORDER_STATISTIC_MEDIAN", difficulty: "Medium" },
  "STA-TEMP-006-MODE-RAW": { solveMode: "FREQUENCY_MODE", difficulty: "Medium" },
};
...
function mode(values: readonly number[]): number {
  ...
  if (ordered.length < 2 || ordered[0]![1] === ordered[1]![1]) throw new Error("STA-001 requires a unique mode.");
  return ordered[0]![0];
}
...
function buildOptions(seed: string, answer: string, candidates: readonly Candidate[]) {
  ...
  add({ text: answer, misconceptionId: "CORRECT", derivation: "Exact recomputation from the STA-001 mathematical state." });
  candidates.forEach(add);
  if (retained.length < 4) {
    throw new Error(`STA-001 could construct only ${retained.length} unique options; 4 are required.`);
  }
  ...
  if (correctIndex < 0) throw new Error("STA-001 lost the correct option during deterministic shuffling.");
...
function buildCorrectedMean(seed: string, profile: Sta001ExamProfile): Draft {
  ...
  if (correctValue <= 0) throw new Error("STA-001 corrected-mean state must remain positive.");
...
function buildDraft(contractId: Sta001ContractId, seed: string, profile: Sta001ExamProfile): Draft {
  switch (contractId) {
    case "STA-TEMP-001-SIMPLE-MEAN": return buildSimpleMean(seed, profile);
    case "STA-TEMP-002-MISSING-OBSERVATION": return buildMissingObservation(seed, profile);
    case "STA-TEMP-003-CORRECTED-MEAN": return buildCorrectedMean(seed, profile);
    case "STA-TEMP-004-COMBINED-MEAN": return buildCombinedMean(seed, profile);
    case "STA-TEMP-005-MEDIAN-RAW": return buildMedian(seed, profile);
    case "STA-TEMP-006-MODE-RAW": return buildMode(seed, profile);
  }
}
...
function validateQuestion(question: Omit<Sta001Question, "validation">) {
  ...
  add("FOUR_OPTIONS", question.options.length === 4, "SSC STA-001 questions require exactly four options.");
  ...
  add("LIFECYCLE_LOCK", !question.traceability.questionStudioDiscoverable && question.traceability.questionBankStatus === "NOT_STORED" && question.traceability.testEligibility === "INELIGIBLE" && !question.traceability.publiclyPublishable, "STA-001 Phase 0 must remain review-only.");
...
export function generateSta001Question(input: {
  ...
  const seed = input.seed ?? "STA-001:P0";
  ...
  const withoutValidation: Omit<Sta001Question, "validation"> = {
    packageId: "STA-001",
    questionId: `STA-001-${hashSeed(`${seed}:${examProfile}:${contractId}`).toString(36)}`,
    seed,
    examProfile,
    contractId,
  ...
  if (!validation.valid) {
    ...
    throw new Error(`STA-001 validation failed: ${failed}`);
  }
