...
for (let seedIndex = 1; seedIndex <= 100; seedIndex += 1) {
  const seed = `STA-001-P0-${seedIndex}`;
  ...
  for (const profile of profiles) {
    for (const contractId of STA001_CONTRACTS) {
      ...
      assert(first.packageId === "STA-001", `${key} ${seed} lost package identity.`);
...
assert(questionCount === 1200, `STA-001 generated ${questionCount}/1200 expected questions.`);
assert(deterministicReplayChecks === 1200, `STA-001 completed ${deterministicReplayChecks}/1200 replay checks.`);
assert(independentVerificationChecks === 1200, `STA-001 completed ${independentVerificationChecks}/1200 independent checks.`);
assert(optionChecks === 4800, `STA-001 completed ${optionChecks}/4800 option checks.`);
assert(lifecycleChecks === 1200, `STA-001 completed ${lifecycleChecks}/1200 lifecycle checks.`);
...
for (const directory of [scope, sharedScope]) {
  for (const file of walkFiles(directory).filter((path) => path.endsWith(".ts"))) {
    ...
    assert(!source.includes(forbiddenRandomToken), `Non-deterministic random source is prohibited in STA-001: ${file}`);
  }
}
