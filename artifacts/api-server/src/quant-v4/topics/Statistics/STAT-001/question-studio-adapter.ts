...
import { generateSta001Question } from "./central-tendency";
...
export const STAT001_QUESTION_STUDIO_RUNTIME_MODE = "STAT001_PERMANENT_ENGLISH_REVIEW_P1" as const;
...
function normalizeProfile(value: unknown): Sta001ExamProfile {
  ...
  if (profile === "SSC_CGL_TIER_II" || profile === "SSC_CGL_TIER_2" || profile === "SSC_CGL_CHSL" || !profile || profile === "GENERIC_PRACTICE") {
    return "SSC_CGL_TIER_II";
  }
...
function eligibleDescriptors(
  profile: Sta001ExamProfile,
  difficulty?: Sta001Difficulty,
) {
  const filtered = STAT001_PERMANENT_QLS.filter((descriptor) =>
    descriptor.supportedProfiles.includes(profile)
    && (!difficulty || descriptor.difficulty === difficulty),
  );
  if (!filtered.length) {
    throw new Error(`STAT-001 has no permanent English review QL for ${profile}${difficulty ? ` at ${difficulty} difficulty` : ""}.`);
  }
  return filtered;
}

function resolveExplicitQl(
  request: Stat001QuestionStudioRequest,
  profile: Sta001ExamProfile,
  difficulty?: Sta001Difficulty,
) {
  const qlId = String(request.questionLanguageId ?? "").trim().toUpperCase();
  ...
  if (!descriptor.supportedProfiles.includes(profile)) {
    throw new Error(`${descriptor.qlId} is not supported for ${profile}.`);
  }
  if (difficulty && descriptor.difficulty !== difficulty) {
    throw new Error(`${descriptor.qlId} is ${descriptor.difficulty}, not requested ${difficulty}.`);
  }
  return descriptor;
}

function assertCanonicalProblem(request: Stat001QuestionStudioRequest) {
  const cpId = String(request.canonicalProblemId ?? request.cpId ?? "").trim().toUpperCase();
  if (cpId && cpId !== STAT001_QUESTION_STUDIO_CANONICAL_PROBLEM_ID) {
    throw new Error(`Unknown canonical problem '${cpId}' for package STAT-001.`);
  }
}
...
function explanationText(explanation: {
  ...
  return [
    `Key idea: ${explanation.keyIdea}`,
    ...explanation.steps,
    `Shortcut: ${explanation.shortcut}`,
    `Common trap: ${explanation.trap}`,
  ].join("\n\n");
}

function toQuestionStudioPreview(
  source: ReturnType<typeof generateSta001Question>,
  descriptor: Stat001PermanentQlDescriptor,
  ...
  return {
    ...
    canonicalAnswer: {
      ...
      value: source.answer,
      display: source.answer,
      rendered: source.answer,
...
export async function generateStat001QuestionStudioBatch(
  request: Stat001QuestionStudioRequest = {},
) {
  assertCanonicalProblem(request);
  const language = String(request.language ?? "en").trim().toLowerCase();
  if (language !== "en") {
    throw new Error("STAT-001 permanent Question Studio review is English-only in P1; localization has not started.");
  }
  ...
  const batchSeed = String(request.seed ?? "").trim()
    || `quant-v4:STAT-001:${profile}:${difficulty ?? "mixed"}:${Date.now()}:${Math.random().toString(36).slice(2)}`;
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const source = generateSta001Question({
      seed,
      examProfile: profile,
      contractId: descriptor.contractId,
    });
    if (source.solveMode !== descriptor.solveMode || source.difficulty !== descriptor.difficulty) {
      throw new Error(`${descriptor.qlId} semantic ownership drifted from its certified Phase 0 contract.`);
    }
...
export function stat001QuestionStudioPackageCard() {
  return {
    ...
    canonicalProblems: [{
      id: STAT001_QUESTION_STUDIO_CANONICAL_PROBLEM_ID,
      label: "Measures of Central Tendency",
    }],
    permanentQlIds: STAT001_PERMANENT_QLS.map((descriptor) => descriptor.qlId),
    permanentQlCount: STAT001_PERMANENT_QLS.length,
    qls: STAT001_PERMANENT_QLS.map((descriptor) => ({
      id: descriptor.qlId,
      label: descriptor.label,
      difficulty: descriptor.difficulty,
      solveMode: descriptor.solveMode,
    })),
