...
assert.equal(numAll.length, 22);
assert.equal(numAll.filter((entry) => entry.examId === "SSC_CGL_TIER_I").length, 16);
...
assert.equal(cglTierI.normalizedCountableObservationCount, 16);
