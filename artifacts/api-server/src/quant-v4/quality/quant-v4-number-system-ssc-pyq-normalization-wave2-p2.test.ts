...
assert.equal(numAll.length, 21);
assert.equal(numAll.filter((entry) => entry.examId === "SSC_CGL_TIER_I").length, 15);
...
assert.equal(cglTierI.normalizedCountableObservationCount, 15);
