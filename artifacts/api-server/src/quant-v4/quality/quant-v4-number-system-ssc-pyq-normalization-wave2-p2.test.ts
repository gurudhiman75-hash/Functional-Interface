...
assert.equal(QUANT_V4_NUMBER_SYSTEM_SSC_WAVE2_PYQ_MIGRATION_AUTHORITY, "QUANT-V4-NUMBER-SYSTEM-SSC-PYQ-NORMALIZATION-WAVE2-P2");
...
assert.deepEqual(QUANT_V4_NUMBER_SYSTEM_SSC_WAVE2_SOURCE_LIMITATIONS.profileObservationCounts, { SSC_CGL_TIER_I: 2, SSC_CHSL: 4 });
assert.deepEqual(QUANT_V4_NUMBER_SYSTEM_SSC_WAVE2_SOURCE_LIMITATIONS.cpCoverage, ["NUM-CP-001", "NUM-CP-002", "NUM-CP-004"]);
...
for (let a = 1; a <= 15; a += 2) {
  for (let b = 1; b <= 15; b += 2) assert.equal(Math.abs((a + b + 2 * a * b) % 2), 0, "2011 Q29 parity check failed.");
}
