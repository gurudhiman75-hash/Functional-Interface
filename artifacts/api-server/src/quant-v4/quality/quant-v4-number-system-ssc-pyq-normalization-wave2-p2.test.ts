...
assert.equal(numAll.length, 24);
assert.equal(numAll.filter((entry) => entry.examId === "SSC_CGL_TIER_I").length, 18);
...
assert.equal(cglTierI.normalizedCountableObservationCount, 18);
