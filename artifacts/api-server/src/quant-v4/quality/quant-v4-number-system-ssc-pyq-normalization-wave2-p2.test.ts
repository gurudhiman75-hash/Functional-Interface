...
assert.ok(numAll.length >= 27);
assert.ok(numAll.filter((entry) => entry.examId === "SSC_CGL_TIER_I").length >= 21);
...
assert.ok(cglTierI.normalizedCountableObservationCount >= 21);
