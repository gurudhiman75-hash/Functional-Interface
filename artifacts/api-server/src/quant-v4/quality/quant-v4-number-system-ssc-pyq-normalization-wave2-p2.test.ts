...
assert.equal(numAll.length, 23);
assert.equal(numAll.filter((entry) => entry.examId === "SSC_CGL_TIER_I").length, 17);
...
assert.equal(cglTierI.normalizedCountableObservationCount, 17);
