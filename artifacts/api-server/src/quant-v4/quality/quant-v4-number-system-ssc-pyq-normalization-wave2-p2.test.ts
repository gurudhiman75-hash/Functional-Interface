...
assert.equal(numAll.length, 27);
assert.equal(numAll.filter((entry) => entry.examId === "SSC_CGL_TIER_I").length, 21);
...
assert.equal(cglTierI.normalizedCountableObservationCount, 21);
