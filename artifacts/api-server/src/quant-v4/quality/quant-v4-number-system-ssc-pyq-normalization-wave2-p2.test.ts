...
assert.equal(numAll.length, 20);
assert.equal(numAll.filter((entry) => entry.examId === "SSC_CGL_TIER_I").length, 14);
...
assert.equal(cglTierI.normalizedCountableObservationCount, 14);
