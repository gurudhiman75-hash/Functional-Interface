...
assert.equal(numAll.length, 26);
assert.equal(numAll.filter((entry) => entry.examId === "SSC_CGL_TIER_I").length, 20);
...
assert.equal(cglTierI.normalizedCountableObservationCount, 20);
