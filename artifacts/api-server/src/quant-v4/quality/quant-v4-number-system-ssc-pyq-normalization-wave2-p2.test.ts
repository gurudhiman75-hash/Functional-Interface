...
assert.equal(numAll.length, 25);
assert.equal(numAll.filter((entry) => entry.examId === "SSC_CGL_TIER_I").length, 19);
...
assert.equal(cglTierI.normalizedCountableObservationCount, 19);
