...
assert.equal(
  QUANT_V4_PERCENTAGE_CHSL_WAVE1_PYQ_MIGRATION_AUTHORITY,
  "QUANT-V4-PERCENTAGE-CHSL-PYQ-NORMALIZATION-WAVE1-P2",
);
...
assert.deepEqual(QUANT_V4_PERCENTAGE_CHSL_WAVE1_SOURCE_LIMITATIONS.packageCounts, {
  "PCT-002": 5,
});
assert.deepEqual(QUANT_V4_PERCENTAGE_CHSL_WAVE1_SOURCE_LIMITATIONS.profileObservationCounts, {
  SSC_CHSL: 5,
});
...
assert.equal(chsl.countableQuestionCount, 15);
assert.equal(chsl.topicCoverageCount, 4);
