...
assert.equal(alg001.length, 38);
assert.equal(alg002.length, 12);
const algebraAll = [...alg001, ...alg002];
assert.equal(algebraAll.length, 50);
assert.equal(algebraAll.filter((entry) => entry.examId === "SSC_CGL_TIER_I").length, 45);
...
assert.equal(cgl.countableQuestionCount, 45);
...
console.log(JSON.stringify({
  ...
  currentProfileCounts: { SSC_CGL_TIER_I: 45, SSC_CHSL: 3, SSC_CGL_TIER_II: 2 },
