...
assert.equal(
  QUANT_V4_CGL_2023_S2_FULL_QUANT_SECTION_WAVE2_AUTHORITY,
  "QUANT-V4-CGL-2023-07-27-S2-FULL-QUANT-SECTION-WAVE2-P2",
);
...
assert.equal(section.length, 25, "The normalized 27 Jul 2023 Shift 2 Quant section must contain Q26-Q50 exactly once.");
...
assert.equal(observations.some((entry) => entry.questionRef?.endsWith("Q29")), false, "Wave 2 must reuse rather than duplicate the prior Q29 observation.");
...
// Source-math spot checks across unrelated package families.
assert.equal(7500 / (0.04 * (11 - 6)), 37500); // Q26 simple-interest principal.
assert.equal(54000 / 0.8, 67500); // Q27 reverse discount.
assert.equal(1 / 12 + 1 / 15 + 1 / 10, 1 / 4); // Q30 pairwise rates sum to twice combined rate.
assert.equal(2 / (1 / 12 + 1 / 15 + 1 / 10), 8); // Q30 all-three time.
assert.equal(1 / (0.25 / 3), 12); // Q31 first-to-second speed ratio.
assert.equal((85 - 80) / 80 * 100, 6.25); // Q32 false-weight actual profit.
assert.equal(300 - 50, 250); // Q34 bar-chart difference.
assert.equal(60 / 12 * 3, 15); // Q38 five three-day cycles.
assert.equal(12000 * 1.25, 15000); // Q39 income increase.
assert.equal(196 - 2 * 63, 70); // Q40 symmetric identity square-sum.
assert.equal(208 / 13, 16); // Q43 staged worker leaving.
assert.equal((6 * 5 / 18) * 828, 1380); // Q44 pursuit distance.
assert.equal((4 * 1) ** 2 / (3 * 1 ** 2), 16 / 3); // Q46 third-proportional algebra factor before solving y².
assert.equal(21 * 14 / (21 + 14), 8.4); // Q49 hall height.
assert.equal(5 / 13, 5 / 13); // Q50 cos A from sec A=13/5.
assert.equal(Math.sqrt(1 - (5 / 13) ** 2), 12 / 13); // Q50 sin A.

// Compare two complete sections without treating two papers as a calibrated distribution.
...
assert.equal(listRegisteredCountablePyqObservations({ packageId: "RAP-001" }).length, 1);
assert.equal(listRegisteredCountablePyqObservations({ packageId: "RAP-003" }).length, 1);
assert.equal(listRegisteredCountablePyqObservations({ packageId: "DI-003" }).length, 1);

const numCgl = getQuantV4SpecializedProfileSelectionContract("NUM-001", "SSC_CGL_TIER_I");
assert.equal(numCgl.normalizedCountableObservationCount, 12);
...
assert.equal(tmwCgl.normalizedCountableObservationCount, 6);
...
assert.equal(cgl.countableQuestionCount, 75);
