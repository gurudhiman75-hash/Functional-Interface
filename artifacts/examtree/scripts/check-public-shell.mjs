...
assert.match(appSource, /path="\/performance" component=\{\(\) => <ProtectedRoute component=\{Performance\} \/>\}/, "Performance must require a student session and stay in the preparation shell once canonical self-analytics is live");
...
assert.match(publicLayout, /label: "Performance"[\s\S]{0,100}href: "\/performance"[\s\S]{0,100}authNext: "\/performance"/, "mobile study navigation must expose live protected Performance once canonical self-analytics is implemented");
assert.doesNotMatch(publicLayout, /label: "Performance"[\s\S]{0,80}disabled: true/, "implemented Performance must not remain disabled in mobile navigation");
...
for (const href of ["/", "/exams", "/dashboard", "/performance", "/bookmarks", "/profile", "/contact"]) {
...
assert.match(publicHomeSidebar, /href: "\/performance"[\s\S]{0,120}label: "Performance"[\s\S]{0,120}authNext: "\/performance"/, "Performance must be a live protected study-sidebar destination once canonical self-analytics is implemented");
assert.doesNotMatch(publicHomeSidebar, /label: "Performance"[\s\S]{0,80}disabled: true/, "implemented Performance must not remain disabled in the public study sidebar");
for (const label of ["Home", "Explore Exams", "My Tests", "Performance", "Bookmarks", "Downloads", "Study Plan", "Rewards", "Support", "Settings"]) {
...
console.log("Public/app shell audit passed (full-width Home + live protected Performance + truthful detailed study navigation + hardened >=44px public header contract).\n");