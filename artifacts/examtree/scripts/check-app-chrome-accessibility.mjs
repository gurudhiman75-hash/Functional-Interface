...
assert.match(sidebar, /<SidebarMenuButton[\s\S]*?className="min-h-11[^\"]*border border-transparent/, "primary sidebar navigation must use 44px-class targets");
...
assert.match(sidebar, /<SidebarMenuButton[\s\S]*?className="min-h-11[^\"]*"[\s\S]*?>\s*<Link href="\/login\/student">Login<\/Link>/, "signed-out sidebar login must meet the 44px-class touch target");
