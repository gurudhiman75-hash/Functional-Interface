...
assert.match(proof, /page\.setViewportSize\(\{ width: 390, height: 844 \}\)/, "runner/result certification must retain a narrow mobile viewport");
...
assert.match(proof, /Pause & Exit[\s\S]*?₹720[\s\S]*?\^Next[\s\S]*?\^Submit/, "runner certification must cover critical exam controls at scale");
...
assert.match(proof, /Canonical saved result|Solution review/, "result certification must cover the canonical saved-result surface");
assert.match(proof, /Back to My Activity[\s\S]*?All \(2\)[\s\S]*?Retake test/, "result certification must cover result navigation, filters, and retake action");
assert.match(proof, /canvas\.getContext\("2d"/, "contrast proof must use the browser CSS color parser for modern color syntax");

console.log("Runner/result zoom and contrast audit passed (12 assertions).");
