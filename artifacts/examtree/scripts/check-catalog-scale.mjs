...
assert.match(testsPage, /attemptDelta = Number\(right\.attemptCount \?\? 0\) - Number\(left\.attemptCount \?\? 0\)/, "popular series must rank by real attempt counts");
assert.match(testsPage, /data-testid="popular-series-tab"/);
assert.match(testsPage, /data-testid="live-mocks-tab"/);
assert.match(testsPage, /role="tablist" aria-label="Featured preparation"/);
...
assert.match(proof, /getByTestId\("popular-series-tab"\)/);
assert.match(proof, /getByTestId\("live-mocks-tab"\)\.click\(\)/);
...
console.log("Catalog scale audit passed (66 assertions).");
