  ...
  return (
    <div className="bg-[#f8fafc] py-5 sm:py-7">
      <div className="mx-auto w-full max-w-7xl space-y-10 px-4 sm:px-6 lg:px-8">
        {sampleMode ? (
          <div className="flex flex-col gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 sm:flex-row sm:items-center sm:justify-between" data-testid="exams-sample-preview-badge">
            <span><strong>Sample data preview</strong> · 7 exam families, 98 tests and 4 test series are loaded only to review this marketplace layout.</span>
            <div className="flex gap-2"><button type="button" className="min-h-11 rounded-lg px-3 text-xs font-black hover:bg-amber-100" onClick={() => setLocation("/?preview=sample")}>Compare Home</button><button type="button" className="min-h-11 rounded-lg px-3 text-xs font-black hover:bg-amber-100" onClick={() => setLocation("/exams")}>Exit preview</button></div>
          </div>
        ) : null}

        <section className="overflow-hidden rounded-2xl bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-800 p-5 text-white shadow-lg shadow-blue-950/10 sm:p-7" aria-labelledby="explore-exams-heading" data-testid="exam-discovery-command-center">
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.72fr)] lg:items-end">
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-[11px] font-black text-blue-50"><span className="rounded-full bg-white/10 px-3 py-1.5 ring-1 ring-white/15">{formatCount(categoryNodes.length)} exam families</span><span className="rounded-full bg-white/10 px-3 py-1.5 ring-1 ring-white/15">{formatCount(tests.length)} published tests</span><span className="rounded-full bg-white/10 px-3 py-1.5 ring-1 ring-white/15">{formatCount(freeTests.length)} free tests</span></div>
            </div>
            <div>
              <label htmlFor="exam-category-search" className="sr-only">Search exam categories</label>
              <div className="relative"><Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" /><input id="exam-category-search" type="search" value={categoryQuery} onChange={(event) => setCategoryQuery(event.target.value)} placeholder="Search SSC, Banking, Railways, Punjab exams…" className="min-h-[52px] w-full rounded-xl border border-white/80 bg-white pl-12 pr-4 text-sm font-semibold text-slate-950 shadow-xl outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-white/30" /></div>
            </div>
          </div>

          <div className="mt-6 flex gap-3 overflow-x-auto pb-1" data-testid="exam-category-logo-row">
            {filteredCategories.map((category) => (
              <button key={category.id} type="button" onClick={() => goCategory(category.id)} className="et-interactive flex min-h-[74px] min-w-[150px] items-center gap-3 rounded-xl border border-white/75 bg-white px-3.5 py-3 text-left text-slate-950 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
          ...
          {featuredSeries.length > 0 ? (
            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {featuredSeries.map((seriesItem, index) => (
                <article key={seriesItem.id} className={`overflow-hidden rounded-2xl border bg-gradient-to-br p-5 shadow-lg shadow-slate-900/10 ${FEATURE_TONES[index % FEATURE_TONES.length]}`}>
                  <div className="flex items-start justify-between gap-3"><ExamLogo name={`${seriesItem.examFamilyName} ${seriesItem.examName}`} size="lg" /><span className="rounded-full bg-white/10 px-2.5 py-1 text-[10px] font-black text-white ring-1 ring-white/15">{index === 0 && seriesItem.attemptCount > 0 ? "MOST ATTEMPTED" : "LIVE"}</span></div>
                  <p className="mt-5 text-[11px] font-black uppercase tracking-wide text-blue-200">{seriesItem.examName}</p><h3 className="mt-1 line-clamp-2 min-h-11 text-base font-black leading-5 text-white">{seriesItem.name}</h3>
        ...
        <section className="overflow-hidden rounded-2xl border border-emerald-200 bg-emerald-50/60" data-testid="full-length-series-section" aria-labelledby="full-length-series-heading">
          ...
          {fullLengthSeries.length > 0 ? (
            <div className="divide-y divide-emerald-200/70">
              {fullLengthSeries.map((seriesItem, index) => (
                <div key={seriesItem.id} className="grid gap-4 bg-white/65 px-5 py-4 transition hover:bg-white sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center sm:px-6">
                  <div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-700 text-xs font-black text-white">{index + 1}</span><ExamLogo name={`${seriesItem.examFamilyName} ${seriesItem.examName}`} /></div>
                  <div className="min-w-0"><p className="truncate text-sm font-black text-slate-950">{seriesItem.name}</p><p className="mt-1 text-xs font-semibold text-slate-500">{seriesItem.examName} · {formatCount(seriesItem.fullLengthTestCount)} full-length tests · {formatCount(seriesItem.questionCount)} questions</p><div className="mt-2 h-1.5 max-w-md overflow-hidden rounded-full bg-emerald-100"><div className="h-full rounded-full bg-emerald-600" style={{ width: `${Math.min(100, 35 + index * 15)}%` }} /></div></div>
                  <Button className="min-h-11 w-full sm:w-auto" variant="outline" onClick={() => goSeries(seriesItem.id)}>Open series <ArrowRight className="ml-1.5 h-4 w-4" /></Button>
                </div>
              ))}
            </div>
        ...
        <section data-testid="free-practice-section" aria-labelledby="free-practice-heading">
          ...
          <div role="tablist" aria-label="Free practice type" className="mt-4 inline-flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm">{([["sectional", "Sectional Tests"], ["topic-wise", "Topic Tests"], ["all", "All Free"]] as const).map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={freePracticeTab === value} onClick={() => setFreePracticeTab(value)} className={`et-interactive min-h-11 rounded-lg px-3.5 text-sm font-black transition ${freePracticeTab === value ? "bg-blue-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}>{label}</button>)}</div>
          {freePracticeTests.length > 0 ? (
            <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="hidden grid-cols-[minmax(0,1.4fr)_140px_110px_110px] gap-3 border-b border-slate-200 bg-slate-50 px-4 py-2.5 text-[10px] font-black uppercase tracking-wide text-slate-500 md:grid"><span>Test</span><span>Exam</span><span>Questions</span><span>Action</span></div>
              <div className="divide-y divide-slate-100">{freePracticeTests.map((test) => (
                <div key={test.id} className="grid gap-3 px-4 py-3 md:grid-cols-[minmax(0,1.4fr)_140px_110px_110px] md:items-center"><div className="min-w-0"><p className="truncate text-sm font-black text-slate-950">{test.name}</p><p className="mt-0.5 text-xs text-slate-500">{test.duration} min · {test.kind?.replace("-", " ")}</p></div><p className="truncate text-xs font-semibold text-slate-600">{test.subcategoryName || test.category}</p><p className="text-xs font-black text-slate-700">{test.totalQuestions} Q</p><Button className="min-h-11" size="sm" onClick={() => goTest(test.id)}>Start Free</Button></div>
              ))}</div>
            </div>
        ...
        <section data-testid="daily-practice-section" aria-labelledby="daily-practice-heading">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {dailyChallenge ? <button type="button" onClick={() => goTest(dailyChallenge.testId)} className="et-interactive rounded-2xl bg-gradient-to-br from-indigo-600 to-blue-700 p-5 text-left text-white shadow-sm"><div className="flex items-center justify-between"><span className="rounded-full bg-white/15 px-2.5 py-1 text-[10px] font-black">DAILY CHALLENGE</span><Target className="h-5 w-5 text-blue-100" /></div><h3 className="mt-4 line-clamp-2 text-base font-black">{dailyChallenge.testName}</h3><p className="mt-2 text-xs font-semibold text-blue-100">{dailyChallenge.date}{dailyChallenge.totalParticipants > 0 ? ` · ${formatCount(dailyChallenge.totalParticipants)} participants` : ""}</p></button> : null}
            {dailyNamedTests.map((test, index) => <button key={test.id} type="button" onClick={() => goTest(test.id)} className={`et-interactive rounded-2xl p-5 text-left shadow-sm ${index === 0 ? "bg-amber-100 text-amber-950" : "bg-emerald-100 text-emerald-950"}`}><div className="flex items-center justify-between"><span className="rounded-full bg-white/50 px-2.5 py-1 text-[10px] font-black">DAILY PRACTICE</span><CalendarDays className="h-5 w-5" /></div><h3 className="mt-4 line-clamp-2 text-base font-black">{test.name}</h3><p className="mt-2 text-xs font-semibold opacity-70">{test.totalQuestions} questions · {test.duration} min</p></button>)}
        ...
        {(pyqTests.length > 0 || subjectSummaries.length > 0) ? (
          <section className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="rounded-2xl border border-amber-200 bg-amber-50/70 p-5 sm:p-6" data-testid="pyq-section">
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-2">{pyqTests.map((test) => <button key={test.id} type="button" onClick={() => goTest(test.id)} className="et-interactive flex min-h-[78px] items-center gap-3 rounded-xl border border-amber-200 bg-white p-3 text-left hover:shadow-sm"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-amber-100 text-amber-800"><FileClock className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block line-clamp-2 text-sm font-black text-slate-950">{test.name}</span><span className="mt-1 block text-xs text-slate-500">{test.totalQuestions} questions</span></span><ChevronRight className="h-4 w-4 shrink-0 text-amber-700" /></button>)}</div>
            </div>

            <div className="rounded-2xl bg-slate-950 p-5 text-white sm:p-6" data-testid="subject-practice-section">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">{subjectSummaries.slice(0, 6).map((subject) => { const Icon = subject.icon; return <button key={subject.label} type="button" onClick={() => subject.firstTestId ? goTest(subject.firstTestId) : setLocation("/exams")} className="et-interactive min-h-[90px] rounded-xl border border-white/10 bg-white/5 p-3 text-left hover:bg-white/10"><Icon className="h-5 w-5 text-blue-300" /><span className="mt-2 block text-sm font-black">{subject.short}</span><span className="mt-0.5 block text-[11px] font-semibold text-slate-400">{formatCount(subject.count)} tests</span></button>; })}</div>
            </div>
          </section>
        ) : null}

        {popularTests.length > 0 ? (
          <section className="overflow-hidden rounded-2xl bg-gradient-to-r from-rose-50 via-white to-orange-50 ring-1 ring-rose-100" data-testid="popular-tests-section" aria-labelledby="popular-tests-heading">
            ...
            <div className="grid divide-y divide-rose-100 md:grid-cols-5 md:divide-x md:divide-y-0">{popularTests.map((test, index) => <button key={test.id} type="button" onClick={() => goTest(test.id)} className="et-interactive min-h-[126px] p-4 text-left hover:bg-white"><div className="flex items-center justify-between"><span className="text-lg font-black text-rose-500">#{index + 1}</span><span className="text-[10px] font-black text-slate-500">{formatCount(test.attempts)} attempts</span></div><h3 className="mt-3 line-clamp-2 text-sm font-black leading-5 text-slate-950">{test.name}</h3><p className="mt-2 truncate text-xs font-semibold text-slate-500">{test.subcategoryName || test.category}</p></button>)}</div>
          </section>
        ) : null}

        <section data-testid="all-tests-section">
