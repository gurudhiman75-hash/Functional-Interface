...
const fallbackCategoryIcons = ["Landmark", "BadgeCheck", "Building2", "GraduationCap", "BriefcaseBusiness", "Monitor"];
...
function makeFallbackCategories(tests: Test[]): Category[] {
  const names = Array.from(new Set(tests.map((test) => test.category).filter(Boolean)));
  return names.map((name, index) => ({
    id: name.toLowerCase().replace(/[^a-z0-9]+/g, "-"),
    name,
    description: `${name} test series and practice blueprints.`,
    icon: fallbackCategoryIcons[index % fallbackCategoryIcons.length],
    color: "indigo",
...
function buildNodes(categories: Category[], subcategories: Subcategory[], tests: Test[]): CategoryNode[] {
  const sourceCategories = categories.length > 0 ? categories : makeFallbackCategories(tests);
  return sourceCategories.map((category, index) => {
    ...
    const explicitSubcategories = subcategories.filter((sub) => sub.categoryId === category.id);
    const subcategoryIdsFromTests = Array.from(
      new Map(
        categoryTests.map((test) => [
          test.subcategoryId ?? test.subcategoryName ?? "general",
          {
    ...
    const mergedSubcategories = explicitSubcategories.length > 0
    ...
    return {
      id: category.id,
      name: category.name,
      description: category.description || `${category.name} focused exam preparation.`,
      icon: category.icon || fallbackCategoryIcons[index % fallbackCategoryIcons.length],
      tests: categoryTests,
      subcategories: mergedSubcategories
        .map((sub) => ({
          ...
          tests: categoryTests.filter(
            (test) =>
              test.subcategoryId === sub.id ||
              test.subcategoryName === sub.name ||
              (sub.id === "general" && !test.subcategoryId),
          ),
        }))
