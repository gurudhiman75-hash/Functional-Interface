  ...
  return (
    <div className="space-y-8">
      {seriesQuery.isError && (
        <section className="mx-auto max-w-7xl rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900" role="status">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p>Test series could not be loaded. The standalone catalog below is still available.</p>
            <Button size="sm" variant="outline" onClick={() => void seriesQuery.refetch()}>Retry series</Button>
          </div>
        </section>
      )}
              ...
              return (
                <article key={seriesItem.id} className="rounded-xl border bg-background p-4 transition hover:border-primary/35 hover:shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div><p className="text-xs font-medium text-muted-foreground">{seriesItem.examName}</p><h3 className="mt-1 font-semibold">{seriesItem.name}</h3></div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                    <span className="inline-flex items-center gap-1"><Layers3 className="h-3.5 w-3.5" />{seriesItem.testCount} tests</span>
                    <span className="inline-flex items-center gap-1"><Target className="h-3.5 w-3.5" />{progressionLabel(seriesItem.progressionMode)}</span>
                    <span>{seriesItem.questionCount} questions</span>
                    <span>{Math.max(1, Math.ceil(seriesItem.durationSeconds / 60))} minutes total</span>
                  </div>
      ...
      {publishedTestsQuery.isError && (
        <section className="mx-auto max-w-7xl rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900" role="status">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p>Live standalone tests could not be loaded. Browse the full catalog below while you retry.</p>
            <Button size="sm" variant="outline" onClick={() => void publishedTestsQuery.refetch()}>Retry live tests</Button>
          </div>
        </section>
      )}
      ...
      {publishedTests.length > 0 ? (
        <section className="mx-auto max-w-7xl rounded-2xl border border-primary/20 bg-primary/5 p-5 shadow-sm" data-testid="published-tests-section">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Standalone tests</p><h2 className="mt-1 text-2xl font-semibold">Live mock tests</h2></div>
            <p className="text-sm text-muted-foreground">{publishedTests.length} published</p>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {visiblePublishedTests.map((test) => (
              <article key={test.id} className="rounded-xl border bg-card p-4">
                ...
                <h3 className="mt-1 line-clamp-2 font-semibold">{test.title}</h3>
      ...
      <section aria-labelledby="catalog-path-heading" className="mx-auto max-w-7xl">
        <div className="mb-4">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-teal-700">Browse by exam path</p>
          <h2 id="catalog-path-heading" className="mt-1 text-2xl font-semibold text-slate-950">Explore the catalog hierarchy</h2>
        </div>
