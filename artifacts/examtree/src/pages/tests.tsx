...
function SectionHeader({ eyebrow, title, description, trailing }: { eyebrow?: string; title: string; description?: string; trailing?: React.ReactNode }) {
  return (
    ...
    <div className="bg-[#f8fafc] py-5 sm:py-7">
      <div className="mx-auto w-full max-w-7xl space-y-9 px-4 sm:px-6 lg:px-8">
        <section aria-labelledby="explore-exams-heading">
          ...
          <div className="mt-5 flex gap-3 overflow-x-auto pb-2" data-testid="exam-category-logo-row">
            ...
            {filteredCategories.length === 0 ? (
              <div className="w-full rounded-xl border border-dashed border-slate-300 bg-white px-5 py-5 text-center text-sm text-muted-foreground">
                No exam category matches “{categoryQuery.trim()}”.
              </div>
            ) : null}
          </div>
        </section>
        ...
        <section data-testid="featured-series-section" aria-labelledby="featured-series-heading">
          <SectionHeader
            eyebrow="Featured preparation"
            title="Featured Test Series"
            description="Start with the strongest live series in the current catalog. Popularity uses real evaluated attempts when available."
            trailing={<span className="text-xs font-bold text-muted-foreground">{series.length} live series</span>}
          />

          {seriesQuery.isError ? (
            <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><span>Featured series could not be loaded.</span><Button className="min-h-11" size="sm" variant="outline" onClick={() => void seriesQuery.refetch()}>Retry series</Button></div>
            </div>
                ...
                return (
                  <article key={seriesItem.id} className={`overflow-hidden rounded-2xl border bg-gradient-to-br p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${FEATURE_TONES[index % FEATURE_TONES.length]}`}>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${index === 0 && seriesItem.attemptCount > 0 ? "bg-amber-100 text-amber-800" : upcoming ? "bg-violet-100 text-violet-700" : "bg-emerald-100 text-emerald-700"}`}>
                        {index === 0 && seriesItem.attemptCount > 0 ? "MOST ATTEMPTED" : upcoming ? "UPCOMING" : "OPEN"}
                      </span>
                    </div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-x-3 gap-y-1 text-xs font-semibold text-slate-600">
                      <span>{seriesItem.testCount} tests</span>
                      <span>{formatCount(seriesItem.questionCount)} questions</span>
                      {seriesItem.attemptCount > 0 ? <span className="inline-flex items-center gap-1"><Flame className="h-3.5 w-3.5 text-orange-500" aria-hidden="true" />{formatCount(seriesItem.attemptCount)} attempts</span> : null}
                    </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/80 p-5 shadow-sm sm:p-6" data-testid="full-length-series-section" aria-labelledby="full-length-series-heading">
          <SectionHeader
            ...
            description="Complete mock sequences for exams that already have full-length tests in the live catalog."
            trailing={<Trophy className="h-5 w-5 text-amber-500" aria-hidden="true" />}
          />
          ...
          {fullLengthSeries.length > 0 ? (
              ...
              {fullLengthSeries.map((seriesItem, index) => (
                <article key={seriesItem.id} className="group rounded-xl border border-slate-200 bg-white p-4 transition hover:border-blue-200 hover:shadow-sm">
                  <div className="flex items-center gap-3">
                    <ExamLogo name={`${seriesItem.examFamilyName} ${seriesItem.examName}`} size="md" />
                    <div className="min-w-0">
                      <p className="truncate text-sm font-black text-slate-950">{seriesItem.examName}</p>
                      <p className="truncate text-xs font-semibold text-slate-500">Full-length series</p>
                    </div>
                  </div>
                  <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                    <div className="rounded-lg bg-slate-50 px-2.5 py-2"><span className="block font-black text-slate-900">{seriesItem.testCount}</span><span className="text-slate-500">Tests</span></div>
                    <div className="rounded-lg bg-slate-50 px-2.5 py-2"><span className="block font-black text-slate-900">{formatCount(seriesItem.questionCount)}</span><span className="text-slate-500">Questions</span></div>
                  </div>
            ...
            <div className="mt-4 flex flex-col gap-3 rounded-xl bg-blue-50/70 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div><p className="font-black text-slate-900">Full-length mocks are live</p><p className="mt-1 text-sm text-slate-600">A canonical full-length series has not been published for these mocks yet.</p></div>
              <Button className="min-h-11" variant="outline" onClick={() => setLocation(`/test/${fullLengthTests[0].id}`)}>Open a full-length mock</Button>
            </div>
          ) : (
        ...
        <section className="rounded-2xl bg-gradient-to-br from-blue-50/80 via-white to-indigo-50/70 p-5 ring-1 ring-blue-100 sm:p-6" data-testid="free-practice-section" aria-labelledby="free-practice-heading">
          <SectionHeader
            eyebrow="Practice without a paywall"
            title="Free Practice"
            description="Jump into published sectional or topic-wise practice."
            trailing={<span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">{formatCount(freeTests.length)} free tests</span>}
          />
          ...
          <div role="tablist" aria-label="Free practice type" className="mt-4 inline-flex rounded-xl border border-blue-100 bg-white p-1 shadow-sm">
            {([
              ...
              <button
                ...
                type="button"
                role="tab"
                aria-selected={freePracticeTab === value}
                onClick={() => setFreePracticeTab(value)}
                className={`et-interactive min-h-11 rounded-lg px-3.5 text-sm font-bold transition ${freePracticeTab === value ? "bg-blue-600 text-white shadow-sm" : "text-slate-600 hover:bg-blue-50 hover:text-blue-700"}`}
              >{label}</button>
            ))}
          </div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {freePracticeTests.map((test) => (
                <article key={test.id} className="rounded-xl border border-blue-100 bg-white p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-blue-700"><Target className="h-5 w-5" aria-hidden="true" /></span>
                    <span className="rounded-md bg-emerald-50 px-2 py-1 text-[10px] font-black uppercase text-emerald-700">Free</span>
                  </div>
                  <p className="mt-3 truncate text-[11px] font-black uppercase tracking-wide text-blue-600">{test.subcategoryName || test.categoryName || test.category}</p>
                  <h3 className="mt-1 line-clamp-2 min-h-10 text-sm font-black leading-5 text-slate-950">{test.name}</h3>
                  <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-xs font-semibold text-slate-500"><span>{test.totalQuestions} questions</span><span>{test.duration} min</span></div>
        ...
        <section data-testid="daily-practice-section" aria-labelledby="daily-practice-heading">
          <SectionHeader eyebrow="Build a habit" title="Daily Practice" description="Short, current practice surfaced from real daily or challenge content." />
          <div className="mt-4 rounded-2xl bg-gradient-to-r from-indigo-50 via-emerald-50 to-orange-50 p-3 ring-1 ring-slate-200/70">
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {dailyChallenge ? (
                <button type="button" onClick={() => setLocation(`/test/${dailyChallenge.testId}`)} className="et-interactive group min-h-[104px] rounded-xl border border-indigo-100 bg-white/90 p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                  <div className="flex items-start gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700"><CalendarDays className="h-5 w-5" /></span><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-wide text-indigo-600">Today</p><h3 className="mt-1 truncate text-sm font-black text-slate-950">{dailyChallenge.testName}</h3><p className="mt-1 text-xs text-slate-500">{dailyChallenge.totalParticipants > 0 ? `${formatCount(dailyChallenge.totalParticipants)} participants` : "Daily challenge"}</p></div></div>
                </button>
              ) : null}
              {dailyNamedTests.map((test, index) => {
                ...
                return (
                  <button key={test.id} type="button" onClick={() => setLocation(`/test/${test.id}`)} className="et-interactive group min-h-[104px] rounded-xl border border-slate-200 bg-white/90 p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                    <div className="flex items-start gap-3"><span className={`flex h-10 w-10 items-center justify-center rounded-xl ${accents[index % accents.length]}`}><Sparkles className="h-5 w-5" /></span><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-wide text-slate-500">Quick practice</p><h3 className="mt-1 line-clamp-2 text-sm font-black text-slate-950">{test.name}</h3><p className="mt-1 text-xs text-slate-500">{test.totalQuestions} questions</p></div></div>
                  </button>
                );
              })}
              {!dailyChallenge && dailyNamedTests.length === 0 ? (
                <div className="col-span-full rounded-xl border border-dashed border-slate-300 bg-white/70 px-5 py-6 text-center text-sm text-muted-foreground">No daily challenge is published right now. This shelf will activate automatically when one is live.</div>
              ) : null}
            </div>
          </div>
        </section>

        <section className="grid gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(300px,0.75fr)]" data-testid="pyq-section" aria-labelledby="pyq-heading">
          <div>
            ...
            {pyqTests.length > 0 ? (
              <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                {pyqTests.map((test, index) => (
                  <button key={test.id} type="button" onClick={() => setLocation(`/test/${test.id}`)} className={`et-interactive flex min-h-[68px] w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-slate-50 ${index > 0 ? "border-t border-slate-100" : ""}`}>
                    <ExamLogo name={`${test.categoryName || test.category} ${test.name}`} size="sm" />
                    <span className="min-w-0 flex-1"><span className="block truncate text-sm font-black text-slate-950">{test.name}</span><span className="mt-0.5 block text-xs text-slate-500">{test.totalQuestions} questions · {test.duration} min</span></span>
                    <ChevronRight className="h-4 w-4 text-slate-400" aria-hidden="true" />
                  </button>
                ))}
              </div>
            ) : (
              <div className="mt-4 rounded-xl border border-dashed border-slate-300 bg-white px-5 py-7 text-sm text-muted-foreground">No catalog tests are currently tagged as PYQs.</div>
            )}
          </div>
          <div className="flex min-h-full flex-col justify-between rounded-2xl bg-slate-950 p-5 text-white shadow-sm">
            <div><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-white/10"><BookOpen className="h-5 w-5" /></span><p className="mt-5 text-xs font-black uppercase tracking-[0.14em] text-blue-200">Dedicated PYQ Hub</p><h3 id="pyq-heading" className="mt-1 text-xl font-black">Revise by exam, year and paper type</h3><p className="mt-2 text-sm leading-6 text-slate-300">Open the dedicated PYQ workspace for exam-wise previous-paper discovery as collections are published.</p></div>
            <Button className="mt-5 min-h-11 bg-white text-slate-950 hover:bg-slate-100" onClick={() => setLocation("/pyqs")}>Open PYQ Hub <ArrowRight className="ml-2 h-4 w-4" /></Button>
          </div>
        </section>

        {subjectSummaries.length > 0 ? (
          <section data-testid="subject-practice-section" aria-labelledby="subject-practice-heading">
            <SectionHeader eyebrow="Drill by skill" title="Subject-wise Practice" description="Subjects are surfaced only when matching published tests exist." />
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
              {subjectSummaries.map((subject, index) => {
                const Icon = subject.icon;
                const tones = ["bg-blue-50 text-blue-700", "bg-violet-50 text-violet-700", "bg-emerald-50 text-emerald-700", "bg-orange-50 text-orange-700", "bg-cyan-50 text-cyan-700"];
                return (
                  <button key={subject.label} type="button" disabled={!subject.firstTestId} onClick={() => subject.firstTestId && setLocation(`/test/${subject.firstTestId}`)} className="et-interactive rounded-xl border border-slate-200 bg-white p-4 text-center shadow-sm transition hover:-translate-y-0.5 hover:shadow-md disabled:cursor-default">
                    <span className={`mx-auto flex h-11 w-11 items-center justify-center rounded-xl ${tones[index % tones.length]}`}><Icon className="h-5 w-5" /></span>
                    <h3 className="mt-3 text-sm font-black text-slate-950">{subject.label}</h3>
                    <p className="mt-1 text-xs font-semibold text-slate-500">{formatCount(subject.count)} matching tests</p>
                  </button>
                );
              })}
            </div>
        ...
        {popularTests.length > 0 ? (
          <section className="overflow-hidden rounded-2xl bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950 p-5 text-white shadow-sm sm:p-6" data-testid="popular-tests-section" aria-labelledby="popular-tests-heading">
            <SectionHeader eyebrow="Real activity" title="Popular / Trending Tests" description="Ranked by the attempt counts already present in the live catalog." trailing={<Flame className="h-5 w-5 text-orange-400" />} />
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              {popularTests.map((test) => (
                <button key={test.id} type="button" onClick={() => setLocation(`/test/${test.id}`)} className="et-interactive rounded-xl border border-white/10 bg-white/5 p-3 text-left transition hover:bg-white/10">
                  <div className="flex items-center gap-2.5"><ExamLogo name={`${test.categoryName || test.category} ${test.name}`} size="sm" /><div className="min-w-0"><p className="truncate text-xs font-bold text-blue-200">{test.subcategoryName || test.categoryName || test.category}</p><h3 className="mt-0.5 line-clamp-2 text-sm font-black text-white">{test.name}</h3></div></div>
                  <div className="mt-3 flex items-center justify-between gap-2 text-[11px] font-semibold text-slate-300"><span>{test.totalQuestions} Qs</span><span className="inline-flex items-center gap-1 text-orange-300"><Flame className="h-3 w-3" />{formatCount(test.attempts)} attempts</span></div>
                </button>
              ))}
            </div>
          </section>
        ...
        <section className="border-t border-slate-200 pt-8" aria-labelledby="full-catalog-heading">
          <SectionHeader eyebrow="Everything published" title="All Tests" description="Use the full filterable catalog when you know exactly what you want to practise." trailing={<LayoutGrid className="h-5 w-5 text-teal-600" />} />
          <div className="mt-4"><CatalogTestBrowser tests={tests} /></div>
        </section>
      </div>
    </div>
  );
