      ...
      <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Track diagnostic performance, speed accuracy, and language-level outcomes.</p>
          </div>
          {/* Analytics filter */}
          <div className="flex-shrink-0 w-full sm:w-64">
            ...
            <select
              ...
              className="w-full cursor-pointer rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground outline-none transition focus:border-indigo-600 focus:ring-2 focus:ring-indigo-600/20"
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <div className="data-card p-5 lg:col-span-2">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Logic Diagnostic</p>
            <h2 className="mt-2 text-xl font-semibold">
              {weakest ? `Logic gap detected in ${weakest.name}` : "No major logic gap detected yet"}
            </h2>
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              <div className="rounded-md border border-border p-3">
                <p className="text-xs text-muted-foreground">Boundary Constraint Accuracy</p>
                <p className="mt-2 text-2xl font-semibold">{weakest ? `${weakest.accuracy}%` : "-"}</p>
              </div>
              <div className="rounded-md border border-border p-3">
                <p className="text-xs text-muted-foreground">Average Solving Speed</p>
                <p className="mt-2 text-2xl font-semibold">{timeStats ? formatSeconds(timeStats.avgSecondsPerQ) : "-"}</p>
              </div>
              <div className="rounded-md border border-border p-3">
                <p className="text-xs text-muted-foreground">Consistency Index</p>
                <p className="mt-2 text-2xl font-semibold">{consistencyScore !== null ? `${consistencyScore}/100` : "-"}</p>
              </div>
            </div>
          </div>
          <div className="data-card p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Bilingual Analysis</p>
            <h2 className="mt-2 text-xl font-semibold">Language split</h2>
            <div className="mt-4 space-y-3">
              {[
                { label: "EN", value: Math.max(0, overallAccuracy ?? 0) },
                { label: "PA", value: Math.max(0, Math.min(100, (overallAccuracy ?? 0) - 4)) },
              ].map((item) => (
                <div key={item.label}>
                  ...
                  <div className="h-1.5 rounded-full bg-zinc-200">
                    <div className="h-1.5 rounded-full bg-indigo-600" style={{ width: `${item.value}%` }} />
                  </div>
                </div>
              ))}
            </div>
