...
import LogicPlaybackRouter from "@/components/learning/LogicPlaybackRouter";
...
function reviewState(item: ReviewItem) {
...
export default function CanonicalResult() {
  ...
  const result = (attemptId ? resultQuery.data : localResult) as
    | CanonicalResult
    | null
    | undefined;
  const review = (result?.questionReview ?? []) as ReviewItem[];
  ...
  const seriesUrl = result.seriesId
    ? `/test-series/${encodeURIComponent(result.seriesId)}`
    : null;
  const retryUrl = `/test/${encodeURIComponent(result.testId)}${
    result.seriesId ? `?seriesId=${encodeURIComponent(result.seriesId)}` : ""
  }`;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="overflow-hidden rounded-2xl border bg-card shadow-sm">
        <div className="bg-gradient-to-br from-primary/15 via-background to-background p-6 text-center sm:p-9">
          ...
          {result.actualScore != null ? (
            <p className="mt-2 text-sm font-semibold text-muted-foreground">
              Marks scored: {result.actualScore}
            </p>
          ) : null}
        ...
        <div className="grid gap-px border-t bg-border sm:grid-cols-2 lg:grid-cols-4">
          <div className="bg-card p-4">
            ...
            <p className="mt-1 text-2xl font-semibold text-emerald-600">{result.correct}</p>
          </div>
          <div className="bg-card p-4">
            <p className="text-xs text-muted-foreground">Wrong</p>
            <p className="mt-1 text-2xl font-semibold text-red-600">{result.wrong}</p>
          </div>
          <div className="bg-card p-4">
            <p className="text-xs text-muted-foreground">Unanswered</p>
            <p className="mt-1 text-2xl font-semibold">{result.unanswered}</p>
          </div>
          <div className="bg-card p-4">
            <p className="text-xs text-muted-foreground">Time spent</p>
            <p className="mt-1 text-2xl font-semibold">{formatDuration(result.timeSpent)}</p>
          </div>
        </div>
      </section>
            ...
            {result.sectionStats.map((section) => (
              <div key={section.name} className="rounded-xl border p-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold">{section.accuracy}%</span>
                </div>
                <Progress value={section.accuracy} className="mt-3 h-2" />
                <p className="mt-2 text-xs text-muted-foreground">
                  {section.correct} correct · {section.wrong} wrong · {section.unanswered}{" "}
                  unanswered
                </p>
              </div>
            ))}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Answers and explanations come from the immutable submitted test version.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {(["all", "wrong", "flagged", "unanswered"] as ReviewFilter[]).map(
              (item) => (
                <Button
                  key={item}
                  size="sm"
          ...
          filteredReview.map((item, index) => {
            ...
            return (
              <article key={item.questionId} className="rounded-xl border p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                      Question {index + 1} · {item.section}
                    </p>
                    <div className="mt-2 font-medium">
                      <QuestionRichText content={item.text} lang="en" />
                    </div>
                  </div>
                  {state === "correct" ? (
                ...
                {item.explanation || item.proceduralLogic ? (
                  <section className="mt-4 space-y-4 rounded-xl bg-muted/40 p-4 text-sm">
                    <h3 className="font-semibold">Explanation</h3>
                    {item.explanation ? (
                      <QuestionRichText content={item.explanation} lang="en" />
                    ) : null}
                    {item.proceduralLogic ? (
                      <LogicPlaybackRouter logic={item.proceduralLogic} />
                    ) : null}
                  </section>
                ) : null}
              </article>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
        {seriesUrl ? (
          <Button variant="outline" onClick={() => setLocation(seriesUrl)}>
            <Target className="mr-2 h-4 w-4" />
            Continue Test Series
          </Button>
        ) : null}
        <Button onClick={() => setLocation(retryUrl)}>
