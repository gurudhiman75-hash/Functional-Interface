# Notes Studio NS-011 — Source-pack diagnostics

NS-011 adds deterministic source-health diagnostics alongside the existing Source Policy gate.
...
NS-011 answers a different question: **is that source mix healthy enough to trust as a research pack?**
...
Publisher/domain and source-type diversity are diagnostics, not hard blockers in NS-011.

## Boundary with the existing hard gate

The existing Source Policy remains the server-enforced evidence rebuild gate. NS-011 is advisory and cannot override it.

NS-011 deliberately does not:
- discover the web;
...
Notes Studio now exposes **Source diagnostics** next to Source Policy. Editors select an authoring job and see:
...
No migration is required. NS-011 reads the existing governed `content.source_documents`, `content.note_authoring_sources`, and `content.note_authoring_jobs` records.
